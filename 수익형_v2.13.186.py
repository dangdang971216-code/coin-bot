...
def build_precision(ticker: str, price: float) -> Dict[str, Any]:
    ...
    current_move_vs_atr = round(abs(ch3) / max(atr_3m_pct, 1e-9), 3) if atr_3m_pct > 0 else 0.0
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.186: 6차 최종진입검증 직원. strict_all 기록, paper OPEN은 최종검증 통과만.",
    }
...
def core_text() -> str:
    ...
    return "\n".join([
        ...
        f"🔎 정보커버리지: 정밀 {s.get('field_coverage',{}).get('precision',0)} / ema {s.get('field_coverage',{}).get('ema12',0)} / bb {s.get('field_coverage',{}).get('bb',0)} / mfi {s.get('field_coverage',{}).get('mfi',0)} / ATR {s.get('field_coverage',{}).get('atr',0)} / 위험캐시 {s.get('execution_risk_cached',0)} / 우선확인 {s.get('execution_risk_sync_checked',0)} / 최종검증 {s.get('final_entry_note','-')}",
        websocket_status_line(),
        candidate_line(),
        paper_bot_line(),
        guard_summary_line(),
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.186 기준: 6차 최종진입검증 + ATR/VWAP/시장장세",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.186.py", "- 페이퍼봇 대상: paper_bot_v0.35.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.186: 6차 최종진입검증 직원 + OPEN 기준정렬 + ATR/VWAP/시장장세", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 최종검증 통과 후보만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.186 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v186 6차 최종진입검증 직원 + OPEN 기준정렬 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
