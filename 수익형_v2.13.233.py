...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v233 snapshot/공장 병목 + 후보품질 2대문제 관찰수술(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
def _v233_quality_text_from_snapshot(snap: Optional[Dict[str, Any]] = None) -> str:
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v233 기본은 캐시 전용: 현재 후보는 clean_candidate_snapshot.json 단일 원천만 표시",
        "- 최근 3시간은 버전 섞임 가능. 최대수익 제외 줄로 한 방 착시를 같이 봅니다.",
        "- 긴 3/6/12 상세와 원자료성 비교는 /quality_full",
        "",
        "[1/5] 성과 요약",
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    if full:
        return _v209_direct_candidate_quality_text(True)
    txt = _read_cached_text(FILES["quality_summary"], "/quality")
    return (txt.replace("v231 기본은 캐시 전용", "v233 기본은 캐시 전용")
               .replace("v230 기본은 캐시 전용", "v233 기본은 캐시 전용")
               .replace("v222 기본은 캐시 전용", "v233 기본은 캐시 전용"))
