...
def run_once()->Dict[str,Any]:
    ...
    fmap=map_rows(features)
    qobj=load_json(TARGET_QUEUE,{}) or {}
    qrows=qobj.get("rows") if isinstance(qobj,dict) else []
    qmap=map_rows(qrows if isinstance(qrows,list) else [])
    ordered=[]; seen=set()
    # target_router 우선순위 순서를 먼저 반영한다. 개수 제한은 하지 않는다.
    if isinstance(qrows,list):
        for qr in qrows:
            t=ticker_norm((qr or {}).get("ticker"))
            if t and t in fmap and t not in seen:
                rr=dict(fmap[t]); rr["router_priority"]=fnum((qr or {}).get("priority"), default=0); rr["router_bucket"]=(qr or {}).get("bucket"); rr["router_need"]=(qr or {}).get("need")
                ordered.append(rr); seen.add(t)
    for t,r in fmap.items():
        if t not in seen:
            ordered.append(r); seen.add(t)
    ws=map_rows(load_json(WS,{}) or {}); micro=map_rows(load_json(MICRO,{}) or {})
    rows=[]; fresh_ws=0; fresh_micro=0; router_fresh=0; ts=now_ts()
    for r in ordered:
        ...
        if (r.get("router_bucket") in {"s_candidate","near_s_info_wait","info_wait_high"}) and wf and mf:
            router_fresh += 1
        ...
        rr={"ticker":t,"orderflow_ts":ts,"ws_fresh":wf,"micro_fresh":mf,"ws_price":ws_price,"ws_gap_pct":round(gap,3),"spread_pct":round(spread,3),"buy_ratio":round(buy_ratio,3),"bid_wall_ratio":round(bid_wall,3),"sell_pressure":sell_pressure,"orderflow_score":strength_score,"micro_status":"fresh" if mf else ("stale" if m else "missing"),"ws_status":"fresh" if wf else ("stale" if w else "missing"),"router_priority":r.get("router_priority"),"router_bucket":r.get("router_bucket"),"router_need":r.get("router_need")}
        rows.append(rr)
    save_json(OUT,{"version":VERSION,"schema":"orderflow_summary_v3","updated_ts":ts,"updated_text":now_text(ts),"row_count":len(rows),"fresh_ws":fresh_ws,"fresh_micro":fresh_micro,"router_queue_age":age_sec(TARGET_QUEUE),"router_target_count":qobj.get("target_count","-"),"router_fresh_ready":router_fresh,"ws_cache_count":len(ws),"micro_cache_count":len(micro),"ws_ttl_sec":WS_TTL_SEC,"micro_ttl_sec":MICRO_TTL_SEC,"rows":rows})
    sec=now_ts()-st; write_status("running", row_count=len(rows), fresh_ws=fresh_ws, fresh_micro=fresh_micro, router_target_count=qobj.get("target_count","-"), router_fresh_ready=router_fresh, ws_cache_count=len(ws), micro_cache_count=len(micro), ws_ttl_sec=WS_TTL_SEC, micro_ttl_sec=MICRO_TTL_SEC, last_sec=round(sec,3))
