...
LIVE_DISPLAY_MIN_5M_PCT = 0.38
LIVE_DISPLAY_MIN_VOL_RATIO = 3.20
LIVE_DISPLAY_MIN_LEADER_SCORE = 0.90
RESTART_DISPLAY_MIN_5M_PCT = 0.25
RESTART_DISPLAY_MIN_VOL_RATIO = 3.00
RESTART_DISPLAY_MIN_LEADER_SCORE = 1.05
RESTART_DISPLAY_MIN_HIGH_GAP_PCT = 1.30
RESTART_DISPLAY_MIN_COMBINED_MOTION_PCT = 1.00
RESTART_DISPLAY_MAX_FROM_LOW_PCT = 1.60
RESTART_DISPLAY_MAX_RISE_PCT = 2.00
...
def get_restart_quality_status(item: dict) -> tuple[bool, str]:
    ...
    if high_gap_pct < 1.35 and max(chg_30, from_low) >= 1.00:
        return False, "조금 반등했어도 이미 많이 올라와서 재출발 자리로 늦음"
    if chg_5 <= 0.12 and chg_30 <= 0.28:
        return False, "재출발 힘이 아직 거의 안 보임"
    if vol_ratio < 2.50 and max(chg_5, chg_30, from_low) < 1.10:
        return False, "거래량과 반등 힘이 약해서 더 지켜봐야 함"
    if from_low >= RESTART_DISPLAY_MAX_FROM_LOW_PCT and high_gap_pct < 1.55:
        return False, "저점에서 이미 꽤 올라와서 재출발 자리로는 늦음"
    if rise_pct >= RESTART_DISPLAY_MAX_RISE_PCT and high_gap_pct < 2.10:
        return False, "차트상 이미 많이 올라온 뒤라 재출발로 보기 어려움"
    if zone_ratio >= CHART_POS_HIGH_ZONE_RATIO and high_gap_pct < 1.35:
        return False, "상단 구역에 가까워서 재출발 여유가 부족"
    if chg_5 < -0.05 and chg_30 < 1.40 and vol_ratio < 3.30:
...
def refresh_watch_item_before_alert(item: dict, section_key: str = "", regime: dict = None):
    ...
    if section_key in {"restart", "recovery"} and safe_float(snap.get("chg_5m", 0), 0) < 0.15 and max(safe_float(snap.get("chg_30m", 0), 0), safe_float(snap.get("from_30m_low_pct", 0), 0)) < 0.70:
