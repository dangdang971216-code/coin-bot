...
VERSION = "bithumb_micro_sidecar_v0.7_priority_batch_stable_fresh_2026-05-16"
...
PRIORITY_KEEP = int(os.getenv("CLEAN_MICRO_PRIORITY_KEEP", "44"))
...
USER_AGENT = "coinbot-bithumb-micro-sidecar/0.7"
...
def select_poll_batch(targets: List[str]) -> List[str]:
    """v0.7 우선순위 poll batch.

    전체 target 180은 유지한다. 다만 REST를 한 번에 너무 많이 때려 fresh가 0으로 떨어진
    v0.6 문제를 막기 위해 batch를 작게 유지하고, 앞쪽 우선 후보/OPEN/직전 후보는 매 poll마다
    먼저 확인한다. 남은 자리는 회전식으로 채운다.
    """
    global ROTATE
    clean: List[str] = []
    seen = set()
    for t in targets or []:
        ...
        if tt and tt not in STABLE_EXCLUDED and tt not in seen:
            ...
            clean.append(tt)
    if not clean:
        STATS.update({"poll_batch": 0, "rotate_pos": ROTATE, "priority_batch": 0, "rotate_batch": 0})
        return []

    max_batch = max(8, min(POLL_BATCH_MAX, len(clean)))
    priority_n = max(0, min(PRIORITY_KEEP, max_batch, len(clean)))
    priority = clean[:priority_n]
    rest = clean[priority_n:]
    room = max(0, max_batch - len(priority))
    ...
    batch = list(dict.fromkeys(priority + rotated))
    STATS["poll_batch"] = len(batch)
    STATS["priority_batch"] = len(priority)
    STATS["rotate_batch"] = len(rotated)
    STATS["rotate_pos"] = ROTATE
    STATS["priority_keep"] = priority_n
    STATS["rest_rotate_size"] = len(rest)
    STATS["poll_policy"] = "v0.7 priority-first stable batch; target stays wide, REST batch stays fresh"
    return batch
