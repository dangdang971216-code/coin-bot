...
USER_ALERT_PROMOTE_MIN_EDGE = 5.85
USER_ALERT_PROMOTE_MIN_VOL = 2.15
USER_ALERT_PROMOTE_MIN_LEADER = 0.85
USER_ALERT_PROMOTE_MIN_5M_PCT = 0.22
USER_ALERT_PROMOTE_MIN_30M_PCT = 0.05
USER_ALERT_PROMOTE_MIN_HIGH_GAP_PCT = 0.95
USER_ALERT_PROMOTE_MAX_FROM_LOW_PCT = 2.65
USER_ALERT_PROMOTE_MAX_CHART_RISE_PCT = 3.85
USER_ALERT_RESCUE_ON = True
USER_ALERT_RESCUE_SHORTLIST_RANK_MAX = 8
USER_ALERT_RESCUE_MIN_EDGE = 5.00
USER_ALERT_RESCUE_MIN_VOL = 1.25
USER_ALERT_RESCUE_MIN_LEADER = 0.70
...
USER_ALERT_RESCUE_MIN_HOT_SCORE = 2
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 좋은 장에서는 상위 후보까지 간 코인을 더 쉽게 살려 좋은 코인 승격을 강화",
        "- 반대로 고점 바로 밑, 이미 많이 오른 자리, 잡음형 후보는 계속 숨김 유지",
        "- /missed 는 왜 놓쳤는지 보조로 계속 보되 순위 기록 범위를 넓혀 분석력을 조금 올림",
        "- shortlist rescue 를 보강해서 봤던 좋은 코인을 마지막 컷에서 덜 놓치게 조정",
        "- 같은 코인 반복 알림과 30분 확인 추적은 기존 정리 방향 유지",
        "- 숫자와 차트가 덜 어긋나게 알림 직전 재계산 / 차트상태 / 4시간 흐름 표시를 추가",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def is_user_alert_rescue_candidate(item: dict, section: str = "", regime: dict = None) -> bool:
    ...
    if section == "live":
        if str(chart_ctx.get("chart_state_code", "")) == "FADE_AFTER_SPIKE" and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not rescue_ok:
            return "차트상 이미 고점 찍고 밀리는 자리라 실시간 강세에서 제외"
        if str(chart_ctx.get("chart_state_code", "")) == "HIGH_WOBBLE" and chg_30 < 0.60 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not rescue_ok:
            return "차트상 위에서 흔들리는 자리라 실시간 강세에서 제외"
