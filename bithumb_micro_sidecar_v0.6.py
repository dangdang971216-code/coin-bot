...
VERSION = "bithumb_micro_sidecar_v0.6_priority_candidate_wide_poll_2026-05-15"
TARGET_SAFETY_MAX = int(os.getenv("CLEAN_MICRO_TARGET_SAFETY_MAX", os.getenv("CLEAN_MICRO_MAX_TICKERS", "180")))
POLL_BATCH_MAX = int(os.getenv("CLEAN_MICRO_POLL_BATCH_MAX", "108"))
PRIORITY_KEEP = int(os.getenv("CLEAN_MICRO_PRIORITY_KEEP", "96"))
...
MAX_WORKERS = int(os.getenv("CLEAN_MICRO_WORKERS", "10"))
USER_AGENT = "coinbot-bithumb-micro-sidecar/0.6"
...
def select_poll_batch(targets: List[str]) -> List[str]:
    """넓은 target 목록을 모두 버리지 않고 우선순위+회전식으로 수집한다.

    - 앞쪽 target은 메인봇이 진입후보/OPEN/직전후보 순으로 정렬해 둔 우선대상이다.
    - 매 poll마다 전체 target을 전부 REST 호출하면 API/CPU가 터질 수 있으므로,
      우선대상은 매번 보고 나머지는 회전으로 계속 본다.
    - 이 값은 전략 제한이 아니라 서버 보호용 수집 배치다.
    """
    global ROTATE
    clean = [to_ticker(t) for t in (targets or []) if to_ticker(t)]
    clean = list(dict.fromkeys(clean))
    if not clean:
        STATS["poll_batch"] = 0
        STATS["rotate_pos"] = ROTATE
        return []
    if POLL_BATCH_MAX <= 0 or len(clean) <= POLL_BATCH_MAX:
        STATS["poll_batch"] = len(clean)
        STATS["rotate_pos"] = ROTATE
        return clean
    head_n = max(0, min(PRIORITY_KEEP, POLL_BATCH_MAX, len(clean)))
    head = clean[:head_n]
    rest = clean[head_n:]
    room = max(0, POLL_BATCH_MAX - len(head))
    batch = list(head)
    if rest and room > 0:
        start = ROTATE % len(rest)
        rotated = [rest[(start + i) % len(rest)] for i in range(min(room, len(rest)))]
        ROTATE = (ROTATE + len(rotated)) % len(rest)
        batch.extend(rotated)
    batch = list(dict.fromkeys(batch))
    STATS["poll_batch"] = len(batch)
    STATS["rotate_pos"] = ROTATE
    STATS["priority_keep"] = head_n
    STATS["rest_rotate_size"] = len(rest)
    STATS["priority_first_note"] = "v0.6 후보/OPEN/직전후보 우선수집, 나머지 회전"
    return batch
