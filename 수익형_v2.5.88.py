...
def send_startup_message():
    lines = [
        ...
        "- 버전이 바뀌면 review_dump 관련 기록은 자동으로 초기화해서 구버전 데이터가 안 섞이게 정리",
        ...
        "- 사용자 화면 제목은 지금 볼 후보 / 미리 볼 후보 중심으로 더 단순화",
        "",
    ]
...
def compare_command(update, context: CallbackContext):
    process_missed_review_queue(time.time())
    update_active_price_check_refs(time.time())
    alert_rows = collect_recent_alert_review_rows(limit=5, summary_limit=20)
...
def build_review_dump_text(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5) -> str:
    ...
    parts = ["🧾 review_dump", f"- 시각: {now_kst().strftime('%m-%d %H:%M:%S')}"]
    if alert_rows:
        summary = summarize_alert_review_rows(alert_rows[:20])
...
def review_dump_command(update, context: CallbackContext):
    try:
        ...
        for idx, chunk in enumerate(split_text_chunks(dump_text), start=1):
            prefix = "🧾 분석용 덤프" if idx == 1 else f"🧾 분석용 덤프 (계속 {idx})"
            send(prefix + "\n\n" + chunk)
    except Exception as e:
