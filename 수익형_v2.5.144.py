...
def process_missed_review_hourly_report(now_ts: float = None):
    ...
    header_lines = [
        ...
        f"- 탐색 놓침 {summary.get('탐색 놓침', 0)}건 / 승격 놓침 {summary.get('승격 놓침', 0)}건 / 늦어서 안 올림 {summary.get('늦어서 안 올림', 0)}건 / 분류 놓침 {summary.get('분류 놓침', 0)}건",
        "",
...
def process_missed_review_queue(now_ts: float = None):
    ...
    for ticker, row in due:
        ...
        if ok_spot and result_quality in {"good", "assist"} and not was_caught:
...
def summarize_missed_review_logs(rows):
    bucket = {"아예 못 잡음": 0, "봤는데 못 올림": 0, "움직였지만 늦음": 0, "판단 빗나감": 0, "움직였지만 애매": 0}
    for row in rows:
        if not isinstance(row, dict):
            continue
...
def build_review_dump_sections(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5):
    ...
    try:
        ...
        if not missed_rows:
            ...
            lines = [
                "[놓친 강한 움직임 요약]",
                f"- 탐색 놓침 {summary.get('탐색 놓침', 0)}건 / 승격 놓침 {summary.get('승격 놓침', 0)}건 / 늦어서 안 올림 {summary.get('늦어서 안 올림', 0)}건 / 분류 놓침 {summary.get('분류 놓침', 0)}건 / 움직였지만 애매 {summary.get('움직였지만 애매', 0)}건",
                "",
    ...
    try:
        ...
        if df is not None:
            ...
            try:
                ...
                return df
        # timeout이면 예외가 아니라 None으로 떨어진다.
        # 이전 버전은 여기서 바로 실패 처리해서 cache_size가 1~2개로 쪼그라들었다.
