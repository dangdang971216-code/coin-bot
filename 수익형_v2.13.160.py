...
CANDIDATE_READ_MAX_LINES = int(os.getenv("CLEAN_CANDIDATE_READ_MAX_LINES", "2500"))
...
FILES = {
    ...
    "baseline": BASE_DIR / "paper_eval_baseline_v157.json",
...
ARCHITECTURE_ROLES = {
    "hub": "허브: ALL_KRW 전체 원자료/가격/24h 거래대금/rank/캐시 수집",
...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v157: 고정 갯수 제한 제거. 전체 bulk 유지, 정밀보강은 장세 적응형, strict/shadow 판단 결과는 상위 N개로 자르지 않음.",
}
...
def refresh_precision(rows: List[Dict[str, Any]], targets: Optional[List[Dict[str, Any]]] = None) -> Tuple[int, int, int]:
    ...
    budget = adaptive_precision_budget(rows, targets)
    nowv = now_ts()
    ...
    with _precision_lock:
        for r in targets:
            ...
            if not cached or age > PRECISION_TTL_SEC:
                need.append(r)
            if budget > 0 and len(need) >= budget:
                break
    with _state_lock:
        STATE["precision_need"] = len(need)
        STATE["precision_budget"] = budget
    ok = fail = 0
    if not need:
        with _precision_lock:
...
def classify_liquidity(turnover_24h: float, turnover_5m: float, price: float) -> Dict[str, Any]:
    ...
    if price <= 0:
        ...
        tick = "보통"
    return {"liquidity_grade": grade, "slippage_risk": slip, "tick_risk": tick}
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.160: 고정 갯수 절단 없음. paper_bot은 latest scan batch만 소비해 과거 TTL 후보 몰아먹기 방지.",
    }
...
def scan_once() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        stage_times.append(("1) 허브: 전체시장 bulk 수집", now_ts() - st, f"rows {len(rows)} / {source}"))
        with _state_lock:
        ...
        stage_times.append(("3) 2차 직원: 정밀값 보강", now_ts() - st, f"refresh {p_ok} / need {STATE.get('precision_need', 0)} / adaptive {STATE.get('precision_budget', 0)} / fail {p_fail} / cached {p_have}"))
        with _state_lock:
        ...
        stage_times.append(("6) 공장: paper/shadow 소비파일 저장", now_ts() - st, f"paper {pipe['paper_written']}/{pipe['paper_attempt']} / shadow {pipe['shadow_written']}/{pipe['shadow_attempt']} / dup {pipe['dup_skip']}"))
...
def core_text() -> str:
    ...
    return "\n".join([
        ...
        f"- 최신 scan_id: {s.get('scan_id','-')} / paper_bot은 최신 scan 묶음만 소비",
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.160 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.160.py",
        "- 페이퍼봇 대상: paper_bot_v0.21.py",
...
def command_paper_handoff(update, context) -> None:
    lines = [
        ...
        "- 기대 paper_bot: paper_bot_v0.21.py / paper_bot_v0.21",
        "",
        "역할",
...
def candidate_fresh_text() -> str:
    p = candidate_file_fresh_stats(FILES["paper"])
    s = candidate_file_fresh_stats(FILES["shadow"])
    return "\n".join([
        "📁 후보파일 최신성",
        f"- 정식 paper: 전체 {line_count(FILES['paper'])} lines / 최근읽기 {p['read_window']} / TTL유효 {p['fresh']} / 만료 {p['expired']}",
        f"- 복기 shadow: 전체 {line_count(FILES['shadow'])} lines / 최근읽기 {s['read_window']} / TTL유효 {s['fresh']} / 만료 {s['expired']}",
        "- 오래된 line은 삭제하지 않고, paper_bot은 최신 scan_id 묶음 + TTL 기준으로만 소비",
    ])
...
def command_score(update, context) -> None:
    ...
    lines = [
        ...
        "[1/8] 한눈에 보기",
        new_period_text(),
        fmt_stats("신규 전체", new_rows),
        fmt_stats("신규 정식 strict", new_strict),
        fmt_stats("신규 복기 shadow", new_shadow),
        fmt_stats(f"신규 {STRATEGY_NAME}", new_strat),
        "⚠️ 신규 전체는 strict+shadow가 섞인 값. 자동매매 판단은 strict/시간대/등급을 따로 봐야 함",
        "",
        "[2/8] 시간대별 3시간 묶음",
...
def command_batch(update, context) -> None:
    funcs = [
        ("core", core_text),
        ("check", check_text),
        ("score", lambda: "\n".join(["📊 paper 성과판 /score", "", "[요약]", fmt_stats("신규 전체", rows_since_baseline(load_closed(), "closed_at")), fmt_stats("신규 strict", [r for r in rows_since_baseline(load_closed(), "closed_at") if str(r.get('lane')) == 'strict']), fmt_stats("신규 shadow", [r for r in rows_since_baseline(load_closed(), "closed_at") if str(r.get('lane')) == 'shadow']), "⚠️ 신규 전체는 전략 단독 성과가 아니라 strict/shadow가 섞인 값", fmt_stats("누적 전체", load_closed()), paper_bot_line()])),
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.160.py", "- 페이퍼봇 대상: paper_bot_v0.21.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.160: 최신 scan 묶음 소비 + 성과판 해석 보강", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot 과거 TTL 몰아먹기 방지"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.160 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v150 수치 이식 + 허브/직원/공장 분리 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
