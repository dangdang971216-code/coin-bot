...
def format_chart_quant_line(item: dict, short: bool = False) -> str:
    ...
    re_success = safe_float(item.get("rebreakout_success_rate", 0), 0) * 100.0
...
def merge_missed_review_row(base: dict, extra: dict):
    ...
    for key in [
        ...
        "compression_score", "rebreakout_score", "rebreakout_attempts", "rebreakout_success_rate", "rebreakout_failures", "hold_near_high_bars", "hold_near_high_bars_05", "hold_near_high_bars_10", "breakout_hold_score",
...
def compute_rebreakout_structure_context(ticker: str, current_price: float, df1=None) -> dict:
    ...
    try:
        ...
        for row in recent_rows:
            ...
            near_zone = hi >= prev_high * 0.992 or cl >= prev_high * 0.989
            ...
            if near_zone and not in_attempt:
                attempts += 1
                in_attempt = True
                current_attempt_had_hold = False
                current_attempt_had_reject = False
            ...
            if in_attempt and reject_now:
                current_attempt_had_reject = True
            if in_attempt and move_away:
                if current_attempt_had_reject and (not current_attempt_had_hold):
                    failed_attempts += 1
                in_attempt = False
                current_attempt_had_hold = False
                current_attempt_had_reject = False
        ...
        if attempts <= 0 and gap_pct <= 1.05 and max(chg_3, recovery_pct, close_pos_3 * 100.0) >= 0.35:
            attempts = 1
        rebreakout_success_rate = min(1.0, hold_near_high_bars_05 / max(attempts, 1)) if attempts > 0 else 0.0
        score = 0.0
        structure_score = 0.0
        if gap_pct <= 1.40:
            score += 0.55
        ...
        if vol_ratio >= 1.35:
            ...
            score -= 0.20
        if rebreakout_success_rate >= 0.65:
            score += 0.25
        elif attempts >= 2 and hold_near_high_bars == 0:
            score -= 0.15
