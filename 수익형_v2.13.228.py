...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v228 실행순서 수술 + WS target/cache 진단 + scan 단계요약(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
def _v218_refresh_external_maps(*, force: bool = False, ttl: float = 0.8) -> Dict[str, Any]:
    ...
    _V218_EXT_CACHE.update({"ts": nowv, "ws": ws_cache, "micro": micro_cache, "ws_targets": ws_targets, "micro_targets": micro_targets, "micro_source": micro_src})
    return _V218_EXT_CACHE
...
def _v221_write_micro_urgent_from_candidates(priority_rows: Optional[List[Dict[str, Any]]], reason: str = "factory_final_candidates") -> None:
    """v225: urgent 파일은 요청용만 담당한다. fresh 판정은 실제 micro cache 기준으로만 적는다."""
    try:
        ...
        payload = {
            ...
            "targets": tickers,
            "target_meta": meta,
            "note": "v225: urgent is request-only; fresh/stale is decided only from clean_bithumb_micro_cache.json",
        }
...
# v226: v221 urgent wrapper가 감쌀 본선 writer를 명시적으로 고정한다.
# 목적은 새 fallback 추가가 아니라, v217에서 정리한 current-first micro target 본선을
# urgent 요청 writer와 같은 경로로 다시 연결하는 것이다.
_v221_base_update_micro_targets = update_micro_targets

def update_micro_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "hub_rank") -> None:  # type: ignore[override]
    # 본선 target writer는 v217 current-first 단일 경로를 먼저 수행한다.
    _v221_base_update_micro_targets(rows, priority_rows=priority_rows, reason=reason)
    # urgent 파일은 수집직원에게 우선확인을 요청하는 보조 출하물일 뿐, fresh 판정 원천은 아니다.
...
def health_text() -> str:  # type: ignore[override]
    return (_v226_base_health_text()
            .replace("수익형 v2.13.225", "수익형 v2.13.226")
            .replace("수익형 v2.13.224", "수익형 v2.13.226")
            .replace("v225 micro fresh 단일판정", "v226 micro target 본선 재연결"))
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    if full:
        return txt
    return txt.replace("v222 기본은 캐시 전용", "v227 기본은 캐시 전용")
...
def _v210_health_text() -> str:  # type: ignore[override]
    ...
    try:
        ...
        pass
    return (txt.replace("수익형 v2.13.227", "수익형 v2.13.228")
               .replace("수익형 v2.13.226", "수익형 v2.13.228"))
...
def _v216_quality_text() -> str:  # type: ignore[override]
    ...
    return (txt.replace("v227 기본은 캐시 전용", "v228 기본은 캐시 전용")
               .replace("v226 기본은 캐시 전용", "v228 기본은 캐시 전용")
               .replace("v222 기본은 캐시 전용", "v228 기본은 캐시 전용"))
