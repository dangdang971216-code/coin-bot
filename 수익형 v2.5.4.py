...
TERM_GUIDE = [
    ...
    ("지금 볼 후보", "바로 사라는 뜻이 아니라 지금 확인할 가치가 있다는 뜻"),
    ("이전 후보 다시 확인", "전에 떴던 후보가 다시 살아나는지 보라는 뜻"),
...
def should_use_compact_watch_alert(item: dict, judgement: str, caution: str) -> bool:
    ...
    if any(word in judgement for word in weak_keywords):
        return True
    if caution:
        return True
    return False
...
def format_watch_alert_line(item: dict, order: int = 1, alert_type: str = "new", reason_text: str = "") -> str:
    ...
    if compact_mode:
        ...
        return "\n".join([
            ...
            f"- 한눈판단: {judgement}",
            f"- 현재가: {fmt_price(item['current_price'])} / 최근상승 {float(item.get('change_pct', 0)):.2f}% / 거래량 {float(item.get('vol_ratio', 0)):.2f}배",
            format_pre_alert_snapshot_line(item),
            f"- 한줄판단: {one_line}",
        ])
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- B급 초반 선점형을 더 보수적으로 차단",
        "- 30분 결과에 추적기록 정상/부족 표시 추가",
        "- 같은 코인 재알림을 더 강하게 막음",
        "- 30분 예약된 코인은 중복 알림을 더 보수적으로 막음",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def should_send_main_watch_alert(item: dict) -> bool:
    ...
    if strategy in ["LEADER_WATCH", "PREWATCH", "NEWLOW_RECOVER"]:
        return False
    ...
    if strategy == "PREPUMP":
        ...
        if tier == "S":
            return True
        return leader_score >= 2.25 and vol_ratio >= 6.40 and edge >= 7.0 and high_gap_pct >= 1.20

    if strategy == "PULLBACK":
        if caution:
            return False
        return leader_score >= 2.40 and vol_ratio >= 2.20 and edge >= 6.6 and high_gap_pct >= 0.90
...
def scan_watchlist(shared_cache=None):
    ...
    for item in top:
        ...
        is_main_watch = should_send_main_watch_alert(item)
        if not is_main_watch:
            aux_watch_entries.append((item, alert_type, reason_text))
        ...
        save_watch_snapshot(ticker, item, now_ts, alert_type=alert_type, main_watch=is_main_watch)

    sent_tickers = {item[0]["ticker"] for item in (new_entries + upgrade_entries + renotice_entries + aux_watch_entries)}
    preview_entries = []
    for item in build_prewatch_candidates(cache, regime, exclude_tickers=sent_tickers):
        ...
        if not should_send_simple_aux_alert(recent_prewatch_alerts, ticker, now_ts, PREWATCH_ALERT_COOLDOWN_SEC):
            continue
        preview_entries.append((item, "new", "아직 사면 안 되고 먼저 볼 후보"))
    ...
    for item in build_newlow_recovery_candidates(cache, regime, exclude_tickers=sent_tickers):
        ...
        if not should_send_simple_aux_alert(recent_newlow_alerts, ticker, now_ts, NEWLOW_ALERT_COOLDOWN_SEC):
            continue
        newlow_entries.append((item, "new", "바닥인지 확인만 할 후보"))
        sent_tickers.add(ticker)

    if new_entries:
        send_watch_alert_bundle("👀 지금 볼 후보", new_entries)
    if upgrade_entries:
        send_watch_alert_bundle("🔁 다시 살아나는 후보", upgrade_entries)
    if renotice_entries:
        send_watch_alert_bundle("🔁 다시 확인할 후보", renotice_entries)

    main_alert_exists = bool(new_entries or upgrade_entries or renotice_entries)
    if aux_watch_entries and not main_alert_exists:
        send_watch_alert_bundle("🧭 먼저 지켜볼 후보", aux_watch_entries[:AUX_WATCH_ALERT_MAX])
    if preview_entries and not main_alert_exists and not aux_watch_entries:
        send_watch_alert_bundle("🧭 아직 사면 안 되는 예비 후보", preview_entries[:PREWATCH_ALERT_MAX])
    if newlow_entries and not main_alert_exists and not aux_watch_entries:
        send_watch_alert_bundle("🪫 바닥 확인 중인 후보", newlow_entries[:NEWLOW_ALERT_MAX])
