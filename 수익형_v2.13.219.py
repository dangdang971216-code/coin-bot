...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v219 scan후 외부정보 재overlay + 빠른스캔 유지(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
def resource_health_lines() -> List[str]:  # type: ignore[override]
    """v219: CPU 사용률과 load 판정을 분리해서 표시한다."""
    r = load_json(FILES["resource_status"], {})
    if not r:
        return ["❔ 서버자원 캐시 준비중"]
    age = now_ts() - fnum(r.get("updated_ts"), 0.0)
    disk_pct = fnum(r.get("disk_used_pct"), 0.0)
    mem_pct = fnum(r.get("mem_used_pct"), 0.0)
    cpu_pct = fnum(r.get("cpu_total_pct"), 0.0)
    load1 = fnum(r.get("load1"), 0.0)
    load5 = fnum(r.get("load5"), 0.0)
    load15 = fnum(r.get("load15"), 0.0)
