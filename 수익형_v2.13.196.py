...
MICRO_TARGET_MAX = int(os.getenv("CLEAN_MICRO_TARGET_MAX", "28"))
...
def update_micro_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "hub_rank") -> None:
    """빗썸 호가/체결 직원이 볼 대상. 후보 제한이 아니라 실제 진입재료 최신화 배관이다."""
    try:
        ticks: List[str] = []
        seen: set = set()
        previous_priority = recent_candidate_priority_rows(limit=80)
        if priority_rows:
            priority_rows = list(priority_rows) + previous_priority
        else:
            priority_rows = previous_priority
        for t in list(MAJOR_WATCH_TICKERS) + ["BTC", "ETH", "XRP"]:
            _push_unique_ticker(ticks, seen, t)
        for t in _paper_open_tickers(limit=20):
            _push_unique_ticker(ticks, seen, t)
        sorted_pri = sorted(priority_rows, key=lambda r: (
            bool((r or {}).get("paper_bot_open") or (r or {}).get("trade_ready") or (r or {}).get("open_eligible")),
            str((r or {}).get("final_entry_action") or "") == "recheck_wait",
        ...
        payload = {
            ...
            "targets": ticks,
            "priority_count": len(priority_rows),
            "note": "main bot writes current candidates/open positions for bithumb micro sidecar",
        }
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.196.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
