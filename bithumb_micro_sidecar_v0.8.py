...
VERSION = "bithumb_micro_sidecar_v0.8_urgent_preopen_quality_fresh_2026-05-16"
...
USER_AGENT = "coinbot-bithumb-micro-sidecar/0.8"
...
def select_poll_batch(targets: List[str]) -> List[str]:
    """v0.8: paper OPEN 직전/보류 품질 후보를 항상 최우선 poll한다.

    전체 target은 넓게 유지하되, URGENT_TARGETS는 hot queue/순환보다 앞선다.
    """
    global ROTATE
    clean: List[str] = []
    seen = set()
    ...
    urgent = list(dict.fromkeys(urgent))[:min(len(urgent), max_batch)]
    rest_clean = [t for t in clean if t not in set(urgent)]
    ...
    STATS["poll_policy"] = "v0.8 urgent paper/preopen/recheck first; current candidates stay wide, REST batch stays fresh"
    return batch
    ...
    while not STOP:
        try:
            ...
            if not targets:
                ...
                time.sleep(POLL_SEC)
                continue
            # v0.6: 후보/OPEN/직전후보는 더 넓은 우선 batch로 먼저 보고, 나머지는 회전식으로 수행한다.
            batch = select_poll_batch(targets)
            if not batch:
