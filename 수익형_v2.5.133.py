...
def format_chart_quant_line(item: dict, short: bool = False) -> str:
    ...
    upper = safe_float(item.get("upper_wick_pct_1", 0), 0)
    upper3 = safe_float(item.get("upper_wick_pct_3", 0), 0)
    lower = safe_float(item.get("lower_wick_pct_1", 0), 0)
    ...
    re_score = safe_float(item.get("rebreakout_score", 0), 0)
    re_text = format_rebreakout_stat_text(item, compact=short)
    ...
    htf_ready = safe_float(item.get("htf_setup_score", 0), 0)
    ...
    if short:
        base = f"- 차트수치: {quant_label} / 몸통1봉 {body1:.0f}% / 종가강도 {close1:.0f}% / 윗꼬리 {upper:.2f}% / 눌림 {pullback:.2f}%→회복 {recovery:.2f}%({recovery_speed:.2f}/봉) / {re_text} / 유지 {hold_bars}봉 / 4시간 {htf_ready:.2f} / 일봉 {daily_ready:.2f}"
        return base + (f" / 거리 {gap_text}" if gap_text else "")
    base = f"- 차트수치: {quant_label} / 몸통1봉 {body1:.0f}%·3봉 {body3:.0f}% / 종가강도 1봉 {close1:.0f}%·3봉 {close3:.0f}% / 윗꼬리 1봉 {upper:.2f}%·3봉 {upper3:.2f}% / 아랫꼬리 1봉 {lower:.2f}%·3봉 {lower3:.2f}% / 연속양봉 {bull_streak} / 연속음봉 {bear_streak} / 강한양봉 {strong_bulls}개 / 눌림 {pullback:.2f}%→회복 {recovery:.2f}%({recovery_speed:.2f}/봉, {pullback_bars}봉) / 압축 {comp_score:.2f} / {re_text} / 유지 {hold_score:.2f}({hold_bars}봉) / 4시간준비 {htf_ready:.2f} / 일봉준비 {daily_ready:.2f}"
    return base + (f" / 거리 {gap_text}" if gap_text else "")
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- refresh 안정화 보강: 유니버스/캐시 부분 갱신이 오래 막히면 더 빨리 끊고 다음 차수로 넘기도록 조정",
        "- 스캔 정체 완화: force 배치 크기와 부분 종료 기준을 낮춰 stale 반복 줄이기",
        "- missed/review_dump는 유지: 긴 메시지는 요약+개별 블록 전송 유지",
...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    lines = [
        ...
        f"- 차트수치: {str(row.get('chart_quant_label', '') or '차트 수치 확인중')} / 몸통1봉 {safe_float(row.get('body_ratio_1', 0), 0)*100:.0f}%·3봉 {safe_float(row.get('body_ratio_3', 0), 0)*100:.0f}%·5봉 {safe_float(row.get('body_ratio_5', 0), 0)*100:.0f}% / 종가강도 1봉 {safe_float(row.get('close_pos_1', 0), 0)*100:.0f}%·3봉 {safe_float(row.get('close_pos_3', 0), 0)*100:.0f}%·5봉 {safe_float(row.get('close_pos_5', 0), 0)*100:.0f}% / 윗꼬리 1봉 {safe_float(row.get('upper_wick_pct_1', 0), 0):.2f}%·3봉 {safe_float(row.get('upper_wick_pct_3', 0), 0):.2f}%·5봉 {safe_float(row.get('upper_wick_pct_5', 0), 0):.2f}% / 아랫꼬리 1봉 {safe_float(row.get('lower_wick_pct_1', 0), 0):.2f}%·3봉 {safe_float(row.get('lower_wick_pct_3', 0), 0):.2f}%·5봉 {safe_float(row.get('lower_wick_pct_5', 0), 0):.2f}% / 연속양봉 {int(safe_float(row.get('bullish_streak_5', 0), 0))} / 눌림 {safe_float(row.get('pullback_pct_18', 0), 0):.2f}%→회복 {safe_float(row.get('recovery_from_low_pct', 0), 0):.2f}%({safe_float(row.get('recovery_speed_pct', 0), 0):.2f}/봉) / 압축 {safe_float(row.get('compression_score', 0), 0):.2f} / {format_recovery_profile_text(row)} / {format_rebreakout_stat_text(row)} / 상단체류 0.5% {int(safe_float(row.get('hold_near_high_bars_05', 0), 0))}봉·1.0% {int(safe_float(row.get('hold_near_high_bars_10', 0), 0))}봉 / 유지 {safe_float(row.get('breakout_hold_score', 0), 0):.2f}({int(safe_float(row.get('hold_near_high_bars', 0), 0))}봉){rebreakout_gap_text}",
...
def format_missed_review_log_block(row: dict, idx: int = 0) -> str:
    ...
    return "\n".join([
        ...
        *([f"- 차트수치: {str(row.get('chart_quant_label', '') or '차트 수치 기록 없음')} / 몸통1봉 {safe_float(row.get('body_ratio_1', 0), 0)*100:.0f}%·3봉 {safe_float(row.get('body_ratio_3', 0), 0)*100:.0f}%·5봉 {safe_float(row.get('body_ratio_5', 0), 0)*100:.0f}% / 종가강도 1봉 {safe_float(row.get('close_pos_1', 0), 0)*100:.0f}%·5봉 {safe_float(row.get('close_pos_5', 0), 0)*100:.0f}% / 윗꼬리 1봉 {safe_float(row.get('upper_wick_pct_1', 0), 0):.2f}%·5봉 {safe_float(row.get('upper_wick_pct_5', 0), 0):.2f}% / 회복속도 {safe_float(row.get('recovery_speed_pct', 0), 0):.2f}/봉 / {format_recovery_profile_text(row, compact=True)} / {format_rebreakout_stat_text(row, compact=True)} / 상단체류 0.5% {int(safe_float(row.get('hold_near_high_bars_05', 0), 0))}봉·1.0% {int(safe_float(row.get('hold_near_high_bars_10', 0), 0))}봉 / 유지 {safe_float(row.get('breakout_hold_score', 0), 0):.2f}"] if row.get('chart_quant_label') else []),
...
def build_fast_scan_one_liner(primary_reason: str, from_low: float, below_high: float, grade: str = "", bucket: str = "", item: dict = None) -> str:
    ...
    if str(bucket or "") == "discard":
        if soft_recovery and str(primary_reason or "") in {"막 뜨기 직전 후보", "초입/회복 확인 후보", "재출발 확인 후보"}:
            return f"{label}. 버리기엔 아까워서 한 번 더 볼 자리 / 30분 저점대비 {from_low:+.2f}% / {gap_text}"
    ...
    if str(primary_reason or "") == "초입/회복 확인 후보":
        if soft_recovery:
            return f"{label}. 회복 질이 나쁘지 않아 한 번 더 볼 자리 / 30분 저점대비 {from_low:+.2f}% / {gap_text}"
...
def build_scan_debug_text():
    ...
    lines.append(f'안내: {note}')
    user_items, internal_items, discard_items = split_scan_debug_results(items)
