...
def build_prewatch_candidates(cache, regime, exclude_tickers=None):
    ...
    for ticker, data in cache.items():
        try:
            ...
            if change_5 >= 2.2 or change_3 >= 1.6:
                continue
            if change_1 <= -0.55 and change_3 <= -0.25:
                continue
            if momentum_hint <= 0.05:
                continue
            if not (1.55 <= vol_ratio <= 4.6):
                continue
            if not (45 <= rsi <= 68):
                continue
            if not (0.55 <= high_gap_pct <= 2.4):
                continue
            if range_pct >= 3.1:
                continue
            if regime_name == "WEAK" and vol_ratio < 1.85:
                continue
            if leader_score < 0.90 and turnover_rank > 42 and surge_rank > 42:
                continue
            ...
            if rebound_from_low_pct >= 1.55 and high_gap_pct <= 0.95:
                continue
            if flip_count >= 7 and not box and change_5 < 0.60:
                continue
            if flip_count >= 8 and range_pct >= 1.6:
                continue
            if not (hl or box or half_hold) and leader_score < 1.10 and vol_ratio < 2.40:
                continue
...
def build_newlow_recovery_candidates(cache, regime, exclude_tickers=None):
    ...
    for ticker, data in cache.items():
        try:
            ...
            if regime_name == "WEAK" and vol_ratio < 1.85:
                continue
            if change_5 <= -4.0 or change_5 >= 3.0:
                continue
            if change_1 < 0.0 and change_3 < 0.0:
                continue
            if vol_ratio < 1.80:
                continue
            if leader_score < 0.90 and vol_ratio < 2.60:
                continue
            ...
            if range_pct >= 3.1:
                continue
            if pullback_from_peak_pct >= 0.75 and change_1 <= 0.10:
                continue
            if flip_count >= 7 and range_pct >= 1.2:
                continue
            if not (fake_recovery or (bullish_reclaim and lower_low_recent and change_1 >= -0.05)):
                continue
