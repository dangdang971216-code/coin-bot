...
def set_auto_buy_value(enabled: bool):
    ...
    AUTO_BUY = enabled
    try:
        raw = Path("bot.py").read_text(encoding="utf-8")
        if enabled:
            raw = raw.replace("AUTO_BUY = False", "AUTO_BUY = True", 1)
            raw = raw.replace("AUTO_BUY=False", "AUTO_BUY=True", 1)
        else:
            raw = raw.replace("AUTO_BUY = True", "AUTO_BUY = False", 1)
            raw = raw.replace("AUTO_BUY=True", "AUTO_BUY=False", 1)
        Path("bot.py").write_text(raw, encoding="utf-8")
    except Exception as e:
        print(f"[AUTO_BUY 저장 오류] {e}")
