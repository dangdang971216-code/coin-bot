...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.194.py",
        "- 페이퍼봇 대상: paper_bot_v0.38.py",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.194.py", "- 페이퍼봇 대상: paper_bot_v0.38.py"])),
    ]
    start = now_ts()
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    block = trade_ready_block_lines(observe_rows, limit=4) if 'trade_ready_block_lines' in globals() else []
    # trade_ready_block_lines는 legacy 내부 함수라 전역에 없을 수 있다. 없으면 간단히 직접 집계한다.
    if not block:
        rc = Counter()
        for r in observe_rows:
            for key in ("final_entry_label", "trade_block_reason", "reason"):
                v = _ctxv(r, key, r.get(key))
                if v:
                    rc[str(v)] += 1
                    break
        block = ["- 진입 보류 이유: " + (" / ".join(f"{k} {v}" for k, v in rc.most_common(4)) if rc else "자료없음")]
    lines = [
        "🔬 후보품질 요약 /quality",
        "- 기본 기준: 최근 3시간 / 6시간 / 12시간 + 현재버전",
        "- 전체 기록 삭제 없음",
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v194 최근3/6/12시간 성과판 + 외부정보 통합상태 + 기본 출력 가독성 정리 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
