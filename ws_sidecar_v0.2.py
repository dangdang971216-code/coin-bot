...
VERSION = "ws_sidecar_v0.2_dynamic_targets_stale_prune_2026-05-15"
...
RESTART_WAIT_SEC = float(os.getenv("CLEAN_WS_HUB_RESTART_SEC", "20.0"))
TARGET_RELOAD_SEC = float(os.getenv("CLEAN_WS_TARGET_RELOAD_SEC", "5.0"))
...
async def run_session(symbols: List[str], target_set: set[str]) -> None:
            ...
            if now() - last_target_check >= TARGET_RELOAD_SEC:
                ...
                if cur_mtime and cur_mtime != last_target_mtime:
                    ...
                    if set(new_symbols) != set(symbols):
                        ...
                        write_status("대상변경", "target file changed; reconnect")
                        log(f"target changed {len(symbols)} -> {len(new_symbols)}; reconnect")
                        return
...
async def main_loop() -> None:
    while not STOP:
        ...
        if not symbols:
            ...
            continue
        try:
            await run_session(symbols, target_set)
        except Exception as exc:
