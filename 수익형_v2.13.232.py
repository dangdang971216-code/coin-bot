...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v232 snapshot/공장 병목 + 후보품질 2대문제 관찰수술(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
# ===============================
# v2.13.232: snapshot/공장 잔병목 + 후보품질 2대문제 관찰수술
# - 조건/청산/BUY_READY/자동매수 변경 없음.
...
def _v232_external_from_snapshot(snap: Optional[Dict[str, Any]] = None, rows: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    snap = snap if isinstance(snap, dict) else _v212_read_candidate_snapshot()
    ...
    out["count_source"] = "v232_same_snapshot_rows"
    return out


def _v210_external_snapshot_dict() -> Dict[str, Any]:  # type: ignore[override]
    """v232: external_status와 quality가 같은 snapshot row 기준으로 fresh 숫자를 계산한다."""
    snap = _v212_read_candidate_snapshot()
    if not isinstance(snap, dict) or not snap:
        return {
            ...
            "ws_fresh": 0, "ws_stale": 0, "ws_missing": 0, "ws_targeted": 0,
            "micro_fresh": 0, "micro_stale": 0, "micro_missing": 0, "micro_targeted": 0,
            "examples": [], "snapshot_missing": True, "count_source": "v232_missing_snapshot",
        }
    rows = [r for r in (snap.get("rows") or []) if isinstance(r, dict)]
    return _v232_external_from_snapshot(snap, rows)


def _v232_overlay_row_with_maps(row: Dict[str, Any], maps: Dict[str, Any]) -> Dict[str, Any]:
    rr = dict(row or {})
    t = _v212_ticker(rr)
    try:
        ws = _v218_ws_snapshot_from_maps(t, maps)
        ...
        log_error("v232_overlay_ws", exc)
    try:
        rr.update(_v218_micro_snapshot_from_maps(t, maps))
    except Exception as exc:
        log_error("v232_overlay_micro", exc)
    rr["ticker"] = t or rr.get("ticker")
    rr["snapshot_overlay_ts"] = now_ts()
    rr["external_refreshed_by"] = "v232_one_map_overlay"
    try:
        rr = _apply_relative_strength_context(rr)
    except Exception as exc:
        log_error("v232_overlay_relative", exc)
    try:
        rr = _v229_apply_urgent_marker(rr, maps)
    except Exception as exc:
        log_error("v232_overlay_urgent_marker", exc)
    return rr


def _overlay_current_external_for_items(items: List[Dict[str, Any]], *, refresh: bool = True) -> List[Dict[str, Any]]:  # type: ignore[override]
    """v232: 공장/표시 overlay는 WS/micro 파일을 row마다 보지 않고 map 1회만 쓴다."""
    try:
        maps = _v218_refresh_external_maps(force=bool(refresh), ttl=0.8)
    except Exception as exc:
        log_error("v232_overlay_maps", exc)
        maps = _v218_refresh_external_maps(force=False, ttl=2.0)
    out: List[Dict[str, Any]] = []
    for r in items or []:
        if not isinstance(r, dict):
            continue
        rr = _v232_overlay_row_with_maps(r, maps)
        ...
        for k in ("ws_row_status", "ws_age_sec", "ws_targeted", "ws_cache_ts", "current_price_ws_gap_pct", "micro_fresh", "micro_row_status", "micro_age_sec", "micro_targeted", "micro_spread_pct", "micro_trade_buy_ratio_30"):
            if k in rr:
                ctx[k] = rr.get(k)
        ctx["external_overlay_at"] = now_ts()
        ctx["external_overlay_note"] = "v232_one_map_overlay_before_display_or_export"
        rr["entry_context"] = ctx
        out.append(rr)
    return out
...
def _v212_snapshot_payload(strict_rows: List[Dict[str, Any]], shadow_rows: List[Dict[str, Any]], *, stage: str, source: str, wait_sec: float = 0.0) -> Dict[str, Any]:  # type: ignore[override]
    maps = _v218_refresh_external_maps(force=True, ttl=0.0)
    ordered = _v212_order_candidates(strict_rows)
    rows = [_v232_overlay_row_with_maps(r, maps) for r in ordered]
    ext = _v232_external_from_snapshot({"updated_ts": now_ts(), "rows": rows, "scan_id": str(STATE.get("scan_id") or "-")}, rows)
    nowv = now_ts()
    return {
        "version": BOT_VERSION,
        "schema": "candidate_snapshot_v232",
        "stage": stage,
        "source": source,
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        ...
        "external": {
            **ext,
            ...
            "micro_worker_targets": STATE.get("micro_targets", 0),
            "micro_worker_fresh": STATE.get("micro_fresh", 0),
            "overlay_mode": "v232_one_map_overlay",
            "cache_map_age_sec": round(now_ts() - fnum(maps.get("ts"), now_ts()), 2),
        },
        "target": {
            "writer": "factory_export_final_only",
        ...
        "note": "v232: snapshot overlay uses one WS/micro map; external counts are recalculated from the same snapshot rows",
    }


def v212_publish_final_candidate_state(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]], market_rows: List[Dict[str, Any]], pipe: Dict[str, Any]) -> str:  # type: ignore[override]
    """v232: snapshot은 방금 만든 후보 메모리 rows를 우선 사용한다. latest 파일 재읽기 경로를 줄인다."""
    strict_rows = [r for r in (strict or []) if isinstance(r, dict)]
    shadow_rows = [r for r in (shadow or []) if isinstance(r, dict)]
    ...
    if wait_sec > 0:
        _stop_event.wait(wait_sec)
    final = _v212_write_candidate_snapshot(strict_rows, shadow_rows, stage="final_after_target_overlay", source="factory_memory_rows_v232", wait_sec=wait_sec)
    scan_id = str(final.get("scan_id") or STATE.get("scan_id") or "") if isinstance(final, dict) else str(STATE.get("scan_id") or "")
    _v219_schedule_external_refresh(scan_id)
    ext = final.get("external", {}) if isinstance(final, dict) else {}
    with _state_lock:
        STATE["v232_snapshot_memory_rows"] = True
        STATE["v232_snapshot_initial_wait_sec"] = wait_sec
    return f"snapshot {final.get('candidate_count',0)} / WS {ext.get('ws_fresh',0)}/{ext.get('total',0)} / micro {ext.get('micro_fresh',0)}/{ext.get('total',0)} / wait {wait_sec:.1f}s / target_writer factory_final_candidates / v232 one-map snapshot"


def _v232_reason_group(row: Dict[str, Any]) -> str:
...
def _v232_entry_value(row: Dict[str, Any], key: str, default: Any = None) -> Any:
...
def _v232_avg(rows: List[Dict[str, Any]], key: str) -> float:
    vals = [fnum(_v232_entry_value(r, key), 0.0) for r in rows or [] if _v232_entry_value(r, key) not in (None, "")]
    return round(sum(vals) / len(vals), 3) if vals else 0.0


def _v232_problem_sample(rows: List[Dict[str, Any]], limit: int = 2) -> List[str]:
    if not rows:
        return ["  · 예시 없음"]
    out: List[str] = []
    ...
    for r in ordered:
        ...
        score = fnum(_v232_entry_value(r, "score", r.get("score")), 0.0)
        money3 = _v232_entry_value(r, "money_flow_3m", r.get("money_flow_3m"))
        pull = fnum(_v232_entry_value(r, "pullback_quality_score", r.get("pullback_quality_score")), 0.0)
        micro = str(_v232_entry_value(r, "micro_row_status", r.get("micro_entry_status") or r.get("micro_row_status") or "-"))
        ws = str(_v232_entry_value(r, "ws_row_status", r.get("ws_entry_status") or r.get("ws_row_status") or "-"))
        spread = fnum(_v232_entry_value(r, "micro_spread_pct", r.get("micro_spread_pct")), 0.0)
        buy = fnum(_v232_entry_value(r, "micro_trade_buy_ratio_30", r.get("micro_trade_buy_ratio_30")), 0.0)
...
def _v232_problem_block(cur_rows: List[Dict[str, Any]]) -> List[str]:
    stop = [r for r in cur_rows or [] if _v232_reason_group(r) == "stop"]
    slow = [r for r in cur_rows or [] if _v232_reason_group(r) == "slow"]
    ...
    for label, arr in (("손절", stop), ("지지부진", slow)):
        ...
        avg_peak = _v232_avg(arr, "peak_pct")
        avg_money = _v232_avg(arr, "money_flow_3m")
        avg_pull = _v232_avg(arr, "pullback_quality_score")
        avg_spread = _v232_avg(arr, "micro_spread_pct")
        ...
        out.extend(_v232_problem_sample(arr, 2))
    out.append("- 용도: 후보품질 원인 확인용. 이 블록은 조건/청산을 바꾸지 않습니다.")
    return out


def _v232_quality_text_from_snapshot(snap: Optional[Dict[str, Any]] = None) -> str:
    snap = snap if isinstance(snap, dict) else _v212_read_candidate_snapshot()
    strict = [r for r in ((snap or {}).get("rows") or []) if isinstance(r, dict)]
    shadow = tail_jsonl(FILES.get("shadow_latest", BASE_DIR / "shadow_candidates_latest.jsonl"), max_lines=V212_SNAPSHOT_READ_MAX)
    shadow = [r for r in shadow if isinstance(r, dict)]
    counts = _v230_candidate_counts(strict)
    ext = _v232_external_from_snapshot(snap, strict)
    closed = _v210_strict_rows(load_closed(V210_QUALITY_CLOSED_TAIL))
    rows3 = _v210_rows_last_hours(closed, 3)
    rows12 = _v210_rows_last_hours(closed, 12)
    cur = _v210_current_version_rows(closed)
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v232 기본은 캐시 전용: 현재 후보는 clean_candidate_snapshot.json 단일 원천만 표시",
        "- 최근 3시간은 버전 섞임 가능. 최대수익 제외 줄로 한 방 착시를 같이 봅니다.",
        "- 긴 3/6/12 상세와 원자료성 비교는 /quality_full",
        "",
        "[1/5] 성과 요약",
...
def _v216_quality_text() -> str:  # type: ignore[override]
    return _v232_quality_text_from_snapshot(_v212_read_candidate_snapshot())


def _v231_build_observation_caches(*, build_quality: bool = True, reason: str = "manual") -> None:  # type: ignore[override]
    """v232: external/health/quality를 같은 snapshot payload 하나로 만든다."""
    global _V231_LAST_QUALITY_SCAN_ID
    with _V231_OBS_CACHE_LOCK:
        snap = _v212_read_candidate_snapshot()
        strict_rows = [r for r in ((snap or {}).get("rows") or []) if isinstance(r, dict)]
        ext_snap = _v232_external_from_snapshot(snap, strict_rows)
        try:
            save_json(FILES["external_snapshot"], _v231_cache_payload(_v210_external_status_text(ext_snap), "external_status", snap=snap))
            save_json(FILES["health_snapshot"], _v231_cache_payload(_v210_health_text(), "health", snap=snap))
        except Exception as exc:
            log_error("v232_observation_light_cache", exc)
        if build_quality:
            try:
                save_json(FILES["quality_summary"], _v231_cache_payload(_v232_quality_text_from_snapshot(snap), "quality", snap=snap))
                _V231_LAST_QUALITY_SCAN_ID = str((snap or {}).get("scan_id") or "")
            except Exception as exc:
                log_error("v232_observation_quality_cache", exc)
        _V231_LAST_OBS_CACHE.update({
            "scan_id": str((snap or {}).get("scan_id") or ""),
            "snapshot_ts": fnum((snap or {}).get("updated_ts"), 0.0) if isinstance(snap, dict) else 0.0,
            "updated_ts": now_ts(),
        ...
        with _state_lock:
            STATE["v232_observation_cache_scan_id"] = _V231_LAST_OBS_CACHE.get("scan_id", "")
            STATE["v232_observation_cache_reason"] = reason
            STATE["v232_observation_quality_built"] = bool(build_quality)
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    if full:
        return _v209_direct_candidate_quality_text(True)
    txt = _read_cached_text(FILES["quality_summary"], "/quality")
    return (txt.replace("v231 기본은 캐시 전용", "v232 기본은 캐시 전용")
               .replace("v230 기본은 캐시 전용", "v232 기본은 캐시 전용")
               .replace("v222 기본은 캐시 전용", "v232 기본은 캐시 전용"))
