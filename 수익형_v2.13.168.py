...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.168: 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def core_text() -> str:
    ...
    return "\n".join([
        ...
        candidate_line(),
        f"- scan_id: {s.get('scan_id','-')} / 최신 scan 후보만 소비",
        f"- 오류: scan {s.get('scan_last_error','') or '-'} / write {s.get('write_error','') or '-'}",
        paper_line,
        "",
        "판독",
        "✅ 전체 시장 스캔 유지 / candidate_events 없음 / 자동매수 OFF",
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.168 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.168.py",
        "- 페이퍼봇 대상: paper_bot_v0.27.py",
...
def command_paper_handoff(update, context) -> None:
    lines = [
        ...
        "- 기대 paper_bot: paper_bot_v0.27.py / paper_bot_v0.27",
        "",
        "역할",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.168.py", "- 페이퍼봇 대상: paper_bot_v0.27.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.168: 오류로그 정리 + 자동묶음 시간표 + 출력 간략화", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.168 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
