...
- 실거래 주문은 v2.7.5 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.5"
BOT_FILE_TAG = "수익형_v2.7.5.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.5는 현재가+체결까지만 붙인다.
WS_HUB_ON = os.getenv("V275_WS_HUB_ON", "1") not in ("0", "false", "False")
WS_URL = os.getenv("V275_WS_URL", "wss://ws-api.bithumb.com/websocket/v1")
WS_LEGACY_URL = os.getenv("V275_WS_LEGACY_URL", "wss://pubwss.bithumb.com/pub/ws")
WS_MAX_CODES = int(os.getenv("V275_WS_MAX_CODES", "160"))
WS_RECV_TIMEOUT_SEC = float(os.getenv("V275_WS_RECV_TIMEOUT_SEC", "1.0"))
WS_RECONNECT_SEC = float(os.getenv("V275_WS_RECONNECT_SEC", "5.0"))
WS_SUBSCRIBE_REFRESH_SEC = float(os.getenv("V275_WS_SUB_REFRESH_SEC", "45"))
WS_NO_MESSAGE_SWITCH_SEC = float(os.getenv("V275_WS_NO_MESSAGE_SWITCH_SEC", "25"))
WS_PRICE_FRESH_SEC = float(os.getenv("V275_WS_PRICE_FRESH_SEC", "8"))
WS_TRADE_FRESH_SEC = float(os.getenv("V275_WS_TRADE_FRESH_SEC", "20"))
WS_CANDLE_FRESH_SEC = float(os.getenv("V275_WS_CANDLE_FRESH_SEC", "12"))
WS_TRADE_ROWS_PER_TICKER = int(os.getenv("V275_WS_TRADE_ROWS", "240"))
WS_CLOSED_CANDLES_PER_TICKER = int(os.getenv("V275_WS_CLOSED_CANDLES", "8"))
...
CANDIDATE_WAREHOUSE_FILE = os.path.join(BASE_DIR, "v275_candidate_warehouse.json")
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.5 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
def render_error_doctor() -> str:
    ...
    return (
        f"🩺 오류/병목 진단 /error_doctor v2.7.5\n\n"
        f"1) 현재 판정\n"
        ...
        f"- 출력은 요청 시 경량 조립\n"
        f"- v2.7.5: 눌림 재확인 라인을 clean core 방식으로 복구\n\n"
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.5: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.5는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.5\n"
        f"- v2.7.5 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.5에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.5\n"
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.5 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
