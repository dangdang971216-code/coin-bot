...
ALERT_PRICE_RECHECK_LIVE_MAX_UP_PCT = 0.22
ALERT_PRICE_RECHECK_RESTART_MAX_UP_PCT = 0.32
ALERT_PRICE_RECHECK_SKIP_UP_PCT = 0.85
ALERT_PRICE_RECHECK_LIVE_NEAR_HIGH_MAX_UP_PCT = 0.12
ALERT_PRICE_RECHECK_RESTART_NEAR_HIGH_MAX_UP_PCT = 0.18
...
RESTART_DISPLAY_MIN_5M_PCT = 0.18
RESTART_DISPLAY_MIN_VOL_RATIO = 2.45
RESTART_DISPLAY_MIN_LEADER_SCORE = 0.95
RESTART_DISPLAY_MIN_HIGH_GAP_PCT = 1.10
RESTART_DISPLAY_MIN_COMBINED_MOTION_PCT = 0.75
RESTART_DISPLAY_MAX_FROM_LOW_PCT = 2.00
RESTART_DISPLAY_MAX_RISE_PCT = 2.45
...
def get_restart_quality_status(item: dict) -> tuple[bool, str]:
    ...
    if high_gap_pct < 0.90:
        return False, "최근 고점이 너무 가까워서 재출발 자리로 애매"
    if high_gap_pct < 1.20 and max(chg_30, from_low) >= 1.10:
        return False, "조금 반등했어도 이미 많이 올라와서 재출발 자리로 늦음"
    if chg_5 <= 0.08 and chg_30 <= 0.20:
        return False, "재출발 힘이 아직 거의 안 보임"
    if vol_ratio < 2.10 and max(chg_5, chg_30, from_low) < 1.00:
        return False, "거래량과 반등 힘이 약해서 더 지켜봐야 함"
    if from_low >= RESTART_DISPLAY_MAX_FROM_LOW_PCT and high_gap_pct < 1.30:
        return False, "저점에서 이미 꽤 올라와서 재출발 자리로는 늦음"
    if rise_pct >= RESTART_DISPLAY_MAX_RISE_PCT and high_gap_pct < 1.80:
        return False, "차트상 이미 많이 올라온 뒤라 재출발로 보기 어려움"
    if zone_ratio >= CHART_POS_HIGH_ZONE_RATIO and high_gap_pct < 1.25:
        return False, "상단 구역에 가까워서 재출발 여유가 부족"
    if chg_5 < -0.15 and chg_30 < 1.25 and vol_ratio < 3.10:
...
def refresh_watch_item_before_alert(item: dict, section_key: str = "", regime: dict = None):
    ...
    if section_key == "live" and 0 <= high_gap_pct <= 0.70:
        return refreshed, False, "실시간 강세로 보기엔 이미 고점이 너무 가까워서 제외"
    if section_key == "live" and safe_float(snap.get("chg_5m", 0), 0) < 0.28 and safe_float(refreshed.get("vol_ratio", 0), 0) < 4.20:
        return refreshed, False, "실시간 강세로 보기엔 방금 힘이 약해져서 제외"
    if section_key in {"restart", "recovery"} and safe_float(snap.get("chg_5m", 0), 0) < 0.08 and max(safe_float(snap.get("chg_30m", 0), 0), safe_float(snap.get("from_30m_low_pct", 0), 0)) < 0.55:
