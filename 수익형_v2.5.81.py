...
def missed_command(update, context: CallbackContext):
    ...
    if not rows:
        send("📌 아직 자동으로 잡힌 놓친 코인은 없어")
        return
    recent = rows[-5:]
    summary = summarize_missed_review_logs(rows[-20:])
    lines = ["📌 놓친 코인 정리", "", "[최근 20건 요약]", f"- 아예 못 잡음 {summary.get('아예 못 잡음', 0)}건", f"- 봤는데 못 올림 {summary.get('봤는데 못 올림', 0)}건", f"- 판단 빗나감 {summary.get('판단 빗나감', 0)}건", "", "[최근 5건]"]
    for idx, row in enumerate(reversed(recent), start=1):
        lines.append("")
        lines.append(format_missed_review_log_block(row, idx=idx))
    send("\n".join(lines))
