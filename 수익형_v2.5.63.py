...
FAST_SCAN_ALERT_COOLDOWN_SEC = 600
...
def build_fast_scan_one_liner(primary_reason: str, from_low: float, below_high: float, grade: str = "", bucket: str = "") -> str:
    ...
    if str(primary_reason or "") == "초입 가능 후보":
        return f"{label}. 막 힘 붙는 자리 / 30분 저점대비 {from_low:+.2f}% / {gap_text}"
        ...
        if state_code in {"FADE_AFTER_SPIKE", "HIGH_WOBBLE", "EXTENDED"}:
            ...
            if section == "strong":
                ...
                if primary == "실시간 급등 초입":
                    primary = "초입/회복 확인 후보"
        if chart_gap <= 0.95 and chart_rise >= 1.20:
            add_reason("고점이 가까워서 늦을 수 있음")
            if section == "strong":
                section = "watch"
        if zone_ratio >= 0.84 and chart_gap <= 1.30 and chart_rise >= 1.00:
            add_reason("위쪽 구간이라 더 확인 필요")
            if section == "strong":
                section = "watch"
        if primary == "재출발 확인 후보":
            strong_restart = (
                change_5 >= 0.80 and vol_ratio >= 3.00 and leader_score >= 1.10 and from_low >= 1.00 and below_high >= 1.80 and chart_gap >= 1.40 and state_code not in {"FADE_AFTER_SPIKE", "HIGH_WOBBLE", "EXTENDED"}
            )
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float) -> tuple[str, str]:
    ...
    if section == "watch":
        ...
        rescue_like = (
            change_5 >= 0.85
            and vol_ratio >= 3.00
            and leader_score >= 1.10
            and below_high >= 1.95
            and 1.00 <= from_low <= 2.40
...
def _build_fast_scan_item(ticker: str, data: dict):
    ...
    grade, bucket = classify_fast_scan_grade_bucket(section, primary, change_5, change_3, vol_ratio, leader_score, from_low, below_high, rsi, safe_float(item.get("lite_score", raw_score), raw_score))
    item["section_rank"] = {"strong": 2, "watch": 1, "weak": 0}.get(section, 0)
...
def _build_fast_alert_signature(item: dict) -> str:
    ...
    change_5 = round(safe_float(item.get("change_5", 0), 0), 2)
    vol_ratio = round(safe_float(item.get("vol_ratio", 0), 0), 2)
    leader = round(safe_float(item.get("leader_score", 0), 0), 2)
    from_low = round(safe_float(item.get("from_low", 0), 0), 2)
    below_high = round(safe_float(item.get("below_high", 999), 999), 2)
    return f"{ticker}|{grade}|{bucket}|{label}|{change_5}|{vol_ratio}|{leader}|{from_low}|{below_high}"
...
def _send_fast_scan_alerts(items, now_ts: float):
    ...
    for item in items:
        ...
        if sig == last_sig and (now_ts - last_ts) < FAST_SCAN_ALERT_COOLDOWN_SEC:
            continue

        last = recent_signal_alerts.get(ticker, 0)
        if (now_ts - last) < FAST_SCAN_ALERT_COOLDOWN_SEC:
            continue

        recent_signal_alerts[ticker] = now_ts
        recent_fast_alert_signatures[ticker] = {"ts": now_ts, "sig": sig}
