...
MICRO_TARGET_MAX = int(os.getenv("CLEAN_MICRO_TARGET_MAX", "32"))
...
def update_ws_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "hub_rank") -> None:
    ...
    v198: WS도 micro처럼 "오래됐으니 막기"가 아니라 "진입 직전 후보를 먼저
    신선하게 받기"가 목적이다. 대형/거래대금 상위보다 현재·직전 진입 후보와
    OPEN 보유 코인을 먼저 clean_ws_targets.json에 올린다.
    """
    try:
        ticks: List[str] = []
        seen: set = set()
        current_priority = list(priority_rows or [])
        previous_priority = recent_candidate_priority_rows(limit=160)
        merged_priority = current_priority + previous_priority

        # 1순위: 이번/직전 진입 직전 후보. 실제 판단에 바로 쓰이는 코인을 먼저 구독한다.
        ...
        # 2순위: 현재 OPEN 보유 코인. 청산/위험 확인용으로 계속 신선하게 유지한다.
        for t in _paper_open_tickers(limit=30):
            _push_unique_ticker(ticks, seen, t)
            if len(ticks) >= max(10, WS_HUB_MAX_TICKERS):
                break

        # 3순위: 현재 row 중 점수/돈흐름이 강한 후보군.
        ...
        # 4순위: BTC/ETH/XRP 같은 장세 기준 코인은 남는 자리에 둔다.
        for t in list(MAJOR_WATCH_TICKERS) + ["BTC", "ETH", "XRP"]:
            _push_unique_ticker(ticks, seen, t)
            if len(ticks) >= max(10, WS_HUB_MAX_TICKERS):
                break

        # 5순위: 남는 자리는 거래대금/rank 상위로 채운다.
        ...
        payload = {
            ...
            "paper_open_count": len(_paper_open_tickers(limit=30)),
            "priority_first": True,
            "note": "v198: current/previous entry candidates and OPEN positions are prioritized for websocket sidecar",
        }
        try:
...
def update_micro_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "hub_rank") -> None:
    ...
    v197: 차단보다 빠른 확인이 우선이다. 대형/거래대금 상위보다
    직전·현재 진입후보와 OPEN 보유 코인을 먼저 micro 대상으로 올린다.
    """
    try:
        ticks: List[str] = []
        seen: set = set()
        previous_priority = recent_candidate_priority_rows(limit=120)
        current_priority = list(priority_rows or [])
        ...
        for t in _paper_open_tickers(limit=24):
            _push_unique_ticker(ticks, seen, t)
            if len(ticks) >= max(8, MICRO_TARGET_MAX):
                break
        ...
        payload = {
            ...
            "priority_count": len(merged_priority),
            "priority_first": True,
            "note": "v197: current/previous entry candidates and OPEN positions are prioritized for bithumb micro sidecar",
        }
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.194: 최근12시간 중심 성과판 + 외부정보 통합상태 + 기본 출력 가독성 정리.",
    }
...
def scan_once() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # v198: 최종검증을 기다리지 말고 후보가 잡힌 즉시 WS/micro 대상파일을 먼저 갱신한다.
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.198.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def _builder_for_command(name: str):
    if not TEXT_COMMAND_BUILDERS:
        TEXT_COMMAND_BUILDERS.update({
            ...
            "help": lambda: "📚 명령어 /help\n- 여러 명령을 줄바꿈으로 보내도 자동 묶음 처리됨\n- /batch /check /core /external_status /quality /score /version_score /version_compare /loss_review /cpu_status /deploy /errorlog_full",
        })
...
def command_help(update, context) -> None:
    reply(update, "\n".join([
        ...
        "",
        "기본 확인",
        "- /batch /core /external_status /quality /score /version_score",
        "흐름/후보",
...
def external_status_text() -> str:
    ...
    lines = [
        ...
        "",
        "[1/3] 직원 상태",
        f"{'✅' if ws_ok else '⚠️'} 웹소켓: {STATE.get('ws_state','-')} / 대상 {STATE.get('ws_targets',0)} / 신선 {STATE.get('ws_fresh',0)} / 최근 {STATE.get('ws_last_age_sec','-')}초",
...
def _version_feature_stats(rows: List[Dict[str, Any]]) -> str:
    ...
    reasons = Counter(str(r.get("exit_reason") or "unknown") for r in rows)
    return (
        ...
        f"- 호가스프레드: {avg_field('micro_spread_pct')} / 매수체결비율: {avg_field('micro_trade_buy_ratio_30')} / WS나이: {avg_field('live_age_sec','ws_age_sec')}"
    )
...
# v2.13.198: WS 빠른재구독판(대상 우선순위/선갱신/reload 개선)
# - 전략/청산/자동매수/BUY_READY는 변경하지 않는다.
...
def _ws_status_of(row: Dict[str, Any]) -> str:
    ...
    if bool(_ctxv(row, "ws_fresh")) or status == "fresh" or str(_ctxv(row, "live_price_source", "")) == "WS_SIDECAR":
        return "fresh"
    if status == "stale" or str(_ctxv(row, "live_price_source", "")) == "WS_STALE":
        return "stale"
    return "missing"
...
def version_compare_text() -> str:
    ...
    preferred = [BOT_VERSION, "수익형 v2.13.195", "수익형 v2.13.194", "수익형 v2.13.193", "수익형 v2.13.192", "수익형 v2.13.190"]
    keys: List[str] = []
    for k in preferred:
        if k in by_ver and k not in keys:
            keys.append(k)
    ...
    for k in keys[:6]:
        ...
        for ln in _outlier_summary(arr)[:2]:
            lines.append(ln)
        lines.append(f"- 손절 {len(stops)}건 / micro 신선 {micro_counts.get('fresh',0)} · 오래됨 {micro_counts.get('stale',0)} · 없음 {micro_counts.get('missing',0)}")
        lines.append(_version_feature_stats(arr))
        lines.append("")
    lines += [
        "판독",
...
def install_commands(updater: Any) -> None:
    ...
    try:
        menu = ["batch", "check", "core", "external_status", "quality", "score", "version_score", "version_compare", "loss_review", "cpu_status", "paper_handoff", "deploy", "trade", "errorlog", "help"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v198 WS 빠른재구독·대상선갱신 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
