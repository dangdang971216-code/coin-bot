...
FAST_SCAN_ALERT_COOLDOWN_SEC = 2400
FAST_SCAN_ALERT_REPEAT_PRICE_MOVE_PCT = 1.80
FAST_SCAN_ALERT_REPEAT_SCORE_JUMP = 5.5
FAST_SCAN_ALERT_REPEAT_VOL_JUMP = 3.2
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'strong':
        ...
        if (
            ...
            if primary_reason == '실시간 급등 초입' and (change_3 < 0.03 or safe_float(item.get('change_30', 0), 0) < -0.20) and from_low >= 1.00:
    ...
    if section == 'watch':
        ...
        prelaunch_like = (
            ...
            and FAST_SCAN_PRELAUNCH_MIN_CHANGE5 <= change_5 <= 0.65
            and change_3 >= FAST_SCAN_PRELAUNCH_MIN_CHANGE3
            and vol_ratio >= max(FAST_SCAN_PRELAUNCH_MIN_VOL, 1.15)
            and leader_score >= max(FAST_SCAN_PRELAUNCH_MIN_LEADER, 0.40)
            and FAST_SCAN_PRELAUNCH_MIN_FROM_LOW <= from_low <= FAST_SCAN_PRELAUNCH_MAX_FROM_LOW
            and below_high >= max(FAST_SCAN_PRELAUNCH_MIN_HIGH_GAP, 0.85)
            and lite_score >= 12.2 and rsi < 79
            and safe_float(item.get('turnover_5m', 0), 0) >= FAST_SCAN_PRELAUNCH_MIN_TURNOVER_5M
            and not chart_bad
            and safe_float(item.get('change_30', 0), 0) >= -3.20
            and (
                int(safe_float(item.get('change5_rank', 999), 999)) <= 8
                or int(safe_float(item.get('turnover_rank', 999), 999)) <= 24
                or int(safe_float(item.get('leader_rank', 999), 999)) <= 18
            )
        )
        if rescue_like:
            return 'A', 'user_alert'
