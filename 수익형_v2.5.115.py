...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    lines = [
        ...
        f"- 최근30분 기준값: 고점 {fmt_price(safe_float(row.get('recent_30m_high_price', 0), 0))} / 저점 {fmt_price(safe_float(row.get('recent_30m_low_price', 0), 0))}" if safe_float(row.get('recent_30m_high_price', 0), 0) > 0 or safe_float(row.get('recent_30m_low_price', 0), 0) > 0 else "",
        f"- 최근60분 기준값: 고점 {fmt_price(safe_float(row.get('recent_60m_high_price', 0), 0))} / 저점 {fmt_price(safe_float(row.get('recent_60m_low_price', 0), 0))}" if safe_float(row.get('recent_60m_high_price', 0), 0) > 0 or safe_float(row.get('recent_60m_low_price', 0), 0) > 0 else "",        f"- 상위봉: 4시간 {format_htf_brief_for_row(row)} / 일봉 {format_daily_brief_for_row(row)}",
