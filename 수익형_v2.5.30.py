...
def get_watch_display_group(item: dict, judgement: str = "", caution: str = "", section_override: str = "", regime: dict = None) -> str:
    ...
    if strategy == "LEADER_WATCH":
        ...
        if chg_5 < LIVE_DISPLAY_MIN_5M_PCT:
            return "restart" if is_chart_position_good_for_restart(item) and max(chg_30, from_low) >= 0.25 and high_gap_pct >= 1.00 else "watch"
        if vol_ratio < LIVE_DISPLAY_MIN_VOL_RATIO:
            return "restart" if is_chart_position_good_for_restart(item) and max(chg_30, from_low) >= 0.30 and high_gap_pct >= 1.00 else "watch"
