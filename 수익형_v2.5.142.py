...
# v2.5.142: 티커 목록은 거의 고정값이라 force 요청마다 다시 받으면 병목이 생긴다.
...
UNIVERSE_COLD_START_TARGET = 24
UNIVERSE_COLD_START_BATCH = 14
UNIVERSE_MIN_PARTIAL_ROWS = 3
UNIVERSE_BUILD_MAX_SEC = 1.8
UNIVERSE_COLD_START_MAX_SEC = 4.2
CACHE_REFRESH_BATCH = 3
CACHE_FORCE_FULL_BATCH = 5
CACHE_COLD_START_TARGET = 12
CACHE_COLD_START_BATCH = 8
CACHE_MIN_PARTIAL_ROWS = 3
CACHE_BUILD_MAX_SEC = 1.8
CACHE_COLD_START_MAX_SEC = 3.8
...
CACHE_REFRESH_MAX_TRIES = 9
CACHE_REFRESH_TARGET_SUCCESS = 2
...
def get_ohlcv(ticker: str, interval: str = "minute3"):
    ...
    if cached is not None:
        return cached
    try:
        df = _call_with_timeout(pybithumb.get_ohlcv, ticker, interval=interval, default=None)
        if df is not None:
        ...
        stale = get_api_cache_entry(API_OHLCV_CACHE, cache_key, ttl_sec * 3, copy_dataframe=True)
        if stale is not None:
            return stale
        return None
