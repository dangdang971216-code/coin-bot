...
def summary_text() -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        "- OPEN 요약:",
        *format_open_list(5).splitlines(),
        "",
        *_v058_strategy_perf_text(recent_closed),
        "",
        f"- 이번 cycle: open +{status.get('opened_this_cycle',0)} / close +{status.get('closed_this_cycle',0)} / {status.get('elapsed_sec','-')}s",
        f"- 직전 cycle 후보: 읽음 {pick.get('paper_file',0)} / OPEN통과 {status.get('opened_this_cycle',0)} / 복기 {pick.get('shadow_review_only_skip',0)} / 중복 {pick.get('same_ticker_skip',0)}",
        f"- 알림: 정식 자동매매급 즉시 / 3전략 실험진입은 시간요약",
        f"- 업데이트: {status.get('updated_at_text','-')}",
        f"- 오류로그 {err_size} bytes / 자세히 /perror",
    ]
    return "\n".join(lines)

# ===============================
# paper_bot v0.60: v271 외부조건 실험 대응
