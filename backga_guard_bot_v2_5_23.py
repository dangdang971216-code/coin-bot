...
VERSION = "guard_v2.5.23_micro_autorepair_external_state_2026-05-15"
...
def load_env(path: Path = ENV_PATH) -> dict:
    ...
    allowed = {
        ...
        "MICRO_ACTIVE_FILE", "MICRO_PID_FILE", "MICRO_LOG_FILE", "MICRO_STATUS_FILE", "MICRO_CACHE_FILE", "MICRO_PYTHON_BIN"
    }
...
def ws_runtime_env() -> dict:
    ...
    if running_pids:
        ...
        for key in ("HOME", "PYTHONPATH", "PYTHONHOME", "VIRTUAL_ENV", "PATH", "LD_LIBRARY_PATH"):
            if penv.get(key):
                env[key] = penv[key]
    home = env.get("HOME") or _guess_bot_owner_home()
    if home:
        env["HOME"] = home
...
def ws_import_check_for(python_bin: str) -> tuple[bool, str]:
    code = (
        "import sys, os; "
...
def resolve_ws_python(preferred: Optional[str] = None) -> dict:
    """웹소켓 실행용 python을 결정한다.
    실패하던 원인은 configured=/usr/bin/python3에는 websockets가 없고,
    실제 실행 중인 /usr/bin/python3.10에는 websockets가 있는 경로 불일치였다.
    따라서 현재 실행 중인 python을 우선하고, 각 후보에서 import websockets를 직접 검사한다.
    """
    candidates = _dedupe_keep_order(([preferred] if preferred else []) + ws_python_candidates())
    notes = []
    for py in candidates:
        ok, out = ws_import_check_for(py)
        first = (out.splitlines()[0] if out else "출력없음")
        notes.append(f"try {py}: {'OK' if ok else 'FAIL'} / {first}")
        if ok:
            return {"ok": True, "python": py, "notes": notes, "output": out, "candidates": candidates}
    return {"ok": False, "python": preferred or WS_PYTHON_BIN, "notes": notes, "output": "", "candidates": candidates}
...
def start_ws_sidecar(python_bin: Optional[str] = None) -> tuple[str, list[str]]:
    notes = []
    ...
    pyinfo = resolve_ws_python(preferred=python_bin)
...
def restart_ws_sidecar(python_bin: Optional[str] = None) -> tuple[str, list[str]]:
    ...
    active, start_notes = start_ws_sidecar(python_bin=selected_py)
...
def ws_sidecar_state_dict() -> dict:
    ...
    if not active_path.exists():
        ...
        verdict = "✅ 정상수신"
        ok = True
    elif age_ok and state in {"연결", "수신대기", "대상준비", "guard_starting"} and err_ok:
        verdict = "❔ 실행중 / 수신대기"
        ok = False
...
def wait_ws_ready(prev_raw: int = 0, prev_match: int = 0, timeout: float = 22.0) -> tuple[dict, list[str]]:
    ...
    while time.time() < deadline:
        ...
        if alive and err in {"", "-", "None", "none"} and (raw > prev_raw or match > prev_match or (raw > 0 and match > 0)):
...
def apply_ws_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    if not target or not target.exists():
        return {"ok": True, "changed": False, "target": explicit or "?", "active": before_state.get("verdict", "유지"), "warning": "ws_sidecar_v*.py 최신 파일 없음 → 기존 상태 유지"}
    ...
    pyinfo = resolve_ws_python()
    import_notes = ["python auto-detect"] + [f"  · {x}" for x in pyinfo.get("notes", [])[:10]]
    if not pyinfo.get("ok"):
        # 실패 시 기존 정상 sidecar를 건드리지 않는다.
        return {"ok": False, "changed": False, "target": target.name, "error": "websockets import 가능한 python 없음", "notes": import_notes, "state": before_state.get("verdict"), "raw": before_raw, "match": before_match}
    selected_py = str(pyinfo.get("python") or WS_PYTHON_BIN)
    active_path = BOT_DIR / WS_ACTIVE_FILE
    same_hash = active_path.exists() and short_hash(active_path) == short_hash(target)
    backup = "없음"
    ...
    notes = import_notes + [f"selected_python={selected_py}"]
    active = "restart 생략"
    if restart:
        active, restart_notes = restart_ws_sidecar(python_bin=selected_py)
        notes += restart_notes
        ready_state, wait_notes = wait_ws_ready(prev_raw=0 if changed else before_raw, prev_match=0 if changed else before_match, timeout=24.0)
        notes += wait_notes
    else:
        ready_state = ws_sidecar_state_dict()
        notes.append("restart 생략")

    ws_ok = bool(ready_state.get("alive")) and int(ready_state.get("parse_ok") or 0) > 0 and int(ready_state.get("match_ok") or 0) > 0 and str(ready_state.get("last_error") or "-") in {"", "-", "None", "none"}
    rollback_note = ""
    # v2.5.20: 새 파일 적용 후 실패하면 백업을 자동복구한다.
    if not ws_ok and changed and backup_path and backup_path.exists():
        try:
            ...
            (BOT_DIR / DEPLOYED_WS_FILE).write_text((read_file(BOT_DIR / DEPLOYED_WS_FILE) or target.name) + "\n", encoding="utf-8")
            active2, rb_notes = restart_ws_sidecar(python_bin=selected_py)
            rb_state, rb_wait = wait_ws_ready(timeout=18.0)
    ...
    final_ok = bool(ready_state.get("alive")) and int(ready_state.get("parse_ok") or 0) > 0 and int(ready_state.get("match_ok") or 0) > 0
    return {"ok": final_ok, "changed": changed, "target": target.name, "version": ws_target_version(target), "active": active, "backup": backup, "hash": short_hash(active_path), "notes": notes, "warning": "변경 없음 → active 파일 hash 동일" if not changed else rollback_note, "state": ready_state.get("verdict"), "raw": ready_state.get("raw_total"), "match": ready_state.get("match_ok"), "selected_python": selected_py}
...
def gws_upgrade_text(force: bool = False, explicit: Optional[str] = None) -> str:
    """웹소켓 직원만 따로 최신 적용한다.
    /gupgrade에서는 main/paper만 처리하고, ws_sidecar는 이 명령으로만 교체·재시작한다.
    """
    t0 = perf_now()
    before = ws_sidecar_state_dict()
    ok_git, steps = git_update()
    if not ok_git:
        return (
            "🛰 ❌ 웹소켓 직원 업그레이드 실패 /gws_upgrade\n"
            "- 단계: GitHub 갱신 실패\n"
            "- main/paper는 건드리지 않음\n\n"
            + "\n\n".join(steps)
        )
    res = apply_ws_sidecar_latest(force=force, explicit=explicit, restart=True)
    after = ws_sidecar_state_dict()
    status = read_json(BOT_DIR / UPGRADE_STATUS_FILE)
    status["ws_sidecar"] = res
    status["guard_version"] = VERSION
    status["ws_upgrade_time"] = now()
    status["ws_upgrade_total_sec"] = perf_now() - t0
    write_json(BOT_DIR / UPGRADE_STATUS_FILE, status)
    icon = "✅" if res.get("ok") else "❌"
    return (
        f"🛰 {icon} 웹소켓 직원 업그레이드 /gws_upgrade\n"
        f"- guard: {VERSION}\n"
        f"- 방식: ws_sidecar만 교체·재시작 / main,paper는 건드리지 않음\n"
        f"- total: {fmt_sec(perf_now() - t0)}\n"
        f"- 이전: {before.get('verdict')} / pid {before.get('used_pid') or '-'} / raw {before.get('raw_total',0)} / match {before.get('match_ok',0)}\n\n"
        f"[적용 결과]\n{format_ws_apply_result(res)}\n\n"
        f"[현재 상태]\n"
        f"- 상태: {after.get('verdict')}\n"
        f"- active_file: {after.get('active_file')} / version: {after.get('version')} / hash: {after.get('hash')}\n"
        f"- pid: {after.get('used_pid') or '-'} / alive: {after.get('alive')} / status_age: {after.get('age_text','-')}\n"
        f"- python: {after.get('python_path') or '-'} / configured: {after.get('configured_python') or '-'}\n"
        f"- 수신: raw {after.get('raw_total',0)} / parse {after.get('parse_ok',0)} / price {after.get('price_ok',0)} / match {after.get('match_ok',0)}\n"
        f"- last_error: {after.get('last_error','-')}\n\n"
        f"다음 확인:\n"
        f"- 가드봇: /gws_state /gdeploy\n"
        f"- 메인봇: /ws_status /core /quality"
    )
...
def format_ws_apply_result(res: dict) -> str:
    ...
    for k in ["target", "version", "active", "selected_python", "state", "raw", "match", "backup", "hash", "error", "warning"]:
...
def apply_latest_auto(force: bool = False, explicit: Optional[str] = None) -> str:
    """v2.5.16: /gupgrade는 메인봇과 페이퍼봇만 적용한다.

    ws_sidecar는 메인봇 생명주기와 분리된 보조 직원이다.
    한 번 정상수신 중인 웹소켓 직원을 /gupgrade 때마다 재시작하면
    python 환경/import/권한 문제로 정상 업그레이드가 확인필요로 바뀐다.
    따라서 /gupgrade에서는 ws_sidecar를 교체·재시작하지 않고 상태만 참고 표시한다.
    웹소켓 직원은 필요할 때만 /gws_state, /gws_restart로 별도 관리한다.
    """
    total_t0 = perf_now()
    ...
    ws_state = ws_sidecar_state_dict()
    ws_note = {
        "ok": True,
        "changed": False,
        "restart": False,
        "target": read_file(BOT_DIR / DEPLOYED_WS_FILE) or "ws_sidecar_v*.py",
        "active": ws_state.get("verdict", "확인중"),
        "warning": "gupgrade에서 제외: 웹소켓직원은 /gws_state, /gws_restart로 별도 관리",
        "notes": [
            f"pid={ws_state.get('used_pid') or '-'} alive={ws_state.get('alive')}",
            f"raw={ws_state.get('raw_total',0)} parse={ws_state.get('parse_ok',0)} match={ws_state.get('match_ok',0)}",
        ],
    }
    micro_repair = micro_health_repair_if_needed()
    status = read_json(BOT_DIR / UPGRADE_STATUS_FILE)
    status["paper"] = paper_res
    status["ws_sidecar"] = ws_note
    status["micro_repair"] = micro_repair
    status["guard_version"] = VERSION
    status["main_paper_total_sec"] = perf_now() - total_t0
    status["ok"] = bool(main_ok and paper_ok)
    write_json(BOT_DIR / UPGRADE_STATUS_FILE, status)
    title = "🚀 ✅ 스마트 업그레이드 완료 /gupgrade" if main_ok and paper_ok else "🚀 ❔ 스마트 업그레이드 확인 필요 /gupgrade"
    return (
        f"{title}\n"
        f"- guard: {VERSION}\n"
        f"- 방식: main/paper만 비교 후 바뀐 것만 교체·재시작\n"
        f"- 웹소켓직원: /gupgrade 제외, 별도 관리(/gws_state /gws_restart)\n"
        f"- total: {fmt_sec(perf_now() - total_t0)}\n\n"
        f"[메인봇]\n{main_text}\n\n"
        f"[페이퍼봇]\n{format_paper_apply_result(paper_res)}\n\n"
        f"[웹소켓직원 - 확인만]\n"
        f"- 상태: {ws_state.get('verdict')}\n"
        f"- pid: {ws_state.get('used_pid') or '-'} / alive: {ws_state.get('alive')} / status_age: {ws_state.get('age_text','-')}\n"
        f"- 수신: raw {ws_state.get('raw_total',0)} / parse {ws_state.get('parse_ok',0)} / match {ws_state.get('match_ok',0)}\n"
        f"- 조치: 이번 업그레이드에서 교체·재시작 안 함\n\n"
        f"[호가·체결직원 - 자동복구]\n"
        f"- desired: {'enabled' if micro_desired_enabled() else 'disabled'}\n"
        f"- repair: {'실행' if micro_repair.get('attempted') else '불필요'}\n"
        f"- 상태: {micro_repair.get('after',{}).get('verdict','-')} / fresh {micro_repair.get('after',{}).get('fresh',0)}\n\n"
        f"다음 확인:\n"
        f"- 가드봇: /gdeploy /gpaper_state /gws_state\n"
...
def micro_state_dict() -> dict:
    ...
    alive = bool(used and pid_alive(used))
    if alive and age >= 0 and age <= 30 and (fresh > 0 or cached > 0):
        verdict = "✅ 정상수집"
        ok = True
...
def start_micro_sidecar(python_bin: Optional[str] = None) -> tuple[str, list[str]]:
    ...
    try:
        ...
        note = write_ws_pid_file.__name__
        try:
    ...
    time.sleep(2)
    return "started" if pid_alive(p.pid) else "exited", notes
...
def wait_micro_ready(prev_poll: int = 0, timeout: float = 18.0) -> tuple[dict, list[str]]:
    notes = []
    deadline = time.time() + max(5.0, timeout)
    ...
    while time.time() < deadline:
        ...
        if st.get("alive") and (fresh > 0 or poll > prev_poll):
            notes.append(f"ready fresh={fresh} cached={st.get('cached')} poll={poll} age={st.get('age_text')}")
            return st, notes
        if not st.get("alive"):
            notes.append("wait: process not alive")
            break
        time.sleep(2)
    notes.append(f"wait_timeout fresh={best.get('fresh',0)} cached={best.get('cached',0)} poll={best.get('poll_count',0)}")
    return best, notes
...
def apply_micro_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    if not target or not target.exists():
        return {"ok": True, "changed": False, "target": explicit or "?", "active": before.get("verdict", "유지"), "warning": "bithumb_micro_sidecar_v*.py 최신 파일 없음 → 기존 상태 유지"}
    okc, out = py_compile(target)
    if not okc:
        return {"ok": False, "changed": False, "target": target.name, "error": "micro py_compile 실패", "notes": [tail(out, 1000)]}
    active_path = BOT_DIR / MICRO_ACTIVE_FILE
    same_hash = active_path.exists() and short_hash(active_path) == short_hash(target)
    backup = "없음"; backup_path = None; changed = False
    if not same_hash or force:
        if active_path.exists():
            backup_path = BOT_DIR / f"{active_path.name}.backup_guard_apply_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            shutil.copy2(active_path, backup_path)
        ...
        if sha256_file(tmp) != sha256_file(target):
            tmp.unlink(missing_ok=True)
            return {"ok": False, "changed": False, "target": target.name, "error": "micro 임시복사 hash 불일치"}
        os.replace(tmp, active_path)
        (BOT_DIR / DEPLOYED_MICRO_FILE).write_text(target.name + "\n", encoding="utf-8")
        changed = True
    else:
        (BOT_DIR / DEPLOYED_MICRO_FILE).write_text(target.name + "\n", encoding="utf-8")
    notes = [f"python={MICRO_PYTHON_BIN}"]
    if restart:
        active, rn = restart_micro_sidecar(python_bin=MICRO_PYTHON_BIN)
        notes += rn
        state, wn = wait_micro_ready(prev_poll=0 if changed else prev_poll, timeout=22.0)
        notes += wn
    else:
        active = "restart 생략"; state = micro_state_dict(); notes.append("restart 생략")
    ok = bool(state.get("alive")) and (int(state.get("fresh") or 0) > 0 or int(state.get("cached") or 0) > 0)
    rollback_note = ""
    if not ok and changed and backup_path and backup_path.exists():
        try:
            ...
            state, wn = wait_micro_ready(timeout=16.0)
            notes += [rollback_note, f"rollback_restart={active2}"] + rn + wn
        except Exception as exc:
            notes.append(f"rollback 실패: {exc.__class__.__name__}: {str(exc)[:160]}")
    final_ok = bool(state.get("alive")) and (int(state.get("fresh") or 0) > 0 or int(state.get("cached") or 0) > 0)
    return {"ok": final_ok, "changed": changed, "target": target.name, "version": micro_target_version(target), "active": active, "backup": backup, "hash": short_hash(active_path), "notes": notes, "warning": "변경 없음 → active 파일 hash 동일" if not changed else rollback_note, "state": state.get("verdict"), "fresh": state.get("fresh"), "cached": state.get("cached"), "orderbook_ok": state.get("orderbook_ok"), "trade_ok": state.get("trade_ok")}
...
def format_micro_apply_result(res: dict) -> str:
    ...
    for k in ["target", "version", "active", "state", "fresh", "cached", "orderbook_ok", "trade_ok", "backup", "hash", "error", "warning"]:
        if res.get(k) not in (None, ""):
            lines.append(f"- {k}: {res.get(k)}")
    if res.get("notes"):
        lines.append("- notes:")
...
def gmicro_state_text() -> str:
    ...
    return "\n".join([
        ...
        f"- pid_file: {d.get('pid_file') or '-'} / proc: {proc} / used: {d.get('used_pid') or '-'} / alive={d.get('alive')}",
        f"- python: {d.get('python_path') or '-'} / configured: {d.get('configured_python') or '-'}",
        f"- desired: {'enabled' if d.get('desired_enabled') else 'disabled'}",
        f"- status_age: {d.get('age_text','-')} / state: {d.get('state','-')} / last_error: {d.get('last_error','-')}",
...
def gmicro_start_text() -> str:
    ...
    return "\n".join(["▶️ 빗썸 호가·체결 직원 시작 /gmicro_start", f"- active: {active}", f"- version: {micro_target_version(BOT_DIR / MICRO_ACTIVE_FILE)}"] + [f"- {x}" for x in notes] + ["", "다음 확인: /gmicro_state /quality"])


def gmicro_stop_text() -> str:
    ok, notes = stop_micro_sidecar()
    return "\n".join(["⏹ 빗썸 호가·체결 직원 중지 /gmicro_stop", f"- ok: {ok}"] + [f"- {x}" for x in notes])


def gmicro_restart_text() -> str:
    active, notes = restart_micro_sidecar()
    return "\n".join(["🔁 빗썸 호가·체결 직원 재시작 /gmicro_restart", f"- active: {active}", f"- version: {micro_target_version(BOT_DIR / MICRO_ACTIVE_FILE)}"] + [f"- {x}" for x in notes] + ["", "다음 확인: /gmicro_state /quality"])


def gmicro_upgrade_text(force: bool = False, explicit: Optional[str] = None) -> str:
    set_micro_desired(True, "gmicro_upgrade")
    t0 = perf_now()
    before = micro_state_dict()
    ok_git, steps = git_update()
    if not ok_git:
        return "🔎 ❌ 빗썸 호가·체결 직원 업그레이드 실패 /gmicro_upgrade\n- 단계: GitHub 갱신 실패\n- main/paper/ws는 건드리지 않음\n\n" + "\n\n".join(steps)
    res = apply_micro_sidecar_latest(force=force, explicit=explicit, restart=True)
    after = micro_state_dict()
    status = read_json(BOT_DIR / UPGRADE_STATUS_FILE)
    status["micro_sidecar"] = res
    status["guard_version"] = VERSION
    status["micro_upgrade_time"] = now()
    status["micro_upgrade_total_sec"] = perf_now() - t0
    write_json(BOT_DIR / UPGRADE_STATUS_FILE, status)
    icon = "✅" if res.get("ok") else "❌"
    return (
        f"🔎 {icon} 빗썸 호가·체결 직원 업그레이드 /gmicro_upgrade\n"
        f"- guard: {VERSION}\n"
        f"- 방식: micro_sidecar만 교체·재시작 / main,paper,ws는 건드리지 않음\n"
        f"- total: {fmt_sec(perf_now() - t0)}\n"
        f"- 이전: {before.get('verdict')} / pid {before.get('used_pid') or '-'} / fresh {before.get('fresh',0)} / cache {before.get('cached',0)}\n\n"
        f"[적용 결과]\n{format_micro_apply_result(res)}\n\n"
        f"[현재 상태]\n"
        f"- 상태: {after.get('verdict')}\n"
        f"- active_file: {after.get('active_file')} / version: {after.get('version')} / hash: {after.get('hash')}\n"
        f"- pid: {after.get('used_pid') or '-'} / alive: {after.get('alive')} / status_age: {after.get('age_text','-')}\n"
        f"- 수집: targets {after.get('targets',0)} / cache {after.get('cached',0)} / fresh {after.get('fresh',0)} / orderbook_ok {after.get('orderbook_ok',0)} / trade_ok {after.get('trade_ok',0)}\n"
        f"- last_error: {after.get('last_error','-')}\n\n"
        f"다음 확인:\n- 가드봇: /gmicro_state /gdeploy\n- 메인봇: /quality /core"
    )
...
def deploy_status() -> str:
    ...
    return (
        ...
        f"🧾 최근 오류 의심\n{focus_text}\n\n"
        f"명령\n"
        f"- /gupgrade : 메인봇 + 페이퍼봇 변경분 최신 적용(웹소켓 제외)\n"
...
# v2.5.22: 외부직원 통합상태. main/paper/ws/micro 업그레이드 구조는 변경하지 않음.
...
def service_status() -> str:
    ...
    return (
        ...
        f"- 업그레이드: /gupgrade 한 번으로 main + paper_bot 확인(웹소켓 제외)\n\n"
        f"📊 상태\n"
        f"- guard: {VERSION}\n"
        ...
        f"🧭 자주 쓰는 명령\n"
        f"- /gdeploy 배포상태\n"
        f"- /gupgrade main+paper 변경분만 자동적용(웹소켓 제외)\n"
        f"- /gpaper_restart 페이퍼봇 재시작\n"
...
def help_text() -> str:
    return (
        ...
        "/gupgrade - GitHub 최신 main + paper_bot 변경분만 자동 적용(웹소켓 제외)\n"
        "/glog - 메인봇 최근 오류 로그\n"
        ...
        "/gmicro_upgrade - 빗썸 호가·체결 직원 최신 적용\n"
        "/gbackups - 메인봇 백업 목록\n"
        ...
        "운영법: main/paper는 /gupgrade, ws_sidecar는 /gws_upgrade, 호가·체결 직원은 /gmicro_upgrade로 별도 적용.\n"
...
def set_commands() -> None:
    commands = [
        ...
        {"command": "gpaper_service", "description": "페이퍼봇 서비스화 안내"},
        ...
        {"command": "gws_upgrade", "description": "웹소켓 직원 최신 적용"},
        {"command": "gmicro_state", "description": "호가·체결 직원 상태"},
        {"command": "gmicro_restart", "description": "호가·체결 직원 재시작"},
        {"command": "gmicro_upgrade", "description": "호가·체결 직원 최신 적용"},
        {"command": "gws_import", "description": "웹소켓 Python 검사"},
...
def handle_single_command(text: str) -> str:
    ...
    if cmd in ("/gws_upgrade", "/gwsupgrade", "/gws_apply"):
        ...
        if force and len(args) > 1:
            ...
            explicit = args[0]
        return gws_upgrade_text(force=force, explicit=explicit)
    ...
    if cmd in ("/gmicro_upgrade", "/gmicroupgrade", "/gmicro_apply"):
        force = bool(args and args[0].lower() == "force")
        explicit = None
        if force and len(args) > 1:
            explicit = args[1]
            ...
            explicit = args[0]
        return gmicro_upgrade_text(force=force, explicit=explicit)
            ...
            for upd in data.get("result", []):
                ...
                if text.startswith("/"):
                    ...
                    if first in ("/gupgrade", "/apply_latest", "/upgradebot"):
                        send(chat_id, "🚀 /gupgrade 접수\n- GitHub 갱신 → main/paper 검수 → 바뀐 것만 교체·재시작 확인 중\n- 웹소켓직원은 업그레이드에서 제외하고 상태만 확인함")
