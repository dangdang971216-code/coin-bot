...
DEFAULT_CONTROL = {
    "running": False,
    "loop_seconds": 8,
    "max_open_strict": 0,  # 0 = 고정 OPEN 갯수 제한 없음(모의매매 검증용)
    "max_open_shadow": 0,  # 0 = 고정 OPEN 갯수 제한 없음(모의매매 검증용)
    ...
    "candidate_read_max_lines": 6000,
}

# 구 control 파일에 남아 있을 수 있는 candidate_events 옵션은 v0.19에서 강제 무시한다.
    ...
    if isinstance(saved, dict):
        for k, v in saved.items():
            ...
            if k in DEFAULT_CONTROL:
                control[k] = v
    # v0.19 고정: candidate_events 소비 옵션은 저장돼 있어도 다시 만들지 않는다.
    for k in REMOVED_CONTROL_KEYS:
        control.pop(k, None)

    # v0.19: 구 control 파일에 남은 24/80/120 고정 갯수 제한은 후보 검증을 막을 수 있으므로
    # 기본값이면 0(무제한)으로 정리한다. 사용자가 나중에 직접 다른 값으로 조정하는 것은 허용한다.
    legacy_defaults = {"max_new_per_cycle": 24, "max_open_strict": 80, "max_open_shadow": 120}
    changed = False
    for k, old_v in legacy_defaults.items():
        try:
            if int(float(control.get(k, 0))) == old_v:
                control[k] = 0
                changed = True
        except Exception:
            pass
    try:
        if int(float(control.get("candidate_read_max_lines", 0))) < 6000:
            control["candidate_read_max_lines"] = 6000
            changed = True
    except Exception:
        control["candidate_read_max_lines"] = 6000
        changed = True
...
def pick_candidates(control: Dict[str, Any], open_pos: Dict[str, Dict[str, Any]]) -> Tuple[List[Tuple[str, Dict[str, Any]]], Dict[str, int]]:
    ...
    for ev in read_jsonl(FILES["paper_candidates"], max_lines=int(float_any(control.get("candidate_read_max_lines"), default=2500))):
        if new_cap_on and len(picked) >= max_new:
            break
        stats["paper_file"] += 1
        try_pick(ev, "strict")
    ...
    for ev in read_jsonl(FILES["shadow_candidates"], max_lines=int(float_any(control.get("candidate_read_max_lines"), default=2500))):
        if new_cap_on and len(picked) >= max_new:
            break
        stats["shadow_file"] += 1
        try_pick(ev, "shadow")

    # candidate_events는 v0.19에서 완전 제거. 읽지도 세지도 않는다.
    stats["events_file"] = 0
    return picked, stats
...
def run_cycle() -> Dict[str, Any]:
    ...
    with _state_lock:
        ...
        status = {
            ...
            "candidate_events_note": "v0.19: candidate_events 읽기/소비 없음",
        }
...
def summary_text() -> str:
    ...
    p_fresh = candidate_fresh_stats(FILES["paper_candidates"], control)
    s_fresh = candidate_fresh_stats(FILES["shadow_candidates"], control)
    ...
    return "\n".join([
        ...
        f"- 소비파일: 정식 {fs.get('paper_candidates',{}).get('lines',0)} lines(TTL유효 {p_fresh.get('fresh',0)}) / 복기 {fs.get('shadow_candidates',{}).get('lines',0)} lines(TTL유효 {s_fresh.get('fresh',0)}) / candidate_events 소비 0",
        f"- 이번 cycle: open +{status.get('opened_this_cycle',0)} / close +{status.get('closed_this_cycle',0)} / {status.get('elapsed_sec','-')}s",
        f"- pick: paper {pick.get('paper_file',0)} / shadow {pick.get('shadow_file',0)} / dup {pick.get('dup_skip',0)} / stale {pick.get('stale_skip',0)} / same_ticker {pick.get('same_ticker_skip',0)} / bad {pick.get('bad_skip',0)}",
...
def files_text() -> str:
    fs = file_stats()
    c = load_control()
    pf = candidate_fresh_stats(FILES["paper_candidates"], c)
    sf = candidate_fresh_stats(FILES["shadow_candidates"], c)
    return "\n".join([
        "📁 페이퍼봇 파일 /pfiles",
        f"- paper_candidates: {fs.get('paper_candidates',{}).get('lines',0)} lines / TTL유효 {pf.get('fresh',0)} / 만료 {pf.get('expired',0)} / {fs.get('paper_candidates',{}).get('size',0)} bytes",
        f"- shadow_candidates: {fs.get('shadow_candidates',{}).get('lines',0)} lines / TTL유효 {sf.get('fresh',0)} / 만료 {sf.get('expired',0)} / {fs.get('shadow_candidates',{}).get('size',0)} bytes",
...
def control_text() -> str:
    c = load_control()
    keys = ["running", "loop_seconds", "max_open_strict", "max_open_shadow", "max_new_per_cycle", "candidate_ttl_sec", "candidate_read_max_lines", "fee_pct_roundtrip", "take_profit_pct", "protect_trigger_pct", "protect_floor_pct", "stop_loss_pct", "slow_minutes", "time_exit_minutes", "block_same_ticker_open"]
    lines = ["⚙️ 페이퍼봇 설정 /pcontrol"]
    for k in keys:
        lines.append(f"- {k}: {c.get(k)}")
    lines.append("- allow_shadow_from_candidate_events: 제거됨/무시됨")
    lines.append("- 갯수 제한: max_* 값 0이면 후보를 작은 숫자로 자르지 않음")
    return "\n".join(lines)
...
def handle_command(chat_id: str, text: str) -> None:
    ...
    if cmd in {"/phelp", "/start"}:
        ...
        send_chat(chat_id, "✅ 페이퍼봇 점검 /pcheck\n\n" + summary_text() + "\n\n" + files_text())
    elif cmd == "/pbatch":
        ...
        send_chat(chat_id, "\n".join([
            ...
            "- 오래된 후보 line은 삭제하지 않고 TTL 지난 후보만 소비 제외",
        ]))
    elif cmd == "/ponce":
        st = run_cycle()
        ...
        lines = ["📊 페이퍼봇 성과 /pscore", "", "[신규 기준점 이후]", fmt_stats("신규 전체", new_rows), "", "[누적]", fmt_stats("전체", rows), fmt_stats("정식 strict", strict), fmt_stats("복기 shadow", shadow), "", "신규 청산사유"]
