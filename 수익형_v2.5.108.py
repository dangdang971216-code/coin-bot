...
def format_mtf_context_line(item: dict, short: bool = False) -> str:
    ...
    if short:
        return f"- 상위봉: 4시간 {htf_label} (고점거리 {htf_gap:.2f}% / 저점대비 {htf_rise:.2f}%) / 일봉 {daily_label} (고점거리 {daily_gap:.2f}% / 저점대비 {daily_rise:.2f}%)"
    return f"- 상위봉: 4시간 {htf_label} (최근4시간 고점거리 {htf_gap:.2f}% / 저점대비 {htf_rise:.2f}%) / 일봉 {daily_label} (일봉 고점거리 {daily_gap:.2f}% / 저점대비 {daily_rise:.2f}%)"
...
def send_startup_message():
    lines = [
        ...
        "- review_dump / missed에서 핵심 놓침과 움직였지만 애매을 같이 확인 가능하게 정리",
...
def format_missed_review_price_context(row: dict) -> str:
    ...
    if low30 > 0:
        parts.append(f"저점 {fmt_price(low30)}")
    if base_price > 0:
        parts.append(f"기준가 {fmt_price(base_price)}")
    return " / ".join(parts)
...
def build_supplemental_missed_review_rows(limit: int = 12):
    ...
    try:
        ...
        for row in reversed(src):
            ...
            stats = get_missed_review_window_stats(ticker, base_price)
            best_up_pct = round(safe_float(stats.get("best_up_pct", 0), 0), 2)
            end_up_pct = round(safe_float(stats.get("end_up_pct", 0), 0), 2)
            worst_down_pct = round(safe_float(stats.get("worst_down_pct", 0), 0), 2)
            # 참고용 표본이라도 실제로 어느 정도 움직였던 코인만 남긴다.
            moved_enough = (
                best_up_pct >= 0.80
                or end_up_pct >= 0.50
                or worst_down_pct <= -0.80
            )
            if not moved_enough:
                continue
            supp = {
