...
ALERT_PRICE_RECHECK_LIVE_MAX_UP_PCT = 0.28
ALERT_PRICE_RECHECK_RESTART_MAX_UP_PCT = 0.45
ALERT_PRICE_RECHECK_SKIP_UP_PCT = 0.95
ALERT_PRICE_RECHECK_LIVE_NEAR_HIGH_MAX_UP_PCT = 0.18
ALERT_PRICE_RECHECK_RESTART_NEAR_HIGH_MAX_UP_PCT = 0.25
...
LIVE_DISPLAY_MIN_5M_PCT = 0.45
LIVE_DISPLAY_MIN_VOL_RATIO = 4.00
LIVE_DISPLAY_MIN_LEADER_SCORE = 1.00
RESTART_DISPLAY_MIN_5M_PCT = 0.12
RESTART_DISPLAY_MIN_VOL_RATIO = 2.20
RESTART_DISPLAY_MIN_LEADER_SCORE = 0.75
RESTART_DISPLAY_MIN_HIGH_GAP_PCT = 1.00
RESTART_DISPLAY_MIN_COMBINED_MOTION_PCT = 0.55
RESTART_DISPLAY_MAX_FROM_LOW_PCT = 2.20
RESTART_DISPLAY_MAX_RISE_PCT = 2.80
...
def get_restart_quality_status(item: dict) -> tuple[bool, str]:
    ...
    if high_gap_pct < 0.75:
        return False, "최근 고점이 너무 가까워서 재출발 자리로 애매"
    if high_gap_pct < 1.00 and max(chg_30, from_low) >= 1.40:
        return False, "조금 반등했어도 고점이 너무 가까움"
    if chg_5 <= 0.05 and chg_30 <= 0.15:
        return False, "재출발 힘이 아직 거의 안 보임"
    if vol_ratio < 1.90 and max(chg_5, chg_30, from_low) < 0.90:
        return False, "거래량과 반등 힘이 약해서 더 지켜봐야 함"
    if from_low >= RESTART_DISPLAY_MAX_FROM_LOW_PCT and high_gap_pct < 1.15:
        return False, "저점에서 이미 꽤 올라와서 재출발 자리로는 늦음"
    if rise_pct >= RESTART_DISPLAY_MAX_RISE_PCT and high_gap_pct < 1.60:
        return False, "차트상 이미 많이 올라온 뒤라 재출발로 보기 어려움"
    if zone_ratio >= CHART_POS_HIGH_ZONE_RATIO and high_gap_pct < 1.10:
        return False, "상단 구역에 가까워서 재출발 여유가 부족"
    if chg_5 < -0.30 and chg_30 < 1.10 and vol_ratio < 2.80:
    ...
    if chg_30 >= 0.20:
        strength += 1
    if 0.18 <= from_low <= RESTART_DISPLAY_MAX_FROM_LOW_PCT:
        strength += 1
...
STRATEGY_GUIDE = {
    ...
    "실시간 감시형": "시장 관심은 붙지만 아직 사기엔 이른 자리",
...
def get_watch_display_group(item: dict, judgement: str = "", caution: str = "", section_override: str = "", regime: dict = None) -> str:
    ...
    if strategy == "LEADER_WATCH":
        ...
        if chg_5 >= LIVE_DISPLAY_OVERHEAT_5M_PCT and max(chg_30, from_low) >= 3.20 and high_gap_pct < 2.60:
            live_strength -= 2
        if live_strength >= 4 and chg_5 >= LIVE_DISPLAY_MIN_5M_PCT and vol_ratio >= LIVE_DISPLAY_MIN_VOL_RATIO and high_gap_pct >= 0.95:
            return "live"
        return "restart" if restart_ok else "watch"
...
def refresh_watch_item_before_alert(item: dict, section_key: str = "", regime: dict = None):
    ...
    if section_key == "live" and safe_float(snap.get("chg_5m", 0), 0) < 0.20 and safe_float(refreshed.get("vol_ratio", 0), 0) < 4.50:
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 실시간 강세 후보 최소 기준을 더 올려 약한 반응형은 덜 뜨게 조정",
        "- 이미 튄 뒤 늦은 실시간 강세형은 더 세게 잘라서 THQ류를 덜 올리게 함",
        "- 재출발 후보는 감시용은 남기되 본선 1순위처럼 과하게 안 올리게 조정",
        "- 알림 직전 현재가 재확인 기준도 더 엄격하게 조여 늦은 가격 괴리 차단",
