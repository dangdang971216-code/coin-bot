...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 가독성 보강: 어려운 수치는 쉬운 말(저점방어 / 돈유지힘 / 고점재도전) 쪽으로 같이 보이게 정리",
        "- 상위 후보 정리: discard 직전 후보는 상위 노출을 더 보수적으로 막고 내부 감시 위주로 남기기",
        "- 문구 완화: 초입/회복 확인형은 과장 대신 실전 진입감과 흐름 확인을 분리해서 표시",
        "- refresh 안정화 보강은 유지: 유니버스/캐시 부분 갱신이 오래 막히면 더 빨리 끊고 다음 차수로 넘기도록 조정",
...
def missed_command(update, context: CallbackContext):
    ...
    header_lines = ["📌 놓친 코인 정리", "- 참고: 저장된 표본 기준으로 바로 보여줘"]
    blocks = []
...
def compare_command(update, context: CallbackContext):
    ...
    header_lines = ["📊 알림 vs 놓친 코인 비교", "- 참고: 저장된 표본 기준으로 바로 보여줘"]
    blocks = []
...
def get_fast_scan_state(now_ts: float = None) -> dict:
    ...
    with fast_scan_state_lock:
        ...
        hide_candidates = bool(
            (age_sec is not None and age_sec >= FAST_SCAN_DISPLAY_HIDE_SEC)
            or (cache_age is not None and cache_age >= FAST_SCAN_DISPLAY_HIDE_SEC)
            or ((cache_age is not None and cache_age >= FAST_SCAN_STATE_STALE_SEC) and not market_refresh_in_progress)
        )
        return {
...
def fast_scan_background_refresh_loop():
    ...
    while True:
        ...
        try:
            ...
            cache_ready = bool(cache) and (cache_age is None or cache_age < FAST_SCAN_DISPLAY_HIDE_SEC)
            if cache_ready:
                need_refresh = (
                ...
                if fast.get("age_sec") is None or safe_float(fast.get("age_sec"), 0) >= FAST_SCAN_BACKGROUND_FORCE_REFRESH_SEC:
...
def get_scan_debug_candidates():
    ...
    fast = get_fast_scan_state(now_ts)
    if fast.get('hide_candidates'):
        request_market_refresh(force=True, reason='status_candidates_stale')
        return [], '예전 후보는 숨겼고 새 시장 데이터 다시 받는 중이야'
    items = list(fast.get('items') or [])
    if items:
        cache_age = fast.get('cache_age')
        if cache_age is not None and cache_age >= FAST_SCAN_STATE_STALE_SEC:
            request_market_refresh(force=True, reason='status_candidates_stale')
            return [], f'시장 데이터가 {cache_age}초 지나서 지금 후보는 잠깐 숨겼어. 새로 다시 받는 중이야'
        note = f'최근 {cache_age}초 안 시장 데이터 기준' if cache_age is not None else '방금 고른 후보 기준'
        return items, note
    cache = shared_market_cache if isinstance(shared_market_cache, dict) else {}
    cache_age = int(now_ts - shared_market_cache_time) if shared_market_cache_time else None
    # 명령어에서 직접 후보를 새로 만들면 사용자가 누른 순간 scan_watchlist_fast가 또 무거워진다.
    # 상태 명령은 마지막 스냅샷만 보여주고, 새 계산은 background/main loop에 맡긴다.
    if cache and (cache_age is None or cache_age < FAST_SCAN_DISPLAY_HIDE_SEC):
        request_market_refresh(force=False, reason='status_wait_fast_scan')
        return [], '새 시장 데이터는 들어왔고 빠른 후보를 다시 고르는 중이야'
    if cache_age is not None and cache_age >= FAST_SCAN_DISPLAY_HIDE_SEC:
        request_market_refresh(force=True, reason='status_cache_stale')
        return [], '시장 데이터가 오래돼서 후보는 잠깐 숨겼어. 새로 다시 받는 중이야'
