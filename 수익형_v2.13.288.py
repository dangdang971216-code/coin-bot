...
def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["phase_note"] = "v288: 7전략 active path 제거, sweep_vwap_recovery 단일 paper 검증."
    return strict, shadow, rejects, examples


def _v288_write_micro_urgent(rows: List[Dict[str, Any]], reason: str = "v288_sweep_vwap_recheck") -> None:
    try:
        targets: List[str] = []
        meta: Dict[str, Any] = {}
        for r in rows or []:
            ...
            if str((r or {}).get("final_entry_action")) != "recheck_wait":
                continue
            targets.append(t)
            meta[t] = {"source": "v288_sweep_vwap", "score": fnum(r.get("score"), 0), "reason": ",".join(_v288_as_list(r.get("final_entry_reasons"))[:4])}
            if len(targets) >= 40:
                break
        if not targets:
            return
...
def _v288_write_strategy_files(strict_rows: List[Dict[str, Any]], shadow_rows: List[Dict[str, Any]], market: Dict[str, Any], rejects: Counter) -> None:
    ...
    payload = {
        ...
        "note": "v288: 7전략 제거 후 저점 쓸림 후 VWAP 회복 단일전략 paper 검증.",
    }
...
def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:  # type: ignore[override]
    ...
    _v288_write_micro_urgent(strict_rows, reason="v288_sweep_vwap_recheck")
    write_error = ""
    ...
    with _state_lock:
        ...
        STATE["factory_mode"] = "v288_sweep_vwap_recovery_factory"
    ...
    return {
        ...
        "dup_skip_reason": {},
        "data_quality_note": "v288_sweep_vwap_recovery",
        "dup_skip": 0,
        "write_error": write_error,
        "last_ticker": str((strict_rows or shadow_rows or [{"ticker":"-"}])[-1].get("ticker") or "-"),
        "factory_mode": "v288_sweep_vwap_recovery_factory",
        "archive_deferred": 0,
...
def select_precision_targets(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    with _state_lock:
        STATE["precision_target_note"] = f"v288 sweep/vwap selector {min(len(arr), limit)}/{len(arr)}"
        STATE["precision_target_source"] = "v288_sweep_vwap_selector"
    return arr[:limit]
...
def _v279_write_target_file(path: Path, tickers: List[str], reason: str) -> None:
    nowv = now_ts()
    payload = {"version": BOT_VERSION, "schema": "v288_sweep_vwap_targets", "updated_ts": nowv, "updated_text": now_text(nowv), "reason": reason, "targets": tickers, "target_count": len(tickers), "target_meta": {t: {"reason": reason, "source": STRATEGY_KEY} for t in tickers}}
    atomic_write(path, payload)


def update_ws_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "v288_sweep_vwap") -> None:  # type: ignore[override]
    ...
    with _state_lock:
        STATE["ws_targets"] = len(tickers); STATE["ws_target_file_written"] = 1; STATE["ws_target_file_targets"] = len(tickers); STATE["ws_target_reason"] = reason; STATE["ws_target_write_note"] = "v288 sweep/vwap target writer"


def update_micro_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "v288_sweep_vwap") -> None:  # type: ignore[override]
    tickers = _v279_target_tickers(rows, priority_rows, limit=180)
    with _micro_lock:
        _micro_targets[:] = tickers
    _v279_write_target_file(FILES.get("micro_targets", BASE_DIR / "clean_micro_targets.json"), tickers, reason)
    with _state_lock:
        STATE["micro_targets"] = len(tickers); STATE["micro_target_file_written"] = 1; STATE["micro_target_file_targets"] = len(tickers); STATE["micro_target_reason"] = reason; STATE["micro_target_write_note"] = "v288 sweep/vwap target writer"
...
def scan_once() -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["scan_display_note"] = "v288 sweep/vwap 스캔 진행중"
    ...
    try:
        ...
        _v278_stage_push(stage_times, "2) 직원: 정밀/VWAP 보강", now_ts() - st, f"targets {len(targets)} / 직접 {p_ok} / cached {p_have} / fail {p_fail}")
        with _state_lock:
        ...
        _v278_stage_push(stage_times, "4) 전략: sweep/vwap 판정", now_ts() - st, f"진입 {ready_n} / 재확인 {recheck_n} / shadow {len(shadow)} / reject {sum(rejects.values())}")
        with _state_lock:
        ...
        _v278_stage_push(stage_times, "5) 공장: latest 저장", now_ts() - st, f"latest {pipe.get('paper_latest_written',0)} / OPEN {pipe.get('latest_trade_ready',0)} / 재확인 {pipe.get('latest_final_recheck_wait',0)}")
        if _stop_event.is_set(): return []
...
def reversion_review_text(full: bool = False) -> str:  # type: ignore[override]
    return "\n".join([
        ...
        "- v288에서는 7전략 실험판을 active path에서 제거했습니다.",
...
try:
    ...
    STATE["phase_note"] = "v288: 7전략 제거, 저점 쓸림 후 VWAP 회복 단일전략 paper 검증."
