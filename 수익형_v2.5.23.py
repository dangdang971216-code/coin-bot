...
LIVE_FORCE_ENTRY_MIN_5M_PCT = 0.45
LIVE_FORCE_ENTRY_MIN_VOL_RATIO = 2.20
LIVE_FORCE_ENTRY_MIN_LEADER_SCORE = 0.80
...
def build_price_check_markup(prepared_entries, section_key_override: str = ""):
    ...
    for idx, prepared in enumerate(prepared_entries, start=1):
        ...
        section_key = section_key_override or get_watch_section_key(item, judgement, caution)
        rows.append([
            InlineKeyboardButton(f"{circled_number(idx)} {ticker} 빗썸 열기", url=get_bithumb_chart_url(ticker)),
        ])
        if section_key == "entry":
            rows.append([
...
TERM_GUIDE = [
    ...
    ("지금 진입 후보", "지금 바로 볼 가치가 있는 메인 후보"),
    ("저점권 회복 후보", "저점권에서 살아나는지 먼저 봐야 하는 후보"),
    ("지금 사면 늦은 후보", "이미 올라와서 지금 추격하면 별로인 후보"),
...
def get_watch_display_conclusion(item: dict, judgement: str = "", caution: str = "") -> str:
    strategy_label = item.get("strategy_label", "")
    judgement = judgement or get_watch_judgement_text(item)
    caution = caution or get_watch_caution_text(item)
    ...
    if caution:
        if "늦음" in caution or "추격" in caution or "사기 별로" in caution:
            return "지금 사기 별로"
        if "힘이 약" in caution:
            return "힘이 약해서 더 확인 필요"
        return caution
    if strategy_label == "신저가 반등 감시":
        return "다시 사는지 확인 먼저"
    if strategy_label == "예비 후보 감시":
        return "지금은 구경만"
    if "볼 만함" in judgement:
        return "지금 볼만함"
    if strategy_label == "눌림 반등형":
        return "다시 사는지 확인 먼저"
    if strategy_label in ["추세 지속형", "급등 돌파형"]:
...
def is_fast_start_entry_candidate(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if strategy not in {"LEADER_WATCH", "PREWATCH", "NEWLOW_RECOVER", "PREPUMP", "PULLBACK"}:
        return False
    ...
    if high_gap_pct < FAST_ENTRY_MIN_HIGH_GAP_PCT or high_gap_pct > 2.80:
        return False
    if from_low > 3.60:
        return False
    if change_pct > 4.60 and high_gap_pct < 1.25:
        return False

    strength = 0
    if chg_5 >= FAST_ENTRY_MIN_5M_PCT:
        strength += 1
    if chg_30 >= FAST_ENTRY_MIN_30M_PCT:
        strength += 1
    if from_low >= FAST_ENTRY_MIN_FROM_LOW_PCT:
        strength += 1
    if vol_ratio >= FAST_ENTRY_MIN_VOL_RATIO:
        strength += 1
    if leader_score >= FAST_ENTRY_MIN_LEADER_SCORE:
        strength += 1
    ...
    if regime_name == "SIDEWAYS":
        if chg_5 >= 0.18 and vol_ratio >= 1.55 and high_gap_pct >= 0.60:
            threshold -= 1
    elif regime_name == "WEAK":
        threshold += 1

    if strategy == "LEADER_WATCH" and vol_ratio >= 1.90 and leader_score >= 0.70 and max(chg_5, chg_30, from_low) >= 0.22:
        threshold -= 1
    if strategy in {"PREWATCH", "NEWLOW_RECOVER"} and vol_ratio >= 1.55 and max(chg_5, from_low, chg_30) >= 0.18:
        threshold -= 1
    if max(chg_5, chg_30, from_low) >= 0.55 and vol_ratio >= 1.35:
        threshold -= 1
...
def should_promote_recovery_candidate_to_entry(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if high_gap_pct < RECOVERY_ENTRY_MIN_HIGH_GAP_PCT:
        return False
    ...
    strength = 0
    if chg_30 >= RECOVERY_ENTRY_MIN_30M_PCT:
        strength += 1
    if from_low >= RECOVERY_ENTRY_MIN_FROM_LOW_PCT:
        strength += 1
    if vol_ratio >= RECOVERY_ENTRY_MIN_VOL_RATIO:
        strength += 1
    if leader_score >= RECOVERY_ENTRY_MIN_LEADER_SCORE:
        strength += 1
    if chg_5 >= RECOVERY_ENTRY_MIN_5M_PCT or change_pct >= 0.35:
        strength += 1
    ...
    if strategy == "LEADER_WATCH" and leader_score >= 1.35 and vol_ratio >= 1.45 and max(from_low, chg_30, change_pct) >= 0.18:
        threshold -= 1
    if strategy in ["PREWATCH", "NEWLOW_RECOVER"] and vol_ratio >= 1.55 and high_gap_pct >= 0.70:
        threshold -= 1
    if chg_5 >= 0.05 and vol_ratio >= 1.35 and high_gap_pct >= 0.55 and max(from_low, chg_30, change_pct) >= 0.20:
        threshold -= 1

    if regime_name == "SIDEWAYS":
        threshold += REGIME_RECOVERY_ENTRY_SIDEWAYS_EXTRA_STRENGTH
        if chg_5 >= 0.08 and vol_ratio >= 1.45 and high_gap_pct >= 0.70:
            threshold -= 1
    if regime_name == "WEAK":
        threshold += 1
...
def is_low_zone_recovery_candidate(item: dict, judgement: str = "", caution: str = "") -> bool:
    ...
    if strategy == "LEADER_WATCH":
        return (
            from_low >= 0.05
            and high_gap_pct >= 0.35
            and (
                (chg_30 >= 0.00 and vol_ratio >= 1.15)
                or (chg_5 >= -0.05 and vol_ratio >= 1.10)
                or (leader_score >= 0.85 and vol_ratio >= 1.05)
            )
        )
    if strategy in ["PREPUMP", "PULLBACK"]:
        return True
    if tags.intersection({"저점높임", "가짜하락회복", "하락복구", "눌림재확인"}) and from_low <= 2.40 and high_gap_pct >= 1.00:
        return True
    if leader_score >= 1.8 and vol_ratio >= 2.1 and from_low <= 2.10 and high_gap_pct >= 1.15:
        return True
    return False
...
def get_watch_section_key(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> str:
    ...
    if strategy in {"PREWATCH", "NEWLOW_RECOVER"} and section == "entry" and not is_fast_start_entry_candidate(item, judgement, caution, regime=regime):
        section = "recovery"
    ...
    if section == "entry" and trade_tier == "B":
        ...
        if high_gap_pct <= 0.25:
            section = "late"
        elif chg_5 < -0.35 and chg_30 < 0.05:
            section = "recovery" if recovery_like else "watch"
        elif leader_score < 0.35 and vol_ratio < 1.05:
            section = "recovery" if recovery_like else "watch"
        elif vol_ratio < 0.95 and not recovery_like:
            section = "watch"
        elif from_low < 0.05 and chg_30 < 0.10 and chg_5 < 0.00:
            section = "recovery"
        elif strategy == "LEADER_WATCH" and (vol_ratio < 1.15 or high_gap_pct < 0.30):
            section = "recovery" if recovery_like else "watch"

    if section == "entry" and strategy == "LEADER_WATCH":
        if chg_5 < 0.00 and chg_30 < 0.18:
            section = "recovery" if is_low_zone_recovery_candidate(item, judgement, caution) else "watch"
...
def get_watch_section_header(section_key: str, regime: dict = None) -> str:
    ...
    elif regime_name == "WEAK" and section_key in {"entry", "recovery", "watch"}:
        note = "- 장 상태: 약한 장이라 저점권 후보도 더 보수적으로 봄"
    elif regime_name == "SIDEWAYS" and section_key in {"entry", "late"}:
        note = "- 장 상태: 횡보 장이라 늦은 추격은 더 막는 중"
    elif regime_name == "STRONG_UP" and section_key in {"entry", "recovery"}:
        note = "- 장 상태: 장이 좋아서 회복 후보가 위로 올라가기 쉬움"
    if note:
        return base + "\n" + note
    return base

def get_watch_tier_text(item: dict) -> str:
    return str(item.get("trade_tier", "B") or "B").upper().strip()


def format_alert_price_line(item: dict) -> str:
    return f"- 가격: {fmt_price(item.get('current_price', 0))} / 상승 {safe_float(item.get('change_pct', 0)):.2f}% / 거래량 {safe_float(item.get('vol_ratio', 0)):.2f}배"
...
def format_alert_flow_line(item: dict, short: bool = False) -> str:
    ...
    if short:
        return f"- 흐름: 5분 {chg_5:+.2f}% / 30분 {chg_30:+.2f}% / 저점대비 {from_low:+.2f}% / {format_recent_high_gap_text(high_gap)}"
    return f"- 흐름: 5분 {chg_5:+.2f}% / 30분 {chg_30:+.2f}% / 저점대비 {from_low:+.2f}% / {format_recent_high_gap_text(high_gap)}"
...
def format_watch_alert_line(item: dict, order: int = 1, alert_type: str = "new", reason_text: str = "", section_override: str = "") -> str:
    ...
    conclusion = get_watch_display_conclusion(item, judgement, caution, section_override=section_override)
    section_key = section_override or get_watch_section_key(item, judgement, caution)
    one_line = get_watch_one_line_text(item, caution, reason_text)

    strategy = str(item.get("strategy", "") or "").strip()
    if strategy in {"PREWATCH", "NEWLOW_RECOVER"}:
        if section_key == "entry" and conclusion in {"지금 매수 금지", "지금은 구경만"}:
            conclusion = "지금 볼만함"
        elif section_key == "recovery" and conclusion in {"지금 매수 금지", "지금은 구경만"}:
            conclusion = "다시 사는지 확인 먼저"

    if section_key in {"recovery", "late", "watch"}:
        summary = one_line
        if section_key == "recovery" and not summary:
            summary = "저점권에서 살아나는지 먼저 확인"
        elif section_key == "late" and not summary:
            summary = "이미 올라온 자리라 지금 추격은 금지"
        elif section_key == "watch" and not summary:
            summary = "지금은 흐름만 확인"
        lines = [
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 늦은 후보 차단은 유지하고 초입 포착 쪽으로 방향 전환",
        "- 회복형/초입형 승격은 더 빨리, 감시 전략 entry 진입은 더 정확하게 조정",
        "- 감시 후보도 5분/30분/저점대비/최근고점까지 같이 보이게 보강",
        "- /scan_debug는 초입 가능 후보 / 회복 확인 후보 / 약한 후보로 다시 정리",
        "- 재시작하면 자동 쉬기 상태는 초기화되고 15분 뒤부터 다시 판단",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def build_watch_snapshot(item: dict, main_watch: bool = False):
    ...
    return {
        ...
        "section_key": get_watch_section_key(item),
    }
...
def build_leader_watch_candidates(cache, regime, relaxed=False):
    ...
    for ticker, data in cache.items():
        try:
            ...
            label = "주도주 후보형"
            if change_3 >= 0.32 and change_5 >= 0.45 and vol_ratio >= 2.60 and high_gap_pct >= 0.85 and leader_score >= 0.90:
                label = "초입 가속형"
            elif change_5 >= 0.75 and vol_ratio >= 1.45:
                label = "상승 시작형"
            elif vol_ratio >= 1.80 and change_3 >= 0.35:
                label = "주도주 가속형"
            elif top_rank_ok and (change_5 >= 0.12 or vol_ratio >= 1.05):
                label = "주도주 관찰형"
...
def is_live_force_candidate(item: dict, regime: dict = None) -> tuple[bool, str, float]:
    ...
    if high_gap_pct < 0.35 or high_gap_pct > 3.20:
        return False, "", 0.0
    ...
    if entry_strength >= 3:
        ...
        return True, "entry", priority

    # 그 다음 recovery라도 먼저 실제 알림으로 올림
    recovery_strength = 0
...
def maybe_send_live_force_alerts(cache, regime, source_candidates, sent_tickers, now_ts):
    ...
    for forced_section, priority, item in picked:
        ...
        if now_ts - prev_ts < LIVE_FORCE_PER_TICKER_COOLDOWN_SEC:
            continue
        reason_text = "상위 초입 후보라 실제 알림으로 먼저 올림" if forced_section == "entry" else "상위 회복 후보라 실제 알림으로 먼저 올림"
        row = (item, "new", reason_text)
        if forced_section == "entry" and len(entry_entries) < LIVE_FORCE_MAX_ENTRY_ITEMS:
            entry_entries.append(row)
        elif forced_section == "recovery" and len(recovery_entries) < LIVE_FORCE_MAX_RECOVERY_ITEMS:
            recovery_entries.append(row)
        if len(entry_entries) >= LIVE_FORCE_MAX_ENTRY_ITEMS and len(recovery_entries) >= LIVE_FORCE_MAX_RECOVERY_ITEMS:
            break

    if not entry_entries and not recovery_entries:
        return False

    if entry_entries:
        send_watch_alert_bundle(get_watch_section_header("entry", regime=regime), entry_entries, section_key="entry")
    if recovery_entries:
        send_watch_alert_bundle(get_watch_section_header("recovery", regime=regime), recovery_entries, section_key="recovery")

    for row in entry_entries + recovery_entries:
        item = row[0]
        ticker = str(item.get("ticker", "") or "").upper().strip()
        live_force_alert_history[ticker] = now_ts
        save_watch_snapshot(ticker, item, now_ts, alert_type="new", main_watch=(row in entry_entries))
...
def scan_watchlist(shared_cache=None):
    ...
    for item in top:
        ...
        judgement = get_watch_judgement_text(item)
        caution = get_watch_caution_text(item)
        section_key = get_watch_section_key(item, judgement, caution)
        section_entries[section_key].append((item, alert_type, reason_text))
        save_watch_snapshot(ticker, item, now_ts, alert_type=alert_type, main_watch=(section_key == "entry"))
    ...
    for item in build_prewatch_candidates(cache, regime, exclude_tickers=sent_tickers):
        ...
        if not should_send_simple_aux_alert(recent_prewatch_alerts, ticker, now_ts, PREWATCH_ALERT_COOLDOWN_SEC):
            continue
        section_key = "entry" if should_promote_recovery_candidate_to_entry(item, regime=regime) else "recovery"
        reason_text = "저점권에서 살아나는 힘이 붙어 바로 볼 후보" if section_key == "entry" else "저점권에서 살아나는지 먼저 볼 후보"
        section_entries[section_key].append((item, "new", reason_text))
        save_watch_snapshot(ticker, item, now_ts, alert_type="new", main_watch=(section_key == "entry"))
    ...
    for item in build_newlow_recovery_candidates(cache, regime, exclude_tickers=sent_tickers):
        ...
        if not should_send_simple_aux_alert(recent_newlow_alerts, ticker, now_ts, NEWLOW_ALERT_COOLDOWN_SEC):
            continue
        section_key = "entry" if should_promote_recovery_candidate_to_entry(item, regime=regime) else "recovery"
        reason_text = "저점권 반등 힘이 붙어 바로 볼 후보" if section_key == "entry" else "바닥권에서 반등하는지 먼저 볼 후보"
        section_entries[section_key].append((item, "new", reason_text))
        save_watch_snapshot(ticker, item, now_ts, alert_type="new", main_watch=(section_key == "entry"))
        sent_tickers.add(ticker)

    entry_entries = section_entries["entry"]
    recovery_entries = section_entries["recovery"]
    late_entries = section_entries["late"]
    watch_entries = section_entries["watch"]

    if len(entry_entries) + len(recovery_entries) < 2 and skipped_top:
        promoted_pool = []
        for item in skipped_top:
            ticker = item["ticker"]
            ...
            judgement = get_watch_judgement_text(item)
            caution = get_watch_caution_text(item)
            section_hint = get_watch_section_key(item, judgement, caution, regime=regime)
            snap = ensure_pre_alert_snapshot(item)
            chg_5 = safe_float(snap.get("chg_5m", 0), 0)
            chg_30 = safe_float(snap.get("chg_30m", 0), 0)
            from_low = safe_float(snap.get("from_30m_low_pct", 0), 0)
            ...
            entry_like = (
                section_hint == "entry"
                or (vol_ratio >= 1.45 and leader_score >= 0.20 and high_gap_pct >= 0.65 and max(chg_5, chg_30, from_low) >= 0.22)
            )
            recovery_like = (
                section_hint in {"entry", "recovery"}
                or (vol_ratio >= 1.20 and high_gap_pct >= 0.75 and max(chg_5, chg_30, from_low) >= 0.15)
            )

            if entry_like:
                promoted_pool.append(("entry", priority + 0.25, item, "new", "상위 초입 후보라 먼저 확인"))
            elif recovery_like:
                promoted_pool.append(("recovery", priority, item, "new", "상위 회복 후보라 먼저 확인"))

        promoted_pool.sort(key=lambda x: x[1], reverse=True)
        add_entry_max = 1 if not entry_entries else 0
        add_recovery_max = max(0, 2 - len(recovery_entries))
        for section_key, _prio, item, alert_type, reason_text in promoted_pool:
            if section_key == "entry" and add_entry_max <= 0:
                section_key = "recovery"
            if section_key == "recovery" and add_recovery_max <= 0:
                continue
            if section_key == "entry":
                add_entry_max -= 1
            else:
                add_recovery_max -= 1
            section_entries[section_key].append((item, alert_type, reason_text))
            save_watch_snapshot(item["ticker"], item, now_ts, alert_type=alert_type, main_watch=(section_key == "entry"))
            sent_tickers.add(item["ticker"])

    if not (entry_entries or recovery_entries):
        aggressive_promoted = []
        remaining_watch = []
        for row in watch_entries:
            ...
            vol_ratio = safe_float(item.get("vol_ratio", 0), 0)
            leader_score = safe_float(item.get("leader_score", 0), 0)
            if high_gap_pct >= 0.30 and vol_ratio >= 1.10 and max(chg_5, chg_30, from_low) >= 0.12:
                aggressive_promoted.append(row)
            elif leader_score >= 1.20 and vol_ratio >= 1.00 and max(chg_30, from_low) >= 0.10:
                aggressive_promoted.append(row)
            else:
                remaining_watch.append(row)
        if aggressive_promoted:
            aggressive_promoted.sort(key=lambda x: signal_priority_value(x[0]), reverse=True)
            recovery_entries.extend(aggressive_promoted[:2])
            watch_entries = remaining_watch

    sent_any_alert = False
    if entry_entries:
        send_watch_alert_bundle(get_watch_section_header("entry", regime=regime), entry_entries[:4], section_key="entry")
        sent_any_alert = True
    if recovery_entries:
        limit = 2 if entry_entries else 3
        send_watch_alert_bundle(get_watch_section_header("recovery", regime=regime), recovery_entries[:limit], section_key="recovery")
        sent_any_alert = True
    if late_entries:
        limit = 1 if entry_entries else 2
        send_watch_alert_bundle(get_watch_section_header("late", regime=regime), late_entries[:limit], section_key="late")
        sent_any_alert = True
    if watch_entries and not (entry_entries or recovery_entries or late_entries):
        send_watch_alert_bundle(get_watch_section_header("watch", regime=regime), watch_entries[:2], section_key="watch")
        sent_any_alert = True

    if not sent_any_alert:
...
def build_scan_debug_text():
    ...
    append_group("✅ 초입 가능 후보", strong_items)
    append_group("🌱 회복 확인 후보", watch_items)
    append_group("⚠️ 아직 약하거나 늦은 후보", weak_items[:3])
    return "\n".join(lines)
...
def append_debug_shortlist_parts(parts: list):
    try:
        ...
        for item in shortlist:
            label = "초입" if str(item.get("section", "")) == "strong" else ("회복" if str(item.get("section", "")) == "watch" else "관찰")
            lines.append(
...
save_pending_buys()
persist_price_check_state()
reset_auto_pause_state(bypass_sec=300, reason="restart")
