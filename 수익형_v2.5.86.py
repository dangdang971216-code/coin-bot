...
USER_ALERT_RESCUE_MIN_EDGE = 4.70
USER_ALERT_RESCUE_MIN_VOL = 1.05
USER_ALERT_RESCUE_MIN_LEADER = 0.45
USER_ALERT_RESCUE_MIN_HIGH_GAP_PCT = 0.40
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 너무 약한 미리 볼 후보는 더 줄이고, 먼저 붙는 기미가 있는 후보만 더 보수적으로 띄우게 조정",
        "- 저점에서만 시작하지 않아도 늦지 않은 상승/회복형은 승격될 수 있게 rescue 기준을 소폭 완화",
        "- /missed 에서 놓친 강한 움직임을 '봤는데 못 올림' 과 '움직였지만 늦음' 으로 나눠서 보이게 정리",
...
PREWATCH_MIN_FROM_LOW_PCT = 0.70
PREWATCH_MIN_3M_PCT = 0.18
PREWATCH_MIN_5M_PCT = 0.05
PREWATCH_MIN_SCORE = 6.00
...
def build_prewatch_candidates(cache, regime, exclude_tickers=None):
    ...
    for ticker, data in cache.items():
        try:
            ...
            continue
    preview.sort(key=lambda x: (safe_float(x.get("edge_score", 0)) + safe_float(x.get("leader_score", 0)) * 0.85), reverse=True)
    return preview[:4]
...
def is_user_alert_rescue_candidate(item: dict, section: str = "", regime: dict = None) -> bool:
    ...
    if section == "restart":
        if max(chg_5, chg_30, from_low) < USER_ALERT_RESCUE_MIN_RESTART_MOTION_PCT and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE:
            return False
        if chg_30 >= 0.15 and high_gap_pct >= 0.75 and from_low <= 2.80:
            return True
        return True
