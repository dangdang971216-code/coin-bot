...
MISSED_REVIEW_MARKET_LIMIT = 10
MISSED_REVIEW_SIGNAL_LIMIT = 12
MISSED_REVIEW_MIN_BEST_UP_PCT = 2.20
MISSED_REVIEW_MIN_END_UP_PCT = 0.60
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 사용자 알림은 해볼만한 후보만 보내고 늦음/감시 후보는 내부 기록만 남기게 조정",
        "- 재출발 후보도 너무 약하면 사용자 알림에서 숨기고 내부 검사용으로만 남기게 조정",
        "- 실시간 급등/거래량/거래대금 상위도 내부에서 같이 기록해 놓친 코인 검사용으로 보강",
        "- /missed 에서 왜 놓쳤는지 볼 때 급등/거래량 순위도 같이 보이게 정리",
        "- 놓친 좋은 코인 자동 요약은 매시간 정각마다 1번씩 보내주게 변경",
        "- 재시작하면 자동매수는 다시 꺼진 상태에서 시작",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def maybe_send_missed_review_alert(row: dict):
    ...
    lines = [
        ...
        f"- 시작흐름: 5분 {safe_float(row.get('chg_5m', 0), 0):+.2f}% / 30분 {safe_float(row.get('chg_30m', 0), 0):+.2f}% / 저점대비 {safe_float(row.get('from_30m_low_pct', 0), 0):+.2f}% / 최근고점까지 {safe_float(row.get('high_gap_pct', 999), 999):.2f}%",
...
def process_missed_review_queue(now_ts: float = None):
    ...
    for ticker, row in due:
        ...
        if ok_spot and qualifies_move and not was_caught:
...
def format_rank_line_for_missed_review(row: dict) -> str:
    ...
    return " / ".join(parts) if parts else "순위 정보 없음"
...
def scan_watchlist(shared_cache=None):
    ...
    immediate_live_entries = live_entries[:2]
    ...
    if early_entries:
        ...
        sent_any_alert = True
    if live_entries:
        send_watch_alert_bundle(get_watch_section_header("live", regime=regime), live_entries[:2], section_key="live")
        sent_any_alert = True
    if restart_entries:
