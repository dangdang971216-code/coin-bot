...
DEFAULT_CONTROL = {
    "running": False,
    "loop_seconds": 8,
    "max_open_strict": 8,  # v0.64: sweep_vwap_recovery 1차 검증 동시 OPEN 보호값
...
# =============================================================================
# paper_bot v0.67: retired strategy guard
# - sweep_vwap_recovery 신규 OPEN은 중지한다.
# - 기존 OPEN은 기존 청산 루프가 계속 관리한다.
# - OPEN/CLOSED/trade_log 장부는 삭제하지 않는다.
# =============================================================================
VERSION = "paper_bot_v0.67"
RETIRED_STRATEGY_KEY = "sweep_vwap_recovery"
RETIRED_STRATEGY_REASON = "메인봇 v2.13.299에서 sweep_vwap_recovery 폐기. 신규 OPEN 중지, 기존 장부 보존."

_v067_base_load_control = load_control

def load_control() -> Dict[str, Any]:  # type: ignore[override]
    control = _v067_base_load_control()
    changed = False
    if control.get("new_open_paused") is not True:
        control["new_open_paused"] = True
        changed = True
    control["retired_strategy_block"] = True
    control["retired_strategy_key"] = RETIRED_STRATEGY_KEY
    if changed:
        try:
            save_json(FILES["control"], control)
        except Exception:
...
_v067_base_pick_candidates = pick_candidates

def pick_candidates(control: Dict[str, Any], open_pos: Dict[str, Dict[str, Any]]) -> Tuple[List[Tuple[str, Dict[str, Any]]], Dict[str, int]]:  # type: ignore[override]
    # 폐기전략은 신규 OPEN을 만들지 않는다. 기존 OPEN은 monitor/close 쪽에서 정상 처리된다.
    stats = {
        "paper_file": 0,
        "shadow_file": 0,
        "events_file": 0,
        ...
        "micro_preopen_wait": 0,
        "new_open_paused": 1,
        "strategy_retired_block": 1,
        "retired_strategy_key": RETIRED_STRATEGY_KEY,
    }
    try:
        rows = read_jsonl(candidate_input_path("strict"), max_lines=int(float_any(control.get("candidate_read_max_lines"), default=800)))
        stats["paper_file"] = len(rows)
        stats["retired_strategy_blocked_rows"] = len(rows)
    except Exception:
        pass
    return [], stats

_v067_base_summary_text = summary_text

def summary_text() -> str:  # type: ignore[override]
    control = load_control()
    status = load_json(FILES["status"], {})
    open_pos = load_open()
    focus_open = _v064_focus_open_positions(open_pos) if "_v064_focus_open_positions" in globals() else {}
    focus_closed = _v064_focus_closed_rows() if "_v064_focus_closed_rows" in globals() else []
    fs = file_stats()
    pick = status.get("pick_stats") if isinstance(status.get("pick_stats"), dict) else {}
    st = _v064_stats(focus_closed) if "_v064_stats" in globals() else {"n":0,"win_rate":0.0,"total":0.0,"avg":0.0}
    return "\n".join([
        f"🧪 페이퍼봇 {VERSION}",
        f"- 실행상태: {'ON' if control.get('running') else 'OFF'} / loop {control.get('loop_seconds')}s",
        f"- 현재전략: {PAPER_FOCUS_STRATEGY_LABEL}",
        "- 신규 OPEN: 중지 / 전략 폐기 상태",
        f"- 폐기 이유: {RETIRED_STRATEGY_REASON}",
        f"- OPEN: 현재전략 {len(focus_open)} / 전체 {len(open_pos)} / 기존 OPEN은 정상 청산",
        f"- CLOSED: 현재전략 {st['n']} / 승률 {st['win_rate']:.1f}% / 합산 {st['total']:+.2f}% / 평균 {st['avg']:+.2f}%",
        "",
        "[후보]",
        f"- 읽음 {pick.get('paper_file',0)} / 폐기전략차단 {pick.get('retired_strategy_blocked_rows',0)} / 신규중지 {pick.get('new_open_paused',0)}",
        "",
        "[원칙]",
        "- 신규 OPEN 중지 / OPEN-CLOSED-trade_log 장부 보존 / 다음 전략은 설계서 확정 후 별도 투입",
        f"- 오류로그 {fs.get('error', {}).get('size', 0)} bytes / 자세히 /perror",
    ])
