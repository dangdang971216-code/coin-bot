...
def get_market_universe(force=False):
    ...
    for idx3, ticker in enumerate(batch, start=1):
        ...
        if elapsed >= UNIVERSE_BUILD_MAX_SEC:
            min_success = max(2, min(UNIVERSE_MIN_PARTIAL_ROWS, batch_size))
            min_tried = max(3, min(batch_size, UNIVERSE_MIN_PARTIAL_ROWS + 1))
...
def build_shared_market_cache(force=False):
    ...
    max_tries = max(CACHE_REFRESH_MAX_TRIES, batch_size) if (force or not base_cache) else CACHE_REFRESH_MAX_TRIES
    ...
    for idx4, row in enumerate(batch_rows, start=1):
        ...
        if refreshed_count >= CACHE_REFRESH_TARGET_SUCCESS and tried_count >= batch_size:
            break
        if elapsed >= CACHE_BUILD_MAX_SEC:
            append_runtime_event_log("cache_partial_break", f"{elapsed:.1f}초 제한으로 부분 종료", extra={"tried": tried_count, "success": refreshed_count, "batch_size": batch_size, "max_tries": max_tries, "force": force})
            break
    if base_cache:
        keep = {}
        keep_times = {}
