    ...
    try:
            ...
            for ticker, data in cache_dict.items():
                try:
                    ...
                    lite_score = (
                        ...
                        + max(change_3, 0) * 1.4
                        + max(change_1, 0) * 0.7
                        + max(vol_ratio - 1.0, 0) * 3.1
                        + min(turnover / 1000000000.0, 6.0)
                        + leader * 1.4
                    )
                    if change_5 <= 0:
                        lite_score -= 8.0
                    ...
                    if change_5 >= 0.20 and vol_ratio >= 1.20 and leader >= 0.50 and rsi < 82:
                        reasons = ["지금 후보로 볼 만함"]
                    elif not reasons:
        ...
        if pending_buy_candidates:
            ...
            for row in sorted_candidates:
                ...
                items.append({
                    ...
                    "lite_score": score,
                    "rsi": 50.0,
                    "reasons": ["지금은 기다릴 후보 기준", f"{strategy_label}", f"반복 포착 {seen_count}회"],
                })
            if items:
                return items, "최근 실시간 후보가 비어 있어 지금은 기다릴 후보 기준으로 보여줌"
        ...
        if recent_entries:
            ...
            for entry in recent_entries:
                ...
                reasons = [f"최근 알림 후보 기준", source_label]
                if reason_text:
            ...
            items.sort(key=lambda x: safe_float(x.get("lite_score", 0)), reverse=True)
            if items:
