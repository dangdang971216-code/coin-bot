...
DEFAULT_CONTROL = {
    ...
    "open_shadow_positions": False,  # v0.24 기본값: shadow는 OPEN하지 않고 복기 전용
    ...
    if isinstance(saved, dict):
        for k, v in saved.items():
            ...
            if k in DEFAULT_CONTROL:
                control[k] = v
    # v0.24 고정: candidate_events 소비 옵션은 저장돼 있어도 다시 만들지 않는다.
...
def pick_candidates(control: Dict[str, Any], open_pos: Dict[str, Dict[str, Any]]) -> Tuple[List[Tuple[str, Dict[str, Any]]], Dict[str, int]]:
    ...
    if open_shadow:
        ...
        # v0.24: shadow는 복기 전용. OPEN 장부에 태우지 않는다.
...
def summary_text() -> str:
    ...
    return "\n".join([
        ...
        f"- shadow 처리: 복기전용 / 신규 OPEN 안 함 / 필요시 open_shadow_positions=True",
        f"- 이번 cycle: open +{status.get('opened_this_cycle',0)} / close +{status.get('closed_this_cycle',0)} / {status.get('elapsed_sec','-')}s",
        f"- pick: paper {pick.get('paper_file',0)} / shadow복기 {pick.get('shadow_review_only_skip',0)} / dup {pick.get('dup_skip',0)} / stale {pick.get('stale_skip',0)} / same_ticker {pick.get('same_ticker_skip',0)} / bad {pick.get('bad_skip',0)}",
...
def handle_command(chat_id: str, text: str) -> None:
    ...
    if cmd in {"/phelp", "/start"}:
        send_chat(chat_id, "\n".join([
            ...
            "소비파일: paper_candidates.jsonl만 OPEN / shadow_candidates는 복기 전용",
