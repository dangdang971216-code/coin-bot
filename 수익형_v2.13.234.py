...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v234 빠른손절/지지부진 조기종료 성과반영 + scan_id 보강(자동매수/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
def _v233_exit_mode_infer(row: Dict[str, Any]) -> Dict[str, Any]:
    """v233: 실제 청산조건 변경 전, CLOSED를 청산모드 후보로 분류한다."""
    reason = _v233_reason_group(row)
    pnl = fnum(row.get("pnl_pct"), 0.0)
    peak = fnum(row.get("peak_pct"), 0.0)
    score = fnum(_v233_entry_value(row, "score", row.get("score")), 0.0)
    money3 = fnum(_v233_entry_value(row, "money_flow_3m", row.get("money_flow_3m")), 0.0)
    pull = fnum(_v233_entry_value(row, "pullback_quality_score", row.get("pullback_quality_score")), 0.0)
    spread = fnum(_v233_entry_value(row, "micro_spread_pct", row.get("micro_spread_pct")), 999.0)
    buy = fnum(_v233_entry_value(row, "micro_trade_buy_ratio_30", row.get("micro_trade_buy_ratio_30")), 0.0)
    wall = fnum(_v233_entry_value(row, "micro_bid_ask_wall_ratio", row.get("micro_bid_ask_wall_ratio")), 0.0)
    micro_fresh = _v233_entry_bool(row, "micro_fresh") or str(_v233_entry_value(row, "micro_row_status", row.get("micro_entry_status"))).lower() == "fresh"
    ws_fresh = _v233_entry_bool(row, "ws_fresh") or str(_v233_entry_value(row, "ws_row_status", row.get("ws_entry_status"))).lower() == "fresh"
    ...
    if reason in {"take_profit", "protect_stop_after_tp"} and len(strong_hits) >= 5:
    ...
    return {"mode": mode, "label": label, "strong_hits": strong_hits, "weak_hits": weak_hits, "peak": peak, "pnl": pnl, "age": age, "score": score, "money3": money3, "pull": pull, "spread": spread, "buy": buy, "wall": wall, "micro_fresh": micro_fresh, "ws_fresh": ws_fresh}
...
def _v233_exit_mode_block(cur_rows: List[Dict[str, Any]]) -> List[str]:
    ...
    eat = [r for r in rows if _v233_exit_mode_infer(r)["mode"] == "eat_more_watch"]
    quick = [r for r in rows if _v233_exit_mode_infer(r)["mode"] == "quick_cut_candidate"]
    slow = [r for r in rows if _v233_exit_mode_infer(r)["mode"] == "slow_cut_candidate"]
    out = ["", "[1-4] 청산모드 기준 후보(현재버전, 실제청산 변경 아님)"]
    out.append("- 기준: 진입 당시 강함 + 진입 후 반응. 지금은 시뮬/관찰용이며 실제 익절·손절값은 변경하지 않음.")
    ...
    out.append("- 다음 판단: 더먹기 후보가 반복되면 일부익절+추적, 빠른손절/지지부진 후보가 반복되면 조기이탈 조건을 검토")
    return out
...
def _v233_quality_text_from_snapshot(snap: Optional[Dict[str, Any]] = None) -> str:
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v234 기본은 캐시 전용: 현재 후보는 clean_candidate_snapshot.json 단일 원천만 표시",
        "- 최근 3시간은 버전 섞임 가능. 최대수익 제외 줄로 한 방 착시를 같이 봅니다.",
        "- 긴 3/6/12 상세와 원자료성 비교는 /quality_full",
        "",
        "[1/5] 성과 요약",
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    if full:
        return _v209_direct_candidate_quality_text(True)
    txt = _read_cached_text(FILES["quality_summary"], "/quality")
    return (txt.replace("v231 기본은 캐시 전용", "v234 기본은 캐시 전용")
               .replace("v230 기본은 캐시 전용", "v234 기본은 캐시 전용")
               .replace("v222 기본은 캐시 전용", "v234 기본은 캐시 전용"))



# ===============================
# v2.13.234: paper 빠른손절/지지부진 조기종료 결과 반영
# - paper_bot_v0.47의 quick_stop / slow_early_exit 종료사유를 메인 성과판에서 한글 표시한다.
