...
WATCH_VOL_IMPROVE_DELTA = 0.24
WATCH_CHANGE_IMPROVE_DELTA = 0.20
WATCH_SCORE_IMPROVE_DELTA = 0.38
...
def format_pre_alert_snapshot_line(item: dict) -> str:
    ...
    return (
        ...
        f"최근고점과 거리 -{snap.get('below_30m_high_pct', 0):.2f}%"
    )
...
def init_price_track(ref: dict):
    ...
    track = ref["track"]
    track.setdefault("max_price", safe_float(ref.get("alert_price", 0)))
    track.setdefault("min_price", safe_float(ref.get("alert_price", 0)))
    track.setdefault("max_up_pct", 0.0)
    track.setdefault("max_down_pct", 0.0)
    track.setdefault("max_ts", safe_float(ref.get("alert_ts", 0)))
    track.setdefault("min_ts", safe_float(ref.get("alert_ts", 0)))
    track.setdefault("last_price", safe_float(ref.get("alert_price", 0)))
    track.setdefault("last_ts", safe_float(ref.get("alert_ts", 0)))
...
def build_price_check_lines(ref: dict, current_price: float, now_ts: float, target_min: int = 0):
    ...
    if snap:
        lines.append(
            ...
            f"최근고점까지 {safe_float(snap.get('below_30m_high_pct', 0)):.2f}% 남음"
        )
    lines.append("")
    ...
    max_price = safe_float(track.get("max_price", 0))
    min_price = safe_float(track.get("min_price", 0))
    if max_price > 0:
        lines.append(f"- 중간 최고가: {fmt_price(max_price)} ({safe_float(track.get('max_up_pct', 0)):+.2f}%) / {format_offset_text(alert_ts, safe_float(track.get('max_ts', 0)))}")
    if min_price > 0:
        lines.append(f"- 중간 최저가: {fmt_price(min_price)} ({safe_float(track.get('max_down_pct', 0)):+.2f}%) / {format_offset_text(alert_ts, safe_float(track.get('min_ts', 0)))}")
    ...
    lines.extend([
        ...
        "",
        get_price_check_note(ref, current_price),
    ])
    return lines
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 알림 전 흐름과 알림 후 30분 결과를 같이 기록",
        "- 30분 결과를 처음 알림에 답장으로 붙임",
        "- 문구를 더 직설적으로 바꿈",
        "- B급 초반형은 이전보다 더 보수적으로 거름",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
