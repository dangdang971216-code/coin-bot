...
WATCH_MIN_EFFECTIVE_30M_MOVE_PCT = 0.12
WATCH_MIN_RECOVERY_FROM_LOW_PCT = 0.15
WATCH_MIN_RANGE_PCT = 0.12
WATCH_HIGH_VOL_MIN = 4.20
WATCH_HIGH_VOL_MIN_MOVE_PCT = 0.12
RECOVERY_ENTRY_MIN_30M_PCT = 0.05
RECOVERY_ENTRY_MIN_FROM_LOW_PCT = 0.15
RECOVERY_ENTRY_MIN_VOL_RATIO = 1.35
RECOVERY_ENTRY_MIN_LEADER_SCORE = 0.45
RECOVERY_ENTRY_MIN_HIGH_GAP_PCT = 0.55
RECOVERY_ENTRY_MIN_5M_PCT = -0.10
FAST_ENTRY_MIN_5M_PCT = 0.12
FAST_ENTRY_MIN_30M_PCT = 0.25
FAST_ENTRY_MIN_FROM_LOW_PCT = 0.25
FAST_ENTRY_MIN_VOL_RATIO = 1.45
FAST_ENTRY_MIN_LEADER_SCORE = 0.45
FAST_ENTRY_MIN_HIGH_GAP_PCT = 0.55
...
def has_meaningful_watch_motion(item: dict) -> bool:
    ...
    if vol_ratio >= 2.20 and (chg_5 >= 0.05 or chg_30 >= 0.10 or from_low >= 0.10):
        return True
    if leader_score >= 1.20 and vol_ratio >= 1.40 and (chg_5 >= 0.00 or chg_30 >= 0.08 or from_low >= 0.08):
        return True
...
def is_low_zone_recovery_candidate(item: dict, judgement: str = "", caution: str = "") -> bool:
    ...
    if strategy == "LEADER_WATCH":
        return (
            ...
            and (
                (chg_30 >= 0.05 and vol_ratio >= 1.45)
                or (chg_5 >= -0.02 and vol_ratio >= 1.35)
                or (leader_score >= 1.10 and vol_ratio >= 1.25)
            )
        )
...
def get_watch_section_key(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> str:
    ...
    if section == "entry" and trade_tier == "B":
        ...
        if high_gap_pct <= 0.45:
            section = "late"
        elif chg_5 < -0.12 and chg_30 < 0.18:
            section = "recovery" if recovery_like else "watch"
        elif leader_score < 0.70 and vol_ratio < 1.45:
            section = "recovery" if recovery_like else "watch"
        elif vol_ratio < 1.25 and not recovery_like:
            section = "watch"
        elif from_low < 0.15 and chg_30 < 0.30 and chg_5 < 0.08:
            section = "recovery"
        elif strategy == "LEADER_WATCH" and (vol_ratio < 1.55 or high_gap_pct < 0.55):
            section = "recovery" if recovery_like else "watch"

    if section == "entry" and strategy == "LEADER_WATCH":
        if chg_5 < 0.08 and chg_30 < 0.30:
            section = "recovery" if is_low_zone_recovery_candidate(item, judgement, caution) else "watch"
