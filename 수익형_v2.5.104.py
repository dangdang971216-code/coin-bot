...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 차트 판단용 핵심 수치를 더 늘려서 몸통/윗꼬리/눌림회복/재돌파/유지까지 같이 기록",
        "- 최근30분·60분 고점/저점 raw 값을 알림/덤프/역추적에서 같이 보이게 정리",
        "- 차트 숫자를 앱 차트와 바로 대조하기 쉽게 문구를 최근30분/60분 기준으로 명확히 정리",
        "- 차트 구조 수치화와 재돌파/2차 파동 점수를 알림 판단에 연결",
        "- 오래된 중복 변경점은 줄이고 최근 핵심만 짧게 표시",
        "",
    ]
...
def summarize_missed_review_logs(rows):
    bucket = {"아예 못 잡음": 0, "봤는데 못 올림": 0, "움직였지만 늦음": 0, "판단 빗나감": 0}
    for row in rows:
        if not isinstance(row, dict):
            continue
        miss_type = str(row.get("miss_type", "") or "")
...
def compare_command(update, context: CallbackContext):
    alert_rows = collect_recent_alert_review_rows(limit=5, summary_limit=20)
    missed_rows = [x for x in missed_review_logs if isinstance(x, dict)]
