...
MISSED_REVIEW_AUTO_ALERT_ON = True
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 실시간 강세 후보 5분/주도도 기준을 더 올려 TREE류 약한 live를 덜 뜨게 조정",
        "- 30분 흐름이 약한데 저점에서 이미 뜬 live 후보는 재출발/감시로 내리게 조정",
        "- 재출발 후보는 기존 강화 유지, 늦은 후보 차단은 그대로 유지",
...
def load_missed_review_state():
    global missed_review_queue, missed_review_logs
...
def merge_missed_review_row(base: dict, extra: dict):
    ...
    if not isinstance(extra, dict):
        return base
    for key in ["seen_market", "seen_shortlist", "seen_signal", "alert_sent", "higher_low_hint"]:
...
def register_missed_review_market_candidates(cache: dict, now_ts: float = None):
    if not MISSED_REVIEW_ON:
        return
    now_ts = safe_float(now_ts, time.time())
    for item in build_missed_review_market_candidates(cache, limit=MISSED_REVIEW_MARKET_LIMIT):
