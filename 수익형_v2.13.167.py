...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.167: 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.167 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_paper_handoff(update, context) -> None:
    lines = [
        ...
        "- 기대 paper_bot: paper_bot_v0.27.py / paper_bot_v0.21",
        "",
        "역할",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.167.py", "- 페이퍼봇 대상: paper_bot_v0.27.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.167: errorlog 수정 + trade_ready 분리 유지", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.167 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def tail_text(path: Path, n: int = 80) -> str:
    try:
        if not path.exists():
            return "최근 오류 없음"
        lines = path.read_text(encoding="utf-8", errors="ignore").splitlines()[-int(n):]
