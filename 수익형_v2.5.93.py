...
def get_shortlist_user_rescue_display_key(item: dict, display_key: str = "", regime: dict = None) -> str:
    ...
    if shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG and high_gap_pct >= 0.72 and motion >= 0.08 and chart_rise <= 4.20:
        if vol_ratio >= 0.95 or leader >= 0.38 or edge >= 4.45:
            return "restart"
    if chg_5 >= 0.34 and high_gap_pct >= 0.78 and (chg_30 >= -0.02 or vol_ratio >= 1.55 or leader >= 0.82 or hot_score >= USER_ALERT_RESCUE_STRONG_HOT_SCORE):
        return "live"
...
def get_user_alert_block_reason(item: dict, display_key: str = "") -> str:
    ...
    if strategy in {"PREWATCH", "NEWLOW_RECOVER"} and USER_ALERT_PREWATCH_STRICT_ON:
        prewatch_rescue_ok = (
            ...
            and high_gap_pct >= 0.90
            and max(chg_5, chg_30, from_low) >= 0.12
            and chart_rise <= 3.60
            and (vol_ratio >= 1.00 or leader >= 0.42 or edge >= 4.55)
        )
