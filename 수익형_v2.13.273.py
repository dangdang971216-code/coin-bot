...
# v260: command registration uses the base install_commands/_builder_for_command single mapping.
...
VWAP_REVERSION_WATCH_PER_SCAN = max(12, STRATEGY_WATCH_TOP_PER_KIND * 3)  # v262: 평균회귀 watch 전용 상한. 구 VWAP_REVERSION_WATCH_PER_SCAN 참조 제거.
...
def _v241_select_watch_candidates(rows: List[Dict[str, Any]], strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    return [ev for _s, ev in events[:VWAP_REVERSION_WATCH_PER_SCAN]]
...
def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    ...
    try:
        # v259: v252 평균회귀 전용 공장이 구 build_candidates wrapper를 삭제하면서
        # strategy_watch_update 호출도 같이 끊겼다. 공장 내부의 단일 위치에서만 관찰기록을 저장한다.
        _v241_strategy_watch_update(rows, strict, shadow)
    except Exception as exc:
        log_error("v259_strategy_watch_update_from_factory", exc)
    return strict, shadow, rejects, examples
...
# v259: _v252_prune_map_text 지도 함수는 코드에서 제거하고 인수인계서/지도 파일로 이동.
...
def _v253_strategy_watch_paper_latest_events(scan_id: str, existing: Optional[set] = None) -> List[Dict[str, Any]]:
    ...
    events.sort(key=lambda x: (x[0], fnum(x[1].get("money_flow_3m"), 0), fnum(x[1].get("current_return_pct"), 0)), reverse=True)
    return [ev for _s, ev in events[:max(8, VWAP_REVERSION_WATCH_PER_SCAN)]]
...
def _v253_select_watch_candidates(rows: List[Dict[str, Any]], strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    events.sort(key=lambda x: (x[0], fnum(x[1].get("money_flow_3m"), 0), fnum(x[1].get("current_return_pct"), 0)), reverse=True)
    return [ev for _s, ev in events[:max(VWAP_REVERSION_WATCH_PER_SCAN, 12)]]
...
def _v241_strategy_watch_update(rows: List[Dict[str, Any]], strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> None:  # type: ignore[override]
    ...
    try:
        ...
        if nowv - fnum(_V243_SW_RUNTIME.get("last_update_ts"), 0.0) < STRATEGY_WATCH_UPDATE_GAP_SEC:
            ...
            return
        cache = _v241_watch_load()
        cache = _v259_vwap_watch_cache_only(cache)
        _v241_update_existing_watch(cache, rows)
        ...
        log_error("v262_strategy_watch_update", exc)

# v259: _v253_prune_map_text 지도 함수는 코드에서 제거하고 인수인계서/지도 파일로 이동.
...
def _v251_build_score_text_for_cache() -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        "- 과거 돈흐름/눌림/구전략 장부는 보관하지만 현재 판단에서는 제외합니다.",
        "- TP분할 없음: 소액 실전 전제라 전량익절/전량손절 기준으로 봅니다.",
        "- v263: 평균회귀 CLOSED 원인분석은 캐시 전용이며, 근접탈락 2차 재확인 큐를 추가했습니다.",
        "",
        "[1/5] 평균회귀 전략 성과",
        _compact_stat_line("평균가 회귀 반등 정식", closed),
        "",
...
def _v250_quality_text() -> str:  # type: ignore[override]
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- 기준: 평균가 회귀 반등 단타 단일검증 전략 누적만 현재 판단에 사용합니다.",
        "- v263: 이전 평균회귀 CLOSED까지 포함하고, 근접탈락 2차 재확인 큐를 함께 봅니다.",
        "- 과거 돈흐름/눌림/구전략 장부는 보관하지만 현재 판단에서는 제외합니다.",
        f"- 참고 snapshot: {len((snap or {}).get('rows') or []) if isinstance(snap, dict) else 0}개 / {max(0.0, now_ts() - fnum((snap or {}).get('updated_ts'), now_ts())) if isinstance(snap, dict) else -1:.1f}초 전",
        "",
        "[1/7] 평균회귀 전략 성과",
...
# v260: 손실 원인분석에서 "진입 후 반응없음/1분 흐름 음수"가 반복되어,
# 평균회귀 후보와 정식 OPEN을 더 명확히 분리한다.
VWAP_REV_CONFIRM_WEAK_SIGNAL_MIN = int(os.getenv("CLEAN_VWAP_REV_CONFIRM_WEAK_SIGNAL_MIN", "2"))
VWAP_REV_CONFIRM_STRONG_CHANGE_1 = float(os.getenv("CLEAN_VWAP_REV_CONFIRM_STRONG_CHANGE_1", "0.07"))
VWAP_REV_CONFIRM_STRONG_RECOVERY = float(os.getenv("CLEAN_VWAP_REV_CONFIRM_STRONG_RECOVERY", "0.10"))
VWAP_REV_CONFIRM_STRONG_AVG_TURN = float(os.getenv("CLEAN_VWAP_REV_CONFIRM_STRONG_AVG_TURN", "0.03"))
VWAP_REV_CONFIRM_FROM_LOW_WARN = float(os.getenv("CLEAN_VWAP_REV_CONFIRM_FROM_LOW_WARN", "2.40"))
...
def _v258_turn_confirm_reasons(item: Dict[str, Any]) -> List[str]:
    ...
    v260 기준:
    - VWAP/매수비/스프레드 수치 자체는 건드리지 않는다.
    - 과거 손실 원인에서 반복된 '진입 후 반응없음'과 '1분 흐름 음수'만 정식 OPEN 전에 거른다.
    - 후보는 남기되 trade_ready만 보류해서 복기/관찰은 계속 가능하게 한다.
    """
    if not _v250_is_focus_item(item):
        return []
    reasons: List[str] = []
    ...
    # 1분 흐름 음수는 과거 손실 원인 2위라 정식 OPEN에서 제외한다.
    if ch1 < VWAP_REV_CONFIRM_MIN_CHANGE_1:
        reasons.append(f"1분 흐름 음수 {ch1:+.2f}%")

    weak: List[str] = []
    if ch1 >= max(0.02, VWAP_REV_CONFIRM_MIN_CHANGE_1):
        weak.append(f"1분 {ch1:+.2f}%")
    if avg_turn > VWAP_REV_CONFIRM_MIN_AVG_TURN:
        weak.append(f"평균가턴 {avg_turn:+.2f}%")
    if recovery >= VWAP_REV_CONFIRM_MIN_RECOVERY:
        weak.append(f"회복 {recovery:+.2f}%")
    if lower_wick >= 0.12:
        weak.append(f"아래꼬리 {lower_wick:.2f}%")

    strong = (
        ch1 >= VWAP_REV_CONFIRM_STRONG_CHANGE_1
        or recovery >= VWAP_REV_CONFIRM_STRONG_RECOVERY
        or avg_turn >= VWAP_REV_CONFIRM_STRONG_AVG_TURN
    )

    # 약한 신호 1개만으로는 '돌아섬'이 아니라 바닥 가능성으로 보고 관찰로 보낸다.
    if not strong and len(weak) < max(1, VWAP_REV_CONFIRM_WEAK_SIGNAL_MIN):
        reasons.append(
            f"돌아섬 확인 부족: 신호 {len(weak)}/{VWAP_REV_CONFIRM_WEAK_SIGNAL_MIN} "
            f"/ 1분 {ch1:+.2f}% / 평균가턴 {avg_turn:+.2f}% / 회복 {recovery:+.2f}%"
        )

    # 이미 저점대비 많이 튄 상태에서 강한 재상승이 없으면 평균회귀 초입이 아니라 늦은 반등일 수 있다.
    if from_low >= VWAP_REV_CONFIRM_FROM_LOW_WARN and not strong:
        reasons.append(f"저점대비 이미 반등 {from_low:.2f}% / 강한 돌아섬 미확인")

    return list(dict.fromkeys(str(x) for x in reasons))[:5]
...
def _v258_reversion_review_text_for_cache() -> str:
    ...
    lines = [
        ...
        "- v263: 기본 명령은 캐시 전용입니다. 긴 직접계산은 /reversion_review_full",
    ]
...
# v259: _v258_map_text 지도 함수는 코드에서 제거하고 인수인계서/지도 파일로 이동.


# ===============================
# v2.13.259: 구조정리판
# - 명령어 등록 경로를 base install_commands/_builder_for_command 단일 mapping으로 정리하고 v241 후행 override를 삭제했다.
# - v252 평균회귀 전용 공장에서 끊겼던 strategy_watch_update를 공장 내부 단일 위치로 재연결했다.
# - strategy_watch cache의 구 돈흐름/다전략 active 찌꺼기를 저장 전에 제거해 평균회귀 관찰이벤트가 막히지 않게 한다.
# - /errorlog 기본 출력에서 과거 종료 중 오류와 v255 명령등록 오류를 현재 오류처럼 노출하지 않게 분리한다.
# - 전략 조건/청산조건/자동매수/BUY_READY/paper 장부는 변경하지 않는다.
# ===============================


def _v259_vwap_watch_cache_only(cache: Dict[str, Any]) -> Dict[str, Any]:
    """strategy_watch 캐시는 이제 평균회귀 전용으로만 유지한다.

    과거 돈흐름/다전략 active가 남아 있으면 _v241_add_new_watch()의 active 상한에 걸려
    평균회귀 이벤트가 0개로 보일 수 있어, 저장 전 단일 지점에서 제거한다.
    """
    if not isinstance(cache, dict):
        return {"active": {}, "recent_done": [], "last_pairs": {}}
    active = cache.get("active") if isinstance(cache.get("active"), dict) else {}
    recent = cache.get("recent_done") if isinstance(cache.get("recent_done"), list) else []
    last_pairs = cache.get("last_pairs") if isinstance(cache.get("last_pairs"), dict) else {}

    new_active = {k: v for k, v in active.items() if isinstance(v, dict) and str(v.get("watch_kind") or "") == VWAP_REVERSION_WATCH_KIND}
    new_recent = [v for v in recent if isinstance(v, dict) and str(v.get("watch_kind") or "") == VWAP_REVERSION_WATCH_KIND]
    new_pairs = {k: v for k, v in last_pairs.items() if str(k).startswith(f"{VWAP_REVERSION_WATCH_KIND}:")}

    removed = max(0, len(active) - len(new_active)) + max(0, len(recent) - len(new_recent))
    if removed:
        with _state_lock:
            STATE["v259_watch_pruned_non_vwap"] = int(STATE.get("v259_watch_pruned_non_vwap", 0) or 0) + removed
            STATE["strategy_watch_note"] = f"v259 평균회귀 전용 watch cache 정리: 구 이벤트 {removed}개 제거"
    cache["active"] = new_active
    # v261: 삭제된 구 완료보관 상수에 기대지 않는다.
    # 기존 _v241_update_existing_watch()의 완료보관 상한(500개)과 같은 기준으로 평균회귀 완료분만 보관한다.
    cache["recent_done"] = new_recent[-500:]
    cache["last_pairs"] = new_pairs
    cache["mode"] = "vwap_reversion_only_v261"
    return cache

# v259: _v259_map_text 지도 함수는 코드에서 제거하고 인수인계서/지도 파일로 이동.


# ===============================
# v2.13.261: strategy_watch NameError 긴급 구조정리
# - v259에서 삭제된 구 완료보관 상수를 watch cache 정리 함수가 계속 참조하던 문제를 제거한다.
# - 새 fallback 상수를 덧붙이지 않고 기존 완료보관 정책(최근 500개)으로 직접 정리한다.
# - 평균회귀 조건/청산조건/자동매수/BUY_READY/paper 장부는 변경하지 않는다.
# ===============================


# ===============================
# v2.13.263: 복기용 재확인 루트 + 기본 명령어 캐시 고착 완화
# - /score, /quality, /reversion_review가 mtime 변경 때마다 빈 화면처럼 보이지 않게 마지막 정상 캐시를 우선 표시한다.
# - 구 직접계산 fallback은 기본 명령어에 되살리지 않는다. 낡은 캐시는 표시만 하고 백그라운드 갱신을 기다린다.
# - 평균회귀 근접탈락/정보부족/스프레드/매수세 약함 후보를 재확인 큐에 기록한다.
# - 재확인 큐는 복기/관찰용이며, 자동매수·BUY_READY·청산조건·평균회귀 조건값은 변경하지 않는다.
# ===============================

FILES.setdefault("vwap_recheck", BASE_DIR / "clean_vwap_recheck_queue.json")
VWAP_RECHECK_MIN_AGE_SEC = float(os.getenv("CLEAN_VWAP_RECHECK_MIN_AGE_SEC", "30"))
VWAP_RECHECK_MAX_AGE_SEC = float(os.getenv("CLEAN_VWAP_RECHECK_MAX_AGE_SEC", "900"))
VWAP_RECHECK_KEEP_DONE = int(os.getenv("CLEAN_VWAP_RECHECK_KEEP_DONE", "160"))
VWAP_RECHECK_ACTIVE_MAX = int(os.getenv("CLEAN_VWAP_RECHECK_ACTIVE_MAX", "90"))
VWAP_RECHECK_URGENT_MAX = int(os.getenv("CLEAN_VWAP_RECHECK_URGENT_MAX", "40"))


def _v263_cache_display_text(title: str, name: str, obj: Any, why: str = "") -> str:
    """기본 명령어는 직접계산을 되살리지 않고, 마지막 정상 캐시를 보여준다.

    v251 이후 캐시 전용화는 유지한다. 다만 strategy_watch/paper_latest mtime이 scan마다 바뀌면
    사용자는 계속 '캐시 갱신중'만 보게 되므로, 이전 캐시 본문을 참고용으로 보여주고 하단에
    갱신중 사유만 붙인다.
    """
    text = str((obj or {}).get("text") or "") if isinstance(obj, dict) else ""
    if text.strip():
        cached_ver = str((obj or {}).get("version") or "-")
        cached_at = str((obj or {}).get("updated_text") or "-")
        lines = [text.rstrip(), "", "[캐시 상태]"]
        lines.append(f"⚠️ 마지막 정상 캐시 표시 중 / 새 기준 갱신 대기")
        lines.append(f"- 현재버전: {BOT_VERSION} / 캐시버전: {cached_ver} / 캐시시각: {cached_at}")
        if why:
            lines.append(f"- 갱신 사유: {why}")
        lines.append("- 기본 명령어에서는 구 직접계산 경로를 타지 않습니다.")
        lines.append("- 백그라운드 요약 직원이 새 캐시를 저장하면 자동으로 최신화됩니다.")
        return "\n".join(lines)
    return _v251_wait_text(title, name, why or "캐시 없음")
...
def score_text() -> str:  # type: ignore[override]
    ...
    if ok:
        return str(obj.get("text") or "")
    return _v263_cache_display_text("📊 모의매매 성과 /score", "/score", obj, why)
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    if ok:
        return str(obj.get("text") or "")
    return _v263_cache_display_text("🔬 후보품질 요약 /quality", "/quality", obj, why)


_v263_direct_reversion_review_text = _v258_direct_reversion_review_text if "_v258_direct_reversion_review_text" in globals() else reversion_review_text


def reversion_review_text(full: bool = False) -> str:  # type: ignore[override]
    if full:
        return _v263_direct_reversion_review_text(True)
    obj = load_json(FILES["reversion_review_summary"], {})
    ok, why = _v251_cache_valid(obj, name="reversion_review")
    if ok:
        return str(obj.get("text") or "")
    return _v263_cache_display_text("🧯 평균회귀 원인분석 /reversion_review", "/reversion_review", obj, why)


def _v263_recheck_ticker(row: Dict[str, Any]) -> str:
    return _ticker_from_any((row or {}).get("ticker") or (row or {}).get("market") or (row or {}).get("symbol"))


def _v263_recheck_price(row: Dict[str, Any]) -> float:
    return fnum((row or {}).get("current_price") or (row or {}).get("price") or (row or {}).get("entry_price") or (row or {}).get("detected_price"), 0.0)


def _v263_row_text(row: Dict[str, Any]) -> str:
    parts: List[str] = []
    for key in ("trade_ready_label", "final_entry_label", "quality_label", "mainline_disabled_reason", "source_reject_bucket", "source_reject_detail", "one_liner"):
        v = (row or {}).get(key)
        if v not in (None, ""):
            parts.append(str(v))
    for key in ("trade_ready_reasons", "final_entry_reasons", "vwap_reversion_blocks", "vwap_reversion_reasons", "block_reasons"):
        vals = (row or {}).get(key)
        if isinstance(vals, list):
            parts.extend(str(x) for x in vals[:6] if x not in (None, ""))
    return " / ".join(parts)[:800]


def _v263_recheck_category(row: Dict[str, Any]) -> Tuple[str, str]:
    txt = _v263_row_text(row)
    spread = _v250_micro_spread(row)
    buy = _v250_micro_buy_ratio(row)
    ws_status = str(_ctxv(row, "ws_row_status", (row or {}).get("ws_row_status") or "missing"))
    micro_status = str(_ctxv(row, "micro_row_status", (row or {}).get("micro_row_status") or "missing"))
    ws_fresh = bool(_ctxv(row, "ws_fresh", (row or {}).get("ws_fresh")))
    micro_fresh = bool(_ctxv(row, "micro_fresh", (row or {}).get("micro_fresh")))

    if "과열" in txt or "펌핑" in txt or "고점 바로" in txt or "추격" in txt:
        return "pump_chase", "과열/추격 위험은 재확인으로 바로 살리지 않음"
    if "정보" in txt or "신선" in txt or (not ws_fresh and ws_status in {"missing", "stale", ""}) or (not micro_fresh and micro_status in {"missing", "stale", ""}):
        return "freshness", "정보 신선도 부족 → urgent 수집 후 재평가"
    if spread < 900 and spread > VWAP_REV_MAX_SPREAD:
        return "spread", f"스프레드 {spread:.2f}% > {VWAP_REV_MAX_SPREAD:.2f}% → 정상화 대기"
    if "매수비" in txt or "매수세" in txt or "매도 우세" in txt or "매도체결" in txt or (buy > 0 and buy < VWAP_REV_MIN_BUY_RATIO):
        return "buy_weak", f"매수세 약함 {buy:.2f} < {VWAP_REV_MIN_BUY_RATIO:.2f} → 가격버팀+매수비 회복 확인"
    if "돌아섬" in txt or "밀림" in txt or "1분 흐름" in txt or str((row or {}).get("final_entry_action") or "") == "recheck_wait":
        return "turn_wait", "돌아섬/가격버팀 재확인"
    if "점수" in txt or "구조" in txt or "VWAP" in txt or "회귀권" in txt:
        return "structure", "구조 근접탈락 → 이후 실제 상승 여부 복기"
    return "other", "기타 근접탈락 복기"


def _v263_trackable_recheck(row: Dict[str, Any]) -> bool:
    if not isinstance(row, dict) or not _v250_is_focus_item(row):
        return False
    if bool(row.get("paper_bot_open") or row.get("open_eligible") or row.get("trade_ready")):
        return False
    cat, _ = _v263_recheck_category(row)
    if cat == "pump_chase":
        return False
    score = fnum(row.get("vwap_reversion_score"), fnum(row.get("score"), 0.0))
    money3 = _v250_money3(row)
    return bool(cat in {"freshness", "spread", "buy_weak", "turn_wait", "structure"} and (score >= 1.8 or money3 >= VWAP_REV_MIN_3M_KRW * 0.6))


def _v263_snapshot_recheck(row: Dict[str, Any], category: str, detail: str) -> Dict[str, Any]:
    ts = now_ts()
    price = _v263_recheck_price(row)
    t = _v263_recheck_ticker(row)
    return {
        "ticker": t,
        "category": category,
        "detail": detail,
        ...
        "last_return_pct": 0.0,
        "last_seen_at": ts,
        "last_seen_text": now_text(ts),
        "highest_return_pct": 0.0,
        "lowest_return_pct": 0.0,
        "score": fnum(row.get("score"), 0.0),
        "vwap_reversion_score": fnum(row.get("vwap_reversion_score"), 0.0),
        "vwap_gap_pct": _v250_vwap_gap(row),
        "money_flow_3m": _v250_money3(row),
        "micro_spread_pct": _v250_micro_spread(row),
        "micro_trade_buy_ratio_30": _v250_micro_buy_ratio(row),
        "micro_fresh": bool(_ctxv(row, "micro_fresh", row.get("micro_fresh"))),
        "ws_fresh": bool(_ctxv(row, "ws_fresh", row.get("ws_fresh"))),
        "change_1": fnum(row.get("change_1"), 0.0),
        "change_3": fnum(row.get("change_3"), 0.0),
        "final_entry_action": str(row.get("final_entry_action") or ""),
        "final_entry_label": str(row.get("final_entry_label") or row.get("trade_ready_label") or ""),
        "source_reason_text": _v263_row_text(row),
        "status": "대기",
        "decision": "recheck_wait",
        "note": "v263 복기용 2차 재확인 큐. 조건완화/자동매수/BUY_READY 아님.",
    }


def _v263_eval_active_recheck(item: Dict[str, Any], cur: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    nowv = now_ts()
    out = dict(item or {})
    age = nowv - fnum(out.get("created_at"), nowv)
    if not isinstance(cur, dict):
        out["status"] = "대상대기" if age < VWAP_RECHECK_MAX_AGE_SEC else "정보부족종료"
        out["decision"] = "keep" if age < VWAP_RECHECK_MAX_AGE_SEC else "done"
        return out
    price0 = fnum(out.get("first_price"), 0.0)
    price = _v263_recheck_price(cur)
    ret = ((price - price0) / price0 * 100.0) if price0 > 0 and price > 0 else fnum(out.get("last_return_pct"), 0.0)
    out["last_price"] = price if price > 0 else out.get("last_price", 0.0)
    out["last_return_pct"] = round(ret, 3)
    out["highest_return_pct"] = round(max(fnum(out.get("highest_return_pct"), ret), ret), 3)
    out["lowest_return_pct"] = round(min(fnum(out.get("lowest_return_pct"), ret), ret), 3)
    out["last_seen_at"] = nowv
    out["last_seen_text"] = now_text(nowv)
    out["micro_spread_pct_now"] = _v250_micro_spread(cur)
    out["micro_trade_buy_ratio_30_now"] = _v250_micro_buy_ratio(cur)
    out["micro_fresh_now"] = bool(_ctxv(cur, "micro_fresh", cur.get("micro_fresh")))
    out["ws_fresh_now"] = bool(_ctxv(cur, "ws_fresh", cur.get("ws_fresh")))
    out["change_1_now"] = fnum(cur.get("change_1"), 0.0)
    out["change_3_now"] = fnum(cur.get("change_3"), 0.0)
    out["vwap_gap_pct_now"] = _v250_vwap_gap(cur)

    if age < VWAP_RECHECK_MIN_AGE_SEC:
        out["status"] = "대기"
        out["decision"] = "keep"
        return out

    cat = str(out.get("category") or "other")
    spread_ok = fnum(out.get("micro_spread_pct_now"), 999.0) <= VWAP_REV_MAX_SPREAD
    buy_ok = fnum(out.get("micro_trade_buy_ratio_30_now"), 0.0) >= VWAP_REV_MIN_BUY_RATIO
    fresh_ok = bool(out.get("micro_fresh_now") or out.get("ws_fresh_now"))
    turn_ok = fnum(out.get("change_1_now"), 0.0) >= 0.0 and ret >= -0.15

    if ret >= 1.20:
        out["status"] = "놓친후보"
        out["decision"] = "done"
        out["review_result"] = "재확인 대상이었고 이후 +1.2% 이상 도달"
    elif ret <= -0.70:
        out["status"] = "버린게맞음"
        out["decision"] = "done"
        out["review_result"] = "재확인 중 -0.7% 먼저 이탈"
    elif cat == "freshness" and fresh_ok and turn_ok:
        out["status"] = "살릴만함"
        out["decision"] = "done"
        out["review_result"] = "정보 fresh 회복 + 가격 방어"
    elif cat == "spread" and spread_ok and turn_ok:
        out["status"] = "살릴만함"
        out["decision"] = "done"
        out["review_result"] = "스프레드 정상화 + 가격 방어"
    elif cat == "buy_weak" and buy_ok and turn_ok:
        out["status"] = "살릴만함"
        out["decision"] = "done"
        out["review_result"] = "매수비 회복 + 가격 방어"
    elif cat == "turn_wait" and turn_ok and ret >= 0.0:
        out["status"] = "살릴만함"
        out["decision"] = "done"
        out["review_result"] = "돌아섬/가격 방어 확인"
    elif age >= VWAP_RECHECK_MAX_AGE_SEC:
        out["status"] = "버린게맞음" if ret < 0 else "판단보류종료"
        out["decision"] = "done"
        out["review_result"] = "재확인 제한시간 종료"
    else:
        out["status"] = "계속확인"
        out["decision"] = "keep"
    return out
...
def _v263_load_recheck_queue() -> Dict[str, Any]:
    obj = load_json(FILES["vwap_recheck"], {})
    if not isinstance(obj, dict):
        obj = {}
    obj.setdefault("active", {})
    obj.setdefault("recent_done", [])
...
def _v263_save_recheck_queue(obj: Dict[str, Any]) -> None:
    obj["version"] = BOT_VERSION
    obj["updated_at"] = now_ts()
    obj["updated_text"] = now_text()
    obj["mode"] = "v263_vwap_reversion_recheck_review_only"
    save_json(FILES["vwap_recheck"], obj)


def _v263_write_micro_urgent_from_recheck(active: Dict[str, Dict[str, Any]]) -> None:
    try:
        rows = list(active.values())
        rows.sort(key=lambda r: (
            0 if str(r.get("category")) in {"freshness", "spread", "buy_weak"} else 1,
            -fnum(r.get("score"), 0.0),
            fnum(r.get("created_at"), 0.0),
        ))
        targets: List[str] = []
        seen: set[str] = set()
        for r in rows:
            t = _ticker_from_any(r.get("ticker"))
            if t and t not in seen:
                seen.add(t)
                targets.append(t)
            if len(targets) >= VWAP_RECHECK_URGENT_MAX:
                break
        if not targets:
            return
        atomic_write(BASE_DIR / "clean_micro_urgent_targets.json", {
            "version": BOT_VERSION,
            "updated_ts": now_ts(),
            "ttl_sec": 45,
            "reason": "v263_vwap_recheck_near_miss",
            "targets": targets,
            "note": "평균회귀 근접탈락 재확인용 micro urgent. 조건완화/자동매수 아님.",
        })
    except Exception as exc:
        log_error("v263_write_micro_urgent", exc)


def _v263_update_recheck_queue(rows: List[Dict[str, Any]], *, source: str = "scan") -> None:
    try:
        obj = _v263_load_recheck_queue()
        active = obj.get("active") if isinstance(obj.get("active"), dict) else {}
        recent_done = obj.get("recent_done") if isinstance(obj.get("recent_done"), list) else []
        current_by_ticker: Dict[str, Dict[str, Any]] = {}
        for r in rows or []:
            t = _v263_recheck_ticker(r)
            if t:
                current_by_ticker[t] = r

        next_active: Dict[str, Dict[str, Any]] = {}
        moved_done: List[Dict[str, Any]] = []
        for key, item in list(active.items()):
            cur = current_by_ticker.get(str((item or {}).get("ticker") or ""))
            ev = _v263_eval_active_recheck(item, cur)
            if str(ev.get("decision")) == "done":
                ev["done_at"] = now_ts()
                ev["done_text"] = now_text()
                moved_done.append(ev)
            else:
                next_active[key] = ev

        done_keys = {f"{_ticker_from_any(x.get('ticker'))}:{x.get('category')}" for x in (recent_done[-80:] + moved_done) if isinstance(x, dict)}
        added = 0
        for r in rows or []:
            if not _v263_trackable_recheck(r):
                continue
            t = _v263_recheck_ticker(r)
            cat, detail = _v263_recheck_category(r)
            key = f"{t}:{cat}"
            if not t or key in next_active or key in done_keys:
                continue
            next_active[key] = _v263_snapshot_recheck(r, cat, detail)
            added += 1
            if len(next_active) >= VWAP_RECHECK_ACTIVE_MAX:
                break

        recent_done = (recent_done + moved_done)[-VWAP_RECHECK_KEEP_DONE:]
        obj["active"] = next_active
        obj["recent_done"] = recent_done
        obj["last_source"] = source
        obj["last_added"] = added
        obj["last_done"] = len(moved_done)
        _v263_save_recheck_queue(obj)
        _v263_write_micro_urgent_from_recheck(next_active)
        with _state_lock:
            STATE["v263_recheck_active"] = len(next_active)
            STATE["v263_recheck_done"] = len(recent_done)
            STATE["v263_recheck_added"] = added
            STATE["v263_recheck_moved_done"] = len(moved_done)
            STATE["v263_recheck_source"] = source
    except Exception as exc:
        log_error("v263_update_recheck_queue", exc)


def _v263_recheck_status_lines(limit: int = 5) -> List[str]:
    obj = _v263_load_recheck_queue()
    active = list((obj.get("active") or {}).values()) if isinstance(obj.get("active"), dict) else []
    done = list(obj.get("recent_done") or []) if isinstance(obj.get("recent_done"), list) else []
    active_c = Counter(str(r.get("category") or "other") for r in active if isinstance(r, dict))
    done_c = Counter(str(r.get("status") or "-") for r in done if isinstance(r, dict))
    cat_kr = {
        "freshness": "정보부족",
        "spread": "스프레드",
        "buy_weak": "매수세약함",
    ...
    lines = [
        "",
        "[v263] 평균회귀 2차 재확인 큐",
        f"- 대기 {len(active)}개 / 최근판정 {len(done)}개 / 추가 {obj.get('last_added',0)} / 완료이동 {obj.get('last_done',0)} / source {obj.get('last_source','-')}",
        "- 목적: 조건완화가 아니라, 정보부족·스프레드·매수세 약함 탈락 후보를 30~90초 뒤 복기합니다.",
    ]
    if active_c:
        lines.append("- 대기분류: " + " / ".join(f"{cat_kr.get(k,k)} {v}" for k, v in active_c.most_common(5)))
    if done_c:
        lines.append("- 최근판정: " + " / ".join(f"{k} {v}" for k, v in done_c.most_common(5)))
    def _row_line(r: Dict[str, Any]) -> str:
        t = str(r.get("ticker") or "-")
        cat = cat_kr.get(str(r.get("category") or "other"), str(r.get("category") or "other"))
        st = str(r.get("status") or "-")
        ret = fnum(r.get("last_return_pct"), 0.0)
        high = fnum(r.get("highest_return_pct"), 0.0)
        sp = fnum(r.get("micro_spread_pct_now"), fnum(r.get("micro_spread_pct"), 999.0))
        buy = fnum(r.get("micro_trade_buy_ratio_30_now"), fnum(r.get("micro_trade_buy_ratio_30"), 0.0))
        return f"- {t}: {cat} / {st} / 현재 {ret:+.2f}% / 최고 {high:+.2f}% / 매수 {buy:.2f} / 스프레드 {sp:.2f}%"
    if active:
        lines.append("대기 예시")
        active.sort(key=lambda r: (fnum(r.get("created_at"), 0.0), -fnum(r.get("score"), 0.0)), reverse=True)
        lines.extend(_row_line(r) for r in active[:limit])
    good_done = [r for r in done if str(r.get("status")) in {"살릴만함", "놓친후보", "버린게맞음"}]
    if good_done:
        lines.append("최근 판정 예시")
        lines.extend(_row_line(r) for r in good_done[-limit:][::-1])
    return lines


_v263_prev_export_candidates = export_candidates


def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:  # type: ignore[override]
    result = _v263_prev_export_candidates(strict, shadow)
    try:
        latest_rows = tail_jsonl(FILES.get("paper_latest", BASE_DIR / "paper_candidates_latest.jsonl"), max_lines=120)
        shadow_rows = tail_jsonl(FILES.get("shadow_latest", BASE_DIR / "shadow_candidates_latest.jsonl"), max_lines=120)
        _v263_update_recheck_queue(list(latest_rows or []) + list(shadow_rows or []), source="factory_latest")
    except Exception as exc:
        log_error("v263_export_recheck_update", exc)
    return result


_v263_prev_strategy_watch_text = strategy_watch_text
...
def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    try:
        latest_rows = tail_jsonl(FILES.get("paper_latest", BASE_DIR / "paper_candidates_latest.jsonl"), max_lines=120)
        shadow_rows = tail_jsonl(FILES.get("shadow_latest", BASE_DIR / "shadow_candidates_latest.jsonl"), max_lines=120)
        _v263_update_recheck_queue(list(latest_rows or []) + list(shadow_rows or []), source="strategy_watch_command")
    except Exception as exc:
        log_error("v263_strategy_watch_recheck_update", exc)
    base = _v263_prev_strategy_watch_text(full)
    lines = base.rstrip().splitlines()
    extra = _v263_recheck_status_lines(limit=8 if full else 5)
    return "\n".join(lines + extra)


# ===============================
# v2.13.264: 기본 명령어 다이어트 + 평균회귀 반응 문턱
# - /health, /score, /quality, /strategy_watch 기본 출력은 한눈에 체크할 수 있게 줄인다.
# - 상세 설명/긴 원인분석은 full 또는 전용 명령으로 분리한다.
# - 평균회귀 조건값(VWAP/3분돈/micro/spread/매수비)은 변경하지 않는다.
# - 정식 OPEN 직전 "진입 전 반응 확인" 문턱만 추가해 움직임 없는 후보를 관찰/복기로 돌린다.
# - 자동매수, BUY_READY, 청산조건, paper 장부는 변경하지 않는다.
# ===============================

VWAP_REACTION_MIN_RET_PCT = float(os.getenv("CLEAN_VWAP_REACTION_MIN_RET_PCT", "0.25"))
VWAP_REACTION_MIN_PEAK_PCT = float(os.getenv("CLEAN_VWAP_REACTION_MIN_PEAK_PCT", "0.30"))
VWAP_REACTION_MIN_CHANGE_1 = float(os.getenv("CLEAN_VWAP_REACTION_MIN_CHANGE_1", "0.02"))
VWAP_REACTION_MIN_CHANGE_3 = float(os.getenv("CLEAN_VWAP_REACTION_MIN_CHANGE_3", "0.05"))
VWAP_REACTION_MIN_RECOVERY = float(os.getenv("CLEAN_VWAP_REACTION_MIN_RECOVERY", "0.05"))
VWAP_REACTION_MIN_AVG_TURN = float(os.getenv("CLEAN_VWAP_REACTION_MIN_AVG_TURN", "0.02"))
VWAP_REACTION_MIN_LOWER_WICK = float(os.getenv("CLEAN_VWAP_REACTION_MIN_LOWER_WICK", "0.12"))


def _v264_fmt_age(ts: Any) -> str:
    v = fnum(ts, 0.0)
    if v <= 0:
        return "-"
    return f"{max(0, now_ts() - v):.0f}초"


def _v264_reaction_signals(row: Dict[str, Any], ret: Optional[float] = None, peak: Optional[float] = None) -> List[str]:
    r = row or {}
    signals: List[str] = []
    ch1 = fnum(r.get("change_1") or r.get("change_1_now"), 0.0)
    ch3 = fnum(r.get("change_3") or r.get("change_3_now"), 0.0)
    recovery = fnum(r.get("recovery_speed_pct") or r.get("recovery_speed_pct_now"), 0.0)
    avg_turn = fnum(r.get("avg_price_turn_pct") or r.get("avg_price_turn_pct_now"), 0.0)
    lower = fnum(r.get("current_lower_wick_pct") or r.get("lower_wick_pct"), 0.0)
    if ret is not None and ret >= VWAP_REACTION_MIN_RET_PCT:
        signals.append(f"현재반응 {ret:+.2f}%")
    if peak is not None and peak >= VWAP_REACTION_MIN_PEAK_PCT:
        signals.append(f"최고반응 {peak:+.2f}%")
    if ch1 >= VWAP_REACTION_MIN_CHANGE_1:
        signals.append(f"1분 {ch1:+.2f}%")
    if ch3 >= VWAP_REACTION_MIN_CHANGE_3:
        signals.append(f"3분 {ch3:+.2f}%")
    if recovery >= VWAP_REACTION_MIN_RECOVERY:
        signals.append(f"회복 {recovery:+.2f}%")
    if avg_turn >= VWAP_REACTION_MIN_AVG_TURN:
        signals.append(f"평균가턴 {avg_turn:+.2f}%")
    if lower >= VWAP_REACTION_MIN_LOWER_WICK:
        signals.append(f"아래꼬리 {lower:.2f}%")
    return list(dict.fromkeys(signals))[:5]


_v264_base_decide_trade_ready = decide_trade_ready

def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:  # type: ignore[override]
    ok, label, reasons = _v264_base_decide_trade_ready(item)
    if not ok or not _v250_is_focus_item(item):
        return ok, label, reasons
    # v264: 평균회귀 조건을 더 조이는 게 아니라, 최종 OPEN 직전 무반응 후보만 관찰로 돌린다.
    signals = _v264_reaction_signals(item)
    if not signals:
        detail = (
            f"진입 전 반응 문턱 미통과: 1분 {fnum(item.get('change_1'),0):+.2f}% / "
            f"3분 {fnum(item.get('change_3'),0):+.2f}% / "
            f"회복 {fnum(item.get('recovery_speed_pct'),0):+.2f}% / "
            f"평균가턴 {fnum(item.get('avg_price_turn_pct'),0):+.2f}%"
        )
        merged = list(reasons or []) + [detail]
        return False, "평균회귀 후보 관찰: 진입 전 반응 확인 대기", list(dict.fromkeys(str(x) for x in merged))[:8]
    return ok, label, reasons


def _v264_resource_brief() -> Tuple[str, str]:
    try:
        r = server_resource_snapshot() if 'server_resource_snapshot' in globals() else {}
        disk = fnum(r.get("disk_pct"), 0.0)
        mem = fnum(r.get("mem_pct"), 0.0)
        load1 = fnum(r.get("load1"), 0.0)
        snap = load_json(FILES.get("resource_status", BASE_DIR / "clean_resource_status.json"), {})
        cpu = fnum((snap or {}).get("cpu_pct"), 0.0) if isinstance(snap, dict) else 0.0
        proc = (snap or {}).get("proc_cpu_pct") if isinstance(snap, dict) else {}
        if not isinstance(proc, dict):
            proc = {}
        icon = "✅" if disk < 85 and mem < 80 and cpu < 80 and load1 < 2.5 else ("⚠️" if disk < 95 and mem < 92 and cpu < 95 else "❌")
        brief = f"{icon} 자원: disk {disk:.1f}% / mem {mem:.1f}% / CPU {cpu:.1f}% / load {load1:.2f}"
        sub = f"- 세부: main {fnum(proc.get('main'),0):.1f}% / paper {fnum(proc.get('paper'),0):.1f}% / WS {fnum(proc.get('ws'),0):.1f}% / micro {fnum(proc.get('micro'),0):.1f}%"
        return brief, sub
    except Exception:
        return "❔ 자원: 확인중", "- 세부: /health_full 또는 가드 상태판 확인"
...
def health_text() -> str:  # type: ignore[override]
    ...
    scan = scan_status_summary()
    paper = read_paper_status().get("status", {})
    res, res_sub = _v264_resource_brief()
    err_ok = not STATE.get("scan_last_error")
    scan_icon = scan.get("icon", "❔")
    total = int(snap.get("total", 0) or 0)
    ws_f = int(snap.get("ws_fresh", 0) or 0)
    micro_f = int(snap.get("micro_fresh", 0) or 0)
    ws_icon = "✅" if total <= 0 or ws_f >= max(1, total) else ("⚠️" if ws_f > 0 else "❔")
    micro_icon = "✅" if total <= 0 or micro_f >= max(1, total) else ("⚠️" if micro_f > 0 else "❔")
    paper_icon = "✅" if str(paper.get("running", "")).lower() in {"true", "1", "yes"} or paper.get("running") is True else "⚠️"
    warn: List[str] = []
    if not err_ok:
        warn.append("오류로그 확인")
    if fnum(scan.get("sec"), fnum(STATE.get("last_done_scan_sec"), 0.0)) >= 20:
        warn.append("scan 20초+")
    if total > 0 and ws_f < total:
        warn.append("WS 후보 일부 부족")
    if total > 0 and micro_f < total:
        warn.append("micro 후보 일부 부족")
    lines = [
        "🧭 건강상태 /health",
        f"{scan_icon} 메인: {BOT_VERSION} / scan {scan.get('label','-')} / {scan.get('stage','-')} / 후보 {total} / 오류 {'없음' if err_ok else '있음'}",
        res,
        f"{ws_icon} WS: 후보 {ws_f}/{total} / 전체신선 {snap.get('ws_worker_fresh',0)} / 최근 {snap.get('ws_last_age_sec','-')}초",
        f"{micro_icon} 호가·체결: 후보 {micro_f}/{total} / 전체신선 {snap.get('micro_worker_fresh',0)} / {snap.get('micro_worker_state','-')}",
        f"{paper_icon} paper: {paper.get('version','paper_bot')} / OPEN {paper.get('open_total', paper.get('open_count','?'))} / CLOSED {paper.get('closed_total','?')} / 상태 {_v264_fmt_age(paper.get('updated_ts'))}",
        f"판독: {'✅ 정상 체크' if not warn else '⚠️ ' + ' / '.join(warn[:4])}",
        res_sub,
    ]
    return "\n".join(lines)


def _v264_top_loss_cause_lines(rows: List[Dict[str, Any]], limit: int = 4) -> List[str]:
    loss = _v254_loss_rows(rows)
    buckets: Dict[str, List[Dict[str, Any]]] = {}
    for r in loss:
        for tag in _v254_cause_tags(r):
    ...
    if not buckets:
        return ["- 손실 원인 없음"]
    out: List[str] = []
    for tag, arr in sorted(buckets.items(), key=lambda kv: (len(kv[1]), abs(_v254_stat(kv[1])["total"])), reverse=True)[:limit]:
        st = _v254_stat(arr)
        out.append(f"- {tag}: {st['n']}건 / 합산 {st['total']:+.2f}% / 평균최고 {st['avg_peak']:+.2f}%")
    return out


def _v264_exit_compact_lines(rows: List[Dict[str, Any]], limit: int = 4) -> List[str]:
    return _v248_exit_reason_lines(rows, limit)
...
def _v251_build_score_text_for_cache() -> str:  # type: ignore[override]
    ...
    focus_rows = _v250_paper_latest_rows()
    missed = _v250_missed_rows(watch_rows, focus_rows)
    st = _v254_stat(closed)
    bad = []
    if st["total"] < 0:
        bad.append(f"합산 {st['total']:+.2f}%")
    if st["win_rate"] < 45:
        bad.append(f"승률 {st['win_rate']:.1f}%")
    lines = [
        "📊 모의매매 성과 /score",
        f"기준: 평균회귀 단일검증 / {BOT_VERSION}",
        "[1] 성과",
        _compact_stat_line("평균회귀 정식", closed),
        f"- 판독: {'❌ 자동매매 불가' if bad else '⚠️ 더 보기 필요'}" + (f" ({' / '.join(bad[:2])})" if bad else ""),
        "",
        "[2] 종료사유 TOP",
        *_v264_exit_compact_lines(closed, 5),
        "",
        "[3] 반복 손실 원인",
        *_v264_top_loss_cause_lines(closed, 5),
        "",
        "[4] 현재 관찰",
        *_v250_strategy_score_summary_lines(),
        f"- 놓친 후보: {len(missed)}개 / 후보 {len(focus_rows)}개 / trade_ready {sum(1 for r in focus_rows if _v247_bool_ready(r))}개",
        "",
        "다음: 무반응·하드가드·놓친후보만 봅니다. 긴 원인분석은 /reversion_review",
    ]
    return "\n".join(lines)
...
def _v250_quality_text() -> str:  # type: ignore[override]
    ...
    lines = [
        "🔬 후보품질 /quality",
        f"기준: 평균회귀 단일검증 / snapshot {len((snap or {}).get('rows') or []) if isinstance(snap, dict) else 0}개 / {snap_age:.1f}초 전",
        "[1] latest 후보",
        f"- 후보 {len(focus_rows)} / trade_ready {len(paper_ready)} / 탈락 {max(0, len(focus_rows)-len(paper_ready))}",
        _simple_ws_line(focus_rows),
        _simple_micro_line(focus_rows),
        "",
        "[2] 탈락 이유",
        *_v250_focus_reject_summary_lines(focus_rows),
        "",
        "[3] 놓친 후보",
        *_v250_missed_lines(watch_rows, focus_rows, 4),
        *_v248_missed_bucket_lines(watch_rows, focus_rows),
        "",
        "[4] 단계 예시",
        "🧪 통과",
        *_v248_stage_example_lines(paper_ready, 2),
        "❔ 탈락",
        *_v248_stage_example_lines([r for r in focus_rows if not _v247_bool_ready(r)], 3),
        "",
        "판독: 조건값보다 무반응·재확인·놓친후보를 우선 봅니다. 과거 손실은 /reversion_review",
    ]
    return "\n".join(lines)


def _v258_reversion_review_text_for_cache() -> str:  # type: ignore[override]
    rows = _v250_closed_rows(limit=30000)
    st = _v254_stat(rows)
    lines = [
        "🧯 평균회귀 원인분석 /reversion_review",
        f"대상: {st['n']}전 {st['wins']}승 {st['losses']}패 / 승률 {st['win_rate']:.1f}% / 합산 {st['total']:+.2f}% / 평균 {st['avg']:+.2f}%",
        "[1] 종료사유 TOP",
        *_v254_by_exit_lines(rows, 6),
        "",
        "[2] 손실 원인 TOP",
        *_v264_top_loss_cause_lines(rows, 6),
        "",
        "[3] 승리군 vs 손실군",
        *_v254_entry_feature_lines(rows),
        "",
        "다음 판단: 1분 흐름 음수 / 진입 후 반응없음 / 하드가드급 큰손실 감소 여부",
    ]
    return "\n".join(lines)
...
def _v263_eval_active_recheck(item: Dict[str, Any], cur: Optional[Dict[str, Any]]) -> Dict[str, Any]:  # type: ignore[override]
    nowv = now_ts()
    out = dict(item or {})
    age = nowv - fnum(out.get("created_at"), nowv)
    if not isinstance(cur, dict):
    ...
    price = _v263_recheck_price(cur)
    ret = ((price - price0) / price0 * 100.0) if price0 > 0 and price > 0 else fnum(out.get("last_return_pct"), 0.0)
    high_prev = fnum(out.get("highest_return_pct"), ret)
    low_prev = fnum(out.get("lowest_return_pct"), ret)
    high = max(high_prev, ret)
    low = min(low_prev, ret)
    out["last_price"] = price if price > 0 else out.get("last_price", 0.0)
    out["last_return_pct"] = round(ret, 3)
    out["highest_return_pct"] = round(high, 3)
    out["lowest_return_pct"] = round(low, 3)
    out["last_seen_at"] = nowv
    out["last_seen_text"] = now_text(nowv)
    out["micro_spread_pct_now"] = _v250_micro_spread(cur)
    out["micro_trade_buy_ratio_30_now"] = _v250_micro_buy_ratio(cur)
    ...
    buy_ok = fnum(out.get("micro_trade_buy_ratio_30_now"), 0.0) >= VWAP_REV_MIN_BUY_RATIO
    fresh_ok = bool(out.get("micro_fresh_now") or out.get("ws_fresh_now"))
    reaction = _v264_reaction_signals(cur, ret=ret, peak=high)
    out["reaction_signals"] = reaction
    normal_ok = spread_ok and (buy_ok or cat != "buy_weak")
    ...
    if ret >= 1.20:
        ...
        out["decision"] = "done"
        out["review_result"] = "재확인 중 -0.7% 먼저 이탈"
    elif reaction and fresh_ok and normal_ok:
        out["status"] = "살릴만함"
        out["decision"] = "done"
        out["review_result"] = "정보/호가 정상 + 가격반응 확인: " + ", ".join(reaction[:3])
    elif fresh_ok and cat == "freshness":
        out["status"] = "정보회복"
        out["decision"] = "keep" if age < VWAP_RECHECK_MAX_AGE_SEC else "done"
        out["review_result"] = "정보는 회복됐지만 가격반응 부족"
    elif spread_ok and cat == "spread":
        out["status"] = "스프레드정상화"
        out["decision"] = "keep" if age < VWAP_RECHECK_MAX_AGE_SEC else "done"
        out["review_result"] = "스프레드는 정상화됐지만 가격반응 부족"
    elif buy_ok and cat == "buy_weak":
        out["status"] = "매수비회복"
        out["decision"] = "keep" if age < VWAP_RECHECK_MAX_AGE_SEC else "done"
        out["review_result"] = "매수비는 회복됐지만 가격반응 부족"
    elif age >= VWAP_RECHECK_MAX_AGE_SEC:
        out["status"] = "무반응종료" if high < VWAP_REACTION_MIN_PEAK_PCT else "판단보류종료"
        out["decision"] = "done"
        out["review_result"] = "재확인 제한시간 종료"
    else:
        out["status"] = "계속관찰"
...
def _v263_recheck_status_lines(limit: int = 5) -> List[str]:  # type: ignore[override]
    obj = _v263_load_recheck_queue()
    active = list((obj.get("active") or {}).values()) if isinstance(obj.get("active"), dict) else []
    done = list(obj.get("recent_done") or []) if isinstance(obj.get("recent_done"), list) else []
    active_c = Counter(str(r.get("category") or "other") for r in active if isinstance(r, dict))
    done_c = Counter(str(r.get("status") or "-") for r in done if isinstance(r, dict))
    cat_kr = {"freshness": "정보부족", "spread": "스프레드", "buy_weak": "매수세", "turn_wait": "반응대기", "structure": "구조근접", "other": "기타"}
    lines = [
        "",
        "[v264] 평균회귀 재확인",
        f"- 대기 {len(active)} / 판정 {len(done)} / 완료이동 {obj.get('last_done',0)} / source {obj.get('last_source','-')}",
    ]
    if active_c:
        lines.append("- 대기: " + " / ".join(f"{cat_kr.get(k,k)} {v}" for k, v in active_c.most_common(5)))
    if done_c:
        lines.append("- 판정: " + " / ".join(f"{k} {v}" for k, v in done_c.most_common(6)))
    def _row_line(r: Dict[str, Any]) -> str:
        t = str(r.get("ticker") or "-")
        cat = cat_kr.get(str(r.get("category") or "other"), str(r.get("category") or "other"))
        st = str(r.get("status") or "-")
        ...
        sp = fnum(r.get("micro_spread_pct_now"), fnum(r.get("micro_spread_pct"), 999.0))
        buy = fnum(r.get("micro_trade_buy_ratio_30_now"), fnum(r.get("micro_trade_buy_ratio_30"), 0.0))
        return f"- {t}: {cat}/{st} 현재 {ret:+.2f}% 최고 {high:+.2f}% 매수 {buy:.2f} 스프 {sp:.2f}%"
    active.sort(key=lambda r: (str(r.get("status") or ""), fnum(r.get("created_at"), 0.0)), reverse=True)
    if active:
        lines += ["대기 예시", *[_row_line(r) for r in active[:limit]]]
    pick = [r for r in done if str(r.get("status")) in {"놓친후보", "살릴만함", "버린게맞음", "무반응종료", "정보회복", "스프레드정상화", "매수비회복"}]
    if pick:
        lines += ["최근 판정", *[_row_line(r) for r in pick[-limit:][::-1]]]
    lines.append("- 기준: 조건완화 아님. 정보회복만으로는 살릴만함 처리하지 않고 가격반응까지 확인합니다.")
    return lines


_v264_prev_strategy_watch_text = strategy_watch_text

def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    txt = _v264_prev_strategy_watch_text(full)
    if full:
        return txt
    # 기본 설명문을 줄인다. 핵심 숫자와 v264 재확인 블록은 그대로 둔다.
    remove_prefixes = (
        "- 정식 paper OPEN 연결:",
        "- 평균 조건:",
        "- 기존 돈흐름/눌림",
    ...
    for line in txt.splitlines():
        s = line.strip()
        if any(s.startswith(p) for p in remove_prefixes):
            continue
        if s == "":
            if blank:
                continue
...
# ===============================
# v2.13.265: 재확인대기 trade_ready 누수 차단 + 성과표 가독성 보강
# - 재확인대기/관찰/반응대기 후보는 paper_latest 저장 직전에도 정식 OPEN 불가로 고정한다.
# - /score에 최근 3/6/12시간 성과를 한눈에 보이게 추가한다.
# - 최대수익 1건 제외 합산을 같이 보여 최대수익 착시를 바로 확인한다.
# - CPU 측정 실패값 0.0%를 실제 0%처럼 보이지 않게 '측정중'으로 표시한다.
# - /strategy_watch 기본 출력 예시를 더 줄이고 상세는 /strategy_watch_full로 분리한다.
# - 평균회귀 조건값, 자동매수, BUY_READY, paper 장부, 청산조건은 변경하지 않는다.
# ===============================


def _v265_is_recheck_or_hold(row: Dict[str, Any]) -> bool:
    """정식 OPEN으로 넘기면 안 되는 최종검증/반응대기 상태.

    v264에서 표시상 '재확인대기'인데 paper trade_ready 통과가 함께 보이는 누수가 있었다.
    여기서는 조건을 새로 조이지 않고, 이미 산출된 상태값이 대기/관찰이면 OPEN 표지만 확실히 끈다.
    """
    if not isinstance(row, dict):
        return False
    ctx = row.get("entry_context") if isinstance(row.get("entry_context"), dict) else {}
    vals = [
        row.get("final_entry_action"), ctx.get("final_entry_action"),
        row.get("final_entry_label"), ctx.get("final_entry_label"),
        row.get("trade_ready_label"), row.get("decision"), row.get("event_type"),
    ]
    blob = " ".join(str(x or "") for x in vals)
    action = str(row.get("final_entry_action") or ctx.get("final_entry_action") or "")
    if action in {"recheck_wait", "observe"}:
        return True
    hold_words = ["재확인대기", "관찰전환", "진입 전 반응", "반응 확인 대기", "돌아섬 확인 대기", "stop/밀림 위험"]
    return any(w in blob for w in hold_words)


def _v265_mark_no_open(row: Dict[str, Any], reason: str) -> Dict[str, Any]:
    rr = dict(row or {})
    rr["trade_ready"] = False
    rr["open_eligible"] = False
    rr["paper_bot_open"] = False
    rr["review_only"] = True
    rr["v265_open_blocked"] = True
    rr["v265_open_block_reason"] = reason
    rr["trade_ready_label"] = "평균회귀 후보 관찰: " + reason
    old = rr.get("trade_ready_reasons") if isinstance(rr.get("trade_ready_reasons"), list) else []
    rr["trade_ready_reasons"] = list(dict.fromkeys([str(x) for x in old] + [reason]))[:8]
    ctx = rr.get("entry_context") if isinstance(rr.get("entry_context"), dict) else {}
    ctx = dict(ctx)
    ctx["trade_ready"] = False
    ctx["open_eligible"] = False
    ctx["paper_bot_open"] = False
    ctx["v265_open_blocked"] = True
    ctx["v265_open_block_reason"] = reason
    rr["entry_context"] = ctx
    return rr


_v265_base_decide_trade_ready = decide_trade_ready

def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:  # type: ignore[override]
    ok, label, reasons = _v265_base_decide_trade_ready(item)
    if not _v250_is_focus_item(item):
        return ok, label, reasons
    if _v265_is_recheck_or_hold(item):
        merged = list(reasons or []) + ["재확인대기/관찰 상태라 정식 OPEN 보류"]
        return False, "평균회귀 후보 관찰: 재확인 후 진입", list(dict.fromkeys(str(x) for x in merged))[:8]
    return ok, label, reasons


_v265_base_consume_row = consume_row

def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:  # type: ignore[override]
    row = _v265_base_consume_row(item, lane, ts, scan_id)
    if lane == "strict" and (_v250_is_focus_item(item) or _v250_is_focus_item(row)):
        if _v265_is_recheck_or_hold(item) or _v265_is_recheck_or_hold(row):
            row = _v265_mark_no_open(row, "재확인대기/반응대기 상태라 정식 OPEN 보류")
    return row


def _v265_window_perf_line(label: str, rows: List[Dict[str, Any]]) -> str:
    rows = list(rows or [])
    st = _v254_stat(rows)
    if not rows:
        return f"{label:<5}  ❔  0전   승률 -     합산 -       최대 -       최대제외 -"
    pnls = [_v254_pnl(r) for r in rows]
    max_pnl = max(pnls) if pnls else 0.0
    total_ex_max = st["total"] - max_pnl if len(rows) >= 2 else 0.0
    icon = "✅" if st["n"] >= 5 and st["total"] > 0 and total_ex_max >= 0 else ("❌" if st["n"] >= 5 and st["total"] < 0 else "⚠️")
    return (
        f"{label:<5}  {icon} {st['n']:>3}전  "
        f"승률 {st['win_rate']:>5.1f}%  "
        f"합산 {st['total']:>+7.2f}%  "
        f"최대 {max_pnl:>+6.2f}%  "
        f"최대제외 {total_ex_max:>+7.2f}%"
    )


def _v265_score_window_lines(rows: List[Dict[str, Any]]) -> List[str]:
    return [
        "[1] 최근 성과",
        _v265_window_perf_line("3시간", rows_recent_hours(rows, 3)),
        _v265_window_perf_line("6시간", rows_recent_hours(rows, 6)),
        _v265_window_perf_line("12시간", rows_recent_hours(rows, 12)),
        "- 최대제외 = 제일 큰 수익 1건을 빼고도 플러스인지 보는 값",
    ]
...
def _v251_build_score_text_for_cache() -> str:  # type: ignore[override]
    ...
    lines = [
        "📊 모의매매 성과 /score",
        f"기준: 평균회귀 단일검증 / {BOT_VERSION}",
        *_v265_score_window_lines(closed),
        "",
        "[2] 누적 성과",
        _compact_stat_line("평균회귀 정식", closed),
        f"- 판독: {'❌ 자동매매 불가' if bad else '⚠️ 더 보기 필요'}" + (f" ({' / '.join(bad[:2])})" if bad else ""),
        ...
        *_v250_strategy_score_summary_lines(),
        f"- 놓친 후보: {len(missed)}개 / 후보 {len(focus_rows)}개 / trade_ready {sum(1 for r in focus_rows if _v247_bool_ready(r))}개",
        "",
        "다음: 재확인대기 누수 / 무반응 / 놓친후보만 봅니다. 상세는 /reversion_review",
    ]
    return "\n".join(lines)


def _v265_cpu_text(v: Any, *, load1: float = 0.0) -> str:
    try:
        x = float(v)
        if x <= 0.0 and load1 >= 0.8:
            return "측정중"
        return f"{x:.1f}%"
    except Exception:
        return "측정중"
...
def _v264_resource_brief() -> Tuple[str, str]:  # type: ignore[override]
    try:
        r = server_resource_snapshot() if 'server_resource_snapshot' in globals() else {}
        disk = fnum(r.get("disk_pct"), 0.0)
        mem = fnum(r.get("mem_pct"), 0.0)
        load1 = fnum(r.get("load1"), 0.0)
        snap = load_json(FILES.get("resource_status", BASE_DIR / "clean_resource_status.json"), {})
        cpu_raw = (snap or {}).get("cpu_pct") if isinstance(snap, dict) else None
        proc = (snap or {}).get("proc_cpu_pct") if isinstance(snap, dict) else {}
        if not isinstance(proc, dict):
            proc = {}
        cpu_val = fnum(cpu_raw, -1.0)
        cpu_unknown = cpu_val <= 0.0 and load1 >= 0.8
        icon = "✅" if disk < 85 and mem < 80 and (cpu_unknown or cpu_val < 80) and load1 < 2.5 else ("⚠️" if disk < 95 and mem < 92 and (cpu_unknown or cpu_val < 95) else "❌")
        brief = f"{icon} 자원: disk {disk:.1f}% / mem {mem:.1f}% / CPU {_v265_cpu_text(cpu_raw, load1=load1)} / load {load1:.2f}"
        sub = (
            f"- 세부: main {_v265_cpu_text(proc.get('main'), load1=load1)} / "
            f"paper {_v265_cpu_text(proc.get('paper'), load1=load1)} / "
            f"WS {_v265_cpu_text(proc.get('ws'), load1=load1)} / "
            f"micro {_v265_cpu_text(proc.get('micro'), load1=load1)}"
        )
        return brief, sub
    except Exception:
        return "❔ 자원: 확인중", "- 세부: /health_full 또는 가드 상태판 확인"
...
def _v265_truncate_strategy_watch_examples(text: str) -> str:
    """기본 /strategy_watch의 예시 줄을 줄인다. full 명령은 그대로 둔다."""
    ...
    done_count = 0
    stage_count = 0
    skip_candidate_detail = False
    for line in lines:
        s = line.strip()
        if s.startswith("[5/6] 단계별 예시"):
            section = "stage"; stage_count = 0; skip_candidate_detail = False; out.append(line); continue
        if s.startswith("[6/6] 눈에 띄는"):
            section = "candidates"; candidate_count = 0; skip_candidate_detail = False; out.append(line); continue
        if s == "대기 예시":
            section = "recheck_active"; active_count = 0; skip_candidate_detail = False; out.append(line); continue
        if s == "최근 판정":
            section = "recheck_done"; done_count = 0; skip_candidate_detail = False; out.append(line); continue
        if s.startswith("[") and not s.startswith("[v264]"):
            section = ""; skip_candidate_detail = False
        if section == "stage" and s.startswith("- "):
            stage_count += 1
            if stage_count > 3:
                continue
            ...
            if s.startswith("- "):
                candidate_count += 1
                skip_candidate_detail = candidate_count > 3
                if skip_candidate_detail:
                    continue
            elif skip_candidate_detail and (s.startswith("·") or line.startswith("  ·") or line.startswith("    ·")):
                continue
        elif section == "recheck_active" and s.startswith("- "):
            active_count += 1
            if active_count > 3:
                continue
        elif section == "recheck_done" and s.startswith("- "):
            done_count += 1
            if done_count > 3:
                continue
        out.append(line)
...
_v265_prev_strategy_watch_text = strategy_watch_text

def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    txt = _v265_prev_strategy_watch_text(full)
    if full:
        return txt
    return _v265_truncate_strategy_watch_examples(txt)



# ===============================
# v2.13.266: 최근 진입문턱 실험 롤백 + 성과판 재정렬
# - v264/v265에서 붙인 평균회귀 반응문턱/재확인 강제차단 실험은 성과 악화 구간과 섞여 롤백한다.
# - v263의 복기용 재확인 큐는 유지하되, 정식 진입 조건으로 쓰지 않는다.
# - /score는 버전별 누적 성과 + 3/6/12시간 성과를 먼저 보여준다.
# - /health 가독성/CPU 측정중 표시와 기본 명령어 다이어트는 유지한다.
# - 평균회귀 조건값, 자동매수, BUY_READY, 청산조건, paper 장부는 변경하지 않는다.
# ===============================

# v266 rollback: v264 reaction gate and v265 hard no-open marker are not used for live paper handoff.
# Keep the pre-v264 trade_ready decision path. That path already treats final_entry_action
# recheck_wait/observe as a reason, but does not add the new v264 reaction threshold.
try:
    decide_trade_ready = _v264_base_decide_trade_ready  # type: ignore[name-defined,assignment]
except Exception:
    pass
try:
    consume_row = _v265_base_consume_row  # type: ignore[name-defined,assignment]
except Exception:
    pass


def _v266_short_ver(ver: Any) -> str:
    s = str(ver or "버전미기록")
    m = re.search(r"v\d+\.\d+\.\d+", s)
    return m.group(0) if m else s.replace("수익형 ", "")[:18]


def _v266_stat(rows: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    arr = list(rows or [])
    n = len(arr)
    wins = sum(1 for r in arr if _v254_pnl(r) > 0)
    total = sum(_v254_pnl(r) for r in arr)
    maxp = max([_v254_pnl(r) for r in arr], default=0.0)
    excl = total - maxp if n >= 2 else 0.0
    return {"n": n, "wins": wins, "losses": n-wins, "win_rate": (wins/n*100.0 if n else 0.0), "total": total, "avg": (total/n if n else 0.0), "max": maxp, "excl": excl}


def _v266_perf_line(label: str, rows: Iterable[Dict[str, Any]], width: int = 7) -> str:
    st = _v266_stat(rows)
    n = int(st["n"])
    if n <= 0:
        return f"{label:<{width}}  ❔   0전  승률   -    합산      -   최대      -   최대제외      -"
    icon = "✅" if st["total"] > 0 and st["excl"] >= 0 else ("⚠️" if st["total"] > 0 and st["excl"] < 0 else "❌")
    return (
        f"{label:<{width}}  {icon} {n:>3}전  "
        f"승률 {st['win_rate']:>5.1f}%  "
        f"합산 {st['total']:>+8.2f}%  "
        f"최대 {st['max']:>+7.2f}%  "
        f"최대제외 {st['excl']:>+8.2f}%"
...
def _v266_version_perf_lines(rows: List[Dict[str, Any]], limit: int = 6) -> List[str]:
    groups: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    latest: Dict[str, float] = {}
    for r in rows or []:
        if str((r or {}).get("lane") or "strict") not in {"strict", ""}:
            continue
        ver = _v266_short_ver(row_brain_version(r))
        groups[ver].append(r)
        latest[ver] = max(latest.get(ver, 0.0), row_ts(r, "closed_at", "exit_at", "opened_at", "source_created_at"))
    out = ["[1] 버전별 누적 성과"]
    if not groups:
        return out + ["- 버전별 CLOSED 없음"]
    keys = sorted(groups.keys(), key=lambda k: (latest.get(k, 0.0), k), reverse=True)[:limit]
    for k in keys:
        out.append(_v266_perf_line(k, groups[k], width=9))
    out.append("- 최대제외 = 해당 구간 최대수익 1건 제외 합산")
    return out


def _v266_current_version_lines(rows: List[Dict[str, Any]]) -> List[str]:
    cur = _v266_short_ver(BOT_VERSION)
    arr = [r for r in rows or [] if _v266_short_ver(row_brain_version(r)) == cur]
    out = ["[2] 현재버전 성과"]
    out.append(_v266_perf_line(cur, arr, width=9))
    # repeated loss counters only for current version; helps avoid mixing previous opened positions.
    if arr:
        loss = [r for r in arr if _v254_pnl(r) <= 0]
        no_react = sum(1 for r in loss if "진입 후 반응없음" in _v254_cause_tags(r))
        hard = sum(1 for r in loss if "하드가드급 큰손실" in _v254_cause_tags(r))
        one_neg = sum(1 for r in loss if "1분 흐름 음수" in _v254_cause_tags(r))
        out.append(f"- 현재판 손실표식: 무반응 {no_react} / 1분음수 {one_neg} / 하드가드 {hard}")
    else:
        out.append("- 현재판 CLOSED 아직 없음. 기존 포지션 종료분과 섞지 말고 다음 결과 확인")
    return out


def _v266_time_perf_lines(rows: List[Dict[str, Any]]) -> List[str]:
    return [
        "[3] 시간별 성과",
        _v266_perf_line("3시간", rows_recent_hours(rows, 3), width=6),
        _v266_perf_line("6시간", rows_recent_hours(rows, 6), width=6),
        _v266_perf_line("12시간", rows_recent_hours(rows, 12), width=6),
        "- 시간별 성과는 여러 버전 종료분이 섞일 수 있음",
    ]


def _v251_build_score_text_for_cache() -> str:  # type: ignore[override]
    closed = _v250_closed_rows(limit=30000)
    watch_rows = _v250_watch_rows(3.0)
    focus_rows = _v250_paper_latest_rows()
    missed = _v250_missed_rows(watch_rows, focus_rows)
    st = _v266_stat(closed)
    bad = []
    if st["total"] < 0:
        bad.append(f"합산 {st['total']:+.2f}%")
    if st["win_rate"] < 45:
    ...
    lines = [
        "📊 모의매매 성과 /score",
        f"기준: 평균회귀 단일검증 / {BOT_VERSION}",
        *_v266_version_perf_lines(closed, limit=6),
        "",
        *_v266_current_version_lines(closed),
        "",
        *_v266_time_perf_lines(closed),
        "",
        "[4] 전체 누적",
        _v266_perf_line("전체", closed, width=9),
        f"- 판독: {'❌ 자동매매 불가' if bad else '⚠️ 더 보기 필요'}" + (f" ({' / '.join(bad[:2])})" if bad else ""),
        "",
        "[5] 손실 원인 TOP",
        *_v264_top_loss_cause_lines(closed, 5),
        ...
        "[6] 현재 관찰",
        *_v250_strategy_score_summary_lines(),
        f"- 놓친 후보 {len(missed)} / latest 후보 {len(focus_rows)} / trade_ready {sum(1 for r in focus_rows if _v247_bool_ready(r))}",
        "",
        "다음: 버전별 성과 / 무반응 / 놓친후보를 분리해서 봅니다. 상세는 /reversion_review",
    ]
    return "\n".join(lines)


# v266: reset v264/v265 recheck wording to version-neutral review-only wording.
def _v263_recheck_status_lines(limit: int = 5) -> List[str]:  # type: ignore[override]
    obj = _v263_load_recheck_queue()
    active = list((obj.get("active") or {}).values()) if isinstance(obj.get("active"), dict) else []
    done = list(obj.get("recent_done") or []) if isinstance(obj.get("recent_done"), list) else []
    ...
    active_c = Counter(str(r.get("category") or "other") for r in active if isinstance(r, dict))
    done_c = Counter(str(r.get("status") or "-") for r in done if isinstance(r, dict))
    lines = ["", "[현재판] 평균회귀 재확인 큐", f"- 대기 {len(active)} / 판정 {len(done)} / source {obj.get('last_source','-')}"]
    if active_c:
        lines.append("- 대기: " + " / ".join(f"{cat_kr.get(k,k)} {v}" for k, v in active_c.most_common(5)))
    if done_c:
        lines.append("- 판정: " + " / ".join(f"{k} {v}" for k, v in done_c.most_common(6)))
    def _row_line(r: Dict[str, Any]) -> str:
        ...
        buy = fnum(r.get("micro_trade_buy_ratio_30_now"), fnum(r.get("micro_trade_buy_ratio_30"), 0.0))
        return f"- {t}: {cat}/{st} 현재 {ret:+.2f}% 최고 {high:+.2f}% 매수 {buy:.2f} 스프 {sp:.2f}%"
    active.sort(key=lambda r: fnum(r.get("created_at"), 0.0), reverse=True)
    if active:
        lines += ["대기 예시", *[_row_line(r) for r in active[:limit]]]
    pick = [r for r in done if str(r.get("status") or "") in {"놓친후보", "살릴만함", "버린게맞음", "판단보류종료", "정보부족종료"}]
    if pick:
        lines += ["최근 판정", *[_row_line(r) for r in pick[-limit:][::-1]]]
    lines.append("- 기준: 복기용입니다. 조건완화/자동매수/BUY_READY 변경 없음")
    return lines


# v266: make cache status shorter in basic command display.
def _v263_cache_display_text(title: str, command_name: str, obj: Dict[str, Any], why: str) -> str:  # type: ignore[override]
    text = str((obj or {}).get("text") or "").strip()
    cur = BOT_VERSION
    cv = str((obj or {}).get("version") or "-")
    ts = str((obj or {}).get("updated_text") or "-")
    if text:
        return text + f"\n\n[캐시] {cv} / {ts} / 갱신대기: {why}"
    return f"{title}\n❔ 캐시 준비중\n- 현재 {cur} / 사유 {why}\n- 기본 명령어는 직접계산을 타지 않습니다."


# v266: keep v265 health/cpu and strategy_watch truncation, but label the rollback in state.
try:
    STATE["phase_note"] = "v266: v264/v265 진입문턱 실험 롤백, 버전별/시간별 성과판 우선. 조건/청산/BUY_READY 변경 없음."
except Exception:
    pass
...
# ===============================
# v2.13.267: 조건 대량 차단이 아니라 조건 기록판 추가
# - 다른 단타/스캘핑 봇들이 보는 조건을 즉시 매수조건으로 넣지 않는다.
# - 후보 row에 VWAP reclaim, 반응, 저점방어, 흐름회복, 호가/체결, 시장폭 같은 probe 태그만 기록한다.
# - /score에는 버전별/시간별 성과를 유지하고, v267 이후 조건별 성과를 추가한다.
# - 평균회귀 조건값, trade_ready 기준, 청산조건, 자동매수, BUY_READY는 변경하지 않는다.
# ===============================

V267_CONDITION_SCHEMA = "condition_probe_v267"


def _v267_bool_label(ok: bool, good: str = "있음", bad: str = "없음") -> str:
    return good if ok else bad


def _v267_ctx(row: Dict[str, Any]) -> Dict[str, Any]:
    ctx = (row or {}).get("entry_context")
    return ctx if isinstance(ctx, dict) else {}


def _v267_num_from(row: Dict[str, Any], key: str, default: float = 0.0) -> float:
    ctx = _v267_ctx(row)
    return fnum((row or {}).get(key, ctx.get(key, default)), default)


def _v267_build_condition_probe(item: Dict[str, Any]) -> Dict[str, Any]:
    """후보 판단에는 쓰지 않는 기록용 조건표.

    목적: 다른 스캘핑/평균회귀 시스템이 흔히 보는 조건을 후보마다 남겨
    나중에 성과표에서 어떤 조건이 진짜 먹히는지 확인한다.
    여기서 만든 값은 trade_ready/청산/BUY_READY를 바꾸지 않는다.
    """
    ctx = item if isinstance(item, dict) else {}
    vwap_gap = fnum(ctx.get("vwap_gap_pct"), 0.0)
    avg_turn = fnum(ctx.get("avg_price_turn_pct"), 0.0)
    recovery = fnum(ctx.get("recovery_speed_pct"), 0.0)
    low_def = fnum(ctx.get("low_defense_pct"), 0.0)
    ch1 = fnum(ctx.get("change_1"), 0.0)
    ch3 = fnum(ctx.get("change_3"), 0.0)
    ch5 = fnum(ctx.get("change_5"), 0.0)
    spread = fnum(ctx.get("micro_spread_pct"), fnum(ctx.get("orderbook_spread_pct"), 999.0))
    buy = fnum(ctx.get("micro_trade_buy_ratio_30"), 0.0)
    wall = fnum(ctx.get("micro_bid_ask_wall_ratio"), 0.0)
    sell_pressure = bool(ctx.get("micro_sell_trade_pressure") or ctx.get("micro_ask_wall_pressure"))
    micro_fresh = bool(ctx.get("micro_fresh"))
    ws_fresh = bool(ctx.get("ws_fresh"))
    market = ctx.get("market_context") if isinstance(ctx.get("market_context"), dict) else {}
    up_ratio = fnum(ctx.get("market_up_ratio", market.get("market_up_ratio", market.get("up_ratio", 0.0))), 0.0)
    atr = fnum(ctx.get("atr_1m_pct"), 0.0)
    from_low = fnum(ctx.get("from_30m_low_pct"), 0.0)
    below_high = fnum(ctx.get("below_30m_high_pct"), 999.0)
    volume_pump = bool(ctx.get("volume_pump_risk"))
    # 기록용 태그. 일부 수치는 현재 후보 생성 시점에서만 판단되며, 30/60/90초 실제 반응은 재확인 큐/closed 성과에서 따로 봐야 한다.
    reclaim = bool(vwap_gap >= -0.15 or avg_turn >= 0.02)
    reaction = bool(ch1 >= 0.02 or ch3 >= 0.05 or recovery >= 0.05 or avg_turn >= 0.02)
    low_hold = bool(low_def >= 0.10 or from_low >= 0.15)
    flow_recover = bool(ch1 >= 0.02 or ch3 >= 0.05 or ch5 >= 0.10)
    spread_ok = bool(spread <= VWAP_REV_MAX_SPREAD)
    buy_ok = bool(buy >= VWAP_REV_MIN_BUY_RATIO)
    wall_ok = bool(wall >= 1.05) if wall > 0 else False
    fresh_ok = bool(micro_fresh or ws_fresh)
    market_ok = bool(up_ratio <= 0 or up_ratio >= 38.0)
    volatility_ok = bool(atr <= 0 or atr <= FINAL_ATR_HIGH_PCT)
    chase_risk = bool(from_low >= FINAL_FROM_LOW_HOT_PCT or below_high <= FINAL_HIGH_CHASE_GAP_PCT or volume_pump)
    positives = sum(1 for x in [reclaim, reaction, low_hold, flow_recover, spread_ok, buy_ok, wall_ok, fresh_ok, market_ok, volatility_ok] if x)
    negatives = sum(1 for x in [sell_pressure, chase_risk, not spread_ok, not fresh_ok] if x)
    if positives >= 8 and negatives <= 1:
        grade = "A_조건좋음"
    elif positives >= 6 and negatives <= 2:
        grade = "B_관찰우수"
    elif positives >= 4:
        grade = "C_관찰"
    else:
        grade = "D_주의"
    tags: List[str] = []
    if reclaim: tags.append("VWAP회복권")
    if reaction: tags.append("초기반응")
    if low_hold: tags.append("저점방어")
    if flow_recover: tags.append("흐름회복")
    if spread_ok: tags.append("스프레드정상")
    if buy_ok: tags.append("매수비정상")
    if wall_ok: tags.append("호가벽양호")
    if fresh_ok: tags.append("정보신선")
    if market_ok: tags.append("시장무난")
    if sell_pressure: tags.append("매도압력")
    if chase_risk: tags.append("추격주의")
    return {
        "schema": V267_CONDITION_SCHEMA,
        "grade": grade,
        "tags": tags[:12],
        ...
        "spread_ok": spread_ok,
        "buy_pressure_ok": buy_ok,
        "orderbook_wall_ok": wall_ok,
        "fresh_ok": fresh_ok,
        "market_ok": market_ok,
        ...
        "values": {
            "vwap_gap_pct": round(vwap_gap, 3),
            "avg_price_turn_pct": round(avg_turn, 3),
            "recovery_speed_pct": round(recovery, 3),
            "low_defense_pct": round(low_def, 3),
            "change_1": round(ch1, 3),
            "change_3": round(ch3, 3),
            "change_5": round(ch5, 3),
            "spread_pct": round(spread, 3),
            "buy_ratio": round(buy, 3),
            "bid_ask_wall_ratio": round(wall, 3),
            "market_up_ratio": round(up_ratio, 2),
            "atr_1m_pct": round(atr, 3),
        },
        "note": "기록용 조건표. 매수조건/청산/BUY_READY에는 직접 사용하지 않음.",
    }


_v267_base_consume_row = consume_row

def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:  # type: ignore[override]
    row = _v267_base_consume_row(item, lane, ts=ts, scan_id=scan_id)
    try:
        probe = _v267_build_condition_probe(item or {})
        row["condition_probe"] = probe
        row["condition_probe_grade"] = probe.get("grade")
        row["condition_probe_tags"] = probe.get("tags", [])
        ctx = row.get("entry_context")
        if isinstance(ctx, dict):
            ctx["condition_probe"] = probe
            ctx["condition_probe_grade"] = probe.get("grade")
            ctx["condition_probe_tags"] = probe.get("tags", [])
        row["pipeline_note"] = "v2.13.267: condition probe recording only. 조건/청산/BUY_READY 변경 없음."
    except Exception as exc:
        log_error("v267_condition_probe_consume", exc)
    return row


def _v267_probe_from_closed(row: Dict[str, Any]) -> Dict[str, Any]:
    p = (row or {}).get("condition_probe")
    if isinstance(p, dict):
        return p
    ctx = _v267_ctx(row)
    p = ctx.get("condition_probe")
    return p if isinstance(p, dict) else {}


def _v267_probe_has(row: Dict[str, Any], key: str) -> Optional[bool]:
    p = _v267_probe_from_closed(row)
    if not p:
        return None
    val = p.get(key)
...
def _v267_condition_perf_lines(rows: List[Dict[str, Any]], limit: int = 8) -> List[str]:
    # v267 이후 condition_probe가 붙은 CLOSED만 본다.
    probed = [r for r in rows or [] if _v267_probe_from_closed(r)]
    lines = ["[4] 조건별 성과(v267 기록)"]
    if not probed:
        return lines + ["- v267 condition_probe CLOSED 아직 없음", "- 후보에는 기록 중이며 CLOSED가 쌓이면 조건별 승률/수익률 표시"]
    spec = [
        ("VWAP회복", "vwap_reclaim"),
        ("초기반응", "initial_reaction"),
        ("저점방어", "low_hold"),
        ("흐름회복", "flow_recover"),
        ("스프레드정상", "spread_ok"),
        ("매수비정상", "buy_pressure_ok"),
        ("호가벽양호", "orderbook_wall_ok"),
        ("정보신선", "fresh_ok"),
        ("시장무난", "market_ok"),
    ]
    for label, key in spec[:limit]:
        yes = [r for r in probed if _v267_probe_has(r, key) is True]
        no = [r for r in probed if _v267_probe_has(r, key) is False]
        ys = _v266_stat(yes); ns = _v266_stat(no)
        if ys["n"] <= 0 and ns["n"] <= 0:
            continue
        lines.append(
            f"{label:<7} 있음 {int(ys['n']):>3}전 승률 {ys['win_rate']:>5.1f}% 합산 {ys['total']:>+7.2f}%  /  "
            f"없음 {int(ns['n']):>3}전 승률 {ns['win_rate']:>5.1f}% 합산 {ns['total']:>+7.2f}%"
        )
    # grade summary
    grades: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for r in probed:
        g = str(_v267_probe_from_closed(r).get("grade") or "미분류")
        grades[g].append(r)
    if grades:
        lines.append("- 등급별: " + " / ".join(f"{g} {len(a)}전 { _v266_stat(a)['total']:+.2f}%" for g, a in sorted(grades.items())[:5]))
    return lines


def _v267_candidate_probe_lines(rows: List[Dict[str, Any]], limit: int = 3) -> List[str]:
    lines = ["[5] 후보 조건표(v267 기록)"]
    if not rows:
        return lines + ["- 후보 없음"]
    for r in rows[:limit]:
        p = (r or {}).get("condition_probe")
        if not isinstance(p, dict):
            p = _v267_build_condition_probe(r)
        vals = p.get("values") if isinstance(p.get("values"), dict) else {}
        tags = p.get("tags") if isinstance(p.get("tags"), list) else []
        lines.append(
            f"- {_ticker_from_any(r.get('ticker'))}: {p.get('grade','-')} / "
            f"VWAP {fnum(vals.get('vwap_gap_pct'),0):+.2f}% / 1m {fnum(vals.get('change_1'),0):+.2f}% / "
            f"스프 {fnum(vals.get('spread_pct'),999):.2f}% / 매수 {fnum(vals.get('buy_ratio'),0):.2f} / "
            f"태그 {', '.join(str(x) for x in tags[:4]) if tags else '-'}"
        )
    return lines


# Override score builder: keep v266 performance board, add v267 condition performance board.
def _v251_build_score_text_for_cache() -> str:  # type: ignore[override]
    closed = _v250_closed_rows(limit=30000)
    watch_rows = _v250_watch_rows(3.0)
    focus_rows = _v250_paper_latest_rows()
    ...
    lines = [
        ...
        *_v266_time_perf_lines(closed),
        "",
        *_v267_condition_perf_lines(closed, limit=8),
        "",
        "[5] 전체 누적",
        _v266_perf_line("전체", closed, width=9),
        f"- 판독: {'❌ 자동매매 불가' if bad else '⚠️ 더 보기 필요'}" + (f" ({' / '.join(bad[:2])})" if bad else ""),
        ...
        *_v250_strategy_score_summary_lines(),
        f"- 놓친 후보 {len(missed)} / latest 후보 {len(focus_rows)} / trade_ready {sum(1 for r in focus_rows if _v247_bool_ready(r))}",
        "",
        "다음: 버전별 성과와 v267 조건별 성과를 보고, 좋은 조건만 나중에 매수문턱으로 승격합니다.",
    ]
    return "\n".join(lines)


_v267_prev_quality_text = _v250_quality_text

def _v250_quality_text() -> str:  # type: ignore[override]
    txt = _v267_prev_quality_text()
    try:
        focus_rows = _v250_paper_latest_rows()
        extra = "\n\n" + "\n".join(_v267_candidate_probe_lines(focus_rows, limit=3))
        return txt + extra + "\n- 조건표는 기록용입니다. 매수조건/청산/BUY_READY 변경 없음."
    except Exception as exc:
        log_error("v267_quality_probe_lines", exc)
        return txt


_v267_prev_strategy_watch_text = strategy_watch_text

def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    txt = _v267_prev_strategy_watch_text(full)
    if full:
        return txt
    try:
        focus_rows = _v250_paper_latest_rows()
        return txt + "\n\n" + "\n".join(_v267_candidate_probe_lines(focus_rows, limit=3)) + "\n- v267 조건표는 성과분석용 기록입니다. 지금은 차단조건으로 쓰지 않습니다."
    except Exception as exc:
        log_error("v267_strategy_probe_lines", exc)
        return txt


try:
    STATE["phase_note"] = "v267: 다른 단타 조건을 매수조건으로 넣지 않고 condition_probe로 기록. 버전별/시간별/조건별 성과판 우선. 조건/청산/BUY_READY 변경 없음."
except Exception:
    pass


# ===============================
# v2.13.268: 3전략 모의검증 + 4점수 압축표
# - 평균회귀를 단독 매수전략으로만 보지 않고, 3개 후보전략으로 분리해 paper 후보에 전략명을 남긴다.
# - 조건을 복잡하게 직접 차단조건으로 늘리지 않고, 자리/반응/체결/장세 4점수로 압축해 기록한다.
# - 자동매수/BUY_READY/청산조건/paper_bot 변경 없음.
# ===============================

V268_STRATEGY_VERSION = "v268_three_strategy_probe"
V268_STRATEGIES = {
    "low_rebound_vwap": "저점방어+평균회귀",
    "low_rebound_reaccel": "저점방어+재가속",
    "breakout_retest": "돌파/재돌파 초입",
}
V268_STRATEGY_ORDER = ["low_rebound_vwap", "low_rebound_reaccel", "breakout_retest"]
V268_STRATEGY_MIN_SCORE = float(os.getenv("CLEAN_V268_STRATEGY_MIN_SCORE", "3.00"))
V268_PER_STRATEGY_MAX = int(os.getenv("CLEAN_V268_PER_STRATEGY_MAX", "10"))
V268_SHADOW_MAX = int(os.getenv("CLEAN_V268_SHADOW_MAX", "24"))
V268_POOL_LIMIT = int(os.getenv("CLEAN_V268_POOL_LIMIT", "280"))


def _v268_num(row: Dict[str, Any], *keys: str, default: float = 0.0) -> float:
    for k in keys:
        if isinstance(row, dict) and k in row:
            v = fnum(row.get(k), default)
            if v != default or str(row.get(k, "")).strip() not in {"", "0", "0.0", "None"}:
                return v
    ctx = (row or {}).get("entry_context") if isinstance((row or {}).get("entry_context"), dict) else {}
    for k in keys:
        if isinstance(ctx, dict) and k in ctx:
            v = fnum(ctx.get(k), default)
            if v != default or str(ctx.get(k, "")).strip() not in {"", "0", "0.0", "None"}:
                return v
    return default


def _v268_common_probe_scores(row: Dict[str, Any]) -> Dict[str, Any]:
    vgap = _v250_vwap_gap(row)
    m3 = _v250_money3(row)
    m1 = _v268_num(row, "money_flow_1m", "turnover_1m", default=0.0)
    ch1 = _v268_num(row, "change_1", default=0.0)
    ch3 = _v268_num(row, "change_3", default=0.0)
    ch5 = _v268_num(row, "change_5", default=0.0)
    from_low = _v268_num(row, "from_30m_low_pct", "from_low_pct", default=0.0)
    below_high = _v268_num(row, "below_30m_high_pct", "below_high_pct", default=999.0)
    low_def = _v268_num(row, "low_defense_pct", default=0.0)
    recovery = _v268_num(row, "recovery_speed_pct", default=0.0)
    avg_turn = _v268_num(row, "avg_price_turn_pct", default=0.0)
    rebreak = _v268_num(row, "rebreakout_strength", default=0.0)
    pullq = _v268_num(row, "pullback_quality_score", default=0.0)
    spread = _v250_micro_spread(row)
    buy = _v250_micro_buy_ratio(row)
    wall = _v268_num(row, "micro_bid_ask_wall_ratio", "bid_ask_wall_ratio", default=0.0)
    atr = _v268_num(row, "atr_1m_pct", default=0.0)
    micro_fresh = _v250_is_micro_fresh(row, max_age=12.0)
    ws_fresh = bool(_ctxv(row, "ws_fresh", row.get("ws_fresh")))
    volume_pump = bool(row.get("volume_pump_risk")) or _v268_num(row, "volume_spike_30x", default=0.0) >= 25
    market = row.get("market_context") if isinstance(row.get("market_context"), dict) else {}
    up_ratio = _v268_num(row, "market_up_ratio", default=fnum(market.get("market_up_ratio", market.get("up_ratio", 0)), 0.0))
    ...
    if -1.50 <= vgap <= -0.25:
        ...
        seat += 1
    if 0.08 <= from_low <= 4.50 or low_def >= 0.10:
        seat += 2
    elif from_low > 0:
        seat += 1
    if below_high >= 0.25:
    ...
    reaction = 0
    if ch1 >= 0.02: reaction += 1
    if ch3 >= 0.05: reaction += 1
    if ch5 >= 0.10: reaction += 1
    if recovery >= 0.05 or avg_turn >= 0.02: reaction += 1
    if rebreak >= 0.35 or pullq >= 0.35: reaction += 1

    micro = 0
    if micro_fresh or ws_fresh: micro += 1
    if spread <= VWAP_REV_MAX_SPREAD: micro += 1
    if buy >= VWAP_REV_MIN_BUY_RATIO: micro += 1
    if wall >= 1.05: micro += 1

    market_score = 0
    if up_ratio <= 0 or up_ratio >= 38: market_score += 1
    if atr <= 0 or atr <= FINAL_ATR_HIGH_PCT: market_score += 1
    if not volume_pump: market_score += 1

    return {
        "seat_score": int(seat), "reaction_score": int(reaction), "micro_score": int(micro), "market_score": int(market_score),
        "vwap_gap_pct": vgap, "money_3m": m3, "money_1m": m1, "change_1": ch1, "change_3": ch3, "change_5": ch5,
        "from_low_pct": from_low, "below_high_pct": below_high, "low_defense_pct": low_def,
        "recovery_speed_pct": recovery, "avg_price_turn_pct": avg_turn, "rebreakout_strength": rebreak,
        "pullback_quality_score": pullq, "spread_pct": spread, "buy_ratio": buy, "wall_ratio": wall,
        "micro_fresh": micro_fresh, "ws_fresh": ws_fresh, "market_up_ratio": up_ratio, "atr_1m_pct": atr,
        "volume_pump_risk": volume_pump,
    }


def _v268_score_strategy(row: Dict[str, Any], key: str) -> Tuple[float, List[str], List[str]]:
    v = _v268_common_probe_scores(row)
    reasons: List[str] = []
    blocks: List[str] = []
    score = 0.0
    price = _v268_num(row, "current_price", "price", "entry_price", default=0.0)
    if price <= 0:
        return 0.0, [], ["가격 없음"]
    m3 = float(v["money_3m"])
    vgap = float(v["vwap_gap_pct"])
    from_low = float(v["from_low_pct"])
    below_high = float(v["below_high_pct"])
    ch1 = float(v["change_1"]); ch3 = float(v["change_3"]); ch5 = float(v["change_5"])
    rebreak = float(v["rebreakout_strength"]); pullq = float(v["pullback_quality_score"])

    if key == "low_rebound_vwap":
        if not (-1.65 <= vgap <= -0.20):
            blocks.append(f"평균가 아래 자리 아님 {vgap:+.2f}%")
        else:
            score += 1.2; reasons.append(f"평균가 아래 {vgap:+.2f}%")
        if not (0.08 <= from_low <= 4.80 or float(v["low_defense_pct"]) >= 0.10):
            blocks.append("저점방어 부족")
        else:
            score += 1.0; reasons.append(f"저점방어 {from_low:.2f}%")
        if m3 < 5_000_000:
            blocks.append(f"3분돈 부족 {krw_m(m3)}")
        else:
            score += min(1.0, m3 / 25_000_000); reasons.append(f"3분돈 {krw_m(m3)}")
        if ch1 >= 0.02 or ch3 >= 0.05 or float(v["avg_price_turn_pct"]) >= 0.02:
            score += 0.7; reasons.append("초기반응 확인")
        else:
            score += 0.15; reasons.append("반응 관찰")

    elif key == "low_rebound_reaccel":
        if not (-1.80 <= vgap <= 0.20):
            blocks.append(f"눌림/회복 자리 아님 {vgap:+.2f}%")
        else:
            score += 0.8; reasons.append(f"눌림 자리 {vgap:+.2f}%")
        if not (0.08 <= from_low <= 5.50 or float(v["low_defense_pct"]) >= 0.10):
            blocks.append("저점방어 부족")
        else:
            score += 0.8; reasons.append("저점 방어")
        if m3 < 7_000_000:
            blocks.append(f"3분돈 부족 {krw_m(m3)}")
        else:
            score += min(1.0, m3 / 30_000_000); reasons.append(f"3분돈 {krw_m(m3)}")
        if ch1 >= 0.03 or ch3 >= 0.08 or float(v["money_1m"]) >= 2_000_000:
            score += 1.0; reasons.append("돈흐름 재유입")
        else:
            blocks.append("재가속 반응 부족")
        if rebreak >= 0.25 or pullq >= 0.25:
            score += 0.4; reasons.append("재돌파/눌림품질 보조")

    elif key == "breakout_retest":
        if m3 < 8_000_000:
            blocks.append(f"돌파 3분돈 부족 {krw_m(m3)}")
        else:
            score += min(1.2, m3 / 35_000_000); reasons.append(f"돌파돈 {krw_m(m3)}")
        if not (-0.45 <= vgap <= 1.25):
            blocks.append(f"돌파/회복 위치 아님 {vgap:+.2f}%")
        else:
            score += 0.7; reasons.append(f"평균가 회복권 {vgap:+.2f}%")
        if ch1 >= 0.08 or ch3 >= 0.18 or rebreak >= 0.35:
            score += 1.1; reasons.append("돌파 반응")
        else:
            blocks.append("돌파 반응 부족")
        if from_low <= 7.0:
            score += 0.4; reasons.append("과한 추격 아님")
        else:
            blocks.append(f"저점대비 과열 {from_low:.2f}%")
        if below_high <= 0.10 and ch1 < 0.12:
            blocks.append("고점 바로 붙음")

    else:
        return 0.0, [], ["알 수 없는 전략"]

    # 공통 위험. 너무 세게 막지 않되, 펌핑/극단 스프레드는 관찰로 돌린다.
    if bool(v["volume_pump_risk"]):
        blocks.append("거래량 과폭발/펌핑 위험")
    if float(v["spread_pct"]) < 900 and float(v["spread_pct"]) > 1.20:
        blocks.append(f"스프레드 과다 {float(v['spread_pct']):.2f}%")
    # 4점수 가산은 기록압축 기준. 매수조건 직접강화가 아니라 전략 간 비교용이다.
    score += min(1.0, (int(v["seat_score"]) + int(v["reaction_score"]) + int(v["micro_score"]) + int(v["market_score"])) / 12.0)
    return round(score, 3), reasons[:8], blocks[:8]


def _v268_strategy_grade(item: Dict[str, Any], score: float) -> str:
    p = _v268_common_probe_scores(item)
    total = int(p["seat_score"]) + int(p["reaction_score"]) + int(p["micro_score"]) + int(p["market_score"])
    if score >= 4.2 and total >= 11:
        return "A_진입우수"
    if score >= 3.5 and total >= 9:
        return "B_모의진입"
    if score >= V268_STRATEGY_MIN_SCORE:
        return "C_관찰진입"
    return "D_관찰"


def _v268_stamp_strategy_item(item: Dict[str, Any], key: str, score: float, reasons: List[str], blocks: List[str]) -> Dict[str, Any]:
    out = dict(item or {})
    label = V268_STRATEGIES.get(key, key)
    out["strategy"] = label
    out["strategy_key"] = key
    out["strategy_label"] = label
    out["strategy_display"] = label
    out["paper_strategy_key"] = key
    out["paper_strategy_label"] = label
    out["paper_route"] = key
    out["route"] = key
    out["watch_kind"] = key
    out["watch_label"] = label
    out["paper_main_mode"] = "v268_three_strategy_mock"
    out["main_mode"] = "v268_three_strategy_mock"
    out["strategy_test_group"] = label
    out["strategy_test_key"] = key
    out["strategy_probe_score"] = round(score, 3)
    out["strategy_probe_reasons"] = list(reasons or [])[:8]
    out["strategy_probe_blocks"] = list(blocks or [])[:8]
    out["condition_probe"] = _v267_build_condition_probe(out) if callable(globals().get("_v267_build_condition_probe")) else {}
    out["four_score"] = _v268_common_probe_scores(out)
    out["four_score_summary"] = {
        "자리": int(out["four_score"].get("seat_score", 0)),
        "반응": int(out["four_score"].get("reaction_score", 0)),
        "체결": int(out["four_score"].get("micro_score", 0)),
        "장세": int(out["four_score"].get("market_score", 0)),
    }
    out["strategy_grade"] = _v268_strategy_grade(out, score)
    out["score"] = max(fnum(out.get("score"), 0), round(4.10 + min(1.25, score * 0.25), 3))
    out["edge_score"] = out["score"]
    out["one_liner"] = f"{label}: " + (" / ".join(list(reasons or [])[:5]) if reasons else "조건 통과")
    out["mainline_note"] = "v268 3전략 모의검증: 전략명 기록 후 성과 비교"
    out["review_only"] = False
    return out


def _v268_candidate_pool(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    seen: set[str] = set()
    pool: List[Dict[str, Any]] = []
    sortable = [r for r in (rows or []) if isinstance(r, dict)]
    for row in sorted(sortable, key=_v252_row_sort_key, reverse=True):
        t = _v241_watch_ticker(row)
        if not t or t in seen:
            continue
        seen.add(t)
        ...
        ch1 = _v268_num(row, "change_1", default=0.0)
        ch3 = _v268_num(row, "change_3", default=0.0)
        # 완전히 죽은/무관한 종목만 초기 제외. 전략 조건은 아래에서 따로 판정한다.
        if m3 < 1_500_000 and abs(ch1) < 0.05 and abs(ch3) < 0.10:
            continue
        if not (-2.30 <= vgap <= 1.60):
            continue
        pool.append(row)
        if len(pool) >= V268_POOL_LIMIT:
            break
    return pool


def _v268_strategy_label_from_row(row: Dict[str, Any]) -> str:
    key = str((row or {}).get("paper_strategy_key") or (row or {}).get("strategy_key") or (row or {}).get("route") or "")
    return V268_STRATEGIES.get(key) or str((row or {}).get("paper_strategy_label") or (row or {}).get("strategy_label") or (row or {}).get("strategy") or key or "전략미기록")


def _v268_is_strategy_row(row: Dict[str, Any]) -> bool:
    key = str((row or {}).get("paper_strategy_key") or (row or {}).get("strategy_key") or (row or {}).get("route") or "")
    ctx = (row or {}).get("entry_context") if isinstance((row or {}).get("entry_context"), dict) else {}
    key2 = str(ctx.get("paper_strategy_key") or ctx.get("strategy_key") or ctx.get("route") or "")
    return key in V268_STRATEGIES or key2 in V268_STRATEGIES


_v268_prev_is_vwap_row = _v250_is_vwap_row

def _v250_is_vwap_row(row: Dict[str, Any]) -> bool:  # type: ignore[override]
    return bool(_v268_is_strategy_row(row) or _v268_prev_is_vwap_row(row))


_v268_prev_build_candidates = build_candidates

def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    pool = _v268_candidate_pool(rows)
    by_strategy: Dict[str, List[Tuple[float, Dict[str, Any], List[str], List[str]]]] = {k: [] for k in V268_STRATEGY_ORDER}
    near: List[Tuple[float, str, Dict[str, Any], List[str], List[str]]] = []
    rejects: Counter = Counter()
    examples: List[Dict[str, Any]] = []
    for item in pool:
        for key in V268_STRATEGY_ORDER:
            s, reasons, blocks = _v268_score_strategy(item, key)
            if s >= V268_STRATEGY_MIN_SCORE and not blocks:
                by_strategy[key].append((s, item, reasons, blocks))
            else:
                if s >= V268_STRATEGY_MIN_SCORE - 0.55 or _v250_money3(item) >= 8_000_000:
                    near.append((s, key, item, reasons, blocks or ["점수/구조 부족"]))
                    if len(examples) < 8:
                        examples.append({"ticker": _v241_watch_ticker(item), "reason": f"{V268_STRATEGIES.get(key,key)}: {(blocks or ['점수 부족'])[0]}", "score": round(s, 2), "line": " / ".join((reasons or blocks or ['-'])[:4])})
    strict: List[Dict[str, Any]] = []
    used: set[str] = set()
    for key in V268_STRATEGY_ORDER:
        arr = by_strategy[key]
        arr.sort(key=lambda x: (x[0], _v250_money3(x[1]), _v268_num(x[1], "change_3", default=0.0)), reverse=True)
        taken = 0
        for s, item, reasons, blocks in arr:
            t = _v241_watch_ticker(item)
            # 같은 티커는 한 스캔에서 대표 전략 하나만 paper에 태운다. 중복 전략은 condition/probe로 남긴다.
            if not t or t in used:
                continue
            strict.append(_v268_stamp_strategy_item(item, key, s, reasons, blocks))
            used.add(t); taken += 1
            if taken >= V268_PER_STRATEGY_MAX:
                break
    # 전체 과다 방지. 단, 전략별 최소 기회를 먼저 준 뒤 점수순으로 정렬한다.
    strict.sort(key=lambda r: (V268_STRATEGY_ORDER.index(str(r.get("strategy_key"))) if str(r.get("strategy_key")) in V268_STRATEGY_ORDER else 99, -fnum(r.get("strategy_probe_score"), 0), -_v250_money3(r)))
    # shadow에는 근접탈락/중복 후보를 복기용으로 남긴다.
    shadow: List[Dict[str, Any]] = []
    seen_shadow: set[str] = set(used)
    near.sort(key=lambda x: (x[0], _v250_money3(x[2])), reverse=True)
    for s, key, item, reasons, blocks in near:
        t = _v241_watch_ticker(item)
        if not t or t in seen_shadow:
            continue
        out = _v268_stamp_strategy_item(item, key, s, reasons, blocks)
        out["review_only"] = True
        out["paper_bot_open"] = False
        out["open_eligible"] = False
        out["trade_ready"] = False
        out["observe_only"] = True
        out["final_entry_action"] = "observe"
        out["final_entry_label"] = f"{V268_STRATEGIES.get(key,key)} 관찰"
        out["quality_label"] = "3전략 근접탈락"
        shadow.append(out)
        seen_shadow.add(t)
        if len(shadow) >= V268_SHADOW_MAX:
            break
    with _state_lock:
        STATE["v268_three_strategy_pool"] = len(pool)
        STATE["v268_three_strategy_strict"] = len(strict)
        STATE["v268_three_strategy_shadow"] = len(shadow)
        STATE["v268_strategy_counts"] = dict(Counter(str(r.get("strategy_key") or "-") for r in strict))
        STATE["v268_note"] = "3전략 모의검증: 조건은 4점수로 압축하고 전략별 성과를 비교"
    try:
        _v241_strategy_watch_update(rows, strict, shadow)
    except Exception as exc:
        log_error("v268_strategy_watch_update", exc)
    return strict, shadow, rejects, examples


_v268_base_consume_row = globals().get("_v250_base_consume_row", consume_row)

def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:  # type: ignore[override]
    key = str((item or {}).get("strategy_key") or (item or {}).get("paper_strategy_key") or (item or {}).get("route") or "")
    if key not in V268_STRATEGIES:
        # v268부터 정식 모의매매는 3전략 후보만 비교한다. 구 평균회귀 단독은 기록/복기로만 둔다.
        return False, "복기 전용", ["v268: 3전략 모의검증 대상 아님"]
    s, reasons, blocks = _v268_score_strategy(item, key)
    fail: List[str] = []
    if s < V268_STRATEGY_MIN_SCORE:
        fail.append(f"전략점수 {s:.2f} < {V268_STRATEGY_MIN_SCORE:.2f}")
    fail.extend(blocks)
    # 여기서는 전략 조건 자체를 과도하게 늘리지 않는다. 극단적 미수집/호가위험만 막고, 나머지는 모의매매로 검증한다.
    spread = _v250_micro_spread(item)
    if spread < 900 and spread > 1.20:
        fail.append(f"스프레드 과다 {spread:.2f}%")
    if not bool(_v250_is_micro_fresh(item, 15.0) or _ctxv(item, "ws_fresh", item.get("ws_fresh"))):
        fail.append("외부정보 미확정")
    if fail:
        return False, f"{V268_STRATEGIES.get(key,key)} 관찰", fail[:8]
    return True, f"{V268_STRATEGIES.get(key,key)} 모의진입", list(reasons or [])[:6]


def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:  # type: ignore[override]
    row = _v268_base_consume_row(item, lane, ts, scan_id)
    key = str((item or {}).get("strategy_key") or (item or {}).get("paper_strategy_key") or (item or {}).get("route") or row.get("strategy_key") or "")
    if key in V268_STRATEGIES:
        s = fnum((item or {}).get("strategy_probe_score"), 0.0)
        reasons = list((item or {}).get("strategy_probe_reasons") or [])
        blocks = list((item or {}).get("strategy_probe_blocks") or [])
        row.update(_v268_stamp_strategy_item(row, key, s, reasons, blocks))
        row["event_id"] = f"clean:{lane}:{key}:{_ticker_from_any(row.get('ticker'))}:{int((ts or now_ts()) // max(60, EVENT_DEDUP_SEC))}"
        row["strategy_test_version"] = V268_STRATEGY_VERSION
        row["paper_strategy_key"] = key
        row["paper_strategy_label"] = V268_STRATEGIES.get(key, key)
        row["entry_strategy_key"] = key
        row["entry_strategy_label"] = V268_STRATEGIES.get(key, key)
        row["strategy"] = V268_STRATEGIES.get(key, key)
        ...
        ctx = dict(ctx)
        ctx.update({
            "strategy_test_version": V268_STRATEGY_VERSION,
            "paper_strategy_key": key,
            "paper_strategy_label": V268_STRATEGIES.get(key, key),
            "entry_strategy_key": key,
            "entry_strategy_label": V268_STRATEGIES.get(key, key),
            "strategy_key": key,
            "route": key,
            "four_score_summary": row.get("four_score_summary", {}),
            "condition_probe": row.get("condition_probe", {}),
        })
        row["entry_context"] = ctx
        row["pipeline_note"] = "v2.13.268: 3전략 모의검증, 전략명/4점수 기록. 자동매수/청산 변경 없음."
    return row


def _v268_strategy_perf_lines(rows: List[Dict[str, Any]]) -> List[str]:
    groups: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for r in rows or []:
        key = str((r or {}).get("paper_strategy_key") or (r or {}).get("strategy_key") or (r or {}).get("route") or "")
        ctx = (r or {}).get("entry_context") if isinstance((r or {}).get("entry_context"), dict) else {}
        key = key or str(ctx.get("paper_strategy_key") or ctx.get("strategy_key") or ctx.get("route") or "")
        if key in V268_STRATEGIES:
            groups[key].append(r)
    lines = ["[4] 전략별 성과(v268 3전략)"]
    if not groups:
        return lines + ["- v268 3전략 CLOSED 아직 없음", "- 신규 CLOSED가 쌓이면 전략별 전적/승률/합산/최대제외 표시"]
    for key in V268_STRATEGY_ORDER:
        lines.append(_v266_perf_line(V268_STRATEGIES.get(key, key), groups.get(key, []), width=14))
    return lines


def _v268_strategy_count_line(rows: List[Dict[str, Any]]) -> str:
    c = Counter(str((r or {}).get("strategy_key") or (r or {}).get("paper_strategy_key") or (r or {}).get("route") or "-") for r in rows or [])
    parts = []
    for key in V268_STRATEGY_ORDER:
        parts.append(f"{V268_STRATEGIES[key]} {c.get(key,0)}")
    return " / ".join(parts)


def _v268_candidate_probe_lines(rows: List[Dict[str, Any]], limit: int = 5) -> List[str]:
    lines = ["[5] 3전략 후보표"]
    if not rows:
        return lines + ["- 후보 없음"]
    for r in rows[:limit]:
        label = _v268_strategy_label_from_row(r)
        fs = (r or {}).get("four_score_summary") if isinstance((r or {}).get("four_score_summary"), dict) else {}
        p = (r or {}).get("condition_probe") if isinstance((r or {}).get("condition_probe"), dict) else _v267_build_condition_probe(r)
        vals = p.get("values") if isinstance(p.get("values"), dict) else {}
        lines.append(
            f"- {_ticker_from_any(r.get('ticker'))}: {label} / "
            f"자리{fs.get('자리','-')} 반응{fs.get('반응','-')} 체결{fs.get('체결','-')} 장세{fs.get('장세','-')} / "
            f"VWAP {fnum(vals.get('vwap_gap_pct'),0):+.2f}% / 스프 {fnum(vals.get('spread_pct'),999):.2f}% / 매수 {fnum(vals.get('buy_ratio'),0):.2f}"
        )
    return lines


# v268 score: 버전/시간/전략/조건을 모두 보되, 복잡한 조건 설명은 줄인다.
def _v251_build_score_text_for_cache() -> str:  # type: ignore[override]
    closed = _v250_closed_rows(limit=30000)
    watch_rows = _v250_watch_rows(3.0)
    focus_rows = _v250_paper_latest_rows()
    ...
    lines = [
        "📊 모의매매 성과 /score",
        f"기준: 3전략 모의검증 / {BOT_VERSION}",
        *_v266_version_perf_lines(closed, limit=7),
        "",
        *_v266_current_version_lines(closed),
        "",
        *_v266_time_perf_lines(closed),
        "",
        *_v268_strategy_perf_lines(closed),
        "",
        *_v267_condition_perf_lines(closed, limit=6),
        "",
        "[6] 전체 누적",
        _v266_perf_line("전체", closed, width=9),
        f"- 판독: {'❌ 자동매매 불가' if bad else '⚠️ 더 보기 필요'}" + (f" ({' / '.join(bad[:2])})" if bad else ""),
        ...
        *_v264_top_loss_cause_lines(closed, 5),
        ...
        "[8] 현재 관찰",
        *_v250_strategy_score_summary_lines(),
        f"- latest 후보 {len(focus_rows)} / trade_ready {sum(1 for r in focus_rows if _v247_bool_ready(r))} / 전략분포: {_v268_strategy_count_line(focus_rows)}",
        f"- 놓친 후보 {len(missed)}",
        "",
        "다음: 3전략별 성과와 4점수 조건별 성과를 보고, 좋은 조합만 나중에 매수문턱으로 승격합니다.",
    ]
    return "\n".join(lines)


_v268_prev_quality_text = _v250_quality_text

def _v250_quality_text() -> str:  # type: ignore[override]
    txt = _v268_prev_quality_text()
    try:
        focus_rows = _v250_paper_latest_rows()
        extra = "\n\n" + "\n".join(_v268_candidate_probe_lines(focus_rows, limit=5))
        extra += "\n- 전략명은 paper 후보/장부에 함께 저장됩니다. 자동매수/청산 변경 없음."
        return txt + extra
    except Exception as exc:
        log_error("v268_quality_strategy_lines", exc)
        return txt


_v268_prev_strategy_watch_text = strategy_watch_text

def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    txt = _v268_prev_strategy_watch_text(full)
    if full:
        return txt
    try:
        focus_rows = _v250_paper_latest_rows()
        return txt + "\n\n" + "\n".join(_v268_candidate_probe_lines(focus_rows, limit=5)) + "\n- v268: 3전략 모의검증. 조건은 4점수로 압축해 기록하고, 성과 좋은 조합만 나중에 승격합니다."
    except Exception as exc:
        log_error("v268_strategy_watch_lines", exc)
        return txt


try:
    STRATEGY_NAME = "3전략 모의검증"
    STRATEGY_KEY = "three_strategy_probe_v268"
    STATE["phase_note"] = "v268: 저점방어+평균회귀 / 저점방어+재가속 / 돌파·재돌파 초입 3전략 모의검증. 조건은 자리/반응/체결/장세 4점수로 압축. 자동매수/청산 변경 없음."
except Exception:
    pass

# ===============================
# v2.13.269: 3전략 모의매매 알림/표시 정리
# - 3전략은 계속 paper OPEN으로 성과를 본다. 표본 확보가 목적이다.
# - 단, 실제 자동매매급처럼 보이지 않게 experiment_paper/실험진입 표시를 남긴다.
# - 알림은 paper_bot v0.58에서 실험진입 즉시알림을 줄이고 시간요약 중심으로 본다.
# - 전략조건/청산/BUY_READY 변경 없음.
# ===============================

V269_VERSION_TAG = "v269_three_strategy_experiment_display"

try:
    _v269_prev_stamp_strategy_item = _v268_stamp_strategy_item
except Exception:
    _v269_prev_stamp_strategy_item = None

def _v268_stamp_strategy_item(item: Dict[str, Any], key: str, score: float, reasons: List[str], blocks: List[str]) -> Dict[str, Any]:  # type: ignore[override]
    out = _v269_prev_stamp_strategy_item(item, key, score, reasons, blocks) if callable(_v269_prev_stamp_strategy_item) else dict(item or {})
    label = V268_STRATEGIES.get(key, key)
    out["strategy_test_version"] = V269_VERSION_TAG
    out["experiment_paper"] = True
    out["paper_experiment"] = True
    out["paper_alert_tier"] = "summary"
    out["paper_alert_policy"] = "실험진입은 즉시알림 대신 시간요약"
    out["paper_entry_class"] = "3전략 실험진입"
    out["entry_class"] = "experiment"
    out["paper_strategy_key"] = key
    out["paper_strategy_label"] = label
    out["entry_strategy_key"] = key
    out["entry_strategy_label"] = label
    out["strategy"] = label
    out["strategy_key"] = key
    out["route"] = key
    out["paper_route"] = key
    # trade_ready 자체는 모의매매 표본 확보용으로 유지한다. 다만 출력/알림상 정식 자동매매급과 분리한다.
    if bool(out.get("trade_ready") or out.get("paper_bot_open") or out.get("open_eligible")):
        out["final_entry_action"] = "experiment_open"
        out["final_entry_label"] = f"3전략 실험진입: {label}"
        out["trade_ready_label"] = f"실험진입: {label}"
    ctx = out.get("entry_context") if isinstance(out.get("entry_context"), dict) else {}
    ctx = dict(ctx)
    ctx.update({
        "strategy_test_version": V269_VERSION_TAG,
        "experiment_paper": True,
        "paper_experiment": True,
        "paper_alert_tier": "summary",
        "paper_alert_policy": "실험진입은 즉시알림 대신 시간요약",
        "paper_entry_class": "3전략 실험진입",
        "entry_class": "experiment",
        "paper_strategy_key": key,
        "paper_strategy_label": label,
        "entry_strategy_key": key,
        "entry_strategy_label": label,
    })
    out["entry_context"] = ctx
    out["mainline_note"] = "v269: 3전략은 모의매매 성과확인용 실험진입으로 표시. 조건/청산/BUY_READY 변경 없음."
    return out

try:
    _v269_prev_decide_trade_ready = decide_trade_ready
except Exception:
    _v269_prev_decide_trade_ready = None

def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:  # type: ignore[override]
    ok, label, reasons = _v269_prev_decide_trade_ready(item) if callable(_v269_prev_decide_trade_ready) else (False, "판정불가", [])
    key = str((item or {}).get("strategy_key") or (item or {}).get("paper_strategy_key") or (item or {}).get("route") or "")
    if key in V268_STRATEGIES:
        if ok:
            return True, f"3전략 실험진입: {V268_STRATEGIES.get(key,key)}", list(reasons or [])[:8]
        return False, f"3전략 관찰: {V268_STRATEGIES.get(key,key)}", list(reasons or [])[:8]
    return ok, label, reasons

try:
    _v269_prev_consume_row = consume_row
except Exception:
    _v269_prev_consume_row = None

def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:  # type: ignore[override]
    row = _v269_prev_consume_row(item, lane, ts, scan_id) if callable(_v269_prev_consume_row) else dict(item or {})
    key = str((row or {}).get("paper_strategy_key") or (row or {}).get("strategy_key") or (row or {}).get("route") or "")
    if key in V268_STRATEGIES:
        label = V268_STRATEGIES.get(key, key)
        row["strategy_test_version"] = V269_VERSION_TAG
        row["experiment_paper"] = True
        row["paper_experiment"] = True
        row["paper_alert_tier"] = "summary"
        row["paper_entry_class"] = "3전략 실험진입"
        row["paper_strategy_key"] = key
        row["paper_strategy_label"] = label
        row["entry_strategy_key"] = key
        row["entry_strategy_label"] = label
        if bool(row.get("trade_ready") or row.get("paper_bot_open") or row.get("open_eligible")):
            row["final_entry_action"] = "experiment_open"
            row["final_entry_label"] = f"3전략 실험진입: {label}"
            row["trade_ready_label"] = f"실험진입: {label}"
        ctx = row.get("entry_context") if isinstance(row.get("entry_context"), dict) else {}
        ctx = dict(ctx)
        ctx.update({
            "strategy_test_version": V269_VERSION_TAG,
        ...
        row["entry_context"] = ctx
        row["pipeline_note"] = "v2.13.269: 3전략 모의매매는 유지하되 실험진입/시간요약 알림으로 분리."
    return row

try:
    _v269_prev_candidate_probe_lines = _v268_candidate_probe_lines
except Exception:
    _v269_prev_candidate_probe_lines = None

def _v268_candidate_probe_lines(rows: List[Dict[str, Any]], limit: int = 5) -> List[str]:  # type: ignore[override]
    lines = ["[5] 3전략 실험진입 후보표"]
    if not rows:
        return lines + ["- 후보 없음"]
    shown = 0
    for r in rows[:limit]:
        label = _v268_strategy_label_from_row(r)
        fs = (r or {}).get("four_score_summary") if isinstance((r or {}).get("four_score_summary"), dict) else {}
        p = (r or {}).get("condition_probe") if isinstance((r or {}).get("condition_probe"), dict) else (_v267_build_condition_probe(r) if callable(globals().get("_v267_build_condition_probe")) else {})
        vals = p.get("values") if isinstance(p.get("values"), dict) else {}
        ready = "실험OPEN" if bool((r or {}).get("trade_ready") or (r or {}).get("paper_bot_open") or (r or {}).get("open_eligible")) else "관찰"
        risk = (r or {}).get("quality_risk_tags") or ((r or {}).get("entry_context") if isinstance((r or {}).get("entry_context"), dict) else {}).get("quality_risk_tags") or []
        risk_txt = ",".join(str(x) for x in risk[:2]) if isinstance(risk, list) and risk else "-"
        lines.append(
            f"- {_ticker_from_any(r.get('ticker'))}: {ready} / {label} / "
            f"자리{fs.get('자리','-')} 반응{fs.get('반응','-')} 체결{fs.get('체결','-')} 장세{fs.get('장세','-')} / "
            f"VWAP {fnum(vals.get('vwap_gap_pct'),0):+.2f}% / 스프 {fnum(vals.get('spread_pct'),999):.2f}% / 매수 {fnum(vals.get('buy_ratio'),0):.2f} / 위험 {risk_txt}"
        )
        shown += 1
    lines.append("- v269: 모의매매 표본 확보용 실험진입입니다. 즉시알림은 줄이고 전략별 성과로 판단합니다.")
    return lines

try:
    STRATEGY_NAME = "3전략 실험진입"
    STRATEGY_KEY = "three_strategy_probe_v269"
    STATE["phase_note"] = "v269: 3전략 모의매매는 유지. 실험진입/전략별 성과/위험태그 표시를 분리하고 즉시알림은 paper_bot v0.58에서 요약 중심으로 전환."
except Exception:
    pass


# ===============================
# v2.13.271: 3전략 개수상한 대신 외부표준조건 선별 문턱
# - 사용자가 전략개수 상한보다, 다른 단타/스캘핑 프로그램들이 많이 쓰는 조건을 선별해 넣는 방향을 요청했다.
# - v270의 전략별/전체 개수 상한 방식은 사용하지 않는다.
# - v269의 3전략 모의매매/실험진입 구조를 유지하되, RSI회복·볼린저복귀·EMA회복·거래량확인·캔들회복·유동성확인·쿨다운을 선별조건으로 본다.
# - 조건을 과하게 늘리지 않기 위해 4점수(자리/반응/체결/장세) 안에 접어 넣고, 모의진입 문턱만 최소 보강한다.
# - 자동매수/BUY_READY/청산조건 변경 없음.
# ===============================

V271_VERSION_TAG = "v271_external_condition_gate"
V271_MIN_EXTERNAL_CONFIRM = int(os.getenv("CLEAN_V271_MIN_EXTERNAL_CONFIRM", "2"))
V271_COOLDOWN_MIN = float(os.getenv("CLEAN_V271_COOLDOWN_MIN", "30"))
V271_COOLDOWN_LOSS_PCT = float(os.getenv("CLEAN_V271_COOLDOWN_LOSS_PCT", "-0.45"))
V271_RSI_LOW = float(os.getenv("CLEAN_V271_RSI_LOW", "28"))
V271_RSI_HIGH = float(os.getenv("CLEAN_V271_RSI_HIGH", "58"))
V271_VOLUME_MIN = float(os.getenv("CLEAN_V271_VOLUME_MIN", "1.15"))
V271_VOLUME_PUMP_MAX = float(os.getenv("CLEAN_V271_VOLUME_PUMP_MAX", "12.0"))
V271_MAX_SPREAD = float(os.getenv("CLEAN_V271_MAX_SPREAD", "0.70"))

_V271_RECENT_LOSS_CACHE: Dict[str, Any] = {"ts": 0.0, "rows": {}}


def _v271_num(row: Dict[str, Any], *keys: str, default: float = 0.0) -> float:
    for k in keys:
        if isinstance(row, dict) and k in row:
            v = fnum(row.get(k), default)
            if v != default or str(row.get(k, "")).strip() not in {"", "0", "0.0", "None"}:
    ...
    for k in keys:
        if isinstance(ctx, dict) and k in ctx:
            ...
            if v != default or str(ctx.get(k, "")).strip() not in {"", "0", "0.0", "None"}:
                return v
    p = (row or {}).get("condition_probe") if isinstance((row or {}).get("condition_probe"), dict) else {}
    vals = p.get("values") if isinstance(p.get("values"), dict) else {}
    for k in keys:
        if k in vals:
            return fnum(vals.get(k), default)
    return default


def _v271_strategy_key(row: Dict[str, Any]) -> str:
    ctx = (row or {}).get("entry_context") if isinstance((row or {}).get("entry_context"), dict) else {}
...
def _v271_recent_losses() -> Dict[str, Dict[str, Any]]:
    try:
        ts = now_ts()
        if ts - float(_V271_RECENT_LOSS_CACHE.get("ts") or 0) < 20:
            return _V271_RECENT_LOSS_CACHE.get("rows") or {}
        out: Dict[str, Dict[str, Any]] = {}
        cutoff = ts - V271_COOLDOWN_MIN * 60.0
        for r in load_closed(limit=900):
            t = _ticker_from_any((r or {}).get("ticker"))
            if not t:
                continue
            cts = closed_ts(r) if callable(globals().get("closed_ts")) else fnum((r or {}).get("closed_at"), 0)
            if cts < cutoff:
                continue
            pnl = fnum((r or {}).get("pnl_pct", (r or {}).get("profit_pct", (r or {}).get("result_pct", 0))), 0.0)
            if pnl <= V271_COOLDOWN_LOSS_PCT:
                old = out.get(t)
                if not old or cts > fnum(old.get("closed_at"), 0):
                    out[t] = {"closed_at": cts, "pnl_pct": pnl, "reason": str((r or {}).get("exit_reason") or (r or {}).get("close_reason") or "loss")[:80]}
        _V271_RECENT_LOSS_CACHE["ts"] = ts
        _V271_RECENT_LOSS_CACHE["rows"] = out
        return out
    except Exception:
        return {}


def _v271_external_probe(row: Dict[str, Any]) -> Dict[str, Any]:
    """다른 단타/스캘핑 프로그램에서 흔한 조건을 선별해 기록/문턱화한다.

    핵심은 조건을 많이 늘리는 게 아니라, 기존 4점수에 접어 넣는 것이다.
    """
    ch1 = _v271_num(row, "change_1", default=0.0)
    ch3 = _v271_num(row, "change_3", default=0.0)
    avg_turn = _v271_num(row, "avg_price_turn_pct", default=0.0)
    rsi = _v271_num(row, "rsi_14", default=50.0)
    ema5 = _v271_num(row, "ema5_gap_pct", default=0.0)
    ema12 = _v271_num(row, "ema12_gap_pct", default=0.0)
    bb_pos = _v271_num(row, "bb_position", default=0.5)
    bb_lower_gap = _v271_num(row, "bb_lower_gap_pct", default=0.0)
    bb_mid_gap = _v271_num(row, "bb_middle_gap_pct", default=0.0)
    vol_ratio = _v271_num(row, "vol_ratio", default=0.0)
    vol_spike = _v271_num(row, "volume_spike_30x", default=0.0)
    lower_wick = _v271_num(row, "current_lower_wick_pct", default=0.0)
    upper_wick = _v271_num(row, "current_upper_wick_pct", default=0.0)
    close_pos = _v271_num(row, "current_close_pos_ratio", default=0.0)
    current_code = str((row or {}).get("current_candle_code") or _ctxv(row, "current_candle_code", ""))
    spread = _v250_micro_spread(row)
    buy = _v250_micro_buy_ratio(row)
    wall = _v271_num(row, "micro_bid_ask_wall_ratio", "bid_ask_wall_ratio", default=0.0)
    micro_fresh = bool(_v250_is_micro_fresh(row, 15.0))
    ws_fresh = bool(_ctxv(row, "ws_fresh", (row or {}).get("ws_fresh")))
    volume_pump = bool((row or {}).get("volume_pump_risk")) or (vol_spike >= V271_VOLUME_PUMP_MAX and vol_spike > 0)

    rsi_recover = bool(V271_RSI_LOW <= rsi <= V271_RSI_HIGH and (ch1 >= 0.02 or avg_turn >= 0.02 or current_code in {"LOWER_RECLAIM", "LIVE_UP"}))
    bb_reclaim = bool((0.0 <= bb_pos <= 0.45 or bool((row or {}).get("bb_lower_touch"))) and (ch1 >= 0.0 or lower_wick >= 0.12 or bb_lower_gap >= -0.10))
    ema_recover = bool(ema5 >= -0.12 and ema12 >= -0.70 and (ch1 >= 0.0 or avg_turn >= 0.02))
    volume_confirm = bool(vol_ratio >= V271_VOLUME_MIN and not volume_pump)
    candle_reclaim = bool(current_code in {"LOWER_RECLAIM", "LIVE_UP"} or (close_pos >= 0.58 and upper_wick < 0.55) or lower_wick >= 0.18)
    liquidity_ok = bool((micro_fresh or ws_fresh) and spread <= V271_MAX_SPREAD and (buy >= 0.45 or wall >= 1.0))

    t = _ticker_from_any((row or {}).get("ticker"))
    recent_loss = _v271_recent_losses().get(t) if t else None
    cooldown_ok = not bool(recent_loss)
    overheat_risk = bool((rsi >= 74 and bb_pos >= 0.85) or volume_pump or (upper_wick >= 0.75 and close_pos < 0.50))

    names = []
    if rsi_recover: names.append("RSI회복")
    if bb_reclaim: names.append("볼린저복귀")
    if ema_recover: names.append("EMA회복")
    if volume_confirm: names.append("거래량확인")
    if candle_reclaim: names.append("캔들회복")
    if liquidity_ok: names.append("유동성확인")
    if cooldown_ok: names.append("쿨다운통과")
    confirm_count = sum(1 for x in [rsi_recover, bb_reclaim, ema_recover, volume_confirm, candle_reclaim, liquidity_ok] if x)
    blocks = []
    if confirm_count < V271_MIN_EXTERNAL_CONFIRM:
        blocks.append(f"외부조건 부족 {confirm_count}/{V271_MIN_EXTERNAL_CONFIRM}")
    if not liquidity_ok:
        blocks.append("유동성/체결 확인 부족")
    if recent_loss:
        blocks.append(f"최근손실쿨다운 {fnum(recent_loss.get('pnl_pct'),0):+.2f}%")
    if overheat_risk:
        blocks.append("과열/펌핑 위험")
    return {
        "schema": "external_scalp_probe_v271",
        "confirm_count": int(confirm_count),
        "names": names,
        "blocks": blocks,
        "pass": bool(confirm_count >= V271_MIN_EXTERNAL_CONFIRM and liquidity_ok and cooldown_ok and not overheat_risk),
        "rsi_recover": rsi_recover,
        "bb_reclaim": bb_reclaim,
        "ema_recover": ema_recover,
        "volume_confirm": volume_confirm,
        "candle_reclaim": candle_reclaim,
        "liquidity_ok": liquidity_ok,
        "cooldown_ok": cooldown_ok,
        "overheat_risk": overheat_risk,
        "recent_loss": recent_loss or {},
        "values": {"rsi": round(rsi, 2), "ema5_gap": round(ema5, 3), "ema12_gap": round(ema12, 3), "bb_pos": round(bb_pos, 3), "bb_mid_gap": round(bb_mid_gap, 3), "vol_ratio": round(vol_ratio, 3), "vol_spike": round(vol_spike, 3), "spread": round(spread, 3), "buy": round(buy, 3), "wall": round(wall, 3)},
    }


try:
    _v271_prev_condition_probe = _v267_build_condition_probe
except Exception:
    _v271_prev_condition_probe = None


def _v267_build_condition_probe(item: Dict[str, Any]) -> Dict[str, Any]:  # type: ignore[override]
    p = _v271_prev_condition_probe(item) if callable(_v271_prev_condition_probe) else {"tags": [], "values": {}}
    ext = _v271_external_probe(item)
    tags = list(p.get("tags") or [])
    tags.extend(ext.get("names") or [])
    if ext.get("blocks"):
        tags.extend(["주의:" + str(x) for x in ext.get("blocks", [])[:3]])
    vals = p.get("values") if isinstance(p.get("values"), dict) else {}
    vals = dict(vals)
    vals.update({"external_confirm": ext.get("confirm_count"), "rsi": ext["values"].get("rsi"), "ema5_gap": ext["values"].get("ema5_gap"), "bb_pos": ext["values"].get("bb_pos"), "vol_ratio": ext["values"].get("vol_ratio")})
    p = dict(p)
    p["schema"] = "condition_probe_v271_external_scalp"
    p["external_scalp_probe"] = ext
    p["tags"] = list(dict.fromkeys(str(x) for x in tags if x))[:18]
    p["values"] = vals
    return p


try:
    _v271_prev_common_scores = _v268_common_probe_scores
except Exception:
    _v271_prev_common_scores = None


def _v268_common_probe_scores(row: Dict[str, Any]) -> Dict[str, Any]:  # type: ignore[override]
    v = _v271_prev_common_scores(row) if callable(_v271_prev_common_scores) else {}
    ext = _v271_external_probe(row)
    # 4점수 압축: 외부조건은 새 점수 칸을 만들지 않고 기존 칸에 소폭만 반영한다.
    try:
        if ext.get("bb_reclaim") or ext.get("cooldown_ok"):
            v["seat_score"] = min(5, int(v.get("seat_score", 0)) + (1 if ext.get("bb_reclaim") else 0))
        if ext.get("rsi_recover") or ext.get("ema_recover") or ext.get("candle_reclaim"):
            v["reaction_score"] = min(5, int(v.get("reaction_score", 0)) + 1)
        if ext.get("liquidity_ok") or ext.get("volume_confirm"):
            v["micro_score"] = min(5, int(v.get("micro_score", 0)) + (1 if ext.get("liquidity_ok") else 0))
        if ext.get("overheat_risk"):
            v["market_score"] = max(0, int(v.get("market_score", 0)) - 1)
    except Exception:
        pass
    v["external_scalp_probe"] = ext
    v["external_confirm"] = ext.get("confirm_count", 0)
    return v


try:
    _v271_prev_stamp_strategy_item = _v268_stamp_strategy_item
except Exception:
    _v271_prev_stamp_strategy_item = None


def _v268_stamp_strategy_item(item: Dict[str, Any], key: str, score: float, reasons: List[str], blocks: List[str]) -> Dict[str, Any]:  # type: ignore[override]
    out = _v271_prev_stamp_strategy_item(item, key, score, reasons, blocks) if callable(_v271_prev_stamp_strategy_item) else dict(item or {})
    ext = _v271_external_probe(out)
    out["strategy_test_version"] = V271_VERSION_TAG
    out["external_scalp_probe"] = ext
    out["external_confirm"] = ext.get("confirm_count", 0)
    out["external_condition_names"] = ext.get("names", [])
    if ext.get("blocks"):
        old = list(out.get("strategy_probe_blocks") or [])
        out["strategy_probe_blocks"] = list(dict.fromkeys(old + list(ext.get("blocks") or [])))[:10]
    ctx = out.get("entry_context") if isinstance(out.get("entry_context"), dict) else {}
    ctx = dict(ctx)
    ctx.update({"strategy_test_version": V271_VERSION_TAG, "external_scalp_probe": ext, "external_condition_names": ext.get("names", []), "external_confirm": ext.get("confirm_count", 0)})
    out["entry_context"] = ctx
    out["mainline_note"] = "v271: 전략개수 상한이 아니라 RSI/볼린저/EMA/거래량/유동성/쿨다운 선별조건으로 실험진입 품질을 본다."
    return out


try:
    _v271_prev_build_candidates = build_candidates
except Exception:
    _v271_prev_build_candidates = None


def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    strict, shadow, rejects, examples = _v271_prev_build_candidates(rows) if callable(_v271_prev_build_candidates) else ([], [], Counter(), [])
    kept: List[Dict[str, Any]] = []
    moved: List[Dict[str, Any]] = []
    for r in strict or []:
        key = _v271_strategy_key(r)
        if key not in V268_STRATEGIES:
            kept.append(r)
            continue
        ext = _v271_external_probe(r)
        if ext.get("pass"):
            r["v271_external_pass"] = True
            r["paper_entry_class"] = "3전략 실험진입(외부조건통과)"
            r["paper_alert_tier"] = "summary"
            kept.append(r)
        else:
            nr = dict(r)
            nr["v271_external_pass"] = False
            nr["review_only"] = True
            nr["observe_only"] = True
            nr["paper_bot_open"] = False
            nr["open_eligible"] = False
            nr["trade_ready"] = False
            nr["final_entry_action"] = "observe"
            nr["final_entry_label"] = "3전략 관찰: 외부표준조건 미통과"
            nr["trade_ready_label"] = "외부조건 대기"
            nr["external_scalp_probe"] = ext
            nr["quality_label"] = "외부조건 관찰전환"
            moved.append(nr)
    shadow = list(shadow or []) + moved[:max(0, V268_SHADOW_MAX - len(shadow or []))]
    with _state_lock:
        STATE["v271_external_open"] = sum(1 for x in kept if _v271_strategy_key(x) in V268_STRATEGIES)
        STATE["v271_external_observe"] = len(moved)
        STATE["v271_note"] = "전략개수 상한 없이 외부표준조건으로 실험진입 품질 필터"
    return kept, shadow, rejects, examples


try:
    _v271_prev_decide_trade_ready = decide_trade_ready
except Exception:
    _v271_prev_decide_trade_ready = None


def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:  # type: ignore[override]
    ok, label, reasons = _v271_prev_decide_trade_ready(item) if callable(_v271_prev_decide_trade_ready) else (False, "판정불가", [])
    key = _v271_strategy_key(item)
    if key in V268_STRATEGIES:
        ext = _v271_external_probe(item)
        if not ext.get("pass"):
            merged = list(reasons or []) + list(ext.get("blocks") or [])
            return False, f"3전략 관찰: {V268_STRATEGIES.get(key,key)} 외부조건 미통과", list(dict.fromkeys(str(x) for x in merged))[:8]
        if ok:
            names = ",".join((ext.get("names") or [])[:3]) or "외부조건"
            return True, f"3전략 실험진입: {V268_STRATEGIES.get(key,key)} / {names}", list(reasons or [])[:8]
    return ok, label, reasons


try:
    _v271_prev_consume_row = consume_row
except Exception:
    _v271_prev_consume_row = None


def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:  # type: ignore[override]
    row = _v271_prev_consume_row(item, lane, ts, scan_id) if callable(_v271_prev_consume_row) else dict(item or {})
    key = _v271_strategy_key(row)
    if key in V268_STRATEGIES:
        ext = _v271_external_probe(row)
        row["strategy_test_version"] = V271_VERSION_TAG
        row["external_scalp_probe"] = ext
        row["external_confirm"] = ext.get("confirm_count", 0)
        row["external_condition_names"] = ext.get("names", [])
        if not ext.get("pass"):
            row["paper_bot_open"] = False
            row["open_eligible"] = False
            row["trade_ready"] = False
            row["final_entry_action"] = "observe"
            row["final_entry_label"] = "3전략 관찰: 외부표준조건 미통과"
            row["trade_ready_label"] = "외부조건 대기"
        else:
            row["paper_entry_class"] = "3전략 실험진입(외부조건통과)"
            row["paper_alert_tier"] = "summary"
        ctx = row.get("entry_context") if isinstance(row.get("entry_context"), dict) else {}
        ctx = dict(ctx)
        ctx.update({"strategy_test_version": V271_VERSION_TAG, "external_scalp_probe": ext, "external_confirm": ext.get("confirm_count", 0), "external_condition_names": ext.get("names", [])})
        row["entry_context"] = ctx
        row["pipeline_note"] = "v2.13.271: RSI/볼린저/EMA/거래량/유동성/쿨다운 선별조건. 전략개수 상한/자동매수/청산 변경 없음."
    return row


try:
    _v271_prev_candidate_probe_lines = _v268_candidate_probe_lines
except Exception:
    _v271_prev_candidate_probe_lines = None


def _v268_candidate_probe_lines(rows: List[Dict[str, Any]], limit: int = 5) -> List[str]:  # type: ignore[override]
    lines = ["[5] 3전략 후보표(v271 외부조건)"]
    if not rows:
        return lines + ["- 후보 없음"]
    for r in rows[:limit]:
        label = _v268_strategy_label_from_row(r)
        ...
        ext = _v271_external_probe(r)
        ready = "실험OPEN" if bool((r or {}).get("trade_ready") or (r or {}).get("paper_bot_open") or (r or {}).get("open_eligible")) else "관찰"
        names = ",".join((ext.get("names") or [])[:4]) or "-"
        blocks = ",".join((ext.get("blocks") or [])[:2]) or "-"
        vals = ext.get("values") if isinstance(ext.get("values"), dict) else {}
        lines.append(
            f"- {_ticker_from_any(r.get('ticker'))}: {ready} / {label} / "
            f"자리{fs.get('자리','-')} 반응{fs.get('반응','-')} 체결{fs.get('체결','-')} 장세{fs.get('장세','-')} / "
            f"외부{ext.get('confirm_count',0)}개 {names} / RSI {fnum(vals.get('rsi'),0):.1f} / EMA5 {fnum(vals.get('ema5_gap'),0):+.2f}% / BB {fnum(vals.get('bb_pos'),0):.2f} / 문제 {blocks}"
        )
    lines.append("- v271: 전략 개수상한이 아니라 RSI/볼린저/EMA/거래량/유동성/쿨다운 조건으로 실험진입 품질을 거릅니다.")
    return lines

try:
    STRATEGY_NAME = "3전략 외부조건 실험"
    STRATEGY_KEY = "three_strategy_external_gate_v271"
    STATE["phase_note"] = "v271: 전략개수 상한 대신 RSI/볼린저/EMA/거래량/유동성/쿨다운 선별조건으로 3전략 모의매매 품질을 개선."
except Exception:
    pass


# ===============================
# v2.13.272: 캐시 버전 불일치 수술 정리
# - v271 실행 중 /score, /quality가 v269 캐시 본문을 보여 판단이 꼬인 문제를 끊는다.
# - 기본 명령어는 구 직접계산 fallback을 타지 않는다.
# - 캐시 버전/전략/종류가 현재와 다르면 과거 본문을 숨기고 현재판 캐시 준비 상태만 표시한다.
# - 3전략/외부조건/자동매수/청산/BUY_READY 변경 없음.
# ===============================

V272_CACHE_SURGERY = "v272_hide_stale_command_cache"

def _v272_cache_is_current(obj: Any, name: str) -> Tuple[bool, str]:
    if not isinstance(obj, dict) or not str(obj.get("text") or "").strip():
        return False, "캐시 없음"
    cv = str(obj.get("version") or "-")
    if cv != BOT_VERSION:
        return False, f"캐시버전 불일치 {cv}"
    sk = str(obj.get("strategy_key") or "")
    if sk and sk != SCORE_CACHE_STRATEGY_KEY:
        return False, f"전략키 불일치 {sk}"
    nm = str(obj.get("name") or "")
    if nm and nm != name.strip("/") and nm != name:
        return False, f"캐시종류 불일치 {nm}"
    return True, ""


def _v263_cache_display_text(title: str, command_name: str, obj: Dict[str, Any], why: str) -> str:  # type: ignore[override]
    """v272: 과거 버전 캐시 본문을 기본 화면에 다시 노출하지 않는다.

    v263~v266에서는 마지막 정상 캐시를 보여줘 빈 화면을 피했지만,
    v271에서 v269 캐시 본문이 현재 결과처럼 보이는 문제가 생겼다.
    이제 버전이 다르면 본문을 숨기고 현재판 캐시 준비 상태만 보여준다.
    """
    obj = obj if isinstance(obj, dict) else {}
    current, current_why = _v272_cache_is_current(obj, command_name)
    text = str(obj.get("text") or "").strip()
    if current and text:
        cv = str(obj.get("version") or BOT_VERSION)
        ts = str(obj.get("updated_text") or "-")
        return text + f"\n\n[캐시] {cv} / {ts} / 갱신대기: {why or '-'}"
    cv = str(obj.get("version") or "-")
    ts = str(obj.get("updated_text") or "-")
    reason = current_why or why or "캐시 준비중"
    lines = [
        title,
        f"❔ 현재판 캐시 준비중",
        f"- 현재버전: {BOT_VERSION}",
        f"- 이전캐시: {cv} / {ts}",
        f"- 사유: {reason}",
        "- 과거 버전 캐시 본문은 현재 결과처럼 보이지 않게 숨깁니다.",
        "- 기본 명령어는 구 직접계산 경로를 타지 않습니다.",
    ]
    return "\n".join(lines)

try:
    STATE["phase_note"] = "v272: v269/v271 캐시 혼선 차단. 과거 캐시 본문 숨김, 현재판 캐시만 표시. 전략/청산/BUY_READY 변경 없음."
    STATE["v272_cache_surgery"] = V272_CACHE_SURGERY
except Exception:
    pass


# ===============================
# v2.13.273: 명령어 역할 재수술 + 3전략 OPEN 문턱 정리
# - /score = 성과와 손실/전략별 결과만 본다.
# - /quality = 현재 후보가 왜 OPEN/관찰인지 본다.
# - /strategy_watch = 놓친 후보와 관찰 요약만 빠르게 본다.
# - 3전략은 유지하되, 외부조건+반응+체결이 확인된 후보만 실험OPEN으로 보낸다.
# - 나머지는 shadow/관찰 기록으로 남겨 성과 비교 재료로만 쓴다.
# - 자동매수/BUY_READY/청산조건 변경 없음.
# ===============================

V273_VERSION_TAG = "v273_command_surgery_open_gate"
V273_MIN_EXTERNAL_CONFIRM = int(os.getenv("CLEAN_V273_MIN_EXTERNAL_CONFIRM", "3"))
V273_MIN_REACTION_SCORE = int(os.getenv("CLEAN_V273_MIN_REACTION_SCORE", "3"))
V273_MIN_MICRO_SCORE = int(os.getenv("CLEAN_V273_MIN_MICRO_SCORE", "3"))
V273_MIN_SEAT_SCORE = int(os.getenv("CLEAN_V273_MIN_SEAT_SCORE", "4"))
V273_MIN_MARKET_SCORE = int(os.getenv("CLEAN_V273_MIN_MARKET_SCORE", "2"))


def _v273_strategy_key(row: Dict[str, Any]) -> str:
    ctx = (row or {}).get("entry_context") if isinstance((row or {}).get("entry_context"), dict) else {}
    for k in ("paper_strategy_key", "entry_strategy_key", "strategy_key", "route", "paper_route"):
        v = (row or {}).get(k)
        if v:
            return str(v)
        if isinstance(ctx, dict) and ctx.get(k):
            return str(ctx.get(k))
    label = str((row or {}).get("paper_strategy_label") or (row or {}).get("strategy_label") or ctx.get("paper_strategy_label") or "")
    for kk, vv in V268_STRATEGIES.items():
        if vv and vv in label:
            return kk
    return ""


def _v273_strategy_label(row: Dict[str, Any]) -> str:
    key = _v273_strategy_key(row)
    if key in V268_STRATEGIES:
        return V268_STRATEGIES[key]
    ctx = (row or {}).get("entry_context") if isinstance((row or {}).get("entry_context"), dict) else {}
    return str((row or {}).get("paper_strategy_label") or (row or {}).get("strategy_label") or ctx.get("paper_strategy_label") or key or "전략미기록")


def _v273_is_three_strategy(row: Dict[str, Any]) -> bool:
    return _v273_strategy_key(row) in V268_STRATEGIES


def _v273_risk_words(row: Dict[str, Any]) -> List[str]:
    vals: List[str] = []
    for k in ("final_entry_label", "trade_ready_label", "quality_label", "pipeline_note", "one_liner"):
        if (row or {}).get(k):
            vals.append(str((row or {}).get(k)))
    for k in ("strategy_probe_blocks", "quality_risk_tags", "condition_tags"):
        v = (row or {}).get(k)
        if isinstance(v, list):
            vals.extend(str(x) for x in v[:12])
        elif v:
            vals.append(str(v))
    ctx = (row or {}).get("entry_context") if isinstance((row or {}).get("entry_context"), dict) else {}
    for k in ("final_entry_label", "trade_ready_label", "quality_risk_tags"):
        v = ctx.get(k)
        if isinstance(v, list):
            vals.extend(str(x) for x in v[:12])
        elif v:
...
def _v273_response_ok(row: Dict[str, Any]) -> bool:
    ch1 = _v271_num(row, "change_1", default=0.0) if callable(globals().get("_v271_num")) else fnum((row or {}).get("change_1"), 0.0)
    ch3 = _v271_num(row, "change_3", default=0.0) if callable(globals().get("_v271_num")) else fnum((row or {}).get("change_3"), 0.0)
    avg_turn = _v271_num(row, "avg_price_turn_pct", default=0.0) if callable(globals().get("_v271_num")) else fnum((row or {}).get("avg_price_turn_pct"), 0.0)
    recovery = _v271_num(row, "recovery_speed_pct", default=0.0) if callable(globals().get("_v271_num")) else fnum((row or {}).get("recovery_speed_pct"), 0.0)
    low_def = _v271_num(row, "low_defense_pct", default=0.0) if callable(globals().get("_v271_num")) else fnum((row or {}).get("low_defense_pct"), 0.0)
    return bool(ch1 >= 0.05 or ch3 >= 0.10 or avg_turn >= 0.03 or recovery >= 0.05 or low_def >= 0.20)


def _v273_open_gate(row: Dict[str, Any]) -> Tuple[bool, List[str]]:
    if not _v273_is_three_strategy(row):
        return False, ["3전략 대상 아님"]
    fs = _v268_common_probe_scores(row) if callable(globals().get("_v268_common_probe_scores")) else {}
    ext = _v271_external_probe(row) if callable(globals().get("_v271_external_probe")) else {"pass": False, "confirm_count": 0, "blocks": ["외부조건 없음"]}
    fail: List[str] = []
    seat = int(fnum(fs.get("seat_score"), 0))
    react = int(fnum(fs.get("reaction_score"), 0))
    micro = int(fnum(fs.get("micro_score"), 0))
    market = int(fnum(fs.get("market_score"), 0))
    confirm = int(fnum(ext.get("confirm_count"), 0))
    if not bool(ext.get("pass")):
        fail.extend([str(x) for x in (ext.get("blocks") or ["외부조건 미통과"] )[:4]])
    if confirm < V273_MIN_EXTERNAL_CONFIRM:
        fail.append(f"외부조건 {confirm}/{V273_MIN_EXTERNAL_CONFIRM}")
    if seat < V273_MIN_SEAT_SCORE:
        fail.append(f"자리 {seat}<{V273_MIN_SEAT_SCORE}")
    if react < V273_MIN_REACTION_SCORE:
        fail.append(f"반응 {react}<{V273_MIN_REACTION_SCORE}")
    if micro < V273_MIN_MICRO_SCORE:
        fail.append(f"체결 {micro}<{V273_MIN_MICRO_SCORE}")
    if market < V273_MIN_MARKET_SCORE:
        fail.append(f"장세 {market}<{V273_MIN_MARKET_SCORE}")
    if not _v273_response_ok(row):
        fail.append("가격반응 부족")
    words = " / ".join(_v273_risk_words(row))
    hard_risks = ["관찰전환", "쓰레기후보", "재확인대기", "stop/밀림", "펌핑 위험", "재돌파힘 약함"]
    for w in hard_risks:
        if w and w in words:
            fail.append(w)
            break
    # extreme spread is not a strategy-count cap; it is a standard execution/liquidity filter.
    spread = _v250_micro_spread(row)
    if spread < 900 and spread > max(0.85, V271_MAX_SPREAD if 'V271_MAX_SPREAD' in globals() else 0.70):
        fail.append(f"스프레드 {spread:.2f}%")
    return (len(fail) == 0), list(dict.fromkeys(str(x) for x in fail if x))[:8]


def _v273_mark_observe(row: Dict[str, Any], reasons: List[str]) -> Dict[str, Any]:
    nr = dict(row or {})
    nr["v273_open_gate"] = False
    nr["v273_gate_reasons"] = list(reasons or [])[:8]
    nr["review_only"] = True
    nr["observe_only"] = True
    nr["paper_bot_open"] = False
    nr["open_eligible"] = False
    nr["trade_ready"] = False
    nr["final_entry_action"] = "observe"
    nr["final_entry_label"] = "3전략 관찰: OPEN 문턱 미통과"
    nr["trade_ready_label"] = "관찰"
    nr["paper_entry_class"] = "3전략 관찰"
    nr["quality_label"] = "3전략 관찰"
    return nr


def _v273_mark_open(row: Dict[str, Any]) -> Dict[str, Any]:
    nr = dict(row or {})
    nr["v273_open_gate"] = True
    nr["v273_gate_reasons"] = []
    nr["review_only"] = False
    nr["observe_only"] = False
    nr["paper_bot_open"] = True
    nr["open_eligible"] = True
    nr["trade_ready"] = True
    nr["paper_entry_class"] = "3전략 실험OPEN"
    nr["paper_alert_tier"] = "summary"
    nr["final_entry_action"] = "experiment_open"
    nr["final_entry_label"] = f"3전략 실험OPEN: {_v273_strategy_label(nr)}"
    nr["trade_ready_label"] = f"실험OPEN: {_v273_strategy_label(nr)}"
    ctx = nr.get("entry_context") if isinstance(nr.get("entry_context"), dict) else {}
    ctx = dict(ctx)
    ctx.update({
        "strategy_test_version": V273_VERSION_TAG,
        "v273_open_gate": True,
        "paper_entry_class": "3전략 실험OPEN",
        "paper_alert_tier": "summary",
        "paper_strategy_key": _v273_strategy_key(nr),
        "paper_strategy_label": _v273_strategy_label(nr),
    })
    nr["entry_context"] = ctx
    return nr


try:
    _v273_prev_build_candidates = build_candidates
except Exception:
    _v273_prev_build_candidates = None


def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    strict, shadow, rejects, examples = _v273_prev_build_candidates(rows) if callable(_v273_prev_build_candidates) else ([], [], Counter(), [])
    strict_out: List[Dict[str, Any]] = []
    moved: List[Dict[str, Any]] = []
    for r in strict or []:
        if not _v273_is_three_strategy(r):
            strict_out.append(r)
            continue
        ok, why = _v273_open_gate(r)
        if ok:
            strict_out.append(_v273_mark_open(r))
        else:
            moved.append(_v273_mark_observe(r, why))
    shadow_out = list(shadow or []) + moved
    with _state_lock:
        STATE["v273_strict_open"] = sum(1 for x in strict_out if _v273_is_three_strategy(x))
        STATE["v273_observe"] = len(moved)
        STATE["v273_gate_note"] = "3전략 유지, 외부조건+반응+체결 확인 후보만 실험OPEN"
    return strict_out, shadow_out, rejects, examples


try:
    _v273_prev_decide_trade_ready = decide_trade_ready
except Exception:
    _v273_prev_decide_trade_ready = None


def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:  # type: ignore[override]
    if _v273_is_three_strategy(item):
        ok, why = _v273_open_gate(item)
        if not ok:
            return False, f"3전략 관찰: {_v273_strategy_label(item)}", why
        return True, f"3전략 실험OPEN: {_v273_strategy_label(item)}", ["외부조건+반응+체결 통과"]
    return _v273_prev_decide_trade_ready(item) if callable(_v273_prev_decide_trade_ready) else (False, "판정불가", [])


try:
    _v273_prev_consume_row = consume_row
except Exception:
    _v273_prev_consume_row = None


def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:  # type: ignore[override]
    row = _v273_prev_consume_row(item, lane, ts, scan_id) if callable(_v273_prev_consume_row) else dict(item or {})
    if _v273_is_three_strategy(row):
        ok, why = _v273_open_gate(row)
        row = _v273_mark_open(row) if ok else _v273_mark_observe(row, why)
        row["pipeline_note"] = "v273: 3전략 유지. 외부조건+반응+체결 통과만 실험OPEN, 나머지는 관찰/복기."
    return row


def _v273_missed_lines(watch_rows: List[Dict[str, Any]], focus_rows: List[Dict[str, Any]], limit: int = 3) -> List[str]:
    missed = _v250_missed_rows(watch_rows, focus_rows) if callable(globals().get("_v250_missed_rows")) else []
    lines = ["[3] 놓친 후보"]
    if not missed:
        return lines + ["- 아직 없음"]
    for r in missed[:limit]:
        lines.append(
            f"- {_ticker_from_any(r.get('ticker'))}: 최고 {fnum(r.get('highest_return_pct'),0):+.2f}% / 현재 {fnum(r.get('current_return_pct'),0):+.2f}% / 탈락 {str(r.get('reject_category') or r.get('reject_reason') or '-')[:28]}"
        )
    return lines


def _v273_strategy_perf_lines(rows: List[Dict[str, Any]]) -> List[str]:
    lines = ["[2] 3전략 성과"]
    groups: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for r in rows or []:
        key = _v273_strategy_key(r)
        if key in V268_STRATEGIES:
            groups[key].append(r)
    if not groups:
        return lines + ["- 3전략 CLOSED 아직 없음"]
    for key in V268_STRATEGY_ORDER:
        lines.append(_v266_perf_line(V268_STRATEGIES.get(key, key), groups.get(key, []), width=14))
    return lines


def _v273_candidate_summary_lines(rows: List[Dict[str, Any]], limit: int = 4) -> List[str]:
    lines = ["[1] 현재 후보"]
    total = len(rows or [])
    ready = [r for r in rows or [] if _v247_bool_ready(r)]
    observe = [r for r in rows or [] if not _v247_bool_ready(r)]
    sc = Counter(_v273_strategy_label(r) for r in rows or [])
    lines.append(f"- 후보 {total} / 실험OPEN {len(ready)} / 관찰 {len(observe)}")
    if sc:
        lines.append("- 전략분포: " + " / ".join(f"{k} {v}" for k, v in sc.most_common(4)))
    shown = 0
    for r in (ready + observe)[:limit]:
        fs = (r or {}).get("four_score_summary") if isinstance((r or {}).get("four_score_summary"), dict) else {}
        gate = "OPEN" if _v247_bool_ready(r) else "관찰"
        why = ",".join(str(x) for x in (r.get("v273_gate_reasons") or [])[:2]) if isinstance(r.get("v273_gate_reasons"), list) else ""
        lines.append(
            f"- {_ticker_from_any(r.get('ticker'))}: {gate} / {_v273_strategy_label(r)} / 자리{fs.get('자리','-')} 반응{fs.get('반응','-')} 체결{fs.get('체결','-')} 장세{fs.get('장세','-')}" + (f" / {why}" if why else "")
        )
        shown += 1
    if total > shown:
        lines.append(f"- 나머지 {total-shown}개는 /quality_full 또는 상세에서 확인")
    return lines


def _v273_loss_top_lines(rows: List[Dict[str, Any]], limit: int = 4) -> List[str]:
    lines = ["[5] 손실 TOP"]
    vals = _v264_top_loss_cause_lines(rows, limit) if callable(globals().get("_v264_top_loss_cause_lines")) else []
    return lines + (vals or ["- 손실 원인 없음"])
...
def _v251_build_score_text_for_cache() -> str:  # type: ignore[override]
    ...
    lines = [
        "📊 성과 요약 /score",
        f"기준: 3전략 실험검증 / {BOT_VERSION}",
        "",
        "[1] 현재버전",
        *_v266_current_version_lines(closed)[1:],
        "",
        *_v273_strategy_perf_lines(closed),
        "",
        *_v273_missed_lines(watch_rows, focus_rows, limit=3),
        "",
        "[4] 시간별",
        _v266_perf_line("3시간", rows_recent_hours(closed, 3), width=6),
        _v266_perf_line("6시간", rows_recent_hours(closed, 6), width=6),
        _v266_perf_line("12시간", rows_recent_hours(closed, 12), width=6),
        "",
        *_v273_loss_top_lines(closed, 4),
        "",
        "[6] 버전별",
        *_v266_version_perf_lines(closed, limit=5)[1:6],
        "",
        "판독: 현재버전·전략별·놓친후보만 먼저 봅니다. 자세한 원인분석은 /reversion_review",
    ]
    return "\n".join(lines)


def _v250_quality_text() -> str:  # type: ignore[override]
    rows = _v250_paper_latest_rows()
    watch_rows = _v250_watch_rows(3.0)
    lines = [
        "🔬 현재 후보 /quality",
        f"기준: {BOT_VERSION}",
        *_v273_candidate_summary_lines(rows, limit=5),
        "",
        *_v273_missed_lines(watch_rows, rows, limit=3),
        "",
        "[4] 외부정보",
    ]
    if not rows:
        ...
        fresh_ws = sum(1 for r in rows if bool(_ctxv(r, "ws_fresh", r.get("ws_fresh"))))
        fresh_micro = sum(1 for r in rows if bool(_v250_is_micro_fresh(r, 15.0)))
        avg_sp = sum(_v250_micro_spread(r) for r in rows if _v250_micro_spread(r) < 900) / max(1, sum(1 for r in rows if _v250_micro_spread(r) < 900))
        lines.append(f"- WS 신선 {fresh_ws}/{len(rows)} / micro 신선 {fresh_micro}/{len(rows)} / 평균스프레드 {avg_sp:.2f}%")
    lines += ["", "판독: 여기는 지금 들어갈 후보와 관찰 후보만 봅니다. 성과는 /score, 놓친 상세는 /strategy_watch"]
    return "\n".join(lines)

try:
    _v273_prev_strategy_watch_text = strategy_watch_text
except Exception:
    _v273_prev_strategy_watch_text = None


def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    if full and callable(_v273_prev_strategy_watch_text):
        return _v273_prev_strategy_watch_text(True)
    rows = _v250_paper_latest_rows()
    watch_rows = _v250_watch_rows(3.0)
    missed = _v250_missed_rows(watch_rows, rows) if callable(globals().get("_v250_missed_rows")) else []
    lines = [
        "🧪 놓친 후보 /strategy_watch",
        f"기준: 최근 3시간 / {BOT_VERSION}",
        f"- 관찰이벤트 {len(watch_rows)} / 현재후보 {len(rows)} / 실험OPEN {sum(1 for r in rows if _v247_bool_ready(r))} / 놓친후보 {len(missed)}",
        "",
        *_v273_missed_lines(watch_rows, rows, limit=5),
        "",
        *_v273_candidate_summary_lines(rows, limit=4),
        "",
        "판독: 기본화면은 놓친 후보와 현재 후보만 봅니다. 긴 후보표는 /strategy_watch_full",
    ]
    return "\n".join(lines)

try:
    STRATEGY_NAME = "3전략 선별 실험"
    STRATEGY_KEY = "three_strategy_v273_selected_open"
    STATE["phase_note"] = "v273: 명령어 역할 재정렬. 3전략 유지, 외부조건+반응+체결 통과만 실험OPEN, 나머지는 관찰."
    STATE["v273_gate"] = V273_VERSION_TAG
except Exception:
    pass
