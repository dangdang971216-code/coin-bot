...
VERSION = "guard_v2.5.22_external_state_2026-05-15"
...
def ws_sidecar_state_dict() -> dict:
    ...
    err_ok = last_error in {"", "-", "None", "none"}
    if not active_path.exists():
...
def gexternal_state_text() -> str:
    ...
    m = micro_state_dict()
    w_ok = bool(w.get("ok") or (w.get("alive") and int(w.get("fresh") or 0) > 0 and str(w.get("last_error") or "-") in {"", "-", "None", "none"}))
    m_ok = bool(m.get("ok") or (m.get("alive") and int(m.get("fresh") or 0) > 0 and str(m.get("last_error") or "-") in {"", "-", "None", "none"}))
