...
def send(msg: str, reply_markup=None, reply_to_message_id=None):
    ...
    try:
        ...
        log_exception_details("telegram_send", e, extra={
            "reply_to_message_id": reply_to_message_id,
            "message_preview": _runtime_safe_preview(text),
        }, prefix="[텔레그램 오류]")
        return None
...
def send_text_chunks(text: str, prefix: str = "", reply_markup=None, reply_to_message_id=None, limit: int = TG_MESSAGE_SOFT_LIMIT):
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        last_sent = send(chunk, reply_markup=chunk_markup, reply_to_message_id=chunk_reply_to)
        if last_sent is None:
            if idx == 1:
                send("⚠️ 메시지 전송 중 문제가 생겼어. 잠깐 뒤에 다시 눌러줘.")
            return None
    return last_sent
