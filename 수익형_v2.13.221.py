...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v208: micro 신선도 표시 기준 통일 + 구버전 배포문구 정리. 조건/청산/BUY_READY 변경 없음.",
}
...
def _micro_status_of(row: Dict[str, Any]) -> str:
    """v208: /health·/external_status·/quality가 같은 micro freshness 기준을 쓰게 한다."""
    status = str(_ctxv(row, "micro_row_status", "missing") or "missing")
    age = fnum(_ctxv(row, "micro_age_sec", -1), -1)
    if bool(_ctxv(row, "micro_fresh")) or status == "fresh" or (0 <= age <= MICRO_STALE_SEC):
        return "fresh"
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v220 자동매매급 WS 강제재구독 + 빠른스캔 유지(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
# ===============================
# v2.13.213: WS/micro hot queue + target priority without scan waiting
# ===============================
# 목적:
# - 전체시장 스캔을 줄이지 않는다.
# - WS/micro가 포착한 빠른 움직임을 다음 정밀대상/target 앞쪽에 넣는다.
# - 스캔이 외부직원을 기다리지 않는다. 최신값 부족은 표시하고 다음 scan에서 보강한다.

V213_HOT_QUEUE_MAX = int(os.getenv("CLEAN_HOT_QUEUE_MAX", "90"))
V213_HOT_PRECISION_LIMIT = int(os.getenv("CLEAN_HOT_PRECISION_LIMIT", "40"))
V213_HOT_TARGET_LIMIT = int(os.getenv("CLEAN_HOT_TARGET_LIMIT", "80"))
V213_HOT_TTL_SEC = float(os.getenv("CLEAN_HOT_TTL_SEC", "45"))
V213_HOT_MIN_MICRO_BUY_KRW_30 = float(os.getenv("CLEAN_HOT_MIN_MICRO_BUY_KRW_30", "1500000"))
V213_HOT_MIN_WS_TURNOVER = float(os.getenv("CLEAN_HOT_MIN_WS_TURNOVER", "1000000"))
...
def _v213_hot_score_from_micro(row: Dict[str, Any], nowv: float) -> float:
    ...
    if age > V213_HOT_TTL_SEC:
        return 0.0
    buy_krw = fnum((row or {}).get("trade_buy_krw_30"), 0.0)
    sell_krw = fnum((row or {}).get("trade_sell_krw_30"), 0.0)
    ratio = fnum((row or {}).get("trade_buy_ratio_30"), 0.0)
    spread = fnum((row or {}).get("micro_spread_pct"), 999.0)
    if buy_krw < V213_HOT_MIN_MICRO_BUY_KRW_30 and ratio < 0.56:
        return 0.0
    score = min(buy_krw / 1_000_000.0, 30.0) + max(0.0, ratio - 0.50) * 80.0
    if sell_krw > 0:
        score += min(max(0.0, (buy_krw - sell_krw) / 1_000_000.0), 20.0)
    if spread <= 0.25:
        score += 5.0
    elif spread >= 0.60:
        score -= 8.0
...
def _v213_hot_queue_rows(refresh: bool = True) -> List[Dict[str, Any]]:
    """WS/micro가 따로 잡은 빠른 움직임을 정밀대상/target 우선순위 재료로 만든다.
    매매조건이 아니라 '어디를 먼저 볼지' 정하는 직원용 큐다.
    """
    nowv = now_ts()
    if refresh:
        try:
            ...
            log_error("v213_hot_refresh_ws", exc)
        try:
            refresh_micro_cache()
        except Exception as exc:
            log_error("v213_hot_refresh_micro", exc)
    merged: Dict[str, Dict[str, Any]] = {}

    def add(ticker: str, source: str, score: float, reason: str) -> None:
        t = _ticker_from_any(ticker)
    ...
    # 현재/직전 후보는 hot 점수와 별개로 target/정밀 우선순위에서 앞쪽에 유지한다.
    for idx, t in enumerate(_v213_current_snapshot_tickers(limit=V213_HOT_TARGET_LIMIT)):
        add(t, "current_candidate", max(1.0, 20.0 - idx * 0.05), "현재 후보 유지")

    try:
        recent = recent_candidate_priority_rows(limit=V213_HOT_TARGET_LIMIT)
        for idx, r in enumerate(recent):
            t = _v213_row_ticker(r)
            add(t, "recent_candidate", max(1.0, 18.0 - idx * 0.04), "직전 후보 유지")
    except Exception as exc:
        log_error("v213_recent_candidate_hot", exc)

    for t in _paper_open_tickers(limit=60):
        add(t, "paper_open", 22.0, "paper OPEN 추적")

    try:
        with _micro_lock:
            micro_items = list(_micro_cache.items())
        for t, row in micro_items:
            score = _v213_hot_score_from_micro(row, nowv)
            if score > 0:
                add(t, "micro", score + 10.0, f"micro 매수체결/호가 hot {score:.1f}")
    except Exception as exc:
        log_error("v213_micro_hot", exc)

    try:
        with _ws_lock:
            ws_items = list(_ws_live_cache.items())
        for t, row in ws_items:
            score = _v213_hot_score_from_ws(row, nowv)
            if score > 0:
                add(t, "ws", score + 6.0, f"WS tick hot {score:.1f}")
    except Exception as exc:
        log_error("v213_ws_hot", exc)

    rows = sorted(merged.values(), key=lambda r: fnum(r.get("hot_score"), 0), reverse=True)[:V213_HOT_QUEUE_MAX]
    payload = {
        "schema": "hot_candidate_queue_v213",
        "version": BOT_VERSION,
        "updated_ts": nowv,
        ...
        "rows": rows,
        "count": len(rows),
        "note": "WS/micro hot queue: precision/target priority only; not a buy condition",
    }
    try:
        save_json(FILES.get("hot_queue", BASE_DIR / "clean_hot_candidate_queue.json"), payload)
    except Exception as exc:
        log_error("v213_save_hot_queue", exc)
    with _state_lock:
...
def select_precision_targets(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    hot = _v213_hot_queue_rows(refresh=True)
    ...
    try:
        ...
        with _state_lock:
            ...
            STATE["precision_target_note"] = str(STATE.get("precision_target_note") or "") + f" / hot_queue {STATE.get('precision_hot_queue_included',0)}"
    except Exception:
        pass
    return out
...
def _v212_snapshot_payload(strict_rows: List[Dict[str, Any]], shadow_rows: List[Dict[str, Any]], *, stage: str, source: str, wait_sec: float = 0.0) -> Dict[str, Any]:  # type: ignore[override]
    ...
    try:
        ...
        payload["hot_queue"] = {
            "count": int((hot or {}).get("count", 0) or 0) if isinstance(hot, dict) else 0,
            "top": [str((r or {}).get("ticker")) for r in ((hot or {}).get("rows") or [])[:8]] if isinstance(hot, dict) else [],
...
def _v216_quality_text() -> str:
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v216 기본은 캐시 전용: 현재 후보는 clean_candidate_snapshot.json 단일 원천만 표시",
    ...
    lines += ["⚠️ 우선 볼 것: 손절 / 지지부진 감소", "✅ 확인할 재료: 3분 지속 돈흐름, 눌림품질, 실제 호가·체결", "✅ 다음 판단: 최대/TOP3 제외 후에도 플러스인지, 지지부진 초반반응 0%가 반복되는지"]
    return "\n".join(lines)
...
def _v218_refresh_external_maps(*, force: bool = False, ttl: float = 0.8) -> Dict[str, Any]:
    """WS/micro 캐시를 한 번만 읽어 같은 scan/snapshot 안에서 재사용한다.

    v217까지는 후보 row마다 ws_snapshot/micro_snapshot이 lock/set/cache 접근을 반복했다.
    v218은 스캔 본선에서는 전체시장 훑기 → 후보 확정 → 현재 후보 보강 순서만 유지하고,
    외부정보 overlay는 한 번 읽은 캐시맵으로 처리한다.
    """
    nowv = now_ts()
    try:
    ...
    try:
        ...
        log_error("v218_refresh_ws_maps", exc)
    try:
        refresh_micro_cache()
    except Exception as exc:
        log_error("v218_refresh_micro_maps", exc)
    with _ws_lock:
    ...
    with _micro_lock:
        micro_cache = {str(k).upper(): dict(v) for k, v in (_micro_cache or {}).items() if k and isinstance(v, dict)}
        micro_targets = set(_ticker_from_any(x) for x in (_micro_targets or []) if _ticker_from_any(x))
    _V218_EXT_CACHE.update({"ts": nowv, "ws": ws_cache, "micro": micro_cache, "ws_targets": ws_targets, "micro_targets": micro_targets})
    return _V218_EXT_CACHE
...
def _v218_micro_snapshot_from_maps(ticker: Any, maps: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    maps = maps or _v218_refresh_external_maps()
    nowv = now_ts()
    row = dict((maps.get("micro") or {}).get(t) or {})
    target_set = maps.get("micro_targets") or set()
    if not row:
        return {"micro_fresh": False, "micro_row_status": "missing", "micro_targeted": t in target_set, "micro_age_sec": -1}
    age = nowv - fnum(row.get("ts"), nowv)
    fresh = age <= MICRO_STALE_SEC
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    return txt.replace("v216 기본은 캐시 전용", "v218 기본은 캐시 전용").replace("v214 기본은 캐시 전용", "v218 기본은 캐시 전용")
...
def _v219_overlay_row_with_maps(row: Dict[str, Any], maps: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rr["ticker"] = t or rr.get("ticker")
    rr["snapshot_overlay_ts"] = now_ts()
    rr["external_refreshed_by"] = "v219_post_scan_refresh"
    # 표시 필드 누락 보강. 조건/판정은 바꾸지 않는다.
    if not rr.get("final_entry_label"):
        rr["final_entry_label"] = _v219_status_label_reason(rr)
    if not rr.get("hold_reason"):
        rr["hold_reason"] = _v219_status_label_reason(rr)
    try:
        rest_price = fnum(rr.get("current_price") or rr.get("entry_price") or rr.get("detected_price"), 0.0)
...
def _v219_write_command_caches_after_snapshot() -> None:
    """refresh된 snapshot을 명령어 캐시에 즉시 반영한다.

    명령어 직접계산 fallback은 사용하지 않는다. 기존 캐시 생성 함수와 같은 lightweight text만 저장한다.
    """
    try:
        ext_snap = _v210_external_snapshot_dict()
        save_json(FILES["external_snapshot"], _cache_payload(_v210_external_status_text(ext_snap), "external_status"))
        ...
        log_error("v219_refresh_light_cache", exc)
    try:
        save_json(FILES["quality_summary"], _cache_payload(_v210_quality_text(), "quality"))
    except Exception as exc:
        log_error("v219_refresh_quality_cache", exc)


def _v219_refresh_candidate_snapshot(expected_scan_id: str = "") -> None:
    try:
        delay = max(0.5, min(V219_EXTERNAL_REFRESH_DELAY_SEC, 8.0))
        ...
        refreshed.update({
            "version": BOT_VERSION,
            "schema": "candidate_snapshot_v219",
            "stage": "external_refreshed_after_scan",
            "source": "v219_post_scan_external_refresh",
            "updated_ts": nowv,
            "updated_text": now_text(nowv),
            "candidate_count": len(rows),
            "rows": rows,
            "external": {
                ...
                **ext,
                ...
                "overlay_mode": "v219_post_scan_external_refresh",
                "refreshed_ts": nowv,
                "refresh_delay_sec": delay,
                "cache_map_age_sec": round(now_ts() - fnum(maps.get("ts"), now_ts()), 2),
            },
            "target": {
                **target,
                "post_scan_refresh": True,
                "post_scan_refresh_delay_sec": delay,
                "priority_rule": "scan_candidates_first; external refreshed after scan without blocking scan",
            },
            "note": "v219: scan exits fast; WS/micro are re-overlaid by a post-scan refresh worker; commands read refreshed snapshot cache only",
        })
        save_json(FILES["candidate_snapshot"], refreshed)
        with _state_lock:
            STATE["candidate_snapshot_ts"] = nowv
            STATE["candidate_snapshot_count"] = len(rows)
            STATE["candidate_snapshot_source"] = "v219_post_scan_external_refresh"
        ...
        _v219_write_command_caches_after_snapshot()
    except Exception as exc:
        log_error("v219_refresh_candidate_snapshot", exc)
...
def v212_publish_final_candidate_state(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]], market_rows: List[Dict[str, Any]], pipe: Dict[str, Any]) -> str:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["v218_scan_first_mainline"] = True
    return f"snapshot {final.get('candidate_count',0)} / WS {ext.get('ws_fresh',0)}/{ext.get('total',0)} / micro {ext.get('micro_fresh',0)}/{ext.get('total',0)} / wait {wait_sec:.1f}s / target_writer factory_final_candidates / v219 post-refresh scheduled"
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    if full:
        return _v209_direct_candidate_quality_text(True)
    txt = _read_cached_text(FILES["quality_summary"], "/quality")
    return txt.replace("v218 기본은 캐시 전용", "v219 기본은 캐시 전용").replace("v216 기본은 캐시 전용", "v219 기본은 캐시 전용").replace("v214 기본은 캐시 전용", "v219 기본은 캐시 전용")
...
def _v221_row_is_quality_watch(row: Dict[str, Any]) -> bool:
    try:
        if not isinstance(row, dict):
            return False
        if bool(row.get("paper_bot_open") or row.get("trade_ready") or row.get("open_eligible")):
            return True
        action = str(row.get("final_entry_action") or row.get("action") or "").lower()
        label = str(row.get("final_entry_label") or row.get("hold_reason") or row.get("quality_label") or "")
        if action in {"recheck_wait", "observe", "paper_open", "trade_ready"}:
            return True
        if any(k in label for k in ("재확인", "보류", "검증", "관찰", "실전위험")):
            return True
        return fnum(row.get("score"), 0.0) >= 4.20
    except Exception:
        return False


def _v221_write_micro_urgent_from_candidates(priority_rows: Optional[List[Dict[str, Any]]], reason: str = "factory_final_candidates") -> None:
    try:
        rows = [r for r in (priority_rows or []) if isinstance(r, dict) and _v221_row_is_quality_watch(r)]
        rows = sorted(rows, key=_v217_priority_key, reverse=True)[:max(1, V221_MICRO_URGENT_MAX)]
        tickers: List[str] = []
        meta: Dict[str, Any] = {}
        nowv = now_ts()
        maps = _v218_ext_maps(force=True)
        micro_cache = maps.get("micro") if isinstance(maps.get("micro"), dict) else {}
        for r in rows:
            t = _ticker_from_any(r.get("ticker") or r.get("market") or r.get("symbol"))
            if not t or t in tickers or t in STABLE_EXCLUDED:
                continue
            tickers.append(t)
            mr = micro_cache.get(t) if isinstance(micro_cache, dict) else None
            age = nowv - fnum((mr or {}).get("ts") or (mr or {}).get("updated_ts"), 0.0) if isinstance(mr, dict) else 9999.0
            st = "fresh" if isinstance(mr, dict) and 0 <= age <= MICRO_FRESH_SEC else ("stale" if isinstance(mr, dict) else "missing")
            meta[t] = {
                "source": "current_quality_candidate",
                "score": fnum(r.get("score"), 0.0),
                "status": st,
                "age_sec": round(age, 2) if age < 9000 else None,
                "trade_ready": bool(r.get("paper_bot_open") or r.get("trade_ready") or r.get("open_eligible")),
                "label": str(r.get("final_entry_label") or r.get("hold_reason") or r.get("quality_label") or "")[:80],
            }
        if not tickers:
            return
        payload = {
            "version": BOT_VERSION,
            "source": "main_bot_v221",
            "reason": reason,
            "updated_ts": nowv,
            "updated_at": now_text(nowv),
            ...
            "targets": tickers,
            "target_meta": meta,
            "note": "v221: auto/preopen/recheck candidates need fresh micro for trading-quality and review-quality; not a strategy-condition change",
        }
        ...
        log_error("v221_write_micro_urgent", exc)


_v221_base_update_micro_targets = update_micro_targets

def update_micro_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "hub_rank") -> None:  # type: ignore[override]
    # 본선 target writer는 유지하되, 별도 urgent 파일로 paper/preopen/recheck 후보를 micro sidecar 최상단에 올린다.
    _v221_base_update_micro_targets(rows, priority_rows=priority_rows, reason=reason)
    _v221_write_micro_urgent_from_candidates(priority_rows, reason=reason)
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    if full:
        return txt
    return txt.replace("v219 기본은 캐시 전용", "v221 기본은 캐시 전용").replace("v218 기본은 캐시 전용", "v221 기본은 캐시 전용")


_v221_base_health_text = health_text

def health_text() -> str:  # type: ignore[override]
    return _v221_base_health_text().replace("수익형 v2.13.220", "수익형 v2.13.221").replace("수익형 v2.13.219", "수익형 v2.13.221")
