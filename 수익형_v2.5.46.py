...
SCAN_HEARTBEAT_WARN_SEC = 90
SCAN_HEARTBEAT_HARD_RESTART_SEC = 210
SCAN_CACHE_WARN_SEC = 120
SCAN_CACHE_HARD_RESET_SEC = 240
SCAN_WATCHDOG_CHECK_SEC = 15
SCAN_WATCHDOG_STARTUP_GRACE_SEC = 180
SCAN_WATCHDOG_RESET_COOLDOWN_SEC = 120
# =========================================================
# 실행 주기
# =========================================================
SCAN_INTERVAL = 6
...
def send_startup_message():
    lines = [
        ...
        "- 사용자 알림 rescue 판정 함수가 깨져 있던 부분을 고쳐 스캔 루프가 중간에 죽지 않게 정리",
        "- shortlist rescue / hidden promotion 기준을 소폭 완화해서 좋은 코인 승격을 조금 더 살림",
    ...
    while True:
        ...
        try:
            ...
            if info["loop_hard_stale"]:
                age = info["loop_age"]
                print(f"[watchdog] 메인 루프 멈춤 감지 / {age}초 / stage={info.get('stage','?')}", flush=True)
                os._exit(1)
...
while True:
    ...
    try:
        ...
        if need_scan_cycle or need_position_cycle:
            mark_main_loop_heartbeat(stage="build_cache")
