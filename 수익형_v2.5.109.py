...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    lines = [
        ...
        f"- 최근60분 기준값: 고점 {fmt_price(safe_float(row.get('recent_60m_high_price', 0), 0))} / 저점 {fmt_price(safe_float(row.get('recent_60m_low_price', 0), 0))}" if safe_float(row.get('recent_60m_high_price', 0), 0) > 0 or safe_float(row.get('recent_60m_low_price', 0), 0) > 0 else "",
        f"- 상위봉: 4시간 {str(row.get('htf_setup_label', '') or '확인중')} (고점거리 {safe_float(row.get('htf_gap_pct', 999), 999):.2f}% / 저점대비 {safe_float(row.get('htf_rise_pct', 0), 0):.2f}%) / 일봉 {str(row.get('daily_setup_label', '') or '확인중')} (고점거리 {safe_float(row.get('daily_gap_pct', 999), 999):.2f}% / 저점대비 {safe_float(row.get('daily_rise_pct', 0), 0):.2f}%)",
        ...
        f"- 최근30분 기준값: 고점 {fmt_price(safe_float(row.get('recent_30m_high_price', 0), 0)) if safe_float(row.get('recent_30m_high_price', 0), 0) > 0 else '-'} / 저점 {fmt_price(safe_float(row.get('recent_30m_low_price', 0), 0)) if safe_float(row.get('recent_30m_low_price', 0), 0) > 0 else '-'}",
...
def get_missed_review_window_stats(ticker: str, base_price: float):
    ...
    try:
        ...
        try:
            if hasattr(recent.index, '__len__') and len(recent.index) > 0:
                first_idx = recent.index[0]
                last_idx = recent.index[-1]
                if hasattr(first_idx, 'timestamp'):
                    stats["eval_start_ts"] = float(first_idx.timestamp())
                if hasattr(last_idx, 'timestamp'):
                    stats["eval_end_ts"] = float(last_idx.timestamp())
        except Exception:
            pass
