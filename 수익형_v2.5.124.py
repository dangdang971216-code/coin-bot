...
def should_promote_recovery_candidate_to_entry(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if chg_30 < -1.20 and from_low < 0.90 and high_gap_pct < 2.10:
        return False
    if strategy in {"LEADER_WATCH", "PREPUMP", "PULLBACK"} and chg_5 < 0.14 and max(chg_30, from_low) < 0.35 and high_gap_pct < 1.80:
        return False
    if strategy in {"LEADER_WATCH", "PREPUMP", "PULLBACK"} and vol_ratio < 1.55 and leader_score < 0.95 and max(chg_5, chg_30, from_low) < 0.55:
        return False
    ...
    if strategy in {"PREPUMP", "PULLBACK"} and high_gap_pct >= 1.10 and vol_ratio >= 1.30 and max(chg_5, chg_30, from_low, change_pct) >= 0.18:
        threshold -= 1
    if high_gap_pct >= 1.35 and vol_ratio >= 1.30 and max(chg_5, chg_30, from_low) >= 0.22:
        threshold -= 1
    if regime_name == "SIDEWAYS":
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 차트 수치 정합성 점검: 4시간봉/일봉 값이 비는 경우 placeholder 대신 실제 계산값 우선 사용",
        "- 놓친 종목 복기 보강: reverse_trace에서 나중에 잘 간 코인을 움직였지만 애매으로 다시 보이게 정리",
        "- 평가 결과 동기화 보강: 시작값/결과값을 같은 minute1 창에서 계산하고 결과 발생 시각도 같이 보이게 정리",
        "- 시작 알림의 오래된 반복 변경점은 줄이고 최근 핵심만 짧게 표시",
        "- 런타임 오류 로그 추가: runtime_error_log.txt / errorlog 명령으로 최근 오류 확인 가능",
        "",
    ]
    ...
    try:
        ...
        if attempts <= 0:
            near_retry_ready = bool(
                gap_pct <= 1.65
                and (
                    last6_high >= prev_high * 0.995
                    or below_30_high_pct <= 1.20
                    or hold_near_high_bars_10 >= 1
                )
                and (
                    recovery_pct >= 0.30
                    or recovery_speed_pct >= 0.10
                    or chg_3 >= 0.10
                    or close_pos_1 >= 0.45
                    or close_pos_3 >= 0.50
                    or vol_ratio >= 1.15
...
def get_shortlist_user_rescue_display_key(item: dict, display_key: str = "", regime: dict = None) -> str:
    ...
    if shortlist_rank <= 7 and high_gap_pct >= 1.35 and from_low <= 2.25 and chart_rise <= 4.45:
        if vol_ratio >= 0.72 or leader >= 0.15 or edge >= 4.05 or daily_ready >= 0.15 or breakout_hold_score >= 0.10 or market_flow_score >= 0.95 or rebreakout_score >= 0.75 or structure_score >= 0.60:
            return "restart"
    ...
    if chg_5 >= 0.42 and high_gap_pct >= 0.82 and (chg_30 >= 0.00 or vol_ratio >= 1.45 or leader >= 0.72 or hot_score >= USER_ALERT_RESCUE_STRONG_HOT_SCORE or rebreakout_score >= 1.00):
        return "live"
    if motion >= 0.02 and high_gap_pct >= 0.48 and (vol_ratio >= 0.82 or leader >= 0.28 or edge >= 4.25 or strong_rank_rescue or market_flow_score >= 0.85 or rebreakout_score >= 0.70 or structure_score >= 0.60):
        return "restart"
    return ""
...
def is_user_alert_rescue_candidate(item: dict, section: str = "", regime: dict = None) -> bool:
    ...
    if section == "restart":
        if top_turnover_rescue and high_gap_pct >= 1.30:
            return True
        if shortlist_rank <= 7 and high_gap_pct >= 0.65 and from_low <= 2.95 and chart_rise <= 4.60:
            if max(chg_5, chg_30, from_low) >= 0.02 or market_flow_score >= 0.95 or daily_ready >= 0.20 or breakout_hold_score >= 0.10 or vol_ratio >= 0.72 or leader >= 0.18 or rebreakout_score >= 0.75 or structure_score >= 0.60:
                return True
