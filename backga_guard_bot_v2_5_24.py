...
VERSION = "guard_v2.5.24_ws_upgrade_safe_reconnect_2026-05-15"
...
def ws_sidecar_state_dict() -> dict:
    ...
    if not active_path.exists():
        ...
        verdict = "✅ 정상수신"
        ok = True
    elif age_ok and state in {"연결", "수신대기", "대상준비", "guard_starting", "재구독중"} and err_ok:
        verdict = "❔ 실행중 / 수신대기"
        ok = False
...
def apply_ws_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    ws_ok = bool(ready_state.get("alive")) and int(ready_state.get("parse_ok") or 0) > 0 and int(ready_state.get("match_ok") or 0) > 0 and (ready_err in {"", "-", "None", "none"} or _is_reconnect_notice(ready_err))
    rollback_note = ""
