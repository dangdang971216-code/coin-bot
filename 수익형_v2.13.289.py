...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v289: v288 stage helper 유령연결 제거. sweep_vwap_recovery 단일전략 / paper v0.64 검증 전용.",
}
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.289.py", "- 페이퍼봇 대상: paper_bot_v0.64.py"])),
    ]
    start = now_ts()
...
def _v288_eval(row: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if high_room >= V288_HIGH_ROOM_MIN:
        score += 1.2; good.append("고점여유")
    else:
        hard.append(f"고점여유부족 {high_room:.2f}%")

    if vwap_gap >= V288_VWAP_PASS_GAP:
    ...
    if money1 >= V288_MONEY1_MIN:
        score += 0.8; good.append("1분돈")
    else:
        wait.append("1분거래대금부족")
    if money3 >= V288_MONEY3_MIN:
        score += 1.0; good.append("3분돈")
    else:
        wait.append("3분거래대금부족")
    if money3 >= V288_BIG_MONEY3 and ch3 < V288_BIG_MONEY_NO_REACTION_CH3:
    ...
    return {
        ...
        "high_room_pct": round(high_room, 3),
        "vwap_gap_pct": round(vwap_gap, 3),
...
def _v288_candidate_row(row: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ctx.update({
        ...
        "strategy_rule_version": "v288_design_fixed_30_closed",
    ...
    item.update({
        "schema": "sweep_vwap_recovery_v288",
        "brain_version": BOT_VERSION,
...
def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["phase_note"] = "v289: 7전략 active path 제거, sweep_vwap_recovery 단일 paper 검증."
    return strict, shadow, rejects, examples


def _v288_write_micro_urgent(rows: List[Dict[str, Any]], reason: str = "v289_sweep_vwap_recheck") -> None:
    try:
        targets: List[str] = []
        meta: Dict[str, Any] = {}
        for r in rows or []:
            ...
            if str((r or {}).get("final_entry_action")) != "recheck_wait":
                continue
            targets.append(t)
            meta[t] = {"source": "v289_sweep_vwap", "score": fnum(r.get("score"), 0), "reason": ",".join(_v288_as_list(r.get("final_entry_reasons"))[:4])}
            if len(targets) >= 40:
                break
        if not targets:
            return
        save_json(FILES.get("micro_urgent", BASE_DIR / "clean_micro_urgent_targets.json"), {
            "version": BOT_VERSION,
            "schema": "v288_micro_urgent_targets",
            "updated_ts": now_ts(),
            "updated_text": now_text(),
            "ttl_sec": 35,
            "reason": reason,
        ...
        log_error("v288_micro_urgent_write", exc)
...
def _v288_health_text() -> str:
    ...
    simple_stages, slow_note = _v285_slow_stage_summary(stage_lines_raw) if callable(globals().get("_v285_slow_stage_summary")) else ([], [])
    last_sec = fnum(s.get("scan_last_sec"), 0)
    if last_sec >= 60:
        bottleneck = f"❌ 최근완료 {last_sec:.1f}초: 병목 확인 필요"
    elif slow_note:
    ...
    running_txt = "ON" if p.get("running") is True else ("OFF" if p.get("running") is False else "확인중")
    open_count = pst.get("open_count") if pst.get("open_count") is not None else line_count(FILES.get("paper_open", BASE_DIR / "paper_bot_open.json"))
    closed_count_v = pst.get("closed_count") if pst.get("closed_count") is not None else line_count(FILES.get("paper_closed", BASE_DIR / "paper_bot_closed.jsonl"))
    lines = [
        "🧭 건강상태 /health",
        ...
        "",
        "[5/6] paper_bot",
        f"{'✅' if p.get('running') else '❔'} {p.get('version','paper_bot')} / 실행 {running_txt} / OPEN {open_count} / CLOSED {closed_count_v} / 상태 {p.get('age_text','-')}",
        "",
        "[6/6] 고정 원칙",
...
def _v288_write_strategy_files(strict_rows: List[Dict[str, Any]], shadow_rows: List[Dict[str, Any]], market: Dict[str, Any], rejects: Counter) -> None:
    ...
    payload = {
        "version": BOT_VERSION,
        "schema": "sweep_vwap_recovery_snapshot_v288",
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        "strategy_key": STRATEGY_KEY,
        "strategy_label": STRATEGY_LABEL,
        ...
        "note": "v289: 7전략 제거 후 저점 쓸림 후 VWAP 회복 단일전략 paper 검증.",
    }
...
def _v288_refresh_latest_row(row: Dict[str, Any], lane: str, scan_id: str) -> Dict[str, Any]:
    ...
    rr.update({
        "brain_version": BOT_VERSION,
        "schema": "sweep_vwap_recovery_latest_v288",
        "strategy_key": STRATEGY_KEY,
        "paper_strategy_key": STRATEGY_KEY,
        "route": STRATEGY_KEY,
        "paper_route": STRATEGY_KEY,
...
def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:  # type: ignore[override]
    ...
    _v288_write_micro_urgent(strict_rows, reason="v289_sweep_vwap_recheck")
    write_error = ""
    ...
    with _state_lock:
        ...
        STATE["factory_mode"] = "v289_sweep_vwap_recovery_factory"
    ...
    return {
        ...
        "dup_skip_reason": {},
        "data_quality_note": "v289_sweep_vwap_recovery",
        "dup_skip": 0,
        "write_error": write_error,
        "last_ticker": str((strict_rows or shadow_rows or [{"ticker":"-"}])[-1].get("ticker") or "-"),
        "factory_mode": "v289_sweep_vwap_recovery_factory",
        "archive_deferred": 0,
...
def select_precision_targets(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    with _state_lock:
        STATE["precision_target_note"] = f"v289 sweep/vwap selector {min(len(arr), limit)}/{len(arr)}"
        STATE["precision_target_source"] = "v289_sweep_vwap_selector"
    return arr[:limit]
...
def _v279_write_target_file(path: Path, tickers: List[str], reason: str) -> None:
    nowv = now_ts()
    payload = {"version": BOT_VERSION, "schema": "v289_sweep_vwap_targets", "updated_ts": nowv, "updated_text": now_text(nowv), "reason": reason, "targets": tickers, "target_count": len(tickers), "target_meta": {t: {"reason": reason, "source": STRATEGY_KEY} for t in tickers}}
    atomic_write(path, payload)


def update_ws_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "v289_sweep_vwap") -> None:  # type: ignore[override]
    ...
    with _state_lock:
        STATE["ws_targets"] = len(tickers); STATE["ws_target_file_written"] = 1; STATE["ws_target_file_targets"] = len(tickers); STATE["ws_target_reason"] = reason; STATE["ws_target_write_note"] = "v289 sweep/vwap target writer"


def update_micro_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "v289_sweep_vwap") -> None:  # type: ignore[override]
    tickers = _v279_target_tickers(rows, priority_rows, limit=180)
    with _micro_lock:
        _micro_targets[:] = tickers
    _v279_write_target_file(FILES.get("micro_targets", BASE_DIR / "clean_micro_targets.json"), tickers, reason)
    with _state_lock:
        STATE["micro_targets"] = len(tickers); STATE["micro_target_file_written"] = 1; STATE["micro_target_file_targets"] = len(tickers); STATE["micro_target_reason"] = reason; STATE["micro_target_write_note"] = "v289 sweep/vwap target writer"



def _v289_stage_push(stage_times: List[Tuple[str, float, str]], name: str, sec: float, note: str = "") -> None:
    """v289 scan 단계기록 단일 helper.
...
def scan_once() -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["scan_display_note"] = "v289 sweep/vwap 스캔 진행중"
    ...
    try:
        ...
        _v289_stage_push(stage_times, "1) 허브", now_ts() - st, f"rows {len(rows)} / {source} / WS fresh {STATE.get('ws_fresh',0)} / micro fresh {STATE.get('micro_fresh',0)}")
        with _state_lock:
        ...
        _v289_stage_push(stage_times, "2) 직원: 정밀/VWAP 보강", now_ts() - st, f"targets {len(targets)} / 직접 {p_ok} / cached {p_have} / fail {p_fail}")
        with _state_lock:
        ...
        _v289_stage_push(stage_times, "3) 직원: 표준값 병합", now_ts() - st, f"precision {cov.get('precision')} / bulk-only {cov.get('bulk_only')}")
        with _state_lock:
        ...
        _v289_stage_push(stage_times, "4) 전략: sweep/vwap 판정", now_ts() - st, f"진입 {ready_n} / 재확인 {recheck_n} / shadow {len(shadow)} / reject {sum(rejects.values())}")
        with _state_lock:
...
def reversion_review_text(full: bool = False) -> str:  # type: ignore[override]
    return "\n".join([
        ...
        "- v289에서는 7전략 실험판을 active path에서 제거했습니다.",
...
try:
    ...
    STATE["phase_note"] = "v289: 7전략 제거, 저점 쓸림 후 VWAP 회복 단일전략 paper 검증."
