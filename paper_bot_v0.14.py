...
# ============================ paper_bot_v0.13 slippage patch ============================
VERSION = "paper_bot_v0.13"
DEFAULT_CONTROL.update({
...
# ============================ end paper_bot_v0.13 patch ============================



# ============================ paper_bot_v0.13 slippage explanation patch ============================
VERSION = "paper_bot_v0.13"
DEFAULT_CONTROL.update({
...
# ============================ paper_bot_v0.14 alert safety patch ============================
# 목적: v0.13에서 strict OPEN 알림 포맷 함수가 format_price 누락으로 터지던 문제를 고친다.
# 전략 판단/청산조건/슬리피지 값은 건드리지 않는다.
VERSION = "paper_bot_v0.14"

def format_price(value: Any) -> str:
    try:
        v = float_any(value, default=0.0)
        if v >= 1000:
            return f"{v:,.0f}"
        if v >= 100:
            return f"{v:,.1f}"
...
try:
    _v13_notify_strict_events = notify_strict_events
except Exception:
    _v13_notify_strict_events = None

def notify_strict_events(opened_items: List[Dict[str, Any]], closed_items: List[Dict[str, Any]], control: Dict[str, Any]) -> None:
    """알림 실패가 worker cycle을 흔들지 않도록 정식 알림만 안전 격리한다."""
    if _v13_notify_strict_events is None:
        return
    try:
        _v13_notify_strict_events(opened_items, closed_items, control)
    except Exception as exc:
        log_error("notify_strict_events_safe_v014", exc)
        # 알림 실패만 기록하고 OPEN/CLOSED 저장 흐름은 유지한다.
        try:
            log(f"notify skipped safely: {exc.__class__.__name__}: {str(exc)[:120]}")
        except Exception:
            pass

def _v06_status_extra_lines(status: Dict[str, Any], control: Dict[str, Any], open_pos: Dict[str, Dict[str, Any]]) -> List[str]:
    # v0.13의 슬리피지 설명과 상태 요약은 유지하되, 구 v0.7 판독 문구는 build_pbatch_text에서 교체한다.
    pick = status.get("pick_stats") if isinstance(status.get("pick_stats"), dict) else {}
    cap = status.get("capacity") if isinstance(status.get("capacity"), dict) else {}
    nt = status.get("notify") if isinstance(status.get("notify"), dict) else notify_config_status()
    recent = status.get("recent_summary") if isinstance(status.get("recent_summary"), dict) else {}
    ...
    lines = [
        f"- 이번 cycle 진입: 전체 +{status.get('opened_this_cycle',0)} / 정식 +{recent.get('opened_strict',0)} / 복기 +{recent.get('opened_shadow',0)} / 종료 +{status.get('closed_this_cycle',0)} / 오래된OPEN정리 {status.get('stale_closed_this_cycle',0)} / 실패 {status.get('open_fail_this_cycle',0)}",
        f"- 후보 읽기: 정식파일 {pick.get('paper_file',0)} / 복기파일 {pick.get('shadow_file',0)} / 전체파일 {pick.get('events_file',0)} / 이벤트중복skip {pick.get('dup_skip',0)} / 같은코인skip {pick.get('dup_ticker_skip',0)} ({', '.join((pick.get('dup_ticker_examples') or [])[:12]) if pick.get('dup_ticker_examples') else '-'}) / bad {pick.get('bad_skip',0)}",
        f"- OPEN 한도: 정식 {len([p for p in open_pos.values() if str(p.get('lane'))=='strict'])}/{cap.get('max_open_strict', control.get('max_open_strict'))} {'FULL' if cap.get('strict_full') else '여유'} / 복기 {len([p for p in open_pos.values() if str(p.get('lane'))=='shadow'])}/{cap.get('max_open_shadow', control.get('max_open_shadow'))} {'FULL' if cap.get('shadow_full') else '여유'} / cycle 제한 사실상 해제 {control.get('max_new_per_cycle')}",
        f"- 알림설정: token {'OK' if nt.get('token') else '없음'} / chat {'OK' if nt.get('chat_id') else '없음'}({nt.get('chat_source','?')}) / strict OPEN {'ON' if nt.get('strict_open_on') else 'OFF'} / strict CLOSE {'ON' if nt.get('strict_close_on') else 'OFF'} / shadow {'ON' if nt.get('shadow_on') else 'OFF'}",
        f"- 최근 OPEN: {', '.join(recent.get('opened_tickers') or []) if recent.get('opened_tickers') else '-'}",
        f"- 자동정리: {', '.join(prune_notes) if prune_notes else '대기/없음'}",
    ]
...
try:
    _v13_build_pbatch_text = build_pbatch_text
except Exception:
    _v13_build_pbatch_text = None

def build_pbatch_text() -> str:
    # 기존 본문을 최대한 유지하고 마지막 판독만 최신 역할 기준으로 교체한다.
    if _v13_build_pbatch_text is None:
        return f"📦 페이퍼봇 묶음점검 /pbatch\n\n✅ 버전: {VERSION}"
    text = _v13_build_pbatch_text()
    text = text.replace("✅ 버전: paper_bot_v0.13", f"✅ 버전: {VERSION}")
    text = text.replace("- v0.7은 전략 후보품질 확인을 위해 OPEN 한도를 크게 완화했다.", "- v0.14는 모의 OPEN/CLOSED 장부와 알림만 담당한다.")
    text = text.replace("- 오래된 OPEN은 CLOSED로 넘겨 장부를 보존하고, 큰 jsonl/log 파일은 최근 줄만 남긴다.", "- 알림 실패는 격리하고, OPEN/CLOSED 저장 흐름은 유지한다.")
    return text
# ============================ end paper_bot_v0.14 alert safety patch ============================
