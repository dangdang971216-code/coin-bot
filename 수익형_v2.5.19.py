...
RECOVERY_ENTRY_MIN_30M_PCT = 0.10
RECOVERY_ENTRY_MIN_FROM_LOW_PCT = 0.25
RECOVERY_ENTRY_MIN_VOL_RATIO = 1.55
RECOVERY_ENTRY_MIN_LEADER_SCORE = 0.65
RECOVERY_ENTRY_MIN_HIGH_GAP_PCT = 0.70
RECOVERY_ENTRY_MIN_5M_PCT = -0.05
FAST_ENTRY_MIN_5M_PCT = 0.22
FAST_ENTRY_MIN_30M_PCT = 0.45
FAST_ENTRY_MIN_FROM_LOW_PCT = 0.45
FAST_ENTRY_MIN_VOL_RATIO = 1.75
FAST_ENTRY_MIN_LEADER_SCORE = 0.70
FAST_ENTRY_MIN_HIGH_GAP_PCT = 0.75
...
def is_fast_start_entry_candidate(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if regime_name == "SIDEWAYS":
        if chg_5 >= 0.28 and vol_ratio >= 1.95 and high_gap_pct >= 0.90:
            threshold -= 1
    elif regime_name == "WEAK":
        threshold += 1

    if strategy == "LEADER_WATCH" and vol_ratio >= 2.70 and leader_score >= 1.05 and max(chg_5, chg_30) >= 0.35:
        threshold -= 1
    if strategy in {"PREWATCH", "NEWLOW_RECOVER"} and vol_ratio >= 2.20 and max(chg_5, from_low) >= 0.30:
        threshold -= 1
...
def get_watch_section_key(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> str:
    ...
    if is_watch_late_candidate(item, judgement, caution):
        ...
        section = "watch" if strategy in {"LEADER_WATCH", "PREWATCH", "NEWLOW_RECOVER"} else "entry"
    ...
    if section == "entry" and trade_tier == "B":
        ...
        if high_gap_pct <= 0.65:
            section = "late"
        elif chg_5 < 0.08 and chg_30 < 0.35:
            section = "recovery" if recovery_like else "watch"
        elif leader_score < 1.05 and vol_ratio < 1.80:
            section = "recovery" if recovery_like else "watch"
        elif vol_ratio < 1.60 and not recovery_like:
            section = "watch"
        elif from_low < 0.35 and chg_30 < 0.55 and chg_5 < 0.22:
            section = "recovery"
        elif strategy == "LEADER_WATCH" and (vol_ratio < 2.20 or high_gap_pct < 0.75):
            section = "recovery" if recovery_like else "watch"

    if section == "entry" and strategy == "LEADER_WATCH":
        if chg_5 < 0.22 and chg_30 < 0.55:
            section = "recovery" if is_low_zone_recovery_candidate(item, judgement, caution) else "watch"
