...
- 실거래 주문은 v2.7.21 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.21"
BOT_FILE_TAG = "수익형_v2.7.21.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.21는 현재가+체결 허브를 후보분류에 더 적극 반영한다.
...
# v2.7.21: 일반 WebSocket 직원과 별도로 M2 눌림 후보 전용 직원을 둔다.
...
class RuntimeState:
    ...
    # v2.7.21: 일반 직원/ M2 전용 직원의 목표 코인을 따로 보관하고 합산해서 진단한다.
...
def ws_candidate_tickers(limit: int = WS_MAX_CODES) -> list[str]:
    # v2.7.21: 구독 = 돈 내는 구독이 아니라 실시간 가격/체결/1분봉 우선감시 목록이다.
    with STATE.lock:
...
def _set_ws_target_group_locked(kind: str, group: str, tickers: list[str]):
    """v2.7.21: 일반 직원과 M2 전용 직원의 감시 목표를 분리해서 합산한다."""
    key = f"{kind}_{group}"
...
def ws_m2_candidate_tickers(limit: int = WS_M2_MAX_CODES) -> list[str]:
    """v2.7.21: M2 눌림 재확인 전용 WebSocket 직원이 맡을 코인."""
    with STATE.lock:
...
def scan_root_cause_from_snapshot(st: dict) -> str:
    """v2.7.21: 구독/수신/활성 부족 원인을 초보자용으로 압축한다."""
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.21 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
def v315_mainline_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool) -> dict:
    """
    v2.7.21: v2.5.3xx 후기판의 본선 판단을 clean core row로 옮긴 1차 컨텍스트.
...
def v317_pullback_risk_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool) -> dict:
    """
    v2.7.21: v2.5.3xx 후기판의 위험진입 차단 2차 이식.
...
def v318_m2_pullback_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool, recent_money_strong: bool, own_supporting: bool, own_recovery_strong: bool, v315_ctx: dict, v317_ctx: dict) -> dict:
    """
    v2.7.21: v2.5.3xx/v2.6 눌림 재확인 라인 2차 이식.
...
def m2_realtime_gate(row: MarketRow) -> tuple[bool, str]:
    """v2.7.21: M2 대기 후보는 가격·체결·봇1분봉이 너무 낡으면 3차 정밀스캔 대기로 둔다."""
    q = str(row.data_quality or "")
    # v2.7.21: M2 대기/재확인은 가격이 너무 늦으면 감시로 둔다. 활성 표시는 20초로 보되 M2 게이트는 25초로 제한.
    ...
    if price_ok and candle_ok and trade_some:
    ...
    if not trade_some:
        miss.append("최근 체결")
    return False, "M2 정밀스캔 대기: " + "/".join(miss)
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    # v2.7.21: v2.5.3xx 본선 기준 이식 컨텍스트.
    ...
    # v2.7.21: 예전 승률 좋던 M2 눌림/준비대기 2차 복구.
    ...
    # v2.7.21에서는 v318 M2 컨텍스트를 우선한다. 기존 v315/v317 게이트는 보조 안전장치로 남긴다.
    pullback_prep_wait = (
    ...
    # v2.7.21에서는 일반 약한 움직임 전체가 재출발감시로 빨려 들어가지 않게,
    ...
    # v2.7.21: 특수 대기상태 보존. TON류 재출발/재돌파/급등후재확인 후보가
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.21: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.21는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.21\n"
        f"- v2.7.21 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.21에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.21\n"
...
def render_paper_stats() -> str:
    ...
    if overdue30:
        lines.append("- ⚠️ 30분 지났는데 미완료 후보가 있음. v2.7.21에서 paper worker/저장 조건을 봐야 함")
    return "\n".join(lines)
...
def render_check_summary() -> str:
    """v2.7.21: 예전처럼 봇/웹소켓/스캔/차트/paper를 한 화면에 모은 통합 점검판."""
    s = snapshot()
    st = ws_hub_snapshot()
    market_age, rank_age, cmd_age, market_ok, rank_ok, candidate_ok, output_ok = status_flags(s)
    ...
    with STATE.lock:
        cands = list(STATE.candidates)
        stats = dict(STATE.factory_stats or {})
        paper = dict(STATE.paper_stats or {})
        hb = dict(STATE.worker_heartbeat)
    status = stats.get('status', {}) if isinstance(stats.get('status', {}), dict) else {}
    ...
    def age_of(name):
        ...
        if not row:
            return "기록없음"
        return f"{int(max(0, now_ts()-float(row.get('ts') or 0)))}초 전/{row.get('stage','-')}"
    m2 = paper.get('main', {}).get('M2_PULLBACK', {}) if isinstance(paper.get('main', {}), dict) else {}
    lines = [
        "🧭 통합 점검 /check",
        "1) 봇 상태",
        f"- 버전: {BOT_VERSION} / 메인루프 {age_text(s['main_loop_ts'])} / 오류 {'없음' if not s.get('last_fetch_error') else '확인필요'}",
        f"- 시장/후보: {s['market_count']}개 → 후보 {s['candidate_count']}개 / 🟢{status.get('BUY_READY',0)} 🟡{status.get('PENDING',0)} ⚪{status.get('WATCH',0)}",
        f"- 서버: {res}",
        "",
        "2) 실시간 직원",
        f"- 일반 가격/체결: {age_of('ws_price_worker')} / {age_of('ws_trade_worker')}",
        f"- M2전용 가격/체결: {age_of('ws_price_m2_worker')} / {age_of('ws_trade_m2_worker')}",
        f"- 활성: 가격 {st.get('price_live',0)}/{st.get('price_total',0)} · 체결 {st.get('trade_live',0)}/{st.get('trade_total',0)} · 1분봉 {st.get('candle_live',0)}/{st.get('candle_total',0)}",
        ...
        "- 참고: 1시간은 현재 히스토리 보관시간 때문에 0일 수 있음. 나중에 상위봉 복구 필요",
        "",
        "5) M2 paper",
        f"- 10분: 완료 {int(m2.get('done10',0) or 0)} / 진행 {int(m2.get('active',0) or 0)} / 승률 {safe_float(m2.get('win_rate10',0)):.0f}% / 평균 {safe_float(m2.get('avg10',0)):+.2f}% / PF {m2.get('pf10','0')}",
        "",
        "6) 지금 큰 막힘",
    ]
    if block:
        for k, v in _top_items(block, 4):
            lines.append(f"- {k}: {v}개")
    else:
        lines.append("- 없음")
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.21 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
...
def send_startup_message():
    txt = (
        ...
        f"현재 버전: {BOT_VERSION}\n\n"
        f"이번 핵심 변경\n"
        f"- ZIL류 급등초입 후 식는 후보를 눌림이 아니라 상승 후 재확인대기로 분리\n"
        f"- paper 병합 시 더 강한 본선전략이 나오면 대표전략을 승격해 통계 왜곡 방지\n"
        f"- BOBA류 약한 재출발은 재출발감시로 내려 대기 상단 오염 방지\n"
        f"- 눌림/초입/재출발/재확인 통계를 더 순수하게 분리\n"
        f"- 실거래 주문은 보호모드. 매매 파트는 아직 미연결\n\n"
        f"확인: /batch\n/ping\n/health_check\n/paper_stats\n/paper_flow\n/paper_check_full\n/factory_check\n/buy_check\n/error_doctor"
    )
    try:
...
def main():
    ...
    # v2.7.21: M2 눌림 재확인 후보만 따로 보는 전용 직원. 일반 스캔 직원이 흔들려도 M2 자료를 최대한 유지한다.
    threading.Thread(target=websocket_worker, args=("price", "m2"), daemon=True, name="ws_price_m2_hub_v2721").start()
    threading.Thread(target=websocket_worker, args=("trade", "m2"), daemon=True, name="ws_trade_m2_hub_v2721").start()
