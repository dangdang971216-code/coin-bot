...
VERSION = "guard_v2.4_fast_validate_2026-05-10"
...
def deploy_status() -> str:
    ...
    if latest != "?" and target not in {latest, "?", ""}:
        # v2.4: DEPLOY_TARGET은 legacy 참고값. latest/bot/deployed/runtime이 맞으면 경고로 보지 않는다.
        target_legacy_note = " / 참고값 낡음"
    ...
    last = "없음"
    timing_brief = "없음"
    if status.get("timings"):
        timing_brief = ", ".join(f"{name} {fmt_sec(sec)}" for name, sec in status.get("timings", [])[:8])
        if status.get("total_sec") is not None:
            timing_brief = f"total {fmt_sec(status.get('total_sec'))} / " + timing_brief
    if status:
    ...
    return (
        ...
        f"- DEPLOY_TARGET 참고: {target}{target_legacy_note}\n"
        f"- .deployed_target: {deployed}\n"
...
def help_text() -> str:
    return (
        ...
        "속도: v2.4부터 validate fast mode. 무거운 alias 검사는 GUARD_STATIC_ALIAS_CHECK=1일 때만"
    )
