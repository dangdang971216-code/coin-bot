...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v173: 대형 재료 보강판. 허브/1~5차 직원/공장으로 나눠 돈흐름·눌림품질·재돌파·무료전략 보조지표·실전위험을 기록한다.",
}
...
def build_precision(ticker: str, price: float) -> Dict[str, Any]:
    ...
    # v173: 무료 공개전략에서 공통으로 보이는 보조지표를 매매조건이 아닌 분석 재료로 계산한다.
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.173: 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def scan_once() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        stage_times.append(("6) 공장: latest 후보파일 + entry_context 저장", now_ts() - st, f"strict_all {pipe['paper_written']}/{pipe['paper_attempt']} / trade_ready {pipe.get('trade_ready_written',0)} / 관찰 {pipe.get('strict_observe_written',0)} / major {pipe.get('major_watch_written',0)} / shadow {pipe['shadow_written']}/{pipe['shadow_attempt']} / dup {pipe['dup_skip']}"))
...
def candidate_line() -> str:
    ...
    return (
        ...
        f"→ trade_ready {STATE.get('trade_ready_written', STATE.get('paper_written',0))} / 관찰 {STATE.get('strict_observe_written',0)} / major {STATE.get('major_watch_written',0)} / 복기 {STATE.get('shadow_written',0)} / "
...
def bot_status_text() -> str:
    ...
    return "\n".join([
        "🤖 봇 통합상태 /bot_status",
        f"✅ 메인봇: {BOT_VERSION} / scan {fnum(STATE.get('scan_last_sec'),0):.2f}s / 시장 {STATE.get('bulk_rows',0)}",
        p_line,
        guard_summary_line(),
...
def core_text() -> str:
    with _state_lock:
        s = dict(STATE)
    scan_sec = fnum(s.get('scan_last_sec'), 0)
    scan_icon = "✅" if scan_sec <= 5 and not s.get('scan_last_error') else ("⚠️" if scan_sec <= 12 else "❌")
    return "\n".join([
        "📌 종합 상태판 /core",
        f"{scan_icon} 메인봇: {BOT_VERSION} / scan {scan_sec:.2f}s / stage {s.get('scan_last_stage','-')}",
        ...
        f"✅ 정밀: 캐시 {s.get('precision_have',0)} / bulk-only {s.get('field_coverage',{}).get('bulk_only',0)} / 대기 {s.get('precision_queue_size',0)}",
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.173 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def check_text() -> str:
    ...
    return "\n".join([
        "✅ 빠른 점검 /check",
        f"- 메인봇: {BOT_VERSION} / scan {fnum(STATE.get('scan_last_sec'),0):.2f}s / stage {STATE.get('scan_last_stage','-')}",
        f"- 시장: {STATE.get('bulk_rows',0)} / 가격 {STATE.get('bulk_price',0)} / 돈 {STATE.get('bulk_money',0)} / 정밀 {STATE.get('precision_have',0)}",
        f"- 후보: strict {STATE.get('strict_decision',0)} / trade_ready {STATE.get('trade_ready_written',0)} / 관찰 {STATE.get('strict_observe_written',0)} / 복기 {STATE.get('shadow_written',0)}",
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    def avg_ctx(arr: List[Dict[str, Any]], keys: Iterable[str]) -> Tuple[float, int]:
        vals: List[float] = []
        for r in arr:
            v = ctx_get(r, keys)
            if v is None:
    ...
    def metric_line(label: str, keys: List[str], pct: bool = False) -> str:
        w, wc = avg_ctx(win_rows, keys)
        l, lc = avg_ctx(loss_rows, keys)
        suffix = "%" if pct else ""
        return f"- {label}: 승리 {w:+.2f}{suffix}({wc}) / 손실 {l:+.2f}{suffix}({lc})"
    ...
    entry_lines = [
        ...
        metric_line("MFI", ["mfi_14"]),
        metric_line("CCI", ["cci_20"]),
        metric_line("Stoch K", ["stoch_k"]),
        metric_line("ADX", ["adx_14"]),
        metric_line("V반등", ["v_rebound_score"]),
        metric_line("거래량과폭발", ["volume_spike_30x"]),
        metric_line("가격재확인", ["price_recheck_pct"], pct=True),
        metric_line("스프레드", ["orderbook_spread_pct"], pct=True),
        metric_line("틱위험", ["tick_pct_est"], pct=True),
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.173.py", "- 페이퍼봇 대상: paper_bot_v0.32.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.173: 통합상태 + 후보품질 명령 + 가벼운 점검", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.173 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v173 대형 재료보강 + 직원분리 + latest 후보파일 + shadow 격리 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
