...
# v2.5.145: 조회 실패 종목은 실시간 스캔을 막지 않게 잠깐 건너뛴다.
...
def build_shared_market_cache(force=False):
    ...
    for idx4, row in enumerate(batch_rows, start=1):
        ...
        try:
            ...
            refreshed_count += 1
            # v2.5.145: 성공한 종목은 배치 종료를 기다리지 않고 즉시 공유 캐시에 반영한다.
...
def build_review_dump_sections(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5):
    ...
    try:
        ...
        shortlist, user_items, internal_items, discard_items = get_scan_debug_display_shortlist(results, limit=3)
        lines = ["[scan_debug 간단요약]"]
        if note:
        ...
        sections.append(("scan_debug", f"[scan_debug 간단요약]\n- 생성 오류: {e}"))
...
def get_ohlcv(ticker: str, interval: str = "minute3"):
    ...
    # v2.5.145: 실패 종목은 네트워크를 다시 물고 있지 않는다.
...
def get_scan_debug_candidates():
    ...
    - market refresh 요청도 하지 않는다.
    - 내부 스캔 생산자가 저장해 둔 마지막 완성 스냅샷만 보여준다.
    """
    now_ts = time.time()
    fast = get_fast_scan_state(now_ts)
    ...
    if items:
        if fast.get('hide_candidates'):
            return [], '마지막 후보도 너무 오래돼서 표시하지 않았어. 내부 스캔은 계속 새 후보를 만드는 중이야'
        ...
        return items, age_text + cache_text + state_text

    # 후보가 없을 때도 명령어가 직접 스캔을 새로 돌리지 않는다.
    if market_refresh_in_progress:
        return [], '아직 완성된 후보 스냅샷은 없고, 내부 스캔이 시장 데이터를 받는 중이야'
    if stage:
        return [], f'아직 완성된 후보 스냅샷은 없어. 내부 스캔 단계: {stage}'
    return [], '아직 완성된 후보 스냅샷이 없어'
...
def build_scan_debug_text():
    ...
    if hide_candidates:
        if cache_age is not None:
            lines.append(f'안내: 예전 후보는 숨겼고 새 시장 데이터 다시 받는 중이야 ({cache_age}초 지난 후보 기준)')
        else:
            lines.append('안내: 새 시장 데이터 다시 받는 중이야. 잠깐 뒤에 다시 보면 돼')
        lines.append('')
        lines.append('지금 볼 후보 없음')
        return "\n".join(lines)
    if not items:
        if cache_age is not None:
            lines.append(f'안내: 최근 {cache_age}초 안 시장 데이터 기준')
        elif age_sec is not None:
            lines.append(f'안내: 최근 빠른 후보 결과 {age_sec}초 전 기준')
        else:
            lines.append('안내: 아직 볼 만한 후보가 안 잡혔어')
        lines.append('')
        lines.append('지금 볼 후보 없음')
        return "\n".join(lines)
    note = f'최근 {cache_age}초 안 시장 데이터 기준' if cache_age is not None else ('방금 고른 후보 기준' if stage else '최근 빠른 후보 기준')
    lines.append(f'안내: {note}')
    shortlist, user_items, internal_items, discard_items = get_scan_debug_display_shortlist(items, limit=3)
