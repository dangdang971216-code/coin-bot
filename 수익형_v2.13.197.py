...
def scan_once() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # v197: 최종검증을 기다리지 말고 후보가 잡힌 즉시 micro 대상파일을 먼저 갱신한다.
        # 이번 scan에서 바로 API를 막아 기다리지는 않고, sidecar가 다음 poll부터 빠르게 따라붙게 한다.
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.197.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
# v2.13.197: micro 빠른확인판(대상 우선순위/수집수/reload 개선)
# - 전략/청산/자동매수/BUY_READY는 변경하지 않는다.
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v196 손절요약·특이값제외·버전비교연결 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
