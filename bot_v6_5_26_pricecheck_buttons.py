...
def build_price_check_markup(entries):
    ...
    for idx, (item, alert_type, _reason_text) in enumerate(entries, start=1):
        ...
        label = f"{circled_number(idx)} {item.get('ticker','?')} 현재가"
        row.append(InlineKeyboardButton(label, callback_data=f"pc:{ref_id}"))
        if len(row) >= 2:
            rows.append(row)
            row = []
    if row:
...
def price_check_callback(update, context: CallbackContext):
    ...
    if current_price <= 0:
        try:
            query.answer("현재가를 못 불러왔어. 잠깐 뒤에 다시 눌러줘.", show_alert=True)
        except Exception:
            pass
        return
    ...
    lines = [
        ...
        f"- 지난 시간: {format_elapsed_text(now_ts - alert_ts)}",
        f"- 변화: {change_pct:+.2f}%",
        "",
        get_price_check_note(change_pct),
    ]
    try:
        query.answer("현재가 확인했어")
    except Exception:
        pass
    try:
        bot.send_message(chat_id=query.message.chat_id, text="\n".join(lines))
    except Exception as e:
        print(f"[텔레그램 현재가 비교 오류] {e}")
