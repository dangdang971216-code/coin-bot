...
def format_chart_quant_line(item: dict, short: bool = False) -> str:
    ...
    if short:
        return f"- 차트수치: {quant_label} / 몸통 {body3:.0f}% / 윗꼬리 {upper:.2f}% / 눌림 {pullback:.2f}% 후 회복 {recovery:.2f}% / 재돌파 {re_score:.2f} / 4시간 {htf_ready:.2f} / 일봉 {daily_ready:.2f}"
    return f"- 차트수치: {quant_label} / 몸통3봉 {body3:.0f}% / 윗꼬리 {upper:.2f}% / 아랫꼬리 {lower:.2f}% / 눌림 {pullback:.2f}% 후 회복 {recovery:.2f}% / 압축 {comp_score:.2f} / 재돌파 {re_score:.2f} / 유지 {hold_score:.2f} / 4시간준비 {htf_ready:.2f} / 일봉준비 {daily_ready:.2f}"
...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    lines = [
        ...
        f"- 차트수치: {str(row.get('chart_quant_label', '') or '차트 수치 확인중')} / 몸통3봉 {safe_float(row.get('body_ratio_3', 0), 0)*100:.0f}% / 윗꼬리 {safe_float(row.get('upper_wick_pct_1', 0), 0):.2f}% / 아랫꼬리 {safe_float(row.get('lower_wick_pct_1', 0), 0):.2f}% / 눌림 {safe_float(row.get('pullback_pct_18', 0), 0):.2f}% 후 회복 {safe_float(row.get('recovery_from_low_pct', 0), 0):.2f}% / 압축 {safe_float(row.get('compression_score', 0), 0):.2f} / 재돌파 {safe_float(row.get('rebreakout_score', 0), 0):.2f} / 유지 {safe_float(row.get('breakout_hold_score', 0), 0):.2f}",
...
def format_missed_review_log_block(row: dict, idx: int = 0) -> str:
    ...
    return "\n".join([
        ...
        *([f"- 최근30분 기준값: {format_missed_review_price_context(row)}"] if format_missed_review_price_context(row) else []),
        f"- 결과: 30분 안 최고 {safe_float(row.get('best_up_pct', 0), 0):+.2f}% / 끝 {safe_float(row.get('end_up_pct', 0), 0):+.2f}%",
        *([f"- 차트수치: {str(row.get('chart_quant_label', '') or '차트 수치 기록 없음')} / 몸통3봉 {safe_float(row.get('body_ratio_3', 0), 0)*100:.0f}% / 윗꼬리 {safe_float(row.get('upper_wick_pct_1', 0), 0):.2f}% / 재돌파 {safe_float(row.get('rebreakout_score', 0), 0):.2f} / 유지 {safe_float(row.get('breakout_hold_score', 0), 0):.2f}"] if row.get('chart_quant_label') else []),
...
def compute_rebreakout_structure_context(ticker: str, current_price: float, df1=None) -> dict:
    ...
    try:
        ...
        vol_ratio = get_vol_ratio(df, 3, 15)
        body_ratio = candle_body_ratio(df)
        upper_wick_pct = _last_upper_wick_pct(df)
        lower_wick_pct = _last_lower_wick_pct(df)
        ...
        if body_ratio >= 0.38:
            structure_score += 0.20
        if upper_wick_pct <= 0.42:
            structure_score += 0.20
        elif upper_wick_pct >= 0.90:
        ...
        if total >= 1.85 and structure_score >= 0.80:
            quant_label = "재돌파/구조 수치 좋음"
        elif total >= 1.10 or structure_score >= 0.55 or compression_score >= 0.45:
            quant_label = "재돌파/구조 수치 무난"
        elif upper_wick_pct >= 0.95 and body_ratio_3 < 0.30:
            quant_label = "윗꼬리/몸통 약함"
        elif pullback_pct >= 1.60 and recovery_pct < 0.35:
            quant_label = "눌림 뒤 회복 약함"
        ctx.update({
            "rebreakout_score": round(total, 2),
            "rebreakout_ok": bool(total >= 1.10),
            ...
            "upper_wick_pct_1": round(upper_wick_pct, 2),
            "lower_wick_pct_1": round(lower_wick_pct, 2),
