...
# ===============================
V286_VERSION_TAG = "v2.13.286_entry_survival_failure_analysis"
...
STRATEGY_KEY = "material_factory_v286"
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        *_v280_strategy_block_lines(lab, 7),
        ...
        "",
        "[6] 재료 예시",
        *_v274_top_material_lines(rows, 10 if full else 6),
        "",
        "판독: /quality는 현재 재료 snapshot입니다. 승률/수익률과 조합 성과는 /score에서 봅니다.",
    ]
    return "\n".join(lines)
...
def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        *_v280_strategy_block_lines(lab, 7),
        "",
        "[5] 고점수 재료",
        *_v274_top_material_lines(rows, 8 if full else 6),
        "",
        "[6] 다음에 반드시 채워야 할 정보",
    ...
    if full:
        lines += ["", "[full] 상세 재료", *_v274_top_material_lines(rows, 30)]
    return "\n".join(lines)
...
def _v277_write_score_cache(payload: Dict[str, Any]) -> None:  # type: ignore[override]
    ...
    try:
        ...
        log_error('v286_score_cache_write', exc)
