...
- 실거래 주문은 v2.7.6 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.6"
BOT_FILE_TAG = "수익형_v2.7.6.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.6는 현재가+체결 허브를 후보분류에 더 적극 반영한다.
WS_HUB_ON = os.getenv("V276_WS_HUB_ON", "1") not in ("0", "false", "False")
WS_URL = os.getenv("V276_WS_URL", "wss://ws-api.bithumb.com/websocket/v1")
WS_LEGACY_URL = os.getenv("V276_WS_LEGACY_URL", "wss://pubwss.bithumb.com/pub/ws")
WS_MAX_CODES = int(os.getenv("V276_WS_MAX_CODES", "160"))
WS_RECV_TIMEOUT_SEC = float(os.getenv("V276_WS_RECV_TIMEOUT_SEC", "1.0"))
WS_RECONNECT_SEC = float(os.getenv("V276_WS_RECONNECT_SEC", "5.0"))
WS_SUBSCRIBE_REFRESH_SEC = float(os.getenv("V276_WS_SUB_REFRESH_SEC", "45"))
WS_NO_MESSAGE_SWITCH_SEC = float(os.getenv("V276_WS_NO_MESSAGE_SWITCH_SEC", "25"))
WS_PRICE_FRESH_SEC = float(os.getenv("V276_WS_PRICE_FRESH_SEC", "8"))
WS_TRADE_FRESH_SEC = float(os.getenv("V276_WS_TRADE_FRESH_SEC", "20"))
WS_CANDLE_FRESH_SEC = float(os.getenv("V276_WS_CANDLE_FRESH_SEC", "12"))
WS_TRADE_ROWS_PER_TICKER = int(os.getenv("V276_WS_TRADE_ROWS", "240"))
WS_CLOSED_CANDLES_PER_TICKER = int(os.getenv("V276_WS_CLOSED_CANDLES", "8"))
...
CANDIDATE_WAREHOUSE_FILE = os.path.join(BASE_DIR, "v276_candidate_warehouse.json")
    ...
    for c in candidates:
        ...
        if not isinstance(old, CandidateStoreRow):
            old = CandidateStoreRow(
                ...
                data_quality=c.data_quality, seen_count=1, best_score=c.score, last_transition=f"NEW->{c.status}",
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.6 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    data_partial_ok = row.data_quality in ("WS_OK", "WS_PRICE_OK", "WS_PRICE_ONLY")
...
def render_error_doctor() -> str:
    ...
    return (
        f"🩺 오류/병목 진단 /error_doctor v2.7.6\n\n"
        f"1) 현재 판정\n"
        ...
        f"- 출력은 요청 시 경량 조립\n"
        f"- v2.7.6: 눌림 재확인 라인을 clean core 방식으로 복구\n\n"
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.6: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.6는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.6\n"
        f"- v2.7.6 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.6에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.6\n"
...
def render_factory_check() -> str:
    ...
    if patterns:
        ...
        lines.append("- 없음")
    lines += ["", "4) 원칙", "- 초록이 0개여도 데이터가 REST_ONLY면 정상일 수 있음", "- 창고에서 대기 후보가 쌓이고 WS/자체봉 확인 후 승격되는 흐름을 본다"]
    return "\n".join(lines)
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.6 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
...
def send_startup_message():
    txt = (
        ...
        f"- v2.6 실사용 경로 중 허브+창고+직원 1차 묶음 복구\n"
        f"- WebSocket 동적 구독 범위 확대, 후보창고/전환기록 추가\n"
        f"- /route_check /warehouse_check /factory_check로 오류 위치 확인\n"
...
def main():
    ...
    for cmd in ["ping", "system_check", "error_doctor", "speed_check", "buy_check", "signal", "data_check", "warehouse_check", "factory_check", "route_check", "core_check", "auto_status", "real_flow", "real_stats", "deploy_status", "admin_check", "server_check", "legacy_check"]:
