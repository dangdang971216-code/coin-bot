...
def send_startup_message():
    lines = [
        ...
        "- /compare /review_dump 는 저장된 표본 기준으로 바로 보여주고, review_dump는 섹션별로 나눠 보내게 정리",
...
def build_scan_debug_text():
    ...
    def append_group(title: str, items, icon: str):
        ...
        for i, item in enumerate(items, 1):
            ...
            lines.append(f"- 구분: {item.get('user_label_text', item.get('label_text','후보'))} / 5분 {safe_float(item.get('change_5',0)):+.2f}% / 거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 주목도 {safe_float(item.get('leader_score',0)):.2f}")
            ...
            if item.get('one_liner'):
                lines.append(f"- 한줄평: {item.get('one_liner')}")
            user_filter_line = build_scan_debug_user_filter_line(item)
            if user_filter_line and item.get('alert_bucket','') != 'user_alert':
                lines.append(user_filter_line)
