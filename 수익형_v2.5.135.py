...
def format_rebreakout_stat_text(item_or_row: dict, compact: bool = False) -> str:
    ...
    if attempts <= 0:
        ...
        if prep_score >= 1.00:
            ...
            prep_label = "아직 시도 전"
        return f"재돌파 준비도 {prep_score:.2f}({prep_label})"
    if success_attempts <= 0 and failed_attempts <= 0:
        return f"재돌파 {re_score:.2f}(확인중)"
    return f"재돌파 {re_score:.2f}({success_rate:.0f}% / 성공 {success_attempts}회 / 실패 {failed_attempts}회)"
...
def format_recovery_profile_text(item_or_row: dict, compact: bool = False) -> str:
    ...
    if compact:
        return f"회복질 {score:.2f} / 저점체류 {dwell}봉·재확인 {reclaim}회 / 거래대금지속 {persist:.2f}"
    return f"회복질 {score:.2f} / 저점체류 {dwell}봉·재확인 {reclaim}회 / 거래대금지속 {persist:.2f}"
...
def format_chart_quant_summary_text(item: dict, compact: bool = False) -> str:
    ...
    if abs(turnover_flash) > 0.01:
        turnover_bits.append(f"반짝점수 {turnover_flash:.2f}")
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 표현 정렬 보강: status / scan_debug / review_dump 상위 후보 문구를 같은 기준으로 맞춤",
...
def is_scan_debug_overblocked_candidate(item: dict) -> bool:
    ...
    if section == "weak":
        return True
    if "지금 사기엔 별로" in one_liner and below_high <= 1.20:
        return True
    if quant_label in {"고점 너무 가까움", "고점 근처 박스 횡보", "직전 단기고점 재접근"} and below_high <= 1.05:
        return True
    if label in {"구경만", "구경만 후보"} and change_5 <= 0.12 and vol_ratio < 1.35:
        return True
    if change_5 <= 0.00 and from_low <= 0.45 and below_high <= 1.35 and vol_ratio < 2.40 and leader < 2.40:
        return True
    return False
...
def get_scan_debug_display_shortlist(results, limit: int = 3):
    ...
    filtered = [x for x in primary_pool if not is_scan_debug_overblocked_candidate(x)]
    shortlist = filtered[:limit]
...
MARKET_REFRESH_STUCK_RESET_SEC = 70
...
FAST_SCAN_BACKGROUND_REFRESH_SEC = 7
FAST_SCAN_BACKGROUND_FORCE_REFRESH_SEC = 45
FAST_SCAN_STALE_REFRESH_COOLDOWN_SEC = 12
        ...
        try:
            ...
            if cache_ready:
                ...
                if (now_ts - last_fast_scan_stale_refresh_request_ts) >= FAST_SCAN_STALE_REFRESH_COOLDOWN_SEC:
                    request_market_refresh(force=True, reason='fast_scan_bg_stale')
...
def build_fast_scan_one_liner(primary_reason: str, from_low: float, below_high: float, grade: str = "", bucket: str = "", item: dict = None) -> str:
    ...
    if str(primary_reason or "") == "막 뜨기 직전 후보":
        return f"{label}. 아직 안 튄 편이라 먼저 붙는지 빨리 볼 자리 / 30분 저점대비 {from_low:+.2f}% / {gap_text}"
    if str(primary_reason or "") == "초입/회복 확인 후보":
        if soft_recovery:
            return f"{label}. 회복 질은 보이지만 실전 진입은 더 확인 필요 / 30분 저점대비 {from_low:+.2f}% / {gap_text}"
