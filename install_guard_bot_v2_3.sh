...
# v2.3 파일을 실제 실행 파일명으로 정규화
if [ -f backga_guard_bot_v2_3.py ]; then
  cp -f backga_guard_bot_v2_3.py backga_guard_bot.py
...
if [ ! -f backga_guard_bot.py ]; then
  echo "backga_guard_bot.py 파일이 없습니다. backga_guard_bot_v2_3.py 또는 backga_guard_bot.py를 올려주세요."
  exit 1
fi
...
if [ -f tradingbot-guard_v2_3.service ]; then
  cp -f tradingbot-guard_v2_3.service tradingbot-guard.service
fi
