...
VERSION = "ws_sidecar_v0.7_auto_candidate_force_reconnect_2026-05-16"
WS_URL = os.getenv("CLEAN_WS_HUB_URL", "wss://pubwss.bithumb.com/pub/ws").strip()
MAX_TICKERS = int(os.getenv("CLEAN_WS_HUB_MAX_TICKERS", "220"))
...
def _refresh_target_group_stats() -> None:
    """v0.7: target_meta 기준으로 현재후보/자동매매급 WS fresh 진단을 status에 넣는다."""
    try:
        ts = now()
        meta = TARGET_META if isinstance(TARGET_META, dict) else {}
        def group_check(pred):
            ticks = [t for t, m in meta.items() if pred(m if isinstance(m, dict) else {})]
            cached = fresh = stale = missing = 0
            for t in ticks:
                row = ROWS.get(t)
                if not row:
                    missing += 1
                ...
                if age <= STALE_SEC:
                    ...
                    stale += 1
            return ticks, cached, fresh, stale, missing
        cur_ticks, cur_cached, cur_fresh, cur_stale, cur_missing = group_check(lambda m: str(m.get("source") or "") == "current_candidate" or int(float(m.get("priority") or 0)) >= 130)
        auto_ticks, auto_cached, auto_fresh, auto_stale, auto_missing = group_check(lambda m: bool(m.get("auto_candidate")))
        STATS["current_candidate_targets"] = len(cur_ticks)
        STATS["current_candidate_cached"] = cur_cached
        STATS["current_candidate_fresh"] = cur_fresh
        STATS["current_candidate_stale"] = cur_stale
        STATS["current_candidate_missing"] = cur_missing
        STATS["auto_candidate_targets"] = len(auto_ticks)
        STATS["auto_candidate_cached"] = auto_cached
        STATS["auto_candidate_fresh"] = auto_fresh
        STATS["auto_candidate_stale"] = auto_stale
        STATS["auto_candidate_missing"] = auto_missing
        STATS["auto_candidate_top"] = auto_ticks[:12]
    except Exception:
        pass
...
def load_targets_from_file() -> List[str]:
    global TARGET_META, TARGET_PAYLOAD
    try:
        if TARGET_FILE.exists():
            ...
            if isinstance(obj, dict):
                TARGET_PAYLOAD = obj
                meta = obj.get("target_meta")
                TARGET_META = meta if isinstance(meta, dict) else {}
                ...
                STATS["force_reconnect"] = bool(obj.get("force_reconnect"))
                STATS["reconnect_seq"] = obj.get("reconnect_seq", "-")
                STATS["reconnect_reason"] = str(obj.get("reconnect_reason") or "-")[:160]
                STATS["auto_candidate_request_count"] = int(float(obj.get("auto_candidate_count") or 0))
                STATS["auto_candidate_request_fresh"] = int(float(obj.get("auto_candidate_ws_fresh") or 0))
                STATS["auto_candidate_request_stale"] = int(float(obj.get("auto_candidate_ws_stale") or 0))
                STATS["auto_candidate_request_missing"] = int(float(obj.get("auto_candidate_ws_missing") or 0))
            else:
                TARGET_PAYLOAD = {}
                TARGET_META = {}
...
async def run_session(symbols: List[str], target_set: set[str]) -> str:
    ...
    async with websockets.connect(WS_URL, ping_interval=20, ping_timeout=5, open_timeout=OPEN_TIMEOUT, close_timeout=1, max_queue=64) as ws:
        ...
        last_reconnect_seq = str(STATS.get("reconnect_seq", "-"))
        while not STOP:
            if now() - last_target_check >= TARGET_RELOAD_SEC:
                ...
                if cur_mtime and cur_mtime != last_target_mtime:
                    ...
                    new_seq = str(STATS.get("reconnect_seq", "-"))
                    force = bool(STATS.get("force_reconnect")) and new_seq != last_reconnect_seq
                    if set(new_symbols) != set(symbols) or force:
                        STATS["target_reload_count"] = int(STATS.get("target_reload_count", 0)) + 1
                        reason = "force_reconnect" if force else "target_changed"
                        notice = f"target file changed; fast reconnect; {reason}; seq={new_seq}; {STATS.get('reconnect_reason','-')}"
                        write_status("재구독중", "-", notice)
                        log(f"target reconnect reason={reason} seq={new_seq} symbols {len(symbols)} -> {len(new_symbols)}")
                        return "target_changed"
                    last_target_mtime = cur_mtime
                    last_reconnect_seq = new_seq
            try:
