...
VERSION = "guard_v2.5.28_fast_smart_upgrade_microfix_2026-05-15"
...
def apply_ws_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    if same_hash and not force and bool(before_state.get("alive")) and 0 <= before_age <= 60:
        try:
        ...
        return {
            ...
            "notes": ["fast-skip: hash 동일 + 실행중 + status 최신 → import/compile/restart 생략"],
            "warning": "변경 없음 → active 파일 유지", "state": before_state.get("verdict"),
            "raw": before_state.get("raw_total"), "match": before_state.get("match_ok"),
    ...
    active = "restart 생략"
    should_restart = bool(restart and (changed or force or not before_state.get("alive")))
    if should_restart:
        ...
        if restart:
            notes.append("hash 동일 + 실행중 → 재시작 생략")
        else:
...
def gws_upgrade_text(force: bool = False, explicit: Optional[str] = None) -> str:
    ...
    return (
        ...
        f"- guard: {VERSION}\n"
        f"- 방식: ws_sidecar만 교체·재시작 / 메인봇/페이퍼봇은 건드리지 않음\n"
...
def format_ws_apply_result(res: dict) -> str:
    ok = "✅" if res.get("ok") else "❌"
    changed = "교체+재시작" if res.get("changed") else ("변경없음/재시작생략" if str(res.get("active")) == "restart 생략" else "변경없음/재시작")
...
def apply_latest_auto(force: bool = False, explicit: Optional[str] = None, skip_guard: bool = False) -> str:
    ...
    progress_notify("[5/5] 외부직원 확인 중\n- WS/micro도 hash 동일 + 실행중이면 재시작하지 않음")
...
def apply_micro_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    if same_hash and not force and bool(before.get("alive")) and 0 <= before_age <= 60:
        try:
        ...
        return {
            ...
            "active": "restart 생략", "backup": "없음", "hash": short_hash(active_path),
            "notes": ["fast-skip: hash 동일 + 실행중 + status 최신 → compile/restart 생략"],
            "warning": "변경 없음 → active 파일 유지", "state": before.get("verdict"),
            "fresh": before.get("fresh"), "cached": before.get("cached"),
    ...
    before_age = float(before.get("status_age_num") or -1)
    should_restart = bool(restart and (changed or force or not before.get("alive") or before_age > 90))
    if should_restart:
        active, rn = restart_micro_sidecar(python_bin=MICRO_PYTHON_BIN)
        notes += rn
        ...
        active = "restart 생략"; state = micro_state_dict(); notes.append("hash 동일 + 실행중 → 재시작 생략")
...
def gmicro_upgrade_text(force: bool = False, explicit: Optional[str] = None) -> str:
    ...
    return (
        ...
        f"- guard: {VERSION}\n"
        f"- 방식: micro_sidecar만 교체·재시작 / 메인봇/페이퍼봇/WS는 건드리지 않음\n"
        f"- total: {fmt_sec(perf_now() - t0)}\n"
...
def service_status() -> str:
    ...
    return (
        ...
        f"- 업그레이드: /gupgrade 한 번으로 가드봇→메인봇→페이퍼봇→WS→micro 변경분만 확인/적용\n\n"
        f"📊 상태\n"
        f"- guard: {VERSION}\n"
...
def help_text() -> str:
    return (
        ...
        "운영법: 기본은 /gupgrade 한 번으로 중요도 순 적용. 개별 확인/수정이 필요할 때만 /gws_upgrade, /gmicro_upgrade를 사용.\n"
...
def set_commands() -> None:
    commands = [
        ...
        {"command": "gdeploy", "description": "배포상태"},
        {"command": "gupgrade", "description": "전체 최신화"},
        ...
        try:
            ...
            for upd in data.get("result", []):
                ...
                if text.startswith("/"):
                    ...
                    elif first in ("/gws_upgrade", "/gwsupgrade", "/gws_apply"):
                        send(chat_id, "🛰 /gws_upgrade 접수\n- WS sidecar 최신화 확인 중\n- hash 동일 + 실행중이면 재시작 생략")
                    elif first in ("/gmicro_upgrade", "/gmicroupgrade", "/gmicro_apply"):
