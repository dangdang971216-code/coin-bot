...
def get_scan_debug_display_shortlist(results, limit: int = 3):
    ...
    shortlist = [x for x in primary_pool if not is_scan_debug_overblocked_candidate(x)][:limit]
...
FAST_SCAN_INPUT_LIMIT = 12
FAST_SCAN_BUILD_STAGE1_MULTIPLIER = 1.0
FAST_SCAN_BUILD_MAX_SEC = 0.8
FAST_SCAN_BUILD_MIN_ITEMS = 3
...
def _build_fast_scan_item(ticker: str, data: dict):
    ...
    if df1 is None or len(df1) < 12:
        # fast scan은 절대 네트워크를 직접 두드리지 않는다.
        # 여기서 get_ohlcv를 부르면 scan_watchlist_fast가 40~190초씩 밀릴 수 있다.
        if FAST_SCAN_CACHE_ONLY:
