...
def format_pre_alert_snapshot_line(item: dict) -> str:
    ...
    return (
        ...
        f"최근고점까지 {snap.get('below_30m_high_pct', 0):.2f}% 남음"
    )
...
def scan_watchlist(shared_cache=None):
    ...
    if preview_entries and not main_alert_exists:
        send_watch_alert_bundle("🧭 아직 사면 안 되는 예비 후보", preview_entries[:PREWATCH_ALERT_MAX])
    if newlow_entries and not main_alert_exists:
