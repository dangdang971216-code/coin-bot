...
VERSION = "guard_v2.5.45_bundle_priority_router_2026-05-21"
...
WORKER_SPECS = {
    ...
    "review": {"label": "복기", "prefix": "review_worker", "active": "review_worker.py", "service": "tradingbot-review-worker", "status": "clean_review_worker_status.json", "log": "clean_review_worker_service.log", "fresh_limit": 240},
}
...
def git_update(allow_local_fallback: bool = False) -> tuple[bool, list[str]]:
    steps = []
    ...
    # v2.5.42: GitHub/네트워크 지연이 잦아 60초 고정 timeout으로 전체 적용이 멈추던 문제 완화.
    rc, out = run(["git", "fetch", "origin"], cwd=BOT_DIR, timeout=150)
    steps.append(f"git fetch origin: rc={rc} / timeout=150s\n{tail(out, 900)}")
    if rc != 0:
        if allow_local_fallback:
            ...
            if present:
                ...
                return True, steps
        return False, steps
    rc, out = run(["git", "reset", "--hard", f"origin/{GIT_BRANCH}"], cwd=BOT_DIR, timeout=120)
    steps.append(f"git reset --hard origin/{GIT_BRANCH}: rc={rc} / timeout=120s\n{tail(out, 900)}")
    if rc != 0:
        if allow_local_fallback:
            present, detail = _local_upgrade_targets_present()
            steps.append("로컬 fallback 확인:\n" + detail)
...
def gworker_state_text(name: Optional[str] = None) -> str:
    ...
    states = [worker_state_dict(n) for n in names if n in WORKER_SPECS]
    ok_all = all(s.get("ok") for s in states) if states else False
    title = "🧩 worker 상태 /gworker_state" if not name else f"🧩 {WORKER_SPECS[name]['label']} worker 상태"
    lines = [title, "✅ 정상" if ok_all else "⚠️ 확인 필요", "", "[worker]"]
    lines += [format_worker_state_line(s) for s in states]
    lines += ["", "[명령]", "- /gworker_upgrade : 변경 worker만 빠르게 최신 적용", "- /gstrategy_upgrade /gorderflow_upgrade /gtarget_upgrade : 개별 worker 최신 적용", "- /gworker_restart : worker 8개 재시작", "- /gscanner_state /gcandle_state /gmarket_state /gfeature_state /gorderflow_state /grisk_state /gstrategy_state /gtarget_state /greview_state"]


def wait_worker_ready(name: str, target_version: str = "", timeout: float = 18.0) -> tuple[dict, list[str]]:
...
def apply_worker_latest(name: str, force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    notes = []
    rc, out = run([WORKER_PYTHON_BIN, "-m", "py_compile", str(target)], timeout=25)
    if rc != 0:
    ...
    # v2.5.43: hash 동일 + service alive + sync이면 복사/재시작/ready 대기를 생략한다.
    # 기존 /gworker_upgrade가 변경 없는 worker도 전부 ready wait 하면서 오래 걸리던 병목 제거.
    if not changed and not force and before.get("service_active") == "active" and before.get("alive") and before.get("sync_ok"):
        return {
            ...
            "service": spec["service"],
            "state": before,
            "notes": ["fast-skip: hash 동일 + service active + sync → 복사/재시작/ready wait 생략"],
    ...
    after, wait_notes = wait_worker_ready(name, target_version=target_ver, timeout=18)
    notes += wait_notes
...
def guard_command_menu_items() -> list[dict]:
    ...
    return [
        ...
        {"command": "gupgrade_bundle", "description": "묶음 zip 1개로 전체 최신화"},
...
def gupgrade_menu_text() -> str:
    ...
    return "\n".join([
        ...
        "/gupgrade nofetch - GitHub가 느릴 때 서버 로컬 파일 기준 적용",
        "",
        "[2] 메인봇",
...
def _copy_bundle_files_to_repo(bundle_dir: Path, manifest: dict) -> list[str]:
    notes=[]
    names=[]
    for key in ["main", "guard", "paper", "ws", "micro"]:
        v = manifest.get(key)
...
def gupgrade_bundle_text(force: bool = False, explicit: Optional[str] = None, skip_git: bool = False) -> str:
    ...
    notes=[]
    if not skip_git:
        ok_git, steps = git_update(allow_local_fallback=True)
        notes += ["[git]"] + steps[-3:]
        if not ok_git:
    ...
    try:
        ...
        # py_compile: bundle에 들어있는 python 파일만 우선 검수
        for n in [x for x in os.listdir(work) if x.endswith('.py')]:
        ...
        if manifest.get('guard'):
            ...
            guard_res = format_guard_apply_result(apply_guard_first_for_smart_upgrade())
        ...
        lines += ["", "다음 확인: 메인봇 /health /score /quality /errorlog"]
        return "\n".join(lines)
    except Exception as exc:
...
def handle_single_command(text: str) -> str:
    ...
    if cmd in ("/gupgrade", "/gupgrade_all", "/apply_latest", "/upgradebot"):
        ...
        lowered_args = {str(a).lower() for a in args}
        skip_git = bool(lowered_args & {"nofetch", "local", "skipgit", "skip_git"})
        allow_local = bool(lowered_args & {"fallback", "localfallback", "local_fallback"})
        return apply_latest_auto(force=force, explicit=explicit, skip_git=skip_git, allow_local_fallback=allow_local)
    return "알 수 없는 명령어야.\n\n" + help_text()
