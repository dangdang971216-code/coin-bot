...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.157: 고정 갯수 제한 제거/적응형 정밀보강. candidate_events 없음. paper/shadow만 소비파일.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.157 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.157.py", "- 페이퍼봇 대상: paper_bot_v0.19.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.157: 고정 갯수 제한 제거 + 장세 적응형 정밀보강", "- 기존 기록 삭제 없음 / 기준점 이후 성과 별도 집계 / 후보판단 상위 N개 고정절단 없음"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.157 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
