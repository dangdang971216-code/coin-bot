...
def missed_command(update, context: CallbackContext):
    ...
    if not rows and not alert_rows:
        ...
        return
    lines = ["📌 놓친 코인 정리", "- 참고: 저장된 표본 기준으로 바로 보여줘"]
    if alert_rows:
    ...
    if rows:
        ...
        if alert_rows:
            lines += ["", f"- 한눈정리: 최근엔 알림 {len(alert_rows[:5])}건 / 놓친 강한 움직임 {len(recent)}건 표본이 있어"]
        lines += ["", "[최근 놓친 5건]"]
        for idx, row in enumerate(reversed(recent), start=1):
            lines.append("")
            lines.append(format_missed_review_log_block(row, idx=idx))
    if alert_rows:
        ...
        for idx, row in enumerate(alert_rows[:5], start=1):
            lines.append("")
            lines.append(format_alert_review_block(row, idx=idx))
    send_text_chunks("\n".join(lines))
...
def compare_command(update, context: CallbackContext):
    ...
    if not alert_rows and not missed_rows:
        ...
        return
    lines = ["📊 알림 vs 놓친 코인 비교", "- 참고: 저장된 표본 기준으로 바로 보여줘"]
    if alert_rows:
