...
# v2.13.283: precision target selector surgery
...
def _v283_material_schema(item: Dict[str, Any], mat: Dict[str, Any], strategy_scores: Dict[str, Dict[str, Any]], strategies: List[str], market: Dict[str, Any]) -> Dict[str, Any]:
    ...
    item.update({
        ...
        "strategy": "재료수집",
        "strategy_key": "material_factory_v283",
        "route": "material_factory_v283",
...
def _v274_write_material_files(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]], market: Dict[str, Any], rejects: Optional[Counter] = None) -> None:  # type: ignore[override]
    ...
    payload = {
        "version": BOT_VERSION,
        "schema": "strategy_material_snapshot_v283",
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        ...
        "note": "v283: 정밀대상 구 wrapper 삭제 + 단일 재료 schema. 기본 명령은 snapshot cache만 읽음.",
    }
...
def _v279_write_target_file(path: Path, tickers: List[str], reason: str) -> None:
    ...
    payload = {
        ...
        "target_meta": {t: {"reason": reason, "source": "material_factory_v283"} for t in tickers},
    }
    atomic_write(path, payload)
...
# ===============================
V282_VERSION_TAG = "v2.13.283_schema_surgery_cache_single_path"
...
# ===============================
# v2.13.283: scan 오류 원천수술 / precision selector 단일화
