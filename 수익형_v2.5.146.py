...
def format_chart_quant_summary_text(item: dict, compact: bool = False) -> str:
    ...
    candle_3_text = _format_chart_pct_text(body3)
    candle_5_text = _format_chart_pct_text(body5)
    close_3_text = _format_chart_pct_text(close3)
    close_5_text = _format_chart_pct_text(close5)
...
def format_mtf_context_line(item: dict, short: bool = False) -> str:
    ...
    if "데이터 부족" in htf_label or safe_float(item.get("htf_high_price", 0), 0) <= 0 or safe_float(item.get("htf_low_price", 0), 0) <= 0:
        htf_text = f"4시간 {htf_label}"
    else:
    ...
    if "데이터 부족" in daily_label or safe_float(item.get("daily_high_price", 0), 0) <= 0 or safe_float(item.get("daily_low_price", 0), 0) <= 0:
        daily_text = f"일봉 {daily_label}"
    else:
...
def build_fast_scan_one_liner(primary_reason: str, from_low: float, below_high: float, grade: str = "", bucket: str = "", item: dict = None) -> str:
    ...
    if str(primary_reason or "") == "막 뜨기 직전 후보":
        return f"{label}. 아직 완전히 붙은 건 아니라 먼저 힘이 이어지는지 볼 자리 / 30분 저점대비 {from_low:+.2f}% / {gap_text}"
...
def _format_scan_candidate_chart_core(item: dict) -> str:
    ...
    return f"- 차트 핵심: {quant} / 저점 버틴 힘 {defense:.2f} / 돈이 계속 붙는 힘 {persist:.2f} / {retry_text} / {flash} / {candle_text}"
