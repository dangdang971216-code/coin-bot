...
FAST_SCAN_INPUT_LIMIT = 18
FAST_SCAN_BUILD_STAGE1_MULTIPLIER = 1.6
FAST_SCAN_BUILD_MAX_SEC = 0.65
FAST_SCAN_BUILD_MIN_ITEMS = 4
FAST_SCAN_BUILD_HEARTBEAT_EVERY = 3
FAST_SCAN_BUILD_HARD_MAX_SEC = 0.80
...
FAST_SCAN_STAGE1_POOL_LIMIT = 64
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'strong':
        direct_live = primary_reason in {'실시간 급등 초입', '실시간 폭발 초입 후보', '초입 가능 후보'}
        ...
        if (
            primary_reason in {'실시간 급등 초입', '실시간 폭발 초입 후보'}
            ...
            return 'S', 'user_alert'
        if (
            primary_reason in {'실시간 급등 초입', '실시간 폭발 초입 후보', '초입 가능 후보', '초반 선점형'}
            ...
            change_30 = safe_float(item.get('change_30', 0), 0)
            if primary_reason in {'실시간 급등 초입', '실시간 폭발 초입 후보'} and (change_3 < 0.10 or change_30 < 0.05 or from_low >= 1.25):
...
def _rescue_internal_watch_items(items):
    ...
    for item in internal_sorted:
        ...
        if primary_reason in {'실시간 급등 초입', '실시간 폭발 초입 후보'}:
...
def _send_fast_scan_alerts(items, now_ts: float):
    ...
    for item in items:
        ...
        if any(word in one_liner for word in ['지금 사기엔 별로', '구경만', '실전 진입감은 약함']) and str(item.get('alert_bucket', '')) == 'user_alert':
            append_reverse_trace(item, stage='alert_blocked', extra_reason='알림 직전 차단: 한줄 판단이 약함', now_ts=now_ts)
            continue
        ...
        snap = prepared_item.get('pre_alert_snapshot', {}) if isinstance(prepared_item.get('pre_alert_snapshot'), dict) else {}
        msg = ''.join([
            f"{header_text}\n"
            f"━━━━━━━━━━\n"
