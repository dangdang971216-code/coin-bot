...
try:
    import pybithumb  # v155: REST bulk가 막힐 때 전체현재가 fallback용. API KEY 불필요.
except Exception:
    pybithumb = None
...
SHADOW_LIMIT = int(os.getenv("CLEAN_SHADOW_LIMIT", "28"))
...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v155: 허브/1차직원/2차직원/공장 분리, v150 전략 수치 이식, candidate_events/BUY_READY/fallback 없음",
}
...
def http_json(url: str, timeout: float = 3.0) -> Any:
    ...
    v155도 User-Agent는 ASCII 고정값만 쓴다.
    """
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.155: 허브/직원/공장 분리. candidate_events 없음. paper/shadow만 소비파일.",
    }
...
def scan_once() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        stage_times.append(("2) 2차 직원: 정밀값 보강", now_ts() - st, f"refresh {p_ok} / fail {p_fail} / cached {p_have}"))
        with _state_lock:
        ...
        stage_times.append(("3) 1차 직원: 표준값/신선도 생성", now_ts() - st, f"precision {cov.get('precision')} / bulk-only {cov.get('bulk_only')}"))
        with _state_lock:
            STATE["field_coverage"] = cov
            STATE["scan_last_stage"] = "strategy"
        st = now_ts()
        strict, shadow, rejects, examples = build_candidates(rows)
        stage_times.append(("4) 2차 직원: v150 수치 전략판단/차단이유", now_ts() - st, f"strict {len(strict)} / shadow {len(shadow)} / reject {sum(rejects.values())}"))
        with _state_lock:
        ...
        stage_times.append(("5) 공장: paper/shadow 소비파일 저장", now_ts() - st, f"paper {pipe['paper_written']}/{pipe['paper_attempt']} / shadow {pipe['shadow_written']}/{pipe['shadow_attempt']} / dup {pipe['dup_skip']}"))
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.155 기준: 새 본선 / paper·shadow 소비파일만 출력",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.155.py",
        "- 페이퍼봇 대상: paper_bot_v0.17.py",
...
def command_paper_handoff(update, context) -> None:
    lines = [
        ...
        "- 기대 paper_bot: paper_bot_v0.17.py / paper_bot_v0.17",
        "",
        "역할",
...
def command_paper_today(update, context) -> None:
    ...
    lines = [
        ...
        "",
        "판독",
        "- 오늘 성과가 0이면 아직 v2.13.155 신규 청산이 쌓이지 않은 것",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.155.py", "- 페이퍼봇 대상: paper_bot_v0.17.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.155: 기능흐름 일괄이식 새 본선", "- 메인봇=스캔·전략·paper/shadow 후보파일 / paper_bot=실행·장부·알림"]))
    rows = []
    ...
    lines = ["🧾 v2.13.155 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
