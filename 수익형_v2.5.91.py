...
def build_review_dump_text(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5) -> str:
    ...
    parts = ["🧾 review_dump", f"- 시각: {now_kst().strftime('%m-%d %H:%M:%S')}", "- 참고: 저장된 표본 기준으로 바로 보여줘"]
    if alert_rows:
        summary = summarize_alert_review_rows(alert_rows[:20])
        parts += ["", "[최근 알림 요약]", f"- 괜찮음 {summary.get('good', 0)}건 / 튀고식음 {summary.get('spike', 0)}건 / 애매 {summary.get('weak', 0)}건 / 바로밀림 {summary.get('bad', 0)}건", "", f"[최근 알림 {min(len(alert_rows), alert_limit)}건]"]
        for idx, row in enumerate(alert_rows[:alert_limit], start=1):
            parts += ["", format_alert_review_block(row, idx=idx)]
    if missed_rows:
        summary = summarize_missed_review_logs(missed_rows[-20:])
        parts += ["", "[놓친 강한 움직임 요약]", f"- 탐색 놓침 {summary.get('탐색 놓침', 0)}건 / 승격 놓침 {summary.get('승격 놓침', 0)}건 / 늦어서 안 올림 {summary.get('늦어서 안 올림', 0)}건 / 분류 놓침 {summary.get('분류 놓침', 0)}건", "", f"[최근 놓친 {min(len(missed_rows), missed_limit)}건]"]
        for idx, row in enumerate(reversed(missed_rows[-missed_limit:]), start=1):
            parts += ["", format_missed_review_log_block(row, idx=idx)]
    if trace_rows:
        parts += ["", f"[최근 역추적 {min(len(trace_rows), trace_limit)}건]"]
        for idx, row in enumerate(reversed(trace_rows), start=1):
            parts += ["", format_reverse_trace_block(row, idx=idx)]
    parts += ["", "[scan_debug]", build_scan_debug_text()]
    return "\n".join(parts)

def review_dump_command(update, context: CallbackContext):
    try:
        dump_text = build_review_dump_text()
        send_text_chunks(dump_text, prefix="🧾 분석용 덤프")
    except Exception as e:
