...
USER_ALERT_MIN_RESTART_EDGE = 5.40
USER_ALERT_MIN_RESTART_VOL = 1.55
USER_ALERT_MIN_RESTART_LEADER = 1.00
USER_ALERT_MIN_RESTART_HIGH_GAP_PCT = 0.95
USER_ALERT_MAX_RESTART_FROM_LOW_PCT = 1.95
USER_ALERT_MAX_RESTART_CHART_RISE_PCT = 3.00
USER_ALERT_PREWATCH_STRICT_ON = True
USER_ALERT_MIN_LIVE_EDGE = 6.10
USER_ALERT_MIN_LIVE_VOL = 4.80
USER_ALERT_MIN_LIVE_LEADER = 1.55
USER_ALERT_MIN_LIVE_HIGH_GAP_PCT = 1.05
USER_ALERT_MAX_LIVE_FROM_LOW_PCT = 1.85
USER_ALERT_MAX_LIVE_CHART_RISE_PCT = 3.10
USER_ALERT_LIVE_MIN_30M_PCT = 0.15
USER_ALERT_LIVE_MIN_5M_PCT = 0.70
USER_ALERT_LIVE_B_FAIL_COUNT_MIN = 2
USER_ALERT_ALLOW_HIDDEN_PROMOTION = True
USER_ALERT_PROMOTE_MIN_EDGE = 6.10
USER_ALERT_PROMOTE_MIN_VOL = 2.40
USER_ALERT_PROMOTE_MIN_LEADER = 0.95
USER_ALERT_PROMOTE_MIN_5M_PCT = 0.30
USER_ALERT_PROMOTE_MIN_30M_PCT = 0.10
USER_ALERT_PROMOTE_MIN_HIGH_GAP_PCT = 1.10
USER_ALERT_PROMOTE_MAX_FROM_LOW_PCT = 2.40
USER_ALERT_PROMOTE_MAX_CHART_RISE_PCT = 3.60
USER_ALERT_RESCUE_ON = True
USER_ALERT_RESCUE_SHORTLIST_RANK_MAX = 6
USER_ALERT_RESCUE_MIN_EDGE = 5.45
USER_ALERT_RESCUE_MIN_VOL = 1.45
USER_ALERT_RESCUE_MIN_LEADER = 0.85
USER_ALERT_RESCUE_MIN_HIGH_GAP_PCT = 0.62
USER_ALERT_RESCUE_MAX_FROM_LOW_PCT = 2.90
USER_ALERT_RESCUE_MIN_LIVE_5M_PCT = 0.25
USER_ALERT_RESCUE_MIN_RESTART_MOTION_PCT = 0.10
USER_ALERT_RESCUE_GOOD_MARKET_DAY_PCT = 1.00
USER_ALERT_RESCUE_GOOD_MARKET_MID_PCT = 0.25
USER_ALERT_LIVE_B_HARD_BLOCK_NEAR_HIGH_PCT = 1.05
USER_ALERT_LIVE_B_HARD_BLOCK_FROM_LOW_PCT = 1.70
USER_ALERT_LIVE_NEG30_BLOCK_FROM_LOW_PCT = 1.95
...
MARKET_PULSE_TOP_N = 12
MARKET_PULSE_RANK_SEEN_CUTOFF = 12
# =========================================================
# 실행 주기
# =========================================================
SCAN_INTERVAL = 6
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 상위 후보까지 간 코인은 너무 쉽게 사용자 알림에서 잘리지 않게 완화",
        "- 대신 이미 늦은 코인, 고점 바로 밑 코인, 애매한 잡음형은 계속 숨김 유지",
        "- 장이 좋은 날엔 상위 후보 승격을 조금 더 쉽게, 약한 장/횡보장은 기존처럼 보수적으로 유지",
        "- /missed 는 진짜 놓친 코인과 반짝 튄 코인을 더 잘 나누도록 품질 보강",
        "- 같은 코인 재알림과 30분 확인 추적 샘플도 같이 다듬음",
        "- 명령어 탭 문구는 쉬운 표현 유지",
        "",
        "자세한 설명은 /info",
    ]
...
def build_shared_market_cache(force=False):
    ...
    if cache:
        ...
        update_market_pulse_state(cache)
    return shared_market_cache
...
def build_market_rank_watch_candidates(cache: dict, limit_each: int = MARKET_PULSE_TOP_N):
    ...
    if not isinstance(cache, dict):
        return candidates
    snapshot = build_market_pulse_state(cache, top_n=limit_each)
...
def register_missed_review_candidate(item: dict, now_ts: float = None, seen_market: bool = False, seen_shortlist: bool = False, seen_signal: bool = False, alert_sent: bool = False, alert_section: str = "", quality_block_reason: str = ""):
    global missed_review_queue
...
def register_missed_review_market_candidates(cache: dict, now_ts: float = None):
    if not MISSED_REVIEW_ON:
        return
    now_ts = safe_float(now_ts, time.time())
    update_market_pulse_state(cache)
    picked = []
    picked.extend(build_missed_review_market_candidates(cache, limit=MISSED_REVIEW_MARKET_LIMIT))
    picked.extend(build_market_rank_watch_candidates(cache, limit_each=MARKET_PULSE_TOP_N))
    seen = set()
    for item in picked:
        ticker = str(item.get("ticker", "") or "").upper().strip()
        if not ticker or ticker in seen:
            continue
        seen.add(ticker)
        register_missed_review_candidate(item, now_ts=now_ts, seen_market=True)
...
def is_user_alert_rescue_candidate(item: dict, section: str = "", regime: dict = None) -> bool:
    ...
    trade_tier = str(item.get("trade_tier", "B") or "B").upper().strip()
    if high_gap_pct < USER_ALERT_RESCUE_MIN_HIGH_GAP_PCT:
        return False
    if from_low > USER_ALERT_RESCUE_MAX_FROM_LOW_PCT:
        return False
    if chart_rise > 4.40 and high_gap_pct < 1.60:
        return False
    if edge < USER_ALERT_RESCUE_MIN_EDGE:
        return False
    if vol_ratio < USER_ALERT_RESCUE_MIN_VOL:
        return False
    if leader < USER_ALERT_RESCUE_MIN_LEADER:
        return False
    if section == "live":
        if chg_5 < USER_ALERT_RESCUE_MIN_LIVE_5M_PCT and chg_30 < 0.20:
            return False
        if trade_tier == "B" and high_gap_pct < 0.80 and from_low > 2.20 and chg_5 < 0.45:
            return False
    elif section == "restart":
        if max(chg_5, chg_30, from_low) < USER_ALERT_RESCUE_MIN_RESTART_MOTION_PCT:
            return False
...
def get_user_alert_block_reason(item: dict, display_key: str = "") -> str:
    ...
    if section == "live":
        if high_gap_pct < USER_ALERT_MIN_LIVE_HIGH_GAP_PCT and not rescue_ok:
            return "실시간 강세로 보기엔 바로 위 고점이 너무 가까움"
        if from_low > USER_ALERT_MAX_LIVE_FROM_LOW_PCT and high_gap_pct < 1.70 and not rescue_ok:
            return "실시간 강세 후보인데 저점에서 이미 꽤 올라와서 제외"
        if chart_rise > USER_ALERT_MAX_LIVE_CHART_RISE_PCT and high_gap_pct < 2.00 and not rescue_ok:
            return "실시간 강세 후보인데 차트상 이미 위쪽 자리라 제외"
        if chg_5 < USER_ALERT_LIVE_MIN_5M_PCT and chg_30 < USER_ALERT_LIVE_MIN_30M_PCT and not rescue_ok:
            return "실시간 강세라기엔 방금 붙는 힘이 약함"
        if chg_30 <= USER_ALERT_LIVE_MIN_30M_PCT and from_low >= 1.30 and high_gap_pct < 1.45 and not rescue_ok:
            return "실시간 강세 후보인데 이미 조금 오른 뒤라 사용자 알림에선 제외"
        if trade_tier == "B":
            if high_gap_pct <= USER_ALERT_LIVE_B_HARD_BLOCK_NEAR_HIGH_PCT and from_low >= USER_ALERT_LIVE_B_HARD_BLOCK_FROM_LOW_PCT and not rescue_ok:
                return "실시간 강세 B인데 이미 조금 오른 뒤라 지금 볼 가치가 낮음"
            if chg_30 < 0 and from_low >= USER_ALERT_LIVE_NEG30_BLOCK_FROM_LOW_PCT and not rescue_ok:
            ...
            if leader < USER_ALERT_MIN_LIVE_LEADER:
                fail_count += 1
            if fail_count >= USER_ALERT_LIVE_B_FAIL_COUNT_MIN and not rescue_ok:
                return "실시간 강세 B지만 아직 해볼만한 수준까진 아님"
    if section != "restart":
        return ""
    if high_gap_pct < USER_ALERT_MIN_RESTART_HIGH_GAP_PCT and not rescue_ok:
        return "재출발로 보기엔 바로 위 고점이 너무 가까움"
    if from_low > USER_ALERT_MAX_RESTART_FROM_LOW_PCT and high_gap_pct < 1.25 and not rescue_ok:
        return "재출발 후보인데 저점에서 이미 꽤 올라와서 제외"
    if chart_rise > USER_ALERT_MAX_RESTART_CHART_RISE_PCT and high_gap_pct < 1.80 and not rescue_ok:
    ...
    if trade_tier == "B" and edge < USER_ALERT_MIN_RESTART_EDGE and vol_ratio < USER_ALERT_MIN_RESTART_VOL and leader < USER_ALERT_MIN_RESTART_LEADER and not rescue_ok:
...
def scan_watchlist(shared_cache=None):
    ...
    regime = get_market_regime()
    try:
        register_missed_review_market_candidates(cache, now_ts=time.time())
    except Exception as e:
    ...
    for item in top:
        ...
        base_display_key = get_watch_display_group(item, base_judgement, base_caution, regime=regime)
        item, refresh_ok, _refresh_reason = refresh_watch_item_before_alert(item, section_key=base_display_key, regime=regime)
        if not refresh_ok:
            continue
    ...
    late_entries = section_entries["late"]
    watch_entries = section_entries["watch"]
    if len(early_entries) + len(restart_entries) + len(live_entries) < 2 and skipped_top:
        promoted_pool = []
    ...
    if restart_entries:
        ...
        sent_any_alert = True
    # 늦음/감시 후보는 사용자 알림으로 보내지 않고 내부 기록만 남긴다.
...
def register_bot_commands():
    try:
        bot.set_my_commands([
            ...
            BotCommand("summary_strategy", "전략 요약"),
            BotCommand("today_strategy", "오늘 전략"),
