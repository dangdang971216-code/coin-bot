...
VERSION = "bithumb_micro_sidecar_v0.5_wide_target_rotate_poll_2026-05-15"
TARGET_SAFETY_MAX = int(os.getenv("CLEAN_MICRO_TARGET_SAFETY_MAX", os.getenv("CLEAN_MICRO_MAX_TICKERS", "180")))
POLL_BATCH_MAX = int(os.getenv("CLEAN_MICRO_POLL_BATCH_MAX", "72"))
PRIORITY_KEEP = int(os.getenv("CLEAN_MICRO_PRIORITY_KEEP", "48"))
...
USER_AGENT = "coinbot-bithumb-micro-sidecar/0.5"
    ...
    while not STOP:
        try:
            ...
            if not targets:
                ...
                time.sleep(POLL_SEC)
                continue
            # v0.5: target 목록은 넓게 유지하고, 실제 REST 호출은 우선순위+회전식 batch로 수행한다.
