...
VERSION = "guard_v2.5.10_paper_pid_status_reason_2026-05-14"
...
def gpaper_state_text() -> str:
    ...
    st = read_json(BOT_DIR / "paper_bot_status.json")
    try:
        status_pid = int(float(st.get("pid") or st.get("process_pid") or 0))
    except Exception:
        status_pid = 0
    ...
    if not alive:
        verdict = "❌ 죽음: 실제 pid가 살아있지 않음"
    elif mismatch:
        verdict = "⚠️ 확인필요: pid 파일과 status pid가 다름"
    elif stale:
    ...
    return "\n".join([
        ...
        f"- service: {PAPER_SERVICE} / exists={svc_exists} / active={svc_active}",
        f"- active_file: {PAPER_ACTIVE_FILE} / version={extract_paper_version(active_path)} / hash={short_hash(active_path)}",
        f"- pid_file: {pid_file or '-'} / status_pid: {status_pid or '-'} / used: {used_pid or '-'} / alive={alive}",
...
def apply_latest_auto(force: bool = False, explicit: Optional[str] = None) -> str:
    ...
    return (
        f"{title}\n"
        f"- guard: {VERSION}\n"
        f"- 방식: main/paper 각각 비교 후 바뀐 것만 교체·재시작\n"
