...
FAST_SCAN_CONSUMER_HARD_HIDE_SEC = 180
...
def get_fast_scan_state(now_ts: float = None) -> dict:
    ...
    with fast_scan_state_lock:
        ...
        hide_candidates = bool(
            (age_sec is not None and age_sec >= FAST_SCAN_CONSUMER_HARD_HIDE_SEC)
            or ((not latest_fast_scan_items) and cache_age is not None and cache_age >= FAST_SCAN_CONSUMER_HARD_HIDE_SEC)
        )
        return {
...
def build_scan_health_warning_text(now_ts: float = None) -> str:
    ...
    if cache_age is not None and cache_age >= FAST_SCAN_STATE_STALE_SEC:
...
def get_latest_scan_display_state(now_ts: float = None, limit: int = 3) -> dict:
    ...
    if items and not hide_candidates:
        ...
        results = []
        note = '마지막 후보도 너무 오래돼서 표시하지 않았어. 내부 스캔은 계속 새 후보를 만드는 중이야'
    else:
        results = []
        if market_refresh_in_progress:
