...
UNIVERSE_FORCE_FULL_BATCH = 8
UNIVERSE_MIN_PARTIAL_ROWS = 4
UNIVERSE_BUILD_MAX_SEC = 3
CACHE_REFRESH_BATCH = 4
CACHE_FORCE_FULL_BATCH = 8
CACHE_MIN_PARTIAL_ROWS = 4
...
def format_chart_quant_summary_text(item: dict, compact: bool = False) -> str:
    ...
    if current_candle_uninformative:
        ...
        candle_1_text = f"{body1:.0f}%"
        close_1_text = f"{close1:.0f}%"
    ...
    if compact:
        return (
            f"{quant_label} / 몸통1봉 {candle_1_text}·3봉 {body3:.0f}%·5봉 {body5:.0f}% / "
            f"종가강도 1봉 {close_1_text}·5봉 {close5:.0f}% / 윗꼬리 1봉 {wick_1_text}·5봉 {upper5:.2f}% / "
    ...
    return (
        f"{quant_label} / 몸통1봉 {candle_1_text}·3봉 {body3:.0f}%·5봉 {body5:.0f}% / "
        f"종가강도 1봉 {close_1_text}·3봉 {close3:.0f}%·5봉 {close5:.0f}% / "
...
def is_scan_debug_overblocked_candidate(item: dict) -> bool:
    ...
    if quant_label in {"고점 너무 가까움", "고점 근처 박스 횡보", "직전 단기고점 재접근"} and below_high <= 1.20:
        return True
    if label in {"구경만", "구경만 후보"} and (change_5 <= 0.18 or recovery_speed <= 0.06) and below_high <= 1.25:
        return True
...
FAST_SCAN_STATE_STALE_SEC = 75
FAST_SCAN_DISPLAY_HIDE_SEC = 120
FAST_SCAN_BACKGROUND_REFRESH_SEC = 4
FAST_SCAN_BACKGROUND_FORCE_REFRESH_SEC = 20
FAST_SCAN_STALE_REFRESH_COOLDOWN_SEC = 8
