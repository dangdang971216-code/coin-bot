...
VERSION = "bithumb_micro_sidecar_v0.9_urgent_fast_lane_2026-05-17"
...
URGENT_POLL_BATCH_MAX = int(os.getenv("CLEAN_MICRO_URGENT_POLL_BATCH_MAX", "18"))
...
USER_AGENT = "coinbot-bithumb-micro-sidecar/0.9"
...
def poll_loop() -> None:
    ...
    while not STOP:
        try:
            ...
            urgent_batch = list(dict.fromkeys([to_ticker(t) for t in URGENT_TARGETS if to_ticker(t)]))[:max(0, URGENT_POLL_BATCH_MAX)]
            urgent_done = 0
            urgent_elapsed = 0.0
            if urgent_batch:
                write_status("긴급수집중", "-")
