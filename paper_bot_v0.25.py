...
FILES = {
    ...
    "baseline": BASE_DIR / "paper_eval_baseline_v165.json",
...
DEFAULT_CONTROL = {
    ...
    "open_shadow_positions": False,  # v0.25 기본값: shadow는 OPEN하지 않고 복기 전용
...
def load_control() -> Dict[str, Any]:
    ...
    if isinstance(saved, dict):
        for k, v in saved.items():
            ...
            if k in DEFAULT_CONTROL:
                control[k] = v
    # v0.25 고정: candidate_events 소비 옵션은 저장돼 있어도 다시 만들지 않는다.
    ...
    for k, old_v in legacy_defaults.items():
        try:
            ...
            pass
    try:
        # v0.25: paper_bot은 *_latest 후보파일을 우선 읽는다. 큰 archive 파일을 매 cycle 읽지 않는다.
...
def quarantine_shadow_open_positions(open_pos: Dict[str, Dict[str, Any]], control: Dict[str, Any]) -> Tuple[Dict[str, Dict[str, Any]], int]:
    ...
    for pid, pos in (open_pos or {}).items():
        ...
        if lane == "shadow":
            ...
            row["quarantine_reason"] = "v0.25_shadow_review_only"
...
def pick_candidates(control: Dict[str, Any], open_pos: Dict[str, Dict[str, Any]]) -> Tuple[List[Tuple[str, Dict[str, Any]]], Dict[str, int]]:
    ...
    if open_shadow:
        ...
        # v0.25: shadow는 복기 전용. OPEN 장부에 태우지 않는다.
...
def file_stats() -> Dict[str, Any]:
    out = {}
    for name in ["paper_candidates", "shadow_candidates", "paper_latest", "shadow_latest", "shadow_quarantine", "open", "closed", "status", "error", "log"]:
        p = FILES[name]
...
def summary_text() -> str:
    ...
    return "\n".join([
        ...
        f"- 소비파일: 정식 latest {fs.get('paper_latest',{}).get('lines',0)} lines(TTL유효 {p_fresh.get('fresh',0)}) / 복기 latest {fs.get('shadow_latest',{}).get('lines',0)} lines(TTL유효 {s_fresh.get('fresh',0)}) / shadow 격리 {fs.get('shadow_quarantine',{}).get('lines',0)}",
        f"- shadow 처리: 복기전용 / 신규 OPEN 안 함 / 기존 shadow OPEN은 격리 보존",
...
def control_text() -> str:
    c = load_control()
    keys = ["running", "loop_seconds", "max_open_strict", "max_open_shadow", "max_new_per_cycle", "candidate_ttl_sec", "candidate_read_max_lines", "consume_latest_scan_only", "consume_latest_candidate_files", "quarantine_shadow_open", "latest_scan_window_sec", "fee_pct_roundtrip", "take_profit_pct", "protect_trigger_pct", "protect_floor_pct", "stop_loss_pct", "slow_minutes", "time_exit_minutes", "block_same_ticker_open"]
