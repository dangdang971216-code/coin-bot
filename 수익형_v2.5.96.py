...
def get_shortlist_user_rescue_display_key(item: dict, display_key: str = "", regime: dict = None) -> str:
    ...
    if not is_good_market_for_user_rescue(regime=regime) and not strong_rank_rescue:
        return ""
    snap = ensure_pre_alert_snapshot(item)
    high_gap_pct = get_effective_high_gap_pct(item)
    from_low = safe_float(snap.get("from_30m_low_pct", 0), 0)
    ...
    hot_score = get_user_alert_hot_score(item)
    motion = max(chg_5, chg_30, from_low)
    if hot_score < USER_ALERT_RESCUE_MIN_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG and not top_turnover_rescue:
        return ""
    if high_gap_pct < 0.48:
        return ""
    if daily_ready <= -0.70 and market_flow_score < 1.40:
        return ""
    if from_low > 3.35 and high_gap_pct < 1.10 and not top_turnover_rescue:
        return ""
    if chart_rise > 4.80 and high_gap_pct < 1.45 and not top_turnover_rescue:
        return ""
    if edge < USER_ALERT_RESCUE_MIN_EDGE and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG and not top_turnover_rescue:
        return ""
    if vol_ratio < USER_ALERT_RESCUE_MIN_VOL and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG and not top_turnover_rescue:
        return ""
    if leader < USER_ALERT_RESCUE_MIN_LEADER and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG and not top_turnover_rescue:
        return ""
    if top_turnover_rescue and high_gap_pct >= 1.60 and motion >= 0.05:
        return "restart"
    if shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG and high_gap_pct >= 0.72 and motion >= 0.08 and chart_rise <= 4.20:
        if vol_ratio >= 0.95 or leader >= 0.38 or edge >= 4.45:
            return "restart"
    if shortlist_rank <= 5 and turnover_rank <= 30 and leader_rank <= 20 and high_gap_pct >= 1.55 and from_low <= 1.45 and chart_rise <= 3.95:
        if chg_30 >= -1.15 and (vol_ratio >= 0.85 or leader >= 0.65 or edge >= 4.55 or market_flow_score >= 1.40 or daily_ready >= 0.55):
            return "restart"
    if shortlist_rank <= 6 and high_gap_pct >= 1.90 and from_low <= 1.95 and chart_rise <= 4.05:
        if vol_ratio >= 0.82 or leader >= 0.18 or edge >= 4.35 or daily_ready >= 0.45 or breakout_hold_score >= 0.45:
            return "restart"
    if chg_5 >= 0.48 and high_gap_pct >= 0.92 and (chg_30 >= 0.05 or vol_ratio >= 1.75 or leader >= 0.92 or hot_score >= USER_ALERT_RESCUE_STRONG_HOT_SCORE):
        return "live"
    if motion >= 0.03 and high_gap_pct >= 0.52 and (vol_ratio >= 0.95 or leader >= 0.40 or edge >= 4.50 or strong_rank_rescue):
        return "restart"
    return ""
...
def is_user_alert_rescue_candidate(item: dict, section: str = "", regime: dict = None) -> bool:
    ...
    chart_ctx = ensure_alert_chart_context(item)
    top_turnover_rescue = is_top_turnover_rescue_candidate(item)
    strong_rank_rescue = (shortlist_rank <= 6 and hot_score >= 2) or top_turnover_rescue

    if not is_good_market_for_user_rescue(regime=regime) and not strong_rank_rescue:
        return False
    if str(chart_ctx.get("chart_state_code", "")) in {"FADE_AFTER_SPIKE", "HIGH_WOBBLE"} and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False
    if high_gap_pct < USER_ALERT_RESCUE_MIN_HIGH_GAP_PCT and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False
    if from_low > USER_ALERT_RESCUE_MAX_FROM_LOW_PCT and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False
    if chart_rise > 4.40 and high_gap_pct < 1.60 and not top_turnover_rescue:
        return False
    if daily_ready <= -0.65 and market_flow_score < 1.35 and not top_turnover_rescue:
        return False
    if breakout_hold_score <= -0.20 and high_gap_pct < 1.00 and chg_5 < 0.20 and chg_30 < 0.20 and not top_turnover_rescue:
        return False
    if edge < USER_ALERT_RESCUE_MIN_EDGE and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False
    if vol_ratio < USER_ALERT_RESCUE_MIN_VOL and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False
    if leader < USER_ALERT_RESCUE_MIN_LEADER and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False

    if section == "live":
        if chg_5 < USER_ALERT_RESCUE_MIN_LIVE_5M_PCT and chg_30 < 0.10 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
            return False
        if trade_tier == "B" and high_gap_pct < 0.70 and from_low > 2.35 and chg_5 < 0.30 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
            return False
        return True

    if section == "restart":
        if max(chg_5, chg_30, from_low) < USER_ALERT_RESCUE_MIN_RESTART_MOTION_PCT and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE:
            if not (shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG and high_gap_pct >= 0.80 and (vol_ratio >= 0.95 or leader >= 0.40)) and not top_turnover_rescue:
                return False
        if top_turnover_rescue and high_gap_pct >= 1.60:
            return True
        if shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG and high_gap_pct >= 0.80 and max(chg_5, chg_30, from_low) >= 0.08:
            if vol_ratio >= 0.95 or leader >= 0.40 or edge >= 4.45:
                return True
        if shortlist_rank <= 5 and turnover_rank <= 30 and leader_rank <= 20 and high_gap_pct >= 1.55 and from_low <= 1.45 and chart_rise <= 3.95:
            if chg_30 >= -1.15 and (vol_ratio >= 0.85 or leader >= 0.65 or edge >= 4.55):
                return True
        if shortlist_rank <= 6 and high_gap_pct >= 1.90 and from_low <= 1.95 and chart_rise <= 4.05:
            if vol_ratio >= 0.82 or leader >= 0.18 or edge >= 4.35 or daily_ready >= 0.45 or breakout_hold_score >= 0.45:
                return True
        if chg_30 >= 0.12 and high_gap_pct >= 0.72 and from_low <= 2.90:
            return True
        return True

    # early rescue는 너무 약한 흐름은 막고, 상위 shortlist + 차트 여유가 있으면 통과
    if high_gap_pct < 0.55 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False
    if max(chg_5, chg_30, from_low) < 0.05 and vol_ratio < 1.05 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not top_turnover_rescue:
        return False
    return True
...
def get_user_alert_block_reason(item: dict, display_key: str = "") -> str:
    ...
    breakout_hold_score = safe_float(item.get("breakout_hold_score", 0), 0)
    breakout_hold_ok = bool(item.get("breakout_hold_ok", False))
    market_flow_score = safe_float(item.get("market_flow_score", 0), 0)
    ...
    if section == "early":
        ...
        if daily_ready <= -0.55 and daily_gap_pct <= 1.20 and not rescue_ok and not trial_ok:
            return "분봉은 보여도 일봉상 이미 위쪽이라 제외"
        if breakout_hold_score <= -0.10 and high_gap_pct < 1.00 and chg_5 < 0.45 and not trial_ok:
            return "초반처럼 보여도 돌파 뒤 버티는 힘이 아직 부족"
        if market_flow_score < 0.55 and vol_ratio < 1.40 and leader < 0.90 and not trial_ok:
            return "개별 움직임은 보여도 시장 돈 흐름이 아직 약함"
        if user_grade == "C" and high_gap_pct < 0.75 and not trial_ok:
            return "초반 선점형이라도 바로 위 고점이 너무 가까워 제외"
        return ""
    if section == "live":
        if is_post_spike_fade_setup(item) and not trial_ok:
            return "실시간 강세처럼 보여도 이미 한 번 쏜 뒤 식는 자리"
        if daily_ready <= -0.60 and daily_zone_ratio >= 0.88 and daily_gap_pct <= 1.40 and not rescue_ok:
            return "분봉은 강해도 일봉상 이미 위쪽이라 제외"
        if breakout_hold_score <= -0.15 and high_gap_pct < 1.05 and chg_5 < 0.70 and not rescue_ok and not trial_ok:
            return "실시간 강세처럼 보여도 돌파 뒤 유지가 약함"
        if market_flow_score < 0.70 and leader < 1.00 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not rescue_ok:
        ...
        if trade_tier == "B":
            ...
            if leader < USER_ALERT_MIN_LIVE_LEADER:
                fail_count += 1
            if fail_count >= (USER_ALERT_LIVE_B_FAIL_COUNT_MIN + (1 if hot_score <= USER_ALERT_RESCUE_MIN_HOT_SCORE else 0)) and not rescue_ok and not trial_ok:
                return "실시간 강세 B지만 아직 해볼만한 수준까진 아님"
        if user_grade in {"B", "C"} and not rescue_ok and not trial_ok:
    ...
    if daily_ready <= -0.55 and daily_zone_ratio >= 0.88 and daily_gap_pct <= 1.30 and not rescue_ok:
        return "재출발처럼 보여도 일봉상 이미 위쪽이라 제외"
    if breakout_hold_score <= -0.20 and high_gap_pct < 1.00 and chg_5 < 0.25 and chg_30 < 0.20 and not rescue_ok:
        return "재출발 후보지만 돌파 뒤 유지가 약해서 제외"
    if market_flow_score < 0.45 and shortlist_rank > 4 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and not rescue_ok:
    ...
    if strategy in {"PREWATCH", "NEWLOW_RECOVER"} and USER_ALERT_PREWATCH_STRICT_ON:
        prewatch_rescue_ok = (
            shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG
            and high_gap_pct >= 1.20
            and from_low <= 1.95
            and chart_rise <= 3.45
            and max(chg_5, chg_30, from_low) >= 0.12
            and (vol_ratio >= 1.00 or leader >= 0.32 or edge >= 4.45)
        )
        if edge < max(USER_ALERT_MIN_RESTART_EDGE, 5.65) or vol_ratio < max(USER_ALERT_MIN_RESTART_VOL, 1.70) or leader < max(USER_ALERT_MIN_RESTART_LEADER, 1.10):
            if not rescue_ok and not prewatch_rescue_ok:
