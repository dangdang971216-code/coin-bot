...
def run_once()->Dict[str,Any]:
    ...
    for r in features:
        ...
        rows.append(rr)
    save_json(OUT,{"version":VERSION,"schema":"orderflow_summary_v2","updated_ts":ts,"updated_text":now_text(ts),"row_count":len(rows),"fresh_ws":fresh_ws,"fresh_micro":fresh_micro,"ws_cache_count":len(ws),"micro_cache_count":len(micro),"ws_ttl_sec":WS_TTL_SEC,"micro_ttl_sec":MICRO_TTL_SEC,"rows":rows})
    sec=now_ts()-st; write_status("running", row_count=len(rows), fresh_ws=fresh_ws, fresh_micro=fresh_micro, ws_cache_count=len(ws), micro_cache_count=len(micro), ws_ttl_sec=WS_TTL_SEC, micro_ttl_sec=MICRO_TTL_SEC, last_sec=round(sec,3))
