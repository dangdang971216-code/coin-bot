    ...
    for ticker, data in cache.items():
        try:
            ...
            if momentum_hint <= 0.12:
                continue
            if not (1.90 <= vol_ratio <= 4.8):
                continue
            if not (47 <= rsi <= 66):
                continue
            if not (0.90 <= high_gap_pct <= 2.60):
                continue
            if range_pct >= 2.6:
                continue
            ...
            if is_recent_spike_near_high(df, current_price) or is_box_top_range_chase(df, current_price) or is_retest_after_failed_peak(df) or is_false_start_near_high(df, current_price):
                continue
            if rebound_from_low_pct >= 1.05 and high_gap_pct <= 1.20:
                continue
    ...
    for ticker, data in cache.items():
        try:
            ...
            if leader_score < 1.05 and vol_ratio < 3.00:
                continue
            if not (0.12 <= low_gap_pct <= 0.95):
                continue
            if high_gap_pct <= 1.20:
                continue
            if range_pct >= 2.4:
                continue
