...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.175: 후보분석은 strict_all 전부 기록, latest는 현재 scan 기준으로 유지한다. paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def core_text() -> str:
    ...
    return "\n".join([
        ...
        f"✅ 직원: 2차 눌림·3차 재돌파·4차 위험·5차 분류 / major {s.get('major_watch_count',0)}",
        f"🔎 정보커버리지: 정밀 {s.get('field_coverage',{}).get('precision',0)} / ema {s.get('field_coverage',{}).get('ema12',0)} / bb {s.get('field_coverage',{}).get('bb',0)} / mfi {s.get('field_coverage',{}).get('mfi',0)} / 위험캐시 {s.get('execution_risk_cached',0)}",
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.175 기준: 정확도 안정화 / latest·중복skip·정보품질 표시 정리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.175.py",
        "- 페이퍼봇 대상: paper_bot_v0.32.py",
...
def candidate_quality_text(full: bool = False) -> str:
    """후보품질 전용 분석. 스캔을 새로 돌리지 않고 저장파일/상태만 읽는다.
    v172: CLOSED에 저장된 entry_context를 우선 읽고, 없으면 과거 top-level 필드 alias를 읽는다.
    """
    rows = load_closed(limit=30000 if full else 12000)
    new_rows = rows_since_baseline(rows, "closed_at")
    ...
    def metric_line(label: str, keys: List[str], pct: bool = False, minv: Optional[float] = None, maxv: Optional[float] = None) -> str:
        w, wc = avg_ctx(win_rows, keys, minv=minv, maxv=maxv)
        l, lc = avg_ctx(loss_rows, keys, minv=minv, maxv=maxv)
        suffix = "%" if pct else ""
        if wc == 0 and lc == 0:
            return f"- {label}: 정보없음 (승리0/손실0) / 원인: 새 CLOSED에 아직 미저장 또는 정밀값 미수집"
        wtxt = f"{w:+.2f}{suffix}({wc})" if wc > 0 else "정보없음(0)"
        ltxt = f"{l:+.2f}{suffix}({lc})" if lc > 0 else "정보없음(0)"
        return f"- {label}: 승리 {wtxt} / 손실 {ltxt}"

    cov_change = avg_ctx(strict, ["change_5", "change_5m"])[1]
    cov_high = avg_ctx(strict, ["below_30m_high_pct", "below_high_pct", "high_gap_pct"])[1]
    cov_vol = avg_ctx(strict, ["vol_ratio", "volume_ratio"])[1]
    cov_pbq = avg_ctx(strict, ["pullback_quality_score"])[1]
    entry_lines = [
        "[4/7] 진입 당시 재료 비교",
        "- 전략화 가능: 진입 전에 이미 알 수 있던 값만 후보로 봄",
        metric_line("점수", ["score", "edge", "edge_score"]),
        metric_line("5분흐름", ["change_5", "change_5m"], pct=True),
        metric_line("고점거리", ["below_30m_high_pct", "below_high_pct", "high_gap_pct"], pct=True),
        metric_line("저점대비", ["from_30m_low_pct", "from_low_pct"], pct=True),
        metric_line("거래량배수", ["vol_ratio", "volume_ratio"]),
        metric_line("1분돈흐름", ["money_flow_1m", "turnover_1m"]),
        metric_line("3분돈흐름", ["money_flow_3m", "turnover_3m"]),
        metric_line("눌림품질", ["pullback_quality_score"]),
        metric_line("재돌파힘", ["rebreakout_strength"]),
        metric_line("가짜반등", ["fake_bounce_score"]),
        metric_line("EMA5거리", ["ema5_gap_pct", "ma5_gap_pct"], pct=True),
        metric_line("EMA12거리", ["ema12_gap_pct"], pct=True),
        metric_line("볼린저위치", ["bb_position"]),
        metric_line("볼린저하단거리", ["bb_lower_gap_pct"], pct=True),
        metric_line("MFI", ["mfi_14"], minv=0, maxv=100),
        metric_line("CCI", ["cci_20"]),
        metric_line("Stoch K", ["stoch_k"], minv=0, maxv=100),
        metric_line("ADX", ["adx_14"], minv=0, maxv=100),
        metric_line("V반등", ["v_rebound_score"]),
        metric_line("거래량과폭발", ["volume_spike_30x"]),
        metric_line("가격재확인", ["price_recheck_pct"], pct=True, minv=-PRICE_RECHECK_ABNORMAL_PCT, maxv=PRICE_RECHECK_ABNORMAL_PCT),
        metric_line("스프레드", ["orderbook_spread_pct"], pct=True, minv=0, maxv=SPREAD_ABNORMAL_PCT),
        metric_line("틱위험", ["tick_pct_est"], pct=True, minv=0, maxv=5.0),
        f"- 추격위험: 승리 {top_ctx(win_rows, ['chase_risk'])} / 손실 {top_ctx(loss_rows, ['chase_risk'])}",
        f"- 실전위험: 승리 {top_ctx(win_rows, ['execution_risk_status'])} / 손실 {top_ctx(loss_rows, ['execution_risk_status'])}",
        f"- 대형주분리: 승리 {top_ctx(win_rows, ['major_watch_label', 'major_watch'])} / 손실 {top_ctx(loss_rows, ['major_watch_label', 'major_watch'])}",
        f"- 위험라벨: 승리 slip {top_ctx(win_rows, ['slippage_risk'], 2)} · tick {top_ctx(win_rows, ['tick_risk'], 2)} / 손실 slip {top_ctx(loss_rows, ['slippage_risk'], 2)} · tick {top_ctx(loss_rows, ['tick_risk'], 2)}",
    ]
    if strict and (cov_change == 0 or cov_high == 0 or cov_vol == 0):
        entry_lines.append(f"⚠️ 일부 과거 CLOSED는 진입재료가 비어 있음: 5분 {cov_change}/{len(strict)} / 고점거리 {cov_high}/{len(strict)} / 거래량 {cov_vol}/{len(strict)} / 눌림품질 {cov_pbq}/{len(strict)}")
    if len(strict) < 50:
        entry_lines.append("❔ 표본 부족: 아직 조건 수정 근거로 쓰지 않음")

    lines = [
        "🔬 후보품질 분석 /quality" + ("_full" if full else ""),
        "- 새 스캔 실행 안 함 / 저장된 CLOSED·latest·상태만 읽음",
        "",
        "[1/7] 핵심 성과",
        fmt_stats("신규 trade_ready/strict", strict),
        group_table("- 시간대", strict, bucket_hour3, limit=8 if full else 4),
        group_table("- 점수/등급", strict, bucket_auto_ready, limit=8 if full else 4),
        "",
        "[2/7] 현재 후보 분리",
        f"- latest strict 전체 {len(latest_strict)} / trade_ready {len(trade_ready)} / 관찰 {len(observe)} / major {sum(1 for r in latest_strict if r.get('major_watch'))}",
        f"- latest shadow 복기 {len(latest_shadow)} / 신규 OPEN 안 함",
        f"- 기준: latest는 현재 scan 후보, archive는 중복 제거된 누적 기록",
        "",
        "[3/7] 사후 결과 분석",
        "⚠️ 아래 청산사유는 매매 후에 알 수 있는 결과입니다. 이 이름 자체를 전략 조건으로 쓰면 안 됩니다.",
    ]
    if by_reason:
        for k, _ in by_reason.most_common(8 if full else 5):
            lines.append(fmt_stats(k, [r for r in strict if str(r.get("exit_reason") or "unknown") == k]))
    else:
        lines.append("- 신규 CLOSED 부족")
    lines += ["", *entry_lines, "", "[5/7] 현재 탈락/차단 상위", *reject_lines(limit=8 if full else 5), "", "[6/7] 운영 상태", f"- scan {scan_status_summary()['label']} / 정밀캐시 {STATE.get('precision_have',0)} / 후보 strict {STATE.get('strict_decision',0)} / trade_ready {STATE.get('trade_ready_written',0)}", paper_bot_line(), "", "[7/7] 다음 판단", "✅ 진입 당시 재료가 좋은 그룹만 전략화 후보", "⚠️ 청산사유·최종수익률은 사후 결과라 직접 조건화 금지", "❌ 반복 stop_loss/slow_no_progress는 진입 전 재료로 원인 확인", "❔ v0.32 이후 entry_context 표본이 쌓이기 전 조건 수정은 보류"]
    if full:
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.175.py", "- 페이퍼봇 대상: paper_bot_v0.32.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.175: 정확도 안정화 + 후보파일/정보품질 정리", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.175 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v175 정확도 안정화 + latest/중복skip/스프레드/정보없음 정리 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
