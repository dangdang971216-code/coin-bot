...
def ws_snapshot(ticker: Any) -> Dict[str, Any]:
    ...
    if not row:
        return {
            ...
            "ws_targeted": t in set(_ws_targets),
        }
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.187: v186 ATR 계산 순서 오류 즉시수정. 정밀직원 복구 + 6차 최종검증 유지.",
    }
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.188.py", "- 페이퍼봇 대상: paper_bot_v0.35.py"])),
    ]
    start = now_ts()
