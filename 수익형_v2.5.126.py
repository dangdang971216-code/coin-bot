...
def format_rebreakout_stat_text(item_or_row: dict, compact: bool = False) -> str:
    ...
    re_score = safe_float(item_or_row.get("rebreakout_score", 0), 0)
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 회복 초입형 승격 보강: 저점권 재출발형/재시도 직전형을 shortlist rescue에서 더 살리도록 조정",
        "- 차트 정교화 후속: 재돌파 준비도/성공/실패 표기와 minute1 표본 신뢰도 문구 정리",
...
def compute_rebreakout_structure_context(ticker: str, current_price: float, df1=None) -> dict:
    ...
    try:
        ...
        ctx.update({
            "rebreakout_score": round(total, 2),
            "rebreakout_ok": bool(total >= 1.20),
