...
def _ticker_to_ws_market_code(ticker: Any, legacy: bool = True) -> str:
    """pybithumb WebSocketManager는 legacy BTC_KRW 형식이 안정적이라 기본값으로 쓴다."""
...
def websocket_hub_worker_loop() -> None:
    """웹소켓 허브. 초단타 실시간 가격/체결 판단의 주 데이터지만, 실패해도 REST 본선을 흔들지 않는다."""
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.178: 웹소켓 중심 허브 재정렬 + 예전 티커 변환/파싱 복구 + REST 안전망 유지.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.178 기준: 웹소켓 실시간 주 데이터 / REST 전체시장 보정 / 후보파일 저장 유지",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.178.py", "- 페이퍼봇 대상: paper_bot_v0.33.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.178: 웹소켓 중심 허브 재정렬 + 티커변환 복구 + WS 진단표", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.178 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v177 웹소켓 보조허브 + 스캔신뢰도 보강 + trade_ready 관찰이동 기준 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
