...
DEFAULT_CONTROL = {
    "running": False,
    "loop_seconds": 8,
    "max_open_strict": 30,  # v0.26: 후보를 자르는 값이 아니라 실제 자동매매식 보유 슬롯 보호값
    ...
    "open_shadow_positions": False,  # v0.26 기본값: shadow는 OPEN하지 않고 복기 전용
...
def load_control() -> Dict[str, Any]:
    ...
    if isinstance(saved, dict):
        for k, v in saved.items():
            ...
            if k in DEFAULT_CONTROL:
                control[k] = v
    # v0.26 고정: candidate_events 소비 옵션은 저장돼 있어도 다시 만들지 않는다.
    for k in REMOVED_CONTROL_KEYS:
        control.pop(k, None)

    # v0.26: 후보 분석은 제한하지 않되, 실제 paper OPEN은 자동매매 검증 슬롯처럼 보호한다.
    ...
    if control.get("track_strict_observe") is not True:
        ...
        changed = True
    try:
        # v0.26: paper_bot은 *_latest 후보파일을 우선 읽는다. 큰 archive 파일을 매 cycle 읽지 않는다.
...
def quarantine_shadow_open_positions(open_pos: Dict[str, Dict[str, Any]], control: Dict[str, Any]) -> Tuple[Dict[str, Dict[str, Any]], int]:
    ...
    for pid, pos in (open_pos or {}).items():
        ...
        if lane == "shadow":
            ...
            row["quarantine_reason"] = "v0.26_shadow_review_only"
...
def pick_candidates(control: Dict[str, Any], open_pos: Dict[str, Dict[str, Any]]) -> Tuple[List[Tuple[str, Dict[str, Any]]], Dict[str, int]]:
    ...
    if open_shadow:
        ...
        # v0.26: shadow는 복기 전용. OPEN 장부에 태우지 않는다.
...
def control_text() -> str:
    c = load_control()
    keys = ["running", "loop_seconds", "max_open_strict", "max_open_shadow", "max_new_per_cycle", "open_trade_ready_only", "track_strict_observe", "candidate_ttl_sec", "candidate_read_max_lines", "consume_latest_scan_only", "consume_latest_candidate_files", "quarantine_shadow_open", "latest_scan_window_sec", "fee_pct_roundtrip", "take_profit_pct", "protect_trigger_pct", "protect_floor_pct", "stop_loss_pct", "slow_minutes", "time_exit_minutes", "block_same_ticker_open"]
