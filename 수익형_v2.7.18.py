...
- 실거래 주문은 v2.7.18 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.18"
BOT_FILE_TAG = "수익형_v2.7.18.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.18는 현재가+체결 허브를 후보분류에 더 적극 반영한다.
...
WS_MAX_CODES = int(os.getenv("V2710_WS_MAX_CODES", "240"))
...
def ws_candidate_tickers(limit: int = WS_MAX_CODES) -> list[str]:
    # v2.7.18: 스캔/허브 복구 1차.
    # M2 눌림 후보와 최근 급변 후보를 WebSocket 우선구독에 먼저 태워
    # 좋은 눌림 재확인 후보를 늦게 보는 문제를 줄인다.
    with STATE.lock:
        ...
        rank = [r.ticker for r in rank_rows]
        market_top = sorted(STATE.market_rows.values(), key=lambda r: r.money24h, reverse=True)
        market = [r.ticker for r in market_top[:max(limit, 160)]]
        ...
        m2_store = [r.ticker for r in stored if (r.pattern in m2_patterns or "눌림" in str(r.label) or "눌림" in str(r.reason))][:90]
        store_tickers = [r.ticker for r in stored[:100]]
        paper_rows = list(STATE.paper_candidates.values())
        m2_paper = [r.ticker for r in paper_rows if r.active and (r.pattern in m2_patterns or str(r.strategy).startswith(("S1", "S2")) or "눌림" in str(r.strategy))][:90]
        movers = [r.ticker for r in sorted(rank_rows, key=lambda r: (abs(r.ch1) + abs(r.ch3) + abs(r.ch5), r.money24h), reverse=True)[:80]]
    out = []
    seen = set()
    priority = ["BTC", "ETH", "XRP", "SOL"] + ready + m2_paper + m2_store + pending + store_tickers + movers + rank + market + watch
    for t in priority:
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.18 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
def v315_mainline_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool) -> dict:
    """
    v2.7.18: v2.5.3xx 후기판의 본선 판단을 clean core row로 옮긴 1차 컨텍스트.
...
def v317_pullback_risk_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool) -> dict:
    """
    v2.7.18: v2.5.3xx 후기판의 위험진입 차단 2차 이식.
...
def v318_m2_pullback_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool, recent_money_strong: bool, own_supporting: bool, own_recovery_strong: bool, v315_ctx: dict, v317_ctx: dict) -> dict:
    """
    v2.7.18: v2.5.3xx/v2.6 눌림 재확인 라인 2차 이식.
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    # v2.7.18: v2.5.3xx 본선 기준 이식 컨텍스트.
    ...
    # v2.7.18: 예전 승률 좋던 M2 눌림/준비대기 2차 복구.
    ...
    # v2.7.18에서는 v318 M2 컨텍스트를 우선한다. 기존 v315/v317 게이트는 보조 안전장치로 남긴다.
    pullback_prep_wait = (
    ...
    # v2.7.18에서는 일반 약한 움직임 전체가 재출발감시로 빨려 들어가지 않게,
    ...
    # v2.7.18: 특수 대기상태 보존. TON류 재출발/재돌파/급등후재확인 후보가
    ...
    if v317_dirty_pullback and (row.pullback_shape_ok or row.low_defense_ok or row.from_low30 <= 3.2):
        return make_candidate(
            ...
            block_reasons=["v317 위험눌림 제외", v317_pullback_risk_reason(v317_ctx)]
        )
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.18: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.18는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.18\n"
        f"- v2.7.18 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.18에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.18\n"
...
def render_paper_stats() -> str:
    ...
    if overdue30:
        lines.append("- ⚠️ 30분 지났는데 미완료 후보가 있음. v2.7.18에서 paper worker/저장 조건을 봐야 함")
    return "\n".join(lines)
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.18 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
