...
PRECISION_SYNC_MAX_PER_SCAN = int(os.getenv("CLEAN_PRECISION_SYNC_MAX_PER_SCAN", "18"))
PRECISION_SYNC_COLD_MAX_PER_SCAN = int(os.getenv("CLEAN_PRECISION_SYNC_COLD_MAX_PER_SCAN", "36"))
...
FILES = {
    ...
    "baseline": BASE_DIR / "paper_eval_baseline_v161.json",
...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v162: 기능은 유지하되 무거운 계산을 직원 큐로 분리. 메인 스캔은 캐시를 조립하고, 2차 직원들이 흐름/자리/현재봉/실전위험을 계속 보강.",
}
...
def websocket_status_line() -> str:
    ...
    icon = "✅" if STATE.get("ws_state") == "수신중" else ("❔" if STATE.get("ws_state") in {"대기", "대상대기", "연결", "재연결대기"} else "❌")
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.162: 고정 갯수 절단 없음. paper_bot은 latest scan batch만 소비해 과거 TTL 후보 몰아먹기 방지.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.162 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.162.py", "- 페이퍼봇 대상: paper_bot_v0.21.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.162: 최신 scan 묶음 소비 + 성과판 해석 보강", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot 과거 TTL 몰아먹기 방지"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.162 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v150 수치 이식 + 직원 분리 + 웹소켓 허브 보조 + 실전위험 보강 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
