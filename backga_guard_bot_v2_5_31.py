...
VERSION = "guard_v2.5.31_disk_retention_resource_health_2026-05-16"
...
def run_retention_cleanup(dry_run: bool = False) -> dict:
    ...
    def age_days(path: Path) -> float:
        try:
            ...
            return 0.0
    protected_names = {"paper_bot_open.json", "paper_bot_closed.jsonl", "paper_bot_status.json", "paper_bot_control.json", "paper_candidates.jsonl", "paper_candidates_latest.jsonl", "shadow_candidates.jsonl", "shadow_candidates_latest.jsonl"}
    for pat in ["*.tmp", "*.swp", "*~"]:
        for path in BOT_DIR.glob(pat):
            ...
            if ok:
                ...
                if len(samples) < 10: samples.append(f"삭제 {name}")
            else:
                errors += 1
    ...
    for pat in ["bot.py.backup*", "*.backup_guard_apply_*", "ws_sidecar.py.backup*", "bithumb_micro_sidecar.py.backup*", "paper_bot.py.backup*", "backga_guard_bot*.backup*"]:
        for path in BOT_DIR.glob(pat):
            if not path.is_file() or age_days(path) <= 2:
                continue
            size = path.stat().st_size if path.exists() else 0
            ok, size, name = (True, size, path.name) if dry_run else _safe_unlink(path)
            if ok:
                ...
                errors += 1
                if len(samples) < 10: samples.append(name)
    for name in ["clean_ws_sidecar.out", "clean_ws_sidecar.log", "ws_sidecar.log", "clean_bithumb_micro.log", "clean_runtime_events.jsonl", "runtime_events.jsonl"]:
        path = BOT_DIR / name
        size = path.stat().st_size if path.exists() else 0
        if size <= 0:
            continue
        ok, size, n = (True, size, name) if dry_run else _safe_truncate(path, min_size=1)
        if ok:
            truncated += 1; freed += size
            if len(samples) < 10: samples.append(f"비움 {n}")
        else:
            errors += 1
            if len(samples) < 10: samples.append(n)
    ce = BOT_DIR / "candidate_events.jsonl"
    try:
        if ce.exists() and ce.stat().st_size > 100 * 1024 * 1024:
            ...
            truncated += 1; freed += max(0, old_size - new_size)
            if len(samples) < 10: samples.append("축소 candidate_events.jsonl")
    except Exception as e:
        errors += 1
        if len(samples) < 10: samples.append(f"candidate_events: {type(e).__name__}")
    for pat in ["*.log.*", "*.out.*", "*.gz", "*.old"]:
        for path in BOT_DIR.glob(pat):
            if not path.is_file() or age_days(path) <= 7 or path.name in protected_names:
                continue
            size = path.stat().st_size if path.exists() else 0
            ok, size, name = (True, size, path.name) if dry_run else _safe_unlink(path)
            if ok:
                ...
                errors += 1
                if len(samples) < 10: samples.append(name)
    return {"deleted": deleted, "truncated": truncated, "errors": errors, "freed": freed, "samples": samples, "dry_run": dry_run}
...
def cleanup_text(dry_run: bool = False) -> str:
    ...
    lines = ["🧹 디스크 순환정리 /gcleanup", "- 장부 paper OPEN/CLOSED/trade_log 계열은 삭제하지 않음", f"- 삭제 {res['deleted']}개 / 비움·축소 {res['truncated']}개 / 오류 {res['errors']}개", f"- 확보 추정: {_fmt_bytes(res['freed'])}", f"- 전: 디스크 {before['disk_pct']:.1f}% / 남음 {_fmt_bytes(before['disk_free'])}", f"- 후: 디스크 {after['disk_pct']:.1f}% / 남음 {_fmt_bytes(after['disk_free'])}"]
    if res.get("samples"):
...
def preupgrade_disk_guard(force: bool = False) -> tuple[bool, str]:
    ...
    text = "\n".join(["[0/5] 디스크 사전점검·순환정리", f"- 전: 사용 {before['disk_pct']:.1f}% / 남음 {_fmt_bytes(before['disk_free'])}", f"- 정리: 삭제 {cleanup['deleted']}개 / 비움 {cleanup['truncated']}개 / 확보 {_fmt_bytes(cleanup['freed'])}", f"- 후: 사용 {after['disk_pct']:.1f}% / 남음 {_fmt_bytes(after['disk_free'])}"])
    if ok or force:
        return True, text
...
def sidecar_management(pid: Optional[int], service: str) -> dict:
    ...
    mode = f"systemd:{service}" if svc_exists else "direct-fallback"
    warnings = []
    if not svc_exists:
        warnings.append("direct-fallback: 가드 재시작 시 같이 꺼질 수 있음")
    if cg and GUARD_SERVICE in cg:
        warnings.append(f"guard cgroup 안에서 실행중: {GUARD_SERVICE} 재시작 연동 종료 의심")
    return {"mode": mode, "service_exists": svc_exists, "cgroup": cg, "cgroup_short": short_cgroup(cg), "warning": " / ".join(warnings) if warnings else "-"}
...
def apply_ws_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    ws_healthy_enough = bool(alive and status_recent and err_ok and has_rx)
    ...
    if same_hash and not force and ws_healthy_enough:
        ...
        return {
            ...
            "notes": [
                "single-path fast-skip: hash 동일 + alive + status 최신 + 수신흔적 있음",
    ...
    notes = ["restart path entered only because: " + (" / ".join(restart_reasons) or "unknown")]
...
def apply_micro_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    micro_healthy_enough = bool(alive and status_recent and err_ok and has_collection)
    ...
    if same_hash and not force and micro_healthy_enough:
        ...
        return {
            ...
            "active": "restart 생략", "backup": "없음", "hash": short_hash(active_path),
            "notes": [
                "single-path fast-skip: hash 동일 + alive + status 최신 + 수집흔적 있음",
    ...
    notes = ["restart path entered only because: " + (" / ".join(restart_reasons) or "unknown"), f"python={MICRO_PYTHON_BIN}"]
    active = "restart 생략"
    state = before
    if restart:
        ...
        active, rn = restart_micro_sidecar(python_bin=MICRO_PYTHON_BIN)
        notes += rn
        state, wn = wait_micro_ready(prev_poll=0 if changed else prev_poll, timeout=16.0)
        notes += wn
    else:
        state = micro_state_dict()
        notes.append("restart=False → 교체만 하고 재시작 생략")
    ok = bool(state.get("alive")) and (int(state.get("fresh") or 0) > 0 or int(state.get("cached") or 0) > 0 or int(state.get("poll_count") or 0) > 0)
    rollback_note = ""
    if not ok and changed and backup_path and backup_path.exists():
        try:
            ...
            state, wn = wait_micro_ready(timeout=12.0)
            notes += [rollback_note, f"rollback_restart={active2}"] + rn + wn
        except Exception as exc:
            notes.append(f"rollback 실패: {exc.__class__.__name__}: {str(exc)[:160]}")
    final_ok = bool(state.get("alive")) and (int(state.get("fresh") or 0) > 0 or int(state.get("cached") or 0) > 0 or int(state.get("poll_count") or 0) > 0)
