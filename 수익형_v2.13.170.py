...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.170: 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def paper_alive_summary() -> Dict[str, Any]:
    """paper_bot 상태를 status 파일보다 실제 pid 생존 여부 우선으로 판정한다.
    status가 최신이어도 pid가 죽었으면 죽음으로 표시한다. status pid와 pid 파일이 다르면 불일치로 표시한다.
    """
    ps = read_paper_status()
    st = ps["status"]
    ctrl = ps["control"]
    pid_file = _read_pid_file(BASE_DIR / "paper_bot.pid")
    status_pid = int(fnum(st.get("pid") or st.get("process_pid"), 0))
    pid = status_pid if status_pid > 1 else pid_file
    file_alive = _pid_alive(pid_file)
    status_alive = _pid_alive(status_pid) if status_pid > 1 else False
    alive = bool(file_alive or status_alive)
    used_pid = status_pid if status_alive else (pid_file if file_alive else (status_pid or pid_file))
    ...
    mismatch = bool(pid_file and status_pid and pid_file != status_pid)
    stale = bool(age < 0 or age > 180)
    # 표시용 판정. pid가 죽었으면 status running=True여도 죽음이 우선이다.
    if not alive:
        verdict = "dead"
        icon = "❌"
        label = "pid 죽음"
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.170 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.170.py",
        "- 페이퍼봇 대상: paper_bot_v0.29.py",
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    v170: 사후 결과와 진입 당시 재료를 명확히 분리해, 청산사유를 전략으로 오해하지 않게 한다.
    """
    rows = load_closed(limit=30000 if full else 10000)
    new_rows = rows_since_baseline(rows, "closed_at")
    ...
    def avg_field(arr, key):
        vals = [fnum(r.get(key), 0) for r in arr if fnum(r.get(key), 0) != 0]
        return sum(vals) / len(vals) if vals else 0.0

    entry_lines = [
        "[4/7] 진입 당시 재료 비교",
        "- 전략화 가능: 진입 전에 이미 알 수 있던 값만 후보로 봄",
        f"✅ 승리군: {len(win_rows)}건 / 점수 {avg_field(win_rows,'score'):.2f} / 5분흐름 {avg_field(win_rows,'change_5m'):+.2f}% / 고점거리 {avg_field(win_rows,'below_high_pct'):+.2f}%",
        f"❌ 손실군: {len(loss_rows)}건 / 점수 {avg_field(loss_rows,'score'):.2f} / 5분흐름 {avg_field(loss_rows,'change_5m'):+.2f}% / 고점거리 {avg_field(loss_rows,'below_high_pct'):+.2f}%",
    ]
    if len(strict) < 50:
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.170.py", "- 페이퍼봇 대상: paper_bot_v0.29.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.170: 통합상태 + 후보품질 명령 + 가벼운 점검", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.170 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def recent_error_full_text(n: int = 100) -> str:
    ...
    if recent:
        return "[새 실행 오류]\n" + "\n".join(recent)
    tail = tail_text(FILES["error"], int(n))
    return tail if tail.strip() else "최근 오류 없음"
