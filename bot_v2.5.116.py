...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    if not label_text:
        label_text = get_reverse_trace_bucket_label(row.get('bucket', ''))
    lines = [
        f"{circled_number(idx)} {row.get('ticker', '?')}",
        f"- 시간: {format_reverse_trace_ts(safe_float(row.get('ts', 0), 0))}",
        f"- 단계: {get_reverse_trace_stage_label(row.get('stage', ''))} / 구분: {get_reverse_trace_bucket_label(row.get('bucket', ''))} / 등급 {str(row.get('grade', '-') or '-')}",
        f"- 표시문구: {label_text}",
...
def compute_rebreakout_structure_context(ticker: str, current_price: float, df1=None) -> dict:
    ...
    try:
        ...
        near_high_hard = gap_pct <= 0.22
        near_high_soft = gap_pct <= 0.40
        if near_high_hard:
            score -= 0.80
