...
VERSION = "guard_v2.5.43_fast_changed_only_upgrade_2026-05-21"
...
def worker_state_dict(name: str) -> dict:
    ...
    if service_active not in {"active", "activating"}:
        ...
        verdict = "⚠️ 파일 불일치"
    metrics = []
    for k in ["row_count", "pool_count", "evaluated", "s_count", "target_count", "fresh_ws", "fresh_micro", "closed_recent", "big_loss", "good_win", "last_sec", "scanner_age"]:
        if k in st:
...
def gworker_state_text(name: Optional[str] = None) -> str:
    ...
    lines += ["", "[명령]", "- /gworker_upgrade : 변경 worker만 빠르게 최신 적용", "- /gstrategy_upgrade /gorderflow_upgrade : 개별 worker 최신 적용", "- /gworker_restart : worker 8개 재시작", "- /gscanner_state /gcandle_state /gmarket_state /gfeature_state /gorderflow_state /grisk_state /gstrategy_state /greview_state"]
...
def gworker_upgrade_text(force: bool = False) -> str:
    ...
    parts = [title, f"- guard: {VERSION}", f"- 대상: scanner → candle → market → feature → orderflow → risk → strategy → review", f"- total: {fmt_sec(res.get('total_sec',0))}"]
    for name in WORKER_SPECS:
...
def guard_command_menu_items() -> list[dict]:
    ...
    return [
        ...
        {"command": "gworker_upgrade", "description": "7 worker 최신 적용"},
