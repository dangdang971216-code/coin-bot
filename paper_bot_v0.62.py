...
def summary_text() -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        "- 이전 버전 잔여 OPEN은 성과판에서 현재판과 분리해서 봅니다.",
        "",
        "[1] 최근 OPEN",
    ]
    if not rows:
    ...
    lines += [
        ...
        *_v058_strategy_perf_text(recent_closed),
        "",
        f"[cycle] open +{status.get('opened_this_cycle',0)} / close +{status.get('closed_this_cycle',0)} / {status.get('elapsed_sec','-')}s",
        f"[후보] 읽음 {pick.get('paper_file',0)} / OPEN통과 {status.get('opened_this_cycle',0)} / 복기 {pick.get('shadow_review_only_skip',0)} / 중복 {pick.get('same_ticker_skip',0)}",
        f"[알림] 정식 자동매매급 즉시 / 3전략 실험진입은 시간요약",
        f"[상태] 업데이트 {status.get('updated_at_text','-')} / 오류로그 {err_size} bytes / 자세히 /perror",
    ]
    return "\n".join(lines)

# ===============================
# paper_bot v0.62: pstatus/pscore 요약 유지
# - v0.58 패치 위치 수정 구조를 유지하고, v271 메인봇 외부조건 실험진입 표시를 그대로 소비한다.
# - /pscore 오류수정, 3전략 실험진입 알림요약, OPEN 요약 출력은 유지한다.
# - 실제 청산조건/장부 삭제 없음.
# ===============================
try:
    VERSION = "paper_bot_v0.62"
...
# v0.62: v273 메인봇의 3전략 선별 실험을 소비한다.
# - OPEN 여러 개는 허용하되 /pstatus는 요약 중심으로 유지한다.
# - 실제 청산조건/장부 삭제 없음.
try:
    VERSION = "paper_bot_v0.62"
except Exception:
