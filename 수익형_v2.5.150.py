...
def compute_alert_chart_context_from_df(df1, df4h=None, current_price: float = 0.0) -> dict:
    ...
    try:
        ...
        if df1 is not None and len(df1) >= 12:
            ...
            ctx.update(prep_ctx)
            if state_code in {"CHECK", "WAKING_UP", "RESTARTING"} and safe_float(prep_ctx.get("prebreakout_ready_score", 0), 0) >= 2.2 and below_high >= 0.8:
...
def format_prebreakout_simple_label(item: dict) -> str:
    ...
    if score >= 2.2:
        return "고점 전 준비 좋음"
    if score >= 1.1 or (hold >= 2 and 0 < pullback <= 1.8):
        return "고점 전 준비 보통"
...
def build_missed_review_market_candidates(cache: dict, limit: int = MISSED_REVIEW_MARKET_LIMIT):
    ...
    for ticker, data, _score in ranked_rows:
        try:
            ...
            hold_score = safe_float(item.get("breakout_hold_score", 0), 0)
            if max(change_5, change_30, from_low) < 0.10 and vol_ratio < 1.35 and leader < 0.35 and rebreakout < 1.60:
                continue
            score = (
                max(change_5, 0) * 6.0
                + max(change_30, 0) * 2.8
...
def process_missed_review_queue(now_ts: float = None):
    ...
    for ticker, row in due:
        ...
        if (ok_spot or data_fail_row) and result_quality in {"good", "assist"} and not was_caught:
