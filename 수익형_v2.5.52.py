...
UNIVERSE_REFRESH_BATCH = 18
UNIVERSE_FORCE_FULL_BATCH = 36
UNIVERSE_MIN_PARTIAL_ROWS = 8
UNIVERSE_BUILD_MAX_SEC = 8
CACHE_REFRESH_BATCH = 12
CACHE_FORCE_FULL_BATCH = 24
CACHE_MIN_PARTIAL_ROWS = 6
CACHE_BUILD_MAX_SEC = 8
...
MARKET_REFRESH_INTERVAL_SEC = 24
...
SCAN_WATCHLIST_SIGNAL_CACHE_LIMIT = 14
SCAN_WATCHLIST_MARKET_CACHE_LIMIT = 12
SCAN_WATCHLIST_PREPARE_MAX_SEC = 9
...
while True:
    ...
    try:
        ...
        if need_market_refresh:
            mark_main_loop_heartbeat(stage="market_refresh")
            refreshed_cache = build_shared_market_cache(force=False)
            if isinstance(refreshed_cache, dict) and refreshed_cache:
                shared_cache = refreshed_cache
                try:
                    update_scan_debug_snapshot(shared_cache, note="분리형 빠른 1차 캐시 갱신")
                except Exception:
                    pass
            elif shared_cache:
                try:
                    update_scan_debug_snapshot(shared_cache, note="직전 캐시 유지 / 느린 유니버스 갱신과 분리 운용")
                except Exception:
                    pass
