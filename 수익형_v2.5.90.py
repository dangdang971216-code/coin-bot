...
def send_startup_message():
    lines = [
        ...
        "- /compare /review_dump 는 저장된 표본 기준으로 바로 보여줘서 다른 명령이 같이 막히지 않게 정리",
        "",
    ]
...
def get_shortlist_user_rescue_display_key(item: dict, display_key: str = "", regime: dict = None) -> str:
    ...
    if shortlist_rank > USER_ALERT_RESCUE_SHORTLIST_RANK_MAX:
        return ""
    hot_score = get_user_alert_hot_score(item)
    strong_rank_rescue = shortlist_rank <= 7 and hot_score >= 1
    ...
    if hot_score < USER_ALERT_RESCUE_MIN_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG:
        return ""
    if high_gap_pct < 0.48:
        return ""
    if from_low > 3.35 and high_gap_pct < 1.10:
        return ""
    if chart_rise > 4.80 and high_gap_pct < 1.45:
        return ""
    if edge < USER_ALERT_RESCUE_MIN_EDGE and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG:
        return ""
    if vol_ratio < USER_ALERT_RESCUE_MIN_VOL and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG:
        return ""
    if leader < USER_ALERT_RESCUE_MIN_LEADER and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG:
        return ""
...
def is_user_alert_rescue_candidate(item: dict, section: str = "", regime: dict = None) -> bool:
    ...
    hot_score = get_user_alert_hot_score(item)
    chart_ctx = ensure_alert_chart_context(item)
    strong_rank_rescue = shortlist_rank <= 6 and hot_score >= 2
    ...
    if section == "restart":
        if max(chg_5, chg_30, from_low) < USER_ALERT_RESCUE_MIN_RESTART_MOTION_PCT and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE:
            if not (shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG and high_gap_pct >= 0.80 and (vol_ratio >= 0.95 or leader >= 0.40)):
                return False
