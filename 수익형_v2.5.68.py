...
MISSED_REVIEW_MARKET_LIMIT = 14
MISSED_REVIEW_SIGNAL_LIMIT = 14
MISSED_REVIEW_MIN_BEST_UP_PCT = 2.20
MISSED_REVIEW_MIN_END_UP_PCT = 1.10
MISSED_REVIEW_MIN_FAST_UP_PCT = 3.00
MISSED_REVIEW_MIN_BASE_HIGH_GAP_PCT = 0.85
MISSED_REVIEW_NEAR_HIGH_BLOCK_PCT = 0.60
MISSED_REVIEW_MAX_BASE_FROM_LOW_PCT = 2.20
MISSED_REVIEW_MAX_BASE_CHART_RISE_PCT = 3.40
...
LIVE_FORCE_MAX_ENTRY_ITEMS = 2
LIVE_FORCE_MAX_RECOVERY_ITEMS = 2
...
def send_startup_message():
    lines = [
        ...
        "- 반대로 고점 바로 밑, 이미 많이 오른 자리, 잡음형 후보 숨김 방향은 유지",
...
SCAN_WATCHLIST_CACHE_LIMIT = 12
SCAN_WATCHLIST_SIGNAL_CACHE_LIMIT = 10
SCAN_WATCHLIST_MARKET_CACHE_LIMIT = 8
SCAN_WATCHLIST_PREPARE_MAX_SEC = 5
SCAN_WATCHLIST_PREPARE_MISSED_LIMIT = 4
SCAN_WATCHLIST_FALLBACK_LIMIT = 3
SCAN_WATCHLIST_COLLECT_HEARTBEAT_EVERY = 3
SCAN_WATCHLIST_MARKET_REGISTER_INTERVAL_SEC = 180
...
FAST_SCAN_RESULT_MAX = 8
FAST_SCAN_INPUT_LIMIT = 10
FAST_SCAN_ALERT_LIMIT = 1
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    item = item or {}
    turnover_user_ok = fast_scan_turnover_gate(item, level="user")
    turnover_strong_ok = fast_scan_turnover_gate(item, level="strong")
    turnover_watch_ok = fast_scan_turnover_gate(item, level="watch")
    if section == "strong":
        if (
            primary_reason == "실시간 급등 초입"
            and change_5 >= 0.95
            and vol_ratio >= 2.60
            and leader_score >= 1.20
            and below_high >= 1.00
            and 0.12 <= from_low <= 2.30
            and rsi < 80
            and turnover_strong_ok
        ):
            return "S", "user_alert"
        if (
            change_5 >= 0.60
            and vol_ratio >= 1.85
            and leader_score >= 1.00
            and below_high >= 1.05
            and 0.15 <= from_low <= 2.20
            and rsi < 82
            and turnover_user_ok
        ):
            return "A", "user_alert"
    ...
    if section == "watch":
        ...
        rescue_like = (
            change_5 >= 0.95
            and vol_ratio >= 3.20
            and leader_score >= 1.15
            and below_high >= 2.05
            and 1.05 <= from_low <= 2.30
            and primary_reason in {"재출발 확인 후보", "초입/회복 확인 후보"}
            and lite_score >= 16.8
            and rsi < 79
            and turnover_user_ok
        )
        ...
        if restart_like and (
            change_5 < 0.95
            or vol_ratio < 3.20
            or leader_score < 1.15
            or below_high < 2.05
            or from_low < 1.05
            or lite_score < 16.8
            or not turnover_user_ok
        ):
            return "B", "internal_watch"
        if (
            from_low >= 0.15
            and vol_ratio >= 1.10
            and leader_score >= 0.40
            and below_high >= 0.60
            and turnover_watch_ok
        ):
            return "B", "internal_watch"
        return "C", "discard"
    return "C", "discard"
...
def _build_fast_scan_item(ticker: str, data: dict):
    ...
    if price <= 0:
        return None
    df1 = data.get("df_1m") if isinstance(data.get("df_1m"), type(data.get("df_1m"))) else data.get("df_1m")
    snap = compute_pre_alert_snapshot_from_df(df1, current_price=price) if df1 is not None else {}
    change_5 = safe_float(data.get("change_5", 0), 0)
    change_3 = safe_float(data.get("change_3", 0), 0)
    vol_ratio = safe_float(data.get("vol_ratio", data.get("vol_ratio_1m", 0)), 0)
    leader_score = safe_float(data.get("leader_score", 0), 0)
    turnover = safe_float(data.get("turnover", 0), 0)
    turnover_rank = int(safe_float(data.get("turnover_rank", 999), 999))
    turnover_metrics = compute_fast_scan_turnover_metrics(df1, price)
    rsi = safe_float(data.get("rsi", data.get("rsi_1m", 50)), 50)
    from_low = safe_float(snap.get("from_30m_low_pct", 0), 0)
    below_high = safe_float(snap.get("below_30m_high_pct", 999), 999)
    raw_score = (
        max(change_5, 0) * 7.0
        + max(change_3, 0) * 2.5
        + max(vol_ratio - 1.0, 0) * 4.2
        + leader_score * 4.5
        + max(from_low, 0) * 1.2
        + min(turnover / 1000000000.0, 2.0)
    )
    if below_high <= 0.25:
        raw_score -= 6.0
    if change_5 <= 0.05 and vol_ratio < 1.15:
        raw_score -= 7.0
    item = finalize_scan_debug_item({
        "ticker": ticker,
        "price": price,
        ...
        "turnover_1m": safe_float(turnover_metrics.get("turnover_1m", 0), 0),
        "turnover_3m": safe_float(turnover_metrics.get("turnover_3m", 0), 0),
        "turnover_5m": safe_float(turnover_metrics.get("turnover_5m", 0), 0),
        "lite_score": raw_score,
        "rsi": rsi,
    })
    section, primary, reasons = classify_fast_scan_alert_section(change_5, change_3, vol_ratio, leader_score, from_low, below_high, rsi)
    item["section"] = section
    item["from_low"] = round(from_low, 2)
    item["below_high"] = round(below_high, 2) if below_high < 900 else 999.0
    item["primary_reason"] = primary
    item["reasons"] = reasons[:4]
    item = refine_fast_scan_item_with_chart(item, df1)
    section = str(item.get("section", section) or section)
    primary = str(item.get("primary_reason", primary) or primary)
    reasons = list(item.get("reasons", reasons)) if isinstance(item.get("reasons"), list) else reasons
    grade, bucket = classify_fast_scan_grade_bucket(section, primary, change_5, change_3, vol_ratio, leader_score, from_low, below_high, rsi, safe_float(item.get("lite_score", raw_score), raw_score), item=item)
    item["section_rank"] = {"strong": 2, "watch": 1, "weak": 0}.get(section, 0)
    item["alert_grade"] = grade
    item["alert_bucket"] = bucket
    item["bucket_rank"] = {"user_alert": 2, "internal_watch": 1, "discard": 0}.get(bucket, 0)
    item["label_text"] = get_fast_scan_label_text(primary)
    item["one_liner"] = build_fast_scan_one_liner(primary, from_low, below_high, grade=grade, bucket=bucket)
    item["reasons"] = reasons[:5]
    return item


def build_fast_scan_candidates_from_cache(cache: dict, limit: int = FAST_SCAN_RESULT_MAX):
    ranked_rows = get_watchlist_ranked_cache_items(cache, limit=max(int(limit) * 2, FAST_SCAN_INPUT_LIMIT))
    items = []
    for ticker, data, _score in ranked_rows:
    ...
    items.sort(
        key=lambda x: (
            ...
            safe_float(x.get("section_rank", 0)),
            safe_float(x.get("lite_score", 0)),
            safe_float(x.get("leader_score", 0)),
            safe_float(x.get("change_5", 0)),
        ),
        reverse=True,
    )
...
def build_scan_health_warning_text(now_ts: float = None) -> str:
    ...
    if fast_age is None:
        lines.append("⚠️ 실시간 후보 계산 기록이 아직 없음")
    elif fast_age >= SCAN_HEARTBEAT_WARN_SEC:
        lines.append(f"⚠️ 실시간 후보 계산이 {fast_age}초째 새로 안 된 것 같아")
    cache_age = fast.get("cache_age")
    if cache_age is not None:
        if cache_age >= SCAN_CACHE_WARN_SEC:
            lines.append(f"ℹ️ 지금 후보 계산은 {cache_age}초 전 시장 데이터 기준")
    elif info.get("cache_age") is not None and info.get("cache_age") >= SCAN_CACHE_WARN_SEC:
        lines.append(f"ℹ️ 시장 캐시는 {info['cache_age']}초 전이야")
    c_stats = info.get("cache_stats") or {}
    u_stats = info.get("universe_stats") or {}
    if c_stats:
        lines.append(f"ℹ️ 최근 캐시 갱신: 시도 {int(safe_float(c_stats.get('tried',0),0))} / 성공 {int(safe_float(c_stats.get('success',0),0))} / 유지 {int(safe_float(c_stats.get('kept',0),0))}")
    ...
    if stage:
        lines.append(f"ℹ️ 최근 스캔 단계: {stage}")
    refresh_info = get_market_refresh_status(now_ts)
    if refresh_info.get("in_progress"):
        lines.append(f"🔄 시장 갱신 백그라운드 진행 중 ({refresh_info.get('age_sec', 0)}초 / {refresh_info.get('reason','loop')})")
    if info.get("reset_reason"):
        lines.append(f"🛠 최근 자동 복구 사유: {info['reset_reason']}")
    return "\n".join(lines).strip()
...
def get_scan_debug_candidates():
    ...
    if fast.get("items"):
        ...
        if cache_age is not None:
            ...
            note = f"최근 {age_sec}초 안에 다시 고른 후보 기준"
        return list(fast.get("items") or []), note
    ...
    if cache:
        ...
        if items:
            note = f"최근 {cache_age}초 안에 모은 시장 데이터 기준" if cache_age is not None else "최근 시장 데이터 기준"
            return items, note
    return [], "최근 시장 데이터가 아직 없음. 후보가 생기면 바로 여기에도 보여줄게"


def split_scan_debug_results(results):
    user_items = [x for x in results if str(x.get("alert_bucket", "")) == "user_alert"][:3]
    internal_items = [x for x in results if str(x.get("alert_bucket", "")) == "internal_watch"][:4]
    discard_items = [x for x in results if str(x.get("alert_bucket", "")) == "discard"][:3]
    return user_items, internal_items, discard_items
...
def build_scan_debug_text():
    ...
    def append_group(title: str, items):
        ...
        for i, item in enumerate(items, 1):
            price_text = f"{safe_float(item.get('price', 0)):,.4f}".rstrip("0").rstrip(".")
            lines.append("")
            lines.append(f"{i}. {item.get('ticker','?')} / 현재가 {price_text} / 등급 {item.get('alert_grade','B')} / {get_fast_scan_bucket_text(item.get('alert_bucket',''))}")
            lines.append(f"   유형 {item.get('label_text','후보')} / 5분 상승 {safe_float(item.get('change_5',0)):.2f}% / 거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 시장주목도 {safe_float(item.get('leader_score',0)):.2f}")
            lines.append(f"   최근 5분 거래대금 {fmt_price(item.get('turnover_5m', 0))}")
            lines.append(f"   30분 저점대비 {safe_float(item.get('from_low',0),0):+.2f}% / {format_fast_scan_high_gap_text(safe_float(item.get('below_high',999),999))}")
            if item.get("one_liner"):
                lines.append(f"   한줄평: {item.get('one_liner')}")
            lines.append(f"   판단: {' / '.join(item.get('reasons', [])[:3])}")
    append_group("🌱 사용자에게 보여줄 후보", user_items)
    append_group("👀 내부 감시 후보", internal_items)
    append_group("⛔ 버릴 후보", discard_items)
    return "\n".join(lines)
...
def append_debug_shortlist_parts(parts: list):
    try:
        ...
        if not results:
            ...
            return
        user_items, internal_items, discard_items = split_scan_debug_results(results)
        shortlist = (user_items + internal_items)[:3] or discard_items[:2]
        lines = [header]
        for item in shortlist:
            lines.append(
                f"• {item.get('ticker','?')} / {item.get('label_text','후보')} / 등급 {item.get('alert_grade','B')} / {get_fast_scan_bucket_text(item.get('alert_bucket',''))} / 5분 {safe_float(item.get('change_5',0)):.2f}% / 거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 주목도 {safe_float(item.get('leader_score',0)):.2f}"
            )
            if item.get("one_liner"):
                lines.append(f"  - {item.get('one_liner')}")
        parts.append("\n".join(lines))
...
def build_fast_scan_alert_markup(ticker: str, ref_id: str = ""):
    ...
    rows = []
    try:
        rows.append([InlineKeyboardButton(f"{ticker} 빗썸 열기", url=get_bithumb_chart_url(ticker))])
        if ref_id:
            rows.append([
...
def schedule_fast_scan_auto_check(ref_id: str, chat_id: int, target_min: int = 30, now_ts: float = None):
    ref_id = str(ref_id or "").strip()
    if not ref_id or target_min <= 0:
        return False
    ref = PRICE_CHECK_REFS.get(ref_id)
    ...
    if alert_ts <= 0:
        alert_ts = now_ts
        ref["alert_ts"] = now_ts
    sched_key = f"{ref_id}:{target_min}:{chat_id}"
    if sched_key in PRICE_CHECK_SCHEDULED:
        return False
    PRICE_CHECK_SCHEDULED[sched_key] = {
        "ref_id": ref_id,
...
def _build_fast_alert_signature(item: dict) -> str:
    ...
    reasons = list(item.get("reasons", [])) if isinstance(item.get("reasons"), list) else []
    normalized_reasons = tuple(sorted(str(x).strip() for x in reasons[:3] if str(x).strip()))
    return f"{ticker}|{grade}|{bucket}|{label}|{'/'.join(normalized_reasons)}"
...
def _fast_alert_has_meaningful_upgrade(prev: dict, item: dict) -> bool:
    ...
    if prev_price > 0:
        price_move = abs((now_price - prev_price) / prev_price) * 100.0
        if price_move >= FAST_SCAN_ALERT_REPEAT_PRICE_MOVE_PCT:
            return True
    ...
    if (now_vol - prev_vol) >= FAST_SCAN_ALERT_REPEAT_VOL_JUMP:
        return True
    prev_label = str(prev.get("label_text", "") or "")
    now_label = str(item.get("label_text", "") or "")
    return prev_label != now_label
...
def _send_fast_scan_alerts(items, now_ts: float):
    ...
    for item in items:
        ...
        recent_signal_alerts[ticker] = now_ts
        recent_fast_alert_signatures[ticker] = {
            "ts": now_ts,
            "sig": sig,
            "grade": str(item.get("alert_grade", "") or ""),
            "price": safe_float(item.get("price", 0), 0),
        ...
        ref_id = make_price_check_ref(
            prepared_item,
            alert_type='fast_scan',
            source_label='빠른 후보 알림',
            judgement_text=one_liner,
        )
        msg = (
            f"🌱 빠른 후보 알림\n\n"
            f"- 코인: {ticker}\n"
...
def scan_watchlist(shared_cache=None):
    ...
    cache = shared_cache if isinstance(shared_cache, dict) and shared_cache else (shared_market_cache if isinstance(shared_market_cache, dict) and shared_market_cache else None)
    if not cache:
        request_market_refresh(force=True, reason="scan_watchlist_empty_cache")
...
# =========================================================
# v2.5.68 fast 경로 rescue 분리 강화 + 실시간 강세형 직접 승격 보정
# =========================================================
FAST_SCAN_ALERT_LIMIT = 3
FAST_SCAN_ALERT_COOLDOWN_SEC = 1200
FAST_SCAN_ALERT_REPEAT_PRICE_MOVE_PCT = 2.80
FAST_SCAN_ALERT_REPEAT_SCORE_JUMP = 9.0
FAST_SCAN_ALERT_REPEAT_VOL_JUMP = 4.5
FAST_SCAN_A_MIN_TURNOVER_3M = 16000000
FAST_SCAN_A_MIN_TURNOVER_5M = 28000000
FAST_SCAN_S_MIN_TURNOVER_3M = 32000000
FAST_SCAN_S_MIN_TURNOVER_5M = 52000000
FAST_SCAN_RESCUE_MIN_TURNOVER_3M = 15000000
FAST_SCAN_RESCUE_MIN_TURNOVER_5M = 26000000
FAST_SCAN_RESCUE_LIMIT = 1
FAST_SCAN_RESCUE_SCORE_MIN = 14.8
...
def _friendly_fast_stage_text(stage: str) -> str:
    ...
    mapping = {
        ...
        'scan_watchlist_done': '후보 정리 끝',
        'scan_watchlist_empty': '후보 대기중',
        'scan_watchlist_empty_done': '후보 대기중',
        'empty_cache': '시장 데이터 기다리는 중',
        'loop_refresh': '시장 데이터 다시 받는 중',
        'scan_cycle_done': '한 바퀴 확인 끝',
    }
...
def _fast_abs_turnover_ok(item: dict, level: str = 'watch') -> bool:
    item = item or {}
    level = str(level or 'watch').lower()
    t3 = safe_float(item.get('turnover_3m', 0), 0)
    t5 = safe_float(item.get('turnover_5m', 0), 0)
    t15 = safe_float(item.get('turnover', 0), 0)
    if level == 's':
        return t3 >= FAST_SCAN_S_MIN_TURNOVER_3M or t5 >= FAST_SCAN_S_MIN_TURNOVER_5M or t15 >= FAST_SCAN_STRONG_ALERT_MIN_TURNOVER_15M
    if level in {'a', 'user'}:
        return t3 >= FAST_SCAN_A_MIN_TURNOVER_3M or t5 >= FAST_SCAN_A_MIN_TURNOVER_5M or t15 >= FAST_SCAN_USER_ALERT_MIN_TURNOVER_15M
    if level in {'rescue', 'r'}:
        return t3 >= FAST_SCAN_RESCUE_MIN_TURNOVER_3M or t5 >= FAST_SCAN_RESCUE_MIN_TURNOVER_5M or t15 >= FAST_SCAN_USER_ALERT_MIN_TURNOVER_15M * 0.9
...
def _fast_chart_bad(item: dict) -> bool:
    ...
    if state_code in {'FADE_AFTER_SPIKE', 'HIGH_WOBBLE', 'EXTENDED'}:
        return True
    if chart_gap <= 0.95 and chart_rise >= 0.85:
        return True
    if zone_ratio >= 0.82 and chart_gap <= 1.35:
        return True
    return False
...
def _fast_rescue_score(item: dict) -> float:
    return (
        ...
        + max(safe_float(item.get('change_5', 0), 0), 0) * 3.2
        + max(safe_float(item.get('vol_ratio', 0), 0) - 1.0, 0) * 2.0
        + safe_float(item.get('leader_score', 0), 0) * 2.4
        + min(safe_float(item.get('turnover_5m', 0), 0) / 15000000.0, 4.5)
        + max(safe_float(item.get('change_30', 0), 0), 0) * 0.35
    )
...
def _fast_rescue_has_weak_reason(item: dict) -> bool:
    weak_text = " ".join(
        [
            ...
            " ".join(str(x) for x in item.get('reasons', []) if isinstance(x, str)),
        ]
    )
    weak_markers = [
        '다시 사는지 확인할 자리',
    ...
    return any(marker in weak_text for marker in weak_markers)
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'strong':
        if (
            primary_reason == '실시간 급등 초입'
            and change_5 >= 0.92 and change_3 >= 0.18
            and vol_ratio >= 2.45 and leader_score >= 1.05
            and 0.10 <= from_low <= 2.05 and below_high >= 1.05
            and rsi < 80 and turnover_s_ok and not chart_bad
        ):
            return 'S', 'user_alert'
        if (
            primary_reason in {'실시간 급등 초입', '초입 가능 후보'}
            and change_5 >= 0.44 and change_3 >= 0.03
            and vol_ratio >= 1.55 and leader_score >= 0.78
            and 0.08 <= from_low <= 2.10 and below_high >= 1.05
            and lite_score >= 10.4 and rsi < 82 and turnover_a_ok and not chart_bad
        ):
            return 'A', 'user_alert'
        if turnover_watch_ok and change_5 >= 0.14 and vol_ratio >= 1.05 and below_high >= 0.70:
            return 'B', 'internal_watch'
        return 'C', 'discard'
    ...
    if section == 'watch':
        ...
        rescue_like = (
            ...
            and change_5 >= 0.78 and vol_ratio >= 2.70 and leader_score >= 0.98
            and 0.45 <= from_low <= 1.90 and below_high >= 1.90
            and lite_score >= max(FAST_SCAN_RESCUE_SCORE_MIN, 15.2) and rsi < 77
...
def _rescue_internal_watch_items(items):
    ...
    for item in internal_sorted:
        if promoted >= FAST_SCAN_RESCUE_LIMIT or user_count + promoted >= FAST_SCAN_ALERT_LIMIT:
            break
        ticker = str(item.get('ticker', '') or '')
        if not ticker:
            continue
        ...
        rescue_ok = (
            ...
            and leader >= 0.90
            and below_high >= 1.60
            and 0.20 <= from_low <= 1.95
            and turnover_5m >= FAST_SCAN_RESCUE_MIN_TURNOVER_5M
            and change_30 >= -0.45
        )
        if primary_reason == '실시간 급등 초입':
            rescue_ok = rescue_ok and change_5 >= 1.05 and vol_ratio >= 3.10 and leader >= 1.05 and below_high >= 2.10 and turnover_5m >= 32000000 and change_30 >= -0.20
            if not good_market:
                rescue_ok = rescue_ok and turnover_5m >= 42000000 and change_5 >= 1.35 and below_high >= 2.60 and change_30 >= 0.00
        elif primary_reason == '재출발 확인 후보':
            rescue_ok = rescue_ok and change_5 >= 0.86 and vol_ratio >= 2.90 and leader >= 1.00 and from_low >= 0.65 and below_high >= 2.40 and turnover_5m >= 26000000 and change_30 >= -0.20
        else:
            rescue_ok = rescue_ok and change_5 >= 0.72 and vol_ratio >= 2.55 and below_high >= 1.90 and turnover_5m >= 28000000
        if not rescue_ok:
            continue
        new_item = dict(item)
        ...
        if '유지력 확인이 더 필요한 자리' not in reasons:
            reasons.append('유지력 확인이 더 필요한 자리')
        new_item['reasons'] = reasons[:5]
        one_liner = str(new_item.get('one_liner', '') or '지금은 흐름만 확인')
        if '다시 사는지 확인할 자리' in one_liner:
            one_liner = one_liner.replace('다시 사는지 확인할 자리', '재출발이 실제로 붙는지 보는 자리')
        if '재출발 힘이 아직 약함' in one_liner:
            one_liner = one_liner.replace('재출발 힘이 아직 약함', '재출발 힘이 붙는지 재확인')
        new_item['one_liner'] = one_liner + ' / 상위 감시 후보에서 다시 올렸지만 유지력도 같이 확인'
...
def build_scan_debug_text():
    ...
    if hidden_count:
        lines.append('')
        lines.append(f'🙈 내부에서만 넘긴 후보 {hidden_count}개 (놓쳤을 때 이유만 따로 확인)')
    return '\n'.join(lines)
...
def append_debug_shortlist_parts(parts: list):
    try:
        ...
        if not shortlist:
            hidden_count = len(discard_items)
            lines.append('• 지금은 바로 볼 후보 없음')
            if hidden_count:
                lines.append(f'  - 내부에서 넘긴 후보 {hidden_count}개 (놓쳤을 때 이유만 확인)')
            parts.append('\n'.join(lines))
            return
        for item in shortlist:
            bucket = str(item.get('alert_bucket', '') or '')
            label = '바로 볼 후보' if bucket == 'user_alert' else '조금 더 볼 후보'
            lines.append(f"• {item.get('ticker','?')} / {label} / 5분 {safe_float(item.get('change_5',0)):.2f}% / 거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 최근 5분 거래대금 {fmt_price(item.get('turnover_5m', 0))}")
            if item.get('one_liner'):
                lines.append(f"  - {item.get('one_liner')}")
        hidden_count = len(discard_items)
        if hidden_count:
