...
def send_startup_message():
    lines = [
        ...
        "- /scan_debug는 최근 후보가 비어도 기다릴 후보를 fallback으로 보여주게 정리",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def get_scan_debug_candidates():
    try:
        ...
        if shared_market_cache and cache_age <= debug_cache_max_age:
            items = []
        ...
        # 3순위: 지금은 기다릴 후보(pending) fallback
        if pending_buy_candidates:
            ...
            for row in sorted_candidates:
                ...
                items.append({
                    ...
                    "lite_score": score,
                    "rsi": 50.0,
                    "reasons": [f"지금은 기다릴 후보 기준", f"{strategy_label}", f"반복 포착 {seen_count}회"],
                })
            if items:
