...
BOT_VERSION="수익형 v2.13.320"
BASE_DIR=Path(__file__).resolve().parent
TELEGRAM_TOKEN=os.getenv("TELEGRAM_TOKEN","").strip()
CHAT_ID=os.getenv("CHAT_ID","").strip()
WORKERS={
    "scanner": {"file":"scanner_worker_v0.3.py", "status":"clean_scanner_status.json"},
    "candle": {"file":"candle_worker_v0.1.py", "status":"clean_candle_status.json"},
    ...
    "risk": {"file":"risk_worker_v0.1.py", "status":"clean_risk_status.json"},
    "strategy": {"file":"strategy_worker_v0.2.py", "status":"clean_strategy_worker_status.json"},
...
def health_text()->str:
    ...
    lines=["🧭 건강상태 /health", f"✅ 메인봇 {BOT_VERSION} / clean worker hub v320 full workers / uptime {int(now_ts()-START_TS)}s", "", "[1] worker 상태"]
    ...
    lines += ["", "[원칙] 메인봇은 스캔/전략판정 직접 실행 안 함. 명령어는 worker cache만 읽음. scanner/candle/market/feature/orderflow/risk/strategy/review worker 분리. 실제 paper OPEN은 전략별 S만."]
    return "\n".join(lines)
