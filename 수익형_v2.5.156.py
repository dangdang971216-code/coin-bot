...
def get_price_check_note(ref: dict, current_price: float, summary: dict = None) -> str:
    ...
    if result_type == "계속 우상향":
        return "판단: 초반 판단 괜찮았음. 끝까지 버틴 편"
    if result_type == "초반 바로 감":
        return "판단: 초반 반응은 맞았음. 다만 추격 재진입은 별개"
    if result_type == "늦게 출발":
        return "판단: 늦게 출발한 타입. 본선 전략과는 조금 다를 수 있음"
    if result_type == "한 번 튀고 식음":
        return "판단: 한 번 튀고 유지 실패. 다시 사기엔 별로"
    if result_type == "눌렸다가 회복":
        return "판단: 중간 눌림은 있었지만 회복은 했음"
    if max_down_pct <= -2.0 or end_pct <= -2.0:
        return "판단: 버릴 후보. 알림 뒤 바로 무너짐"
    if peak_fade_pct >= 1.50 and end_pct <= 0.30:
        return "판단: 잠깐 튄 건 맞지만 실전 매매감은 약했음"
    if end_pct <= -0.5:
...
def classify_alert_follow_quality(best_up_pct: float, end_up_pct: float, worst_down_pct: float) -> str:
    ...
    if best_up_pct >= 2.50 and end_up_pct >= 1.00:
        return "good"
    if best_up_pct >= 1.80 and end_up_pct >= 0.50 and worst_down_pct > -1.60:
        return "good"
    if best_up_pct >= 2.00 and end_up_pct <= 0.20:
        return "spike"
...
def summarize_alert_review_rows(rows):
    bucket = {"good": 0, "spike": 0, "weak": 0, "bad": 0}
    for row in rows:
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 실시간 스캔 지연/복구 안내를 1~2줄 요약형으로 정리",
        "- missed 자동 알림은 결과와 핵심 이유만 짧게 표시",
        "- 자세한 복기와 긴 설명은 /review_dump에서 확인",
        "",
    ]
...
def build_review_dump_sections(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5):
    ...
    try:
        ...
        if not alert_rows:
            ...
            lines = [
                "[최근 알림 요약]",
                f"- 괜찮음 {summary.get('good', 0)}건 / 튀고식음 {summary.get('spike', 0)}건 / 애매 {summary.get('weak', 0)}건 / 바로밀림 {summary.get('bad', 0)}건",
                "",
...
def is_live_early_trial_candidate(item: dict, display_key: str = "", regime: dict = None) -> bool:
    ...
    chart_rise = safe_float(profile.get("chart_rise_pct", from_low), from_low)
    hot_score = get_user_alert_hot_score(item)
    if high_gap_pct < 0.82 or high_gap_pct > 2.35:
        return False
    if section == "live":
        if from_low > 1.95:
            return False
        if chart_rise > 3.05 and high_gap_pct < 1.40:
            return False
    else:
        if from_low > 2.85:
            return False
