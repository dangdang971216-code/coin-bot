...
def get_watch_bucket(item: dict):
    ...
    if strategy_label in ["상승 시작형", "초반 선점형"]:
        if change_pct <= 1.65 and vol_ratio >= 2.05 and leader_score >= 1.55 and high_gap_pct >= 1.20:
            return 3, "🟢 지금 사도 될 쪽"
        if change_pct <= 2.90 and vol_ratio >= 1.85:
            return 2, "✅ 막 오르기 시작한 후보"
        return 1, "👀 한 번 더 볼 후보"

    if strategy_label == "눌림 반등형":
        if change_pct <= 1.45 and vol_ratio >= 2.10 and high_gap_pct >= 1.05:
            return 2, "✅ 다시 힘 붙는 후보"
        return 1, "👀 조금 더 지켜볼 후보"

    if strategy_label in ["추세 지속형", "급등 돌파형"]:
        if change_pct <= 1.45 and high_gap_pct >= 1.15 and vol_ratio >= 2.00:
            return 2, "✅ 힘 붙는 후보"
...
def send_startup_message():
    lines = [
        ...
        "- 초저가 급등 왜곡 후보 더 보수화",
        "- 고점 근처 후보는 등급을 더 보수적으로 조정",
        "- 직전 탐지(저점 선점 / 신저가 회복) 더 정교화",
        "- 후보별 현재가 / 경과시간 버튼 추가",
        "- 후보별 빗썸 열기 버튼 추가",
        "- 본 후보에 진입가/손절가/목표가/추격금지선 표시 복구",
        "- 예약된 5/15/30분 확인은 재시작 후에도 이어짐",
        "- 시작 알림 정리 포인트를 현재 기준에 맞게 갱신",
        "- 가짜 회복 / 이미 튄 뒤 재반등 후보 더 보수화",
        "- 알림 문구를 더 쉬운 말로 정리",
        "- 알림 안에서 지금 사도 될 쪽 / 한번 볼 후보 / 더 지켜볼 후보를 더 분명히 나눔",
        "- 자세한 설명은 /info",
    ]
    send("\n".join(lines))
    ...
    for ticker, data in cache.items():
        try:
            ...
            if regime_name == "WEAK" and vol_ratio < 2.95:
                continue
            if leader_score < 1.20 and turnover_rank > 32 and surge_rank > 32:
                continue
            ...
            if not (hl or box or half_hold or body_ok):
                continue
            if leader_score < 1.35 and vol_ratio < 3.00:
                continue
            ...
            preview.append(make_signal(
                ...
                reference_high=reference_high,
                reference_low=recent_low,
                reason=f"저점 선점 / 고점까지 {high_gap_pct:.2f}% 여유 / 저가에서 {rebound_from_low_pct:.2f}% / 거래량 {vol_ratio:.2f}배",
            ))
        except Exception:
            continue
