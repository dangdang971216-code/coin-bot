...
def core_text() -> str:
    ...
    return "\n".join([
        ...
        f"🔎 정보: 정밀 {cov.get('precision',0)} / ATR {cov.get('atr',0)} / 위험확인 {s.get('execution_risk_cached',0)} / WS대상 {s.get('ws_target_file_targets',0)} / 최종검증 {s.get('final_entry_note','-')}",
        websocket_status_line(),
        candidate_line(),
        paper_bot_line(),
        guard_summary_line(),
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.191.py", "- 페이퍼봇 대상: paper_bot_v0.37.py"])),
    ]
    start = now_ts()
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v191 출력문구 한글화 + 버전별 성과 10개 추적 + WS표시 보정 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
