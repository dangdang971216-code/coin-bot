...
- 후보 생성 흐름 추적 중(cache/universe 쪽 계속 점검 필요)
- 연속 실패 시 자동 쉬기
...
def build_leader_watch_candidates(cache, regime, relaxed=False):
    ...
    for ticker, data in cache.items():
        try:
            ...
            top_rank_ok = turnover_rank <= 35 or surge_rank <= 35

            min_leader = 0.20 if relaxed else 0.35
            if leader_score < min_leader and not top_rank_ok and activity_score < (0.45 if relaxed else 0.65):
                continue
            if change_1 <= -1.10 and change_3 <= -1.30 and change_5 <= -1.60:
                continue
            if rsi >= 88 and change_3 < 1.10:
                continue
            if upper_wick_too_large(df) and change_3 < (0.35 if relaxed else 0.60) and vol_ratio < (1.10 if relaxed else 1.35):
                continue
            if regime_name == "WEAK" and change_5 > 3.2 and vol_ratio < 1.4:
                continue
            if regime_name == "SIDEWAYS" and change_3 > 2.4 and vol_ratio < 1.3:
                continue
...
def get_scan_debug_candidates():
    try:
        ...
        if (not shared_market_cache) or cache_age > 180:
            try:
                build_shared_market_cache(force=True)
            except Exception:
                pass
            now_ts = time.time()
            cache_age = int(now_ts - shared_market_cache_time) if shared_market_cache_time else 999999
