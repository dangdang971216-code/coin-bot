...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v224 micro urgent 상수복구 + 자원표시 안정화(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
# v222 compatibility: v221 tail accidentally called _v218_ext_maps, but the real v218 cache-map reader is _v218_refresh_external_maps.
...
def _v221_write_micro_urgent_from_candidates(priority_rows: Optional[List[Dict[str, Any]]], reason: str = "factory_final_candidates") -> None:
    try:
        ...
        for r in rows:
            ...
            st = "fresh" if isinstance(mr, dict) and 0 <= age <= MICRO_STALE_SEC else ("stale" if isinstance(mr, dict) else "missing")
            meta[t] = {
        ...
        payload = {
            ...
            "targets": tickers,
            "target_meta": meta,
            "note": "v222: auto/preopen/recheck candidates need fresh micro; fixed broken ext-map dependency; not a strategy-condition change",
        }
...
def _v216_quality_text() -> str:  # type: ignore[override]
    ...
    return (txt.replace("v223 기본은 캐시 전용", "v224 기본은 캐시 전용")
               .replace("v222 기본은 캐시 전용", "v224 기본은 캐시 전용")
               .replace("v221 기본은 캐시 전용", "v224 기본은 캐시 전용")
               .replace("v219 기본은 캐시 전용", "v224 기본은 캐시 전용"))
