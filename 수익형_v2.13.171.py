...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.171: 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def architecture_text() -> str:
    return "- 구조: 허브(시장수집) → 1차직원(표준화) → 2차직원(정밀/위험) → 공장(후보파일) → paper_bot(장부)"
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.171 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.171.py",
        "- 페이퍼봇 대상: paper_bot_v0.30.py",
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    v171: CLOSED에 저장된 entry_context를 우선 읽고, 없으면 과거 top-level 필드 alias를 읽는다.
    """
    ...
    if strict and (cov_change == 0 or cov_high == 0 or cov_vol == 0):
        entry_lines.append(f"⚠️ 일부 과거 CLOSED는 진입재료가 비어 있음: 5분 {cov_change}/{len(strict)} / 고점거리 {cov_high}/{len(strict)} / 거래량 {cov_vol}/{len(strict)}")
    if len(strict) < 50:
    ...
    lines = [
        ...
        "",
        "[2/7] 현재 후보 분리",
        f"- latest strict 전체 {len(latest_strict)} / trade_ready {len(trade_ready)} / 관찰 {len(observe)}",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.171.py", "- 페이퍼봇 대상: paper_bot_v0.30.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.171: 통합상태 + 후보품질 명령 + 가벼운 점검", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.171 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
