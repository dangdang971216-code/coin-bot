...
EXEC_RISK_SYNC_MIN_SCORE = float(os.getenv("CLEAN_EXEC_RISK_SYNC_MIN_SCORE", str(TRADE_READY_MIN_SCORE)))
...
def ws_snapshot(ticker: Any) -> Dict[str, Any]:
    ...
    return {
        "live_price_source": "WS_SIDECAR" if age <= WS_HUB_STALE_SEC else "WS_STALE",
...
def apply_ws_cache_to_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rows or []:
        ...
        if ws.get("ws_fresh") and fnum(ws.get("live_price"), 0) > 0:
            fresh += 1
            # 웹소켓은 실시간 확인용으로 보존한다. v181에서는 REST 가격을 덮지 않고
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.184: 실전위험 우선확인 + 손실유형 분리분석. strict_all 기록, paper OPEN은 trade_ready만.",
    }
...
def core_text() -> str:
    ...
    return "\n".join([
        ...
        f"🔎 정보커버리지: 정밀 {s.get('field_coverage',{}).get('precision',0)} / ema {s.get('field_coverage',{}).get('ema12',0)} / bb {s.get('field_coverage',{}).get('bb',0)} / mfi {s.get('field_coverage',{}).get('mfi',0)} / 위험캐시 {s.get('execution_risk_cached',0)} / 우선확인 {s.get('execution_risk_sync_checked',0)}",
        websocket_status_line(),
        candidate_line(),
        paper_bot_line(),
        guard_summary_line(),
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.184 기준: 실전위험 우선확인 + slow/stop 손실유형 분리분석",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.184.py",
        "- 페이퍼봇 대상: paper_bot_v0.34.py",
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    def latest_ws_line(arr: List[Dict[str, Any]]) -> str:
        ...
        age_txt = f" / 평균나이 {avg_age:.1f}s" if avg_age >= 0 else ""
        return f"- WS 반영: 신선 {fresh}/{len(arr)} / 오래됨 {stale} / REST-WS 가격차 {avg_gap:+.3f}%({len(gaps)}){age_txt}"
    ...
    def reason_profile(label: str, arr: List[Dict[str, Any]]) -> str:
        ...
        return f"- {label}: {len(arr)}건 / 3분돈 {m3/10000:,.0f}만({c3}) / 눌림 {pb:.2f}({cp}) / 재돌파 {rb:.2f}({cr}) / 과폭발 {sp:.2f}({cs}) / 가격재확인 {pr:+.2f}%({cpr})"
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.184.py", "- 페이퍼봇 대상: paper_bot_v0.34.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.184: 실전위험 확인 병목 완화 + slow/stop 분리분석 + DEPLOY_TARGET 정리", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.184 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v183 WS 후보품질 진입문맥 보강 + 배포표시 정리 / 조건·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
