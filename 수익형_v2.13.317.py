...
# ──────────────────────────────────────────────────────────────────────────────
# v2.13.316: command UX rollback + strategy-count primary-source surgery
# - v314 single-response was too short and removed needed /health details.
# - Restore one message per command in auto bundle, but keep v314 timeout suppression.
# - /health stays full compact-health, not hard-trimmed.
# - Strategy grade table uses one effective source: strategy_eval_grades when valid,
#   otherwise primary strategy + candidate grade. This prevents 합산 B/C while every
#   strategy row displays C/X only.
# - No strategy condition/closing/paper ledger changes.
# ──────────────────────────────────────────────────────────────────────────────
BOT_VERSION = "수익형 v2.13.316"
V316_NOTE = "v316: 자동묶음은 다시 명령별 전송, /health 정보 보존, 전략별 등급표 원본 보정. 조건/청산 변경 없음."
V316_REPLY_TIMEOUT_SEC = float(os.getenv("CLEAN_V316_REPLY_TIMEOUT_SEC", "2.8"))
V316_MULTI_MAX_BODY = int(os.getenv("CLEAN_V316_MULTI_MAX_BODY", "5200"))


def _v316_safe_send(text: str) -> float:
    """Send one command block without polluting errorlog on Telegram timeout."""
    st = now_ts()
    body = str(text or "")
    if len(body) > V316_MULTI_MAX_BODY:
        body = body[:V316_MULTI_MAX_BODY - 120] + "\n…\n- v316: 자동묶음 안정성을 위해 일부 생략. 상세는 해당 명령 단독 실행."
    try:
        # v314 path suppresses telegram send timeout from errorlog.
        if '_v314_send_one_no_error' in globals():
            _v314_send_one_no_error(body, timeout=V316_REPLY_TIMEOUT_SEC)
        else:
            _telegram_send_api(body, limit=3900, timeout=V316_REPLY_TIMEOUT_SEC)
    except Exception as exc:
        try:
            log_error("v316_safe_send", exc)
        except Exception:
            pass
    return max(0.0, now_ts() - st)


def _v316_multi_body(name: str, body: str) -> str:
    """Keep useful content. Health is not hard-trimmed; strategy commands use v307 compact not v314 ultra trim."""
    n = str(name or "").lower()
    txt = str(body or "")
    if n == "health":
        return txt  # user needs all health/stage/resource lines
    if n in {"score", "quality", "strategy_watch", "watch"}:
        try:
            return _v307_compact_for_multi(n, txt)
        except Exception:
            return txt
    return txt


def _v316_process_command_lines(update, context, lines: List[str], source: str = "auto") -> bool:
    """v316: restore per-command messages.

    v314 fixed repeated timeout by one-message output, but it hid too much information.
    v316 keeps timeout suppression, yet returns each command as its own message so /health remains useful.
    """
    lines = [ln for ln in (lines or []) if str(ln).strip().startswith("/")]
    if not lines:
        return False
            ...
            if not fn:
                ...
                try:
                    if name in {"loss_review", "version_loss", "loss"}:
                        ...
                        body = str(fn())
                    body = _v316_multi_body(name, body)
                except Exception as exc:
                    log_error(f"multi:{name}", exc)
                    body = f"오류: {exc.__class__.__name__}: {exc}"
                    res = f"ERR {exc.__class__.__name__}"
        build_sec = max(0.0, now_ts() - st)
        send_sec = _v316_safe_send(f"[{idx}/{total}] /{name} (계산 {build_sec:.2f}s / {res})\n" + body)
        rows.append((name, build_sec, send_sec, res))
    build_total = sum(x[1] for x in rows)
    send_total = sum(x[2] for x in rows)
    summary = ["🧾 자동 묶음 시간표 · v316 명령별 전송", "- v314처럼 한 줄로 과하게 줄이지 않고, 명령별로 보냅니다."]
    for name, b, s, res in rows:
        icon = "✅" if res == "OK" else ("❔" if res in {"SKIP", "UNKNOWN"} else "❌")
        summary.append(f"- {icon} /{name}: 계산 {b:.2f}s / 전송 {s:.2f}s / {res}")
    summary.append(f"- 합계: 계산 {build_total:.2f}s / 전송 {send_total:.2f}s / 전체 {now_ts() - total_start:.2f}s")
    summary.append("- 상세 전문은 각 명령을 단독 실행")
    _v316_safe_send("\n".join(summary))
    return True
...
def handle_multi_command_message(update, context) -> bool:  # type: ignore[override]
    ...
    if len(lines) <= 1:
        return False
    return _v316_process_command_lines(update, context, lines, source="auto")
...
def command_batch(update, context) -> None:  # type: ignore[override]
    ...
    try:
        ...
        pass
    _v316_process_command_lines(update, context, lines, source="batch")


def _v316_candidate_grade(row: Dict[str, Any]) -> str:
    g = str((row or {}).get("candidate_grade") or (row or {}).get("shadow_eval_grade") or "C").upper().strip()
    return g if g in {"S", "A", "B", "C", "X"} else "C"


def _v316_primary_strategy(row: Dict[str, Any]) -> str:
    keys = list(V310_STRATEGIES.keys()) if 'V310_STRATEGIES' in globals() else []
    for k in ("strategy_bucket_primary", "primary_strategy_key", "strategy_key", "paper_strategy_key"):
        v = str((row or {}).get(k) or "")
        if v in keys:
            return v
    hits = (row or {}).get("strategy_bucket_hits")
    if isinstance(hits, list):
        for v in hits:
            if str(v) in keys:
                return str(v)
    return keys[0] if keys else "leader_momentum_continuation"


def _v316_effective_strategy_grades(row: Dict[str, Any]) -> Dict[str, str]:
    keys = list(V310_STRATEGIES.keys()) if 'V310_STRATEGIES' in globals() else []
    cg = _v316_candidate_grade(row)
    raw = (row or {}).get("strategy_eval_grades") if isinstance((row or {}).get("strategy_eval_grades"), dict) else {}
    grades: Dict[str, str] = {}
    for k in keys:
        g = str(raw.get(k) or "").upper().strip()
        grades[k] = g if g in {"S", "A", "B", "C", "X"} else ""
    non_empty = [g for g in grades.values() if g]
    # Corrupt patterns seen in v313/v314: aggregate B/C but every strategy is C or X.
    all_c_or_empty = all((g in {"", "C"}) for g in grades.values()) if keys else True
    all_x_or_empty = all((g in {"", "X"}) for g in grades.values()) if keys else False
    conflict = cg in {"S", "A", "B"} and (all_c_or_empty or all_x_or_empty)
    if not non_empty or conflict:
        primary = _v316_primary_strategy(row)
        grades = {k: "C" for k in keys}
        if primary in grades:
            grades[primary] = cg
    else:
        grades = {k: (grades.get(k) or "C") for k in keys}
    return grades


def _v314_strategy_counts_from_rows(rows: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Dict[str, int]]:  # type: ignore[override]
    rows = rows if isinstance(rows, list) else _v301_rows_from_snapshot(200)
    out = {k: {"S": 0, "A": 0, "B": 0, "C": 0, "X": 0} for k in V310_STRATEGIES.keys()}
    for r in rows or []:
        if not isinstance(r, dict):
            continue
        gs = _v316_effective_strategy_grades(r)
        for k in out:
            g = str(gs.get(k) or "C").upper()
            if g not in out[k]:
                g = "C"
...
def _v311_strategy_counts_lines(rows: Optional[List[Dict[str, Any]]] = None) -> List[str]:  # type: ignore[override]
    rows = rows if isinstance(rows, list) else _v301_rows_from_snapshot(200)
    counts = _v314_strategy_counts_from_rows(rows)
    lines = ["[전략별 현재 등급 · v316 보정원본]", "- 합산 B/C와 전략별 표가 따로 놀지 않게 primary strategy 보정 적용"]
    for k, label in V310_STRATEGIES.items():
        c = counts.get(k, {})
        lines.append(f"- {label}: S {c.get('S',0)} / A {c.get('A',0)} / B {c.get('B',0)} / C {c.get('C',0)} / 제외 {c.get('X',0)}")
    return lines
...
def _v313_strategy_grade_from_row(row: Dict[str, Any], key: str) -> str:  # type: ignore[override]
    g = _v316_effective_strategy_grades(row).get(key, "C")
    return g if g in {"S", "A", "B", "C", "X"} else "C"
...
_v316_base_health_text = _v288_health_text

def _v288_health_text() -> str:  # type: ignore[override]
    text = _v316_base_health_text()
    try:
        text = text.replace("수익형 v2.13.315", BOT_VERSION).replace("수익형 v2.13.314", BOT_VERSION).replace("수익형 v2.13.313", BOT_VERSION)
        text = text.replace("v315 main-end 고정 + v314 수술 활성화", "v316 명령별 전송 복구 + 전략표 보정")
        if "v316" not in text:
            text += "\n- v316: 자동묶음 명령별 전송 복구, /health 정보 보존, 전략별 등급표 보정."
    except Exception as exc:
        try:
            log_error("v316_health_text", exc)
        except Exception:
            pass
    return text
...
try:
    STATE["version"] = BOT_VERSION
    STATE["strategy"] = STRATEGY_NAME
    STATE["phase_note"] = V316_NOTE
    STATE["v316_multi_per_command_restored"] = True
    STATE["v316_strategy_count_primary_fix"] = True
except Exception as exc:
    try:
        log_error("v316_init", exc)
    except Exception:
        pass


# ──────────────────────────────────────────────────────────────────────────────
# v2.13.317: B/C 고정 80 해제 + 후보 흐름/표시상한 분리 수술
# - v316에서 합산 B/C가 80~81개처럼 보인 직접 원인은 build_candidates의 shadow[:80]
#   계열 상한과 material snapshot 상한이 전략 결과처럼 보였기 때문이다.
# - 전략 조건을 풀거나 새 전략을 넣지 않는다. B/C 전체 복기 후보는 가능한 한 보존하되,
#   WS/micro target은 v313의 차등 우선순위를 유지해 서버 부담을 막는다.
# - 표시/저장/target 개수를 분리해서 "전략이 80개로 자른 것"과 "서버보호 상한"을 구분한다.
# ──────────────────────────────────────────────────────────────────────────────
BOT_VERSION = "수익형 v2.13.317"
V317_NOTE = "v317: B/C 80 고정 해제. 전체 B/C 복기 후보는 보존하고, WS/micro target만 차등 우선순위로 제한. 조건/청산 변경 없음."

# 전략 조건이 아닌 서버 보호용 안전상한. 현재 빗썸 KRW 전체가 450개 안팎이라 기본 600은 사실상 고정 80 제거용이다.
V317_BC_REVIEW_SAFETY_MAX = int(os.getenv("CLEAN_V317_BC_REVIEW_SAFETY_MAX", "600"))
V317_MATERIAL_SAFETY_MAX = int(os.getenv("CLEAN_V317_MATERIAL_SAFETY_MAX", "600"))
V317_DASHBOARD_READ_MAX = int(os.getenv("CLEAN_V317_DASHBOARD_READ_MAX", "600"))

try:
    # Active build_candidates() still reads these globals at runtime. Raise them above market size
    # so B/C is not artificially fixed at 80. This is not a buy-condition loosen.
    V288_SHADOW_MAX_ROWS = max(int(globals().get("V288_SHADOW_MAX_ROWS", 80)), V317_BC_REVIEW_SAFETY_MAX)
    V288_MATERIAL_MAX_ROWS = max(int(globals().get("V288_MATERIAL_MAX_ROWS", 120)), V317_MATERIAL_SAFETY_MAX)
    V311_SHADOW_LATEST_MAX = max(int(globals().get("V311_SHADOW_LATEST_MAX", 120)), V317_MATERIAL_SAFETY_MAX)
    V311_STRATEGY_LATEST_MAX = max(int(globals().get("V311_STRATEGY_LATEST_MAX", 120)), V317_MATERIAL_SAFETY_MAX)
    V313_MATERIAL_MAX_ROWS = max(int(globals().get("V313_MATERIAL_MAX_ROWS", 90)), V317_MATERIAL_SAFETY_MAX)
except Exception as exc:
    try:
        log_error("v317_raise_review_caps", exc)
    except Exception:
        pass


def _v317_rows_for_dashboard(limit: Optional[int] = None) -> List[Dict[str, Any]]:
    lim = int(limit or V317_DASHBOARD_READ_MAX)
    try:
        return _v301_rows_from_snapshot(lim)
    except Exception:
        return []


def _v317_overall_counts(rows: Optional[List[Dict[str, Any]]] = None) -> Dict[str, int]:
    try:
        return _v311_overall_counts(rows if isinstance(rows, list) else _v317_rows_for_dashboard())
    except Exception:
        out = {"S": 0, "A": 0, "B": 0, "C": 0, "X": 0}
        for r in rows or []:
            g = str((r or {}).get("candidate_grade") or (r or {}).get("shadow_eval_grade") or "C").upper().strip()
            if g not in out:
                g = "C"
            out[g] += 1
        return out
...
def _v317_strategy_counts(rows: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Dict[str, int]]:
    rows = rows if isinstance(rows, list) else _v317_rows_for_dashboard()
    out = {k: {"S": 0, "A": 0, "B": 0, "C": 0, "X": 0} for k in V310_STRATEGIES.keys()}
    for r in rows or []:
        if not isinstance(r, dict):
            continue
        try:
            gs = _v316_effective_strategy_grades(r) if '_v316_effective_strategy_grades' in globals() else (_v313_recompute_strategy_fields(r).get("strategy_eval_grades") or {})
        except Exception:
            gs = {}
        for k in out:
            g = str(gs.get(k) or "C").upper().strip()
...
def _v311_strategy_counts(rows: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Dict[str, int]]:  # type: ignore[override]
    return _v317_strategy_counts(rows)


def _v311_strategy_counts_lines(rows: Optional[List[Dict[str, Any]]] = None) -> List[str]:  # type: ignore[override]
    rows = rows if isinstance(rows, list) else _v317_rows_for_dashboard()
    counts = _v317_strategy_counts(rows)
    total = len(rows or [])
    lines = [
        "[전략별 현재 등급 · v317 표시/전체 분리]",
        f"- 표시/집계 rows {total}개. B/C 전체 복기 후보는 80 고정으로 자르지 않음. 단, WS/micro target은 차등 우선순위로 제한.",
    ]
    for k, label in V310_STRATEGIES.items():
        c = counts.get(k, {})
        # Per-strategy X is not always global 차단; many rows are simply not suitable for that strategy.
        lines.append(
            f"- {label}: S {c.get('S',0)} / A {c.get('A',0)} / B {c.get('B',0)} / C {c.get('C',0)} / 비해당·차단 {c.get('X',0)}"
        )
    return lines


def _v317_flow_lines(rows: Optional[List[Dict[str, Any]]] = None) -> List[str]:
    rows = rows if isinstance(rows, list) else _v317_rows_for_dashboard()
    overall = _v317_overall_counts(rows)
    review_total = int(STATE.get("v288_review_rows", 0) or 0)
    shadow_written = int(STATE.get("shadow_decision", STATE.get("shadow_latest_written", 0)) or 0)
    target_rows = int(STATE.get("v313_review_target_rows", STATE.get("v317_target_rows", 0)) or 0)
    evaluated = int(STATE.get("v295_evaluated_rows", STATE.get("v293_evaluated_rows", 0)) or 0)
    return [
        "[후보 개수 흐름 · v317]",
        f"- 전략판정 대상 {evaluated or '-'} / B·C 복기 전체 {review_total or '-'} / B·C 저장 {shadow_written or '-'} / 현재 표시 {len(rows or [])}",
        f"- 현재 표시 합산: S {overall.get('S',0)} / A {overall.get('A',0)} / B {overall.get('B',0)} / C {overall.get('C',0)} / 제외 {overall.get('X',0)}",
        f"- 정보보강 target {target_rows}개 / B·C 전체를 target으로 보내지 않고 S/A·상위 B/C·정보부족 고점수만 우선 보강",
    ]


_v317_base_quality_text = _v288_quality_text

def _v288_quality_text(full: bool = False) -> str:  # type: ignore[override]
    rows = _v317_rows_for_dashboard(V317_DASHBOARD_READ_MAX if full else V317_DASHBOARD_READ_MAX)
    try:
        rows = [_v313_recompute_strategy_fields(r) for r in rows or [] if isinstance(r, dict)]
    except Exception:
        rows = [r for r in rows or [] if isinstance(r, dict)]
    overall = _v317_overall_counts(rows)
    lines = [
        "🔍 /quality 요약 · v317 B/C 고정해제",
        f"- 합산: S {overall.get('S',0)} / A {overall.get('A',0)} / B {overall.get('B',0)} / C {overall.get('C',0)} / 제외 {overall.get('X',0)}",
        "- 실제 paper OPEN: 각 전략 S/A만. B/C는 OPEN 없이 가상복기. 제외는 차단.",
        *_v317_flow_lines(rows),
        *_v311_strategy_counts_lines(rows),
        "",
        "[S/A 실제 OPEN 후보]",
        *_v311_top_rows(rows, {"S", "A"}, 6),
        ...
        *_v311_top_rows(rows, {"B", "C"}, 8 if full else 5),
        "",
        "- v317: B/C 80개 고정 표시를 해제. target은 서버보호용 차등 우선순위로 별도 제한.",
    ]
    return "\n".join(lines)
...
def _v288_strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    try:
        ...
        rows = [r for r in rows or [] if isinstance(r, dict)]
    lines = [
        "👀 /strategy_watch 요약 · v317 전략별 B/C 전체복기",
        "- S/A=실제 paper OPEN, B/C=가상복기, X=차단. 전략별 표의 비해당은 해당 전략 후보가 아니라는 뜻.",
        *_v317_flow_lines(rows),
        *_v311_strategy_counts_lines(rows),
        "",
        "[전략별 S/A 후보]",
    ...
    for key, label in V310_STRATEGIES.items():
        sub = [r for r in rows if (_v313_strategy_grade_from_row(r, key) if '_v313_strategy_grade_from_row' in globals() else 'C') in {"S", "A"}]
        lines.append(f"[{label} S/A]")
        lines.extend(_v311_top_rows(sub, {"S", "A", "B", "C", "X"}, 3))
    lines += ["", "[가상복기 B/C 대표]", *_v311_top_rows(rows, {"B", "C"}, 6), "- 상세 감시는 /strategy_watch_full"]
    return "\n".join(lines)


_v317_base_score_text = _v288_score_text

def _v288_score_text(rows: Optional[List[Dict[str, Any]]] = None) -> str:  # type: ignore[override]
    text = _v317_base_score_text(rows)
    try:
        rr = rows if isinstance(rows, list) else _v317_rows_for_dashboard()
        insert = "\n".join(_v317_flow_lines(rr))
        if "[후보 개수 흐름 · v317]" not in text:
            text = text.replace("전략: 전략 바구니 S/A 검증 (5전략)", "전략: 전략 바구니 S/A 검증 (5전략)\n" + insert)
        text = text.replace("전략별 현재 등급 · v316 보정원본", "전략별 현재 등급 · v317 표시/전체 분리")
        text = text.replace("/ 제외 ", " / 비해당·차단 ")
    except Exception as exc:
        try:
            log_error("v317_score_text", exc)
        except Exception:
            pass
    return text


_v317_base_health_text = _v288_health_text

def _v288_health_text() -> str:  # type: ignore[override]
    text = _v317_base_health_text()
    try:
        text = text.replace("v316 명령별 전송 복구 + 전략표 보정", "v317 B/C 고정해제 + 차등 target")
        text = text.replace("수익형 v2.13.316", BOT_VERSION).replace("수익형 v2.13.315", BOT_VERSION).replace("수익형 v2.13.313", BOT_VERSION)
        extra = "\n".join(_v317_flow_lines(_v317_rows_for_dashboard()))
        if "[후보 개수 흐름 · v317]" not in text:
            text += "\n" + extra
    except Exception as exc:
        try:
            log_error("v317_health_text", exc)
        except Exception:
            pass
    return text
...
try:
    STATE["version"] = BOT_VERSION
    STATE["strategy"] = STRATEGY_NAME
    STATE["phase_note"] = V317_NOTE
    STATE["v317_bc_fixed_80_removed"] = True
    STATE["v317_bc_review_safety_max"] = V317_BC_REVIEW_SAFETY_MAX
    STATE["v317_material_safety_max"] = V317_MATERIAL_SAFETY_MAX
except Exception as exc:
    try:
        log_error("v317_init", exc)
    except Exception:
        pass
