    ...
    if isinstance(saved, dict):
        for k, v in saved.items():
            ...
            if k in DEFAULT_CONTROL:
                control[k] = v
    # v0.21 고정: candidate_events 소비 옵션은 저장돼 있어도 다시 만들지 않는다.
    for k in REMOVED_CONTROL_KEYS:
        control.pop(k, None)

    # v0.21: 구 control 파일에 남은 24/80/120 고정 갯수 제한은 후보 검증을 막을 수 있으므로
...
def pick_candidates(control: Dict[str, Any], open_pos: Dict[str, Dict[str, Any]]) -> Tuple[List[Tuple[str, Dict[str, Any]]], Dict[str, int]]:
    ...
    paper_events = read_jsonl(FILES["paper_candidates"], max_lines=read_max)
    shadow_events = read_jsonl(FILES["shadow_candidates"], max_lines=read_max)
    combined: List[Tuple[str, Dict[str, Any]]] = [("strict", ev) for ev in paper_events] + [("shadow", ev) for ev in shadow_events]
    ...
    # 최신 scan 묶음 안에서는 갯수로 자르지 않는다. 같은 코인 OPEN/TTL/중복만 거른다.
...
def control_text() -> str:
    c = load_control()
    keys = ["running", "loop_seconds", "max_open_strict", "max_open_shadow", "max_new_per_cycle", "candidate_ttl_sec", "candidate_read_max_lines", "consume_latest_scan_only", "latest_scan_window_sec", "fee_pct_roundtrip", "take_profit_pct", "protect_trigger_pct", "protect_floor_pct", "stop_loss_pct", "slow_minutes", "time_exit_minutes", "block_same_ticker_open"]
...
def handle_command(chat_id: str, text: str) -> None:
    ...
    if cmd in {"/phelp", "/start"}:
        ...
        lines = [
            ...
            fmt_stats("신규 shadow", new_shadow),
            "",
            "[2/7] 시간대별 3시간 묶음",
...
def main() -> None:
    ...
    t_tg.start()
    log(f"{VERSION} started pid={os.getpid()} token={'yes' if TOKEN else 'no'} chat={'yes' if ALLOWED_CHAT_ID else 'no'}")
    while not _stop_event.is_set():
        time.sleep(1)
