...
def send_startup_message():
    lines = [
        ...
        "- 저가 코인 결과 정밀도 보강: 기준가/최고가/종가 raw 값과 평가구간 시각까지 같이 보이게 정리",
...
def get_missed_review_window_stats(ticker: str, base_price: float, eval_start_ts: float = 0.0, eval_end_ts: float = 0.0):
    ...
    try:
        ...
        try:
            ...
            window = recent[(recent["_ts"] >= start_ts - 60) & (recent["_ts"] <= end_ts + 60)]
            if len(window) >= 1:
                recent = window
            else:
                target_bars = max(2, min(len(recent), int(max(1, round((end_ts - start_ts) / 60.0))) + 2))
                recent = recent.tail(target_bars)
        except Exception:
            recent = recent.tail(min(len(recent), 32))
...
def format_missed_review_eval_window(row: dict) -> str:
    ...
    if start_ts <= 0 or end_ts <= 0:
        return ""
    return f"평가구간 {format_ts_hm(start_ts)} ~ {format_ts_hm(end_ts)}"
...
def build_supplemental_missed_review_rows(limit: int = 12):
    ...
    try:
        ...
        for row in reversed(src):
            ...
            stats = get_missed_review_window_stats(ticker, base_price, safe_float(row.get("ts", time.time()), time.time()), safe_float(row.get("ts", time.time()), time.time()) + MISSED_REVIEW_DELAY_SEC)
            best_up_pct = safe_float(stats.get("best_up_pct", 0), 0)
            end_up_pct = safe_float(stats.get("end_up_pct", 0), 0)
            ...
            eval_start_ts = safe_float(stats.get("eval_start_ts", row.get("ts", time.time())), row.get("ts", time.time()))
            eval_end_ts = safe_float(stats.get("eval_end_ts", time.time()), time.time())
...
def _promote_survivor_user_alerts(items):
    ...
    now_ts = time.time()
    drought_sec = _fast_user_alert_drought_sec(now_ts)
    if drought_sec < FAST_SCAN_SURVIVOR_DROUGHT_SEC:
        return list(items)
    user_count = sum(1 for x in items if str(x.get('alert_bucket', '')) == 'user_alert' and str(x.get('alert_grade', '')) in {'S', 'A'})
    if user_count >= 1:
        return list(items)

    def _is_survivor_candidate(item):
        primary_reason = str(item.get('primary_reason', '') or '')
        if primary_reason not in {'재출발 확인 후보', '초입/회복 확인 후보', '막 뜨기 직전 후보', '초입 가능 후보', '초반 선점형'}:
            return False
        reasons = [str(x).strip() for x in item.get('reasons', [])] if isinstance(item.get('reasons'), list) else []
        if any(kw in ' / '.join(reasons) for kw in ['고점이 너무 가까움', '이미 많이 올라와서 늦음', '지금은 구경만']):
            return False
        below_high = safe_float(item.get('below_high', 999), 999)
        from_low = safe_float(item.get('from_low', 0), 0)
        change_5 = safe_float(item.get('change_5', 0), 0)
        ...
        leader = safe_float(item.get('leader_score', 0), 0)
        turnover_5m = safe_float(item.get('turnover_5m', 0), 0)
        market_flow = safe_float(item.get('market_flow_score', 0), 0)
        daily_ready = safe_float(item.get('daily_ready_score', 0), 0)
        breakout_hold = safe_float(item.get('breakout_hold_score', 0), 0)
        chart_bad = _fast_chart_bad(item)
        if chart_bad:
            return False
        if not (0.85 <= below_high <= 2.80):
            return False
        if from_low > 1.85:
            return False
        backdrop = (
            vol_ratio >= 1.20
            or leader >= 1.00
            or turnover_5m >= 12000000
            or market_flow >= 0.85
            or daily_ready >= 0.05
            or breakout_hold >= -0.05
        )
        if not backdrop:
            return False
        if primary_reason == '재출발 확인 후보':
            return (
                max(change_3, change_1, change_30) >= -0.05
                and (vol_ratio >= 1.20 or leader >= 0.90 or turnover_5m >= 12000000)
            )
        if primary_reason == '초입/회복 확인 후보':
            return (
                max(change_5, change_3, change_1) >= 0.08
                and (vol_ratio >= 1.20 or leader >= 1.00 or turnover_5m >= 12000000)
            )
        if primary_reason in {'막 뜨기 직전 후보', '초입 가능 후보', '초반 선점형'}:
            return (
                max(change_5, change_3, change_1) >= 0.10
                and change_30 >= -0.30
                and (vol_ratio >= 1.25 or leader >= 1.00 or turnover_5m >= 15000000)
            )
        return False
    ...
    for item in items:
        if str(item.get('alert_bucket', '')) == 'user_alert':
            continue
        if not _is_survivor_candidate(item):
            continue
        candidates.append(item)
    candidates.sort(
        key=lambda x: (
            _fast_rescue_score(x),
            safe_float(x.get('market_flow_score', 0), 0),
            safe_float(x.get('turnover_5m', 0), 0),
            safe_float(x.get('leader_score', 0), 0),
        ),
        reverse=True,
    )
    for item in candidates[:max(1, int(FAST_SCAN_SURVIVOR_LIMIT))]:
        ticker = str(item.get('ticker', '') or '')
        if not ticker:
            continue
        new_item = dict(item)
        ...
        reasons = list(new_item.get('reasons', [])) if isinstance(new_item.get('reasons'), list) else []
        reasons = [r for r in reasons if '거래량이 아직 약함' not in str(r) and '5분 상승이 아직 약함' not in str(r)]
        reasons.insert(0, '상위 후보라 실제 알림으로 올림')
        new_item['reasons'] = reasons[:5]
        one_liner = str(new_item.get('one_liner', '') or '지금은 흐름만 확인')
        if '지금 사기엔 별로' in one_liner:
            if str(new_item.get('primary_reason', '')) == '재출발 확인 후보':
                one_liner = one_liner.replace('지금 사기엔 별로.', '재출발형 상위 후보.')
            else:
                one_liner = one_liner.replace('지금 사기엔 별로.', '상위 후보라 바로 확인해볼 자리.')
        if '상위 후보라 실제 알림으로 올림' not in one_liner:
            one_liner += ' / 상위 후보라 실제 알림으로 올림'
        new_item['one_liner'] = one_liner
        promoted[ticker] = new_item
    if not promoted:
        return list(items)
    merged = [promoted.get(str(x.get('ticker', '') or ''), x) for x in items]
    merged.sort(
        key=lambda x: (
            safe_float(x.get('bucket_rank', 0)),
            3 if str(x.get('alert_grade', '')) == 'S' else 2 if str(x.get('alert_grade', '')) == 'A' else 1 if str(x.get('alert_grade', '')) == 'B' else 0,
