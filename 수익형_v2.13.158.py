...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.158: 고정 갯수 제한 제거/적응형 정밀보강. candidate_events 없음. paper/shadow만 소비파일.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.158 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.158.py",
        "- 페이퍼봇 대상: paper_bot_v0.19.py",
...
def command_paper_handoff(update, context) -> None:
    lines = [
        ...
        "- 기대 paper_bot: paper_bot_v0.19.py / paper_bot_v0.19",
        "",
        "역할",
...
def candidate_fresh_text() -> str:
    ...
    return "\n".join([
        ...
        "- 오래된 line은 삭제하지 않고, paper_bot 소비 기준만 TTL/최신 event_id 중심으로 제한",
    ])
...
def command_score(update, context) -> None:
    ...
    lines = [
        ...
        f"- 누적 제외된 중복 추정: {max(0, len(rows)-len(deduped))}건",
        f"- 신규 제외된 중복 추정: {max(0, len(new_rows)-len(new_deduped))}건",
        "",
        "[4/5] 신규 청산사유",
    ]
    if by_reason:
    ...
    lines += ["", "[5/5] paper_bot", paper_bot_line(), "", "판독", "- 기존 장부는 삭제하지 않고 보존", "- 자동매매 판단은 신규 CLOSED가 충분히 쌓인 뒤에만 본다"]
    reply(update, "\n".join(lines))
...
def command_batch(update, context) -> None:
    funcs = [
        ("core", core_text),
        ("check", check_text),
        ("score", lambda: "\n".join(["📊 paper 성과판 /score", "", "[요약]", fmt_stats("신규 전체", rows_since_baseline(load_closed(), "closed_at")), fmt_stats("누적 전체", load_closed()), paper_bot_line()])),
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.158.py", "- 페이퍼봇 대상: paper_bot_v0.19.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.158: 고정 갯수 제한 제거 + 장세 적응형 정밀보강", "- 기존 기록 삭제 없음 / 기준점 이후 성과 별도 집계 / 후보판단 상위 N개 고정절단 없음"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.158 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
