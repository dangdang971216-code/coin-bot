...
def build_event(row,of,skey,reasons):
    t=ticker_norm(row.get("ticker")); price=fnum(row.get("current_price"), row.get("price"), default=0)
    label=STRATEGIES.get(skey,skey)
    return {"event_ts":now_ts(),"event_text":now_text(),"ticker":t,"strategy_key":skey,"strategy_name":label,"strategy_bucket_primary":skey,"strategy_bucket_primary_label":label,"candidate_grade":"S","grade":"S","candidate_grade_label":"✅ S급 자동매매 상위","paper_bot_open":True,"open_eligible":True,"trade_ready":True,"lane":"strict","current_price":price,"entry_price":price,"detected_price":price,"score":round(13.0+len(reasons)*0.9+fnum(of.get('orderflow_score'))*0.15,2),"take_profit_pct":1.80,"extended_take_profit_pct":2.80,"protect_trigger_pct":1.00,"protect_floor_pct":0.45,"stop_loss_pct":-0.45,"time_exit_minutes":60,"reasons":reasons[:6],"quality_risk_tags":[],"entry_context":{**{k:row.get(k) for k in ["change_3m","change_5m","change_15m","change_30m","vwap_gap_pct","ema5_gap_pct","relative_strength_pct","turnover_24h","market_mode"]}, **{k:of.get(k) for k in ["spread_pct","buy_ratio","sell_pressure","ws_fresh","micro_fresh","orderflow_score"]},"strategy_reasons":reasons[:6],"brain_version":VERSION}}
def run_once()->Dict[str,Any]:
    st=now_ts(); ts=now_ts(); fobj=load_json(FEATURE,{}) or {}; rows=fobj.get("rows") if isinstance(fobj,dict) else []
    ...
    events.sort(key=lambda e:fnum(e.get("score")), reverse=True); latest=events[:30]
    ...
    save_json(SUMMARY,{"version":VERSION,"updated_ts":ts,"updated_text":now_text(ts),"s_count":len(events),"paper_latest_count":len(latest),"strategies":strat_counts,"events":latest[:12]})
    ...
    sec=now_ts()-st; write_status("running", evaluated=len(rows), s_count=len(events), target_count=len(target), last_sec=round(sec,3)); return {"eval":len(rows),"s":len(events)}
def main()->None:
