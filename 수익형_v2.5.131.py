...
def send_text_block_series(header_text: str = "", blocks=None, footer_text: str = "", limit: int = TG_MESSAGE_SOFT_LIMIT):
    blocks = list(blocks or [])
    sent_any = False
    if str(header_text or "").strip():
        sent = send_text_chunks(header_text, limit=limit)
        if not sent:
            return False
        sent_any = True
    for block in blocks:
        ...
        if not block_text:
            continue
        sent = send_text_chunks(block_text, limit=limit)
        if not sent:
            return False
        sent_any = True
    if str(footer_text or "").strip():
        sent = send_text_chunks(footer_text, limit=limit)
        if not sent:
            return False
        sent_any = True
    return sent_any
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 놓친 코인/자동요약 전송 보강: 긴 메시지는 요약+개별 블록으로 나눠 전송",
        "- 저점 방어 품질 보강: 재이탈 실패/첫 반등/회복 시작 봉 수 반영",
        "- 거래대금 유지 보강: 유지 봉 수/최근 3분·5분 비율/반짝 점수 반영",
        "- 명령어 탭 등록 hotfix: register_bot_commands 누락으로 시작 실패하던 문제 수정",
        "",
    ]
...
def process_missed_review_hourly_report(now_ts: float = None):
    ...
    blocks = [format_missed_review_log_block(row, idx=idx) for idx, row in enumerate(rows[:MISSED_REVIEW_HOURLY_MAX_ITEMS], start=1)]
    remain = len(rows) - MISSED_REVIEW_HOURLY_MAX_ITEMS
    footer_text = f"- 그외 {remain}건은 /missed 에서 더 볼 수 있어" if remain > 0 else ""
    sent = send_text_block_series(header_text=header_text, blocks=blocks, footer_text=footer_text)
    if sent:
        missed_review_meta["last_hourly_cutoff_ts"] = current_hour_start
        persist_missed_review_state()
...
def build_missed_review_row_from_item(item: dict, now_ts: float = None) -> dict | None:
    ...
    return {
        ...
        "low_zone_stay_bars": int(safe_float(item.get("low_zone_stay_bars", 0), 0)),
        "low_zone_reclaim_count": int(safe_float(item.get("low_zone_reclaim_count", 0), 0)),
        "low_zone_defense_score": safe_float(item.get("low_zone_defense_score", 0), 0),
        "low_zone_break_fail_count": int(safe_float(item.get("low_zone_break_fail_count", 0), 0)),
        "low_zone_first_rebound_pct": safe_float(item.get("low_zone_first_rebound_pct", 0), 0),
        "low_zone_recovery_start_bars": int(safe_float(item.get("low_zone_recovery_start_bars", 0), 0)),
        "turnover_persistence_score": safe_float(item.get("turnover_persistence_score", 0), 0),
        "turnover_sustain_ratio_3v10": safe_float(item.get("turnover_sustain_ratio_3v10", 0), 0),
        "turnover_sustain_ratio_5v20": safe_float(item.get("turnover_sustain_ratio_5v20", 0), 0),
        ...
        "turnover_recent_3m_vs_5m_ratio": safe_float(item.get("turnover_recent_3m_vs_5m_ratio", 0), 0),
...
def merge_missed_review_row(base: dict, extra: dict):
    ...
    for key in [
        ...
        "pullback_pct_18", "recovery_from_low_pct", "recovery_speed_pct", "recovery_quality_score", "low_zone_stay_bars", "low_zone_reclaim_count", "low_zone_defense_score", "low_zone_break_fail_count", "low_zone_first_rebound_pct", "low_zone_recovery_start_bars", "turnover_persistence_score", "turnover_sustain_ratio_3v10", "turnover_sustain_ratio_5v20", "turnover_keep_bars_5", "turnover_keep_bars_10", "turnover_recent_3m_vs_5m_ratio", "turnover_flash_score", "pullback_duration_bars",
...
def format_low_zone_turnover_detail_text(item_or_row: dict) -> str:
    item_or_row = item_or_row if isinstance(item_or_row, dict) else {}
    defense = safe_float(item_or_row.get("low_zone_defense_score", 0), 0)
    fail_count = int(safe_float(item_or_row.get("low_zone_break_fail_count", 0), 0))
    first_rebound_pct = safe_float(item_or_row.get("low_zone_first_rebound_pct", 0), 0)
    recovery_start_bars = int(safe_float(item_or_row.get("low_zone_recovery_start_bars", 0), 0))
    keep_bars_5 = int(safe_float(item_or_row.get("turnover_keep_bars_5", 0), 0))
    keep_bars_10 = int(safe_float(item_or_row.get("turnover_keep_bars_10", 0), 0))
    flash_score = safe_float(item_or_row.get("turnover_flash_score", 0), 0)
    ratio_3v5 = safe_float(item_or_row.get("turnover_recent_3m_vs_5m_ratio", 0), 0)
    parts = [f"방어 {defense:.2f}"]
    if fail_count > 0:
        parts.append(f"재이탈실패 {fail_count}회")
    if first_rebound_pct > 0:
        parts.append(f"첫반등 {first_rebound_pct:.2f}%")
    if recovery_start_bars > 0:
        parts.append(f"회복시작 {recovery_start_bars}봉")
    if keep_bars_5 > 0 or keep_bars_10 > 0:
        parts.append(f"거래대금유지 5봉중 {keep_bars_5} / 10봉중 {keep_bars_10}")
    if ratio_3v5 > 0:
        parts.append(f"3분비중 {ratio_3v5:.2f}")
    if abs(flash_score) >= 0.01:
        parts.append(f"반짝점수 {flash_score:.2f}")
    return " / ".join(parts)
...
def format_missed_review_log_block(row: dict, idx: int = 0) -> str:
    ...
    extra_detail = format_low_zone_turnover_detail_text(row)
    return "\n".join([
        ...
        *([f"- 보강수치: {extra_detail}"] if extra_detail else []),
...
def missed_command(update, context: CallbackContext):
    ...
    if rows:
        ...
        if alert_rows:
            header_lines += ["", f"- 한눈정리: 최근엔 알림 {len(alert_rows[:5])}건 / 놓친 강한 움직임 {len(recent)}건 표본이 있어"]
        header_lines += ["", "[최근 놓친 5건]"]
        missed_blocks = [format_missed_review_log_block(row, idx=idx) for idx, row in enumerate(reversed(recent), start=1)]
    if alert_rows:
        alert_blocks = ["[최근 알림 5건 결과]"] + [format_alert_review_block(row, idx=idx) for idx, row in enumerate(alert_rows[:5], start=1)]
    send_text_block_series(header_text="\n".join(header_lines), blocks=missed_blocks + alert_blocks)
