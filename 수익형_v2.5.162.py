# v2.5.162: review_dump 차트 정교화 상세 복구 + 30초 스캔 목표 유지
import os
import time
import json
import math
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 초입 후보 생존력 보강: 거래대금 지속형·저점 방어형은 내부 감시에 더 잘 남김",
        "- JSON 저장 충돌 방지: 저장 전 스냅샷을 떠서 동시 수정 오류를 줄임",
        "- market_refresh 지연 로그 남발을 줄이고 /review_dump 빠른 요약을 추가",
        "",
    ]
...
def clear_review_dump_state():
    ...
    global RECENT_CANDIDATE_ALERTS
    PRICE_CHECK_REFS = {}
    PRICE_CHECK_SCHEDULED = {}
    reverse_trace_logs = []
    reverse_trace_unsaved_count = 0
...
def format_hourly_momentum_watch_report(cache: dict, limit: int = 8) -> str:
    ...
    for idx, (ticker, item, score) in enumerate(rows, start=1):
        ...
        lines.append(
            ...
            f"저점대비 {safe_float(item.get('from_low', item.get('from_30m_low_pct',0)),0):+.2f}% / 고점거리 {safe_float(item.get('below_high', item.get('below_30m_high_pct',999)),999):.2f}%"
        )
        lines.append(
            f"- 차트: 저점방어 {safe_float(item.get('low_zone_defense_score', item.get('recovery_quality_score',0)),0):.2f} / "
            f"돈유지 {safe_float(item.get('turnover_persistence_score',0),0):.2f} / "
            f"재돌파 {safe_float(item.get('rebreakout_score', item.get('rebreakout_prep_score',0)),0):.2f} / "
            f"고점전준비 {safe_float(item.get('prebreakout_ready_score',0),0):.2f} / "
            f"반짝 {safe_float(item.get('turnover_flash_score',0),0):.2f}({format_turnover_flash_label(item.get('turnover_flash_score',0))})"
        )
        reasons = item.get('reasons', []) if isinstance(item.get('reasons', []), list) else []
        if reasons:
