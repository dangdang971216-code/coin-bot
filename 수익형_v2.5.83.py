...
def missed_command(update, context: CallbackContext):
    ...
    if rows:
        ...
        lines += [
            "",
            "[최근 놓친 강한 움직임 쪽]",
            f"- 아예 못 잡음 {summary.get('아예 못 잡음', 0)}건",
            f"- 봤는데 못 올림 {summary.get('봤는데 못 올림', 0)}건",
            f"- 움직였지만 늦음 {summary.get('움직였지만 늦음', 0)}건",
            f"- 판단 빗나감 {summary.get('판단 빗나감', 0)}건",
        ]
        if alert_rows:
...
PREWATCH_MIN_VOL_RATIO = 2.15
PREWATCH_MIN_LEADER_SCORE = 1.05
PREWATCH_MIN_HIGH_GAP_PCT = 0.85
PREWATCH_MAX_HIGH_GAP_PCT = 2.20
PREWATCH_MIN_FROM_LOW_PCT = 0.40
PREWATCH_MIN_3M_PCT = 0.10
PREWATCH_MIN_5M_PCT = -0.05
PREWATCH_MIN_SCORE = 5.45
