...
REGIME_RECOVERY_ONLY_MIN_VOL_WEAK = 3.00
REGIME_RECOVERY_ONLY_MIN_LEADER_WEAK = 1.70
...
WATCH_EXCLUDED_TICKERS = {"USDC", "USDT", "BUSD", "USDP", "DAI", "TUSD", "FDUSD", "USDS"}
WATCH_MIN_EFFECTIVE_30M_MOVE_PCT = 0.28
WATCH_MIN_RECOVERY_FROM_LOW_PCT = 0.45
WATCH_MIN_RANGE_PCT = 0.18
WATCH_HIGH_VOL_MIN = 5.50
WATCH_HIGH_VOL_MIN_MOVE_PCT = 0.18
RECOVERY_ENTRY_MIN_30M_PCT = 0.55
RECOVERY_ENTRY_MIN_FROM_LOW_PCT = 0.85
RECOVERY_ENTRY_MIN_VOL_RATIO = 2.80
RECOVERY_ENTRY_MIN_LEADER_SCORE = 1.40
RECOVERY_ENTRY_MIN_HIGH_GAP_PCT = 1.00
RECOVERY_ENTRY_MIN_5M_PCT = 0.08
...
def should_promote_recovery_candidate_to_entry(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if regime_name == "BLOCK":
        return False
    if regime_name == "WEAK" and REGIME_RECOVERY_ENTRY_BLOCK_IN_WEAK:
        return False
    ...
    if leader_score >= RECOVERY_ENTRY_MIN_LEADER_SCORE:
        strength += 1
    if chg_5 >= RECOVERY_ENTRY_MIN_5M_PCT or change_pct >= 0.45:
        strength += 1
    if tags.intersection({"저점높임", "가짜하락회복", "하락복구", "눌림재확인", "저가회복"}):
        strength += 1

    threshold = 4
    if strategy in ["PREWATCH", "NEWLOW_RECOVER", "LEADER_WATCH"]:
        threshold = 5
...
def is_low_zone_recovery_candidate(item: dict, judgement: str = "", caution: str = "") -> bool:
    ...
    if strategy == "LEADER_WATCH":
        return from_low >= 0.60 and chg_30 >= 0.35 and high_gap_pct >= 1.20 and vol_ratio >= 2.80
    if strategy in ["PREPUMP", "PULLBACK"]:
        return True
    if tags.intersection({"저점높임", "가짜하락회복", "하락복구", "눌림재확인"}) and from_low <= 2.20 and high_gap_pct >= 1.20:
        return True
    if leader_score >= 2.2 and vol_ratio >= 2.4 and from_low <= 1.80 and high_gap_pct >= 1.35:
        return True
    return False
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- BTC 장이 약하면 회복 후보/진입 후보를 더 보수적으로 분류",
        "- 횡보 장에서는 늦은 추격 후보를 더 아래로 내림",
        "- BTC 리포트는 단기/중기/일봉을 같이 보도록 정리",
        "- 장 상태 문구를 더 짧고 읽기 쉽게 정리",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
