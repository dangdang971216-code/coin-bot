...
VERSION="review_worker_v0.1"
INTERVAL_SEC=float(os.getenv("REVIEW_WORKER_INTERVAL_SEC","60"))
...
SUMMARY=BASE_DIR/"clean_review_summary.json"
...
def closed_profit(row:Dict[str,Any])->float:
...
def run_once()->Dict[str,Any]:
    ...
    rows=[r for r in read_jsonl(CLOSED, max_lines=3000) if fnum(r.get("closed_at"),r.get("sell_ts"),r.get("closed_ts"),r.get("updated_ts"), default=now_ts()) >= cutoff]
    big_loss=sorted([r for r in rows if closed_profit(r)<=-0.8], key=closed_profit)[:12]
    good_win=sorted([r for r in rows if closed_profit(r)>=1.0], key=closed_profit, reverse=True)[:12]
    reject=load_json(REJECT,{}) or {}
    save_json(SUMMARY,{"version":VERSION,"updated_ts":now_ts(),"updated_text":now_text(),"retention_days":RETENTION_DAYS,
        "closed_recent":len(rows),"big_loss":[{k:r.get(k) for k in ["ticker","strategy_name","strategy_key","profit_pct","return_pct","close_reason","sell_reason","opened_at_text","closed_at_text"]} for r in big_loss],
        "good_win":[{k:r.get(k) for k in ["ticker","strategy_name","strategy_key","profit_pct","return_pct","close_reason","sell_reason","opened_at_text","closed_at_text"]} for r in good_win],
        "s_reject_reason_top":reject.get("reason_top",[])[:12] if isinstance(reject,dict) else []})
