...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.163: 고정 갯수 절단 없음. paper_bot은 latest scan batch만 소비해 과거 TTL 후보 몰아먹기 방지.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.163 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.163.py",
        "- 페이퍼봇 대상: paper_bot_v0.23.py",
...
def command_paper_handoff(update, context) -> None:
    lines = [
        ...
        "- 기대 paper_bot: paper_bot_v0.23.py / paper_bot_v0.21",
        "",
        "역할",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.163.py", "- 페이퍼봇 대상: paper_bot_v0.23.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.163: 비상 안정화 + 직원 분리 유지 + 웹소켓 기본 OFF", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot 과거 TTL 몰아먹기 방지"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.163 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
