...
def format_chart_quant_summary_text(item: dict, compact: bool = False) -> str:
    ...
    if compact:
        return (
            ...
            f"상단체류 0.5% {int(safe_float(item.get('hold_near_high_bars_05', 0), 0))}봉·1.0% {int(safe_float(item.get('hold_near_high_bars_10', 0), 0))}봉 / 유지 {hold_score:.2f}{turnover_tail}"
        )
    return (
        f"{quant_label} / 몸통1봉 {candle_1_text}·3봉 {candle_3_text}·5봉 {candle_5_text} / "
        ...
        f"{format_recovery_profile_text(item)} / {re_text} / 상단체류 0.5% {int(safe_float(item.get('hold_near_high_bars_05', 0), 0))}봉·1.0% {int(safe_float(item.get('hold_near_high_bars_10', 0), 0))}봉 / 유지 {hold_score:.2f}({hold_bars}봉){turnover_tail}"
    )
...
def format_mtf_context_line(item: dict, short: bool = False) -> str:
    ...
    return f"- 상위봉: {htf_text} / {daily_text}"
...
def get_alert_survival_display_key(item: dict, regime: dict = None) -> str:
    ...
    if strategy in {"PREWATCH", "NEWLOW_RECOVER", "LEADER_WATCH", "PREPUMP", "PULLBACK"}:
        if high_gap_pct >= 1.15 and chart_rise <= 3.90 and from_low <= 2.10 and (
            chg_5 >= 0.14 or chg_30 >= 0.08 or market_flow_score >= 0.95 or leader >= 1.00 or breakout_hold_score >= 0.10 or rebreakout_score >= 0.95 or structure_score >= 0.85
        ):
            return "early"
        if rebreakout_score >= 0.70 or structure_score >= 0.60:
            return "restart"
        return "restart"
    if high_gap_pct >= 1.00 and chart_rise <= 3.80 and from_low <= 1.95 and (
        chg_5 >= 0.16 or chg_30 >= 0.10 or market_flow_score >= 0.95 or leader >= 0.95 or breakout_hold_score >= 0.10 or rebreakout_score >= 1.00 or structure_score >= 0.90
    ):
        return "early"
    if high_gap_pct >= 0.82 and (
        motion >= 0.00 or market_flow_score >= 0.95 or leader >= 0.95 or breakout_hold_score >= 0.10 or vol_ratio >= 1.55 or rebreakout_score >= 0.65 or structure_score >= 0.55
    ):
        return "restart"
    return ""
...
def is_top_turnover_rescue_candidate(item: dict) -> bool:
    ...
    if chart_rise > 4.80 and high_gap_pct < 2.20:
        return False
    if motion < 0.05 and leader < 0.18 and rebreakout_score < 0.70 and structure_score < 0.60:
        return False
    if leader_rank <= 30 or leader >= 0.18:
        return True
    return False
