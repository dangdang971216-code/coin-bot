...
UNIVERSE_FORCE_FULL_BATCH = 6
UNIVERSE_MIN_PARTIAL_ROWS = 3
UNIVERSE_BUILD_MAX_SEC = 2.2
CACHE_REFRESH_BATCH = 3
CACHE_FORCE_FULL_BATCH = 6
CACHE_MIN_PARTIAL_ROWS = 3
CACHE_BUILD_MAX_SEC = 2.2
...
def format_rebreakout_stat_text(item_or_row: dict, compact: bool = False) -> str:
    ...
    if attempts <= 0:
        ...
        if prep_score >= 1.00:
            ...
            prep_label = "아직 시도 전"
        return f"고점재도전 준비도 {prep_score:.2f}({prep_label})"
    if success_attempts <= 0 and failed_attempts <= 0:
        return f"고점재도전 {re_score:.2f}(확인중)"
    return f"고점재도전 {re_score:.2f}({success_rate:.0f}% / 성공 {success_attempts}회 / 실패 {failed_attempts}회)"
...
def format_recovery_profile_text(item_or_row: dict, compact: bool = False) -> str:
    ...
    if compact:
        return f"저점방어 {score:.2f} / 저점체류 {dwell}봉·재확인 {reclaim}회 / 돈유지힘 {persist:.2f}"
    return f"저점방어 {score:.2f} / 저점체류 {dwell}봉·재확인 {reclaim}회 / 돈유지힘 {persist:.2f}"
...
def format_chart_quant_summary_text(item: dict, compact: bool = False) -> str:
    ...
    if current_candle_uninformative:
        ...
        candle_1_text = _format_chart_pct_text(body1)
        close_1_text = _format_chart_pct_text(close1)
...
# v2.5.56 빠른 스캔 재구성
# =========================================================
API_CALL_TIMEOUT_SEC = 1.4
MARKET_REFRESH_STUCK_RESET_SEC = 55
...
FAST_SCAN_DISPLAY_HIDE_SEC = 45
FAST_SCAN_BACKGROUND_REFRESH_SEC = 3
FAST_SCAN_BACKGROUND_FORCE_REFRESH_SEC = 10
FAST_SCAN_STALE_REFRESH_COOLDOWN_SEC = 5
...
def build_scan_health_warning_text(now_ts: float = None) -> str:
    ...
    if cache_age is not None and cache_age >= FAST_SCAN_STATE_STALE_SEC:
        lines.append(f'⚠️ 지금 후보는 {cache_age}초 지난 데이터 기준이라 잠깐 숨길 수 있어')
    ...
    if fast.get('hide_candidates'):
        lines.append('🚫 예전 후보는 숨기고 새 시장 데이터 기다리는 중')
...
def build_scan_debug_text():
    ...
    if hide_candidates:
        if cache_age is not None:
            lines.append(f'안내: 시장 데이터가 오래돼서 후보는 잠깐 숨겼어. 새로 다시 받는 중이야 ({cache_age}초 지난 데이터)')
        else:
            lines.append('안내: 시장 데이터 다시 받는 중이야. 잠깐 뒤에 다시 보면 돼')
        lines.append('')
        lines.append('지금 볼 후보 없음')
        return '\n'.join(lines)
    if not items:
    ...
    def append_group(title: str, rows, icon: str):
        ...
        for i, item in enumerate(rows[:3], 1):
            ...
            lines.append(f"- 상태: {item.get('user_label_text', item.get('label_text','후보'))} / 등급 {item.get('alert_grade','B')} / 현재가 {price_text}")
            lines.append(f"- 흐름: 5분 {safe_float(item.get('change_5',0)):+.2f}% / 거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 주목도 {safe_float(item.get('leader_score',0)):.2f}")
            lines.append(f"- 30분: 저점대비 {safe_float(item.get('from_low',0),0):+.2f}% / {format_fast_scan_high_gap_text(safe_float(item.get('below_high',999),999))}")
            snap = item.get('pre_alert_snapshot', {}) if isinstance(item.get('pre_alert_snapshot'), dict) else item
            raw30 = format_recent_30m_price_context(snap, current_price=safe_float(item.get('price', item.get('current_price', 0)), 0), prefix='- 30분 원값')
            if raw30:
                lines.append(raw30)
            raw60 = format_recent_60m_price_context(item, prefix='- 60분 원값')
            if raw60:
                lines.append(raw60)
            qline = format_chart_quant_line(item, short=True)
            if qline:
                lines.append(qline)
            one_liner = str(item.get('one_liner', '') or '').strip()
            if one_liner:
...
def append_debug_shortlist_parts(parts: list):
    try:
        ...
        for idx, item in enumerate(shortlist, 1):
            ...
            lines.append(f"- 상태: {label} / {user_label} / 5분 {safe_float(item.get('change_5',0)):+.2f}% / 거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 최근 5분 거래대금 {fmt_price(item.get('turnover_5m', 0))}")
            if item.get('one_liner'):
