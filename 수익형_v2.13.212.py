...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.212.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
# ===============================
# v2.13.212: 캐시 생성 저부하 수술 + 스캔 병목 복구
...
def _v210_quality_text() -> str:
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v212 기본은 캐시 전용: 명령어는 캐시만 읽고, 요약직원은 scan 중 무거운 계산을 건너뜁니다.",
...
# ===============================
# v2.13.212: 후보/target/snapshot 단일 본선 수술
# - v211 뒤붙임식 보정은 폐기한다.
...
def _v210_quality_text() -> str:  # type: ignore[override]
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v212 기본은 캐시 전용: 현재 후보는 clean_candidate_snapshot.json 단일 원천만 표시",
        "- 긴 3/6/12 상세와 원자료성 비교는 /quality_full",
        "",
        "[1/5] 성과 요약",
        _compact_stat_line("최근 3시간 정식", rows3),
