...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.169: 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def paper_alive_summary() -> Dict[str, Any]:
    ...
    st = ps["status"]
    ctrl = ps["control"]
    pid = _read_pid_file(BASE_DIR / "paper_bot.pid")
    ...
    return {"status": st, "control": ctrl, "pid": pid, "alive": alive, "age": age, "status_running": status_running, "ctrl_running": ctrl_running}
...
def bot_status_text() -> str:
    ...
    age = p["age"]
    if not p["alive"]:
        p_line = f"❌ 페이퍼봇: pid 죽음 / status_age {_age_text(fnum(st.get('updated_at'),0))}"
    elif age > 180:
        p_line = f"⚠️ 페이퍼봇: alive / 상태갱신 {_age_text(fnum(st.get('updated_at'),0))} 전 / OPEN {st.get('open_total','?')}"
    else:
        p_line = f"✅ 페이퍼봇: {st.get('version','?')} / alive / OPEN {st.get('open_total','?')} / cycle {st.get('elapsed_sec','-')}s"
    return "\n".join([
        "🤖 봇 통합상태 /bot_status",
...
def paper_bot_line() -> str:
    ...
    if not p["alive"]:
        return f"- paper_bot: ❌ pid 죽음 / OPEN {len(open_pos)} / CLOSED {closed_total} / status {_age_text(fnum(st.get('updated_at'),0))} 전"
    age = p["age"]
    icon = "✅" if age >= 0 and age <= 90 else "⚠️"
    age_part = f"상태 {age:.0f}초 전" if age >= 0 else "상태파일 대기"
    return (
        f"- paper_bot: {icon} {st.get('version','?')} / alive / running {p.get('ctrl_running')} / "
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.169 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.169.py",
        "- 페이퍼봇 대상: paper_bot_v0.28.py",
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    lines += [
        "",
        "[4/6] 현재 탈락/차단 상위",
        *reject_lines(limit=8 if full else 5),
        "",
        "[5/6] 운영 상태",
        f"- scan {fnum(STATE.get('scan_last_sec'),0):.2f}s / 정밀캐시 {STATE.get('precision_have',0)} / 후보 strict {STATE.get('strict_decision',0)} / trade_ready {STATE.get('trade_ready_written',0)}",
        paper_bot_line(),
        "",
        "[6/6] 다음 판단",
        "✅ trade_ready 평균이 계속 +면 자동매매 후보로 살림",
        "⚠️ 관찰 후보가 더 잘 가면 trade_ready 기준이 너무 빡센지 확인",
        "❌ 반복 stop_loss 구간은 진입 전 위험필터 보강 후보",
        "❔ 표본 50건 전까지 조건 수정은 보류",
    ]
    if full:
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.169.py", "- 페이퍼봇 대상: paper_bot_v0.28.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.169: 통합상태 + 후보품질 명령 + 가벼운 점검", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.169 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def recent_error_text(n: int = 60) -> str:
    """현재 실행 중 새로 잡힌 오류를 우선 보여준다. 과거 웹소켓 로그가 현재 문제처럼 보이지 않게 분리한다."""
    recent = list(_recent_errors)[-int(n):]
    if recent:
        return "\n".join(recent)
    tail = tail_text(FILES["error"], int(n))
    if "NameError" in tail or "Traceback" in tail:
        return "❔ 새 실행 중 오류는 없음\n\n[과거 오류 tail]\n" + tail[-2600:]
    return tail
...
def _builder_for_command(name: str):
    if not TEXT_COMMAND_BUILDERS:
        TEXT_COMMAND_BUILDERS.update({
            ...
            "help": lambda: "📚 명령어 /help\n- 여러 명령을 줄바꿈으로 보내도 자동 묶음 처리됨\n- /batch /check /core /bot_status /quality /score /cpu_status /paper_handoff /deploy",
        })
...
def install_commands(updater: Any) -> None:
    ...
    try:
        menu = ["batch", "check", "check_full", "core", "bot_status", "quality", "quality_full", "score", "paper_today", "flow_check", "candidate_reason", "cpu_status", "paper_handoff", "paper_handoff_full", "deploy", "trade", "errorlog", "help"]
