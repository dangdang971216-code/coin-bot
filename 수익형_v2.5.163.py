# v2.5.163: 내부 감시 성과추적 + 거래대금 지속성 2차 + 저점방어 승격 2차
import os
import time
import json
import math
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 내부 감시 후보 성과추적: 10분/30분 뒤 실제 움직임을 review_dump에 남김",
...
def format_review_dump_saved_chart_detail(item: dict, idx: int = 1, group_label: str = "후보") -> str:
    ...
    lines = [
        ...
        f"- 분류: bucket {item.get('alert_bucket','-')} / 등급 {item.get('alert_grade', item.get('grade','-'))} / 표시라벨 {item.get('user_label_text', item.get('label_text','후보'))} / primary {item.get('primary_reason','-')}",
    ...
    lines.extend([
        ...
        f"- 상위봉 4시간: {item.get('htf_setup_label','4시간 확인중')} / 위치 {item.get('htf_box_position_label','-')} / 고점거리 {safe_float(item.get('htf_gap_pct',999),999):.2f}% / 저점대비 {safe_float(item.get('htf_rise_pct',0),0):.2f}% / 저점높임3봉 {bool(item.get('htf_higher_low_3', False))}",
        f"- 상위봉 일봉: {item.get('daily_setup_label','일봉 확인중')} / 위치 {item.get('daily_box_position_label','-')} / 고점거리 {safe_float(item.get('daily_gap_pct',999),999):.2f}% / 저점대비 {safe_float(item.get('daily_rise_pct',0),0):.2f}% / 반등일차 {int(safe_float(item.get('daily_rebound_day',0),0))} / 준비점수 {safe_float(item.get('daily_ready_score',0),0):.2f}",
    ...
    lines.append("- 우리 목표 체크: 초반선점/재출발/실시간강세초입 후보인지, 늦은추격/반짝형인지 위 수치로 검증")
    return "\n".join([x for x in lines if str(x).strip()])

def build_review_dump_current_candidates_detail(scan_display_at_start=None, limit_each: int = 8) -> str:
...
def build_review_dump_sections(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5):
    ...
    try:
        sections.append(("scan_debug", build_review_dump_current_candidates_detail(scan_display_at_start, limit_each=8)))
    except Exception as e:
    ...
    try:
        ...
        sections.append(("hourly_momentum", format_hourly_momentum_watch_report(cache_for_hourly, limit=8)))
    except Exception as e:
...
def format_hourly_momentum_watch_report(cache: dict, limit: int = 8) -> str:
    rows = build_hourly_momentum_watch_rows(cache, limit=limit)
    lines = ["[1시간 급등 감시판 상세]", "- 참고: 빗썸 실시간 순위 직접값이 아니라 봇 내부 캐시 기준이야"]
    if not rows:
    ...
    for idx, (ticker, item, score) in enumerate(rows, start=1):
        ...
        lines.append(
            f"- 세부: 1분 {safe_float(item.get('change_1',0)):+.2f}% / 3분 {safe_float(item.get('change_3',0)):+.2f}% / "
            f"거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 시장주목 {safe_float(item.get('leader_score',0)):.2f} / "
            f"저점대비 {safe_float(item.get('from_low', item.get('from_30m_low_pct',0)),0):+.2f}% / 고점거리 {_rd_gap_or_pending(item.get('below_high', item.get('below_30m_high_pct',999)))}"
        )
        lines.append(
            f"- 차트: 저점방어 {_rd_score_or_pending(item, 'low_zone_defense_score')} / "
            f"돈유지 {_rd_score_or_pending(item, 'turnover_persistence_score')} / "
            f"재돌파 {_rd_score_or_pending(item, 'rebreakout_score')} / "
            f"고점전준비 {_rd_score_or_pending(item, 'prebreakout_ready_score')} / "
            f"반짝 {_rd_score_or_pending(item, 'turnover_flash_score')}({format_turnover_flash_label(item.get('turnover_flash_score',0))})"
        )
        reasons = item.get('reasons', []) if isinstance(item.get('reasons', []), list) else []
        if reasons:
            lines.append(f"- 이유: {' / '.join([str(x) for x in reasons[:5]])}")
    return "\n".join(lines)
