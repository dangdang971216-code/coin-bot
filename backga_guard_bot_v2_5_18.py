...
VERSION = "guard_v2.5.18_ws_python_auto_detect_2026-05-15"
...
def ws_import_check_for(python_bin: str) -> tuple[bool, str]:
    code = "import sys; print(sys.executable); import websockets; print('websockets OK', getattr(websockets, '__version__', '?'))"
    env = os.environ.copy()
    env.update(ENV)
    rc, out = run([python_bin, "-c", code], cwd=BOT_DIR, timeout=10)
...
def apply_ws_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    if not pyinfo.get("ok"):
        init_ws_status_file("apply_import_failed")
        return {"ok": False, "changed": False, "target": target.name, "error": "websockets import 가능한 python 없음", "notes": import_notes}
    selected_py = str(pyinfo.get("python") or WS_PYTHON_BIN)
    active_path = BOT_DIR / WS_ACTIVE_FILE
    same_hash = active_path.exists() and short_hash(active_path) == short_hash(target)
    backup = "없음"
    if not same_hash or force:
        ...
        (BOT_DIR / DEPLOYED_WS_FILE).write_text(target.name + "\n", encoding="utf-8")
    if restart:
        active, notes = restart_ws_sidecar(python_bin=selected_py)
        notes = import_notes + [f"selected_python={selected_py}"] + notes
    else:
        state = ws_sidecar_state_dict()
        active = state.get("verdict", "unknown")
        notes = import_notes + ["restart 생략"]
    state = ws_sidecar_state_dict()
    ws_ok = bool(state.get("alive")) and active == "active"
    return {"ok": ws_ok, "changed": changed, "target": target.name, "version": ws_target_version(target), "active": active, "backup": backup, "hash": short_hash(active_path), "notes": notes, "warning": "변경 없음 → active 파일 hash 동일" if not changed else "", "state": state.get("verdict"), "raw": state.get("raw_total"), "match": state.get("match_ok"), "selected_python": locals().get("selected_py", "") }
