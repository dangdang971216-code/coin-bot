...
def _market_refresh_worker(force: bool = False, reason: str = "loop"):
    ...
    try:
        ...
        log_exception_details("market_refresh_worker", e, extra={"force": force, "reason": reason}, prefix="[market_refresh 오류]")
    finally:
...
def build_shared_market_cache(force=False):
    ...
    for idx4, row in enumerate(batch_rows, start=1):
        ...
        if elapsed >= CACHE_BUILD_MAX_SEC:
            min_success = max(2, min(CACHE_MIN_PARTIAL_ROWS, batch_size))
            min_tried = max(3, min(batch_size, CACHE_MIN_PARTIAL_ROWS + 1))
            if refreshed_count >= min_success or tried_count >= min_tried:
                append_runtime_event_log("cache_partial_break", f"{elapsed:.1f}초 제한으로 부분 종료", extra={"tried": tried_count, "success": refreshed_count, "batch_size": batch_size, "force": force})
                break
    if base_cache:
        keep = {}
        keep_times = {}
...
# v2.5.56 빠른 스캔 재구성
# =========================================================
API_CALL_TIMEOUT_SEC = 1.2
MARKET_REFRESH_STUCK_RESET_SEC = 45
FAST_SCAN_RESULT_MAX = 18
FAST_SCAN_INPUT_LIMIT = 24
FAST_SCAN_BUILD_STAGE1_MULTIPLIER = 1.3
FAST_SCAN_BUILD_MAX_SEC = 1.6
FAST_SCAN_BUILD_MIN_ITEMS = 6
FAST_SCAN_BUILD_HEARTBEAT_EVERY = 4
...
FAST_SCAN_STATE_STALE_SEC = 15
FAST_SCAN_DISPLAY_HIDE_SEC = 25
FAST_SCAN_BACKGROUND_REFRESH_SEC = 2
FAST_SCAN_BACKGROUND_FORCE_REFRESH_SEC = 6
FAST_SCAN_STALE_REFRESH_COOLDOWN_SEC = 2
...
def build_fast_scan_candidates_from_cache(cache: dict, limit: int = FAST_SCAN_RESULT_MAX):
    ...
    for idx, (ticker, data, _score) in enumerate(stage1_rows, start=1):
        ...
        if item:
            ...
            built_count += 1
        elapsed = time.time() - started
        enough_items = built_count >= max(limit, FAST_SCAN_BUILD_MIN_ITEMS)
        if elapsed >= FAST_SCAN_BUILD_MAX_SEC and enough_items:
            append_runtime_event_log("fast_scan_partial_break", f"{elapsed:.1f}초 제한으로 후보 조립 부분 종료", extra={"tried": idx, "built": built_count, "stage1_limit": stage1_limit, "limit": limit})
            break
