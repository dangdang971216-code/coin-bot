...
VERSION = "guard_v2.2_exact_apply_2026-05-10"
...
RESTART_WAIT_SEC = int(ENV.get("GUARD_RESTART_WAIT_SEC", "150") or "150")
...
def extract_recent_runtime_version(lines: int = 220, since: Optional[str] = None) -> str:
    try:
        log = recent_journal(lines=lines, since=since)
...
def apply_latest_auto(force: bool = False, explicit: Optional[str] = None) -> str:
    ...
    try:
        ...
        if not target:
            write_json(status_path, {"time": now(), "ok": False, "stage": "find_latest"})
            return "🚫 업그레이드 실패 /gupgrade\n- 수익형_v*.py 파일을 못 찾음"
        if not target.exists():
            write_json(status_path, {"time": now(), "ok": False, "stage": "target_missing", "target": str(target)})
        ...
        dst = BOT_DIR / "bot.py"
        if not dst.exists():
            write_json(status_path, {"time": now(), "ok": False, "stage": "bot_missing", "target": target.name})
            return "🚫 업그레이드 실패\n- bot.py가 없음"
        ...
        if sha256_file(target) != sha256_file(tmp):
            ...
            write_json(status_path, {"time": now(), "ok": False, "stage": "tmp_hash", "target": target.name, "backup": backup.name})
            return "🚫 업그레이드 실패\n- 임시파일 복사 hash 검증 실패"

        os.replace(tmp, dst)
        if sha256_file(target) != sha256_file(dst):
            shutil.copy2(backup, dst)
            write_json(status_path, {"time": now(), "ok": False, "stage": "replace_hash", "target": target.name, "backup": backup.name})
        ...
        if not same_version_label(bot_version_after_copy, target_version_text):
            shutil.copy2(backup, dst)
            write_json(status_path, {"time": now(), "ok": False, "stage": "bot_version_after_copy", "target": target.name, "target_version": target_version_text, "bot_version": bot_version_after_copy, "backup": backup.name})
            return (
                "🚫 업그레이드 실패\n"
        ...
        if not ok:
            shutil.copy2(backup, dst)
            write_json(status_path, {"time": now(), "ok": False, "stage": "post_compile", "target": target.name, "backup": backup.name})
        ...
        log_since = ""
        waited = 0
        wait_limit = max(10, int(RESTART_WAIT_SEC))
        while waited < wait_limit:
            ...
            # active가 살아 있고 fresh fatal이 없으면 더 기다리지 않는다.
            if active == "active" and not fatal:
                break
            # fatal이 보이면 롤백을 빠르게 결정한다.
            if fatal:
                break
        ...
        if active != "active" or fatal:
            ...
            if AUTO_ROLLBACK and backup and backup.exists():
                ...
                write_json(status_path, {"time": now(), "ok": False, "stage": "post_restart", "target": target.name, "target_version": target_version_text, "backup": backup.name, "rolled_back": True, "reason": reason, "runtime_version": runtime_version})
                return (
                    f"🚫 업그레이드 실패 후 자동복구\n"
                    f"- target: {target.name} / {target_version_text}\n"
                    f"- reason: {reason}\n"
            ...
            write_json(status_path, {"time": now(), "ok": False, "stage": "post_restart", "target": target.name, "target_version": target_version_text, "backup": backup.name if backup else "?", "rolled_back": False, "reason": reason, "runtime_version": runtime_version})
        ...
        if runtime_version not in {"시작로그 대기", "시작로그 확인불가"} and not same_version_label(runtime_version, target_version_text):
            # v2.2: 로그 버전 불일치만으로 즉시 롤백하지 않는다. bot.py가 맞으면 일단 보존하고 사람이 확인한다.
        ...
        return (
            ...
            f"- active: {active}\n"
            f"- wait: {waited}s / limit {wait_limit}s\n"
            f"- hash: target {target_hash} / bot.py {bot_hash}\n"
            f"- 경고:\n{warn_text}\n\n"
            f"다음 확인: /gdeploy 또는 메인봇 /batch /core /trade /deploy"
        )
    except Exception as e:
        write_json(status_path, {"time": now(), "ok": False, "stage": "exception", "target": target.name if target else "?", "backup": backup.name if backup else "?", "error": f"{type(e).__name__}: {e}"})
        return f"🚫 업그레이드 예외\n- {type(e).__name__}: {e}"
...
def deploy_status() -> str:
    ...
    runtime_ver = extract_recent_runtime_version(lines=260)
    ...
    if latest != "?" and target not in {latest, "?", ""}:
        verdict_bits.append("DEPLOY_TARGET이 과거 파일을 가리킴")
    ...
    if status:
        ...
        if status.get("reason"):
            last += f" / {status.get('reason')}"

    recent = recent_journal(lines=80)
    focus = []
