...
FAST_SCAN_ALERT_COOLDOWN_SEC = 240
...
def _send_fast_scan_alerts(items, now_ts: float):
    ...
    for item in items:
        ...
        msg = (
            f"🌱 빠른 후보 알림\n\n"
            f"- 코인: {ticker}\n"
            f"- 유형: {item.get('label_text','후보')} / 등급 {item.get('alert_grade','A')}\n"
            ...
            f"- {format_fast_scan_high_gap_text(safe_float(item.get('below_high',999),999))}\n"
            f"- 한줄평: {item.get('one_liner') or '지금은 흐름만 확인'}\n"
