# v2.5.158: v157 안정화 핫픽스 + 미정의 상수/중복 설정 정리
import os
import time
import json
import math
...
USER_ALERT_RESCUE_MIN_VOL = 0.98
USER_ALERT_RESCUE_MIN_LEADER = 0.40
USER_ALERT_RESCUE_MIN_HIGH_GAP_PCT = 0.38
USER_ALERT_RESCUE_MAX_FROM_LOW_PCT = 3.75
USER_ALERT_RESCUE_MIN_LIVE_5M_PCT = 0.08
...
MARKET_REFRESH_INTERVAL_SEC = 3
MARKET_REFRESH_EMPTY_RETRY_SEC = 8
# =========================================================
# 주문 / 자금
# =========================================================
MIN_ORDER_KRW = 5000
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 초입 강세/재출발 후보가 너무 말라붙지 않게 rescue·승격 기준 완화",
        "- 30분 확인/최근 알림 복기 문구를 성공·애매·가짜상승·실패로 더 직설화",
        "- 데이터 조회/분석 실패 세부내용은 일반 알림에서 줄이고 /review_dump 중심으로 정리",
        "",
    ]
...
MARKET_REFRESH_STUCK_RESET_SEC = 35
...
FAST_SCAN_BACKGROUND_REFRESH_SEC = 1.5
FAST_SCAN_BACKGROUND_FORCE_REFRESH_SEC = 4
FAST_SCAN_STALE_REFRESH_COOLDOWN_SEC = 1.5
...
OBS_REEVAL_MIN_TURNOVER_5M = 14_000_000
OBS_REEVAL_MIN_VOL_RATIO = 1.25
...
FAST_SCAN_BUILD_CACHE_MAX_AGE_SEC = 45
...
def request_market_refresh(force: bool = False, reason: str = "loop") -> bool:
    ...
    with market_refresh_lock:
        if market_refresh_in_progress:
            ...
            append_runtime_event_log("market_refresh_stuck_reset", f"{age:.1f}초 정체", extra={"force": force, "reason": reason, "last_reason": last_market_refresh_reason})
            market_refresh_in_progress = False
            market_refresh_started_ts = 0.0
