...
VERSION = "guard_v2.5.39_worker_systemd_manager_2026-05-21"
...
# 메인봇이 무거운 스캔을 직접 하지 않도록 scanner/feature/orderflow/strategy/review를
...
def apply_latest_auto(force: bool = False, explicit: Optional[str] = None, skip_guard: bool = False) -> str:
    ...
    progress_notify("[3/6] 리뉴얼 worker 확인 중\n- scanner/feature/orderflow/strategy/review systemd 관리 적용")
...
def gworker_state_text(name: Optional[str] = None) -> str:
    ...
    lines += ["", "[명령]", "- /gworker_upgrade : worker 5개 최신 적용", "- /gworker_restart : worker 5개 재시작", "- /gscanner_state /gfeature_state /gorderflow_state /gstrategy_state /greview_state"]
    return "\n".join(lines)
...
def gworker_upgrade_text(force: bool = False) -> str:
    ...
    parts = [title, f"- guard: {VERSION}", f"- 대상: scanner → feature → orderflow → strategy → review", f"- total: {fmt_sec(res.get('total_sec',0))}"]
    for name in WORKER_SPECS:
