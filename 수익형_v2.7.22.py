...
- 실거래 주문은 v2.7.22 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.22"
BOT_FILE_TAG = "수익형_v2.7.22.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.22는 현재가+체결 허브를 후보분류에 더 적극 반영한다.
...
WS_MAX_CODES = int(os.getenv("V2710_WS_MAX_CODES", "360"))
# v2.7.22: 일반 WebSocket 직원과 별도로 M2 눌림 후보 전용 직원을 둔다.
WS_M2_WORKER_ON = os.getenv("V2721_WS_M2_WORKER_ON", "1") not in ("0", "false", "False")
WS_M2_MAX_CODES = int(os.getenv("V2721_WS_M2_MAX_CODES", "180"))
# v2.7.22: 조건잠금. 스캔/직원 복구 중에는 M2 후보 승격/강등 조건을 새로 만지지 않는다.
# 실시간자료 부족은 /check와 /data_check에 진단만 남기고, 후보 상태를 강제로 WATCH로 내리지 않는다.
CONDITION_LOCK_ON = os.getenv("V2722_CONDITION_LOCK_ON", "1") not in ("0", "false", "False")
...
WS_PRICE_FRESH_SEC = float(os.getenv("V2710_WS_PRICE_FRESH_SEC", "20"))
WS_TRADE_FRESH_SEC = float(os.getenv("V2710_WS_TRADE_FRESH_SEC", "20"))
WS_CANDLE_FRESH_SEC = float(os.getenv("V2710_WS_CANDLE_FRESH_SEC", "30"))
...
PAPER_PATTERNS = {
    "PULLBACK_RECONFIRM", "PULLBACK_CONFIRM_WAIT", "PULLBACK_PREP_WAIT", "PULLBACK_WAIT",
    "START_EARLY", "START_EARLY_BREAK_HOLD", "START_EARLY_WAIT", "START_EARLY_WEAK_WAIT",
    "SURGE_COOLDOWN_WAIT", "RESTART_WAIT", "REBREAK_WAIT", "SURGE_RECHECK_WAIT", "DATA_CONFIRM_WAIT",
}
PAPER_EXCLUDE_PATTERNS = {"WARMUP", "WATCH", "EARLY_WATCH", "MONEY_WAIT_WATCH", "TICK_NOISE_WATCH", "FLASH_MONEY_WATCH", "CHASE_RISK_WATCH", "OVERHEAT_WATCH", "PULLBACK_RISK_WATCH", "REJECT"}
...
def runtime_resource_snapshot() -> dict:
    """봇 프로세스 CPU/메모리와 서버 load를 가볍게 표시한다."""
    out = {"cpu": "?", "mem_pct": "?", "rss_mb": 0.0, "load1": 0.0, "ok": False}
    try:
        ok, raw, _sec = run_cmd(["bash", "-lc", f"ps -p {os.getpid()} -o %cpu,%mem,rss --no-headers"], timeout=1.0)
        if ok and raw.strip():
            parts = raw.strip().split()
            if len(parts) >= 3:
                out["cpu"] = parts[0]
                out["mem_pct"] = parts[1]
                out["rss_mb"] = round(float(parts[2]) / 1024.0, 1)
                out["ok"] = True
        try:
...
def runtime_resource_text() -> str:
    r = runtime_resource_snapshot()
    if r.get("ok"):
        return f"CPU {r['cpu']}% / MEM {r['mem_pct']}%({r['rss_mb']:.1f}MB) / Load {r.get('load1',0):.2f}"
    return f"CPU 확인불가 / Load {r.get('load1',0):.2f}"
...
class RuntimeState:
    ...
    # v2.7.22: 일반 직원/ M2 전용 직원의 목표 코인을 따로 보관하고 합산해서 진단한다.
    ws_target_groups: dict[str, set[str]] = field(default_factory=lambda: {
        "price_main": set(), "trade_main": set(), "price_m2": set(), "trade_m2": set()
    })
    ws_state: dict = field(default_factory=lambda: {
        ...
        "scan_stage1_count": 0, "scan_stage2_count": 0, "scan_stage3_count": 0,
        "scan_ws_priority_count": 0, "scan_stage2_sample": "", "scan_stage3_sample": "",
        "scan_target_price_count": 0, "scan_target_trade_count": 0,
        "scan_price_no_data": 0, "scan_price_stale": 0,
        "scan_trade_no_data": 0, "scan_trade_stale": 0,
        "scan_m2_target_count": 0, "scan_m2_all_ok": 0,
        "scan_m2_price_wait": 0, "scan_m2_trade_wait": 0, "scan_m2_candle_wait": 0,
        "scan_root_cause": "",
        "price_m2_status": "대기", "trade_m2_status": "대기",
        "price_m2_codes": 0, "trade_m2_codes": 0,
        "price_m2_last_ts": 0.0, "trade_m2_last_ts": 0.0,
        "scan_m2_worker_priority_count": 0, "scan_m2_worker_sample": "",
...
def paper_strategy_name(pattern: str, status: str = "") -> str:
    """
    전략번호 고정.
    이름만 보면 헷갈리므로 paper/후보판에서는 S번호를 함께 쓴다.
    S1~S8은 모의검증 전용 번호이며 실거래 전략번호가 아니다.
    """
    p = str(pattern or status or "")
    if p == "PULLBACK_PREP_WAIT":
        return "S1 눌림준비 검증"
    ...
    if p in ("START_EARLY", "START_EARLY_BREAK_HOLD", "START_EARLY_WAIT", "START_EARLY_WEAK_WAIT"):
        return "S3 상승초입 검증"
    if p == "SURGE_COOLDOWN_WAIT":
        return "S4 상승후 재확인 검증"
    if p == "RESTART_WAIT":
        return "S5 재출발 검증"
    if p == "REBREAK_WAIT":
        return "S6 재돌파 검증"
    if p == "SURGE_RECHECK_WAIT":
        return "S7 급등후 재확인 검증"
    if p == "DATA_CONFIRM_WAIT":
        return "S8 데이터확인 검증"
    return p


def paper_strategy_legend() -> list[str]:
    return [
        "M2 눌림 재확인: 현재 우선 복원 대상. S1/S2만 메인 성과로 본다",
        "M1 저점선점: 당장 메인 아님. S3/S4는 보조 선행감지로만 본다",
        "보조태그: S5 재출발 / S6 재돌파 / S7 급등후 재확인 / S8 데이터확인",
        "S1 눌림준비: 예전 승률형 준비대기 2차, 진입 전 확인",
        "S2 눌림: 저점방어/저점높임/1분 재회복 후보",
        "S3 상승초입: 저점선점 보조 선행감지",
        "S4 상승후 재확인: 저점선점 보조 유지확인",
        "S5 재출발: M2 눌림 판단을 돕는 보조태그",
        "S6 재돌파: M2 눌림 판단을 돕는 보조태그",
        "S7 급등후 재확인: M2 눌림 판단을 돕는 보조태그",
        "S8 데이터확인: 실시간자료 확인용 보조태그",
    ]


def paper_main_strategy_name(pattern: str, strategy: str = "") -> str:
    """
    v2.7.22: 실제 운영 전략은 2개만 본다.
    - M2 눌림 재확인: 현재 우선 복원 대상
    - M1 저점선점: 당장 메인은 아니며 보조/선행감지로만 둔다
    나머지 S5~S8은 전략이 아니라 M2 판단을 돕는 보조태그다.
    """
    p = str(pattern or "")
    s = str(strategy or paper_strategy_name(p))
    if p in ("PULLBACK_PREP_WAIT", "PULLBACK_RECONFIRM", "PULLBACK_CONFIRM_WAIT", "PULLBACK_WAIT") or "눌림" in s:
        return "M2 눌림 재확인"
    if p in ("START_EARLY", "START_EARLY_BREAK_HOLD", "START_EARLY_WAIT", "START_EARLY_WEAK_WAIT", "SURGE_COOLDOWN_WAIT") or "상승초입" in s or "상승후" in s:
        return "M1 저점선점 보조"
    return "보조태그"
...
def paper_strategy_priority(pattern: str, strategy: str = "") -> int:
    """
    v2.7.22: 대표전략 우선순위 재정렬.
    실제 우선 복원 대상은 M2 눌림 재확인이다.
    상승초입/재출발/재돌파/급등후재확인은 폐기하지 않고 보조태그로 남긴다.
    """
    p = str(pattern or "")
    s = str(strategy or paper_strategy_name(p))
    if p in ("PULLBACK_RECONFIRM", "PULLBACK_CONFIRM_WAIT"):
        return 100
    if p == "PULLBACK_PREP_WAIT" or "눌림준비" in s:
        return 95
    if p == "PULLBACK_WAIT" or "눌림" in s:
        return 90
    if p in ("START_EARLY", "START_EARLY_BREAK_HOLD", "START_EARLY_WAIT", "START_EARLY_WEAK_WAIT", "SURGE_COOLDOWN_WAIT") or "상승초입" in s or "상승후" in s:
        return 55
    if p in ("RESTART_WAIT", "REBREAK_WAIT", "SURGE_RECHECK_WAIT") or "재출발" in s or "재돌파" in s or "급등후" in s:
        return 45
    if p == "DATA_CONFIRM_WAIT" or "데이터" in s:
        return 25
    return 10


def upgrade_paper_representative_locked(row: PaperCandidateRow, pattern: str, strategy: str, score: float = 0.0) -> bool:
    """
    병합 대상 paper row의 대표전략이 더 약한 분류면 더 강한 본선 분류로 승격한다.
    기존 대표는 보조태그로 남겨 통계 왜곡을 줄인다.
    """
    old_pattern = row.pattern
    old_strategy = row.strategy
    if paper_strategy_priority(pattern, strategy) <= paper_strategy_priority(old_pattern, old_strategy):
        return False
    if old_pattern and old_pattern != pattern and old_pattern not in row.aux_patterns:
        row.aux_patterns.append(old_pattern)
    if old_strategy and old_strategy != strategy and old_strategy not in row.aux_strategies:
        row.aux_strategies.append(old_strategy)
    row.pattern = pattern
...
def update_paper_store_locked(candidates: list[Candidate], market_rows: dict[str, MarketRow]):
    ...
    for c in candidates:
        ...
        if merged:
            changed = False
            upgraded = upgrade_paper_representative_locked(merged, pattern, strategy, float(c.score))
            if upgraded:
                changed = True
            ...
            if changed:
                action = "대표승격" if upgraded else "paper병합"
                STATE.paper_events.appendleft(f"{fmt_time(ts)} {c.ticker} {action} / 대표 {merged.strategy} / 보조 {strategy} / 중복집계 방지")
            continue
...
def _unique_tickers(seq, limit: int | None = None) -> list[str]:
    out = []
    seen = set()
    for t in seq:
...
def scan_stage_pools_locked(limit: int = WS_MAX_CODES) -> dict:
    """
    v2.7.22 스캔 구조 복구 1차.
    1차 넓은 스캔 → 2차 정밀감시 → 3차 M2 정밀스캔 순서로 WebSocket 우선감시 목록을 만든다.
    """
    rank_rows = list(STATE.rank_rows)
    market_rows = list(STATE.market_rows.values())
    ...
    paper_rows = list(STATE.paper_candidates.values())
    m2_patterns = {"PULLBACK_RECONFIRM", "PULLBACK_CONFIRM_WAIT", "PULLBACK_PREP_WAIT", "PULLBACK_WAIT"}
    m2_watch_patterns = m2_patterns | {"PULLBACK_RISK_WATCH"}

    by_money = sorted(market_rows, key=lambda r: r.money24h, reverse=True)[:220]
    by_move = sorted(market_rows, key=lambda r: (abs(r.ch1) * 1.2 + abs(r.ch3) + abs(r.ch5) * 0.8, r.money24h), reverse=True)[:220]
    # 1차는 넓게 본다. 단, 실제 정밀판단은 2차/3차에서 한다.
    stage1 = _unique_tickers([r.ticker for r in rank_rows] + [r.ticker for r in by_move] + [r.ticker for r in by_money], 300)

    pending = [c.ticker for c in candidates if c.status in ("BUY_READY", "PENDING")]
    watch_top = [c.ticker for c in candidates if c.status == "WATCH"][:120]
    active_paper = [r.ticker for r in paper_rows if r.active]
    store_recent = [r.ticker for r in stored[:180]]
    # 2차는 후보판/창고/paper/급변 후보를 묶어 실시간 우선감시한다.
    stage2 = _unique_tickers(pending + active_paper + store_recent + [r.ticker for r in by_move[:140]] + watch_top, 260)

    m2_candidates = [c.ticker for c in candidates if c.pattern in m2_watch_patterns or "눌림" in str(c.label) or "눌림" in str(c.reason) or "M2" in str(c.reason)]
    m2_store = [r.ticker for r in stored if r.pattern in m2_watch_patterns or "눌림" in str(r.label) or "눌림" in str(r.reason)][:120]
    m2_paper = [r.ticker for r in paper_rows if r.active and (r.pattern in m2_patterns or str(r.strategy).startswith(("S1", "S2")) or "눌림" in str(r.strategy))][:120]
    # 3차M2는 예전 눌림 재확인 라인 복구용 최우선 풀이다.
    # 여기 들어온 코인은 가격/체결/봇1분봉이 최신이어야 후보판 M2 대기로 올라갈 수 있다.
    stage3 = _unique_tickers(m2_paper + m2_candidates + m2_store, 180)
    priority = _unique_tickers(["BTC", "ETH", "XRP", "SOL"] + stage3 + stage2 + stage1, limit)
    return {"stage1": stage1, "stage2": stage2, "stage3": stage3, "priority": priority}


def ws_candidate_tickers(limit: int = WS_MAX_CODES) -> list[str]:
    # v2.7.22: 구독 = 돈 내는 구독이 아니라 실시간 가격/체결/1분봉 우선감시 목록이다.
    with STATE.lock:
        pools = scan_stage_pools_locked(limit)
        out = list(pools.get("priority", []))[:limit]
        STATE.ws_state["scan_stage1_count"] = len(pools.get("stage1", []))
        STATE.ws_state["scan_stage2_count"] = len(pools.get("stage2", []))
        STATE.ws_state["scan_stage3_count"] = len(pools.get("stage3", []))
        STATE.ws_state["scan_ws_priority_count"] = len(out)
        STATE.ws_state["scan_stage2_sample"] = ",".join(pools.get("stage2", [])[:8])
        STATE.ws_state["scan_stage3_sample"] = ",".join(pools.get("stage3", [])[:8])
    return out
...
def _set_ws_target_group_locked(kind: str, group: str, tickers: list[str]):
    """v2.7.22: 일반 직원과 M2 전용 직원의 감시 목표를 분리해서 합산한다."""
    key = f"{kind}_{group}"
    STATE.ws_target_groups[key] = set(tickers or [])
    if kind == "price":
        merged = set()
        for k, v in STATE.ws_target_groups.items():
            if k.startswith("price_"):
                merged |= set(v or [])
        STATE.ws_price_targets = merged
    else:
        merged = set()
        for k, v in STATE.ws_target_groups.items():
            if k.startswith("trade_"):
                merged |= set(v or [])
        STATE.ws_trade_targets = merged


def ws_m2_candidate_tickers(limit: int = WS_M2_MAX_CODES) -> list[str]:
    """v2.7.22: M2 눌림 재확인 전용 WebSocket 직원이 맡을 코인."""
    with STATE.lock:
        pools = scan_stage_pools_locked(max(WS_MAX_CODES, limit))
        stage3 = list(pools.get("stage3", []))
        # M2 worker는 BTC/ETH/SOL/XRP 기준 흐름 + M2 3차 후보만 집중한다.
        out = _unique_tickers(["BTC", "ETH", "XRP", "SOL"] + stage3, limit)
        STATE.ws_state["scan_m2_worker_priority_count"] = len(out)
        STATE.ws_state["scan_m2_worker_sample"] = ",".join(out[:10])
    return out

def _m2_focus_tickers_locked() -> set[str]:
    """v2.7.22: M2 3차 정밀스캔 대상 후보 집합."""
    m2_patterns = {"PULLBACK_RECONFIRM", "PULLBACK_CONFIRM_WAIT", "PULLBACK_PREP_WAIT", "PULLBACK_WAIT", "PULLBACK_RISK_WATCH"}
    out = set()
    try:
        for c in list(STATE.candidates):
            txt = " ".join([str(getattr(c, "pattern", "")), str(getattr(c, "label", "")), str(getattr(c, "reason", "")), str(getattr(c, "caution", ""))])
            if getattr(c, "pattern", "") in m2_patterns or "눌림" in txt or "M2" in txt:
                out.add(str(c.ticker).upper())
        for r in list(STATE.paper_candidates.values()):
            txt = " ".join([str(getattr(r, "pattern", "")), str(getattr(r, "strategy", ""))])
            if getattr(r, "active", False) and (getattr(r, "pattern", "") in m2_patterns or "S1" in txt or "S2" in txt or "눌림" in txt):
                out.add(str(r.ticker).upper())
        for r in list(STATE.candidate_store.values()):
            txt = " ".join([str(getattr(r, "pattern", "")), str(getattr(r, "label", "")), str(getattr(r, "reason", ""))])
            if getattr(r, "pattern", "") in m2_patterns or "눌림" in txt or "M2" in txt:
                out.add(str(r.ticker).upper())
    except Exception:
        pass
    out.discard("USDT")
...
def scan_root_cause_from_snapshot(st: dict) -> str:
    """v2.7.22: 구독/수신/활성 부족 원인을 초보자용으로 압축한다."""
    priority = int(st.get("scan_ws_priority_count", 0) or 0)
    price_targets = int(st.get("scan_target_price_count", 0) or 0)
    price_live = int(st.get("price_live", 0) or 0)
    price_total = int(st.get("price_total", 0) or 0)
    price_no = int(st.get("scan_price_no_data", 0) or 0)
    price_stale = int(st.get("scan_price_stale", 0) or 0)
    trade_live = int(st.get("trade_live", 0) or 0)
    m2_target = int(st.get("scan_m2_target_count", 0) or 0)
    m2_ok = int(st.get("scan_m2_all_ok", 0) or 0)
    if priority <= 0 or price_targets <= 0:
        return "우선감시 목록 생성 전"
    if price_no > max(20, price_targets * 0.50):
        return f"우선감시는 {priority}개지만 가격 무수신 {price_no}개가 큼"
    if price_stale > max(20, price_targets * 0.35):
        return f"우선감시는 {priority}개지만 가격 갱신 오래됨 {price_stale}개가 큼"
    if price_live < max(10, price_total * 0.20) and trade_live < max(10, int(st.get("trade_total", 0) or 0) * 0.20):
        return "실제 수신 활성 낮음: 거래 없는 코인/거래소 송신 적음 가능"
    if m2_target and m2_ok < max(1, m2_target // 3):
        return f"M2 정밀대상 {m2_target}개 중 실시간 정상 {m2_ok}개라 M2 판단 보수 필요"
    return "우선감시/수신 정상 범위"
...
def websocket_worker(kind: str, group: str = "main"):
    if not WS_HUB_ON:
        _ws_state_set(**{f"{prefix}_status": "꺼짐"})
        return
    mode_index = 0
    modes = ["v2", "legacy"]
    while not STATE.shutdown:
        ...
        prefix = kind if group == "main" else f"{kind}_{group}"
        worker_name = f"ws_{kind}_worker" if group == "main" else f"ws_{kind}_{group}_worker"
        try:
            if group == "m2":
                if not WS_M2_WORKER_ON:
                    _ws_state_set(**{f"{prefix}_status": "꺼짐", f"{prefix}_connected": False})
                    time.sleep(10)
                    continue
                tickers = ws_m2_candidate_tickers(WS_M2_MAX_CODES)
            else:
                tickers = ws_candidate_tickers(WS_MAX_CODES)
            with STATE.lock:
                _set_ws_target_group_locked(kind, group, tickers)
            set_worker_heartbeat(worker_name, stage="select_codes", codes=len(tickers), mode=mode)
            if not tickers:
                _ws_state_set(**{f"{prefix}_status": "구독대기", f"{prefix}_connected": False, f"{prefix}_mode": mode})
                time.sleep(3)
                continue
            try:
                import websocket  # websocket-client
            except Exception as e:
                _ws_state_set(**{f"{prefix}_status": "라이브러리없음", f"{prefix}_connected": False, f"{prefix}_error": str(e)[:120]})
                time.sleep(30)
                continue
            url = WS_LEGACY_URL if mode == "legacy" else WS_URL
            _ws_state_set(**{f"{prefix}_status": "연결중", f"{prefix}_connected": False, f"{prefix}_codes": len(tickers), f"{prefix}_mode": mode, f"{prefix}_error": ""})
            ...
            if mode == "legacy":
                ...
                req = [{"ticket": f"coinbot-{kind}-{group}-{int(time.time())}"}, {"type": "ticker" if kind == "price" else "trade", "codes": codes}, {"format": "DEFAULT"}]
            ws.send(json.dumps(req))
            opened = now_ts()
            _ws_state_set(**{f"{prefix}_status": "수신중", f"{prefix}_connected": True, f"{prefix}_codes": len(tickers), f"{prefix}_mode": mode})
            set_worker_heartbeat(worker_name, stage="subscribed", codes=len(tickers), mode=mode)
            got = False
            while not STATE.shutdown:
                if now_ts() - opened >= WS_SUBSCRIBE_REFRESH_SEC:
                    break
                if not got and now_ts() - opened >= WS_NO_MESSAGE_SWITCH_SEC:
                    _ws_state_set(**{f"{prefix}_status": "무수신전환", f"{prefix}_connected": False})
                    break
                try:
                    raw = ws.recv()
                except Exception as e:
                ...
                if not raw:
                    continue
                with STATE.lock:
                    STATE.ws_state[f"{prefix}_raw"] = int(STATE.ws_state.get(f"{prefix}_raw", 0)) + 1
                parsed = parse_ws_ticker_payload(raw) if kind == "price" else parse_ws_trade_payload(raw)
                if not parsed:
                    with STATE.lock:
                        STATE.ws_state[f"{prefix}_empty"] = int(STATE.ws_state.get(f"{prefix}_empty", 0)) + 1
                    continue
                got = True
                set_worker_heartbeat(worker_name, stage="recv", parsed=len(parsed), mode=mode)
                for item in parsed:
                    if kind == "price":
            ...
            _ws_state_set(**{f"{prefix}_status": "재연결대기", f"{prefix}_connected": False, f"{prefix}_error": f"{e.__class__.__name__}: {str(e)[:90]}"})
            ...
            try:
                ...
                pass
            _ws_state_set(**{f"{prefix}_connected": False})
        mode_index += 1
...
def ws_hub_snapshot() -> dict:
    ...
    with STATE.lock:
        ...
        price_rows = dict(STATE.ws_price_hub)
        trade_rows = {t: rows for t, rows in STATE.ws_trade_hub.items()}
        candle_rows = dict(STATE.candle_1m_hub)
        price_targets = set(STATE.ws_price_targets)
        trade_targets = set(STATE.ws_trade_targets)
        m2_targets = _m2_focus_tickers_locked()
        price_live = len([r for r in price_rows.values() if isinstance(r, WsPriceRow) and now - r.ts <= WS_PRICE_FRESH_SEC])
        price_total = len(price_rows)
        trade_live = len([t for t, rows in trade_rows.items() if rows and now - rows[-1].ts <= WS_TRADE_FRESH_SEC])
        trade_total = len(trade_rows)
        candle_live = len([c for c in candle_rows.values() if isinstance(c, Candle1mRow) and now - c.updated_ts <= WS_CANDLE_FRESH_SEC])
        candle_total = len(candle_rows)

        def _price_ok(t: str) -> bool:
            r = price_rows.get(t)
            return isinstance(r, WsPriceRow) and now - r.ts <= WS_PRICE_FRESH_SEC
        def _trade_ok(t: str) -> bool:
            rows = trade_rows.get(t)
            return bool(rows and now - rows[-1].ts <= WS_TRADE_FRESH_SEC)
        def _candle_ok(t: str) -> bool:
            c = candle_rows.get(t)
            return isinstance(c, Candle1mRow) and now - c.updated_ts <= WS_CANDLE_FRESH_SEC

        price_no_data = len([t for t in price_targets if t not in price_rows])
        price_stale = len([t for t in price_targets if t in price_rows and not _price_ok(t)])
        trade_no_data = len([t for t in trade_targets if t not in trade_rows])
        trade_stale = len([t for t in trade_targets if t in trade_rows and not _trade_ok(t)])
        m2_price_wait = len([t for t in m2_targets if not _price_ok(t)])
        m2_trade_wait = len([t for t in m2_targets if not _trade_ok(t)])
        m2_candle_wait = len([t for t in m2_targets if not _candle_ok(t)])
        m2_all_ok = len([t for t in m2_targets if _price_ok(t) and _trade_ok(t) and _candle_ok(t)])
    st.update({
        "price_live": price_live, "price_total": price_total,
        "trade_live": trade_live, "trade_total": trade_total,
        "candle_live": candle_live, "candle_total": candle_total,
        "scan_target_price_count": len(price_targets), "scan_target_trade_count": len(trade_targets),
        "scan_price_no_data": price_no_data, "scan_price_stale": price_stale,
        "scan_trade_no_data": trade_no_data, "scan_trade_stale": trade_stale,
        "scan_m2_target_count": len(m2_targets), "scan_m2_all_ok": m2_all_ok,
        "scan_m2_price_wait": m2_price_wait, "scan_m2_trade_wait": m2_trade_wait, "scan_m2_candle_wait": m2_candle_wait,
    })
    st["scan_root_cause"] = scan_root_cause_from_snapshot(st)
    return st
...
def render_data_check() -> str:
    ...
    lines = [
        "🧱 데이터 허브 /data_check v2.7.22",
        "- 목적: 1차→2차→3차 스캔과 실시간 가격·체결·봇 1분봉 상태를 확인",
        "",
        "1) 스캔 단계",
        f"- 1차 넓은 스캔: {st.get('scan_stage1_count',0)}개 / 전체 시장·순위·급변 후보",
        f"- 2차 정밀감시: {st.get('scan_stage2_count',0)}개 / 후보판·창고·paper·급변 후보",
        f"- 3차 M2 정밀스캔: {st.get('scan_stage3_count',0)}개 / S1/S2·눌림·위험눌림 상위",
        f"- WebSocket 우선감시 목표: {st.get('scan_ws_priority_count',0)}개 / 가격목표 {st.get('scan_target_price_count',0)} / 체결목표 {st.get('scan_target_trade_count',0)}",
        f"- M2 전용직원: 가격 {st.get('price_m2_status','-')} {st.get('price_m2_codes',0)}개 / 체결 {st.get('trade_m2_status','-')} {st.get('trade_m2_codes',0)}개 / 목표 {st.get('scan_m2_worker_priority_count',0)}개",
        f"- 수신부족 원인: {st.get('scan_root_cause','-')}",
        f"- 2차 샘플: {st.get('scan_stage2_sample','-') or '-'}",
        f"- 3차M2 샘플: {st.get('scan_stage3_sample','-') or '-'}",
        "",
        "2) 실시간 허브",
        ...
        f"- 가격 무수신/오래됨: 무수신 {st.get('scan_price_no_data',0)} / 오래됨 {st.get('scan_price_stale',0)}",
        f"- 체결 무수신/오래됨: 무수신 {st.get('scan_trade_no_data',0)} / 오래됨 {st.get('scan_trade_stale',0)}",
        f"- M2 정밀대상: {st.get('scan_m2_target_count',0)}개 / 실시간정상 {st.get('scan_m2_all_ok',0)} / 가격대기 {st.get('scan_m2_price_wait',0)} / 체결대기 {st.get('scan_m2_trade_wait',0)} / 1분봉대기 {st.get('scan_m2_candle_wait',0)}",
    ]
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.22 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
SPECIAL_WAIT_PATTERNS = {"RESTART_WAIT", "REBREAK_WAIT", "SURGE_RECHECK_WAIT", "SURGE_COOLDOWN_WAIT"}
...
def special_wait_label(pattern: str) -> tuple[str, str, str]:
    if pattern == "SURGE_COOLDOWN_WAIT":
        return "보조 상승후 재확인", "초입 급등 뒤 식는 중. 눌림이 아니라 유지 확인 상태", "상승 후 재확인"
    if pattern == "RESTART_WAIT":
        return "보조 재출발 확인", "급등 후 눌림에서 다시 회복 중. 재출발 상태 유지", "재출발 확인"
    if pattern == "SURGE_RECHECK_WAIT":
        return "보조 급등후 확인", "강한 반등 후 고점권 재확인 상태 유지", "급등 후 재확인"
    if pattern == "REBREAK_WAIT":
        return "보조 재돌파 확인", "고점 찍고 밀린 뒤 재도전 상태 유지", "고점 재돌파 확인"
    return "재확인대기", "특수 대기 상태 유지", "재확인"



def v315_mainline_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool) -> dict:
    """
    v2.7.22: v2.5.3xx 후기판의 본선 판단을 clean core row로 옮긴 1차 컨텍스트.
    핵심 원칙은 '바로진입 위험은 준비/눌림대기', '현재봉 약함·돈 얇음은 차단',
    '추세 2개 이상 + 체결/돈 + 현재봉이 죽지 않음'이다.
    """
    trend_count = sum(1 for x in (row.ch1, row.ch3, row.ch5) if x >= 0.10)
    live_trade_amount = safe_float(row.ws_trade_amount_60, 0)
    live_trade_count = int(safe_float(row.ws_trade_count_60, 0))
    buy_ratio = safe_float(row.ws_buy_ratio_60, 0)
    money3 = safe_float(row.money3m, 0)
    money5 = safe_float(row.money5m, 0)

    # v2.5.3xx common_confirm의 clean core 버전.
    # 거래건수/대금/매수비율 중 하나가 확실해야 하고, 현재봉이 죽으면 준비대기로도 올리지 않는다.
    trade_ok = (
        live_trade_amount >= 2_000_000
        or live_trade_count >= 5
        or buy_ratio >= 70
        or money3 >= 10_000_000
    )
    trade_strong = (
        live_trade_amount >= 8_000_000
        or live_trade_count >= 10
        or money3 >= 25_000_000
        or money5 >= 35_000_000
    )
    current_bad = row.own1m_state in ("밀림", "윗꼬리") or one_min_milrim or row.current_milrim
    current_alive = row.own1m_state in ("회복", "보합") and not current_bad
    weak_now = current_bad or row.money_flash_risk or (live_trade_count <= 1 and live_trade_amount < 1_000_000)
    common_confirm = trend_count >= 2 and trade_ok and current_alive and not weak_now

    # v2.5.310의 'BUY_OK 바로진입 위험 → 준비/눌림대기'에 해당하는 후보.
    # 힘은 있는데 바로 사기엔 고점/현재봉/재확인 리스크가 있어 준비대기로 빼는 라인.
    direct_to_prepare = (
        data_3m_ok
        and data_5m_ok
        and common_confirm
        and row.money24h >= 500_000_000
        and row.ch3 >= 0.12
        and row.ch5 >= 0.10
        and row.from_low30 <= 3.05
        and row.high_gap30 >= 0.28
        and not row.tick_noise
        and not row.money_flash_risk
        and not late_chase
    )

    # 저점방어/눌림모양이 있으면 가장 우선으로 눌림 준비대기에 태운다.
    low_pullback_prepare = (
        data_3m_ok
        and data_5m_ok
        and trade_ok
        and current_alive
        and row.money24h >= 500_000_000
        and (row.low_defense_ok or row.pullback_shape_ok or (0.20 <= row.from_low30 <= 2.70 and row.high_gap30 >= 0.35))
        and row.ch3 >= -0.10
        and row.ch5 >= -0.15
        and not row.tick_noise
        and not row.money_flash_risk
    ...
    # 돌파 실패형은 바로 돌파로 보지 않고 눌림/재감시 쪽으로 보낸다.
    breakout_fail_prepare = (
        data_3m_ok
        and data_5m_ok
        and row.failed_peak_retest
        and trade_ok
        and current_alive
        and row.high_gap30 >= 0.20
        and not late_chase
    )

    # 강한 놓침/재감시 복귀: 이미 한 번 강하게 갔지만 지금 추격은 금지, 살아남으면 준비대기.
    strong_miss_prepare = (
        data_3m_ok
        and data_5m_ok
        and trade_ok
        and row.range30_pct >= 1.2
        and row.from_low30 <= 3.2
        and row.high_gap30 >= 0.35
        and current_alive
        and not weak_now
        and not late_chase
    )
    ...
    return {
        ...
        "trade_ok": trade_ok,
        "trade_strong": trade_strong,
        "current_alive": current_alive,
        "current_bad": current_bad,
        "weak_now": weak_now,
        "common_confirm": common_confirm,
        "direct_to_prepare": direct_to_prepare,
        "low_pullback_prepare": low_pullback_prepare,
        "breakout_fail_prepare": breakout_fail_prepare,
        "strong_miss_prepare": strong_miss_prepare,
        "prepare_any": direct_to_prepare or low_pullback_prepare or breakout_fail_prepare or strong_miss_prepare,
    }


def v315_prepare_reason(ctx: dict) -> str:
    reasons = []
    if ctx.get("low_pullback_prepare"):
        reasons.append("저점방어/눌림 준비")
    if ctx.get("direct_to_prepare"):
        reasons.append("바로진입 보류")
    if ctx.get("breakout_fail_prepare"):
        reasons.append("돌파실패 후 눌림")
    if ctx.get("strong_miss_prepare"):
        reasons.append("강한놓침 재감시복귀")
    if not reasons:
        reasons.append("본선 준비")
    return " / ".join(reasons[:3])


def v317_pullback_risk_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool) -> dict:
    """
    v2.7.22: v2.5.3xx 후기판의 위험진입 차단 2차 이식.
    목적은 눌림 검증에 GRND/PRL류 급락·고점권 식음·현재봉 밀림 후보가 섞이는 것을 줄이는 것.
    """
    current_reject = (
        row.own1m_state in ("밀림", "윗꼬리")
        or one_min_milrim
        or row.current_milrim
        or row.own1m_upper_wick_pct >= 0.35
        or row.ch1 <= -0.35
    )
    late_cooling_risk = (
        data_3m_ok and data_5m_ok
        and (row.ch3 >= 1.20 or row.ch5 >= 1.20)
        and row.ch1 <= 0.05
        and (late_chase or row.from_low30 >= 2.0 or row.high_gap30 <= 0.45)
    )
    weak_money_pullback = (
        not recent_money_ok
        and row.ws_trade_count_60 <= 2
        and row.ws_trade_amount_60 < 1_500_000
        and not row.money_sustain_ok
    )
    bad_low_break = (
        row.low30 > 0
        and row.price <= row.low30 * 1.002
        and row.ch1 <= -0.10
    )
    pullback_like = (
        row.pullback_shape_ok
        or row.low_defense_ok
        or (0.20 <= row.from_low30 <= 3.10 and row.high_gap30 >= 0.20)
    )
    dirty_pullback = bool(pullback_like and (current_reject or late_cooling_risk or weak_money_pullback or bad_low_break))
    return {
        "current_reject": current_reject,
        "late_cooling_risk": late_cooling_risk,
        "weak_money_pullback": weak_money_pullback,
        "bad_low_break": bad_low_break,
        "dirty_pullback": dirty_pullback,
    }


def v317_pullback_risk_reason(ctx: dict) -> str:
    reasons = []
    if ctx.get("current_reject"):
        reasons.append("현재봉 밀림/윗꼬리")
    if ctx.get("late_cooling_risk"):
        reasons.append("고점권 식는 자리")
    if ctx.get("weak_money_pullback"):
        reasons.append("돈/체결 약함")
    if ctx.get("bad_low_break"):
        reasons.append("저점 이탈 의심")
    return " / ".join(reasons) if reasons else "위험눌림"



def v318_m2_pullback_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool, recent_money_strong: bool, own_supporting: bool, own_recovery_strong: bool, v315_ctx: dict, v317_ctx: dict) -> dict:
    """
    v2.7.22: v2.5.3xx/v2.6 눌림 재확인 라인 2차 이식.
    새 전략이 아니라 M2 눌림 재확인의 실제 조건을 더 분명히 나눈다.
    핵심: 상승→눌림→저점방어/저점높임→1분 재회복→돈 유지.
    """
    trend_count = sum(1 for x in (row.ch1, row.ch3, row.ch5) if x >= 0.08)
    current_reclaim = (
        row.own1m_state == "회복"
        or own_recovery_strong
        or (row.own1m_state == "보합" and row.ch1 >= -0.05 and not one_min_milrim)
    )
    chart_low_struct = (
        row.low_defense_ok
        or (row.pullback_shape_ok and row.pullback_rebound_pct >= 0.18)
        or bool(v315_ctx.get("low_pullback_prepare"))
        or (0.28 <= row.from_low30 <= 2.95 and row.high_gap30 >= 0.28 and row.range30_pct >= 0.75)
    )
    low_higher_like = (
        row.low_defense_ok
        or (row.pullback_rebound_pct >= 0.32 and row.from_low30 >= 0.25)
        or (row.own1m_lower_wick_pct >= 0.16 and row.own1m_state in ("회복", "보합"))
    )
    money_held = (
        row.money_sustain_ok
        and (
            recent_money_ok
            or row.ws_trade_amount_60 >= 1_500_000
            or row.ws_trade_count_60 >= 4
            or row.money1m >= 3_000_000
            or row.money3m >= 10_000_000
            or bool(v315_ctx.get("trade_ok"))
        )
    )
    money_reconfirm = (
        row.money_sustain_ok
        and (
            recent_money_strong
            or row.ws_trade_amount_60 >= 5_000_000
            or row.ws_trade_count_60 >= 8
            or row.money1m >= 8_000_000
            or row.money3m >= 25_000_000
            or bool(v315_ctx.get("trade_strong"))
        )
    )
    danger = (
        bool(v317_ctx.get("dirty_pullback"))
        or bool(v315_ctx.get("weak_now"))
        or one_min_milrim
        or row.money_flash_risk
        or row.tick_noise
        or row.own1m_upper_wick_pct >= 0.48
        or row.ch1 <= -0.45
    )
    too_late = (
        late_chase
        or (row.from_low30 >= 3.60 and row.high_gap30 <= 0.30)
        or (row.range30_pct >= 8.0 and row.high_gap30 <= 0.55)
    )
    m2_candidate = (
        data_3m_ok and data_5m_ok
        and row.money24h >= 500_000_000
        and chart_low_struct
        and trend_count >= 2
        and row.ch3 >= -0.18
        and row.ch5 >= -0.25
        and money_held
    ...
    m2_reconfirm = (
        ...
        and money_reconfirm
        and row.ch3 >= 0.00
        and row.high_gap30 >= 0.22
    )
    m2_prepare = (
        m2_candidate
        and not m2_reconfirm
        and (current_reclaim or row.low_defense_ok or row.pullback_shape_ok)
    )
    return {
        "trend_count": trend_count,
        "current_reclaim": current_reclaim,
        "chart_low_struct": chart_low_struct,
        "low_higher_like": low_higher_like,
        "money_held": money_held,
        "money_reconfirm": money_reconfirm,
        "danger": danger,
        "too_late": too_late,
        "m2_candidate": m2_candidate,
        "m2_prepare": m2_prepare,
        "m2_reconfirm": m2_reconfirm,
    }


def v318_m2_reason(ctx: dict) -> str:
    reasons = []
    if ctx.get("chart_low_struct"):
        reasons.append("눌림/저점방어 구조")
    if ctx.get("low_higher_like"):
        reasons.append("저점방어·저점높임")
    if ctx.get("current_reclaim"):
        reasons.append("1분 재회복")
    if ctx.get("money_held"):
        reasons.append("돈 유지")
    if ctx.get("money_reconfirm"):
        reasons.append("체결 재확인")
    if not reasons:
        reasons.append("M2 눌림 확인중")
    return " / ".join(reasons[:4])


def m2_realtime_gate(row: MarketRow) -> tuple[bool, str]:
    """v2.7.22: 조건잠금. M2 실시간자료 상태는 진단만 하고 후보 강등 조건으로 쓰지 않는다."""
    q = str(row.data_quality or "")
    price_ok = q in ("WS_OK", "WS_PRICE_OK", "WS_PRICE_ONLY") and row.ws_price_age_sec <= 25
    candle_ok = bool(row.own1m_ok) and row.own1m_state not in ("정보없음",)
    trade_some = (row.ws_trade_count_60 >= 2 and row.ws_trade_amount_60 >= 500_000) or row.money1m >= 2_000_000
    miss = []
    if not price_ok:
        miss.append("가격 최신성")
    if not candle_ok:
    ...
    if not trade_some:
        miss.append("최근 체결")
    if not miss:
        return True, "실시간 M2 자료 정상"
    # 조건잠금: 여기서 False를 반환하면 후보 상태를 감시로 강등하는 조건 변경이 된다.
    return True, "진단전용: M2 자료 보강 필요(" + "/".join(miss) + ")"
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    v2.7.22 추가:
    - 예전 승률 좋던 눌림/준비대기 본선 라인 1차 이식
    - 눌림준비: 저점방어/저점높임/1분 재회복/3분 유지/돈 유지 기준을 별도 라우트로 분리
    - 급등초입 후 식는 후보와 진짜 눌림 후보를 paper 통계에서 분리
    - 약한 체결/제자리 1분봉 후보는 본선감시로 유지
    """
    ...
    # v2.7.22 본선 공장 재료.
    # 후보 수를 억지로 줄이는 게 아니라, 눌림/초입 본선에 필요한 최소 재료를 먼저 본다.
    live_trade_alive = (row.ws_trade_count_60 >= 3 and row.ws_trade_amount_60 >= 1_000_000) or row.money1m >= 4_000_000
    live_trade_strong = (row.ws_trade_count_60 >= 6 and row.ws_trade_amount_60 >= 8_000_000) or row.money1m >= 12_000_000
    own_supporting = row.own1m_state in ("회복", "보합") and not own_bad
    own_recovery_strong = row.own1m_state == "회복" and row.own1m_body_pct >= 0.03
    mainline_money_ok = row.money_sustain_ok and (recent_money_ok or live_trade_alive or row.money3m >= 15_000_000)
    mainline_money_strong = row.money_sustain_ok and (recent_money_strong or live_trade_strong or row.money3m >= 35_000_000)
    pullback_mainline_ready = (
        money_ok and data_3m_ok and mainline_money_ok
        and (row.pullback_shape_ok or row.low_defense_ok)
        and (row.low_defense_ok or row.pullback_rebound_pct >= 0.35)
        and own_supporting
        and not one_min_milrim
        and not row.money_flash_risk
    )
    start_mainline_ready = (
        money_ok and data_3m_ok and mainline_money_ok
        and row.ch1 >= 0.02 and row.ch3 >= 0.15
        and row.from_low30 <= 1.85
        and own_supporting
        and not late_chase
        and not one_min_milrim
        and not row.money_flash_risk
    ...
    warmup_strong_start = (
        (not data_3m_ok) and money_ok and data_green_ok
        and row.ch1 >= 0.35
        and live_trade_strong
        and own_recovery_strong
        and not row.money_flash_risk
        and not row.tick_noise
    )

    # v2.7.22: v2.5.3xx 본선 기준 이식 컨텍스트.
    # 바로진입 위험→준비/눌림대기, 위험진입 차단, 강한놓침 재감시복귀를 clean core row 기준으로 해석한다.
    v315_ctx = v315_mainline_context(
        row, data_3m_ok=data_3m_ok, data_5m_ok=data_5m_ok,
        one_min_milrim=one_min_milrim, late_chase=late_chase,
    )
    v315_prepare_wait = bool(v315_ctx.get("prepare_any"))
    v317_ctx = v317_pullback_risk_context(
        row, data_3m_ok=data_3m_ok, data_5m_ok=data_5m_ok,
        one_min_milrim=one_min_milrim, late_chase=late_chase, recent_money_ok=recent_money_ok,
    )
    v317_dirty_pullback = bool(v317_ctx.get("dirty_pullback"))
    v318_ctx = v318_m2_pullback_context(
        row, data_3m_ok=data_3m_ok, data_5m_ok=data_5m_ok,
        one_min_milrim=one_min_milrim, late_chase=late_chase,
        recent_money_ok=recent_money_ok, recent_money_strong=recent_money_strong,
        own_supporting=own_supporting, own_recovery_strong=own_recovery_strong,
        v315_ctx=v315_ctx, v317_ctx=v317_ctx,
    )
    m2_rt_ok, m2_rt_reason = m2_realtime_gate(row)

    # v2.7.22: 예전 승률 좋던 M2 눌림/준비대기 2차 복구.
    # 핵심은 급등 추격이 아니라, 눌린 뒤 저점이 무너지지 않고
    # 1분 재회복/3분 유지/돈 유지가 붙는 구간을 따로 살리는 것이다.
    pullback_prep_base = (
        money_ok
        and data_3m_ok
        and data_5m_ok
        and mainline_money_ok
        and own_supporting
        and not one_min_milrim
        and not row.money_flash_risk
        and not row.tick_noise
        and not v317_dirty_pullback
        and row.from_low30 <= 3.15
        and row.high_gap30 >= 0.22
        and row.range30_pct <= 6.8
        and -0.38 <= row.ch1 <= 0.82
        and -0.20 <= row.ch3 <= 1.65
        and row.ch5 >= -0.25
        and (
            row.low_defense_ok
            or (row.pullback_shape_ok and row.pullback_rebound_pct >= 0.18)
            or v315_prepare_wait
            or bool(v318_ctx.get("m2_candidate"))
        )
    )
    # v2.7.22에서는 v318 M2 컨텍스트를 우선한다. 기존 v315/v317 게이트는 보조 안전장치로 남긴다.
    pullback_prep_wait = (
        (pullback_prep_base or bool(v318_ctx.get("m2_prepare")))
        and (recent_money_ok or live_trade_alive or bool(v315_ctx.get("trade_ok")) or bool(v318_ctx.get("money_held")))
        and not bool(v315_ctx.get("weak_now"))
        and not v317_dirty_pullback
        and not bool(v318_ctx.get("danger"))
    )
    pullback_prep_reconfirm_wait = (
        (pullback_prep_base or bool(v318_ctx.get("m2_reconfirm")))
        and (one_min_recovered or own_recovery_strong or bool(v315_ctx.get("current_alive")) or bool(v318_ctx.get("current_reclaim")))
        and (mainline_money_strong or bool(v315_ctx.get("trade_strong")) or bool(v318_ctx.get("money_reconfirm")))
        and row.ch3 >= -0.02
        and not bool(v315_ctx.get("weak_now"))
        and not v317_dirty_pullback
        and not bool(v318_ctx.get("danger"))
    ...
    restart_wait = (
        ...
        and data_3m_ok
        and (recent_money_ok or live_trade_alive)
    ...
    # ZIL류: 초입 급등은 맞았지만 1분이 식는 구간.
    # 이걸 눌림대기로 보내면 paper 눌림 통계가 망가지므로 별도 본선 라우트로 분리한다.
    surge_cooldown_wait = (
        money_ok
        and data_3m_ok
        and data_5m_ok
        and (row.ch3 >= 1.25 or row.ch5 >= 1.20)
        and row.ch5 >= 0.75
        and (recent_money_ok or live_trade_alive or row.money_sustain_ok)
        and (late_chase or row.from_low30 >= 1.75 or row.high_gap30 <= 2.20)
        and (not one_min_recovered or one_min_milrim or row.own1m_state in ("보합", "밀림", "윗꼬리"))
        and not row.tick_noise
        and not row.money_flash_risk
    )

    # BOBA류: 이름은 재출발이지만 최근 체결/돈이 너무 약하면 노랑이 아니라 재출발감시다.
    # v2.7.22에서는 일반 약한 움직임 전체가 재출발감시로 빨려 들어가지 않게,
    # 기존 재출발 상태/고점권 재시도/급등흔적이 있는 경우로 좁힌다.
    restart_like_context = (
        old_pattern == "RESTART_WAIT"
        or row.recent_spike_near_high
        or row.box_top_chase
        or row.overheat_watch
        or (row.high_gap30 <= 1.20 and row.from_low30 >= 1.20)
    )
    weak_restart_flow = (
        data_3m_ok
        and restart_like_context
        and (weak_live_flow or not recent_money_ok)
        and not live_trade_alive
        and row.ws_trade_amount_60 < 2_000_000
        and not v315_prepare_wait
    )
    ...
    if not data_3m_ok:
        if warmup_strong_start:
            return make_candidate(
                row, "PENDING", "보조 강한초입", "🟡",
                "3분 자료 전이지만 최근 체결·1분봉 회복이 강해 본선 감시",
                pattern="START_EARLY_WAIT", caution="초기자료 강함",
                entry_note="3분 자료가 쌓인 뒤 상승초입 본선으로 재평가",
                reject_reason="3분 기준 부족",
                block_reasons=["3분 기준 부족", "체결 강함", "1분봉 회복"]
            )
        if money_ok and row.ch1 >= 0.20 and not row.money_flash_risk and (live_trade_alive or own_recovery_strong):
            return make_candidate(
                row, "WATCH", "초기강세 감시", "⚪",
                "1분 움직임은 있으나 3분 자료 전이라 대기보다 감시가 맞음",
                pattern="EARLY_WATCH", caution="초기자료",
                entry_note="3분 흐름·최근 체결 유지 확인 후 대기 후보로 승격",
                reject_reason="3분 기준 부족",
                block_reasons=["3분 기준 부족", "본선 확인 전"]
            )
        if row.ch1 > 0.05:
    ...
    # 과열/추격이라고 전부 버리지 않는다. 다만 ZIL류 급등초입 후 식는 후보는 눌림이 아니라 상승후 재확인으로 분리한다.
    if surge_cooldown_wait:
        return make_candidate(
            row, "PENDING", "보조 상승후 재확인", "🟡",
            "초입 급등 뒤 1분이 식는 중. 바로 추격보다 유지 확인 필요",
            pattern="SURGE_COOLDOWN_WAIT", caution="상승 후 재확인",
            entry_note="5분 유지·1분봉 재회복·최근 체결 유지 확인",
            reject_reason="상승 후 유지 확인 전",
            block_reasons=["상승 후 재확인", "추격 확인 필요"]
        )

    # v2.7.22: v2.5.3xx 본선 이식.
    # 힘은 있지만 바로진입 위험이 있는 후보는 재출발감시로 묻기 전에 준비/눌림대기로 먼저 보낸다.
    if pullback_prep_reconfirm_wait:
        if not m2_rt_ok:
            return make_candidate(
                row, "WATCH", "M2 정밀스캔 대기", "⚪",
                "M2 구조는 보이나 가격·체결·1분봉 최신성이 부족해 3차 정밀스캔 대기",
                pattern="PULLBACK_PREP_WAIT", caution=m2_rt_reason,
                entry_note="실시간 가격·체결·봇1분봉이 붙으면 눌림 재확인으로 승격",
                reject_reason="M2 실시간자료 부족",
                block_reasons=["M2 정밀스캔 대기", m2_rt_reason, v318_m2_reason(v318_ctx)]
            )
        caution = []
        ...
        if not recent_money_strong and not bool(v315_ctx.get("trade_strong")):
            caution.append("돈 유지 추가확인")
        return make_candidate(
            row, "PENDING", "눌림 재확인대기", "🟡",
            "v2.5.3xx/v2.6 M2 이식: 저점방어·1분 재회복·돈 유지가 맞아 눌림 재확인 후보",
            pattern="PULLBACK_CONFIRM_WAIT", caution=" / ".join(caution),
            entry_note="1분봉 유지·3분/5분 유지·최근 체결 유지 확인",
            reject_reason="눌림 재확인 전",
            block_reasons=["M2 본선", "눌림 재확인", v318_m2_reason(v318_ctx), v315_prepare_reason(v315_ctx)]
        )

    if pullback_prep_wait:
        if not m2_rt_ok:
            return make_candidate(
                row, "WATCH", "M2 정밀스캔 대기", "⚪",
                "M2 눌림 준비 구조는 보이나 가격·체결·1분봉 최신성이 부족해 대기보다 감시가 맞음",
                pattern="PULLBACK_PREP_WAIT", caution=m2_rt_reason,
                entry_note="실시간 가격·체결·봇1분봉이 붙으면 눌림 준비대기로 승격",
                reject_reason="M2 실시간자료 부족",
                block_reasons=["M2 정밀스캔 대기", m2_rt_reason, v318_m2_reason(v318_ctx)]
            )
        caution = []
        ...
        if not one_min_recovered:
            caution.append("1분 재회복 확인")
        if not recent_money_strong and not bool(v315_ctx.get("trade_strong")):
            caution.append("돈 유지 확인")
        return make_candidate(
            row, "PENDING", "눌림 준비대기", "🟡",
            "v2.5.3xx/v2.6 M2 이식: 바로진입보다 눌림 확인이 맞는 준비대기 후보",
            pattern="PULLBACK_PREP_WAIT", caution=" / ".join(caution),
            entry_note="1분 재회복·3분/5분 유지·돈 유지가 붙으면 눌림 재확인으로 승격",
            reject_reason="눌림 준비 확인 전",
            block_reasons=["M2 본선", "눌림 준비대기", v318_m2_reason(v318_ctx), v315_prepare_reason(v315_ctx)]
        )

    if weak_restart_flow:
        return make_candidate(
            row, "WATCH", "재출발감시", "⚪",
            "재출발처럼 보이나 최근 체결·돈 유지가 약해 대기보다 감시가 맞음",
            pattern="RESTART_WEAK_WATCH", caution="약한 재출발",
            entry_note="최근 체결과 1분봉 회복이 붙으면 재출발대기로 승격",
            reject_reason="재출발 돈/체결 부족",
            block_reasons=["재출발 약함", "최근 체결 약함", "돈 유지 확인 필요"]
        )

    if restart_wait:
        return make_candidate(
            row, "PENDING", "보조 재출발 확인", "🟡",
    ...
    # v2.7.22: 특수 대기상태 보존. TON류 재출발/재돌파/급등후재확인 후보가
    ...
    if old_pattern in SPECIAL_WAIT_PATTERNS and money_watch and data_1m_ok and not row.tick_noise and not row.money_flash_risk:
        broken = (
            (row.ch1 <= -0.45)
            or (row.from_low30 >= 6.5 and row.high_gap30 <= 0.25)
            or (row.ws_trade_count_60 == 0 and row.data_quality in ("REST_ONLY", "WS_SUBSCRIBED_NO_DATA"))
            or (old_pattern == "RESTART_WAIT" and weak_restart_flow)
            or (old_pattern == "SURGE_COOLDOWN_WAIT" and row.ch1 <= -0.35 and not recent_money_ok)
        )
        if not broken:
    ...
    if row.failed_peak_retest:
        return make_candidate(row, "PENDING", "보조 재돌파 확인", "🟡", "고점 찍고 밀린 뒤 재도전 중이라 유지 확인 필요", pattern="REBREAK_WAIT", caution="고점 재돌파 확인", entry_note="재돌파 후 1분봉 유지 확인", reject_reason="재돌파 확인 전")
    ...
    if v317_dirty_pullback and (row.pullback_shape_ok or row.low_defense_ok or row.from_low30 <= 3.2):
        return make_candidate(
            row, "WATCH", "위험눌림 감시", "⚪",
            "눌림처럼 보이나 위험진입 조건이 있어 눌림 검증에서 제외",
            pattern="PULLBACK_RISK_WATCH", caution=v317_pullback_risk_reason(v317_ctx),
            entry_note="현재봉 회복·저점방어·돈 유지가 다시 붙으면 눌림 준비로 재평가",
            reject_reason="위험눌림 제외",
            block_reasons=["M2 위험눌림 제외", v317_pullback_risk_reason(v317_ctx)]
        )
    ...
    if pullback_prep_reconfirm_wait:
        ...
        return make_candidate(
            row, "PENDING", "눌림 재확인대기", "🟡",
            "M2 눌림 재확인: 저점방어 후 1분 재회복과 돈 유지가 붙는 준비대기 라인",
            pattern="PULLBACK_CONFIRM_WAIT", caution=" / ".join(caution),
            entry_note="1분봉 유지·3분 유지·최근 체결 유지 확인",
            reject_reason="눌림 재확인 전",
            block_reasons=["M2 본선", "눌림 준비대기", "재확인 필요", v318_m2_reason(v318_ctx)]
        )
    ...
    if pullback_prep_wait:
        ...
        return make_candidate(
            row, "PENDING", "눌림 준비대기", "🟡",
            "M2 눌림 준비: 저점은 버티고 있으나 아직 진입 전 확인이 필요한 준비대기",
            pattern="PULLBACK_PREP_WAIT", caution=" / ".join(caution),
            entry_note="1분 재회복·3분 유지·돈 유지가 붙으면 눌림 재확인으로 승격",
            reject_reason="눌림 준비 확인 전",
            block_reasons=["M2 본선", "눌림 준비대기", "재확인 필요", v318_m2_reason(v318_ctx)]
        )
    ...
    if start_early or start_break_hold:
        ...
        if not strong_green:
            if not weak_but_candidate:
                caution.append("최근 체결/1분봉 약함")
                return make_candidate(
                    row, "WATCH", "초입감시", "⚪",
                    "초입 방향은 보이나 본선 체결·1분봉 근거가 아직 약함",
                    pattern="MAINLINE_WATCH", caution=" / ".join(caution),
                    entry_note="최근 체결과 1분봉 회복이 붙으면 초입대기로 승격",
                    reject_reason="본선 근거 부족",
                    block_reasons=["본선 체결 부족", "1분봉 확인 부족"]
                )
            if not recent_money_strong:
                caution.append("최근 거래 약함")
            label = "보조 약한초입"
    ...
    if candidate_like:
        mainline_wait_ok = (
            pullback_mainline_ready
            or start_mainline_ready
            or (row.low_defense_ok and own_supporting and mainline_money_ok)
            or (three_min_alive and recent_money_ok and own_supporting and not late_chase)
        )
        if not mainline_wait_ok:
            return make_candidate(
                row, "WATCH", "본선감시", "⚪",
                "움직임은 있으나 눌림/초입 본선 근거가 아직 부족",
                pattern="MAINLINE_WATCH", caution="본선 확인 전",
                entry_note="저점방어·1분회복·돈유지 중 본선 재료가 붙으면 대기로 승격",
                reject_reason="본선 근거 부족",
                block_reasons=["본선 근거 부족", "재확인 필요"]
            )
        ...
        if surge_cooldown_wait:
            pattern = "SURGE_COOLDOWN_WAIT"
            label = "보조 상승후 재확인"
        elif pullback_prep_wait:
            pattern = "PULLBACK_PREP_WAIT"
            label = "눌림 준비대기"
        elif row.pullback_shape_ok or row.low_defense_ok:
            pattern = "PULLBACK_WAIT"
            label = "눌림대기"
        elif start_mainline_ready or (three_min_alive and recent_money_ok and own_supporting):
            pattern = "START_EARLY_WAIT"
            label = "보조 초입 확인"
        else:
            pattern = "MAINLINE_WATCH"
            label = "본선감시"
            return make_candidate(row, "WATCH", label, "⚪", "움직임은 있으나 눌림/초입 본선 분류 전", pattern=pattern, caution="본선 확인 전", entry_note="3분 흐름·돈 유지·1분봉 회복 확인", reject_reason="본선 분류 전", block_reasons=["본선 분류 전"])
...
def pattern_kor(pattern: str) -> str:
    table = {
        ...
        "PULLBACK_PREP_WAIT": "눌림 준비대기",
        "PULLBACK_WAIT": "눌림대기",
        "START_EARLY": "상승시작 초입",
        "START_EARLY_BREAK_HOLD": "상승시작 초입(돌파유지)",
        "START_EARLY_WAIT": "보조 초입 확인",
        "START_EARLY_WEAK_WAIT": "보조 약한초입",
        "SURGE_COOLDOWN_WAIT": "보조 상승후 재확인",
        "PULLBACK_CONFIRM_WAIT": "눌림 재확인대기",
        "RESTART_WAIT": "보조 재출발 확인",
        "SURGE_RECHECK_WAIT": "보조 급등후 확인",
        "REBREAK_WAIT": "보조 재돌파 확인",
        "OVERHEAT_WATCH": "과열감시",
        "TICK_NOISE_WATCH": "틱노이즈 감시",
        "FLASH_MONEY_WATCH": "반짝돈 감시",
        "CHASE_RISK_WATCH": "추격주의 감시",
        "MONEY_WAIT_WATCH": "돈유지 감시",
        "RESTART_WEAK_WATCH": "재출발감시",
        "WEAK_PENDING_WATCH": "약한대기 감시",
        "PULLBACK_RISK_WATCH": "위험눌림 감시",
        "MAINLINE_WATCH": "본선감시",
        "EARLY_WATCH": "초기감시",
        "WARMUP": "자료쌓는중",
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.22: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.22는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.22\n"
        f"- v2.7.22 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.22에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.22\n"
...
def render_health_check() -> str:
    ...
    lines = [
        ...
        f"- 봇 1분봉: 활성 {st.get('candle_live',0)}/{st.get('candle_total',0)}",
        f"- 스캔단계: 1차 {st.get('scan_stage1_count',0)} / 2차 {st.get('scan_stage2_count',0)} / 3차M2 {st.get('scan_stage3_count',0)} / 우선감시 {st.get('scan_ws_priority_count',0)}",
        f"- M2전용직원: 가격 {st.get('price_m2_status','-')} {st.get('price_m2_codes',0)}개 / 체결 {st.get('trade_m2_status','-')} {st.get('trade_m2_codes',0)}개",
        f"- 구독/수신 원인: {st.get('scan_root_cause','-')}",
        f"- M2자료: 대상 {st.get('scan_m2_target_count',0)} / 정상 {st.get('scan_m2_all_ok',0)} / 가격대기 {st.get('scan_m2_price_wait',0)} / 체결대기 {st.get('scan_m2_trade_wait',0)} / 1분봉대기 {st.get('scan_m2_candle_wait',0)}",
        "",
        "2) 후보 품질/차단",
...
def _paper_group_stats(rows: list[PaperCandidateRow]) -> list[tuple[str, dict]]:
    ...
    for strategy, arr in groups.items():
        ...
        gross_win10 = sum(max(0.0, r.result10_pct) for r in done10)
        gross_loss10 = abs(sum(min(0.0, r.result10_pct) for r in done10))
        pf10 = (gross_win10 / gross_loss10) if gross_loss10 > 0 else (999.0 if gross_win10 > 0 else 0.0)
        out.append((strategy, {
            "total": len(arr),
            "active": len(active),
            "done10": len(done10),
            ...
            "worst_run": worst_run,
            "pf10": pf10,
            "gross_win10": gross_win10,
            "gross_loss10": gross_loss10,
        }))
    out.sort(key=lambda x: (x[1]["done10"], x[1]["avg10"], x[1]["total"]), reverse=True)
    return out


def _paper_main_group_stats(rows: list[PaperCandidateRow]) -> list[tuple[str, dict]]:
    groups = defaultdict(list)
    for r in rows:
        groups[paper_main_strategy_name(r.pattern, r.strategy)].append(r)
    order = {"M2 눌림 재확인": 0, "M1 저점선점 보조": 1, "보조태그": 2}
    out = []
    for name, arr in groups.items():
        ...
        pf10 = (gross_win10 / gross_loss10) if gross_loss10 > 0 else (999.0 if gross_win10 > 0 else 0.0)
        out.append((name, {
            "total": len(arr), "active": len(active), "done10": len(done10), "done30": len(done30),
            "avg10": avg10, "avg30": avg30, "win10": win10, "win30": win30,
            "best10": best10, "worst10": worst10, "pf10": pf10,
        }))
    out.sort(key=lambda x: order.get(x[0], 99))
    return out


def unique_paper_rows_by_ticker(rows: list[PaperCandidateRow], *, reverse: bool = True, key_attr: str = "result10_pct", limit: int = 6) -> list[PaperCandidateRow]:
    """paper_flow TOP에서 같은 코인이 여러 전략으로 도배되는 것을 줄인다."""
    sorted_rows = sorted(rows, key=lambda r: getattr(r, key_attr, 0.0), reverse=reverse)
    out = []
    seen = set()
...
def render_paper_stats() -> str:
    ...
    main_groups = _paper_main_group_stats(rows)
    ...
    lines = [
        ...
        f"- 서버: {runtime_resource_text()}",
        "",
        "메인전략",
        "- M2 눌림 재확인: 우선 복원 대상. S1/S2만 메인으로 본다",
        "- M1 저점선점: 현재는 보조/선행감지. 성과가 안 나왔던 라인이라 메인 아님",
        "- S3~S8: 폐기 아님. M2 판단을 돕는 보조태그",
        "",
        "1) 메인전략 10분 성과",
    ]
    if not main_groups:
        ...
        for strategy, g in main_groups:
            win10 = (g["win10"] / g["done10"] * 100.0) if g["done10"] else 0.0
            pf_txt = "∞" if g.get("pf10", 0) >= 999 else f"{g.get('pf10', 0):.2f}"
            lines.append(
                f"- {strategy}: 완료 {g['done10']} / 진행 {g['active']} / 승률 {win10:.0f}% / "
                f"평균 {g['avg10']:+.2f}% / PF {pf_txt} / 최고 {g['best10']:+.2f}% / 최저 {g['worst10']:+.2f}%"
            )
    lines += ["", "2) 세부태그 S1~S8 10분 성과"]
    if not groups:
        lines.append("- 아직 세부태그 없음")
    else:
    ...
    for strategy, g in main_groups:
        ...
        lines.append(f"- {strategy}: 완료 {g['done30']} / 평균 {g['avg30']:+.2f}% / 승률 {win30:.0f}%")
    if not has30:
        lines.append("- 아직 30분 완료 없음. 계속 0이면 30분 저장 경로 점검 필요")
    lines += ["", "4) 세부태그 안내"]
    lines.extend([f"- {x}" for x in paper_strategy_legend()[:6]])
    lines += ["", "5) 판단용 요약"]
    if done10:
        ...
        best = unique_paper_rows_by_ticker(done10, reverse=True, limit=3)
        worst = unique_paper_rows_by_ticker(done10, reverse=False, limit=3)
    ...
    if overdue30:
        lines.append("- ⚠️ 30분 지났는데 미완료 후보가 있음. v2.7.22에서 paper worker/저장 조건을 봐야 함")
    return "\n".join(lines)
...
def render_paper_flow() -> str:
    ...
    best10 = unique_paper_rows_by_ticker(done10, reverse=True, limit=6)
    worst10 = unique_paper_rows_by_ticker(done10, reverse=False, limit=6)
    watch30 = unique_paper_rows_by_ticker([r for r in rows if r.result10_done and not r.result30_done], reverse=True, key_attr="result10_pct", limit=8)
    lines = [
        "🧭 모의검증 흐름 /paper_flow",
        "- 최근 후보가 실제로 살았는지/죽었는지 보는 화면이야.",
        "- 메인은 M2 눌림 재확인. 나머지는 보조태그로 본다.",
...
def render_error_doctor_summary() -> str:
    ...
    lines = [
        ...
        f"- 직원: 시장 {age_of('market_worker')} · 가격WS {age_of('ws_price_worker')} · 체결WS {age_of('ws_trade_worker')} · M2가격 {age_of('ws_price_m2_worker')} · M2체결 {age_of('ws_trade_m2_worker')}",
...
def render_check_summary() -> str:
    """v2.7.22: 예전처럼 한 화면에서 보는 운영용 점검판. 숫자 나열보다 정상/주의/문제 중심."""
    s = snapshot()
    st = ws_hub_snapshot()
    res = runtime_resource_text()
    with STATE.lock:
        cands = list(STATE.candidates)
        stats = dict(STATE.factory_stats or {})
        hb = dict(STATE.worker_heartbeat)
        paper_rows = list(STATE.paper_candidates.values())
    status = stats.get('status', {}) if isinstance(stats.get('status', {}), dict) else {}
    block = stats.get('block_reason', {}) if isinstance(stats.get('block_reason', {}), dict) else {}
    quality = stats.get('quality', {}) if isinstance(stats.get('quality', {}), dict) else {}
    ch1_ok = sum(1 for c in cands if getattr(c, 'ch1_ready', False))
    ch3_ok = sum(1 for c in cands if getattr(c, 'ch3_ready', False))
    ch5_ok = sum(1 for c in cands if getattr(c, 'ch5_ready', False))
    ch15_ok = 0
    h1_ok = 0
    try:
        with STATE.lock:
            rows_by_ticker = dict(STATE.market_rows)
        for c in cands:
            r = rows_by_ticker.get(c.ticker)
            if r and getattr(r, 'hist_span_sec', 0) >= 900:
                ch15_ok += 1
            if r and getattr(r, 'hist_span_sec', 0) >= 3600:
                h1_ok += 1
    except Exception:
        pass

    def age_of(name: str) -> str:
        row = hb.get(name) or {}
        if not row:
            return "기록없음"
        age = int(max(0, now_ts() - float(row.get('ts') or 0)))
        stage = row.get('stage', '-')
        mark = "정상" if age <= 5 and str(stage) == 'recv' else ("주의" if age <= 30 else "지연")
        return f"{mark}({age}초/{stage})"

    def yn(ok: bool) -> str:
        return "✅" if ok else "⚠️"

    price_live = int(st.get('price_live', 0) or 0)
    price_total = max(1, int(st.get('price_total', 0) or 0))
    trade_live = int(st.get('trade_live', 0) or 0)
    trade_total = max(1, int(st.get('trade_total', 0) or 0))
    candle_live = int(st.get('candle_live', 0) or 0)
    candle_total = max(1, int(st.get('candle_total', 0) or 0))
    m2_target = int(st.get('scan_m2_target_count', 0) or 0)
    m2_ok = int(st.get('scan_m2_all_ok', 0) or 0)
    m2_ratio = (m2_ok / m2_target * 100.0) if m2_target else 0.0
    ws_ok = price_live >= max(20, int(price_total * 0.45)) and trade_live >= max(20, int(trade_total * 0.10))
    chart_ok = (ch1_ok == len(cands) and ch3_ok == len(cands) and ch5_ok == len(cands)) if cands else False

    m2_stats = {}
    try:
        for name, g in _paper_main_group_stats(paper_rows):
            if name == "M2 눌림 재확인":
                m2_stats = g
                break
    except Exception:
        m2_stats = {}
    m2_done = int(m2_stats.get('done10', 0) or 0)
    m2_active = int(m2_stats.get('active', 0) or 0)
    m2_win = (safe_float(m2_stats.get('win10', 0)) / m2_done * 100.0) if m2_done else 0.0
    m2_avg = safe_float(m2_stats.get('avg10', 0))
    pf = m2_stats.get('pf10', 0)
    pf_txt = "∞" if safe_float(pf) >= 999 else f"{safe_float(pf):.2f}"

    issue_lines = []
    if not ws_ok:
        issue_lines.append("실시간 가격/체결 활성 출렁임")
    if m2_target and m2_ratio < 50:
        issue_lines.append(f"M2 자료 정상률 낮음({m2_ok}/{m2_target})")
    if not chart_ok:
        issue_lines.append("차트 재료 일부 부족")
    if not issue_lines and not block:
        issue_lines.append("큰 문제 없음")

    lines = [
        "🧭 통합 점검 /check",
        "한줄판정",
        f"- {yn(not s.get('last_fetch_error'))} 봇 / {yn(ws_ok)} 실시간 / {yn(chart_ok)} 1·3·5분차트 / {yn(m2_ratio >= 50 or m2_target == 0)} M2자료",
        "- 조건잠금: ON — 스캔 복구 중 M2 조건/초록·노랑 조건 변경 금지",
        "",
        "1) 봇 상태",
        f"- 버전: {BOT_VERSION} / 메인루프 {age_text(s['main_loop_ts'])} / 오류 {'없음' if not s.get('last_fetch_error') else '확인필요'}",
        f"- 후보판: 🟢{status.get('BUY_READY',0)} / 🟡{status.get('PENDING',0)} / ⚪{status.get('WATCH',0)} / 서버 {res}",
        "",
        "2) 직원·실시간",
        f"- 일반 가격/체결: {age_of('ws_price_worker')} / {age_of('ws_trade_worker')}",
        f"- M2전용 가격/체결: {age_of('ws_price_m2_worker')} / {age_of('ws_trade_m2_worker')}",
        f"- 활성: 가격 {price_live}/{price_total} · 체결 {trade_live}/{trade_total} · 1분봉 {candle_live}/{candle_total}",
        f"- 원인: {st.get('scan_root_cause','-')}",
        "",
        "3) 스캔 단계",
        f"- 1차 {st.get('scan_stage1_count',0)} / 2차 {st.get('scan_stage2_count',0)} / 3차M2 {st.get('scan_stage3_count',0)} / 우선감시 {st.get('scan_ws_priority_count',0)}",
        f"- M2자료: 정상 {m2_ok}/{m2_target} / 가격대기 {st.get('scan_m2_price_wait',0)} / 체결대기 {st.get('scan_m2_trade_wait',0)} / 1분봉대기 {st.get('scan_m2_candle_wait',0)}",
        "",
        "4) 차트 재료",
        f"- 후보 기준: 1분 {ch1_ok}/{len(cands)} / 3분 {ch3_ok}/{len(cands)} / 5분 {ch5_ok}/{len(cands)} / 15분 {ch15_ok}/{len(cands)} / 1시간 {h1_ok}/{len(cands)}",
        "",
        "5) M2 paper",
        f"- 10분: 완료 {m2_done} / 진행 {m2_active} / 승률 {m2_win:.0f}% / 평균 {m2_avg:+.2f}% / PF {pf_txt}",
        "",
        "6) 지금 봐야 할 것",
    ]
    for msg in issue_lines[:3]:
        lines.append(f"- {msg}")
    if block:
        lines.append("- 막힌 이유 TOP: " + ", ".join([f"{k} {v}개" for k, v in _top_items(block, 3)]))
    lines.append("\n자세히: /data_check_full /factory_check_full /paper_flow")
    return "\n".join(lines)
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.22 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
...
def send_startup_message():
    txt = (
        ...
        f"현재 버전: {BOT_VERSION}\n\n"
        f"이번 핵심 변경\n"
        f"- 조건잠금 ON: 스캔 복구 중 M2/초록/노랑 조건은 새로 만지지 않음\n"
        f"- v2.7.19~21에서 들어간 M2 실시간자료 부족 강등 게이트를 진단 전용으로 되돌림\n"
        f"- /check를 예전식 통합 점검판에 가깝게 다시 정리\n"
        f"- 스캔/직원/허브 점검은 유지, 실거래 주문은 보호모드 유지\n\n"
        f"확인: /batch\n/ping\n/deploy_status\n/check\n/health_check\n/data_check\n/paper_stats\n/paper_flow\n/buy_check\n/error_doctor"
    )
    try:
...
def main():
    ...
    threading.Thread(target=websocket_worker, args=("price", "main"), daemon=True, name="ws_price_hub_v275").start()
    threading.Thread(target=websocket_worker, args=("trade", "main"), daemon=True, name="ws_trade_hub_v275").start()
    # v2.7.22: M2 눌림 재확인 후보만 따로 보는 전용 직원. 일반 스캔 직원이 흔들려도 M2 자료를 최대한 유지한다.
    threading.Thread(target=websocket_worker, args=("price", "m2"), daemon=True, name="ws_price_m2_hub_v2722").start()
    threading.Thread(target=websocket_worker, args=("trade", "m2"), daemon=True, name="ws_trade_m2_hub_v2722").start()
