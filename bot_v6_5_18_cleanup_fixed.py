...
def get_watch_caution_text(item: dict) -> str:
    ...
    if change_pct >= 18.0:
        return "이미 많이 오른 자리라 추격은 조심"
    if change_pct >= 10.0 and vol_ratio < 2.2:
        return "상승폭이 커서 늦은 자리일 수 있어"
    return ""
...
def format_watch_alert_line(item: dict, order: int = 1, alert_type: str = "new", reason_text: str = "") -> str:
    ...
    if reason_text:
        lines.append(f"- 다시 보는 이유: {reason_text}")
    return "\n".join(lines)
...
- /autobuy_on, /autobuy_off 복구
- status / 자동쉬기 / 수동해제 상태 표시 정리
- 후보 생성 기준 테스트용 완화 반영
- scan_debug는 빠른 조회용으로만 동작
- /info로 전략/용어 설명 확인 가능
- 연속 실패 시 자동 쉬기
...
def watch_overheat_block(df, current_price, change_3, change_5, vol_ratio):
    ...
    if change_3 >= 8.0:
        return True, f"최근 3봉 상승이 너무 커서 늦은 자리 ({change_3:.2f}%)"
    if change_5 >= 8.5 and vol_ratio < 3.2:
        return True, f"상승폭 대비 거래량이 약해서 늦은 자리 ({change_5:.2f}% / {vol_ratio:.2f}배)"
    if 0 <= dist_to_high_pct <= 0.12 and change_3 >= 2.8:
        return True, f"고점 바로 밑 추격 자리 ({dist_to_high_pct:.2f}%)"
...
def should_allow_time_renotice(item):
    ...
    if strategy == "LEADER_WATCH":
        if change_pct > 8.0:
            return False
        if edge < 6.1 and vol_ratio < 1.55 and leader_score < 2.2:
            return False
        return True

    if change_pct > 10.0:
        return False
    return edge >= 6.0 or vol_ratio >= 1.7 or leader_score >= 2.0
...
def scan_watchlist(shared_cache=None):
    ...
    for item in top:
        ...
        if alert_type == "upgrade":
            upgrade_lines.append(format_watch_alert_line(item, order=len(upgrade_lines) + 1, alert_type=alert_type, reason_text=reason_text))
        elif alert_type == "renotice":
            renotice_lines.append(format_watch_alert_line(item, order=len(renotice_lines) + 1, alert_type=alert_type, reason_text=reason_text))
        else:
            new_lines.append(format_watch_alert_line(item, order=len(new_lines) + 1, alert_type=alert_type))
...
def register_bot_commands():
    try:
        bot.set_my_commands([
            ...
            BotCommand("scan_debug", "스캔 디버그 보기"),
...
def get_scan_debug_candidates():
    try:
        ...
        if shared_market_cache and cache_age <= 180:
            ...
            note = f"최근 {cache_age}초 shared cache"
            if cache_age > 180:
                note += " (오래됨)"
            return items[:8], note
...
def build_scan_debug_text():
    ...
    for i, item in enumerate(results, 1):
        ...
        lines.append(
            f"   5분상승 {safe_float(item.get('change_5',0)):.2f}% / 거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / 주도 {safe_float(item.get('leader_score',0)):.2f}"
        )
