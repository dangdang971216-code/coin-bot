...
ENTRY_CONTEXT_KEYS = [
    ...
    "live_price_source", "live_price", "live_age_sec", "ws_fresh", "ws_turnover", "ws_volume", "current_price_ws_gap_pct",
...
def format_open_alert(pos: Dict[str, Any]) -> str:
    ...
    if isinstance(tags, list):
        tag_txt = ", ".join(str(x) for x in tags[:4]) or "-"
    else:
        tag_txt = str(tags or "-")
    final_label = pos.get("final_entry_label") or ctx.get("final_entry_label") or pos.get("trade_ready_label") or "-"
    source = pos.get("open_source") or "trade_ready"
    raw = pos.get("raw") if isinstance(pos.get("raw"), dict) else {}
    scan_id = pos.get("scan_id") or raw.get("scan_id") or ""
    return "\n".join([
        "✅ [모의매매 OPEN] 정식 후보 진입",
        f"- 코인: {short_ticker(pos.get('ticker'))}",
        f"- 진입가: {format_price(pos.get('entry_price'))}",
        f"- source: {source} / scan_id: {scan_id or '-'}",
        f"- 최종검증: {final_label}",
        ...
        f"- 태그: {tag_txt}",
        "- 참고: 실제 주문 아님 / paper_bot 모의매매 장부 기록",
    ])

def format_close_alert(closed: Dict[str, Any]) -> str:
    return "\n".join([
        "📘 [모의매매 CLOSED] 정식 후보 종료",
...
def summary_text() -> str:
    ...
    return "\n".join([
        ...
        f"- 직전 cycle 후보: 읽음 {pick.get('paper_file',0)} / OPEN통과 {status.get('opened_this_cycle',0)} / 관찰제외 {pick.get('strict_observe_skip',0)} / 복기 {pick.get('shadow_review_only_skip',0)} / 중복 {pick.get('same_ticker_skip',0)}",
...
def command_response(text: str) -> str:
    ...
    if cmd in {"/phelp", "/start"}:
        return "\n".join([
            f"🧪 {VERSION}",
            "명령어: /pbatch /pstatus /pcheck /ponce /pstart /pstop /prestart /plog /perror /pscore /pversion_score /popen /pfiles /pcontrol",
