...
def compact_candidate_file(path: Path, keep_lines: int = CANDIDATE_FILE_KEEP_LINES) -> None:
    ...
    v164: 파일 보존은 유지하되, 공장 단계가 5초 이상 막히지 않도록 시간 간격을 둔다.
    """
    try:
...
def append_jsonl_many(path: Path, rows: List[Dict[str, Any]]) -> Tuple[bool, str]:
    ...
    v164: 공장 단계 병목 원인이던 후보별 ensure/open/write 반복을 제거한다.
    """
    if not rows:
        return True, "empty"
    ok, note = ensure_candidate_file(path)
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.164: 고정 갯수 절단 없음. paper_bot은 latest scan batch만 소비해 과거 TTL 후보 몰아먹기 방지.",
    }
...
def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    result = {
        ...
        "write_error": "",
        "last_ticker": "-",
        "factory_mode": "v164_batch_append_shadow_review",
    }
    # v164: 압축은 시간 간격을 두고 수행한다. 후보별 파일 open/write 반복 금지.
    ...
    for lane, items, path in [("strict", strict, FILES["paper"]), ("shadow", shadow, FILES["shadow"] )]:
        ...
        for item in items:
            ...
            if lane == "shadow":
                # shadow는 복기 전용이다. paper_bot v0.24 기본값에서는 OPEN하지 않는다.
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.164 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.164.py",
        "- 페이퍼봇 대상: paper_bot_v0.24.py",
...
def command_paper_handoff(update, context) -> None:
    lines = [
        ...
        "- 기대 paper_bot: paper_bot_v0.24.py / paper_bot_v0.21",
        "",
        "역할",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.164.py", "- 페이퍼봇 대상: paper_bot_v0.24.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.164: 비상 안정화 + 직원 분리 유지 + 웹소켓 기본 OFF", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot 과거 TTL 몰아먹기 방지"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.164 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v150 수치 이식 + 직원 분리 + shadow 복기전용 + 공장 배치쓰기 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
