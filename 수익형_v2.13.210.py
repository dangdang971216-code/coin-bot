...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.210.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def start_background_workers() -> None:
    ...
    log(f"background_workers started precision={PRECISION_BACKGROUND_WORKERS} execution_risk={EXEC_RISK_BACKGROUND_WORKERS} command_cache=v210 websocket=external_sidecar_cache requested={WS_HUB_REQUESTED}")
...
# ===============================
# v2.13.210: 캐시 생성 저부하 수술 + 스캔 병목 복구
...
def _v210_quality_text() -> str:
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v210 기본은 캐시 전용: 명령어는 캐시만 읽고, 요약직원은 scan 중 무거운 계산을 건너뜁니다.",
...
def server_resource_snapshot() -> Dict[str, Any]:  # type: ignore[override]
    ...
    return {
        ...
        "pids": pids,
        "rss": rss,
        "note": "v210 resource monitor cache; commands read cache only",
    }
...
def command_cache_worker_loop() -> None:  # type: ignore[override]
    log("v210 command_cache_worker started")
    try:
        _write_resource_status()
    except Exception:
        pass
    while not _stop_event.is_set():
        ...
        _stop_event.wait(2.0)
    log("v210 command_cache_worker stopped")
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v210 캐시생성 저부하 + 스캔병목 복구 + CPU% 안정화(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
