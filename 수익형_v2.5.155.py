...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 내부 스캔 생산자 / 텔레그램 표시 소비자 분리",
        "- /status, /scan_debug, /review_dump는 새 스캔을 돌리지 않고 내부에서 저장한 최신 스냅샷만 보여줌",
        "- 시장 캐시가 잠깐 늦어도 기존 후보 스냅샷을 바로 지우지 않고, 내부 스캔은 계속 새 후보를 갱신",
        "- stale 상태는 숨김보다 '마지막 완성 후보 기준'으로 명확히 표시",
        "- 오류 추적 유지: runtime_error_log.txt / errorlog 로 최근 런타임 오류 확인 가능",
        "",
    ]
...
def maybe_send_missed_review_alert(row: dict):
    ...
    lines = [
        ...
        *([format_gap_context_line(row)] if format_gap_context_line(row) else []),
...
def build_scan_health_warning_text(now_ts: float = None) -> str:
    ...
    if cache_age is not None and cache_age >= FAST_SCAN_DISPLAY_HIDE_SEC:
        lines.append(f'⚠️ 시장 데이터가 {cache_age}초 전이라 후보 표시는 보류하고 새 데이터 받는 중이야')
    elif cache_age is not None and cache_age >= FAST_SCAN_STATE_STALE_SEC:
        lines.append(f'⚠️ 지금 후보 기준이 {cache_age}초 지나서 새 후보로 갈아끼우는 중이야')
    elif cache_age is not None and cache_age >= SCAN_CACHE_WARN_SEC:
        lines.append(f'ℹ️ 지금 후보는 {cache_age}초 전 시장 데이터 기준이야')
    if fast.get('hide_candidates'):
        lines.append('🚫 예전 후보는 숨겼고 새 시장 데이터 다시 받는 중이야')
