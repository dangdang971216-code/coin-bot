...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v175: 정확도 안정화판. 최신 후보파일/중복skip/스프레드 이상값/정보없음 표시를 정리한다.",
}
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.177: 웹소켓 보조허브 기본 ON + 스캔 신뢰도 보강 + trade_ready 약한 손실유형은 관찰 이동.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.176 기준: 결과창 가독성 / paper_bot 기준 통일 / 정보품질 표시 유지",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.177.py", "- 페이퍼봇 대상: paper_bot_v0.33.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.177: 웹소켓 보조허브 + 스캔정보 신뢰도 + trade_ready 품질 보강", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.176 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def _builder_for_command(name: str):
    if not TEXT_COMMAND_BUILDERS:
        TEXT_COMMAND_BUILDERS.update({
            ...
            "check": check_text, "check_full": check_full_text, "bot_status": bot_status_text, "quality": lambda: candidate_quality_text(False), "quality_full": lambda: candidate_quality_text(True),
            "flow_check": lambda: "\n".join(["🧭 흐름검사 /flow_check", field_line(), candidate_line()] + stage_lines()),
            "candidate_reason": lambda: "\n".join(["🔎 후보·전략 원인판 /candidate_reason", field_line(), candidate_line()] + reject_lines()),
            "cpu_status": lambda: "\n".join(["🧠 CPU/메모리 /cpu_status", f"- scan최근 {fnum(STATE.get('scan_last_sec'),0):.2f}s / 최대 {fnum(STATE.get('scan_max_sec'),0):.2f}s / stage {STATE.get('scan_last_stage','-')}"] + stage_lines()),
            ...
            "help": lambda: "📚 명령어 /help\n- 여러 명령을 줄바꿈으로 보내도 자동 묶음 처리됨\n- /batch /check /core /bot_status /quality /score /cpu_status /paper_handoff /deploy /errorlog_full",
        })
...
def install_commands(updater: Any) -> None:
    ...
    try:
        menu = ["batch", "check", "check_full", "core", "bot_status", "quality", "quality_full", "score", "paper_today", "flow_check", "candidate_reason", "cpu_status", "paper_handoff", "paper_handoff_full", "deploy", "trade", "errorlog", "errorlog_full", "help"]
