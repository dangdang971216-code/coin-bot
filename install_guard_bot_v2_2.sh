...
# 휴대폰 다운로드/업로드 때 -1, _v2_2 이름이 붙어도 설치 가능하게 정규화
if [ -f backga_guard_bot_v2_2.py ]; then
  cp -f backga_guard_bot_v2_2.py backga_guard_bot.py
...
if [ ! -f backga_guard_bot.py ]; then
  echo "backga_guard_bot.py 파일이 없습니다. backga_guard_bot_v2_2.py 또는 backga_guard_bot.py를 올려주세요."
  exit 1
fi
