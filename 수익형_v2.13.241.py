...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v241 strategy-watch observe only(정식 paper OPEN/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
def _v238_quality_cache_valid(obj: Dict[str, Any]) -> bool:
    ...
    if fnum(obj.get("paper_closed_mtime"), -1) != fnum(sig.get("paper_closed_mtime"), -2):
        return False
    if int(obj.get("paper_closed_size", -1) or -1) != int(sig.get("paper_closed_size", -2) or -2):
        return False
    return True
...
def _v238_quality_text_from_snapshot(snap: Optional[Dict[str, Any]] = None) -> str:  # type: ignore[override]
    ...
    for old in ("v239", "v238", "v237", "v236", "v235", "v234", "v233", "v231", "v230", "v222"):
        txt = txt.replace(f"{old} 기본은 캐시 전용", "v240 기본은 캐시 전용")
    return txt
...
def _v241_score_low_rebound(row: Dict[str, Any]) -> Tuple[float, List[str]]:
    price = _v241_watch_price(row)
    if price <= 0 or bool(row.get("major_watch")):
        return 0.0, []
    from_low = fnum(row.get("from_30m_low_pct"), 0)
    ch1 = fnum(row.get("change_1"), 0)
    ch3 = fnum(row.get("change_3"), 0)
    m3 = fnum(row.get("money_flow_3m") or row.get("turnover_3m"), 0)
    close_pos = fnum(row.get("current_close_pos_ratio"), 0)
    rsi = fnum(row.get("rsi_14"), 0)
    buy_ratio = fnum(row.get("micro_trade_buy_ratio_30"), 0)
    score = 0.0
    reasons: List[str] = []
    if 0.15 <= from_low <= 4.2:
        score += 1.25; reasons.append(f"저점근처 {from_low:.2f}%")
    if ch1 > 0 or ch3 > 0.04:
        score += 0.85; reasons.append(f"초반반등 1m {ch1:+.2f}% / 3m {ch3:+.2f}%")
    if m3 >= 5_000_000:
        score += min(1.2, m3 / 25_000_000); reasons.append(f"3분돈 {krw_m(m3)}")
    if close_pos >= 0.48:
        score += 0.45; reasons.append("현재봉 회복")
    if 0 < rsi <= 52:
        score += 0.35; reasons.append(f"RSI {rsi:.0f}")
    if buy_ratio >= 0.50:
        score += 0.35; reasons.append(f"매수체결 {buy_ratio:.2f}")
    if fnum(row.get("current_upper_wick_pct"), 0) >= 0.8:
        score -= 0.45; reasons.append("위꼬리주의")
    return round(score, 3), reasons


def _v241_score_surge_start(row: Dict[str, Any]) -> Tuple[float, List[str]]:
    price = _v241_watch_price(row)
    if price <= 0 or bool(row.get("major_watch")):
        return 0.0, []
    ch1 = fnum(row.get("change_1"), 0)
    ch3 = fnum(row.get("change_3"), 0)
    vol = fnum(row.get("vol_ratio"), 0)
    m1 = fnum(row.get("money_flow_1m") or row.get("turnover_1m"), 0)
    m3 = fnum(row.get("money_flow_3m") or row.get("turnover_3m"), 0)
    from_low = fnum(row.get("from_30m_low_pct"), 0)
    upper = fnum(row.get("current_upper_wick_pct"), 0)
    buy_ratio = fnum(row.get("micro_trade_buy_ratio_30"), 0)
    score = 0.0
    reasons: List[str] = []
    if ch1 >= 0.22 or ch3 >= 0.45:
        score += 1.2; reasons.append(f"급등초입 1m {ch1:+.2f}% / 3m {ch3:+.2f}%")
    if vol >= 1.45:
        score += min(1.0, vol / 4.0); reasons.append(f"거래량 {vol:.2f}배")
    if m1 >= 2_000_000 or m3 >= 8_000_000:
        score += min(1.0, max(m1 / 8_000_000, m3 / 30_000_000)); reasons.append(f"돈유입 1m {krw_m(m1)} / 3m {krw_m(m3)}")
    if from_low <= 8.5:
        score += 0.35; reasons.append("과한 추격 전")
    if buy_ratio >= 0.54:
        score += 0.35; reasons.append(f"매수체결 {buy_ratio:.2f}")
    if upper >= 0.85 or bool(row.get("volume_pump_risk")):
        score -= 0.65; reasons.append("펌핑/위꼬리주의")
    return round(score, 3), reasons


def _v241_score_money_reaccel(row: Dict[str, Any]) -> Tuple[float, List[str]]:
    price = _v241_watch_price(row)
    if price <= 0 or bool(row.get("major_watch")):
        return 0.0, []
    ch1 = fnum(row.get("change_1"), 0)
    ...
    m1 = fnum(row.get("money_flow_1m") or row.get("turnover_1m"), 0)
    m3 = fnum(row.get("money_flow_3m") or row.get("turnover_3m"), 0)
    m5 = fnum(row.get("money_flow_5m") or row.get("turnover_5m"), 0)
    rebreak = fnum(row.get("rebreakout_strength"), 0)
    pb = fnum(row.get("pullback_quality_score"), 0)
    vol = fnum(row.get("vol_ratio"), 0)
    score = 0.0
    reasons: List[str] = []
    if m3 >= 7_000_000 and m5 >= m3:
        score += min(1.2, m3 / 28_000_000); reasons.append(f"돈흐름 재유입 3m {krw_m(m3)}")
    if m1 >= 1_500_000 and m3 > m1 * 1.4:
        score += 0.5; reasons.append("1분+3분 동시 유입")
    if ch1 >= -0.05 and ch3 > 0.05 and ch5 > -0.15:
        score += 0.75; reasons.append(f"가격 재반응 1m {ch1:+.2f}% / 3m {ch3:+.2f}%")
    if rebreak >= 0.7 or pb >= 1.1:
        score += 0.55; reasons.append(f"재돌파/눌림 {rebreak:.2f}/{pb:.2f}")
    if vol >= 1.15:
        score += 0.35; reasons.append(f"거래량 {vol:.2f}배")
    return round(score, 3), reasons


def _v241_score_pullback(row: Dict[str, Any]) -> Tuple[float, List[str]]:
    score = fnum(row.get("score"), 0)
    if score <= 0 or bool(row.get("major_watch")):
        return 0.0, []
    reasons = ["기존 눌림 재돌파 기준선", f"점수 {score:.2f}"]
    m3 = fnum(row.get("money_flow_3m") or row.get("turnover_3m"), 0)
    if m3 > 0:
        reasons.append(f"3분돈 {krw_m(m3)}")
    return round(score, 3), reasons


def _v241_select_watch_candidates(rows: List[Dict[str, Any]], strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    scan_id = str(STATE.get("scan_id") or f"scan-{int(now_ts())}")
    selected: List[Dict[str, Any]] = []
    # 기존 본선은 이미 점수가 붙은 strict/shadow에서만 작게 샘플링한다.
    pools: Dict[str, List[Tuple[float, Dict[str, Any], List[str]]]] = {k: [] for k in STRATEGY_WATCH_LABELS}
    for row in list(strict or []) + list(shadow or []):
        s, reasons = _v241_score_pullback(row)
        if s >= 1.7:
            pools["pullback_rebreakout"].append((s, row, reasons))
    # 새 전략검증은 전체 표준 row를 쓰되, scan당 상위 몇 개만 기록한다.
    for row in rows or []:
        for kind, fn, threshold in (
            ("low_rebound_early", _v241_score_low_rebound, 1.85),
            ("surge_start", _v241_score_surge_start, 1.95),
            ("money_reaccel", _v241_score_money_reaccel, 1.85),
        ):
            s, reasons = fn(row)
            if s >= threshold:
                pools[kind].append((s, row, reasons))
    for kind, arr in pools.items():
        seen: set[str] = set()
        for s, row, reasons in sorted(arr, key=lambda x: (x[0], fnum(x[1].get("money_flow_3m") or x[1].get("turnover_3m"), 0)), reverse=True):
            t = _v241_watch_ticker(row)
            if not t or t in seen:
                continue
            seen.add(t)
            selected.append(_v241_watch_event(kind, row, s, reasons, scan_id))
            if len(seen) >= max(1, STRATEGY_WATCH_TOP_PER_KIND):
                break
    return selected
...
def _v241_strategy_watch_update(rows: List[Dict[str, Any]], strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        with _state_lock:
            ...
            STATE["strategy_watch_note"] = "v241 observe-only; paper OPEN 연결 없음"
    except Exception as exc:
...
def _v241_strategy_stat_line(label: str, rows: List[Dict[str, Any]]) -> str:
    n = len(rows)
    if not n:
    ...
    icon = "✅" if n >= 5 and hit / n >= 0.35 and avg_max >= 1.0 else ("❌" if n >= 5 and minus_first / n >= 0.45 else "❔")
    return f"{icon} {label}: 후보 {n} / +1.2도달 {hit}({hit/n*100:.0f}%) / 현재양수 {positive_now}({positive_now/n*100:.0f}%) / 평균현재 {avg_cur:+.2f}% / 평균최고 {avg_max:+.2f}% / 평균최저 {avg_min:+.2f}% / -0.7먼저 {minus_first}"
...
def _v241_watch_candidate_lines(rows: List[Dict[str, Any]], limit: int = 8) -> List[str]:
    ...
    out: List[str] = []
    for r in ranked:
        age = fnum(r.get("age_sec"), max(0.0, now_ts() - fnum(r.get("created_at"), now_ts())))
        age_txt = f"{age/60:.0f}분" if age < 3600 else f"{age/3600:.1f}시간"
        r5 = r.get("return_5m_pct", "-")
        r10 = r.get("return_10m_pct", "-")
        r20 = r.get("return_20m_pct", "-")
        def _fmt(v):
            return f"{fnum(v,0):+.2f}%" if isinstance(v, (int, float)) or str(v).replace('.','',1).replace('-','',1).isdigit() else "-"
        out.append(
            f"- {_v241_watch_ticker(r)} / {r.get('watch_label','-')} / {age_txt} / 현재 {fnum(r.get('current_return_pct'),0):+.2f}% / 최고 {fnum(r.get('max_return_pct'),0):+.2f}% / 최저 {fnum(r.get('min_return_pct'),0):+.2f}% / 5m {_fmt(r5)} 10m {_fmt(r10)} 20m {_fmt(r20)}"
        )
    return out
...
def strategy_watch_text(full: bool = False) -> str:
    ...
    lines = [
        ...
        f"- 기준: 최근 3시간 / active {active_n}개 / 완료보관 {done_n}개 / scan추가 {STATE.get('strategy_watch_added',0)}개",
        "",
        "[1/4] 전략별 3시간 성과",
    ]
    for kind in ("pullback_rebreakout", "low_rebound_early", "surge_start", "money_reaccel"):
        lines.append(_v241_strategy_stat_line(STRATEGY_WATCH_LABELS.get(kind, kind), by_kind.get(kind, [])))
    lines += [
        "",
        "[2/4] 후보별 상위 흐름",
        *_v241_watch_candidate_lines(rows, 8 if not full else 20),
        "",
        "[3/4] 복기 기준",
    ...
    if len(rows) < 20:
        lines.append("❔ 표본 부족: 최소 20개 이상 쌓인 뒤 전략 비교")
    else:
        best = sorted(((kind, by_kind.get(kind, [])) for kind in by_kind), key=lambda kv: (_v241_avg([fnum(r.get('max_return_pct'), 0) for r in kv[1]]), len(kv[1])), reverse=True)[0]
        lines.append(f"✅ 우선 관찰: {STRATEGY_WATCH_LABELS.get(best[0], best[0])} / 평균최고 {_v241_avg([fnum(r.get('max_return_pct'), 0) for r in best[1]]):+.2f}%")
    lines.append("- 다음 판단은 /score가 아니라 /strategy_watch 표본으로 본다.")
    return "\n".join(lines)
...
def score_text() -> str:  # type: ignore[override]
    txt = _v241_base_score_text()
    try:
        rows = _v241_watch_rows(3.0)
        if rows:
            hit = sum(1 for r in rows if bool(r.get("hit_plus_1_2")) or fnum(r.get("max_return_pct"), 0) >= 1.2)
            best = max(rows, key=lambda r: fnum(r.get("max_return_pct"), 0))
            txt += "\n\n[전략검증 3시간]\n"
            txt += f"- 관찰후보 {len(rows)}개 / +1.2도달 {hit}개 / 최고 {_v241_watch_ticker(best)} {fnum(best.get('max_return_pct'),0):+.2f}% / 상세 /strategy_watch"
        else:
            txt += "\n\n[전략검증 3시간]\n- 아직 관찰 표본 없음 / 상세 /strategy_watch"
    except Exception as exc:
        log_error("v241_score_strategy_watch_line", exc)
    return txt
