...
- 실거래 주문은 v2.7.16 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.16"
BOT_FILE_TAG = "수익형_v2.7.16.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.16는 현재가+체결 허브를 후보분류에 더 적극 반영한다.
...
def paper_strategy_legend() -> list[str]:
    return [
        ...
        "S3 상승초입: 초반 힘 붙는 후보",
        "S4 상승후 재확인: 급등초입 뒤 식는 중 유지 확인",
        "S5 재출발: 급등 후 눌림에서 다시 살아나는 후보",
        "S6 재돌파: 고점 찍고 밀린 뒤 재도전 후보",
        "S7 급등후 재확인: 고점권 강한 반등 후 유지 확인",
        "S8 데이터확인: 구조는 보이나 실시간자료 확인 필요",
    ]
...
def paper_strategy_priority(pattern: str, strategy: str = "") -> int:
    """
    paper 중복 병합 시 대표전략 우선순위.
    먼저 등록된 약한 눌림이 나중에 확인된 급등/상승후재확인 신호를 눌러버리면
    ZIL/B3류 통계가 눌림으로 왜곡된다.
    """
    p = str(pattern or "")
    s = str(strategy or paper_strategy_name(p))
    if p in ("SURGE_COOLDOWN_WAIT", "SURGE_RECHECK_WAIT") or "급등후" in s or "상승후" in s:
        return 90
    if p in ("START_EARLY", "START_EARLY_BREAK_HOLD", "START_EARLY_WAIT", "START_EARLY_WEAK_WAIT") or "상승초입" in s:
        return 80
    if p in ("RESTART_WAIT", "REBREAK_WAIT") or "재출발" in s or "재돌파" in s:
        return 70
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.16 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
def special_wait_label(pattern: str) -> tuple[str, str, str]:
    if pattern == "SURGE_COOLDOWN_WAIT":
        return "상승 후 재확인대기", "초입 급등 뒤 식는 중. 눌림이 아니라 유지 확인 상태", "상승 후 재확인"
...
def v315_mainline_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool) -> dict:
    """
    v2.7.16: v2.5.3xx 후기판의 본선 판단을 clean core row로 옮긴 1차 컨텍스트.
...
def v316_pullback_risk_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool) -> dict:
    """
    v2.7.16: v2.5.3xx 후기판의 위험진입 차단 2차 이식.
...
def v316_pullback_risk_reason(ctx: dict) -> str:
    reasons = []
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    # v2.7.16: v2.5.3xx 본선 기준 이식 컨텍스트.
    ...
    v316_ctx = v316_pullback_risk_context(
        row, data_3m_ok=data_3m_ok, data_5m_ok=data_5m_ok,
        one_min_milrim=one_min_milrim, late_chase=late_chase, recent_money_ok=recent_money_ok,
    )
    v316_dirty_pullback = bool(v316_ctx.get("dirty_pullback"))

    # v2.7.16: 예전 승률 좋던 눌림/준비대기 1차 복구.
    ...
    # v2.7.16에서는 일반 약한 움직임 전체가 재출발감시로 빨려 들어가지 않게,
    ...
    # v2.7.16: 특수 대기상태 보존. TON류 재출발/재돌파/급등후재확인 후보가
    ...
    if v316_dirty_pullback and (row.pullback_shape_ok or row.low_defense_ok or row.from_low30 <= 3.2):
        return make_candidate(
            row, "WATCH", "위험눌림 감시", "⚪",
            "눌림처럼 보이나 위험진입 조건이 있어 눌림 검증에서 제외",
            pattern="PULLBACK_RISK_WATCH", caution=v316_pullback_risk_reason(v316_ctx),
            entry_note="현재봉 회복·저점방어·돈 유지가 다시 붙으면 눌림 준비로 재평가",
            reject_reason="위험눌림 제외",
            block_reasons=["v316 위험눌림 제외", v316_pullback_risk_reason(v316_ctx)]
        )
...
def pattern_kor(pattern: str) -> str:
    table = {
        ...
        "SURGE_COOLDOWN_WAIT": "상승 후 재확인대기",
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.16: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.16는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.16\n"
        f"- v2.7.16 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.16에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.16\n"
...
def render_paper_stats() -> str:
    ...
    lines = [
        ...
        "",
        "전략번호",
        *[f"- {x}" for x in paper_strategy_legend()],
        "",
        "1) 전략별 10분 성과",
    ]
    if not groups:
    ...
    if overdue30:
        lines.append("- ⚠️ 30분 지났는데 미완료 후보가 있음. v2.7.16에서 paper worker/저장 조건을 봐야 함")
    return "\n".join(lines)
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.16 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
