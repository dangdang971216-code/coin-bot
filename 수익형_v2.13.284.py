...
def _v284_build_watch_summary(items: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for sname in V284_STRATEGY_ORDER:
        arr = by.get(sname, [])
        first_done = [x for x in arr if x.get("first_hit") in {"+1.2먼저", "-0.7먼저"}]
        plus = sum(1 for x in first_done if x.get("first_hit") == "+1.2먼저")
        minus = sum(1 for x in first_done if x.get("first_hit") == "-0.7먼저")
        data = {
            "label": sname,
            "watch_count": len(arr),
            "decision_count": len(first_done),
            "plus_first": plus,
            "minus_first": minus,
            "watch_win_rate": round(plus / len(first_done) * 100.0, 1) if first_done else None,
            "avg_current_pct": round(sum(fnum(x.get("last_pct"), 0) for x in arr) / len(arr), 3) if arr else None,
            "avg_peak_pct": round(sum(fnum(x.get("peak_pct"), 0) for x in arr) / len(arr), 3) if arr else None,
    ...
    return {
        "version": BOT_VERSION,
        "schema": "strategy_material_watch_summary_v284",
        "updated_ts": now_ts(),
        "updated_text": now_text(),
...
def _v284_update_material_watch(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for it in list(items.values()):
        if isinstance(it, dict):
            p = price_by_ticker.get(str(it.get("ticker") or ""))
            if p:
                _v284_update_item_with_price(it, p, ts)
    # 새 관찰 항목 추가. 상위 재료만 쓰지 않고 7전략 중 하나라도 잡힌 후보를 전부 추적한다.
    for r in current_rows:
        t = _v274_ticker(r)
        p = _v284_row_price(r)
        ...
        if fid not in items:
            items[fid] = {
                ...
                "score_groups": (r or {}).get("material_score_groups") if isinstance((r or {}).get("material_score_groups"), dict) else {},
                "milestones": {},
            }
    # 오래된 항목 정리 + 크기 제한. CLOSED/trade_log는 건드리지 않는다.
    ...
    if len(items) > V284_FOLLOW_MAX_ITEMS:
        ...
        items = dict(ordered)
    state = {"schema": "strategy_material_watch_state_v284", "version": BOT_VERSION, "updated_ts": ts, "updated_text": now_text(ts), "items": items}
...
def _v284_strategy_observation_lines(limit: int = 7) -> List[str]:
    ...
    for i, name in enumerate(V284_STRATEGY_ORDER[:limit]):
        ...
        n = int(v.get("watch_count") or 0)
        dec = int(v.get("decision_count") or 0)
        wr = v.get("watch_win_rate")
        wr_txt = "-" if wr is None else f"{fnum(wr):.1f}%"
...
def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    if not high:
        lines.append("- 아직 없음")
    lines += ["", "[4] 다음에 반드시 채워야 할 정보", "- 후보 5/10/20분 추적 표본 누적", "- 같은 코인/패턴 반복손실 기억", "- 지지/저항/쓸림 전용값", "- BB폭/ATR 수축률", "- 호가/체결 변화량", "- 예상 슬리피지"]
    if not full:
...
def _v277_write_score_cache(payload: Dict[str, Any]) -> None:  # type: ignore[override]
    """v284: /score cache writer. 7전략 관찰성과, 기존 CLOSED 성과, 재료수집 현황을 분리해서 저장한다."""
    try:
        ...
        text = "\n".join([
            ...
            "[1] 캐시 상태",
            "- /score는 저장된 캐시만 표시합니다. 명령어에서 장부를 직접 계산하지 않습니다.",
            "- 7개 전략은 먼저 재료 관찰 성과로 비교합니다.",
            "- 기존 CLOSED 승률·수익률은 과거 장부에 전략명이 남은 기록만 표시합니다.",
            "",
            "[2] 7전략 관찰 승률·수익률 · 5/10/20분 추적 기준",
            "- 관찰승률 = 재료 발생 뒤 +1.2%가 -0.7%보다 먼저 온 비율입니다. 모의매매 승률이 아닙니다.",
            *_v284_strategy_observation_lines(7),
            "",
            "[3] 7전략 기존 CLOSED 승률·수익률 · 참고용",
            *_v280_strategy_performance_lines(7),
            "",
            "[4] 전략별 재료 수집 현황 · 성과 아님",
            ...
            *_v280_strategy_block_lines(lab, 7),
            ...
            "",
            "[7] 다음 판단",
            "- 관찰 5/10/20분 표본이 쌓이면 양수 전략만 모의매매 후보로 승격합니다.",
            "- 승률 50%가 안 돼도 최대제외 수익률이 플러스면 후보가 될 수 있지만, 지금은 관찰 표본이 먼저 필요합니다.",
        ])
        save_json(FILES.get('score_summary', BASE_DIR / 'clean_score_summary.json'), _cache_payload(text, 'score'))
    except Exception as exc:
        log_error('v284_score_cache_write', exc)

def health_text() -> str:  # type: ignore[override]
    """v284 readable health. CPU/memory/disk free remain visible; mainbot section is user-facing Korean."""
    try:
        _write_resource_status()
    except Exception:
        pass
    ...
    stage_lines_raw = STATE.get('stage_times') if isinstance(STATE.get('stage_times'), list) else []
    slow_note = []
    for item in stage_lines_raw[-8:]:
        try:
            name, sec, note = item
            sec = fnum(sec, 0)
            if sec >= 20:
                slow_note.append(f"❌ {str(name).split(':')[0]} {sec:.1f}초")
            elif sec >= 8:
                slow_note.append(f"⚠️ {str(name).split(':')[0]} {sec:.1f}초")
        except Exception:
            pass
    ...
    lines = [
        ...
        f"- 느린 구간: {(' / '.join(slow_note[:4]) if slow_note else '✅ 큰 병목 표시 없음')}",
        "",
        "[2/6] 스캔 흐름",
        "- 허브: 시장 원자료 수집",
        "- 직원: 정밀재료 보강 → 표준값 병합 → 재료점수 계산",
        "- 공장: 재료 저장 → 스냅샷/외부직원 대상 저장",
    ]
    # 단계 상세는 너무 길지 않게 핵심 6줄만 표시
    for item in stage_lines_raw[-6:]:
        try:
            name, sec, note = item
            sec = fnum(sec, 0)
            icon = "❌" if sec >= 20 else ("⚠️" if sec >= 8 else "✅")
            nm = str(name).replace("material latest", "재료 저장").replace("snapshot/target", "스냅샷/대상")
            lines.append(f"  · {icon} {nm}: {sec:.1f}초 / {note}")
        except Exception:
            pass
    if not stage_lines_raw:
        lines.append("  · 아직 단계 기록 없음")
    lines += [
        "",
        "[3/6] 서버자원",
        *resource_health_lines(),
        ...
        "",
        "[5/6] 재료공장",
        f"✅ 재료후보 {len(rows)} / 등급 " + (" / ".join(f"{k} {v}" for k, v in grades.most_common(4)) if grades else "없음"),
...
try:
    STRATEGY_NAME = "외부전략 재료수집"
    STRATEGY_KEY = "material_factory_v284"
    STATE["phase_note"] = "v283: old precision target wrapper 삭제, 재료공장 단일 정밀대상 selector로 재연결. 신규 모의매매 OPEN 닫음."
