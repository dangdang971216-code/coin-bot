...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.209.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def start_background_workers() -> None:
    ...
    threading.Thread(target=command_cache_worker_loop, name="command_cache_worker_v209", daemon=True).start()
    websocket_hub_worker_loop()  # v180: legacy WS는 스레드도 만들지 않고 하드 격리
    log(f"background_workers started precision={PRECISION_BACKGROUND_WORKERS} execution_risk={EXEC_RISK_BACKGROUND_WORKERS} command_cache=v209 websocket=external_sidecar_cache requested={WS_HUB_REQUESTED}")
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v209 캐시전용 명령어 + WS 후보신선 보강 + CPU% 표시(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
