...
def signal_priority_value(signal):
    ...
    return float(signal.get("edge_score", signal.get("signal_score", 0))) + tier_bonus + leader_bonus + flow_bonus + day_bonus + hold_bonus
...
def get_alert_survival_display_key(item: dict, regime: dict = None) -> str:
    ...
    if daily_ready <= -0.90 and daily_gap_pct <= 0.90:
        return ""
    if breakout_hold_score <= -0.45 and chg_5 < 0.10 and chg_30 < 0.05:
        return ""
    ...
    if strategy in {"PREWATCH", "NEWLOW_RECOVER", "LEADER_WATCH", "PREPUMP", "PULLBACK"}:
        if high_gap_pct >= 1.15 and chart_rise <= 3.90 and from_low <= 2.10 and (
            chg_5 >= 0.14 or chg_30 >= 0.08 or market_flow_score >= 0.95 or leader >= 1.00 or breakout_hold_score >= 0.10
        ):
            return "early"
        return "restart"
    if high_gap_pct >= 1.00 and chart_rise <= 3.80 and from_low <= 1.95 and (
        chg_5 >= 0.16 or chg_30 >= 0.10 or market_flow_score >= 0.95 or leader >= 0.95 or breakout_hold_score >= 0.10
    ):
        return "early"
    if high_gap_pct >= 0.82 and (
        motion >= 0.00 or market_flow_score >= 0.95 or leader >= 0.95 or breakout_hold_score >= 0.10 or vol_ratio >= 1.55
    ):
        return "restart"
    return ""
...
def get_alert_survival_reason_text(item: dict, display_key: str = "restart") -> str:
    ...
    if section == "early":
        return f"알림 생존층 · {flow_label or '시장 흐름'} + {daily_label or '일봉'} 기준으로 초입 확인".strip()
    return f"알림 생존층 · {flow_label or '시장 흐름'} / {daily_label or '일봉'} / {hold_label or '유지'} 기준으로 재출발 확인".strip()
...
def is_top_turnover_rescue_candidate(item: dict) -> bool:
    ...
    if chart_rise > 4.80 and high_gap_pct < 2.20:
        return False
    if motion < 0.05 and leader < 0.18:
        return False
...
def get_shortlist_user_rescue_display_key(item: dict, display_key: str = "", regime: dict = None) -> str:
    ...
    if drought_sec >= 5400 and shortlist_rank <= 6 and high_gap_pct >= 0.85 and chart_rise <= 4.60:
        if (market_flow_score >= 1.00 or daily_ready >= 0.20 or breakout_hold_score >= 0.10) and (vol_ratio >= 0.72 or leader >= 0.18 or edge >= 4.10 or motion >= 0.06):
            return "restart"

    if shortlist_rank <= 5 and turnover_rank <= 35 and leader_rank <= 30 and high_gap_pct >= 1.20 and from_low <= 1.90 and chart_rise <= 4.35:
        if chg_30 >= -1.25 and (vol_ratio >= 0.72 or leader >= 0.35 or edge >= 4.15 or market_flow_score >= 0.95 or daily_ready >= 0.20 or breakout_hold_score >= 0.10):
            return "restart"

    if shortlist_rank <= 7 and high_gap_pct >= 1.35 and from_low <= 2.25 and chart_rise <= 4.45:
        if vol_ratio >= 0.72 or leader >= 0.15 or edge >= 4.05 or daily_ready >= 0.15 or breakout_hold_score >= 0.10 or market_flow_score >= 0.95:
            return "restart"

    if shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG and high_gap_pct >= 0.68 and motion >= 0.05 and chart_rise <= 4.35:
        if vol_ratio >= 0.82 or leader >= 0.30 or edge >= 4.25 or market_flow_score >= 0.85:
            return "restart"

    if chg_5 >= 0.42 and high_gap_pct >= 0.82 and (chg_30 >= 0.00 or vol_ratio >= 1.45 or leader >= 0.72 or hot_score >= USER_ALERT_RESCUE_STRONG_HOT_SCORE):
        return "live"
    if motion >= 0.02 and high_gap_pct >= 0.48 and (vol_ratio >= 0.82 or leader >= 0.28 or edge >= 4.25 or strong_rank_rescue or market_flow_score >= 0.85):
        return "restart"
    return ""
...
def is_user_alert_rescue_candidate(item: dict, section: str = "", regime: dict = None) -> bool:
    ...
    if daily_ready <= -1.05 and market_flow_score < 0.95 and not top_turnover_rescue and drought_sec < 5400:
        return False
    if breakout_hold_score <= -0.35 and high_gap_pct < 0.85 and chg_5 < 0.10 and chg_30 < 0.05 and not top_turnover_rescue and drought_sec < 5400:
        return False
    ...
    if section == "restart":
        ...
        if shortlist_rank <= 7 and high_gap_pct >= 0.65 and from_low <= 2.95 and chart_rise <= 4.60:
            if max(chg_5, chg_30, from_low) >= 0.02 or market_flow_score >= 0.95 or daily_ready >= 0.20 or breakout_hold_score >= 0.10 or vol_ratio >= 0.72 or leader >= 0.18:
                return True
