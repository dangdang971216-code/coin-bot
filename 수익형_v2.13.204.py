...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.204.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def command_version_score(update, context) -> None:
    ...
    lines = ["📊 현재버전 성과 /version_score", _compact_stat_line("현재버전 정식 모의매매", rows), f"- 기준: {BOT_VERSION} / {version_baseline_text()} / 기존 기록 삭제 없음", f"- 주요종료: {_reason_summary(rows)}", "- 참고: 상세 손절은 /loss_review 204"]
    reply(update, "\n".join(lines))
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v204 상대강도 기록 + 저장전 overlay 유지 + health/external 기준 통일(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
