...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.202.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def _overlay_current_external_for_items(items: List[Dict[str, Any]], *, refresh: bool = True) -> List[Dict[str, Any]]:
    ...
    for r in items or []:
        ...
        try:
            ws = ws_snapshot(t)
            # 현재 상태판은 최신 캐시 기준을 우선한다. cache missing이면 targeted 표시 정도만 반영한다.
            rr.update(ws)
...
def command_version_score(update, context) -> None:
    # v202: /version_score는 현재버전만 짧게 계산한다. CLOSED 파일은 load_closed 캐시를 사용한다.
    rows = [r for r in rows_since_current_version(load_closed()) if str(r.get("lane")) == "strict"]
    lines = [
        "📊 현재버전 성과 /version_score",
        _compact_stat_line("현재버전 정식 모의매매", rows),
        f"- 기준: {BOT_VERSION} / {version_baseline_text()} / 기존 기록 삭제 없음",
        f"- 주요종료: {_reason_summary(rows)}",
        "- 참고: 상세 손절은 /loss_review 202",
    ]
    reply(update, "\n".join(lines))
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v202 명령어 속도개선 + 현재후보 WS/micro 최신 overlay(조건/청산/BY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
