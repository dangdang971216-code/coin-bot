...
# v179: 웹소켓은 초단타 실시간 주 데이터다. 단, 연결부는 메인봇 생명주기와 분리해 REST 본선을 절대 막지 않는다.
WS_HUB_ON = str(os.getenv("CLEAN_WS_HUB_ON", "1")).strip().lower() not in {"0", "false", "no", "off"}
...
WS_HUB_URL = os.getenv("CLEAN_WS_HUB_URL", "wss://pubwss.bithumb.com/pub/ws").strip()
WS_HUB_OPEN_TIMEOUT_SEC = float(os.getenv("CLEAN_WS_HUB_OPEN_TIMEOUT_SEC", "4.0"))
WS_HUB_READ_TIMEOUT_SEC = float(os.getenv("CLEAN_WS_HUB_READ_TIMEOUT_SEC", "3.0"))
WS_HUB_IDLE_RECONNECT_SEC = float(os.getenv("CLEAN_WS_HUB_IDLE_RECONNECT_SEC", "12.0"))
WS_HUB_TICK_TYPES = [x.strip() for x in os.getenv("CLEAN_WS_HUB_TICK_TYPES", "24H").split(",") if x.strip()]
...
# v177: 후보를 죽이지 않고 trade_ready만 관찰로 돌리는 품질 보강값.
# 최근 표본에서 손실군은 가격재확인 음수, 1분 과열, 거래량 과폭발, 낮은 눌림/재돌파가 반복됐다.
TRADE_READY_MIN_PRICE_RECHECK_PCT = float(os.getenv("CLEAN_TRADE_READY_MIN_PRICE_RECHECK_PCT", "0.0"))
TRADE_READY_MAX_VOLUME_SPIKE_30X = float(os.getenv("CLEAN_TRADE_READY_MAX_VOLUME_SPIKE_30X", "2.0"))
TRADE_READY_MAX_1M_TO_3M_RATIO = float(os.getenv("CLEAN_TRADE_READY_MAX_1M_TO_3M_RATIO", "0.28"))
TRADE_READY_MIN_PULLBACK_QUALITY = float(os.getenv("CLEAN_TRADE_READY_MIN_PULLBACK_QUALITY", "1.60"))
TRADE_READY_MIN_REBREAKOUT = float(os.getenv("CLEAN_TRADE_READY_MIN_REBREAKOUT", "1.15"))
...
STATE: Dict[str, Any] = {
    ...
    "ws_last_keys": "-",
    "ws_unmatched_sample": [],
    "ws_last_price_gap_pct": 0.0,
    "ws_worker_mode": "direct_websockets",
    "ws_connect_ok": 0,
    "ws_connect_fail": 0,
    "ws_recv_timeout": 0,
    "ws_target_key": "-",
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v178: 웹소켓 중심 허브 재정렬. 예전 티커 변환/파싱 흐름을 현재 구조에 재연결하고 REST는 안전망으로 둔다.",
}
...
def _ws_market_code_to_ticker(symbol: Any) -> str:
    """예전 v2.5.x 흐름 복구: KRW-BTC / BTC_KRW / BTC/KRW / content.cd 모두 내부 티커로 통일한다."""
...
def _ticker_to_ws_market_code(ticker: Any, legacy: bool = True) -> str:
    """Bithumb legacy websocket 구독은 BTC_KRW 형식이 안정적이라 기본값으로 쓴다."""
    t = _ws_market_code_to_ticker(ticker)
    if not t or t in STABLE_EXCLUDED:
        return ""
    return f"{t}_KRW" if legacy else f"KRW-{t}"


def ws_symbol(ticker: Any) -> str:
    return _ticker_to_ws_market_code(ticker, legacy=True)


def ws_plain_ticker(symbol: Any) -> str:
    return _ws_market_code_to_ticker(symbol)


def _ws_symbol_format(symbol: Any) -> str:
    s = str(symbol or "").upper().strip()
    if not s:
        return "unknown"
    if s.startswith("KRW-"):
...
def update_ws_targets(rows: List[Dict[str, Any]]) -> None:
    """웹소켓은 실시간 판단의 주 데이터다. 단, 구독대상은 유동성+급변 중심으로 잡고 REST가 누락을 보정한다."""
    try:
        ranked = sorted(rows or [], key=lambda r: (fnum(r.get("turnover_rank"), 9999), -abs(fnum(r.get("change_24h"), 0))))
        ticks: List[str] = []
        seen: set[str] = set()
        # major는 매매 후보가 아니라 시장 기준값이므로 항상 앞쪽에 둔다.
        for t in sorted(MAJOR_WATCH_TICKERS):
            if t not in seen:
                seen.add(t); ticks.append(t)
        for r in ranked:
            t = _ws_market_code_to_ticker(r.get("ticker"))
...
def _ws_payload_rows(data: Any) -> List[Tuple[Dict[str, Any], Dict[str, Any]]]:
    """웹소켓 raw payload에서 ticker row 후보를 모두 꺼낸다. v2/list/content/legacy 모두 지원."""
    try:
    ...
    if isinstance(data, list):
        ...
        for one in data:
            out.extend(_ws_payload_rows(one))
        return out
    if not isinstance(data, dict):
        return []
    wrapper = data
...
def parse_ws_price_payloads(data: Any) -> List[Dict[str, Any]]:
    """예전 350 계열의 느슨한 파서를 현재 허브에 맞게 정리한 단일 입구."""
    out: List[Dict[str, Any]] = []
    rows = _ws_payload_rows(data)
    for row, wrapper in rows:
        if not isinstance(row, dict):
            continue
        ...
        price = first_number(row, [
            "closePrice", "close_price", "trade_price", "price", "closing_price", "current_price",
            "tp", "close", "c", "last", "contPrice"
        ])
        if price <= 0:
            price = first_number(wrapper, ["closePrice", "close_price", "trade_price", "price", "tp", "close", "c", "last"])
        turnover = first_number(row, [
            "value", "trade_value", "tradeValue", "acc_trade_price", "acc_trade_price_24h", "turnover",
            "amount", "contAmt", "total", "volumePower"
        ])
        volume = first_number(row, [
            "quantity", "qty", "volume", "trade_volume", "units", "contQty", "acc_trade_volume", "acc_trade_volume_24h"
        ])
        ...
        out.append({
            ...
            "ws_volume": volume,
            "ws_amount_ok": bool(turnover > 0 or volume > 0),
            "raw_type": typ or "ticker",
            "symbol_format": _ws_symbol_format(code),
...
def parse_ws_price_payload(data: Any) -> Tuple[str, float, Dict[str, Any]]:
    parsed = parse_ws_price_payloads(data)
    if not parsed:
        return "", 0.0, {}
    first = parsed[0]
    ticker = str(first.get("ticker") or "")
    price = fnum(first.get("live_price"), 0)
    extra = {k: v for k, v in first.items() if k not in {"ticker", "live_price"}}
    return ticker, price, extra


def _update_ws_diag_on_raw(data: Any, parsed: List[Dict[str, Any]], target_set: set[str]) -> None:
    try:
        with _state_lock:
            STATE["ws_raw_total"] = int(STATE.get("ws_raw_total", 0) or 0) + 1
            if parsed:
                STATE["ws_parse_ok"] = int(STATE.get("ws_parse_ok", 0) or 0) + len(parsed)
                STATE["ws_price_ok"] = int(STATE.get("ws_price_ok", 0) or 0) + sum(1 for r in parsed if fnum(r.get("live_price"), 0) > 0)
                STATE["ws_amount_ok"] = int(STATE.get("ws_amount_ok", 0) or 0) + sum(1 for r in parsed if r.get("ws_amount_ok"))
                STATE["ws_match_ok"] = int(STATE.get("ws_match_ok", 0) or 0) + sum(1 for r in parsed if str(r.get("ticker") or "") in target_set)
                STATE["ws_match_miss"] = int(STATE.get("ws_match_miss", 0) or 0) + sum(1 for r in parsed if str(r.get("ticker") or "") not in target_set)
                last = parsed[-1]
                STATE["ws_last_symbol"] = last.get("code") or last.get("ticker") or "-"
                STATE["ws_last_format"] = last.get("symbol_format") or "-"
                STATE["ws_last_keys"] = last.get("raw_keys") or "-"
            else:
                STATE["ws_parse_empty"] = int(STATE.get("ws_parse_empty", 0) or 0) + 1
    except Exception:
        pass


def _ws_process_incoming(data: Any, target_set: set[str]) -> int:
    """v179: direct websocket worker 공통 수신 처리. 메인 스캔/REST 경로에는 영향을 주지 않는다."""
    global _ws_last_msg_ts
    parsed = parse_ws_price_payloads(data)
    _update_ws_diag_on_raw(data, parsed, target_set)
    if not parsed:
        return 0
    ts = now_ts()
    applied = 0
    with _ws_lock:
        for one in parsed:
            ticker = str(one.get("ticker") or "").upper()
            price = fnum(one.get("live_price"), 0)
            if not ticker or price <= 0:
                continue
            row = {"ticker": ticker, "live_price": price, "ts": ts, **{k: v for k, v in one.items() if k not in {"ticker", "live_price"}}}
            _ws_live_cache[ticker] = row
            applied += 1
        if applied:
            _ws_last_msg_ts = ts
        fresh_cache = sum(1 for v in _ws_live_cache.values() if ts - fnum(v.get("ts"), 0) <= WS_HUB_STALE_SEC)
        STATE["ws_cached"] = len(_ws_live_cache)
        STATE["ws_fresh_pass"] = fresh_cache
        STATE["ws_last_age_sec"] = 0 if applied else STATE.get("ws_last_age_sec", -1)
        if applied:
            STATE["ws_state"] = "수신중"
            STATE["ws_last_error"] = ""
    return applied
...
async def _ws_direct_session(symbols: List[str], target_set: set[str], target_key: str) -> None:
    """v179: pybithumb WebSocketManager 대신 직접 websockets를 쓴다.
    - subprocess를 만들지 않는다.
    - 짧은 open/read timeout으로 종료와 재시작을 물고 늘어지지 않는다.
    - 실패해도 REST 본선은 그대로 돈다.
    """
    try:
        ...
        with _state_lock:
            STATE["ws_state"] = "불가"
            STATE["ws_last_error"] = f"websockets import 실패: {exc.__class__.__name__}"
        return

    sub = {"type": "ticker", "symbols": symbols, "tickTypes": WS_HUB_TICK_TYPES or ["24H"]}
    with _state_lock:
        STATE["ws_state"] = "연결중"
        STATE["ws_targets"] = len(symbols)
        STATE["ws_target_key"] = target_key[:120]
        STATE["ws_worker_mode"] = "direct_websockets"
    try:
        async with websockets.connect(
            WS_HUB_URL,
            ping_interval=20,
            ping_timeout=5,
            open_timeout=WS_HUB_OPEN_TIMEOUT_SEC,
            close_timeout=1,
            max_queue=64,
        ) as websocket:
            await asyncio.wait_for(websocket.send(json.dumps(sub)), timeout=2.0)
            with _state_lock:
                STATE["ws_state"] = "연결"
                STATE["ws_connect_ok"] = int(STATE.get("ws_connect_ok", 0) or 0) + 1
                STATE["ws_last_error"] = ""
            log(f"direct websocket subscribed {len(symbols)}")
            last_data_ts = now_ts()
            while not _stop_event.is_set():
                # 대상 목록이 크게 바뀌면 세션을 새로 열어 구독을 갱신한다.
                with _ws_lock:
                    current_targets = list(_ws_targets)[:max(1, WS_HUB_MAX_TICKERS)]
                current_symbols = [_ticker_to_ws_market_code(t, legacy=True) for t in current_targets if _ticker_to_ws_market_code(t, legacy=True)]
                current_key = ",".join(current_symbols)
                if current_key and current_key != target_key:
                    return
                try:
                    raw = await asyncio.wait_for(websocket.recv(), timeout=WS_HUB_READ_TIMEOUT_SEC)
                except asyncio.TimeoutError:
                    with _state_lock:
                        STATE["ws_recv_timeout"] = int(STATE.get("ws_recv_timeout", 0) or 0) + 1
                        age = now_ts() - _ws_last_msg_ts if _ws_last_msg_ts > 0 else now_ts() - last_data_ts
                        STATE["ws_last_age_sec"] = round(age, 1)
                        STATE["ws_state"] = "수신대기"
                    if now_ts() - last_data_ts >= WS_HUB_IDLE_RECONNECT_SEC:
                        return
                    continue
                last_data_ts = now_ts()
                _ws_process_incoming(raw, target_set)
    except Exception as exc:
        with _state_lock:
            STATE["ws_connect_fail"] = int(STATE.get("ws_connect_fail", 0) or 0) + 1
            STATE["ws_state"] = "재연결대기"
            STATE["ws_last_error"] = f"{exc.__class__.__name__}: {str(exc)[:120]}"
            age = now_ts() - _ws_last_msg_ts if _ws_last_msg_ts > 0 else -1
            STATE["ws_last_age_sec"] = round(age, 1) if age >= 0 else -1
        log_error("direct_websocket_session", exc)
        return


def websocket_hub_worker_loop() -> None:
    """v179 웹소켓 허브.
    웹소켓은 실시간 주 데이터지만, 연결/종료 생명주기는 메인봇과 분리한다.
    pybithumb WebSocketManager는 subprocess/종료 timeout을 만들 수 있어 사용하지 않는다.
    """
    if not WS_HUB_ON:
        with _state_lock:
            STATE["ws_state"] = "OFF"
        return
    log("direct websocket_hub_worker started")
    last_error_sleep = 1.0
    while not _stop_event.is_set():
        try:
            with _ws_lock:
            ...
            if not targets:
                ...
                continue
            target_set = {_ws_market_code_to_ticker(t) for t in targets if _ws_market_code_to_ticker(t)}
            symbols = [_ticker_to_ws_market_code(t, legacy=True) for t in targets if _ticker_to_ws_market_code(t, legacy=True)]
            symbols = list(dict.fromkeys([x for x in symbols if x]))
            if not symbols:
                with _state_lock:
                    STATE["ws_state"] = "대상없음"
                    STATE["ws_last_error"] = "구독 symbol 없음"
                _stop_event.wait(2.0)
                continue
            key = ",".join(symbols)
            # asyncio.run은 세션 단위로 짧게 돈다. daemon thread라 systemd stop을 막지 않는다.
            asyncio.run(_ws_direct_session(symbols, target_set, key))
            last_error_sleep = 1.0
        except Exception as exc:
            with _state_lock:
                STATE["ws_state"] = "재연결대기"
                STATE["ws_last_error"] = f"worker {exc.__class__.__name__}: {str(exc)[:120]}"
            log_error("direct_websocket_worker", exc)
        _stop_event.wait(min(max(1.0, last_error_sleep), WS_HUB_RESTART_SEC))
        last_error_sleep = min(WS_HUB_RESTART_SEC, last_error_sleep * 1.5)
    with _state_lock:
        STATE["ws_state"] = "종료"
    log("direct websocket_hub_worker stopped")
...
def ws_snapshot(ticker: Any) -> Dict[str, Any]:
    ...
    if not row:
        return {"live_price_source": "REST", "live_price": 0.0, "live_age_sec": -1, "ws_fresh": False, "ws_turnover": 0.0, "ws_volume": 0.0}
    age = nowv - fnum(row.get("ts"), nowv)
    fresh = age <= WS_HUB_STALE_SEC
    return {
        "live_price_source": "WS" if fresh else "WS_STALE",
        "live_price": fnum(row.get("live_price"), 0.0),
        "live_age_sec": round(age, 2),
        "ws_fresh": fresh,
        "ws_turnover": fnum(row.get("ws_turnover"), 0.0),
        "ws_volume": fnum(row.get("ws_volume"), 0.0),
        "ws_symbol_format": row.get("symbol_format", "-"),
    }
...
def apply_ws_cache_to_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    fresh = 0
    ws_used = 0
    row_tickers = {_ws_market_code_to_ticker(r.get("ticker")) for r in (rows or [])}
    nowv = now_ts()
    for r in rows or []:
        rr = dict(r)
        ...
        if ws.get("ws_fresh") and fnum(ws.get("live_price"), 0) > 0:
            ...
            if live > 0:
                rr["ws_price"] = live
                rr["live_price_realtime"] = live
                rr["current_price_ws_gap_pct"] = round(((live - rest_price) / rest_price) * 100.0, 3) if rest_price > 0 else 0.0
                # 실시간 현재가는 허브에서 1순위로 본다. REST 가격은 rest_price로 보존한다.
                rr["rest_price"] = rest_price
                rr["current_price"] = live
                rr["price"] = live
                ws_used += 1
        out.append(rr)
    with _ws_lock:
        cached_keys = set(_ws_live_cache.keys())
    unmatched = sorted(list(cached_keys - row_tickers))[:8]
    with _state_lock:
        STATE["ws_fresh"] = fresh
        STATE["ws_fresh_fail"] = max(0, int(STATE.get("ws_cached", 0) or 0) - fresh)
        STATE["ws_match_ok"] = fresh
        STATE["ws_unmatched_sample"] = unmatched
        STATE["ws_price_used"] = ws_used
        if _ws_last_msg_ts > 0:
            STATE["ws_last_age_sec"] = round(nowv - _ws_last_msg_ts, 1)
    return out


def ws_diag_short() -> str:
    return f"파싱 {STATE.get('ws_parse_ok',0)} / 가격 {STATE.get('ws_price_ok',0)} / 금액 {STATE.get('ws_amount_ok',0)} / 매칭 {STATE.get('ws_match_ok',0)} / 빈파싱 {STATE.get('ws_parse_empty',0)} / 형식 {STATE.get('ws_last_format','-')}"


def websocket_status_text() -> str:
    age = STATE.get("ws_last_age_sec", -1)
    age_txt = f"{age}초" if fnum(age, -1) >= 0 else "-"
    unmatched = STATE.get("ws_unmatched_sample") if isinstance(STATE.get("ws_unmatched_sample"), list) else []
    lines = [
        "🌐 웹소켓 상태 /ws_status",
        websocket_status_line(),
        f"- 진단: {ws_diag_short()}",
        f"- 마지막: symbol {STATE.get('ws_last_symbol','-')} / keys {STATE.get('ws_last_keys','-')}",
        f"- 가격사용: rows에 WS 적용 {STATE.get('ws_price_used',0)}개 / 최근수신 {age_txt}",
        f"- 미매칭 샘플: {', '.join(unmatched[:6]) if unmatched else '-'}",
        "- 원칙: WS는 실시간 주 데이터, REST는 전체시장/누락 보정",
    ]
    return "\n".join(lines)
...
def websocket_status_line() -> str:
    ...
    if state == "수신중" and int(STATE.get("ws_fresh", 0) or 0) > 0:
        icon = "✅"
    elif state in {"수신중", "연결"}:
        icon = "⚠️"
    elif state in {"OFF", "대기", "대상대기", "재연결대기", "-"}:
        icon = "❔"
    else:
        icon = "⚠️"
    return f"{icon} 웹소켓 허브: {STATE.get('ws_state','-')} / 대상 {STATE.get('ws_targets',0)} / 캐시 {STATE.get('ws_cached',0)} / 신선 {STATE.get('ws_fresh',0)} / 최근 {age_txt} / {ws_diag_short()} / 오류 {err}"
...
def precision_priority(row: Dict[str, Any]) -> float:
    ...
    v177: 정보 오래됨이 남으면 후보품질 검증을 믿기 어렵다.
    그래서 상위/급변뿐 아니라 오래된 정밀값도 우선 갱신한다.
    """
    ...
    if not cached or not cached.get("precision_ok"):
        ...
        age_score = min(900.0, max(0.0, now_ts() - fnum(cached.get("precision_ts"), 0)))
    return -(rank_score * 3.0 + move * 80.0 + min(turnover / 10_000_000, 200.0) + age_score * 1.2)
...
def build_execution_risk(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # v177: 웹소켓 보조허브가 신선하면 가격재확인에는 WS를 우선 사용한다.
    # WS가 없거나 오래되면 기존 REST 단건 조회로 내려간다.
    ws = ws_snapshot(t)
    live_source = str(ws.get("live_price_source") or "REST")
    live = fnum(ws.get("live_price"), 0) if ws.get("ws_fresh") else 0.0
    if live <= 0:
        live = fetch_single_price_public(t, timeout=1.3) or detected
        live_source = "REST" if live > 0 else "정보없음"
...
def execution_risk_snapshot(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "live_price_source": str(ws_snapshot(t).get("live_price_source") or "REST"),
        "price_recheck_pct": 0.0,
...
def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:
    ...
    # v177: 최근 표본 기준으로 손실이 반복된 유형은 paper OPEN만 보류하고 strict관찰로 남긴다.
    if price_recheck < TRADE_READY_MIN_PRICE_RECHECK_PCT:
        reasons.append(f"가격재확인 음수 {price_recheck:.2f}%")
    if price_recheck > TRADE_READY_MAX_PRICE_RECHECK_PCT:
        reasons.append(f"알림가 대비 급등 {price_recheck:.2f}%")
    volume_spike = fnum(item.get("volume_spike_30x"), 0)
    if volume_spike > TRADE_READY_MAX_VOLUME_SPIKE_30X:
        reasons.append(f"거래량 과폭발 {volume_spike:.2f}배")
    money1 = fnum(item.get("money_flow_1m"), 0)
    money3 = fnum(item.get("money_flow_3m"), 0)
    if money1 > 0 and money3 > 0 and (money1 / max(money3, 1.0)) > TRADE_READY_MAX_1M_TO_3M_RATIO and price_recheck <= 0:
        reasons.append("1분 과열/3분 지속 부족")
    pullback_q = fnum(item.get("pullback_quality_score"), 0)
    if pullback_q > 0 and pullback_q < TRADE_READY_MIN_PULLBACK_QUALITY:
        reasons.append(f"눌림품질 낮음 {pullback_q:.2f}")
    reb = fnum(item.get("rebreakout_strength"), 0)
    if reb > 0 and reb < TRADE_READY_MIN_REBREAKOUT:
        reasons.append(f"재돌파힘 약함 {reb:.2f}")
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "live_price_source": item.get("live_price_source", item.get("live_price_source", "REST")),
        ...
        "pipeline_note": "v2.13.179: 웹소켓 중심 허브 재정렬 + 예전 티커 변환/파싱 복구 + REST 안전망 유지.",
    }
...
def scan_once() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        stage_times.append(("1) 웹소켓 중심 허브 + REST 전체시장 보정", now_ts() - st, f"rows {len(rows)} / {source} / ws {STATE.get('ws_state','-')} fresh {STATE.get('ws_fresh',0)} / {ws_diag_short()}"))
        with _state_lock:
        ...
        stale_count = sum(1 for r in rows if r.get("precision_source") == "candlestick_1m" and fnum(r.get("precision_age_sec"), 0) > FRESH_WEAK_SEC)
        risk_missing = max(0, len(rows) - int(STATE.get("execution_risk_cached", 0) or 0))
        major_market_count = sum(1 for r in rows if str(r.get("ticker") or "").upper() in MAJOR_WATCH_TICKERS)
        with _state_lock:
            STATE["field_coverage"] = cov
            STATE["precision_stale_count"] = stale_count
            STATE["risk_missing_count"] = risk_missing
            STATE["major_market_count"] = major_market_count
            STATE["scan_confidence"] = compute_scan_confidence(STATE)
            STATE["scan_last_stage"] = "worker2_strategy"

        st = now_ts()
        strict, shadow, rejects, examples = build_candidates(rows)
        stage_times.append(("5) 3~5차 직원: 재돌파·실전위험·등급분류", now_ts() - st, f"strict {len(strict)} / shadow {len(shadow)} / major후보 {sum(1 for r in (strict + shadow) if r.get('major_watch'))} / reject {sum(rejects.values())}"))
        with _state_lock:
        ...
        stage_times.append(("6) 공장: latest 후보파일 + entry_context 저장", now_ts() - st, f"archive strict {pipe['paper_written']}/{pipe['paper_attempt']} / latest strict {pipe.get('paper_latest_written',0)} / trade_ready {pipe.get('latest_trade_ready',0)} / 관찰 {pipe.get('latest_strict_observe',0)} / major {pipe.get('major_watch_written',0)} / shadow latest {pipe.get('shadow_latest_written',0)} / dup {pipe['dup_skip']}"))
        ...
        with _state_lock:
            ...
            STATE["major_watch_count"] = sum(1 for r in (strict + shadow) if r.get("major_watch"))
...
def compute_scan_confidence(s: Optional[Dict[str, Any]] = None) -> str:
    s = s or STATE
    cov = s.get("field_coverage", {}) if isinstance(s.get("field_coverage"), dict) else {}
    rows = max(1, int(s.get("bulk_rows", 0) or cov.get("rows", 0) or 1))
    precision = int(cov.get("precision", 0) or 0)
    ema = int(cov.get("ema12", 0) or 0)
    risk = int(s.get("execution_risk_cached", 0) or 0)
    stale = int(s.get("precision_stale_count", 0) or 0)
    ws_fresh = int(s.get("ws_fresh", 0) or 0)
    pct = min(100.0, (precision / rows) * 100.0)
    if precision >= 420 and risk >= 400 and stale <= 10:
        icon, grade = "✅", "높음"
    elif precision >= 360 and risk >= 340 and stale <= 40:
        icon, grade = "✅", "양호"
    elif precision >= 300:
        icon, grade = "⚠️", "보강중"
    else:
        icon, grade = "❔", "낮음"
    return f"{icon} 스캔신뢰도 {grade}: 정밀 {precision}/{rows}({pct:.0f}%) / 보조지표 {ema} / 위험 {risk} / 오래됨 {stale} / WS신선 {ws_fresh}"
...
def field_line() -> str:
    ...
    return (
        ...
        f"보조지표 ema {cov.get('ema12',0)} bb {cov.get('bb',0)} mfi {cov.get('mfi',0)} stoch {cov.get('stoch',0)} adx {cov.get('adx',0)}"
    )
...
def candidate_line() -> str:
    ...
    state_part = (
        f"판단 strict {STATE.get('strict_decision',0)} / shadow {STATE.get('shadow_decision',0)} "
        f"→ archive신규 {STATE.get('paper_written',0)} / trade_ready {STATE.get('trade_ready_written',0)} / 관찰 {STATE.get('strict_observe_written',0)} / "
        f"major {STATE.get('major_watch_written',0)} / 복기 {STATE.get('shadow_written',0)}"
    )
    latest_part = f"latest 정식 {latest.get('paper_latest',0)}(현재scan {STATE.get('paper_latest_written', latest.get('paper_latest',0))}) / 복기 {latest.get('shadow_latest',0)}"
    scan = scan_status_summary()
    note = ""
    if scan.get("running"):
        note = " / 현재 scan 진행중이라 판단값과 latest가 다를 수 있음"
    return (
        f"- 후보파일: {state_part} / {latest_part} / 중복skip {STATE.get('dup_skip',0)}({STATE.get('dup_skip_reason',{})}) / "
        f"candidate_events 없음 / 최근 {STATE.get('last_ticker','-')}{note}{err_part}"
    )

def stage_lines() -> List[str]:
    rows = STATE.get("stage_times") if isinstance(STATE.get("stage_times"), list) else []
    out = ["🧩 v2.13.179 scan 단계표"]
    if not rows:
...
def core_text() -> str:
    ...
    return "\n".join([
        "📌 종합 상태판 /core",
        f"{scan['icon']} 메인봇: {BOT_VERSION} / scan {scan['label']} / stage {scan.get('stage','-')}",
        f"✅ 시장: bulk {s.get('bulk_rows',0)} / 가격 {s.get('bulk_price',0)} / 돈 {s.get('bulk_money',0)} / rank {s.get('field_coverage',{}).get('rank', s.get('bulk_rows',0))}",
        precision_status_note(s),
        f"✅ 직원: 2차 눌림·3차 재돌파·4차 위험·5차 분류 / major후보 {s.get('major_watch_count',0)} / 대형주시장 {s.get('major_market_count',0)}",
        compute_scan_confidence(s),
        f"🔎 정보커버리지: 정밀 {s.get('field_coverage',{}).get('precision',0)} / ema {s.get('field_coverage',{}).get('ema12',0)} / bb {s.get('field_coverage',{}).get('bb',0)} / mfi {s.get('field_coverage',{}).get('mfi',0)} / stoch {s.get('field_coverage',{}).get('stoch',0)} / adx {s.get('field_coverage',{}).get('adx',0)} / 위험캐시 {s.get('execution_risk_cached',0)}",
        websocket_status_line(),
        candidate_line(),
        paper_bot_line(),
        guard_summary_line(),
        "",
        "판독",
        "✅ 전체시장 스캔 유지 / candidate_events 없음 / 자동매수 OFF",
        "⚠️ 정밀값이 낮으면 후보품질 판단보다 정밀직원 보강/오류부터 확인",
        "⚠️ 성과 판단은 trade_ready 신규 결과 중심",
    ])
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.179 기준: 웹소켓 실시간 주 데이터 / REST 전체시장 보정 / 후보파일 저장 유지",
...
def command_flow(update, context) -> None:
    lines = [
        ...
        "",
        "현재 고정 본선",
        "1) 웹소켓 허브: 실시간 가격/체결/신선도 주 데이터",
        "2) REST 허브+1차 직원: 전체 KRW 목록/rank/24h 거래대금/누락 보정",
        "3) 2차 직원: 캔들 기반 눌림/지표 보강",
        "4) 3차 직원: WS 현재가로 재돌파/가격재확인",
        "5) 4~5차 직원: 호가위험/등급분류/trade_ready",
        "6) 공장: paper_candidates / shadow_candidates + entry_context 저장",
        "7) paper_bot: trade_ready만 OPEN, 관찰/shadow는 복기",
        "",
        field_line(),
        candidate_line(),
        "",
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.179.py",
        "- 페이퍼봇 대상: paper_bot_v0.33.py",
...
def check_text() -> str:
    ...
    return "\n".join([
        ...
        f"- 후보: 판단 strict {STATE.get('strict_decision',0)} / latest {STATE.get('paper_latest_written', line_count(FILES['paper_latest']))} / trade_ready {STATE.get('latest_trade_ready',0)} / 관찰 {STATE.get('latest_strict_observe',0)} / 복기 {STATE.get('shadow_latest_written',0)}",
...
def score_text() -> str:
    """짧은 성과판. 기준은 paper_bot 신규기준으로 통일한다."""
    rows = load_closed()
    ...
    strict_all = [r for r in rows if str(r.get("lane")) == "strict"]
    ...
    lines = [
        ...
        "",
        "[1/5] 전적",
        fmt_stats("신규 strict", new_strict),
        fmt_stats("신규 shadow 참고", new_shadow),
        fmt_stats("누적 strict 참고", strict_all),
        paper_bot_line(),
        "",
        "[2/5] 손실/익절 원인",
    ]
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    lines = [
        ...
        "",
        "[1/5] 전적",
        fmt_stats("신규 strict", strict),
        f"- 현재 후보: strict {len(latest_strict)}개 / trade_ready {len(trade_ready)}개 / 관찰 {len(observe)}개 / shadow {len(latest_shadow)}개",
        paper_bot_line(),
        "",
        "[2/5] 손실 원인 TOP",
    ]
    ...
    lines += [
        ...
        *reject_lines(limit=8 if full else 4),
        "",
        "[5/5] 판독",
        "❌ 지금 성과는 자동매매 불가" if score_stats(strict).get("avg", 0) < 0 else "✅ 신규 strict 평균은 +권",
        "⚠️ 우선 볼 것: slow_no_progress / stop_loss 줄이기",
        "✅ 좋아 보이는 재료: 3분 지속 돈흐름, 눌림품질, 가격재확인",
        "❌ 위험해 보이는 재료: 1분만 큰 돈흐름, 거래량 과폭발, 가격재확인 음수",
        ("✅ 표본이 어느 정도 쌓임: trade_ready 관찰이동 기준을 검토 가능" if enough else "❔ entry_context 표본 더 필요: 조건 대수정은 보류"),
    ]
    if full:
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.179.py", "- 페이퍼봇 대상: paper_bot_v0.33.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.179: 독립 direct WS worker + pybithumb WS 제거 + REST 커버리지 보호", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.179 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def _builder_for_command(name: str):
    if not TEXT_COMMAND_BUILDERS:
        TEXT_COMMAND_BUILDERS.update({
            ...
            "check": check_text, "check_full": check_full_text, "bot_status": bot_status_text, "ws_status": websocket_status_text, "quality": lambda: candidate_quality_text(False), "quality_full": lambda: candidate_quality_text(True),
            "flow_check": lambda: "\n".join(["🧭 흐름검사 /flow_check", field_line(), candidate_line()] + stage_lines()),
            "candidate_reason": lambda: "\n".join(["🔎 후보·전략 원인판 /candidate_reason", field_line(), candidate_line()] + reject_lines()),
            "cpu_status": lambda: "\n".join(["🧠 CPU/메모리 /cpu_status", f"- scan최근 {fnum(STATE.get('scan_last_sec'),0):.2f}s / 최대 {fnum(STATE.get('scan_max_sec'),0):.2f}s / stage {STATE.get('scan_last_stage','-')}"] + stage_lines()),
            ...
            "help": lambda: "📚 명령어 /help\n- 여러 명령을 줄바꿈으로 보내도 자동 묶음 처리됨\n- /batch /check /core /bot_status /ws_status /quality /score /cpu_status /paper_handoff /deploy /errorlog_full",
        })
...
def command_help(update, context) -> None:
    reply(update, "\n".join([
        ...
        "흐름/후보",
        "- /flow_check /candidate_reason /ws_status /paper_handoff /deploy",
        "안전/오류",
...
def install_commands(updater: Any) -> None:
    ...
    try:
        menu = ["batch", "check", "check_full", "core", "bot_status", "ws_status", "quality", "quality_full", "score", "paper_today", "flow_check", "candidate_reason", "cpu_status", "paper_handoff", "paper_handoff_full", "deploy", "trade", "errorlog", "errorlog_full", "help"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v179 독립 웹소켓 worker + REST 본선 보호 + WS 진단표 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
