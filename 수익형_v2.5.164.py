# v2.5.164: review_dump 전 섹션 상세 통일 + 명령어 no-scan 점검
import os
import time
import json
import math
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- review_dump 전 섹션 상세 통일: 미리볼/실시간급등/내부감시도 역추적급으로 표시",
...
def format_internal_watch_tracking_report(limit: int = 8) -> str:
    active, logs = _get_internal_watch_state_parts()
    ...
    lines.append(f"- 활성 추적 {len(active_rows)}개 / 완료 기록 {len(log_rows)}개")
    if active_rows:
        lines.append("")
        lines.append("[진행 중]")
...
def process_missed_review_hourly_report(now_ts: float = None):
    ...
    header_lines = [
        ...
        f"- 탐색 놓침 {summary.get('탐색 놓침', 0)}건 / 승격 놓침 {summary.get('승격 놓침', 0)}건 / 늦어서 안 올림 {summary.get('늦어서 안 올림', 0)}건 / 분류 놓침 {summary.get('분류 놓침', 0)}건 / 데이터 실패 {summary.get('데이터 실패', 0)}건",
        "",
        f"[최근 {len(recent_rows)}건]",
    ]
    blocks = [format_missed_review_log_block(row, idx=idx) for idx, row in enumerate(recent_rows, start=1)]
    remain = len(rows) - len(recent_rows)
    if remain > 0:
        header_lines += ["", f"- 그외 {remain}건은 /missed 에서 더 볼 수 있어"]

    send_report_blocks(header_lines, blocks, title_prefix="자동요약")
    missed_review_meta["last_hourly_cutoff_ts"] = current_hour_start
    persist_missed_review_state()
...
def append_missed_review_parts(parts: list, limit: int = 2):
    rows = get_all_missed_review_rows(include_supplemental=True, supplemental_limit=12)[-max(1, int(limit)):]
    if not rows:
        return
    rows = list(reversed(rows))
    lines = ["📌 최근 놓친 코인"]
...
def missed_command(update, context: CallbackContext):
    ...
    if rows:
        ...
        for idx, row in enumerate(reversed(recent), start=1):
            blocks.append(format_missed_review_log_block(row, idx=idx))
    ...
    if alert_rows:
        ...
        for idx, row in enumerate(alert_rows[:5], start=1):
            blocks.append(format_alert_review_block(row, idx=idx))

    send_report_blocks(header_lines, blocks, title_prefix="상세")

def compare_command(update, context: CallbackContext):
    alert_rows = collect_recent_alert_review_rows(limit=5, summary_limit=20)
    missed_rows = get_all_missed_review_rows(include_supplemental=True, supplemental_limit=12)
...
def format_review_dump_saved_chart_detail(item: dict, idx: int = 1, group_label: str = "후보") -> str:
    ...
    lines = [
        ...
        f"- 돈/주도: 거래량 {safe_float(item.get('vol_ratio',0),0):.2f}배 / 시장주목 {safe_float(item.get('leader_score',0),0):.2f} / 최근5분거래대금 {fmt_price(item.get('turnover_5m', item.get('turnover_recent_5m', 0)))} / 1분 {fmt_price(item.get('turnover_recent_1m',0))} / 3분 {fmt_price(item.get('turnover_recent_3m',0))}",
        f"- 위치: 30분저점대비 {safe_float(item.get('from_low', item.get('from_30m_low_pct',0)),0):+.2f}% / 최근30분고점거리 {safe_float(item.get('below_high', item.get('below_30m_high_pct',999)),999):.2f}% / 단기고점거리 {safe_float(item.get('rebreakout_gap_pct', item.get('high_gap_pct',999)),999):.2f}% / RSI {safe_float(item.get('rsi',0),0):.2f}",
    ]
...
def build_review_dump_current_candidates_detail(scan_display_at_start=None, limit_each: int = 8, detail_index: dict = None) -> str:
    ...
    def append_group(title: str, rows, group_label: str, max_rows: int):
        ...
        if len(rows) > max_rows:
            lines.append(f"- 나머지 {len(rows) - max_rows}개는 메시지 길이 때문에 생략. 필요하면 /scan_debug와 다음 review_dump 표본으로 확인")
    append_group("🌱 [바로 볼 후보 상세]", user_items, "바로 볼 후보", limit_each)
    append_group("👀 [조금 더 볼 후보 상세]", internal_items, "조금 더 볼 후보", limit_each)
    append_group("🙈 [내부에서만 넘긴 후보 상세]", discard_items, "내부 넘김/버릴 후보", max(3, min(limit_each, 5)))
    if not user_items and not internal_items and shortlist:
        append_group("📌 [상위 shortlist 상세]", shortlist, "shortlist", limit_each)
    return "\n".join(lines)

def build_review_dump_text(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5) -> str:
...
def build_review_dump_sections(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5):
    sections = []
    try:
        scan_display_at_start = get_latest_scan_display_state(now_ts=time.time(), limit=12)
    except Exception:
    ...
    try:
        sections.append(("scan_debug", build_review_dump_current_candidates_detail(scan_display_at_start, limit_each=8, detail_index=detail_index)))
    except Exception as e:
        sections.append(("scan_debug", f"[현재 후보 상세 / 차트 정교화 전체 표시]\n- 생성 오류: {e}"))

    try:
        sections.append(("internal_watch_tracking", format_internal_watch_tracking_report(limit=8)))
    except Exception as e:
    ...
    try:
        alert_rows = collect_recent_alert_review_rows(limit=alert_limit, summary_limit=20)
        if not alert_rows:
    ...
    try:
        missed_rows = get_all_missed_review_rows(include_supplemental=True, supplemental_limit=max(missed_limit, 8))
        if not missed_rows:
            ...
            lines = [
                ...
                "",
                f"[최근 놓친 {min(len(missed_rows), missed_limit)}건]",
            ]
    ...
    try:
        sections.append(("internal_failures", collect_internal_failure_summary_for_review_dump(limit=8)))
    except Exception as e:
