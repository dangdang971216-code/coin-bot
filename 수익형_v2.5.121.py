...
def build_pseudo_htf_from_minute60(ticker: str, chunk: int = 4):
    try:
        df60 = get_ohlcv(str(ticker or "").upper().strip(), "minute60")
        if df60 is None or len(df60) < max(chunk * 3, 8):
            return None
        rows = []
        idx = []
        usable = len(df60) - (len(df60) % chunk)
        if usable < chunk * 3:
            return None
        work = df60.tail(usable)
...
def compute_htf_setup_context(ticker: str, current_price: float) -> dict:
    ...
    try:
        used_fallback = False
        df = get_ohlcv(str(ticker or "").upper().strip(), ALERT_HTF_INTERVAL)
        if (df is None or len(df) < 5) and current_price > 0:
            fallback_df = build_pseudo_htf_from_minute60(ticker)
            if fallback_df is not None and len(fallback_df) >= 3:
                df = fallback_df
                used_fallback = True
        if df is None or len(df) < 3 or current_price <= 0:
            return ctx
        ...
        ma7v = ma(df, 7)
        ma30v = ma(df, 30)
        ma7_prev = ma(df.iloc[:-3], 7) if len(df) >= 10 else ma(df.iloc[:-1], 7)
        ma30_prev = ma(df.iloc[:-3], 30) if len(df) >= 33 else ma(df.iloc[:-1], 30)
        ...
        elif abs(ma7_slope_pct) < 0.05 and abs(ma30_slope_pct) < 0.03:
            trend_label = "4시간 횡보"
        label = "4시간봉 애매"
        if len(df) < 5:
            label = "4시간봉 표본 적음"
        ...
        ctx.update({
            ...
            "htf_data_source": "minute60x4" if used_fallback else "minute240",
        })
    except Exception:
        return ctx
    return ctx
...
def compute_rebreakout_structure_context(ticker: str, current_price: float, df1=None) -> dict:
    ...
    try:
        ...
        for row in recent_rows:
            ...
            if touch:
                attempts += 1
                rejected = (cl < prev_high * 0.992) or (close_pos <= 0.35)
                if rejected:
                    failed_attempts += 1
            if hold_05:
        ...
        rebreakout_success_rate = min(1.0, hold_near_high_bars / max(attempts, 1)) if attempts > 0 else 0.0
        score = 0.0
        structure_score = 0.0
        if gap_pct <= 1.40:
            score += 0.55
