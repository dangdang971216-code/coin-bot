# v2.5.157: 공격성 복구 + 실패/검증 문구 정리 + 데이터 실패 노출 축소
import os
import time
import json
import math
...
def get_quick_promotion_block_reason(item: dict, target_display_key: str = "", now_ts: float = None) -> str:
    ...
    if chg_30 < -1.20 and from_low < 0.90 and change_delta < QUICK_PROMOTION_MIN_CHANGE3:
    ...
    if prev_strategy == "PREWATCH" and (chg_5 < 0.52 or change_delta < QUICK_PROMOTION_MIN_CHANGE3 or (vol_delta < 0.30 and leader_delta < 0.22 and score_delta < 0.45)):
...
def get_shortlist_user_rescue_display_key(item: dict, display_key: str = "", regime: dict = None) -> str:
    ...
    if chart_rise > 5.10 and high_gap_pct < 1.25 and not top_turnover_rescue:
        return ""
    if edge < USER_ALERT_RESCUE_MIN_EDGE - 0.25 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG and not top_turnover_rescue and drought_sec < 5400:
        return ""
    if vol_ratio < USER_ALERT_RESCUE_MIN_VOL - 0.08 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG and not top_turnover_rescue and drought_sec < 5400:
        return ""
    if leader < USER_ALERT_RESCUE_MIN_LEADER - 0.08 and hot_score < USER_ALERT_RESCUE_STRONG_HOT_SCORE and shortlist_rank > USER_ALERT_MIN_SHORTLIST_RANK_STRONG and not top_turnover_rescue and drought_sec < 5400:
        return ""
    ...
    if shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG and high_gap_pct >= 0.68 and motion >= 0.05 and chart_rise <= 4.35:
...
def get_user_alert_block_reason(item: dict, display_key: str = "") -> str:
    ...
    if strategy in {"PREWATCH", "NEWLOW_RECOVER"} and USER_ALERT_PREWATCH_STRICT_ON:
        prewatch_rescue_ok = (
            shortlist_rank <= USER_ALERT_MIN_SHORTLIST_RANK_STRONG + 1
