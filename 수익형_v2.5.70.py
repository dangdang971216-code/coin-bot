...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'strong':
        ...
        # 알림이 너무 마르면 직접 잡힌 실시간/초반형만 아주 조금 살린다.
...
def _rescue_internal_watch_items(items):
    ...
    for item in internal_sorted:
        ...
        if primary_reason == '실시간 급등 초입':
            rescue_ok = rescue_ok and change_5 >= 0.98 and change_3 >= 0.08 and vol_ratio >= 2.85 and leader >= 1.00 and below_high >= 1.90 and turnover_5m >= 30000000 and change_30 >= -0.25
            if drought_sec >= 7200:
                rescue_ok = rescue_ok and change_5 >= 0.88 and vol_ratio >= 2.65 and below_high >= 1.65 and turnover_5m >= 26000000
            if not good_market:
                rescue_ok = rescue_ok and turnover_5m >= 40000000 and change_5 >= 1.25 and below_high >= 2.40 and change_30 >= 0.00
        elif primary_reason == '초반 선점형':
            rescue_ok = rescue_ok and change_5 >= 0.82 and change_3 >= 0.04 and vol_ratio >= 2.45 and leader >= 0.92 and below_high >= 1.75 and turnover_5m >= 26000000 and from_low <= 1.70
            if drought_sec >= 7200:
                rescue_ok = rescue_ok and change_5 >= 0.74 and vol_ratio >= 2.20 and below_high >= 1.55 and turnover_5m >= 22000000
