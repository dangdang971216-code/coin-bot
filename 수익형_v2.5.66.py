...
FAST_SCAN_RESCUE_MIN_TURNOVER_3M = 11000000
FAST_SCAN_RESCUE_MIN_TURNOVER_5M = 18000000
FAST_SCAN_RESCUE_LIMIT = 2
FAST_SCAN_RESCUE_SCORE_MIN = 12.6
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'watch':
        ...
        rescue_like = (
            ...
            and change_5 >= 0.42 and vol_ratio >= 1.90 and leader_score >= 0.70
            and 0.20 <= from_low <= 2.50 and below_high >= 0.95
            and lite_score >= FAST_SCAN_RESCUE_SCORE_MIN and rsi < 81
        ...
        if restart_like:
            if (
                change_5 >= 0.58 and vol_ratio >= 2.10 and leader_score >= 0.78
                and from_low >= 0.45 and below_high >= 1.10 and lite_score >= 12.8
                and turnover_rescue_ok and not chart_bad
            ):
                return 'A', 'user_alert'
...
def _rescue_internal_watch_items(items):
    ...
    for item in internal_sorted:
        ...
        rescue_ok = (
            ...
            and leader >= 0.65
            and below_high >= 0.85
            and 0.15 <= from_low <= 2.60
            and turnover_5m >= FAST_SCAN_RESCUE_MIN_TURNOVER_5M * 0.72
        )
        if primary_reason == '실시간 급등 초입':
            rescue_ok = rescue_ok and change_5 >= 0.48 and vol_ratio >= 1.95 and below_high >= 0.95
        elif primary_reason == '재출발 확인 후보':
            rescue_ok = rescue_ok and from_low >= 0.35 and below_high >= 1.00
        if not rescue_ok:
            continue
        new_item = dict(item)
