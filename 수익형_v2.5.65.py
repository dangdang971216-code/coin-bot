...
# =========================================================
# v2.5.65 fast 경로 회귀복구 + 승격 기준/중복 억제 보강
# =========================================================
FAST_SCAN_ALERT_LIMIT = 2
FAST_SCAN_ALERT_COOLDOWN_SEC = 1200
FAST_SCAN_ALERT_REPEAT_PRICE_MOVE_PCT = 2.20
FAST_SCAN_ALERT_REPEAT_SCORE_JUMP = 7.5
FAST_SCAN_ALERT_REPEAT_VOL_JUMP = 3.5
...
FAST_SCAN_RESCUE_MIN_TURNOVER_3M = 14000000
...
def _fast_rescue_score(item: dict) -> float:
    return (
        ...
        + max(safe_float(item.get('change_5', 0), 0), 0) * 3.0
        + max(safe_float(item.get('vol_ratio', 0), 0) - 1.0, 0) * 2.2
        + safe_float(item.get('leader_score', 0), 0) * 2.3
        + min(safe_float(item.get('turnover_5m', 0), 0) / 12000000.0, 4.0)
    )
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'strong':
        if (
            primary_reason == '실시간 급등 초입'
            and change_5 >= 0.95 and change_3 >= 0.20
            and vol_ratio >= 2.60 and leader_score >= 1.15
            and 0.12 <= from_low <= 1.95 and below_high >= 1.15
            and rsi < 80 and turnover_s_ok and not chart_bad
        ):
            return 'S', 'user_alert'
        if (
            primary_reason in {'실시간 급등 초입', '초입 가능 후보'}
            and change_5 >= 0.58 and change_3 >= 0.08
            and vol_ratio >= 1.85 and leader_score >= 0.95
            and 0.12 <= from_low <= 1.85 and below_high >= 1.30
            and lite_score >= 12.0 and rsi < 82 and turnover_a_ok and not chart_bad
        ):
            return 'A', 'user_alert'
        if turnover_watch_ok and change_5 >= 0.18 and vol_ratio >= 1.10 and below_high >= 0.80:
            return 'B', 'internal_watch'
        return 'C', 'discard'
    ...
    if section == 'watch':
        ...
        rescue_like = (
            ...
            and change_5 >= 0.62 and vol_ratio >= 2.35 and leader_score >= 0.90
            and 0.40 <= from_low <= 2.20 and below_high >= 1.50
            and lite_score >= FAST_SCAN_RESCUE_SCORE_MIN and rsi < 80
            and turnover_rescue_ok and not chart_bad
        )
        if rescue_like:
            return 'A', 'user_alert'
        if restart_like:
            if (
                change_5 >= 0.78 and vol_ratio >= 2.70 and leader_score >= 0.95
                and from_low >= 0.70 and below_high >= 1.70 and lite_score >= 15.4
                and turnover_rescue_ok and not chart_bad
            ):
                return 'A', 'user_alert'
            if turnover_watch_ok and change_5 >= 0.18 and from_low >= 0.25 and below_high >= 0.90:
                return 'B', 'internal_watch'
            return 'C', 'discard'
        if turnover_watch_ok and change_5 >= 0.15 and vol_ratio >= 1.08 and leader_score >= 0.38 and below_high >= 0.65:
            return 'B', 'internal_watch'
        return 'C', 'discard'
...
def _rescue_internal_watch_items(items):
    ...
    for item in internal_sorted:
        ...
        if _fast_rescue_score(item) < FAST_SCAN_RESCUE_SCORE_MIN:
            continue
        if not _fast_abs_turnover_ok(item, 'rescue') or _fast_chart_bad(item):
            continue
        change_5 = safe_float(item.get('change_5', 0), 0)
        vol_ratio = safe_float(item.get('vol_ratio', 0), 0)
        leader = safe_float(item.get('leader_score', 0), 0)
        below_high = safe_float(item.get('below_high', 999), 999)
        from_low = safe_float(item.get('from_low', 0), 0)
        if change_5 < 0.55 or vol_ratio < 2.20 or leader < 0.85 or below_high < 1.35 or not (0.20 <= from_low <= 2.30):
            continue
        new_item = dict(item)
        ...
        new_item['one_liner'] = str(new_item.get('one_liner', '') or '지금은 흐름만 확인') + ' / 상위 감시 후보에서 다시 올림'
...
def build_scan_debug_text():
    ...
    append_group('⛔ 지금은 넘길 후보', discard_items)
    return '\n'.join(lines)
...
def append_debug_shortlist_parts(parts: list):
    try:
        ...
        for item in shortlist:
            bucket = str(item.get('alert_bucket', '') or '')
            label = '바로 볼 후보' if bucket == 'user_alert' else ('조금 더 볼 후보' if bucket == 'internal_watch' else '넘길 후보')
