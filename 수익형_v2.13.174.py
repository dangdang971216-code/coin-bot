...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v174: 대형 재료 보강판. 허브/1~5차 직원/공장으로 나눠 돈흐름·눌림품질·재돌파·무료전략 보조지표·실전위험을 기록한다.",
}
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.174: 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만. shadow는 복기전용.",
    }
...
def candidate_line() -> str:
    ...
    state_part = (
        ...
        f"→ trade_ready {STATE.get('trade_ready_written', STATE.get('paper_written',0))} / 관찰 {STATE.get('strict_observe_written',0)} / "
        f"major {STATE.get('major_watch_written',0)} / 복기 {STATE.get('shadow_written',0)}"
    )
    latest_part = f"latest 정식 {latest.get('paper_latest',0)} / 복기 {latest.get('shadow_latest',0)}"
    scan = scan_status_summary()
    note = ""
    if scan.get("running"):
        note = " / 현재 scan 진행중이라 판단값과 latest가 다를 수 있음"
    return (
        f"- 후보파일: {state_part} / {latest_part} / 중복skip {STATE.get('dup_skip',0)} / "
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.174 기준: 새 본선 / 점검체계 정리 / 신규성과 분리",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.174.py", "- 페이퍼봇 대상: paper_bot_v0.32.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.174: 통합상태 + 후보품질 명령 + 가벼운 점검", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.174 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v174 대형 재료보강 + 직원분리 + latest 후보파일 + shadow 격리 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
