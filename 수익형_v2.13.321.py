...
WORKERS={
    ...
    "strategy": {"file":"strategy_worker_v0.3.py", "status":"clean_strategy_worker_status.json"},
...
def health_text()->str:
    ...
    lines=[
        "🧭 건강상태 /health",
        f"✅ 메인봇 {BOT_VERSION} / clean worker hub v321 / uptime {int(now_ts()-START_TS)}s",
...
def batch_handler(update, context):
    ...
    try:
        send(update, f"📦 자동 묶음 명령 접수\n- 실행 {len(cmds)}개\n- v321: 한 번에 모아 보내지 않고 명령별로 바로 전송")
    except Exception:
        pass
    total=len(cmds)
    for raw in cmds:
    ...
    send(update, "🧾 자동 묶음 시간표 · v321 명령별 전송\n"+"\n".join(timings)+f"\n- 합계 {time.time()-st:.2f}s")
