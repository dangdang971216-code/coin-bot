...
CHART_POS_LIVE_MAX_RISE_PCT = 4.20
CHART_POS_RESTART_MAX_RISE_PCT = 3.40
ALERT_PRICE_RECHECK_ON = True
ALERT_PRICE_RECHECK_ENTRY_MAX_UP_PCT = 0.55
ALERT_PRICE_RECHECK_LIVE_MAX_UP_PCT = 0.40
ALERT_PRICE_RECHECK_RESTART_MAX_UP_PCT = 0.70
...
def is_chart_position_good_for_live(item: dict) -> bool:
    ...
    if 0 <= gap_pct <= 0.45:
        return False
    if rise_pct >= CHART_POS_LIVE_MAX_RISE_PCT and gap_pct <= 1.8:
        return False
    if zone_ratio >= CHART_POS_HIGH_ZONE_RATIO and rise_pct >= 2.2 and gap_pct <= CHART_POS_EXTENDED_NEAR_HIGH_PCT:
        return False
    return True
...
STRATEGY_GUIDE = {
    ...
    "실시간 관심형": "시장 관심이 붙는 중이지만 아직 더 확인이 필요한 자리",
...
def is_low_zone_recovery_candidate(item: dict, judgement: str = "", caution: str = "") -> bool:
    ...
    tags = set(item.get("pattern_tags", []))
    chg_5 = safe_float(snap.get("chg_5m", 0), 0)
    if high_gap_pct <= 0.35:
        return False
    if strategy in ["PREWATCH", "NEWLOW_RECOVER"]:
        return (
            high_gap_pct >= 0.85
            and (
                (chg_5 >= 0.12 and vol_ratio >= 1.55 and max(chg_30, from_low) >= 0.30)
                or (chg_30 >= 0.35 and from_low >= 0.40 and vol_ratio >= 1.45)
                or (leader_score >= 1.35 and vol_ratio >= 1.85 and from_low >= 0.35 and chg_5 >= 0.10)
            )
        )
    if strategy == "LEADER_WATCH":
        return (
            from_low >= 0.18
            and high_gap_pct >= 0.65
            and (
                (chg_30 >= 0.18 and vol_ratio >= 1.30)
                or (chg_5 >= 0.08 and vol_ratio >= 1.25 and from_low >= 0.20)
                or (leader_score >= 1.05 and vol_ratio >= 1.20 and from_low >= 0.20)
            )
        )
    if strategy in ["PREPUMP", "PULLBACK"]:
        return high_gap_pct >= 0.80 and (chg_5 >= 0.10 or chg_30 >= 0.25 or from_low >= 0.40)
...
def get_watch_display_group(item: dict, judgement: str = "", caution: str = "", section_override: str = "", regime: dict = None) -> str:
    ...
    if strategy == "PREPUMP":
        if high_gap_pct >= 1.10 and from_low <= 1.60 and chg_5 >= 0.05 and not is_chart_position_extended(item):
            return "early"
        return "restart" if is_chart_position_good_for_restart(item) else "watch"
    if strategy == "LEADER_WATCH":
        ...
        if vol_ratio >= 1.60:
            live_strength += 1
        if leader_score >= 1.20:
            live_strength += 1
        if max(chg_30, from_low) >= 0.45:
            live_strength += 1
        if change_pct >= 0.65:
            live_strength += 1
        if rise_pct >= 3.60 and high_gap_pct < 2.40:
            live_strength -= 2
        if chg_5 >= 1.80 and max(chg_30, from_low) >= 3.80 and high_gap_pct < 2.60:
            live_strength -= 2
        if live_strength >= 3 and chg_5 >= 0.15:
            return "live"
        return "restart" if is_chart_position_good_for_restart(item) else "watch"
    if strategy in {"TREND_CONT", "BREAKOUT", "CHASE"}:
        if high_gap_pct < 0.45 or not is_chart_position_good_for_live(item):
            return "late"
        if (
            max(chg_5, chg_30, from_low) >= 0.35
            and vol_ratio >= 1.45
            and high_gap_pct >= 0.70
        ):
            return "live"
        return "restart"
    if strategy in {"EARLY", "PRE_BREAKOUT"}:
        return "early" if is_chart_position_good_for_live(item) else "restart"
    if raw_key == "recovery":
        return "restart"
    return "early"
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 신저가 반등/예비 감시형이 진입 후보로 너무 쉽게 올라오지 않게 더 조임",
        "- 재출발 후보도 최근 고점이 너무 가까우면 더 쉽게 빼도록 조정",
        "- 실시간 강세는 살리되 이미 튄 뒤 늦은 코인은 더 세게 차단",
        "- 차트 위치(저점대비/고점거리/상단구역)도 같이 보게 바꿈",
    ...
    for ticker, data in cache.items():
        try:
            ...
            if chart_profile.get("chart_zone_ratio", -1) >= CHART_POS_HIGH_ZONE_RATIO and chart_profile.get("chart_rise_pct", 0) >= 2.2 and chart_profile.get("chart_gap_pct", 999) <= CHART_POS_EXTENDED_NEAR_HIGH_PCT:
                continue
            if chart_profile.get("chart_rise_pct", 0) >= 3.40 and chart_profile.get("chart_gap_pct", 999) <= 2.20 and change_5 >= 1.40:
                continue
            if change_5 >= 1.90 and max(change_3, 0) >= 1.10 and chart_profile.get("chart_gap_pct", 999) <= 2.60:
                continue
            if change_5 <= 0.12 and (change_3 <= 0.08 or vol_ratio < 1.35):
                continue
            label = "실시간 관심형"
            if change_5 >= 0.55 and change_3 >= 0.15 and vol_ratio >= 1.45 and chart_profile.get("chart_gap_pct", 999) >= 0.95:
                label = "실시간 강세형"
            elif top_rank_ok and change_5 >= 0.20:
                label = "실시간 주도형"
    ...
    for ticker, data in cache.items():
        try:
            ...
            if chart_profile.get("chart_zone_ratio", -1) >= CHART_POS_HIGH_ZONE_RATIO and chart_profile.get("chart_rise_pct", 0) >= 2.2 and chart_profile.get("chart_gap_pct", 999) <= CHART_POS_EXTENDED_NEAR_HIGH_PCT and change_5 < 1.40:
                continue
            if chart_profile.get("chart_rise_pct", 0) >= 3.50 and chart_profile.get("chart_gap_pct", 999) <= 2.20 and change_5 >= 1.20:
                continue
            if change_5 >= 2.10 and max(change_3, 0) >= 1.10 and chart_profile.get("chart_gap_pct", 999) <= 2.60:
                continue
            if change_5 <= 0.10 and change_3 <= 0.08 and vol_ratio < 2.40:
                continue
            pattern_tags = []
            if has_higher_lows(df):
            ...
            label = "실시간 관심형"
            if change_3 >= 0.22 and change_5 >= 0.55 and vol_ratio >= 1.65 and high_gap_pct >= 0.95 and leader_score >= 1.00 and chart_profile.get("chart_rise_pct", 0) <= 3.60:
                label = "실시간 강세형"
            elif change_5 >= 0.45 and vol_ratio >= 1.35 and high_gap_pct >= 1.00:
                label = "상승 시작형"
            elif vol_ratio >= 1.80 and change_3 >= 0.25 and high_gap_pct >= 0.80:
                label = "실시간 주도형"
...
def is_live_force_candidate(item: dict, regime: dict = None) -> tuple[bool, str, float]:
    ...
    if high_gap_pct <= 0.20:
        return False, "", 0.0
    if rise_pct >= 3.80 and high_gap_pct < 2.40:
        return False, "", 0.0
    if chg_5 >= 1.80 and max(chg_30, from_low) >= 3.80 and high_gap_pct < 2.60:
        return False, "", 0.0
    if not is_chart_position_good_for_live(item):
...
def scan_watchlist(shared_cache=None):
    ...
    for item in top:
        ...
        judgement = get_watch_judgement_text(item)
        caution = get_watch_caution_text(item)
        display_key = get_watch_display_group(item, judgement, caution, regime=regime)
        ...
        for item in skipped_top:
            ...
            if display_key == "live":
                ...
                if high_gap_pct < 0.85 or vol_ratio < 1.15 or motion < 0.18:
                    continue
                promoted_pool.append(("restart", priority, item, "new", "재출발 힘이 보여 먼저 볼 후보"))
    ...
    if early_entries:
        send_watch_alert_bundle(get_watch_section_header("early", regime=regime), early_entries[:3], section_key="early")
        sent_any_alert = True
    if live_entries:
        send_watch_alert_bundle(get_watch_section_header("live", regime=regime), live_entries[:3], section_key="live")
        sent_any_alert = True
    if restart_entries:
        limit = 2 if (early_entries or live_entries) else 3
    ...
    if late_entries:
        ...
        sent_any_alert = True
    if watch_entries and not (early_entries or restart_entries or live_entries or late_entries):
