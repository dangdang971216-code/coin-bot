...
def _v274_market_context(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    total = 0
    ...
    ctx["major_pressure"] = "하락압력" if min(btc3, eth3, xrp3) <= -0.4 else ("우호" if max(btc3, eth3, xrp3) >= 0.25 and ratio(up3) >= 40 else "중립")
    return ctx
...
def _v274_material_row(row: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:
    prof = score_row(row)
    item = dict(row or {})
    item.update({
        "ticker": _v274_ticker(row),
        "strategy": "재료수집",
        "strategy_key": "material_factory_v274",
        "route": "material_factory_v274",
        "current_price": fnum(prof.get("price"), fnum(row.get("current_price") or row.get("price"), 0)),
        "entry_price": fnum(prof.get("price"), fnum(row.get("current_price") or row.get("price"), 0)),
        "detected_price": fnum(prof.get("price"), fnum(row.get("current_price") or row.get("price"), 0)),
        "score": fnum(prof.get("score"), 0),
        "edge_score": fnum(prof.get("score"), 0),
        "change_1": prof.get("change_1"), "change_3": prof.get("change_3"), "change_5": prof.get("change_5"),
        "change_15": prof.get("change_15"), "change_30": prof.get("change_30"),
        "turnover_1m": prof.get("turnover_1m"), "turnover_3m": prof.get("turnover_3m"), "turnover_5m": prof.get("turnover_5m"),
        "money_flow_1m": prof.get("money_flow_1m"), "money_flow_3m": prof.get("money_flow_3m"), "money_flow_5m": prof.get("money_flow_5m"),
        "turnover_24h": prof.get("turnover_24h"),
        "vol_ratio": prof.get("vol_ratio"),
        "from_30m_low_pct": prof.get("from_low_pct"), "below_30m_high_pct": prof.get("high_gap_pct"),
        "low_defense_pct": prof.get("low_defense_pct"), "pullback_depth_pct": prof.get("pullback_depth_pct"),
        "recovery_speed_pct": prof.get("recovery_speed_pct"), "rebreakout_strength": prof.get("rebreakout_strength"),
        "rsi_14": prof.get("rsi_14"), "ema5_gap_pct": prof.get("ema5_gap_pct"), "ema12_gap_pct": prof.get("ema12_gap_pct"), "ema21_gap_pct": prof.get("ema21_gap_pct"),
        "bb_position": prof.get("bb_position"), "bb_lower_gap_pct": prof.get("bb_lower_gap_pct"), "bb_middle_gap_pct": prof.get("bb_middle_gap_pct"),
        "mfi_14": prof.get("mfi_14"), "cci_20": prof.get("cci_20"), "stoch_k": prof.get("stoch_k"), "stoch_d": prof.get("stoch_d"), "adx_14": prof.get("adx_14"),
        "current_close_pos_ratio": prof.get("close_pos"), "current_upper_wick_pct": prof.get("upper_wick"),
        "volume_spike_30x": prof.get("volume_spike_30x"), "volume_pump_risk": prof.get("volume_pump_risk"),
        "current_candle_code": prof.get("current_candle_code"), "current_candle_label": prof.get("current_candle_label"),
        "precision": prof.get("precision"), "freshness": prof.get("freshness"), "data_age_sec": prof.get("data_age_sec"),
    })
    try:
        item.update(ws_snapshot(item.get("ticker")))
        item.update(micro_snapshot(item.get("ticker")))
        item.update(execution_risk_snapshot(item))
    except Exception as exc:
        log_error("v274_material_external_overlay", exc)
    mat = _v274_score_groups(item, market)
    strategies = _v274_strategy_candidates(item, mat)
    item.update({
        "schema": "strategy_material_v274",
        "brain_version": BOT_VERSION,
        "material_only": True,
        "v274_material_only": True,
        "candidate_mode": "material_observe",
        "material_strategy_candidates": strategies,
        "material_primary_strategy": strategies[0] if strategies else "재료관찰",
        "material_score_total": mat.get("total", 0),
        "material_score_groups": mat.get("groups", {}),
        "material_grade": mat.get("grade"),
        "material_tags": mat.get("tags", []),
        "material_needs": mat.get("needs", []),
        "material_hard_blocks": mat.get("hard_blocks", []),
        ...
        "final_entry_label": "v274 전략층 초기화: 신규 모의매매 닫음 / 재료만 수집",
        "trade_ready_label": "신규 모의매매 중지",
        "trade_ready_reasons": ["v274 전략층 초기화", "재료수집 전용", "신규 paper OPEN 닫음"],
        "one_liner": f"{strategies[0] if strategies else '재료관찰'} / 총점 {mat.get('total',0)} / " + ",".join(mat.get("needs", [])[:3]),
    })
    return item


_v274_base_consume_row = consume_row
_v274_base_export_candidates = export_candidates


def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    market = _v274_market_context(rows or [])
    ...
    for raw in rows or []:
        ...
        try:
            ...
            log_error("v274_material_row", exc)
            continue
        # 재료공장은 넓게 보되, 아예 의미 없는 무거운 row는 latest에서 제외한다.
        money3 = fnum(item.get("turnover_3m") or item.get("money_flow_3m"), 0)
    ...
    materials.sort(key=lambda x: (fnum(x.get("material_score_total"), 0), fnum(x.get("turnover_3m") or x.get("money_flow_3m"), 0), fnum(x.get("turnover_24h"), 0)), reverse=True)
    ...
    try:
        _v274_write_material_files(strict, shadow, market, rejects)
    except Exception as exc:
        log_error("v274_write_material_files_in_build", exc)
    with _state_lock:
        ...
        STATE["phase_note"] = "v274: 전략층 초기화. 신규 모의매매 OPEN 닫고 재료 snapshot만 수집."
    return strict, shadow, rejects, examples


def decide_trade_ready(item: Dict[str, Any]) -> Tuple[bool, str, List[str]]:  # type: ignore[override]
    return False, "v274 신규 모의매매 중지", ["전략층 초기화", "재료수집 전용", "paper OPEN 생성 안 함"]


def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:  # type: ignore[override]
    row = _v274_base_consume_row(item, lane, ts, scan_id)
    row.update({
        "brain_version": BOT_VERSION,
        "schema": "candidate_consume_v274_material",
        "strategy": "재료수집",
        "strategy_key": "material_factory_v274",
        "route": "material_factory_v274",
        "material_only": True,
        ...
        "final_entry_label": "v274 전략층 초기화: 신규 모의매매 닫음 / 재료만 수집",
        "trade_ready_label": "신규 모의매매 중지",
        "trade_ready_reasons": ["v274 전략층 초기화", "재료수집 전용"],
        "material_strategy_candidates": item.get("material_strategy_candidates", []),
        "material_primary_strategy": item.get("material_primary_strategy", "재료관찰"),
        "material_score_total": item.get("material_score_total", 0),
        "material_score_groups": item.get("material_score_groups", {}),
        "material_grade": item.get("material_grade", "-"),
        "material_tags": item.get("material_tags", []),
        "material_needs": item.get("material_needs", []),
        "material_hard_blocks": item.get("material_hard_blocks", []),
        "market_context": item.get("market_context", {}),
        "pipeline_note": "v274: strategy layer removed from paper OPEN path. material snapshot only.",
    })
    ctx = row.get("entry_context") if isinstance(row.get("entry_context"), dict) else {}
    ctx = dict(ctx)
    ctx.update({
        ...
        "paper_bot_open": False,
        "trade_ready": False,
        "material_score_total": row.get("material_score_total", 0),
        "material_score_groups": row.get("material_score_groups", {}),
        "material_primary_strategy": row.get("material_primary_strategy", "재료관찰"),
    })
    row["entry_context"] = ctx
    return row


def _v274_write_material_files(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]], market: Dict[str, Any], rejects: Optional[Counter] = None) -> None:
    rows = list(strict or []) + list(shadow or [])
    nowv = now_ts()
    payload = {
        "version": BOT_VERSION,
        "schema": "strategy_material_snapshot_v274",
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        "mode": "material_only_new_paper_open_paused",
        ...
        "note": "v274: 기존 전략층/OPEN 판단 제거. 허브/직원/공장은 재료 snapshot만 저장.",
    }
...
def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:  # type: ignore[override]
    res = _v274_base_export_candidates(strict, shadow)
    try:
        rows = tail_jsonl(FILES.get("paper_latest", BASE_DIR / "paper_candidates_latest.jsonl"), max_lines=V274_MATERIAL_MAX_ROWS)
        rows = [r for r in rows if isinstance(r, dict)]
        market = rows[0].get("market_context") if rows and isinstance(rows[0].get("market_context"), dict) else {}
        save_json(FILES["strategy_material_snapshot"], {
            "version": BOT_VERSION,
            "schema": "strategy_material_snapshot_v274_after_export",
            "updated_ts": now_ts(),
            "updated_text": now_text(),
            "mode": "material_only_new_paper_open_paused",
            "material_count": len(rows),
            "market_context": market,
            "rows": rows,
            "note": "paper_latest also contains material rows, but all paper open flags are false.",
        })
        write_jsonl_replace(FILES["strategy_material_latest"], rows)
        with _state_lock:
            STATE["latest_trade_ready"] = 0
            STATE["trade_ready_written"] = 0
            STATE["v274_material_latest"] = len(rows)
            STATE["v274_new_paper_open"] = "paused"
    except Exception as exc:
        log_error("v274_export_material_snapshot", exc)
    if isinstance(res, dict):
        res["latest_trade_ready"] = 0
        res["trade_ready_written"] = 0
        res["factory_mode"] = "v274_material_only_no_new_paper_open"
    return res


def _v274_read_material_rows(limit: int = 120) -> List[Dict[str, Any]]:
...
def _v274_material_line(r: Dict[str, Any]) -> str:
    ...
    return (
        f"- {_v274_ticker(r)}: {r.get('material_grade','-')} / {r.get('material_primary_strategy','재료관찰')} / 총점 {fint(r.get('material_score_total'),0)} "
...
def score_text() -> str:  # type: ignore[override]
    ...
    current = _v274_current_version_rows(closed)
    lines = [
        "📊 성과 요약 /score",
        f"기준: {BOT_VERSION} / 전략층 초기화 재료수집",
        "",
        "[1] 현재버전",
        _v274_perf_line("v274", current, width=8),
        "- v274는 신규 모의매매 OPEN을 닫고 재료만 수집합니다.",
        "",
        "[2] 시간별",
        _v274_perf_line("3시간", rows_recent_hours(closed, 3), width=8),
        _v274_perf_line("6시간", rows_recent_hours(closed, 6), width=8),
        _v274_perf_line("12시간", rows_recent_hours(closed, 12), width=8),
        "",
        "[3] 버전별",
        *_v274_version_lines(closed, 6),
        "",
        "[4] 다음 판단",
        "- 지금은 전략 성과가 아니라 재료 수집 품질을 봅니다.",
        "- 모의매매 재개는 strategy_material_latest 재료와 5/10/20분 복기표가 안정된 뒤 결정합니다.",
    ]
    return "\n".join(lines)


def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    rows = _v274_read_material_rows(220 if full else 80)
    snap = load_json(FILES.get("strategy_material_snapshot", BASE_DIR / "strategy_material_snapshot.json"), {})
    market = snap.get("market_context") if isinstance(snap.get("market_context"), dict) else {}
    ...
    strat = Counter(str(r.get("material_primary_strategy") or "재료관찰") for r in rows)
    need_counter = Counter()
    block_counter = Counter()
    for r in rows:
        for n in (r.get("material_needs") or []):
            need_counter[str(n)] += 1
        for b in (r.get("material_hard_blocks") or []):
    ...
    lines = [
        ...
        "- 전략재료: " + (" / ".join(f"{k} {v}" for k, v in strat.most_common(5)) if strat else "없음"),
        "",
        "[2] 장세재료",
        f"- 상승비율 1m {fnum(market.get('up1_ratio'),0):.1f}% / 3m {fnum(market.get('up3_ratio'),0):.1f}% / 5m {fnum(market.get('up5_ratio'),0):.1f}% / 주요코인 {market.get('major_pressure','-')}",
        "",
        "[3] 부족한 재료 TOP",
        "- " + (" / ".join(f"{k} {v}" for k, v in need_counter.most_common(6)) if need_counter else "부족 사유 없음"),
        "[4] 하드문제 TOP",
        "- " + (" / ".join(f"{k} {v}" for k, v in block_counter.most_common(6)) if block_counter else "하드 문제 없음"),
        "",
        "[5] 재료 예시",
    ]
    if not rows:
        lines.append("- 아직 재료 snapshot 없음")
    else:
        for r in rows[:(12 if full else 5)]:
            lines.append(_v274_material_line(r))
    lines += ["", "판독: 여기는 매수판이 아니라 전략을 다시 만들 재료판입니다. 신규 모의매매 OPEN은 닫혀 있어야 정상입니다."]
    return "\n".join(lines)


def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    rows = _v274_read_material_rows(240 if full else 80)
    high = [r for r in rows if fnum(r.get("material_score_total"), 0) >= 16]
    blocked = [r for r in rows if r.get("material_hard_blocks")]
    needs = [r for r in rows if r.get("material_needs")]
    lines = [
        "🧪 전략실험 재료 /strategy_watch",
        f"기준: {BOT_VERSION} / 최근 재료 snapshot",
        f"- 재료 {len(rows)} / 고점수 {len(high)} / 하드문제 {len(blocked)} / 보강필요 {len(needs)}",
        "",
        "[1] 고점수 재료",
    ]
    if high:
        for r in high[:(12 if full else 5)]:
            lines.append(_v274_material_line(r))
    else:
        lines.append("- 아직 없음")
    lines += ["", "[2] 하드문제 재료"]
    if blocked:
        for r in blocked[:(10 if full else 4)]:
            lines.append(_v274_material_line(r))
    else:
        lines.append("- 하드문제 없음")
    lines += ["", "[3] 다음에 채워야 할 것", "- 5/10/20분 후보 추적", "- 같은 코인/같은 패턴 반복손실 기억", "- 지지/저항/돌파선", "- 캔들 꼬리/몸통", "- 예상 슬리피지"]
    if not full:
        lines.append("- 상세 재료표는 /strategy_watch_full")
    return "\n".join(lines)
...
def _build_quality_cache() -> None:  # type: ignore[override]
    try:
        save_json(FILES["quality_summary"], _cache_payload(candidate_quality_text(False), "quality"))
    except Exception as exc:
        log_error("v274_quality_cache", exc)
...
def _v274_material_row(row: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:  # type: ignore[override]
    ...
    try:
        ...
        log_error("v275_material_external_overlay", exc)
    mat = _v274_score_groups(item, market)
    strategy_scores = _v275_strategy_eval(item, mat, market)
    strategies = _v275_primary_strategies(strategy_scores)
    missing_info = _v275_missing_info(item)
    # 전략별 부족 재료도 전체 부족 목록에 합친다.
    strategy_missing = []
    for v in strategy_scores.values():
        for n in v.get("needs") or []:
            strategy_missing.append(str(n))
    item.update({
        "schema": "strategy_material_v275",
        "brain_version": BOT_VERSION,
        "material_only": True,
        "v274_material_only": True,
        ...
        "material_missing_fields": missing_info,
        "material_score_total": mat.get("total", 0),
        "material_score_groups": mat.get("groups", {}),
        "material_grade": mat.get("grade"),
        "material_tags": mat.get("tags", []),
        "material_needs": list(dict.fromkeys((mat.get("needs", []) or []) + strategy_missing + missing_info))[:16],
        "material_hard_blocks": mat.get("hard_blocks", []),
        "market_context": market,
        "paper_bot_open": False,
        "open_eligible": False,
        ...
        "event_type": "material_observe",
        "final_entry_action": "material_observe",
        "final_entry_label": "v275 외부전략 재료수집: 신규 모의매매 닫음 / 전략별 필요정보 기록",
        "trade_ready_label": "신규 모의매매 중지",
        "trade_ready_reasons": ["v275 전략재료 수집", "신규 paper OPEN 닫음", "전략별 필요정보 기록"],
        "one_liner": f"{strategies[0] if strategies else '재료관찰'} / 총점 {mat.get('total',0)} / " + ",".join(list(dict.fromkeys(strategy_missing + missing_info))[:3]),
    })
    return item


def _v275_strategy_lab(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for key, meta in V275_EXTERNAL_STRATEGIES.items():
        ...
        evs = Counter()
        for r in rows or []:
            sc = (r.get("strategy_material_scores") or {}).get(key, {}) if isinstance(r.get("strategy_material_scores"), dict) else {}
            if not sc:
                continue
            score = fnum(sc.get("score"), 0)
            if score >= 5:
                arr.append((score, r))
            for n in sc.get("needs") or []:
                miss[str(n)] += 1
...
def _v274_write_material_files(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]], market: Dict[str, Any], rejects: Optional[Counter] = None) -> None:  # type: ignore[override]
    ...
    payload = {
        "version": BOT_VERSION,
        "schema": "strategy_material_snapshot_v275",
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        "mode": "external_strategy_materials_new_paper_open_paused",
        "strict_material_count": len(strict or []),
        "shadow_material_count": len(shadow or []),
        "material_count": len(rows),
        "market_context": market,
        ...
        "rejects": dict(rejects or {}),
        "rows": rows[:V274_MATERIAL_MAX_ROWS],
        "note": "v275: 기존 전략층은 계속 닫고, 외부 전략 4종+기존 후보전략 재료와 부족정보를 snapshot으로 저장.",
    }
    save_json(FILES["strategy_material_snapshot"], payload)
    save_json(FILES["strategy_lab_summary"], {"version": BOT_VERSION, "updated_ts": nowv, "updated_text": now_text(nowv), "strategy_lab": lab})
    write_jsonl_replace(FILES["strategy_material_latest"], rows[:V274_MATERIAL_MAX_ROWS])
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    rows = _v274_read_material_rows(260 if full else 100)
    snap = load_json(FILES.get("strategy_material_snapshot", BASE_DIR / "strategy_material_snapshot.json"), {})
    market = snap.get("market_context") if isinstance(snap.get("market_context"), dict) else {}
    ready = [r for r in rows if bool(r.get("paper_bot_open") or r.get("trade_ready") or r.get("open_eligible"))]
    grades = Counter(str(r.get("material_grade") or "-") for r in rows)
    ...
    lines = [
        "🔬 재료 현황 /quality",
        f"기준: {BOT_VERSION} / 외부전략 4종 재료 포함",
        "[1] 재료공장",
        f"- 재료 후보 {len(rows)} / 신규 모의매매 OPEN {len(ready)} / 상태 {'✅ 닫힘' if not ready else '❌ 열림'}",
        "- 등급: " + (" / ".join(f"{k} {v}" for k, v in grades.most_common(5)) if grades else "없음"),
        "",
        "[2] 전략실험 후보",
        *_v275_lab_lines(rows, 7),
        "",
        "[3] 아직 채워야 할 정보 TOP",
        "- " + (" / ".join(f"{k} {v}" for k, v in missing.most_common(8)) if missing else "부족 정보 없음"),
        "",
        "[4] 장세재료",
        f"- 상승비율 1m {fnum(market.get('up1_ratio'),0):.1f}% / 3m {fnum(market.get('up3_ratio'),0):.1f}% / 5m {fnum(market.get('up5_ratio'),0):.1f}% / 평균3m {fnum(market.get('avg_change_3'),0):+.2f}% / 주요코인 {market.get('major_pressure','-')}",
        "",
        "[5] 재료 예시",
    ]
    if not rows:
        ...
        for r in rows[:(14 if full else 6)]:
            lines.append(_v274_material_line(r))
    lines += ["", "판독: 여기는 매수판이 아니라 전략을 다시 만들 재료판입니다. 신규 모의매매 OPEN은 닫혀 있어야 정상입니다."]
    return "\n".join(lines)
...
def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    rows = _v274_read_material_rows(280 if full else 100)
    lines = [
        "🧪 전략실험 재료 /strategy_watch",
        f"기준: {BOT_VERSION} / 외부 전략요소 비교",
        f"- 재료 {len(rows)} / 신규 모의매매 OPEN 중지 / 전략 후보는 점수만 비교",
        "",
        "[1] 전략별 재료 요약",
        *_v275_lab_lines(rows, 7),
        "",
    ...
    high = [r for r in rows if fnum(r.get("material_score_total"), 0) >= 16]
    if high:
        for r in high[:(14 if full else 6)]:
            lines.append(_v274_material_line(r))
    else:
        lines.append("- 아직 없음")
    lines += ["", "[3] 다음에 반드시 채워야 할 정보", "- 후보 5/10/20분 추적", "- 같은 코인/패턴 반복손실 기억", "- 지지/저항/쓸림 전용값", "- BB폭/ATR 수축률", "- 호가/체결 변화량", "- 예상 슬리피지"]
    if not full:
        lines.append("- 상세 재료표는 /strategy_watch_full")
    return "\n".join(lines)
...
def score_text() -> str:  # type: ignore[override]
    closed = load_closed(30000)
    current = _v274_current_version_rows(closed)
    rows = _v274_read_material_rows(180)
    lines = [
        "📊 성과 요약 /score",
        f"기준: {BOT_VERSION} / 전략층 닫힘 · 재료수집",
        "",
        "[1] 현재버전",
        _v274_perf_line("v276", current, width=8),
        "- v276은 전략층 철거 후 남은 명령어 연결 오류를 수술하고 외부전략 재료를 수집합니다.",
        "",
        "[2] 전략실험 재료 현황",
        *_v275_lab_lines(rows, 7),
        "",
        ...
        *_v274_version_lines(closed, 6),
        "",
        "[5] 다음 판단",
        "- 지금은 전략 성과가 아니라 전략별 재료와 부족정보를 봅니다.",
        "- 다음 수술은 5/10/20분 추적과 반복손실 기억입니다.",
    ]
    return "\n".join(lines)
...
# - 재료공장은 old score_row / execution_risk heavy path를 타지 않는다.
...
def _v277_light_material_row(row: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:
    """v277: 재료공장용 가벼운 row 생성. old score_row/old strategy path 금지."""
    item = dict(row or {})
    t = _v274_ticker(item)
    ...
    # sidecar cache overlay only; heavy execution_risk snapshot is intentionally not called here.
    try:
        item.update(ws_snapshot(t))
    except Exception as exc:
        log_error("v277_light_ws_overlay", exc)
    try:
        item.update(micro_snapshot(t))
    except Exception as exc:
        log_error("v277_light_micro_overlay", exc)
    ...
    item.update({
        ...
        "event_type": "material_observe",
        "final_entry_action": "material_observe",
        "final_entry_label": "v277 재료수집: old strategy/score_row 경로 차단, 신규 모의매매 닫음",
        "trade_ready_label": "신규 모의매매 중지",
        "trade_ready_reasons": ["v277 재료공장", "신규 paper OPEN 닫음", "old 전략층 미사용"],
        "one_liner": f"{strategies[0] if strategies else '재료관찰'} / 총점 {mat.get('total',0)} / " + ",".join(list(dict.fromkeys(strategy_missing + missing_info))[:3]),
    })
    return item


# override material builder to prevent old score_row path from being used.
def _v274_material_row(row: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:  # type: ignore[override]
...
def _v277_strategy_lab(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for key, meta in V275_EXTERNAL_STRATEGIES.items():
        ...
        for r in rows or []:
            sc = (r.get("strategy_material_scores") or {}).get(key, {}) if isinstance(r.get("strategy_material_scores"), dict) else {}
            score = fnum(sc.get("score"), -1) if sc else -1
            # fallback for rows written by older material factory versions.
            if score < 0 and (label in (r.get("material_strategy_candidates") or []) or str(r.get("material_primary_strategy")) == label):
                score = fnum(r.get("material_score_total"), 0) / 3.0
                for n in r.get("material_needs") or []:
                    miss[str(n)] += 1
            if score >= 5:
                arr.append((score, r))
...
def _v277_write_score_cache(payload: Dict[str, Any]) -> None:
    try:
        ...
        text = "\n".join([
            "📊 성과 요약 /score",
            f"기준: {BOT_VERSION} / 전략층 닫힘 · 재료수집 · 캐시전용",
            "",
            "[1] 현재버전",
            "- 신규 모의매매 OPEN 닫힘 / 현재버전 CLOSED 성과 판단 안 함",
            "",
            "[2] 전략실험 재료 현황",
            *_v277_lab_lines_from_lab(lab, 7),
            "",
            "[3] 재료공장",
            f"- 재료 후보 {len(rows)} / 신규 모의매매 OPEN 0 / 등급 " + (" / ".join(f"{k} {v}" for k, v in grades.most_common(5)) if grades else "없음"),
            "",
            "[4] 아직 채워야 할 정보 TOP",
            "- " + (" / ".join(f"{k} {v}" for k, v in missing.most_common(8)) if missing else "부족 정보 없음"),
            "",
            "[5] 다음 판단",
            "- /score는 paper_closed 직접계산을 하지 않고 재료 snapshot cache만 읽습니다.",
            "- 기존 장부 성과는 페이퍼봇 /pscore에서 봅니다.",
        ])
        save_json(FILES.get("score_summary", BASE_DIR / "clean_score_summary.json"), _cache_payload(text, "score"))
    except Exception as exc:
        log_error("v277_score_cache_write", exc)
...
def _v274_write_material_files(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]], market: Dict[str, Any], rejects: Optional[Counter] = None) -> None:  # type: ignore[override]
    ...
    payload = {
        "version": BOT_VERSION,
        "schema": "strategy_material_snapshot_v277",
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        ...
        "rejects": dict(rejects or {}),
        "rows": rows[:V274_MATERIAL_MAX_ROWS],
        "note": "v277: old score_row/strategy path 차단. 기본 명령은 snapshot cache만 읽음.",
    }
    save_json(FILES["strategy_material_snapshot"], payload)
    save_json(FILES["strategy_lab_summary"], {"version": BOT_VERSION, "updated_ts": nowv, "updated_text": now_text(nowv), "strategy_lab": lab})
    write_jsonl_replace(FILES["strategy_material_latest"], rows[:V274_MATERIAL_MAX_ROWS])
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    rows = _v277_rows_from_snapshot(260 if full else 100)
    snap = _v277_snapshot()
    market = snap.get("market_context") if isinstance(snap.get("market_context"), dict) else {}
    ...
    lines = [
        "🔬 재료 현황 /quality",
        f"기준: {BOT_VERSION} / cache-only / old 전략경로 미사용",
        "[1] 재료공장",
        f"- 재료 후보 {len(rows)} / 신규 모의매매 OPEN {len(ready)} / 상태 {'✅ 닫힘' if not ready else '❌ 열림'}",
        "- 등급: " + (" / ".join(f"{k} {v}" for k, v in grades.most_common(5)) if grades else "없음"),
        "",
    ...
    if not rows:
        lines.append("- 아직 재료 snapshot 없음")
    lines += ["", "판독: /quality는 snapshot cache만 읽습니다. 신규 모의매매 OPEN은 닫혀 있어야 정상입니다."]
    return "\n".join(lines)


def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    rows = _v277_rows_from_snapshot(280 if full else 100)
    snap = _v277_snapshot()
    lab = snap.get("strategy_lab") if isinstance(snap.get("strategy_lab"), dict) else _v277_strategy_lab(rows)
    lines = [
        "🧪 전략실험 재료 /strategy_watch",
...
try:
    STRATEGY_NAME = "외부전략 재료수집"
    STRATEGY_KEY = "material_factory_v277"
    STATE["phase_note"] = "v277: 명령어 cache-only, material factory old score_row 경로 차단, 신규 모의매매 OPEN 닫음."
    STATE["v274_new_paper_open"] = "paused"
except Exception:
    pass
