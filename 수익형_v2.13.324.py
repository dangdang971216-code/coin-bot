...
WORKERS={
    ...
    "orderflow": {"file":"orderflow_worker_v0.3.py", "status":"clean_orderflow_status.json"},
    "risk": {"file":"risk_worker_v0.2.py", "status":"clean_risk_status.json"},
    "strategy": {"file":"strategy_worker_v0.5.py", "status":"clean_strategy_worker_status.json"},
...
def health_text()->str:
    ...
    lines=[
        "🧭 건강상태 /health",
        f"✅ 메인봇 {BOT_VERSION} / clean worker hub v324 / uptime {int(now_ts()-START_TS)}s",
...
def score_text()->str:
    ...
    if strat:
        ...
        lines.append(f"- 정보보강 대기 {strat.get('info_pending_count','-')} / S근접 정보대기 {strat.get('near_s_info_pending','-')} / target {strat.get('target_count','-')}")
        ...
        lines.append(f"- 정보배차 target {router.get('target_count','-')} / S근접 {router.get('near_s_count','-')} / fresh회복 {router.get('fresh_recovered','-')}")
        for k,v in (strat.get('strategies') or {}).items(): lines.append(f"- 현재 {k}: S {v}")
    strategy_stats = cache.get('strategy_s_stats') or cache.get('strategy_stats')
    if isinstance(strategy_stats, list) and strategy_stats:
        lines += ["", "[전략별 S 전적]"]
        label_map={'money_reaccel_s':'돈흐름 재가속','leader_momentum_s':'주도추세 지속','breakout_early_s':'돌파 초입','sweep_vwap_recovery_s':'저점 VWAP 회복','surge_rebreak_s':'급등 후 재돌파'}
        for st in strategy_stats[:10]:
            if not isinstance(st,dict): continue
            key=str(st.get('key') or st.get('label') or 'unknown')
            label=label_map.get(key, str(st.get('label') or key))
            n=st.get('n', st.get('count',0)); wins=st.get('wins','-'); losses=st.get('losses','-')
            wr=st.get('win_rate', st.get('win_rate_pct',0)); total=st.get('total', st.get('total_pct', st.get('profit_sum',0)))
            lines.append(f"- {label}: {n}전 {wins}승 {losses}패 / 승률 {wr}% / 합산 {float(total):+.2f}%")
    return "\n".join(lines)
...
def quality_text()->str:
    ...
    router=load_json(FILES.get('target_router', BASE_DIR/'clean_target_router_queue.json'),{}) or {}
    lines=["🔍 후보품질 /quality · S 후보와 탈락사유", f"- 실제 OPEN: 전략별 S만. A/B/C 실시간 후보표 제거.", f"- S 후보 {strat.get('s_count',0)} / 평가 {reject.get('evaluated','-')} / 탈락 {reject.get('reject_count','-')}", f"- 정보보강 대기 {reject.get('info_pending_count','-')} / S근접 정보대기 {reject.get('near_s_info_pending','-')} / priority요청 {reject.get('priority_request_count','-')}", f"- 정보배차: target {router.get('target_count','-')} / S근접 {router.get('near_s_count','-')} / fresh회복 {router.get('fresh_recovered','-')} / WS필요 {router.get('need_ws','-')} / micro필요 {router.get('need_micro','-')}", f"- paper 전달: latest파일 {latest_lines}줄 / archive {'대형' if archive_lines<0 else str(archive_lines)+'줄'} / strategy handoff {hand.get('paper_latest_count','-')} / paper_bot 병합 {pbh.get('merged_fresh','-')}"]
    lines += ["", "[S 후보 TOP]"]
