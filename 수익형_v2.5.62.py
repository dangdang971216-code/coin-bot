...
def build_fast_scan_alert_markup(ticker: str):
    ticker = str(ticker or "").upper().strip()
    if not ticker:
        return None
    try:
        return InlineKeyboardMarkup([[InlineKeyboardButton(f"{ticker} 빗썸 열기", url=get_bithumb_chart_url(ticker))]])
    except Exception:
        return None
...
def _send_fast_scan_alerts(items, now_ts: float):
    ...
    for item in items:
        ...
        markup = build_fast_scan_alert_markup(ticker)
        send(msg, reply_markup=markup)
        sent += 1
