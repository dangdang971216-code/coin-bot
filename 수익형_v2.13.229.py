...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v229: micro fresh를 후보 snapshot/paper 성과에 반영하는 관찰 안정화 수술. 조건/청산/BUY_READY 변경 없음.",
}
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v229 micro fresh snapshot/paper 반영 수술 + WS 진단 유지(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
def _v216_quality_text() -> str:
    ...
    lines = [
        "🔬 후보품질 요약 /quality",
        "- v229 기본은 캐시 전용: 현재 후보는 clean_candidate_snapshot.json 단일 원천만 표시",
...
def health_text() -> str:  # type: ignore[override]
    return (_v226_base_health_text()
            .replace("수익형 v2.13.225", "수익형 v2.13.229")
            .replace("수익형 v2.13.224", "수익형 v2.13.229")
            .replace("수익형 v2.13.226", "수익형 v2.13.229")
            .replace("v225 micro fresh 단일판정", "v229 micro fresh snapshot/paper 반영"))
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    if full:
        return txt
    return txt.replace("v222 기본은 캐시 전용", "v229 기본은 캐시 전용").replace("v227 기본은 캐시 전용", "v229 기본은 캐시 전용")
...
def _v210_health_text() -> str:  # type: ignore[override]
    ...
    try:
        ...
        pass
    return (txt.replace("수익형 v2.13.228", "수익형 v2.13.229")
               .replace("수익형 v2.13.227", "수익형 v2.13.229")
               .replace("수익형 v2.13.226", "수익형 v2.13.229"))
...
def _v216_quality_text() -> str:  # type: ignore[override]
    ...
    return (txt.replace("v228 기본은 캐시 전용", "v229 기본은 캐시 전용")
               .replace("v227 기본은 캐시 전용", "v229 기본은 캐시 전용")
               .replace("v226 기본은 캐시 전용", "v229 기본은 캐시 전용")
               .replace("v222 기본은 캐시 전용", "v229 기본은 캐시 전용"))
