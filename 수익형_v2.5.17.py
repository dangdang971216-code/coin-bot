...
def get_watch_section_key(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> str:
    ...
    if regime_name == "BLOCK":
        return "late" if section == "late" else "watch"

    if regime_name == "WEAK":
        if section == "entry":
            if is_low_zone_recovery_candidate(item, judgement, caution):
                return "recovery" if should_keep_recovery_candidate_in_weak_regime(item, regime=regime) else "watch"
            return "late" if strategy in ["TREND_CONT", "BREAKOUT", "CHASE"] else "watch"
        if section == "recovery" and not should_keep_recovery_candidate_in_weak_regime(item, regime=regime):
            return "watch"

    if regime_name == "SIDEWAYS" and section == "entry" and strategy in ["TREND_CONT", "BREAKOUT", "CHASE"]:
        return "late"
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- BTC 장이 약해도 강한 회복형은 너무 쉽게 안 죽게 조정",
        "- 횡보 장에선 회복형은 조금 더 살리고 늦은 추격은 계속 아래로 내림",
        "- BTC 리포트는 단기/중기/일봉을 같이 보도록 정리",
        "- /scan_debug는 최근 시장 후보가 비어도 최근/기다릴 후보를 같이 보여주게 보강",
...
def get_scan_debug_candidate_section(change_5: float, vol_ratio: float, leader_score: float, rsi: float) -> str:
    ...
    if rsi >= 82:
        return "weak"
    if change_5 >= 0.35 and vol_ratio >= 1.80 and leader_score >= 0.80:
        return "strong"
    if change_5 >= 0.15 and vol_ratio >= 1.20 and leader_score >= 0.50:
        return "watch"
    return "weak"
...
def normalize_scan_debug_score(base_score: float, change_5: float, vol_ratio: float, leader_score: float, rsi: float, turnover: float = 0.0) -> float:
    score = min(max(safe_float(base_score, 0), 0), 42.0)
    change_5 = safe_float(change_5, 0)
    vol_ratio = safe_float(vol_ratio, 0)
    leader_score = safe_float(leader_score, 0)
    rsi = safe_float(rsi, 50)
    ...
    score += min(max(change_5, 0), 1.60) * 5.0
    score += min(max(vol_ratio - 1.0, 0), 5.0) * 1.5
    score += min(leader_score, 4.0) * 1.6
    score += min(turnover / 1000000000.0, 2.5)
    ...
    if vol_ratio >= 10.0 and change_5 <= 0.20:
...
def get_scan_debug_candidates():
    try:
        ...
        recent_entries = get_recent_candidate_entries(limit=8)
        if recent_entries:
            items = []
            for entry in recent_entries:
                ...
                vol_ratio = safe_float(item.get("vol_ratio", 0))
                change_5 = safe_float(item.get("change_pct", 0))
