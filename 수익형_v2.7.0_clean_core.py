...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.0 clean_core"
BOT_FILE_TAG = "수익형_v2.7.0_clean_core.py"
TIMEZONE = "Asia/Seoul"
...
def nearest_old_price(hist: deque, target_age_sec: int) -> float:
    if not hist:
        return 0.0
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque) -> MarketRow:
    ...
    if money >= 50_000_000_000:
        ...
        money_score = 0.3
    # clean core 점수: 실시간 움직임 + 돈 + 고점추격 페널티
    raw = (ch1 * 2.2) + (ch3 * 1.6) + (ch5 * 0.8) + money_score
    if high_gap < 0.15 and from_low > 1.2:
        raw -= 1.0  # 고점 바로 위 추격 방지
    if ch1 <= 0 and ch3 <= 0:
        raw -= 0.4
    return MarketRow(
        ticker=ticker, price=price, money24h=money, ch1=ch1, ch3=ch3, ch5=ch5, ch15=ch15,
        high30=h30, low30=l30, from_low30=from_low, high_gap30=high_gap, score=raw, updated_ts=ts
    )


def classify_candidate(row: MarketRow) -> Candidate:
    # 기본 후보 판정. 레거시 trace/paper/C등급은 절대 사용하지 않는다.
    stale = now_ts() - row.updated_ts > 45
    stop = row.low30 * 0.995 if row.low30 else row.price * 0.985

    # 초록은 최신 실시간 가격 변화가 있어야 한다. 0.00% 후보 금지.
    live_move_ok = (row.ch1 >= 0.12 or row.ch3 >= 0.22)
    money_ok = row.money24h >= 500_000_000
    not_chasing = row.high_gap30 >= 0.20 or row.from_low30 <= 1.1
    score_ok = row.score >= 1.05

    if stale:
        return Candidate(row.ticker, "WATCH", "자료확인", "⚪", "자료가 오래되어 감시만", row.price, row.score, row.ch1, row.ch3, row.ch5, row.money24h, row.from_low30, row.high_gap30, stop, "자료 갱신 후 재확인", "자료 오래됨", row.updated_ts)

    if live_move_ok and money_ok and not_chasing and score_ok:
        return Candidate(row.ticker, "BUY_READY", "진입가능 검토", "🟢", "실시간 움직임+돈+추격위험 통과", row.price, row.score, row.ch1, row.ch3, row.ch5, row.money24h, row.from_low30, row.high_gap30, stop, "실거래 전 호가/슬리피지 재검사 필요", "", row.updated_ts)

    # 눌림대기: 돈은 있는데 아직 재확인 부족하거나 1분/3분이 약함
    if row.money24h >= 500_000_000 and row.score >= 0.35 and (row.ch5 >= 0.10 or row.from_low30 <= 0.8):
        return Candidate(row.ticker, "PENDING", "눌림대기", "🟡", "돈은 있으나 재확인 전", row.price, row.score, row.ch1, row.ch3, row.ch5, row.money24h, row.from_low30, row.high_gap30, stop, "1분/3분 회복과 고점추격 여부 확인", "아직 PENDING", row.updated_ts)

    # 감시만
    if row.score >= 0.15 or row.ch1 > 0.05 or row.ch3 > 0.08:
        return Candidate(row.ticker, "WATCH", "감시만", "⚪", "움직임은 있으나 매수후보 부족", row.price, row.score, row.ch1, row.ch3, row.ch5, row.money24h, row.from_low30, row.high_gap30, stop, "강해지면 재평가", "매수 근거 부족", row.updated_ts)

    return Candidate(row.ticker, "REJECT", "버림", "🔴", "점수/움직임 부족", row.price, row.score, row.ch1, row.ch3, row.ch5, row.money24h, row.from_low30, row.high_gap30, stop, "", "점수 낮음", row.updated_ts)
...
def rebuild_hubs(raw: dict[str, dict]):
    ...
    with STATE.lock:
        ...
        candidates_all = [classify_candidate(r) for r in ranked[:120]]
        candidates = [c for c in candidates_all if c.status in ("BUY_READY", "PENDING", "WATCH")][:MAX_CANDIDATES]
        rejected = [c for c in candidates_all if c.status == "REJECT"][:30]
...
def render_error_doctor() -> str:
    ...
    return (
        f"🩺 오류/병목 진단 /error_doctor v2.7.0\n\n"
        f"1) 현재 판정\n"
...
def render_buy_check() -> str:
    ...
    lines.append("판단 기준")
    lines.append("- 🟢: 최신 실시간 움직임+돈+고점추격 위험 통과. 그래도 실제 주문 전 호가/슬리피지 재검사 필요")
    lines.append("- 🟡: 후보는 맞지만 아직 재확인 전")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.0은 clean core 검증판이라 실제 주문은 막아둠.\n"
        f"- 후보 품질 확인 후 v2.7.2에서 실거래 직원 재연결 예정.\n"
    )
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.0\n"
        f"- v2.7.0 clean core는 실거래 주문 보호모드야.\n"
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.0\n"
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.0 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 v2.7.2에서 실거래 직원 재연결 예정.")
