...
def should_promote_recovery_candidate_to_entry(item: dict, judgement: str = "", caution: str = "") -> bool:
...
def get_watch_section_key(item: dict, judgement: str = "", caution: str = "") -> str:
    judgement = judgement or get_watch_judgement_text(item)
    caution = caution or get_watch_caution_text(item)
    ...
    if is_low_zone_recovery_candidate(item, judgement, caution):
        if should_promote_recovery_candidate_to_entry(item, judgement, caution):
            return "entry"
        return "recovery"
    return "watch" if str(item.get("strategy", "") or "").strip() == "LEADER_WATCH" else "entry"


def get_watch_section_header(section_key: str) -> str:
    return {
        "entry": "✅ 지금 진입 후보",
        "recovery": "🌱 저점권 회복 후보",
        "late": "⛔ 지금 사면 늦은 후보",
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 저점권 회복 후보가 힘이 붙으면 위 섹션으로 승격되게 보강",
        "- USDC 같은 저변동/실전가치 낮은 후보는 더 강하게 제외",
        "- 자동 확인 예약 메시지는 더 짧게 정리",
        "- 약한 후보는 계속 짧게 보이도록 유지",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def scan_watchlist(shared_cache=None):
    ...
    for item in build_prewatch_candidates(cache, regime, exclude_tickers=sent_tickers):
        ...
        if not should_send_simple_aux_alert(recent_prewatch_alerts, ticker, now_ts, PREWATCH_ALERT_COOLDOWN_SEC):
            continue
        section_key = "entry" if should_promote_recovery_candidate_to_entry(item) else "recovery"
    ...
    if entry_entries:
        send_watch_alert_bundle(get_watch_section_header("entry"), entry_entries[:3])
    if recovery_entries:
        limit = 1 if entry_entries else 2
        send_watch_alert_bundle(get_watch_section_header("recovery"), recovery_entries[:limit])
    if late_entries:
        limit = 1 if entry_entries else 2
        send_watch_alert_bundle(get_watch_section_header("late"), late_entries[:limit])
    if watch_entries and not (entry_entries or recovery_entries or late_entries):
        send_watch_alert_bundle(get_watch_section_header("watch"), watch_entries[:1])
