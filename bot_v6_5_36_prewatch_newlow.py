...
def get_recent_candidate_entries(limit: int = 5):
    ...
    for row in rows:
        ...
        if not isinstance(item, dict):
            continue
        entries.append((item, row.get("alert_type", "new"), row.get("reason_text", "")))
    return entries
...
def build_prewatch_candidates(cache, regime, exclude_tickers=None):
    ...
    for ticker, data in cache.items():
        try:
            ...
            if change_5 >= 2.8 or change_3 >= 2.0:
                continue
            if change_1 <= -0.9 and change_3 <= -0.7:
                continue
            if not (1.15 <= vol_ratio <= 4.8):
                continue
            if not (43 <= rsi <= 69):
                continue
            if not (0.35 <= high_gap_pct <= 2.8):
                continue
            if range_pct >= 3.8:
                continue
            if regime_name == "WEAK" and vol_ratio < 1.55:
                continue
            if is_recent_spike_near_high(df, current_price) or is_box_top_range_chase(df, current_price) or is_retest_after_failed_peak(df):
                continue

            tags = []
            if has_higher_lows(df):
                tags.append("저점높임")
            if detect_box_top_compression(df):
                tags.append("상단압축")
            if big_bull_half_hold(df):
                tags.append("양봉절반유지")
            if not tags and leader_score < 0.5 and turnover_rank > 45 and surge_rank > 45:
                continue

            score = 4.2 + max(vol_ratio - 1.0, 0) * 0.6 + max(change_3, 0) * 0.35 + max(leader_score, 0) * 0.35
            preview.append(make_signal(
                ...
                reason=f"아직 본 후보 전 / 고점까지 {high_gap_pct:.2f}% 여유 / 거래량 {vol_ratio:.2f}배 / 순위 T{turnover_rank}/S{surge_rank}",
            ))
        except Exception:
            continue
...
def build_newlow_recovery_candidates(cache, regime, exclude_tickers=None):
    ...
    for ticker, data in cache.items():
        try:
            ...
            if vol_ratio < 1.45:
                continue
            if not (0.20 <= low_gap_pct <= 2.20):
                continue
            if high_gap_pct <= 0.50:
                continue
            if not (fake_recovery or (bullish_reclaim and lower_low_recent and change_1 >= -0.35)):
                continue
            if is_recent_spike_near_high(df, current_price) or is_box_top_range_chase(df, current_price):
                continue
            ...
            score = 4.4 + max(vol_ratio - 1.0, 0) * 0.55 + max(change_1, 0) * 0.30 + max(leader_score, 0) * 0.25
            recovered.append(make_signal(
                ...
                reference_high=reference_high,
                reference_low=recent_low,
                reason=f"최근 저가권 회복 / 저가에서 {low_gap_pct:.2f}% 회복 / 거래량 {vol_ratio:.2f}배",
            ))
        except Exception:
            continue
...
def scan_watchlist(shared_cache=None):
    ...
    if preview_entries:
        send_watch_alert_bundle("🧭 예비 후보 감시", preview_entries)
    if newlow_entries:
        send_watch_alert_bundle("🪫 신저가 반등 감시", newlow_entries)
