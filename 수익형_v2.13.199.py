...
def update_ws_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "hub_rank") -> None:
    ...
    try:
        ...
        previous_priority = recent_candidate_priority_rows(limit=180)
        ...
        for t in _paper_open_tickers(limit=35):
        ...
        payload = {
            ...
            "paper_open_count": len(_paper_open_tickers(limit=35)),
            "priority_first": True,
            ...
            "note": "v199: target TTL/debounce keeps current/previous entry candidates and OPEN positions warm for websocket sidecar",
        }
        if should_write:
            try:
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.199.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v199 WS 신선도 고정(target TTL/debounce + age 기준 통일) / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
