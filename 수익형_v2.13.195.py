...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.195.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def _builder_for_command(name: str):
    if not TEXT_COMMAND_BUILDERS:
        TEXT_COMMAND_BUILDERS.update({
            ...
            "help": lambda: "📚 명령어 /help\n- 여러 명령을 줄바꿈으로 보내도 자동 묶음 처리됨\n- /batch /check /core /bot_status /ws_status /external_status /quality /score /version_score /version_compare /cpu_status /paper_handoff /deploy /errorlog_full",
        })
...
def install_commands(updater: Any) -> None:
    ...
    try:
        menu = ["batch", "check", "check_full", "core", "bot_status", "external_status", "quality", "quality_full", "score", "paper_today", "flow_check", "candidate_reason", "cpu_status", "paper_handoff", "paper_handoff_full", "deploy", "trade", "errorlog", "errorlog_full", "help"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v195 micro 자동복구 대비 + 대상 선반영 + 버전비교 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
