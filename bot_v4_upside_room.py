...
STRATEGY_GUIDE = {
    ...
    "예비 후보 감시": "아직 본 후보 전이지만 출발 전일 수 있어 먼저 보는 후보",
    "신저가 반등 감시": "저가 찍은 뒤 바로 회복하는지 감시하는 후보",
}
...
def get_watch_caution_text(item: dict) -> str:
    ...
    if strategy_label == "예비 후보 감시":
        return "아직 감시 단계라 바로 진입보다 흐름 확인이 먼저야"
    if strategy_label == "신저가 반등 감시":
        return "저가 재이탈하면 약할 수 있어 회복 확인이 먼저야"
...
def should_show_trade_plan_in_alert(item: dict) -> bool:
    strategy_label = item.get("strategy_label", "")
    return strategy_label not in ["예비 후보 감시", "신저가 반등 감시"]
...
def scan_watchlist(shared_cache=None):
    ...
    for item in build_prewatch_candidates(cache, regime, exclude_tickers=sent_tickers):
        ...
        if not should_send_simple_aux_alert(recent_prewatch_alerts, ticker, now_ts, PREWATCH_ALERT_COOLDOWN_SEC):
            continue
        preview_entries.append((item, "new", "아직 본 후보 전 단계라 먼저 보는 감시 후보"))
    ...
    for item in build_newlow_recovery_candidates(cache, regime, exclude_tickers=sent_tickers):
        ...
        if not should_send_simple_aux_alert(recent_newlow_alerts, ticker, now_ts, NEWLOW_ALERT_COOLDOWN_SEC):
            continue
        newlow_entries.append((item, "new", "신저가 근처에서 바로 회복하는지 보는 감시 후보"))
        sent_tickers.add(ticker)
    ...
    if preview_entries:
        send_watch_alert_bundle("🧭 예비 후보 감시", preview_entries[:PREWATCH_ALERT_MAX])
    if newlow_entries:
        send_watch_alert_bundle("🪫 신저가 반등 감시", newlow_entries[:NEWLOW_ALERT_MAX])
