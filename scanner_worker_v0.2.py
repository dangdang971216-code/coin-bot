...
VERSION="scanner_worker_v0.2"
INTERVAL_SEC=float(os.getenv("SCANNER_WORKER_INTERVAL_SEC","2.5"))
HTTP_TIMEOUT_SEC=float(os.getenv("SCANNER_WORKER_HTTP_TIMEOUT_SEC","3.0"))
STATUS=BASE_DIR/"clean_scanner_status.json"
MARKET=BASE_DIR/"clean_scanner_market_cache.json"
POOL=BASE_DIR/"clean_scanner_candidate_pool.json"
ERROR=BASE_DIR/"clean_scanner_error.log"
STABLE={"USDT","USDC","DAI","BUSD","FDUSD","TUSD","USDS","USD1","PYUSD","USDE","RLUSD"}
MAJOR={"BTC","ETH","XRP"}

def http_json(url:str)->Any:
    req=urllib.request.Request(url,headers={"Accept":"application/json","User-Agent":"coinbot-scanner-worker/0.2"})
...
def parse_rows(payload:Any)->List[Dict[str,Any]]:
    ...
    for k,row in data.items():
        ...
        chg=fnum(row.get("fluctate_rate_24H"),row.get("fluctate_rate_24h"),row.get("fluctate_rate"),row.get("change_rate"),default=0)
        rows.append({"ticker":t,"market":f"{t}_KRW","current_price":price,"price":price,"turnover_24h":turnover,"volume_24h":vol,"change_24h":chg,"major_watch":t in MAJOR,"fresh_ts":ts,"source":"scanner_ALL_KRW"})
...
def build_pool(rows:List[Dict[str,Any]])->List[Dict[str,Any]]:
    out=[]
    for r in rows:
        if r.get("major_watch"):
            # 대형주는 시장참고로 남기되 전략후보 우선순위에서는 낮춘다.
            base=0.2
        else:
            base=1.0
        score=base*(min(8.0, fnum(r.get("turnover_24h"))/800_000_000.0)+min(7.0, abs(fnum(r.get("change_24h")))))
        rr=dict(r); rr["scanner_priority_score"]=round(score,3)
        out.append(rr)
    out.sort(key=lambda r:(fnum(r.get("scanner_priority_score")), fnum(r.get("turnover_24h"))), reverse=True)
    # 전략조건이 아니라 worker 전달 안전상한. 현재 KRW 전체보다 넓은 값.
    return out[:600]
...
def run_once()->Dict[str,Any]:
    st=now_ts()
    rows=parse_rows(http_json("https://api.bithumb.com/public/ticker/ALL_KRW"))
    if not rows: raise RuntimeError("scanner empty rows")
    pool=build_pool(rows)
    ts=now_ts()
    save_json(MARKET,{"version":VERSION,"schema":"scanner_market_cache_v2","updated_ts":ts,"updated_text":now_text(ts),"row_count":len(rows),"rows":rows})
    save_json(POOL,{"version":VERSION,"schema":"scanner_candidate_pool_v2","updated_ts":ts,"updated_text":now_text(ts),"row_count":len(pool),"rows":pool})
    sec=now_ts()-st
    write_status("running", row_count=len(rows), pool_count=len(pool), last_sec=round(sec,3), source="ALL_KRW")
