...
VERSION = "guard_v2.5.17_ws_upgrade_separate_2026-05-15"
...
# v2.5.14: ws_sidecar는 메인/페이퍼와 분리된 실시간 직원이다.
# 서버에서 수동 확인된 안정 경로를 기본값으로 고정해 guard.env의 애매한 python3 해석을 피한다.
...
def ws_import_check() -> tuple[bool, list[str]]:
    ...
    env = os.environ.copy()
    env.update(ENV)
    rc, out = run([WS_PYTHON_BIN, "-c", code], cwd=BOT_DIR, timeout=10)
    lines = [f"python={WS_PYTHON_BIN}", tail(out, 700) if out else "import output 없음"]
    return rc == 0, lines
...
def start_ws_sidecar() -> tuple[str, list[str]]:
    ...
    if pids:
        pid_note = write_ws_pid_file(pids[-1])
        return "active", [f"이미 실행중 pid={pids[-1]}", pid_note, f"python_exe={proc_exe(pids[-1]) or '-'}"]

    import_ok, import_notes = ws_import_check()
    notes += ["import check"] + [f"  · {x}" for x in import_notes]
    if not import_ok:
        init_ws_status_file("import_failed")
        return "import_failed", notes + ["websockets import 실패 → sidecar 실행 중단"]
    ...
    try:
        ...
        p = subprocess.Popen([WS_PYTHON_BIN, str(active_path)], cwd=str(BOT_DIR), env=env, stdout=log_f, stderr=subprocess.STDOUT, start_new_session=True)
        pid_note = write_ws_pid_file(p.pid)
        time.sleep(3)
        alive = pid_alive(p.pid)
        notes.append(f"direct start pid={p.pid} alive={alive}")
        notes.append(pid_note)
        notes.append(f"python_exe={proc_exe(p.pid) or WS_PYTHON_BIN}")
...
def apply_ws_sidecar_latest(force: bool = False, explicit: Optional[str] = None, restart: bool = True) -> dict:
    ...
    if not import_ok:
        ...
        return {"ok": False, "changed": False, "target": target.name, "error": "websockets import 실패", "notes": import_notes}
    active_path = BOT_DIR / WS_ACTIVE_FILE
    same_hash = active_path.exists() and short_hash(active_path) == short_hash(target)
    backup = "없음"
    ...
    return {"ok": ws_ok, "changed": changed, "target": target.name, "version": ws_target_version(target), "active": active, "backup": backup, "hash": short_hash(active_path), "notes": notes, "warning": "변경 없음 → active 파일 hash 동일" if not changed else "", "state": state.get("verdict"), "raw": state.get("raw_total"), "match": state.get("match_ok")}
...
def format_ws_apply_result(res: dict) -> str:
    ...
    for k in ["target", "version", "active", "state", "raw", "match", "backup", "hash", "error", "warning"]:
