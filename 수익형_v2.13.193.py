...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.193: 빗썸 호가/체결 미세구조 sidecar 캐시를 읽어 최종진입검증과 entry_context에 저장.",
    }
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.192.py",
        "- 페이퍼봇 대상: paper_bot_v0.37.py",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.192.py", "- 페이퍼봇 대상: paper_bot_v0.37.py"])),
    ]
    start = now_ts()
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v192 WS대상 표시 원인수정 + 허브/스캔 후보대상 플래그 보존 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
