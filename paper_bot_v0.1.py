...
FILES = {
    ...
    "flag": BASE_DIR / "external_paper_runner_on.flag",
}
...
def fetch_upbit_price(ticker: str, timeout: float = 2.5) -> Optional[float]:
    market = normalize_ticker(ticker)
    if not market:
        return None
    url = "https://api.upbit.com/v1/ticker?" + urllib.parse.urlencode({"markets": market})
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": VERSION})
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            if isinstance(data, list) and data:
                price = float_any(data[0].get("trade_price"), default=0.0)
                return price if price > 0 else None
    except Exception as exc:
        log_error("fetch_upbit_price", exc)
    return None
...
def open_position(pos_id: str, ev: Dict[str, Any], control: Dict[str, Any]) -> Dict[str, Any]:
    ...
    live_price = fetch_upbit_price(ticker) or detected_price
...
def update_position(pos: Dict[str, Any], control: Dict[str, Any]) -> Tuple[Dict[str, Any], Optional[Dict[str, Any]]]:
    ...
    if entry <= 0:
        return pos, None
    price = fetch_upbit_price(ticker) or float_any(pos.get("current_price"), pos.get("entry_price"), default=entry)
        ...
        for pos_id, ev in picked:
            try:
                if pos_id not in open_pos:
                    open_pos[pos_id] = open_position(pos_id, ev, control)
                    opened += 1
            except Exception as exc:
...
def summary_text() -> str:
    ...
    return (
        ...
        f"- flag: {'ON' if FILES['flag'].exists() else 'OFF'} / 오류로그 {err_size} bytes"
    )
...
def handle_command(text: str) -> str:
    ...
    if cmd == "/pstart":
        ...
        FILES["flag"].write_text(f"paper_bot_on {iso_ts()}\n", encoding="utf-8")
    ...
    if cmd == "/perror":
        return "⚠️ 오류 로그\n" + tail_file(FILES["error"], 30)
    if cmd == "/pflag_on":
        FILES["flag"].write_text(f"paper_bot_on {iso_ts()}\n", encoding="utf-8")
        return "✅ external_paper_runner_on.flag 생성 완료\n메인봇 내부 paper pause 신호야."
    if cmd == "/pflag_off":
        try:
            FILES["flag"].unlink(missing_ok=True)
        except Exception:
            pass
        return "✅ external_paper_runner_on.flag 제거 완료\n메인봇 내부 paper 유지/재개 신호야."
