...
def compute_rebreakout_structure_context(ticker: str, current_price: float, df1=None) -> dict:
    ...
    try:
        ...
        if upper_wick_pct_1 <= 0.40:
            ...
            compression_score -= 0.20
        highs_tail = list(last6["high"].astype(float).values)
        closes_tail = list(last6["close"].astype(float).values)
        attempts = 0
        failed_attempts = 0
        for hv, cv in zip(highs_tail, closes_tail):
            if prev_high > 0 and hv >= prev_high * 0.996:
                attempts += 1
                if cv < prev_high * 0.993:
                    failed_attempts += 1
        hold_near_high_bars = 0
        for v in closes_tail[::-1]:
            if prev_high > 0 and v >= prev_high * 0.993:
                hold_near_high_bars += 1
            else:
                break
        hold_near_high_bars_05 = sum(1 for v in closes_tail if prev_high > 0 and v >= prev_high * 0.995)
        hold_near_high_bars_10 = sum(1 for v in closes_tail if prev_high > 0 and v >= prev_high * 0.990)
