...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    lines = [
        ...
        f"- 차트수치: {str(row.get('chart_quant_label', '') or '차트 수치 확인중')} / 몸통1봉 {safe_float(row.get('body_ratio_1', 0), 0)*100:.0f}%·3봉 {safe_float(row.get('body_ratio_3', 0), 0)*100:.0f}% / 종가강도 1봉 {safe_float(row.get('close_pos_1', 0), 0)*100:.0f}%·3봉 {safe_float(row.get('close_pos_3', 0), 0)*100:.0f}% / 윗꼬리 1봉 {safe_float(row.get('upper_wick_pct_1', 0), 0):.2f}%·3봉 {safe_float(row.get('upper_wick_pct_3', 0), 0):.2f}% / 아랫꼬리 1봉 {safe_float(row.get('lower_wick_pct_1', 0), 0):.2f}%·3봉 {safe_float(row.get('lower_wick_pct_3', 0), 0):.2f}% / 연속양봉 {int(safe_float(row.get('bullish_streak_5', 0), 0))} / 눌림 {safe_float(row.get('pullback_pct_18', 0), 0):.2f}%→회복 {safe_float(row.get('recovery_from_low_pct', 0), 0):.2f}%({safe_float(row.get('recovery_speed_pct', 0), 0):.2f}/봉) / 압축 {safe_float(row.get('compression_score', 0), 0):.2f} / 재돌파 {safe_float(row.get('rebreakout_score', 0), 0):.2f}({safe_float(row.get('rebreakout_success_rate', 0), 0)*100:.0f}%) / 유지 {safe_float(row.get('breakout_hold_score', 0), 0):.2f}({int(safe_float(row.get('hold_near_high_bars', 0), 0))}봉){rebreakout_gap_text}",
...
def merge_missed_review_row(base: dict, extra: dict):
    ...
    for key in [
        ...
        "body_ratio_1", "body_ratio_3", "upper_wick_pct_1", "upper_wick_pct_3", "lower_wick_pct_1", "lower_wick_pct_3",
        "close_pos_1", "close_pos_3", "bullish_streak_5", "bearish_streak_5", "strong_bullish_count_5",
        "pullback_pct_18", "recovery_from_low_pct", "recovery_speed_pct", "pullback_duration_bars",
        "compression_score", "rebreakout_score", "rebreakout_attempts", "rebreakout_success_rate", "hold_near_high_bars", "breakout_hold_score",
        "chart_quant_label",
    ]:
...
def format_missed_review_log_block(row: dict, idx: int = 0) -> str:
    ...
    return "\n".join([
        ...
        *([f"- 차트수치: {str(row.get('chart_quant_label', '') or '차트 수치 기록 없음')} / 몸통1봉 {safe_float(row.get('body_ratio_1', 0), 0)*100:.0f}%·3봉 {safe_float(row.get('body_ratio_3', 0), 0)*100:.0f}% / 종가강도 {safe_float(row.get('close_pos_1', 0), 0)*100:.0f}% / 윗꼬리 {safe_float(row.get('upper_wick_pct_1', 0), 0):.2f}% / 회복속도 {safe_float(row.get('recovery_speed_pct', 0), 0):.2f}/봉 / 재돌파 {safe_float(row.get('rebreakout_score', 0), 0):.2f} / 유지 {safe_float(row.get('breakout_hold_score', 0), 0):.2f}"] if row.get('chart_quant_label') else []),
...
def compute_htf_setup_context(ticker: str, current_price: float) -> dict:
    ...
    try:
        ...
        if 0 <= zone_ratio <= 0.68:
            ...
            score -= 0.55
        if ma7v > 0 and current_price >= ma7v:
            score += 0.20
        if ma30v > 0 and current_price >= ma30v:
            score += 0.20
...
def compute_rebreakout_structure_context(ticker: str, current_price: float, df1=None) -> dict:
    ...
    try:
        ...
        if upper_wick_pct_1 <= 0.40:
            ...
            compression_score -= 0.20
        highs = [safe_float(v, 0) for v in list(recent["high"].values)]
        attempts = 0
        for v in highs[-6:]:
            if prev_high > 0 and v >= prev_high * 0.996:
                attempts += 1
        hold_near_high_bars = 0
        closes_tail = [safe_float(v, 0) for v in list(last6["close"].values)]
