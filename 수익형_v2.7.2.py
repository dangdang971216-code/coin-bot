...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.2"
BOT_FILE_TAG = "수익형_v2.7.2.py"
TIMEZONE = "Asia/Seoul"
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque) -> MarketRow:
    ...
    # v2.7.2 점수: 단순 급등보다 눌림 재확인/저점방어/돈 유지에 가중.
    ...
    return MarketRow(
        ...
        range30_pct=range30_pct, score=raw, updated_ts=ts
    )
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    예전 v2.6에서 좋았던 눌림 판단을 코드 덩어리째 복붙하지 않고 기준만 재구성한다.
    - 좋은 눌림: 저점방어 + 1분/3분 회복 + 돈 유지 + 과열/고점추격 아님
    - 좋은 초입: 아직 과하게 안 뜬 상태에서 돈과 1분/3분 흐름이 막 붙는 구간
    - 돌파유지(BREAK_HOLD)는 별도 전략으로 키우지 않고, 상승시작 초입 안의 세부 상태로만 본다.
    """
    ...
    # 현재 API 값은 24h 누적 대금이라, 실전 유동성 최소 문턱으로 사용한다.
    money_watch = row.money24h >= 100_000_000     # 1억: 감시 가능
    money_ok = row.money24h >= 500_000_000        # 5억: 초록/노랑 본선 가능
    money_strong = row.money24h >= 3_000_000_000  # 30억: 강한 돈 유입

    high_touch = row.high_gap30 < 0.12
    one_min_recovered = data_1m_ok and row.ch1 >= 0.03
    three_min_alive = data_3m_ok and row.ch3 >= 0.12
    five_min_alive = data_5m_ok and row.ch5 >= 0.10
    one_min_milrim = data_1m_ok and row.ch1 < -0.10 and data_3m_ok and row.ch3 >= 0.20

    # v2.6 눌림라인의 핵심 guard를 clean core에 맞게 재구성.
    # 많이 오른 뒤 고점에 딱 붙은 상태는 바로 추격하지 않고 대기시킨다.
    hot_near_high = False
    ...
    late_chase = hot_near_high or too_extended
    ...
    # 1) 눌림 재확인: 예전 승률 좋았던 쪽을 우선 복구.
    # 완전히 바닥에서 안 뜬 후보보다, 약간 눌렸다가 1분/3분이 다시 살아나는 후보를 본다.
    pullback_reconfirm = (
        money_ok
        and one_min_recovered
        and three_min_alive
        and row.score >= 0.85
        and 0.18 <= row.from_low30 <= 2.60
        and not late_chase
        and not one_min_milrim
    )

    # 2) 상승시작 초입: 많이 뜨기 전, 돈과 1분/3분이 동시에 붙는 후보.
    # BREAK_HOLD는 여기 안에서만 허용한다. 돈이 얇은 단순 돌파는 초록 금지.
    max_short = max(row.ch1 if data_1m_ok else -999, row.ch3 if data_3m_ok else -999, row.ch5 if data_5m_ok else -999)
    start_early = (
        money_ok
        and data_3m_ok
        and 0.18 <= max(row.ch1, row.ch3) <= 1.35
        and row.ch1 >= -0.03
        and row.ch3 >= 0.15
        and row.from_low30 <= 1.75
        and not late_chase
    )
    start_break_hold = (
        money_ok
        ...
        and row.range30_pct <= 3.2
        and not hot_near_high
    )

    # 5분 값이 없으면 초록을 완전히 막지는 않되, 강한 초입/눌림 근거가 있어야 한다.
    # 실전 표본에서 5분 정보가 쌓이면 더 안정적으로 분류된다.
    ...
    # 노랑: 좋은 후보일 수 있으나 아직 재확인 전.
    pending_reasons = []
    if money_ok and (three_min_alive or five_min_alive or row.ch1 >= 0.18 or one_min_milrim or row.from_low30 <= 1.0):
        if one_min_milrim:
        ...
        pattern = "PULLBACK_WAIT" if row.from_low30 >= 0.15 or high_touch else "START_EARLY_WAIT"
        label = "눌림대기" if pattern == "PULLBACK_WAIT" else "초입대기"
        return make_candidate(row, "PENDING", label, "🟡", " / ".join(pending_reasons), pattern=pattern, caution="재확인 필요", entry_note="1분 회복·저점방어·고점추격 여부 확인", reject_reason="아직 PENDING")
...
def render_error_doctor() -> str:
    ...
    return (
        f"🩺 오류/병목 진단 /error_doctor v2.7.2\n\n"
        f"1) 현재 판정\n"
        ...
        f"- 출력은 요청 시 경량 조립\n"
        f"- v2.7.2: 눌림 재확인 라인을 clean core 방식으로 복구\n\n"
...
def render_buy_check() -> str:
    ...
    def add_group(title, items, limit):
        ...
        for idx, c in enumerate(items[:limit], 1):
            ...
            lines.append(f"- 돈 {krw(c.money24h)} / 저점대비 {c.from_low30:+.2f}% / 고점거리 {c.high_gap30:.2f}% / 30분폭 {c.range30_pct:.2f}%")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.2는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.2\n"
        f"- v2.7.2 clean core는 실거래 주문 보호모드야.\n"
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.2\n"
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.2 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
