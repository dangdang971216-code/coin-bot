...
def websocket_status_text() -> str:
    ...
    lines = [
        ...
        "",
        "판독",
        "- v181은 메인봇이 웹소켓을 직접 실행하지 않고 외부 sidecar 캐시만 읽습니다.",
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.181: 결과창은 paper_bot 기준으로 통일, 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만.",
    }
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.181.py", "- 페이퍼봇 대상: paper_bot_v0.33.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.181: 웹소켓 원인제거 + 안정스캔 복구 + 기준 통일 유지", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v181 외부 websocket sidecar 캐시 연동 + 안정스캔 유지 + paper_bot 기준 통일 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
