...
- 실거래 주문은 v2.7.10 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.10"
BOT_FILE_TAG = "수익형_v2.7.10.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.10는 현재가+체결 허브를 후보분류에 더 적극 반영한다.
...
def paper_active_for_ticker_locked(ticker: str, pattern: str) -> PaperCandidateRow | None:
    ...
    for r in STATE.paper_candidates.values():
        if r.ticker == ticker and r.active and (now - r.entry_ts) < PAPER_TRACK_SEC_30M:
            if r.pattern == pattern or paper_strategy_name(r.pattern) == paper_strategy_name(pattern):
                return r
    return None
...
def update_paper_store_locked(candidates: list[Candidate], market_rows: dict[str, MarketRow]):
    ...
    for c in candidates:
        ...
        if paper_active_for_ticker_locked(c.ticker, pattern):
            continue
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.10 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    # v2.7.10: 특수 대기상태 보존. TON류 재출발/재돌파/급등후재확인 후보가
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.10: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.10는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.10\n"
        f"- v2.7.10 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.10에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.10\n"
...
def _paper_line(r: PaperCandidateRow) -> str:
    ...
    return f"- {r.ticker}: {r.strategy} / {live} / 진입 {price_text(r.entry_price)} / 현재 {r.best_pct:+.2f}~{r.worst_pct:+.2f}% / {age}"
...
def render_paper_check_full() -> str:
    ...
    if active_rows:
        for r in active_rows[:20]:
            ...
            lines.append(f"- {r.ticker} / {r.strategy} / {r.pattern} / 진입 {price_text(r.entry_price)} → 현재 {price_text(r.last_price)} ({cur_pct:+.2f}%) / 최고 {r.best_pct:+.2f}% / 최저 {r.worst_pct:+.2f}% / {age_text(r.entry_ts)}")
    else:
        lines.append("- 없음")
    lines += ["", "2) 완료/결과"]
    if done_rows:
        for r in done_rows[:20]:
            ...
            lines.append(f"- {r.ticker} / {r.strategy} / {r10} / {r30} / 최고 {r.best_pct:+.2f}% / 최저 {r.worst_pct:+.2f}%")
    else:
...
RENDERERS = {
    ...
    "/paper_flow": render_paper_check_full,
    "/paper_stats": render_paper_check_full,
...
RENDERERS.update({
    ...
    "/paper_flow": render_paper_check,
    "/paper_stats": render_paper_check,
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.10 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
