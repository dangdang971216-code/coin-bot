...
def send_startup_message():
    lines = [
        ...
        "- 놓친 종목 복기 보강: reverse_trace에서 나중에 잘 간 코인을 보조 놓침으로 다시 보이게 정리",
        "- review_dump / missed에서 핵심 놓침과 보조 놓침을 같이 확인 가능하게 정리",
    ...
    for ticker, row in due:
        ...
        if ok_spot and result_quality in {"good", "assist"} and not was_caught:
            ...
            if result_quality == "assist":
                row["reason_text"] = f"보조 놓침: {row['reason_text']}"
...
def summarize_missed_review_logs(rows):
    bucket = {"아예 못 잡음": 0, "봤는데 못 올림": 0, "움직였지만 늦음": 0, "판단 빗나감": 0, "보조 놓침": 0}
    for row in rows:
        if not isinstance(row, dict):
            continue
        miss_type = str(row.get("miss_type", "") or "")
        ...
        if str(row.get("miss_level", "핵심") or "핵심") == "보조":
            bucket["보조 놓침"] = bucket.get("보조 놓침", 0) + 1
...
def build_review_dump_sections(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5):
    ...
    try:
        ...
        if not missed_rows:
            ...
            lines = [
                "[놓친 강한 움직임 요약]",
                f"- 탐색 놓침 {summary.get('탐색 놓침', 0)}건 / 승격 놓침 {summary.get('승격 놓침', 0)}건 / 늦어서 안 올림 {summary.get('늦어서 안 올림', 0)}건 / 분류 놓침 {summary.get('분류 놓침', 0)}건 / 보조 놓침 {summary.get('보조 놓침', 0)}건",
                "",
