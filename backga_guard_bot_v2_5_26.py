...
VERSION = "guard_v2.5.26_smart_changed_only_progress_2026-05-15"
...
def apply_latest_auto(force: bool = False, explicit: Optional[str] = None, skip_guard: bool = False) -> str:
    ...
    중요도 순서: guard → main → paper → ws → micro.
    ...
    if not skip_guard:
        ...
        if guard_res.get("action") == "restart_guard":
            msg = (
                ...
                "- 다음 처리: 새 가드봇이 부팅되면 main → paper → ws → micro를 자동으로 이어서 확인/적용\n\n"
                + format_guard_apply_result(guard_res)
            )
            progress_notify(msg)
            return msg
    ...
    title = "🚀 ✅ 스마트 업그레이드 완료 /gupgrade" if all_ok else "🚀 ❔ 스마트 업그레이드 확인 필요 /gupgrade"
    progress_notify("✅ 스마트 업그레이드 단계 완료\n- 최종 결과 정리 전송 중")
    return (
        f"{title}\n"
        f"- guard: {VERSION}\n"
        f"- 순서: guard → main → paper → ws → micro\n"
        f"- 방식: hash 비교 후 바뀐 파일만 교체·재시작\n"
        f"- 최신 동일 항목: 재시작 생략\n"
...
def deploy_status() -> str:
    ...
    return (
        ...
        f"🧾 최근 오류 의심\n{focus_text}\n\n"
        f"명령\n"
        f"- /gupgrade : guard→main→paper→ws→micro 변경분만 스마트 적용\n"
...
# v2.5.26: 외부직원 통합상태. /gupgrade는 guard→main→paper→ws→micro 순서로 변경분만 적용.
...
def service_status() -> str:
    ...
    return (
        ...
        f"- 업그레이드: /gupgrade 한 번으로 guard→main→paper→ws→micro 변경분만 확인/적용\n\n"
        f"📊 상태\n"
        f"- guard: {VERSION}\n"
        ...
        f"🧭 자주 쓰는 명령\n"
        f"- /gdeploy 배포상태\n"
        f"- /gupgrade guard→main→paper→ws→micro 변경분만 자동적용\n"
        f"- /gpaper_restart 페이퍼봇 재시작\n"
...
def help_text() -> str:
    return (
        ...
        "/gupgrade - guard→main→paper→ws→micro 변경분만 자동 적용\n"
        "/glog - 메인봇 최근 오류 로그\n"
...
def set_commands() -> None:
    commands = [
        ...
        {"command": "gupgrade", "description": "전체 스마트 최신 적용"},
...
def main():
    ...
    if post_req:
        try:
            send(ALLOWED_CHAT_ID, "🛡 가드봇 최신화 완료\n- 이어서 main → paper → ws → micro 스마트 업그레이드를 자동 진행합니다.")
