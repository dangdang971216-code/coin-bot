# v2.5.161: 30초 스캔 목표 복구 + JSON 저장 안정화 유지
import os
import time
import json
import math
...
def build_review_dump_quick_summary(scan_display_at_start=None) -> str:
    ...
    lines = ["📌 빠른 요약"]
    try:
        display = scan_display_at_start if isinstance(scan_display_at_start, dict) else get_latest_scan_display_state(now_ts=time.time(), limit=3)
...
def build_review_dump_sections(alert_limit: int = 5, missed_limit: int = 5, trace_limit: int = 5):
    sections = []
    try:
        scan_display_at_start = get_latest_scan_display_state(now_ts=time.time(), limit=3)
    except Exception:
    ...
    try:
        display = scan_display_at_start if isinstance(scan_display_at_start, dict) else get_latest_scan_display_state(limit=3)
        note = format_scan_display_state_line(display)
        ...
        lines = ["[현재 스캔 상태 / 명령 입력 시점]"]
        if note:
            lines.append(f"- 안내: {note}")
        lines.append(f"- 바로 볼 후보 {len(user_items)}개 / 조금 더 볼 후보 {len(internal_items)}개 / 내부에서 넘긴 후보 {len(discard_items)}개")
        if shortlist:
            lines += ["", f"[상위 후보 {len(shortlist)}개]"]
            for idx, item in enumerate(shortlist, start=1):
                lines.append(
                    f"{circled_number(idx)} {item.get('ticker','?')} / "
                    f"{item.get('user_label_text', item.get('label_text','후보'))} / "
                    f"5분 {safe_float(item.get('change_5',0)):+.2f}% / "
                    f"거래량 {safe_float(item.get('vol_ratio',0)):.2f}배 / "
                    f"최근30분 {safe_float(item.get('below_30m_high_pct', item.get('below_high',999)),999):.2f}% / "
                    f"단기 {safe_float(item.get('rebreakout_gap_pct', item.get('high_gap_pct', 999)),999):.2f}%"
                )
                one_liner = str(item.get('one_liner', '') or '').strip()
                if one_liner:
                    lines.append(f"- 한줄: {one_liner}")
        else:
            lines.append("- 지금은 상단에 보여줄 후보 없음")
        sections.append(("scan_debug", "\n".join(lines)))
    except Exception as e:
        sections.append(("scan_debug", f"[현재 스캔 상태]\n- 생성 오류: {e}"))
...
def format_hourly_momentum_watch_report(cache: dict, limit: int = 8) -> str:
    ...
    lines = ["[1시간 급등 감시판]", "- 참고: 빗썸 실시간 순위 직접값이 아니라 봇 내부 캐시 기준이야"]
    if not rows:
