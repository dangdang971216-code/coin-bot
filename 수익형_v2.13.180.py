...
def websocket_hub_worker_loop() -> None:
    """v180 원인제거: legacy pybithumb WebSocketManager 실행 금지.

    v177/v178 문제의 직접 원인은 WebSocketManager가 자식 bot.py 프로세스를 만들고
    종료 때 systemd deactivating timeout을 만드는 것이었다.
    따라서 이 함수는 어떤 웹소켓 프로세스도 시작하지 않고 상태만 남긴다.
    실시간 WS 중심 구조는 메인봇 생명주기와 분리된 안전 worker로 별도 재도입한다.
    """
    with _state_lock:
        STATE["ws_state"] = "격리"
    ...
    log("websocket_hub_worker blocked in v180: legacy pybithumb WebSocketManager removed")
    return
...
def websocket_status_line() -> str:
    ...
    state = str(STATE.get("ws_state") or "격리")
    if state == "수신중":
        icon = "✅"
...
def websocket_status_text() -> str:
    return "\n".join([
        ...
        "",
        "판독",
        "- v180은 메인봇 먹통 원인인 pybithumb WebSocketManager를 완전히 격리한 복구판입니다.",
        "- REST 전체시장/정밀/위험 본선은 유지합니다.",
        "- 웹소켓 중심 구조는 별도 안전 worker로 재설계 후 다시 넣어야 합니다.",
    ])
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.180: 결과창은 paper_bot 기준으로 통일, 후보분석은 strict_all 전부 기록, paper OPEN은 trade_ready만.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.180 기준: 결과창 가독성 / paper_bot 기준 통일 / 정보품질 표시 유지",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.180.py", "- 페이퍼봇 대상: paper_bot_v0.33.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.180: 웹소켓 원인제거 + 안정스캔 복구 + 기준 통일 유지", "- 기존 기록 삭제 없음 / 후보판단 고정절단 없음 / paper_bot은 trade_ready만 OPEN"]))
    rows = []
    total = len(funcs)
    ...
    lines = ["🧾 v2.13.180 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def start_background_workers() -> None:
    ...
    log(f"background_workers started precision={PRECISION_BACKGROUND_WORKERS} execution_risk={EXEC_RISK_BACKGROUND_WORKERS} websocket=False requested={WS_HUB_REQUESTED}")
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v180 웹소켓 원인제거+안정복구 가독성 정리 + paper_bot 기준 통일 + 후보품질 요약 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
