...
def format_rebreakout_stat_text(item_or_row: dict, compact: bool = False) -> str:
    ...
    if attempts <= 0:
        return f"재돌파 {re_score:.2f}(시도없음)"
    if success_attempts <= 0 and failed_attempts <= 0:
        return f"재돌파 {re_score:.2f}(확인중)"
    if compact:
        return f"재돌파 {re_score:.2f}({success_rate:.0f}% / 실패 {failed_attempts}회)"
...
def build_missed_review_reason_text(row: dict, miss_type: str) -> str:
    ...
    if miss_type == "판단 빗나감":
        if alert_section == "late":
            return "움직이긴 했는데 늦은 후보로 너무 아래에 내려버림"
        if alert_section == "watch":
            return "움직이긴 했는데 감시만 할 후보로만 둠"
        return "움직일 자리였는데 보수적으로 분류함"
    if miss_type == "움직였지만 늦음":
        return "움직임은 컸지만 우리가 잡아야 할 초입보단 늦은 쪽이라 무조건 살릴 표본은 아님"
    if miss_type == "봤는데 못 올림":
        if block_reason:
...
def summarize_missed_review_logs(rows):
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        miss_type = str(row.get("miss_type", "") or "")
        if str(row.get("miss_level", "핵심") or "핵심") == "보조":
...
def get_missed_review_display_type(row: dict) -> str:
    ...
    if str((row or {}).get("miss_level", "핵심") or "핵심") == "보조":
        return "움직였지만 애매"
    return base
