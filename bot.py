...
def format_watch_alert_line(item: dict) -> str:
    tag_text = ""
    tags = item.get("pattern_tags", [])
    if tags:
        tag_text = f" / 구조 {','.join(tags[:3])}"
    return (
        f"• {item['ticker']} / 현재가 {fmt_price(item['current_price'])} / 상승 {float(item.get('change_pct', 0)):.2f}% / 거래량 {float(item.get('vol_ratio', 0)):.2f}배"
        f"\n  ↳ 전략: {format_strategy_with_help(item['strategy_label'])}"
        f"\n  ↳ 등급 {item.get('trade_tier','B')} / 시장주목도 {safe_float(item.get('leader_score',0)):.2f}{tag_text}"
    )
