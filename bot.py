...
# =========================================================
TOP_TICKERS = 60
ABSOLUTE_TURNOVER_CANDIDATES = 40
SURGE_CANDIDATES = 50
UNIVERSE_REFRESH_SEC = 30
MARKET_CACHE_TTL_SEC = 12
...
PULLBACK_LATE_FROM_LOW_WARN_PCT = 8.0
PULLBACK_LATE_FROM_LOW_MAX_PCT = 8.8
PULLBACK_LATE_FROM_LOW_HARD_PCT = 10.2
PULLBACK_LATE_RETEST_GAP_MAX_PCT = 4.8
PULLBACK_LATE_WEAK_REBOUND_PCT = 1.35

TREND_CONT_LATE_FROM_LOW_WARN_PCT = 4.8
TREND_CONT_LATE_FROM_LOW_MAX_PCT = 5.8
TREND_CONT_LATE_RETEST_GAP_MAX_PCT = 0.85
TREND_CONT_OBSERVE_MIN_VOL = 1.90
TREND_CONT_OBSERVE_MIN_LEADER = 1.55
...
def build_price_check_markup(entries):
    ...
    for idx, entry in enumerate(entries, start=1):
        item, alert_type, _reason_text, header = unpack_recent_entry(entry)
        ...
        ticker = item.get("ticker", "?")
        rows.append([
            InlineKeyboardButton(f"{circled_number(idx)} {ticker} 빗썸 열기", url=get_bithumb_chart_url(ticker)),
        ])
        rows.append([
...
def build_price_check_lines(ref: dict, current_price: float, now_ts: float, target_min: int = 0):
    ...
    if target_min <= 0:
        title = f"💰 {ticker} 지금 확인"
    else:
        title = f"⏱ {ticker} {target_min}분 확인"

    lines = [title, ""]
    if source_label:
        lines.append(f"- 알림종류: {display_strategy_label(source_label)}")
    if strategy_label:
        lines.append(f"- 전략: {display_strategy_label(strategy_label)}")
    if judgement_text:
        lines.append(f"- 한눈판단: {judgement_text}")
    if source_label or strategy_label or judgement_text:
        lines.append("")
...
def recent5_command(update, context: CallbackContext):
    ...
    body = []
    for idx, entry in enumerate(entries, start=1):
        item, alert_type, reason_text, _header = unpack_recent_entry(entry)
    ...
    send("🗂 최근 후보 5개\n\n최근에 뜬 후보를 버튼과 함께 다시 보여줄게.\n\n" + "\n\n".join(body), reply_markup=markup)


def sort_watch_entries(entries):
    rows = []
    for entry in entries:
        item, alert_type, reason_text, header = unpack_recent_entry(entry)
        rows.append((get_watch_display_priority(item, alert_type=alert_type), item, alert_type, reason_text, header))
    rows.sort(key=lambda x: x[0], reverse=True)
    return [(item, alert_type, reason_text, header) for _score, item, alert_type, reason_text, header in rows]


def send_watch_alert_bundle(header: str, entries):
    if not entries:
        return
    sorted_entries = sort_watch_entries(entries)
    save_recent_candidate_alerts([(item, alert_type, reason_text) for item, alert_type, reason_text, _header in sorted_entries], header=header)
    body = []
    for idx, entry in enumerate(sorted_entries, start=1):
        item, alert_type, reason_text, _header = unpack_recent_entry(entry)
        body.append(format_watch_alert_line(item, order=idx, alert_type=alert_type, reason_text=reason_text))
    markup = build_price_check_markup([(item, alert_type, reason_text) for item, alert_type, reason_text, _header in sorted_entries])
    send(header + "\n\n" + "\n\n".join(body), reply_markup=markup)
...
def display_strategy_label(strategy_label: str) -> str:
    return DISPLAY_STRATEGY_LABEL.get(strategy_label, strategy_label)

def display_pattern_tag(tag: str) -> str:
    return DISPLAY_PATTERN_TAG.get(tag, tag)

STRATEGY_GUIDE = {
    "초반 선점형": "막 움직이기 전 먼저 보는 자리",
    "상승 시작형": "막 오르기 시작한 자리",
    "눌림 반등형": "올랐다가 눌린 뒤 다시 오르는 자리",
    "추세 지속형": "이미 오르던 흐름이 이어지는 자리",
    "쏘기 직전형": "강하게 튀기 직전처럼 보이는 자리",
    "급등 돌파형": "저항을 세게 뚫는 자리",
    "추격형": "이미 오른 흐름을 따라가는 자리",
    "주도주 후보형": "지금 시장에서 눈에 띄는 코인",
    "저점 선점형": "저점 근처에서 먼저 보는 초기 자리",
    "신저가 후 회복 감시": "바닥 근처 찍고 다시 오르는지 보는 자리",
}

TERM_GUIDE = [
    ("등급 S/A/B", "S가 제일 좋고 B는 후보는 맞지만 힘이 약한 편"),
    ("시장주목도", "지금 시장에서 얼마나 눈에 띄는지"),
    ("지금 사도 될 쪽", "상대적으로 더 이른 자리라 우선해서 보는 후보"),
    ("한 번 볼 후보", "볼만은 하지만 바로 따라붙기 전 한 번 더 보는 후보"),
    ("조금 더 지켜볼 후보", "지금 바로 사기보다 흐름을 더 확인하는 후보"),
    ("이전 후보 다시 확인", "전에 본 후보를 다시 체크하라는 뜻"),
    ("자동 쉬기", "손실이나 흐름 문제로 자동매수를 잠깐 쉬는 상태"),
]
...
def format_strategy_with_help(strategy_label: str) -> str:
    shown = display_strategy_label(strategy_label)
    return f"{shown} ({strategy_help_text(strategy_label)})"
...
def get_watch_item_icon(item: dict, alert_type: str = "new") -> str:
    ...
    if strategy_label == "저점 선점형":
        return "🎯"
    if strategy_label == "신저가 후 회복 감시":
        return "🧲"
    if change_pct >= 12.0:
        return "⚠️"
...
def get_watch_caution_text(item: dict) -> str:
    ...
    strategy_label = item.get("strategy_label", "")
    current_price = safe_float(item.get("current_price", 0))
    reference_low = safe_float(item.get("reference_low", 0))
    high_gap_pct = get_reference_high_gap_pct(item)
    if strategy_label == "저점 선점형":
        return "저점 초입 후보라 재이탈하면 바로 약해질 수 있어"
    if strategy_label == "신저가 후 회복 감시":
        return "신저가 회복형이라 저가 재이탈하면 바로 약해질 수 있어"
    if strategy_label == "눌림 반등형" and reference_low > 0 and current_price > reference_low:
        rebound_from_low_pct = ((current_price - reference_low) / reference_low) * 100.0
        if rebound_from_low_pct >= PULLBACK_LATE_FROM_LOW_WARN_PCT:
            return "눌림 반등형인데 저점 대비 이미 많이 올라 늦을 수 있어"
    if strategy_label == "추세 지속형" and reference_low > 0 and current_price > reference_low:
        rebound_from_low_pct = ((current_price - reference_low) / reference_low) * 100.0
        if rebound_from_low_pct >= TREND_CONT_LATE_FROM_LOW_WARN_PCT and high_gap_pct <= 1.20:
            return "추세 지속형인데 저점 대비 이미 많이 올라 늦을 수 있어"
    if strategy_label == "주도주 관찰형" and vol_ratio < 1.80 and change_pct < 1.00:
        return "관찰 후보이긴 하지만 아직 힘이 충분히 붙었다고 보긴 어려워"
    if 0 <= high_gap_pct <= 0.70:
        return "최근 고점 근처라 늦은 자리일 수 있어"
    if change_pct >= 18.0:
    ...
    if strategy_label == "추세 지속형" and change_pct >= 1.05 and high_gap_pct <= 0.95:
        return "추세 지속형인데 최근 고점 바로 아래라 추격이 될 수 있어"
    if change_pct >= 10.0 and vol_ratio < 2.4:
        return "상승폭 대비 거래량이 아주 강한 편은 아니야"
    return ""


def get_watch_bucket(item: dict):
    strategy_label = item.get("strategy_label", "")
    change_pct = safe_float(item.get("change_pct", 0))
    vol_ratio = safe_float(item.get("vol_ratio", 0))
    leader_score = safe_float(item.get("leader_score", 0))
    high_gap_pct = get_reference_high_gap_pct(item)
    daily_ready = safe_float(item.get("daily_ready_score", 0))
    caution = get_watch_caution_text(item)

    if strategy_label == "저점 선점형":
        return 3, "🎯 바닥 근처 후보"
    if strategy_label == "신저가 후 회복 감시":
        return 3, "🧲 바닥 찍고 반등 보는 후보"
    if caution:
        return 0, "⚠️ 늦어서 조심"

    if strategy_label in ["상승 시작형", "초반 선점형"]:
        if change_pct <= 1.55 and vol_ratio >= 1.95 and leader_score >= 1.45 and high_gap_pct >= 1.10 and daily_ready >= 0.95:
            return 3, "🟢 지금 사도 될 쪽"
        if change_pct <= 2.70 and vol_ratio >= 1.70:
            if daily_ready >= 0.85 and high_gap_pct >= 0.95:
                return 2, "✅ 막 오르기 시작한 후보"
        return 1, "👀 한 번 더 볼 후보"

    if strategy_label == "눌림 반등형":
        if change_pct <= 1.40 and vol_ratio >= 2.00 and high_gap_pct >= 1.00 and daily_ready >= 0.75:
            return 2, "✅ 다시 힘 붙는 후보"
        return 1, "👀 조금 더 지켜볼 후보"

    if strategy_label in ["추세 지속형", "급등 돌파형"]:
        if change_pct <= 1.35 and high_gap_pct >= 1.10 and vol_ratio >= 1.90 and daily_ready >= 0.70:
            return 2, "✅ 힘 붙는 후보"
        return 1, "👀 힘은 있는데 더 지켜볼 후보"

    return 1, "👀 한 번 볼 후보"


def get_watch_judgement_text(item: dict) -> str:
    return get_watch_bucket(item)[1]


def get_watch_action_badge(item: dict) -> str:
    bucket, label = get_watch_bucket(item)
    if bucket >= 3:
        return "🟢"
    if bucket == 2:
        return "🟡"
...
def get_watch_display_priority(item: dict, alert_type: str = "new") -> float:
    bucket, _ = get_watch_bucket(item)
    edge = safe_float(item.get("edge_score", item.get("signal_score", 0)))
    ...
    score += edge * 3.0
    score += leader * 2.0
    score += min(vol_ratio, 10.0) * 1.1
    score += daily_ready * 3.4
    score -= max(change_pct - 1.35, 0.0) * 2.0
    if 0 <= high_gap_pct < 0.75:
        score -= 4.5
...
def should_show_trade_plan_in_alert(item: dict) -> bool:
    strategy_label = item.get("strategy_label", "")
    return strategy_label not in ["저점 선점형", "신저가 후 회복 감시"]


def get_alert_no_chase_pct(item: dict) -> float:
    strategy = item.get("strategy", "")
    if strategy in ["BREAKOUT", "CHASE"]:
        return 0.60
    if strategy in ["TREND_CONT", "PULLBACK"]:
        return 0.80
    return 1.00
...
def format_watch_alert_line(item: dict, order: int = 1, alert_type: str = "new", reason_text: str = "") -> str:
    ...
    tag_text = ", ".join(display_pattern_tag(tag) for tag in tags[:3]) if tags else ""
    icon = get_watch_item_icon(item, alert_type=alert_type)
    caution = get_watch_caution_text(item)
    judgement = get_watch_judgement_text(item)
    action_badge = get_watch_action_badge(item)
    tier_badge = format_tier_badge(item.get('trade_tier','B'))
    lines = [
        f"{circled_number(order)} {action_badge} {tier_badge} {icon} {item['ticker']}",
        f"- 지금판단: {action_badge} {judgement}",
        ...
        f"- 등급/눈에띔: {tier_badge} / 👁 {safe_float(item.get('leader_score',0)):.2f}",
    ]
    daily_ready = safe_float(item.get("daily_ready_score", 0))
    if daily_ready > 0:
        lines.append(f"- 큰흐름준비: 🌙 {daily_ready:.2f}")
...
def send_startup_message():
    lines = [
        ...
        "",
        "정리 포인트",
        "- 시작 알림은 최근 핵심 변경만 짧게 표시",
        "- 큰 흐름 준비도(일봉 쪽) 점수를 같이 반영",
        "- 저점 선점 후보를 너무 안 뜨지 않게 소폭 완화",
        "- 막 오르기 시작함 / 돌파형은 큰 흐름 준비가 약하면 보수화",
        "- 자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def get_daily_ready_score(df, current_price: float) -> float:
    try:
        ...
        day_change_3 = get_recent_change_pct(recent, 3)

        lows = recent["low"].tail(6).tolist()
        rising_lows = 0
        for i in range(1, len(lows)):
            if float(lows[i]) >= float(lows[i - 1]) * 0.995:
                rising_lows += 1
        ...
        if ma5 > 0 and current_price >= ma5:
            score += 0.35
        if ma20 > 0 and ma5 >= ma20 * 0.96:
            score += 0.30
        if -1.8 <= day_change_3 <= 7.0:
            score += 0.20
        if rising_lows >= 3:
            score += 0.35
        if current_price >= float(close.tail(10).min()) * 1.03:
            score += 0.25
        if hi / lo >= 1.60 and pos <= 0.58:
            score += 0.15

        return round(min(score, 1.80), 2)
    except Exception:
        return 0.0
...
def bars_since_recent_low(df, lookback=12):
    try:
        recent = df.tail(lookback).reset_index(drop=True)
        if len(recent) <= 0:
            return 999
        low_pos = int(recent["low"].idxmin())
        return int(len(recent) - 1 - low_pos)
    except Exception:
        return 999


def recent_reclaim_body_ok(df, min_body_ratio=0.30):
    try:
        ...
        if float(last["close"]) < float(last["open"]):
            return False
        if float(last["close"]) < float(prev["close"]) * 1.0005:
            return False
        if candle_body_ratio(df) < min_body_ratio:
            return False
        return True
    except Exception:
        return False
...
def has_recent_spike_then_fade(df, lookback=12, min_spike_pct=0.85, min_pullback_pct=0.25, min_peak_age=2):
    try:
        recent = df.tail(lookback).reset_index(drop=True)
        if len(recent) < 6:
            return False
        low = float(recent["low"].min())
        peak_pos = int(recent["high"].idxmax())
        high = float(recent["high"].iloc[peak_pos])
        current = float(recent["close"].iloc[-1])
        if low <= 0 or high <= low or current <= 0:
            return False
        spike_pct = ((high - low) / low) * 100.0
        pullback_pct = ((high - current) / high) * 100.0
        bars_since_peak = int(len(recent) - 1 - peak_pos)
        return spike_pct >= min_spike_pct and pullback_pct >= min_pullback_pct and bars_since_peak >= min_peak_age
    except Exception:
        return False
...
def build_shared_market_cache(force=False):
    ...
    for row in universe:
        ...
        try:
            ...
            df_day = get_ohlcv(ticker, "day")
            daily_ready_score = get_daily_ready_score(df_day, price)
            ...
            cache[ticker] = {
                ...
                "df_3m": row["df_3m"],
                "df_day": df_day,
                "daily_ready_score": daily_ready_score,
...
def base_signal(ticker, strategy, strategy_label, current_price, vol_ratio, change_pct, rsi=None, range_pct=None,
    ...
    return {
        ...
        "reason": reason,
        "daily_ready_score": r(daily_ready_score, 2),
        "trade_tier": "B",
    }
...
def analyze_early_entry(ticker: str, data: dict):
    ...
    if rebound_from_low_pct > EARLY_CFG["rebound_max"]:
        return None
    if rebound_from_low_pct > 2.10 and high_gap_pct < 0.90:
        return None
    ...
    return base_signal(
        ...
        daily_ready_score=safe_float(data.get("daily_ready_score", 0)),
    )
...
def analyze_pullback_entry(ticker: str, data: dict):
    ...
    rebound_from_low_pct = ((current_price - recent_low) / recent_low) * 100 if recent_low > 0 else 0.0
    ...
    # v3: 눌림 반등형인데 이미 저점에서 너무 많이 올라온 늦은 재반등 후보는 더 줄인다.
    if rebound_from_low_pct >= PULLBACK_LATE_FROM_LOW_HARD_PCT:
        return None
    if total_pump_pct >= 9.0 and rebound_from_low_pct >= PULLBACK_LATE_FROM_LOW_MAX_PCT and retest_gap_pct <= PULLBACK_LATE_RETEST_GAP_MAX_PCT:
        return None
    if total_pump_pct >= 12.0 and rebound_from_low_pct >= 7.2 and rebound_pct < PULLBACK_LATE_WEAK_REBOUND_PCT:
        return None
...
def analyze_trend_cont_entry(ticker: str, data: dict):
    ...
    rebound_from_low_pct = ((current_price - rise_low) / rise_low) * 100 if rise_low > 0 else 0.0
    if rebound_from_low_pct >= TREND_CONT_LATE_FROM_LOW_MAX_PCT:
        return None
    if trend_change_pct >= 4.0 and rebound_from_low_pct >= TREND_CONT_LATE_FROM_LOW_WARN_PCT and retest_gap_pct <= TREND_CONT_LATE_RETEST_GAP_MAX_PCT:
        return None
...
def get_watch_quality_block_reason(item: dict, alert_type: str = "new"):
    ...
    rebound_from_low_pct = ((current_price - reference_low) / reference_low) * 100.0 if reference_low > 0 and current_price > reference_low else 0.0
    high_gap_pct = get_reference_high_gap_pct(item)

    if strategy == "PREPUMP" and tier == "B":
        if high_gap_pct < 0.55:
            return "최근 고점과 너무 가까운 상승 시작형 B라 제외"
        if leader_score < 1.90 and vol_ratio < 4.80:
            return "주목도/거래량이 약한 상승 시작형 B라 제외"
        if change_pct < 1.35 and vol_ratio < 4.20:
            return "초반 힘이 약한 상승 시작형 B라 제외"

    if strategy == "PULLBACK" and tier == "B":
        if rebound_from_low_pct >= PULLBACK_LATE_FROM_LOW_WARN_PCT and high_gap_pct < 1.10:
            return "저점 대비 이미 많이 오른 눌림 반등형 B라 제외"
        if leader_score < 1.35 and vol_ratio < 1.90:
            return "주목도/거래량이 약한 눌림 반등형 B라 제외"
        if change_pct < 1.10 and vol_ratio < 2.10:
            return "반등 힘이 약한 눌림 반등형 B라 제외"

    if strategy == "TREND_CONT" and tier == "B":
        if high_gap_pct < 0.95:
            return "최근 고점과 너무 가까운 추세 지속형 B라 제외"
        if rebound_from_low_pct >= TREND_CONT_LATE_FROM_LOW_WARN_PCT and high_gap_pct < 1.20:
            return "저점 대비 이미 많이 오른 추세 지속형 B라 제외"
        if leader_score < TREND_CONT_OBSERVE_MIN_LEADER and vol_ratio < TREND_CONT_OBSERVE_MIN_VOL:
            return "주목도/거래량이 약한 추세 지속형 B라 제외"
        if change_pct < 0.45 and vol_ratio < 2.05:
            return "재상승 힘이 약한 추세 지속형 B라 제외"

    if strategy == "LEADER_WATCH" and strategy_label == "주도주 관찰형":
        if leader_score < 1.25 and vol_ratio < 1.55:
            return "주도주 관찰형인데 주목도/거래량이 약해 제외"
        if high_gap_pct < 0.90 and change_pct > 0.70:
            return "주도주 관찰형인데 최근 고점에 너무 가까워 제외"
        if edge < 4.9 and vol_ratio < 1.75:
            return "관찰 후보치고 점수/거래량이 약해 제외"
    ...
    if alert_type in ["upgrade", "renotice"]:
        ...
        if strategy == "PREPUMP" and leader_score < 2.20 and vol_ratio < 5.80:
            return "재알림치고 힘이 약한 상승 시작형 B라 제외"
        if strategy == "TREND_CONT" and rebound_from_low_pct >= TREND_CONT_LATE_FROM_LOW_WARN_PCT and high_gap_pct < 1.35:
            return "재알림 시점이 이미 늦어진 추세 지속형이라 제외"
        if strategy == "PULLBACK" and rebound_from_low_pct >= PULLBACK_LATE_FROM_LOW_WARN_PCT and high_gap_pct < 1.30:
            return "재알림 시점이 이미 늦어진 눌림 반등형이라 제외"
    ...
    for ticker, data in cache.items():
        try:
            ...
            if not top_rank_ok and activity_score < 0.85:
                continue
            if vol_ratio < 1.00 and change_5 < 0.20 and daily_ready_score < 0.95:
                continue
            ...
            label = "테스트 관찰형"
            if change_5 >= 0.45 and vol_ratio >= 1.15 and daily_ready_score >= 0.90:
                label = "테스트 상승형"
            elif top_rank_ok or daily_ready_score >= 1.00:
                label = "테스트 주도주형"
            ...
            quick.append(make_signal(
                ...
                time_stop_sec=600,
                pattern_tags=[],
                reason=f"테스트 완화 후보 / 5분상승 {change_5:.2f}% / 거래량 {vol_ratio:.2f}배 / 주도 {leader_score:.2f} / 큰흐름 {daily_ready_score:.2f} / 순위 T{turnover_rank}/S{surge_rank}",
                daily_ready_score=daily_ready_score,
            ))
        except Exception:
            continue
...
def build_prewatch_candidates(cache, regime, exclude_tickers=None):
    ...
    for ticker, data in cache.items():
        try:
            ...
            low_age = bars_since_recent_low(df, 12)
            body_ok = recent_reclaim_body_ok(df, min_body_ratio=0.32)

            if change_5 >= 1.6 or change_3 >= 1.05:
                continue
            if change_1 <= -0.35 and change_3 <= -0.08:
                continue
            if max(change_1, change_3, change_5) < 0.15:
                continue
            if momentum_hint <= 0.15:
                continue
            if daily_ready_score >= 0.95:
                if not (1.85 <= vol_ratio <= 5.2):
                    continue
            else:
                if not (2.10 <= vol_ratio <= 5.0):
                    continue
            if not (48 <= rsi <= 64):
                continue
            if daily_ready_score >= 0.95:
                if not (0.85 <= high_gap_pct <= 3.10):
                    continue
            else:
                if not (1.00 <= high_gap_pct <= 2.80):
                    continue
            if daily_ready_score >= 0.95:
                if not (0.12 <= rebound_from_low_pct <= 0.92):
                    continue
            else:
                if not (0.16 <= rebound_from_low_pct <= 0.78):
                    continue
            if low_age > (6 if daily_ready_score >= 0.95 else 4):
                continue
            if range_pct >= 2.35:
                continue
            if regime_name == "WEAK" and vol_ratio < 2.95:
                continue
            if leader_score < 1.20 and turnover_rank > 32 and surge_rank > 32 and daily_ready_score < 1.05:
                continue
            if is_recent_spike_near_high(df, current_price) or is_box_top_range_chase(df, current_price) or is_retest_after_failed_peak(df) or is_false_start_near_high(df, current_price):
                continue
            if has_recent_spike_then_fade(df, lookback=12, min_spike_pct=0.85, min_pullback_pct=0.20, min_peak_age=2):
                continue
            if flip_count >= 6 and not box and change_5 < 0.85:
                continue
            if flip_count >= 7 and range_pct >= 1.15:
                continue
            if not body_ok and change_1 < 0.22:
                continue
            if not (hl or box or half_hold or body_ok):
                continue
            if leader_score < 1.35 and vol_ratio < 3.00 and daily_ready_score < 1.05:
                continue
            ...
            score = 4.35 + max(vol_ratio - 1.0, 0) * 0.58 + max(change_3, 0) * 0.32 + max(leader_score, 0) * 0.36
            score += max(0.0, 0.55 - abs(rebound_from_low_pct - 0.35)) * 0.45
            score += daily_ready_score * 0.75
            if score < (5.20 if daily_ready_score >= 0.95 else 5.55):
                continue
            preview.append(make_signal(
                ticker=ticker,
                strategy="PREWATCH",
                strategy_label="저점 선점형",
                current_price=current_price,
                vol_ratio=vol_ratio,
                change_pct=max(change_1, change_3, change_5),
                rsi=rsi,
                ...
                reference_high=reference_high,
                reference_low=recent_low,
                reason=f"저점 선점 / 고점까지 {high_gap_pct:.2f}% 여유 / 저가에서 {rebound_from_low_pct:.2f}% / 거래량 {vol_ratio:.2f}배 / 큰흐름 {daily_ready_score:.2f}",
                daily_ready_score=daily_ready_score,
            ))
        except Exception:
            continue
...
def build_newlow_recovery_candidates(cache, regime, exclude_tickers=None):
    ...
    for ticker, data in cache.items():
        try:
            ...
            last = recent.iloc[-1]
            prev = recent.iloc[-2]
            bullish_reclaim = float(last["close"]) >= float(last["open"]) and float(last["close"]) >= float(prev["close"]) * 1.0005
            ...
            flip_count = direction_flip_count(df, 12)
            low_age = bars_since_recent_low(df, 14)
            body_ok = recent_reclaim_body_ok(df, min_body_ratio=0.30)

            if regime_name == "WEAK" and vol_ratio < 2.70:
                continue
            if change_5 <= -3.0 or change_5 >= 1.8:
                continue
            if change_1 < 0.06 and change_3 < 0.14:
                continue
            if vol_ratio < 2.15:
                continue
            if leader_score < 1.15 and vol_ratio < 3.10:
                continue
            if not (0.08 <= low_gap_pct <= 0.68):
                continue
            if high_gap_pct <= 1.25:
                continue
            if low_age > 3:
                continue
            if range_pct >= 2.1:
                continue
            if pullback_from_peak_pct >= 0.38:
                continue
            if flip_count >= 5 and range_pct >= 0.9:
                continue
            if has_recent_spike_then_fade(df, lookback=14, min_spike_pct=0.80, min_pullback_pct=0.18, min_peak_age=2):
                continue
            if not (fake_recovery or (body_ok and bullish_reclaim and lower_low_recent and change_1 >= 0.06 and change_3 >= 0.14)):
                continue
            ...
            score = 4.55 + max(vol_ratio - 1.0, 0) * 0.54 + max(change_1, 0) * 0.32 + max(leader_score, 0) * 0.24
            score += max(0.0, 0.42 - abs(low_gap_pct - 0.24)) * 0.40
            if score < 5.30:
                continue
            recovered.append(make_signal(
                ticker=ticker,
                strategy="NEWLOW_RECOVER",
                strategy_label="신저가 후 회복 감시",
                current_price=current_price,
                vol_ratio=vol_ratio,
                change_pct=max(change_1, change_3, change_5),
                rsi=rsi,
                range_pct=range_pct,
                signal_score=max(4.35, score),
                edge_score=max(4.25, score),
                leader_score=leader_score,
                stop_loss_pct=-1.0,
                take_profit_pct=3.2,
                time_stop_sec=720,
                ...
                reference_high=reference_high,
                reference_low=recent_low,
                reason=f"신저가 후 회복 / 저가에서 {low_gap_pct:.2f}% 회복 / 되밀림 {pullback_from_peak_pct:.2f}% / 거래량 {vol_ratio:.2f}배",
            ))
        except Exception:
            continue
...
def build_watch_fallback_candidates(cache, regime=None):
    ...
    if fallback_results:
        return fallback_results
    return build_quick_watch_candidates(cache, regime)
...
def scan_watchlist(shared_cache=None):
    ...
    if not results:
        fallback_results = build_watch_fallback_candidates(cache, regime)
        if fallback_results:
    ...
    sent_tickers = {item[0]["ticker"] for item in (new_entries + upgrade_entries + renotice_entries)}
    preview_entries = []
    for item in build_prewatch_candidates(cache, regime, exclude_tickers=sent_tickers):
        ticker = item["ticker"]
        if not should_send_simple_aux_alert(recent_prewatch_alerts, ticker, now_ts, PREWATCH_ALERT_COOLDOWN_SEC):
            continue
        preview_entries.append((item, "new", "저점 근처에서 막 돌기 시작하는지 먼저 보는 선점 후보"))
        sent_tickers.add(ticker)

    newlow_entries = []
    for item in build_newlow_recovery_candidates(cache, regime, exclude_tickers=sent_tickers):
        ticker = item["ticker"]
        if not should_send_simple_aux_alert(recent_newlow_alerts, ticker, now_ts, NEWLOW_ALERT_COOLDOWN_SEC):
            continue
        newlow_entries.append((item, "new", "신저가 근처를 찍고 바로 회복하는지 보는 선점 후보"))
        sent_tickers.add(ticker)
    ...
    if preview_entries:
        send_watch_alert_bundle("🎯 저점 선점 후보", preview_entries[:PREWATCH_ALERT_MAX])
    if newlow_entries:
        send_watch_alert_bundle("🧲 신저가 후 회복 후보", newlow_entries[:NEWLOW_ALERT_MAX])
...
def register_bot_commands():
    try:
        bot.set_my_commands([
            ...
            BotCommand("recent5", "최근 후보 5개 다시보기"),
        ])
    except Exception as e:
        print(f"[명령어 등록 오류] {e}")
...
while True:
    ...
    try:
        ...
        shared_cache = None
        if need_scan_cycle or need_position_cycle:
