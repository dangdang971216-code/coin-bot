# Refreshed copy for download workaround. If this file still shows immediate checks for delayed buttons, do not deploy.
import os
import time
import json
import math
...
def cleanup_price_check_refs(now_ts: float = None):
    ...
    expired_sched = []
    for key, target_ts in PRICE_CHECK_SCHEDULED.items():
        target_ts = safe_float(target_ts, 0)
...
def price_check_callback(update, context: CallbackContext):
    ...
    if target_min <= 0:
        title = f"💰 {ticker} 지금 가격 비교"
        guide_line = "- 비교 기준: 지금 시점 확인"
    else:
        title = f"💰 {ticker} {target_min}분 확인"
        if elapsed_sec >= target_min * 60:
            guide_line = f"- 비교 기준: 알림 뒤 {target_min}분 확인용"
        else:
            guide_line = f"- 참고: {target_min}분 버튼이지만 아직 {format_elapsed_text(elapsed_sec)} 지남"
