...
def send_startup_message():
    lines = [
        ...
        "",
        "정리 포인트",
        "- 막 오르기 시작함 후보 중 약한 급등 후 식는 타입 더 보수화",
        "- 큰 흐름 준비가 약한 상승 시작형 B 후보 더 보수화",
        "- 거래량이 다시 붙는 재등장형/계단식 회복형은 덜 막음",
        "- 예비 후보 / 신저가 반등 감시 추가",
        "- 후보별 현재가 / 경과시간 버튼 추가",
        "- 후보별 빗썸 열기 버튼 추가",
...
def analyze_prepump_entry(ticker: str, data: dict):
    ...
    if high_gap_pct < 0.45:
        return None
    if rebound_from_low_pct > 4.20:
        return None
    if rebound_from_low_pct > 2.70 and high_gap_pct < 1.00:
        return None
    ...
    if recent_hot_gap_pct >= 3.40 and vol_ratio < 6.20 and change_pct < 2.60:
        return None
...
def get_watch_quality_block_reason(item: dict, alert_type: str = "new"):
    ...
    if strategy == "PREPUMP" and tier == "B":
        ...
        if leader_score < 0.95:
            return "큰 흐름 준비가 너무 약한 상승 시작형 B라 제외"
        if not recovery_like and leader_score < 1.45 and vol_ratio < 5.60:
            return "주목도/거래량이 약한 상승 시작형 B라 제외"
        if leader_score < 1.60 and change_pct < 1.55 and vol_ratio < 5.20:
            return "초반 힘이 약한 상승 시작형 B라 제외"
        if high_gap_pct < 1.00 and vol_ratio < 7.00 and leader_score < 1.75:
            return "한 번 튄 뒤 식는 자리 같아 상승 시작형 B에서 제외"
