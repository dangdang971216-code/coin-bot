...
def send_startup_message():
    allowed_strategies = ["초반 선점형", "쏘기 직전형", "추세 지속형", "급등 돌파형"]
    if ALLOW_CHASE:
        allowed_strategies.append("추격형")
    lines = [
        "✅ 봇 시작 완료",
        "",
        ...
        "[이번에 반영된 내용]",
        "- 초저가 급등 왜곡 후보 더 보수화",
        "- 가짜 초반형 / 고점 근처 후보 더 보수화",
        "- 후보 알림 기준 미세 조정 반영",
        "- 자동 쉬기 / 수동 해제 / 상태 표시 유지",
        "- /info 에서 용어와 전략 설명 확인 가능",
        "",
        "[자동매수 허용 전략]",
    ]
    lines.extend([f"- {name}" for name in allowed_strategies])
    lines.extend([
        "",
        "[알림만 보는 전략]",
        "- 상승 시작형",
