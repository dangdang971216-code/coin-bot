...
def get_watch_bucket(item: dict):
    ...
    if strategy_label in ["상승 시작형", "초반 선점형"]:
        if change_pct <= 1.40 and vol_ratio >= 2.30 and leader_score >= 1.70 and high_gap_pct >= 1.35:
            return 3, "🟢 지금 사도 될 쪽"
        if change_pct <= 2.60 and vol_ratio >= 2.0:
            return 2, "✅ 막 오르기 시작한 후보"
        return 1, "👀 한 번 더 볼 후보"

    if strategy_label == "눌림 반등형":
        if change_pct <= 1.20 and vol_ratio >= 2.40 and high_gap_pct >= 1.20:
            return 2, "✅ 다시 힘 붙는 후보"
        return 1, "👀 조금 더 지켜볼 후보"

    if strategy_label in ["추세 지속형", "급등 돌파형"]:
        if change_pct <= 1.20 and high_gap_pct >= 1.30 and vol_ratio >= 2.20:
            return 2, "✅ 힘 붙는 후보"
...
def get_watch_display_priority(item: dict, alert_type: str = "new") -> float:
    ...
    score -= max(change_pct - 1.2, 0.0) * 2.4
    if 0 <= high_gap_pct < 0.90:
        score -= 6.0
