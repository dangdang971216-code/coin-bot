...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v290: 재확인 분류 정상화 + 결측값 고득점 방지 + health 단계요약 복구. sweep_vwap_recovery 조건값 변경 없음.",
}
...
def build_candidates(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Counter, List[Dict[str, Any]]]:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["phase_note"] = "v290: 결측값 고득점/재확인 과다 분류 수정, 단일전략 paper 검증."
...
def scan_once() -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["scan_display_note"] = "v290 sweep/vwap 스캔 진행중"
