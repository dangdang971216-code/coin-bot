...
_v213_base_select_precision_targets = globals().get("select_precision_targets", lambda *a, **k: None)
...
_v213_base_precision_priority = globals().get("precision_priority", lambda *a, **k: None)
...
def _v277_light_material_row(row: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mat = _v274_score_groups(item, market) or {}
    strategy_scores = _v275_strategy_eval(item, mat, market) or {}
    strategies = _v275_primary_strategies(strategy_scores)
    raw_missing_info = _v275_missing_info(item)
    missing_info: List[str] = list(raw_missing_info) if isinstance(raw_missing_info, list) else []
    strategy_missing: List[str] = []
    for v in (strategy_scores or {}).values():
        if not isinstance(v, dict):
            continue
        for n in (v.get("needs") or []):
            strategy_missing.append(str(n))
    mat_needs: List[str] = list(mat.get("needs") or []) if isinstance(mat.get("needs") or [], list) else []
    material_needs_combined = list(dict.fromkeys(mat_needs + strategy_missing + missing_info))[:16]
    one_liner_needs = list(dict.fromkeys(strategy_missing + missing_info))[:3]
    item.update({
        "material_only": True,
        ...
        "material_needs": material_needs_combined,
        ...
        "one_liner": f"{strategies[0] if strategies else '재료관찰'} / 총점 {mat.get('total',0)} / " + ",".join(one_liner_needs),
    })
    return item
...
STRATEGY_KEY = "material_factory_v279"
...
def _v278_refresh_latest_row(row: Dict[str, Any], lane: str, scan_id: str) -> Dict[str, Any]:
    ...
    rr.update({
        ...
        "strategy_key": "material_factory_v279",
        "route": "material_factory_v279",
        "factory_mode": "v279_material_only_light_factory",
        "lane": lane,
        "created_at": nowv,
...
def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:  # type: ignore[override]
    """v278 material-only factory. old export_candidates/consume_row/risk queue path is not used."""
    ts = now_ts()
    ...
    return {
        ...
        "dup_skip_reason": {},
        "data_quality_note": "v279_material_only_light_factory",
        "dup_skip": 0,
...
def _v277_write_score_cache(payload: Dict[str, Any]) -> None:  # type: ignore[override]
    try:
        ...
        text = "\n".join([
            ...
            "",
            "[2] 전략실험 재료 현황",
            *_v278_strategy_block_lines(lab, 7),
            "",
            "[3] 재료공장",
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        *_v278_strategy_block_lines(lab, 7),
        ...
        "",
        "[4] 장세재료",
        f"- 상승비율 1m {fnum(market.get('up1_ratio'),0):.1f}% / 3m {fnum(market.get('up3_ratio'),0):.1f}% / 5m {fnum(market.get('up5_ratio'),0):.1f}%",
        f"- 평균흐름 3m {fnum(market.get('avg_change_3'),0):+.2f}% / 주요코인 {market.get('major_pressure','-')}",
        "",
        "[5] 재료 예시",
    ]
...
def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        f"- 재료 {len(rows)} / 고점수 {len(high)} / 신규 모의매매 OPEN 중지",
        "",
        "[1] 전략별 재료 요약",
        *_v278_strategy_block_lines(lab, 7),
        "",
...
def scan_once() -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["scan_display_note"] = "v279 재료공장 스캔 진행중"
...
def _v279_write_target_file(path: Path, tickers: List[str], reason: str) -> None:
    ...
    payload = {
        ...
        "target_meta": {t: {"reason": reason, "source": "material_factory_v279"} for t in tickers},
    }
    atomic_write(path, payload)
...
def update_ws_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "material_factory") -> None:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["ws_target_write_note"] = "v279 material target writer"
...
def update_micro_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "material_factory") -> None:  # type: ignore[override]
    ...
    with _state_lock:
        ...
        STATE["micro_target_write_note"] = "v279 material target writer"
...
def _v280_strategy_performance_lines(limit: int = 8) -> List[str]:
    """Cached builder use only. Commands should not call this directly."""
    try:
        rows = load_closed(limit=6000)
    except Exception as exc:
        log_error('v280_strategy_perf_load_closed', exc)
        rows = []
    groups: Dict[str, List[Dict[str, Any]]] = {}
    for r in rows or []:
        if not isinstance(r, dict):
        ...
        # keep broad: old records often omit lane, but avoid obvious shadow-only records where possible.
        if lane.lower() in {'shadow', 'review'}:
            continue
        name = _v280_strategy_of_closed(r)
        if name in {'전략표시 없음', '재료관찰'}:
            continue
        groups.setdefault(name, []).append(r)
    if not groups:
        return ["- 전략별 CLOSED 장부가 아직 충분하지 않음"]
    order = sorted(groups.items(), key=lambda kv: len(kv[1]), reverse=True)[:limit]
    out: List[str] = []
    icons = ["①", "②", "③", "④", "⑤", "⑥", "⑦", "⑧"]
    for i, (name, arr) in enumerate(order):
        st = _v280_stat(arr)
        icon = "✅" if st['n'] >= 50 and st['total'] > 0 and st['max_excl'] > 0 else ("⚠️" if st['n'] < 50 else "❌")
        out += [
            f"📊 전략 {icons[i] if i < len(icons) else i+1}. {name}",
...
def _v277_write_score_cache(payload: Dict[str, Any]) -> None:  # type: ignore[override]
    """v280 cache builder. It may do heavier CLOSED reading in the background, but /score itself remains cache-only."""
    try:
        rows = payload.get('rows') if isinstance(payload.get('rows'), list) else []
        lab = payload.get('strategy_lab') if isinstance(payload.get('strategy_lab'), dict) else _v277_strategy_lab(rows)
        grades = Counter(str(r.get('material_grade') or '-') for r in rows or [])
        ...
        text = "\n".join([
            ...
            "",
            "[1] 캐시 상태",
            f"- 현재 snapshot 재료 {len(rows)}개 / 신규 모의매매 OPEN 0",
            "- /score는 저장된 캐시만 보여줍니다. 직접 장부 계산은 하지 않습니다.",
            "",
            "[2] 전략별 승률·수익률",
            *_v280_strategy_performance_lines(8),
            "",
            "[3] 전략별 재료 수집 현황 · 성과 아님",
            "- 아래 숫자는 후보 재료가 얼마나 잡혔는지입니다. 승률/수익률이 아닙니다.",
            *_v280_strategy_block_lines(lab, 7),
            ...
            "",
            "[6] 다음 판단",
            "- 재료 수집 현황과 전략 성과를 분리해서 봅니다.",
            "- 5/10/20분 추적이 붙으면 재료관찰 성과를 별도 표시합니다.",
        ])
        save_json(FILES.get('score_summary', BASE_DIR / 'clean_score_summary.json'), _cache_payload(text, 'score'))
    except Exception as exc:
        log_error('v280_score_cache_write', exc)
...
def health_text() -> str:  # type: ignore[override]
    """v280 readable health. Keep CPU/memory/free space visible."""
    try:
        _write_resource_status()
    except Exception:
        pass
    ...
    ready = [r for r in rows if bool(r.get('paper_bot_open') or r.get('trade_ready') or r.get('open_eligible'))]
    ext = _external_health_snapshot()
    p = paper_alive_summary()
    pst = p.get('status') if isinstance(p.get('status'), dict) else {}
    lines = [
        "🧭 건강상태 /health",
        "- 메인봇방용 간단 상태판입니다. 재시작/업그레이드는 가드봇 담당.",
        "",
        "[1/6] 메인봇",
        f"{scan['icon']} {BOT_VERSION} / scan {scan['label']} / 단계 {scan.get('stage','-')} / 재료 {len(rows)}개 / snapshot {snap_age:.0f}초 전" if snap_age >= 0 else f"{scan['icon']} {BOT_VERSION} / scan {scan['label']} / 단계 {scan.get('stage','-')} / 재료 {len(rows)}개 / snapshot 대기",
        f"- 오류: {'❌ ' + str(s.get('scan_last_error')) if s.get('scan_last_error') else '✅ 새 실행 중 오류 없음'}",
        "- 단계요약:",
    ]
    st_lines = stage_lines()
    lines += (st_lines if st_lines else ["  · 아직 없음"])
    lines += [
        "",
        "[2/6] 서버자원",
        *resource_health_lines(),
        "",
        "[3/6] 외부정보",
        f"{'✅' if ext.get('ws_worker_ok') else '⚠️'} WS: 후보신선 {ext.get('ws_fresh',0)}/{ext.get('total',0)} / 오래됨 {ext.get('ws_stale',0)} / 없음 {ext.get('ws_missing',0)} / 전체신선 {s.get('ws_fresh', ext.get('ws_fresh',0))}",
        f"{'✅' if ext.get('micro_worker_ok') else '⚠️'} 호가·체결: 후보신선 {ext.get('micro_fresh',0)}/{ext.get('total',0)} / 오래됨 {ext.get('micro_stale',0)} / 없음 {ext.get('micro_missing',0)} / 전체신선 {s.get('micro_fresh', ext.get('micro_fresh',0))}",
        "",
        "[4/6] 재료공장",
        f"✅ 재료후보 {len(rows)} / 신규 모의매매 OPEN {len(ready)} / 상태 {'✅ 닫힘' if not ready else '❌ 열림'}",
        "- 등급: " + (" / ".join(f"{k} {v}" for k, v in grades.most_common(4)) if grades else "없음"),
        "- 부족TOP: " + (" / ".join(f"{k} {v}" for k, v in missing.most_common(4)) if missing else "부족 정보 없음"),
        "",
        "[5/6] paper_bot",
        f"{p.get('icon','❔')} {pst.get('version','?')} / running {p.get('ctrl_running')} / OPEN {len(read_open())} / CLOSED {pst.get('closed_total', line_count(FILES['paper_closed']))} / 상태 {_age_text(fnum(pst.get('updated_at'),0))} 전",
        "",
        "[6/6] 판독",
    ]
    warns = []
    if scan.get('running'):
        warns.append("⚠️ scan 진행중")
    if len(ready) > 0:
        warns.append("❌ 신규 모의매매 OPEN 열림 확인 필요")
    if missing:
        warns.append("⚠️ 부족정보 보강 필요")
    if not warns:
        warns.append("✅ 재료수집 정상 / 신규 모의매매 닫힘")
    lines += warns
    return "\n".join(lines)
...
def candidate_quality_text(full: bool = False) -> str:  # type: ignore[override]
    mat = _v280_material_snapshot_summary()
    rows = mat['rows'][:(260 if full else 100)]
    snap = mat['snap']
    market = snap.get('market_context') if isinstance(snap.get('market_context'), dict) else {}
    lab = mat['lab']
    ready = [r for r in rows if bool(r.get('paper_bot_open') or r.get('trade_ready') or r.get('open_eligible'))]
    ...
    if not rows:
        lines.append("- 아직 재료 snapshot 없음")
    lines += ["", "판독: /quality는 현재 재료 snapshot입니다. 승률/수익률은 /score에서 봅니다."]
    return "\n".join(lines)


def strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    mat = _v280_material_snapshot_summary()
    rows = mat['rows'][:(280 if full else 100)]
    lab = mat['lab']
    high = [r for r in rows if fnum(r.get('material_score_total'), 0) >= 16]
    lines = [
        "🧪 전략실험 재료 /strategy_watch",
...
# ===============================
V281_VERSION_TAG = "v2.13.281_scan_typeerror_health_korean_score_perf"
...
def _v277_write_score_cache(payload: Dict[str, Any]) -> None:  # type: ignore[override]
    """v281: cache writer. /score command remains cache-only; this writer runs after material snapshot save."""
    try:
        rows = payload.get('rows') if isinstance(payload.get('rows'), list) else []
        lab = payload.get('strategy_lab') if isinstance(payload.get('strategy_lab'), dict) else _v277_strategy_lab(rows)
        grades = Counter(str(r.get('material_grade') or '-') for r in rows or [])
        ...
        text = "\n".join([
            ...
            f"기준: {BOT_VERSION} / cache-only / 신규 모의매매 OPEN 닫힘",
            "",
            *_v281_score_header_note(),
            "",
            "[2] 전략별 승률·수익률 · 기존 CLOSED 장부 기준",
            *_v280_strategy_performance_lines(8),
            "",
            "[3] 전략별 재료 수집 현황 · 성과 아님",
            ...
            *_v280_strategy_block_lines(lab, 7),
            ...
            "",
            "[6] 다음 판단",
            "- 먼저 scan 오류가 사라지고 v281 캐시가 새로 저장되는지 확인합니다.",
            "- 그다음 5/10/20분 추적과 반복손실 기억을 붙여 관찰 성과를 봅니다.",
        ])
        save_json(FILES.get('score_summary', BASE_DIR / 'clean_score_summary.json'), _cache_payload(text, 'score'))
    except Exception as exc:
        log_error('v281_score_cache_write', exc)

def health_text() -> str:  # type: ignore[override]
    """v281 readable health. CPU/memory/disk free must remain visible."""
    try:
        _write_resource_status()
    except Exception:
        pass
    ...
    p = paper_alive_summary()
    pst = p.get('status') if isinstance(p.get('status'), dict) else {}
    stage_ko = _v281_stage_label(scan.get('stage'))
    snap_txt = f"스냅샷 {snap_age:.0f}초 전" if snap_age >= 0 else "스냅샷 대기"
    lines = [
        "🧭 건강상태 /health",
        "- 메인봇방용 상태판입니다. CPU·메모리·디스크 남은 용량은 항상 표시합니다.",
        "",
        "[1/6] 메인봇",
        f"{scan['icon']} {BOT_VERSION} / 스캔 {scan['label']} / 현재단계 {stage_ko} / 재료 {len(rows)}개 / {snap_txt}",
        f"- 오류: {'❌ ' + str(s.get('scan_last_error')) if s.get('scan_last_error') else '✅ 새 실행 중 오류 없음'}",
        "- 단계요약:",
    ]
    st_lines = _v281_stage_lines()
    lines += (st_lines if st_lines else ["  · 아직 없음"])
    lines += [
        "",
        "[2/6] 서버자원",
        *resource_health_lines(),
        "",
        "[3/6] 외부정보",
        f"{'✅' if ext.get('ws_worker_ok') else '⚠️'} 웹소켓: 후보신선 {ext.get('ws_fresh',0)}/{ext.get('total',0)} / 오래됨 {ext.get('ws_stale',0)} / 없음 {ext.get('ws_missing',0)} / 전체신선 {s.get('ws_fresh', ext.get('ws_fresh',0))}",
        f"{'✅' if ext.get('micro_worker_ok') else '⚠️'} 호가·체결: 후보신선 {ext.get('micro_fresh',0)}/{ext.get('total',0)} / 오래됨 {ext.get('micro_stale',0)} / 없음 {ext.get('micro_missing',0)} / 전체신선 {s.get('micro_fresh', ext.get('micro_fresh',0))}",
        "",
        "[4/6] 재료공장",
        f"✅ 재료후보 {len(rows)} / 신규 모의매매 OPEN {len(ready)} / 상태 {'✅ 닫힘' if not ready else '❌ 열림'}",
    ...
    warns = []
    if scan.get('running'):
        warns.append("⚠️ 스캔 진행중")
    if s.get('scan_last_error'):
        warns.append("❌ 스캔 오류 확인 필요")
    if len(ready) > 0:
        warns.append("❌ 신규 모의매매 OPEN 열림 확인 필요")
    if missing:
        warns.append("⚠️ 부족정보 보강 필요")
...
try:
    STRATEGY_NAME = "외부전략 재료수집"
    STRATEGY_KEY = "material_factory_v281"
    STATE["phase_note"] = "v281: scan NoneType 오류 수술, health 한국어 단계명, /score 전략별 승률·수익률 표시 보강."
    STATE["v281_scan_typeerror_fixed"] = True
except Exception:
    pass
