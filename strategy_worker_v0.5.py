...
def persist_paper_handoff(events: List[Dict[str, Any]], evaluated: int = 0) -> Tuple[List[Dict[str, Any]], int]:
    ...
    save_json(HANDOFF, {
        ...
        "note": "v0.5: 정보부족을 최종탈락과 분리하고 S근접 후보를 WS/micro target으로 우선 보강. handoff TTL 유지.",
    })
...
def run_once()->Dict[str,Any]:
    ...
    events=[]; rejects=[]; target=[]; strat_counts={k:0 for k in STRATEGIES}; info_pending=0; near_s_info_pending=0; hard_blocked=0
    for row in rows:
        t=ticker_norm(row.get("ticker"));
        if not t: continue
        of=ofmap.get(t,{}) or {}
        ...
        if info_bad:
            info_pending += 1
        if core_hits and info_bad and not hard_bad:
            ...
            # 일반 탈락. 정보부족은 최종 실패가 아니라 target 보강 후보로 분리하되, 표시에는 남긴다.
            reasons += ([f"정보보강대기:{x}" for x in info_bad[:2]] if info_bad else []) + hard_bad[:2] + core_fail[:4]
            if info_bad and len(target)<90:
                target.append(t)
        if s_hits:
            ...
            rejects.append({"ticker":t,"score":fnum(row.get("scanner_priority_score")),"reasons":reasons[:8],"price":row.get("current_price"),"updated_ts":ts,"info_pending":bool(info_bad),"near_s_info_pending":bool(core_hits and info_bad and not hard_bad)})
    ...
    target=list(dict.fromkeys(target))[:120]
    save_json(SUMMARY,{"version":VERSION,"updated_ts":ts,"updated_text":now_text(ts),"s_count":len(events),"paper_latest_count":len(latest),"archive_appended":appended,"strategies":strat_counts,"events":latest[:12],"info_pending_count":info_pending,"near_s_info_pending":near_s_info_pending,"hard_blocked_near_s":hard_blocked,"target_count":len(target)})
    save_json(REJECT,{"version":VERSION,"updated_ts":ts,"updated_text":now_text(ts),"evaluated":len(rows),"s_count":len(events),"reject_count":len(rejects),"info_pending_count":info_pending,"near_s_info_pending":near_s_info_pending,"hard_blocked_near_s":hard_blocked,"target_count":len(target),"reason_top":[{"reason":k,"count":v} for k,v in top],"sample":rejects[:30]})
    payload={"version":VERSION,"updated_ts":ts,"updated_text":now_text(ts),"targets":target,"target_count":len(target),"reason":"S 후보·S근접 정보보강 우선 target. 정보부족은 최종탈락이 아니라 보강대기로 분리."}
    save_json(WS_TARGETS,payload); save_json(MICRO_TARGETS,payload); save_json(MICRO_URGENT,payload)
    sec=now_ts()-st; write_status("running", evaluated=len(rows), s_count=len(events), paper_latest_count=len(latest), info_pending_count=info_pending, near_s_info_pending=near_s_info_pending, target_count=len(target), last_sec=round(sec,3)); return {"eval":len(rows),"s":len(events),"paper_latest":len(latest)}
def main()->None:
