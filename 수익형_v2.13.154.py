...
try:
    import pybithumb  # v154: REST bulk가 막힐 때 전체현재가 fallback용. API KEY 불필요.
except Exception:
    pybithumb = None
...
def http_json(url: str, timeout: float = 3.0) -> Any:
    ...
    v154부터 User-Agent는 ASCII 고정값만 쓴다.
    """
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.154: candidate_events 없음. paper/shadow만 paper_bot 소비파일.",
    }
...
def command_candidate_reason(update, context) -> None:
    lines = [
        ...
        "",
        f"전략: {STRATEGY_NAME}",
        "- v2.13.154 기준: 새 본선 / paper·shadow 소비파일만 출력",
...
def command_paper_today(update, context) -> None:
    ...
    lines = [
        ...
        "",
        "판독",
        "- 오늘 성과가 0이면 아직 v2.13.154 신규 청산이 쌓이지 않은 것",
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.154.py", "- 페이퍼봇 대상: paper_bot_v0.17.py"])),
    ]
    start = now_ts()
    reply(update, "\n".join(["📦 묶음 명령 접수", "- 출처: /batch", f"- 실행 {len(funcs)}개", "- v2.13.154: 기능흐름 일괄이식 새 본선", "- 메인봇=스캔·전략·paper/shadow 후보파일 / paper_bot=실행·장부·알림"]))
    rows = []
    ...
    lines = ["🧾 v2.13.154 batch 요약", f"- 출력무결성: 실행 {len(funcs)}개", "", "⏱ 명령어별 시간표"]
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "스캔 시작부 User-Agent 인코딩 오류 수정 / 구버전 tail 없는 새 본선", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
