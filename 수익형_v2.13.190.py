...
STATE: Dict[str, Any] = {
    ...
    "last_rows_sample": [],
    "compat_commands": [],
    "phase_note": "v183: 웹소켓 정상수신 이후 후보품질 분석 보강 + 배포표시 찌꺼기 정리. 조건/청산/BY_READY 변경 없음.",
}
...
def update_ws_targets(rows: List[Dict[str, Any]], priority_rows: Optional[List[Dict[str, Any]]] = None, reason: str = "hub_rank") -> None:
    ...
    후보를 자르는 제한이 아니라, 전체 453개 중 지금 판단에 필요한 코인을
    웹소켓 대상 80개 안에 우선 올리는 배관이다. 오래된 WS를 차단조건으로
    쓰는 대신, 필요한 코인이 빨리 최신 수신되게 하는 것이 목적이다.
    """
    try:
        ticks: List[str] = []
        seen: set = set()
        priority_rows = priority_rows or []
        # 장세 기준 코인은 항상 최우선.
        for t in list(MAJOR_WATCH_TICKERS) + ["BTC", "ETH", "XRP"]:
            _push_unique_ticker(ticks, seen, t)
        # 현재 paper 보유 코인은 청산/성과 판단에 필요하므로 우선 수신.
        for t in _paper_open_tickers(limit=25):
            _push_unique_ticker(ticks, seen, t)
        # 현재 strict/paper_OPEN/재확인/관찰 후보를 rank보다 우선한다.
        sorted_pri = sorted(priority_rows, key=lambda r: (
        ...
        payload = {
            ...
            "paper_open_count": len(_paper_open_tickers(limit=25)),
            "note": "main bot writes current candidates/open positions so sidecar follows needed tickers",
        }
        try:
            atomic_write(FILES.get("ws_targets", BASE_DIR / "clean_ws_targets.json"), payload)
        except Exception as exc:
        ...
        with _state_lock:
            ...
            STATE["ws_target_file_written"] = now_ts()
...
def refresh_external_ws_cache() -> None:
    ...
    if isinstance(status, dict) and status_ts > 0:
        if status_age <= WS_SIDECAR_STATUS_STALE_SEC:
            ...
            state = "외부오래됨"
        reason = str(status.get("last_error") or status.get("state") or reason)[:160]
    elif rows:
...
def ws_snapshot(ticker: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "ws_fresh": fresh,
        "ws_row_status": "fresh" if fresh else "stale",
        "ws_targeted": t in target_set,
...
def apply_ws_cache_to_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rows or []:
        ...
        if status == "fresh" and fnum(ws.get("live_price"), 0) > 0:
            fresh += 1
            # 웹소켓은 실시간 확인용으로 보존한다. REST 가격을 덮지 않고
            # 신뢰도/가격재확인 재료로만 남겨 안정스캔을 보호한다.
...
def websocket_status_text() -> str:
    ...
    lines = [
        ...
        f"- worker: external_sidecar_cache / pid {STATE.get('ws_sidecar_pid','-')} / status_age {STATE.get('ws_sidecar_age_sec','-')}초",
        ...
        f"- 대상갱신: {STATE.get('ws_target_file_targets',0)}개 / reason {STATE.get('ws_target_reason','-')} / missing {STATE.get('ws_missing_rows',0)} / stale {STATE.get('ws_stale_rows',0)}",
        "",
        "판독",
...
def consume_row(item: Dict[str, Any], lane: str, ts: Optional[float] = None, scan_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "pipeline_note": "v2.13.190: v189 WS target atomic_write NameError 즉시수정 + 버전성과/쓰레기분리팩 유지.",
    }
...
def candidate_line() -> str:
    ...
    state_part = (
        ...
        f"→ archive신규 {STATE.get('paper_written',0)} / paper_OPEN {STATE.get('trade_ready_written',0)} / 재확인 {STATE.get('latest_final_recheck_wait',0)} / 관찰 {STATE.get('strict_observe_written',0)} / "
        f"major {STATE.get('major_watch_written',0)} / 복기 {STATE.get('shadow_written',0)}"
    )
    latest_part = f"latest 정식 {latest.get('paper_latest',0)}(현재scan {STATE.get('paper_latest_written', latest.get('paper_latest',0))}) / paper_OPEN대상 {STATE.get('latest_trade_ready',0)} / 재확인 {STATE.get('latest_final_recheck_wait',0)} / 복기 {latest.get('shadow_latest',0)}"
...
def core_text() -> str:
    ...
    return "\n".join([
        ...
        f"✅ 직원: 2차 눌림·3차 재돌파·4차 위험·5차 분류·6차 최종검증 / major {s.get('major_watch_count',0)}",
        f"🔎 정보커버리지: 정밀 {s.get('field_coverage',{}).get('precision',0)} / ema {s.get('field_coverage',{}).get('ema12',0)} / bb {s.get('field_coverage',{}).get('bb',0)} / mfi {s.get('field_coverage',{}).get('mfi',0)} / ATR {s.get('field_coverage',{}).get('atr',0)} / 위험캐시 {s.get('execution_risk_cached',0)} / 우선확인 {s.get('execution_risk_sync_checked',0)} / WS대상 {s.get('ws_target_file_targets',0)} / 최종검증 {s.get('final_entry_note','-')}",
        websocket_status_line(),
        candidate_line(),
        paper_bot_line(),
        guard_summary_line(),
...
def command_deploy(update, context) -> None:
    text = "\n".join([
        ...
        "- 메인봇 대상: 수익형_v2.13.190.py",
        "- 페이퍼봇 대상: paper_bot_v0.36.py",
...
def score_text() -> str:
    ...
    lines = [
        ...
        fmt_stats("현재버전 strict", [r for r in rows_since_current_version(rows) if str(r.get("lane")) == "strict"]),
        "",
        "[2/5] 손실/익절 원인",
    ]
    if by_reason:
...
def command_version_score(update, context) -> None:
    ...
    lines = ["📊 현재버전 성과 /version_score", f"- 기준: {BOT_VERSION} / {version_baseline_text()} / 기존 기록 삭제 없음", fmt_stats("현재버전 strict", vrows), "", "[사유]"]
    if by_reason:
        for k, _ in by_reason.most_common(5):
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    def latest_ws_line(arr: List[Dict[str, Any]]) -> str:
        ...
        for r in arr:
            ...
            if bool(ctx_get(r, ["ws_targeted"]) or r.get("ws_targeted")):
                targeted += 1
        ...
        for r in arr:
            ...
            for x in reasons:
                ...
                if sx:
                    c[sx] += 1
        if not c:
            return ["- trade_ready 막힌 이유: 현재 관찰 후보 없음 또는 이유 미저장"]
        return ["- trade_ready 막힌 이유: " + " / ".join(f"{k} {v}" for k, v in c.most_common(limit))]
    ...
    lines = [
        ...
        f"- 현재 후보: strict {len(latest_strict)}개 / paper_OPEN {len(trade_ready)}개 / 재확인대기 {sum(1 for r in latest_strict if r.get('final_entry_action') == 'recheck_wait')}개 / 관찰 {len(observe)}개 / shadow {len(latest_shadow)}개",
        ...
        *trade_ready_block_lines(observe, limit=6 if full else 4),
        f"- 관찰태그: {tag_summary(latest_strict)}",
        f"- 최종검증 라벨: {top_ctx(latest_strict, ['final_entry_label'], limit=4)}",
        f"- 실전위험 우선확인: {STATE.get('execution_risk_sync_note','-')}",
        "좋은 후보 예시",
        *top_candidate_lines(latest_strict, limit=6 if full else 3),
        "",
        "[3/6] 손실 원인 TOP",
    ]
    if by_reason:
    ...
    lines += [
        ...
        *reject_lines(limit=8 if full else 4),
        ...
        ("✅ 표본이 어느 정도 쌓임: 조건 조정 후보를 검토 가능" if enough else "❔ entry_context 표본 더 필요: 조건 대수정은 보류"),
    ]
    if full:
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.190.py", "- 페이퍼봇 대상: paper_bot_v0.36.py"])),
    ]
    start = now_ts()
...
def _builder_for_command(name: str):
    if not TEXT_COMMAND_BUILDERS:
        TEXT_COMMAND_BUILDERS.update({
            ...
            "paper_today": lambda: "\n".join(["📌 오늘 paper·모의매매 /paper_today", "", new_period_text(), candidate_fresh_text()]),
            "score": score_text,
            "version_score": lambda: "📊 현재버전 성과 /version_score\n" + fmt_stats("현재버전 strict", [r for r in rows_since_current_version(load_closed()) if str(r.get("lane")) == "strict"]) + f"\n- 기준: {BOT_VERSION} / {version_baseline_text()} / 기존 기록 삭제 없음",
            ...
            "help": lambda: "📚 명령어 /help\n- 여러 명령을 줄바꿈으로 보내도 자동 묶음 처리됨\n- /batch /check /core /bot_status /ws_status /quality /score /version_score /cpu_status /paper_handoff /deploy /errorlog_full",
        })
...
def command_help(update, context) -> None:
    reply(update, "\n".join([
        ...
        "",
        "기본 확인",
        "- /batch /core /ws_status /score /paper_today /cpu_status",
        "흐름/후보",
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v190 WS대상 쓰기 오류 수정 + 6차 최종검증 유지 / 자동매수·청산 변경 없음", "확인: /batch /core /cpu_status /paper_handoff /deploy"]))
