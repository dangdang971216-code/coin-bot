...
# =========================================================
# v2.5.67 fast 경로 rescue/재출발 승격 재조정
# =========================================================
...
FAST_SCAN_RESCUE_MIN_TURNOVER_5M = 24000000
FAST_SCAN_RESCUE_LIMIT = 1
FAST_SCAN_RESCUE_SCORE_MIN = 14.2
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'strong':
        ...
        if (
            ...
            and change_5 >= 0.48 and change_3 >= 0.04
            and vol_ratio >= 1.65 and leader_score >= 0.85
            and 0.08 <= from_low <= 2.10 and below_high >= 0.95
            and lite_score >= 10.8 and rsi < 82 and turnover_a_ok and not chart_bad
        ):
            return 'A', 'user_alert'
    ...
    if section == 'watch':
        ...
        rescue_like = (
            ...
            and change_5 >= 0.62 and vol_ratio >= 2.35 and leader_score >= 0.88
            and 0.35 <= from_low <= 2.10 and below_high >= 1.35
            and lite_score >= max(FAST_SCAN_RESCUE_SCORE_MIN, 14.8) and rsi < 79
            and turnover_rescue_ok and not chart_bad and not weak_restart
            and safe_float(item.get('change_30', 0), 0) >= -0.80
        )
        if rescue_like:
            return 'A', 'user_alert'
        if restart_like:
...
def _rescue_internal_watch_items(items):
    ...
    for item in internal_sorted:
        ...
        rescue_ok = (
            ...
            and leader >= 0.85
            and below_high >= 1.35
            and 0.20 <= from_low <= 2.10
            and turnover_5m >= FAST_SCAN_RESCUE_MIN_TURNOVER_5M
            and change_30 >= -0.90
        )
        if primary_reason == '실시간 급등 초입':
            rescue_ok = rescue_ok and change_5 >= 0.95 and vol_ratio >= 3.00 and leader >= 1.00 and below_high >= 1.60 and turnover_5m >= 28000000 and change_30 >= -0.60
            if not good_market:
                rescue_ok = rescue_ok and turnover_5m >= 36000000 and change_5 >= 1.20 and below_high >= 2.20 and change_30 >= -0.20
        elif primary_reason == '재출발 확인 후보':
            rescue_ok = rescue_ok and change_5 >= 0.72 and vol_ratio >= 2.60 and leader >= 0.95 and from_low >= 0.55 and below_high >= 2.10 and turnover_5m >= 22000000 and change_30 >= -0.45
        else:
            rescue_ok = rescue_ok and change_5 >= 0.62 and vol_ratio >= 2.35 and below_high >= 1.45
        if not rescue_ok:
            continue
        new_item = dict(item)
        ...
        new_item['one_liner'] = one_liner + ' / 상위 감시 후보에서 다시 올림'
