...
def _v284_strategy_observation_lines(limit: int = 7) -> List[str]:
    ...
    for i, name in enumerate(V284_STRATEGY_ORDER[:limit]):
        ...
        c10 = int(m10.get("count") or 0)
        avg10_txt = "대기" if avg10 is None else f"{fnum(avg10):+.2f}%"
        peak10_txt = "대기" if peak10 is None else f"{fnum(peak10):+.2f}%"
        if dec >= 20 and wr_num >= 45 and avg10 is not None and fnum(avg10) > 0:
            status = "✅"; verdict = "모의매매 후보"
        elif dec >= 20 and wr_num < 25:
            status = "❌"; verdict = "현재는 버림 후보"
        elif dec >= 10:
...
def _v277_write_score_cache(payload: Dict[str, Any]) -> None:  # type: ignore[override]
    """v285: /score cache writer. 7전략 관찰성과와 자동매매 가능 여부를 먼저 표시한다."""
    try:
        ...
        best_name = '-' ; best_wr = -1.0 ; best_dec = 0
        auto_ok = False
        for name in V284_STRATEGY_ORDER:
            v = strategies.get(name) if isinstance(strategies.get(name), dict) else {}
            wr = fnum(v.get('watch_win_rate'), -1)
            dec = int(v.get('decision_count') or 0)
            m10 = v.get('milestones', {}).get('10분', {}) if isinstance(v.get('milestones'), dict) else {}
            avg10 = m10.get('avg_pct')
            if wr > best_wr:
                best_name, best_wr, best_dec = name, wr, dec
            if dec >= 20 and wr >= 45 and avg10 is not None and fnum(avg10) > 0:
                auto_ok = True
        if auto_ok:
            verdict_line = "✅ 관찰상 모의매매 후보가 생겼습니다. 그래도 자동매매는 아직 금지입니다."
        elif best_dec >= 10:
            verdict_line = f"❌ 현재 관찰 성과 기준 자동매매 후보 없음 / 최고: {best_name} {best_wr:.1f}% ({best_dec}판정)"
        else:
            verdict_line = "❔ 관찰 표본 부족 / 자동매매 판단 보류"
        text = "\n".join([
            "📊 성과 요약 /score",
            ...
            *_v280_strategy_block_lines(lab, 7),
            ...
            "",
            "[7] 다음 판단",
            "- 현재 관찰승률이 낮으면 전략 추가가 아니라 어떤 조건이 쓰레기 후보를 살리는지 줄입니다.",
            "- 10분/20분 평균과 최대제외가 플러스인 전략만 나중에 모의매매 후보로 승격합니다.",
        ])
        save_json(FILES.get('score_summary', BASE_DIR / 'clean_score_summary.json'), _cache_payload(text, 'score'))
    except Exception as exc:
        log_error('v285_score_cache_write', exc)
...
def health_text() -> str:  # type: ignore[override]
    """v285 readable health. CPU/memory/disk free remain visible."""
    res = _v285_read_or_refresh_resource_status()
    with _state_lock:
        s = dict(STATE)
    scan = scan_status_summary(s)
    mat = _v280_material_snapshot_summary()
    ...
    ws = ext.get('ws') if isinstance(ext.get('ws'), dict) else {}
    micro = ext.get('micro') if isinstance(ext.get('micro'), dict) else {}
    lines = [
        '🧭 건강상태 /health',
        ...
        '',
        '[4/6] 외부정보',
        f"✅ 웹소켓: 후보신선 {ws.get('fresh_on_candidates', 0)}/{ws.get('candidate_total', 0)} / 오래됨 {ws.get('stale_on_candidates', 0)} / 없음 {ws.get('missing_on_candidates', 0)} / 전체신선 {ws.get('fresh_total', 0)}",
        f"✅ 호가·체결: 후보신선 {micro.get('fresh_on_candidates', 0)}/{micro.get('candidate_total', 0)} / 오래됨 {micro.get('stale_on_candidates', 0)} / 없음 {micro.get('missing_on_candidates', 0)} / 전체신선 {micro.get('fresh_total', 0)}",
        '',
        '[5/6] 재료공장',
