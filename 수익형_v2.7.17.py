...
- 실거래 주문은 v2.7.17 이후 재연결 전까지 실행하지 않는다.
"""
...
# 기본 설정
# =========================================================
BOT_VERSION = "수익형 v2.7.17"
BOT_FILE_TAG = "수익형_v2.7.17.py"
TIMEZONE = "Asia/Seoul"
...
# WebSocket / 데이터 허브 1차
# =========================================================
# 공식 Public WebSocket은 ticker/trade/orderbook을 제공한다. v2.7.17는 현재가+체결 허브를 후보분류에 더 적극 반영한다.
...
def paper_strategy_legend() -> list[str]:
    return [
        ...
        "S1 눌림준비: 예전 승률형 준비대기 1차, 아직 진입 전 확인",
        "S2 눌림: 눌림/저점방어/재회복 후보",
...
def compute_row(ticker: str, price: float, money: float, ts: float, hist: deque, data_quality: dict | None = None) -> MarketRow:
    ...
    # v2.7.17 점수: 후보 발견용 점수다. 실제 초록/노랑은 classify에서 다시 가른다.
...
def v315_mainline_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool) -> dict:
    """
    v2.7.17: v2.5.3xx 후기판의 본선 판단을 clean core row로 옮긴 1차 컨텍스트.
...
def v317_pullback_risk_context(row: MarketRow, *, data_3m_ok: bool, data_5m_ok: bool, one_min_milrim: bool, late_chase: bool, recent_money_ok: bool) -> dict:
    """
    v2.7.17: v2.5.3xx 후기판의 위험진입 차단 2차 이식.
...
def classify_candidate(row: MarketRow) -> Candidate:
    ...
    # v2.7.17: v2.5.3xx 본선 기준 이식 컨텍스트.
    ...
    # v2.7.17: 예전 승률 좋던 눌림/준비대기 1차 복구.
    ...
    pullback_prep_base = (
        ...
        and row.from_low30 <= 3.05
        and row.high_gap30 >= 0.25
        and row.range30_pct <= 6.2
        and -0.35 <= row.ch1 <= 0.75
        and -0.18 <= row.ch3 <= 1.55
        and row.ch5 >= -0.20
        and (
            row.low_defense_ok
            or (row.pullback_shape_ok and row.pullback_rebound_pct >= 0.20)
            or v315_prepare_wait
        )
    )
    pullback_prep_wait = (
        pullback_prep_base
        and (recent_money_ok or live_trade_alive or bool(v315_ctx.get("trade_ok")))
        and not bool(v315_ctx.get("weak_now"))
        and not v317_dirty_pullback
    )
    pullback_prep_reconfirm_wait = (
        pullback_prep_base
        and (one_min_recovered or own_recovery_strong or bool(v315_ctx.get("current_alive")))
        and (mainline_money_strong or bool(v315_ctx.get("trade_strong")))
        and row.ch3 >= 0.03
        and not bool(v315_ctx.get("weak_now"))
        and not v317_dirty_pullback
    )
    ...
    # v2.7.17에서는 일반 약한 움직임 전체가 재출발감시로 빨려 들어가지 않게,
    ...
    # v2.7.17: 특수 대기상태 보존. TON류 재출발/재돌파/급등후재확인 후보가
    ...
    if pullback_prep_reconfirm_wait:
        ...
        return make_candidate(
            row, "PENDING", "눌림 재확인대기", "🟡",
            "저점방어 후 1분 재회복과 돈 유지가 붙는 준비대기 라인",
            pattern="PULLBACK_CONFIRM_WAIT", caution=" / ".join(caution),
            entry_note="1분봉 유지·3분 유지·최근 체결 유지 확인",
            reject_reason="눌림 재확인 전",
            block_reasons=["눌림 준비대기", "재확인 필요"]
        )
    ...
    if pullback_prep_wait:
        ...
        return make_candidate(
            row, "PENDING", "눌림 준비대기", "🟡",
            "저점은 버티고 있으나 아직 진입 전 확인이 필요한 준비대기",
            pattern="PULLBACK_PREP_WAIT", caution=" / ".join(caution),
...
def render_buy_check() -> str:
    ...
    lines.append("- v2.7.17: WebSocket 현재가·체결·자체1분봉 허브를 먼저 지어 데이터 신뢰도를 표시")
...
def render_auto_status() -> str:
    ...
    return (
        ...
        f"중요\n"
        f"- v2.7.17는 clean core 눌림/상승초입 검증판이라 실제 주문은 막아둠.\n"
...
def render_real_stats() -> str:
    return (
        f"📊 실거래 성과 /real_stats v2.7.17\n"
        f"- v2.7.17 clean core는 실거래 주문 보호모드야.\n"
        f"- 기존 v2.6 실거래 로그를 여기서 섞어 계산하지 않음.\n"
        f"- 실거래 직원은 v2.7.17에서 체결확정/보유보호와 함께 재연결 예정.\n"
    )
...
def render_legacy_check() -> str:
    return (
        f"🧹 레거시 격리 확인 /legacy_check v2.7.17\n"
...
def render_paper_stats() -> str:
    ...
    if overdue30:
        lines.append("- ⚠️ 30분 지났는데 미완료 후보가 있음. v2.7.17에서 paper worker/저장 조건을 봐야 함")
    return "\n".join(lines)
...
def auto_on_handler(update, context: CallbackContext):
    ...
    send_text(update.effective_chat.id,
              "🟢 자동매매 스위치 ON\n- 단, v2.7.17 clean core는 실거래 주문 보호모드라 실제 주문은 아직 차단돼.\n- 후보 흐름 검증 후 후보판 검증 후 실거래 직원 재연결 예정.")
