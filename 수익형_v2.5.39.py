...
MISSED_REVIEW_MIN_END_UP_PCT = 0.90
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 실시간 강세형 사용자 알림은 더 엄격하게 조여 굳이 볼 가치 낮은 B급 후보를 더 잘 숨기게 조정",
        "- 늦음/감시로 숨겨진 후보 중 정말 강한 애만 재출발/실시간 후보로 살짝 승격할 수 있게 보강",
        "- /missed 는 반짝 튄 코인을 덜 잡고, 진짜 놓친 코인 위주로 더 깔끔하게 정리",
        "- /missed 의 최근고점 거리 999%는 정보 없음으로 바꾸고, 상위 순위권 범위도 넓혀 순위 정보 부족을 줄임",
        "- 같은 코인 재알림은 더 드물게 오게 조정하고, 30분 확인 추적 샘플도 조금 더 촘촘히 수집",
        "- 명령어 탭 문구도 더 짧고 쉽게 정리",
        "",
        "자세한 설명은 /info",
    ]
    send("\n".join(lines))
...
def get_user_alert_block_reason(item: dict, display_key: str = "") -> str:
    ...
    strategy = str(item.get("strategy", "") or "").strip()
    if section == "live":
        if high_gap_pct < USER_ALERT_MIN_LIVE_HIGH_GAP_PCT:
            return "실시간 강세로 보기엔 바로 위 고점이 너무 가까움"
        if from_low > USER_ALERT_MAX_LIVE_FROM_LOW_PCT and high_gap_pct < 1.70:
            return "실시간 강세 후보인데 저점에서 이미 꽤 올라와서 제외"
        if chart_rise > USER_ALERT_MAX_LIVE_CHART_RISE_PCT and high_gap_pct < 2.00:
            return "실시간 강세 후보인데 차트상 이미 위쪽 자리라 제외"
        if chg_5 < USER_ALERT_LIVE_MIN_5M_PCT and chg_30 < USER_ALERT_LIVE_MIN_30M_PCT:
            return "실시간 강세라기엔 방금 붙는 힘이 약함"
        if chg_30 <= USER_ALERT_LIVE_MIN_30M_PCT and from_low >= 1.30 and high_gap_pct < 1.45:
        ...
        if trade_tier == "B":
            ...
            if leader < USER_ALERT_MIN_LIVE_LEADER:
                fail_count += 1
            if fail_count >= USER_ALERT_LIVE_B_FAIL_COUNT_MIN:
                return "실시간 강세 B지만 아직 해볼만한 수준까진 아님"
    if section != "restart":
        return ""
    if high_gap_pct < USER_ALERT_MIN_RESTART_HIGH_GAP_PCT:
        return "재출발로 보기엔 바로 위 고점이 너무 가까움"
    if from_low > USER_ALERT_MAX_RESTART_FROM_LOW_PCT and high_gap_pct < 1.25:
        return "재출발 후보인데 저점에서 이미 꽤 올라와서 제외"
    if chart_rise > USER_ALERT_MAX_RESTART_CHART_RISE_PCT and high_gap_pct < 1.80:
    ...
    if trade_tier == "B" and edge < USER_ALERT_MIN_RESTART_EDGE and vol_ratio < USER_ALERT_MIN_RESTART_VOL and leader < USER_ALERT_MIN_RESTART_LEADER:
