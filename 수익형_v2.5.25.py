...
def is_fast_start_entry_candidate(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if high_gap_pct < max(FAST_ENTRY_MIN_HIGH_GAP_PCT, 0.70) or high_gap_pct > 2.80:
        return False
    if from_low > 3.20:
        return False
    if chg_5 < 0.05:
...
def should_promote_recovery_candidate_to_entry(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if high_gap_pct < max(RECOVERY_ENTRY_MIN_HIGH_GAP_PCT, 0.70):
        return False
    if chg_5 < 0.05:
        return False
...
def is_low_zone_recovery_candidate(item: dict, judgement: str = "", caution: str = "") -> bool:
    ...
    if strategy in ["PREWATCH", "NEWLOW_RECOVER"]:
        return (
            high_gap_pct >= 0.65
            and (
                (chg_5 >= 0.08 and vol_ratio >= 1.35)
                or (chg_30 >= 0.25 and from_low >= 0.30 and vol_ratio >= 1.25)
                or (leader_score >= 1.20 and vol_ratio >= 1.55 and from_low >= 0.25)
            )
        )
    ...
    if strategy == "LEADER_WATCH":
        return (
            from_low >= 0.10
            and high_gap_pct >= 0.55
            and (
                (chg_30 >= 0.10 and vol_ratio >= 1.20)
                or (chg_5 >= 0.00 and vol_ratio >= 1.15 and from_low >= 0.20)
                or (leader_score >= 0.95 and vol_ratio >= 1.10 and from_low >= 0.15)
            )
        )
    if strategy in ["PREPUMP", "PULLBACK"]:
        return high_gap_pct >= 0.65 and (chg_5 >= 0.05 or chg_30 >= 0.20 or from_low >= 0.35)
    if tags.intersection({"저점높임", "가짜하락회복", "하락복구", "눌림재확인"}) and from_low <= 2.40 and high_gap_pct >= 1.00 and (chg_5 >= 0.00 or chg_30 >= 0.10):
        return True
    if leader_score >= 1.8 and vol_ratio >= 2.1 and from_low <= 2.10 and high_gap_pct >= 1.15 and (chg_5 >= 0.00 or chg_30 >= 0.15):
        return True
    return False
