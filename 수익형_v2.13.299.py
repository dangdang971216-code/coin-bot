...
# =============================================================================
# v2.13.299: retire sweep_vwap strategy + information-pipeline TTL/dedup surgery
# - 전략 조건 보정이 아니라, 실패한 단일전략 신규 OPEN을 중지한다.
# - 기존 paper OPEN/CLOSED 장부는 보존한다.
# - WS/micro/정밀값 정보 배관은 새 전략에도 재사용할 수 있게 TTL/중복조회 기준으로 정리한다.
# =============================================================================
BOT_VERSION = "수익형 v2.13.299"
STRATEGY_RETIRED = True
STRATEGY_RETIRED_REASON = "20/30 전 중간판정: 승률 20%대·순손실 확대. sweep_vwap_recovery 신규 OPEN 중지"
V299_NOTE = "v299: sweep_vwap_recovery 폐기/신규 OPEN 중지 + 정보배관 TTL·중복조회 정리. paper 장부 보존."
V299_RECHECK_TTL_SEC = float(os.getenv("CLEAN_V299_RECHECK_TTL_SEC", "90"))
V299_PRECISION_STALE_SEC = float(os.getenv("CLEAN_V299_PRECISION_STALE_SEC", str(PRECISION_TTL_SEC * 3)))
V299_TARGET_REFRESH_SEC = float(os.getenv("CLEAN_V299_TARGET_REFRESH_SEC", "18"))
V299_SIDE_TARGET_SAFETY = int(os.getenv("CLEAN_V299_SIDE_TARGET_SAFETY", str(PRECISION_REFRESH_MAX_SAFETY)))
FILES.setdefault("retired_strategy_status", BASE_DIR / "clean_retired_strategy_status.json")
FILES.setdefault("retired_strategy_blocked", BASE_DIR / "retired_strategy_blocked_candidates.jsonl")
...
def _v299_reasons(row: Dict[str, Any]) -> List[str]:
    vals: List[str] = []
    for key in ("final_entry_reasons", "block_reasons", "quality_risk_tags", "sweep_vwap_wait", "sweep_vwap_blocks"):
        vals.extend([str(x) for x in _v288_as_list((row or {}).get(key)) if str(x)])
    return vals


def _v299_needs_external(row: Dict[str, Any]) -> bool:
    if not isinstance(row, dict):
        return False
    reasons = ",".join(_v299_reasons(row))
    return any(k in reasons for k in ("micro", "WS", "웹소켓", "호가", "체결", "긴급수집", "재확인")) or str(row.get("final_entry_action")) == "recheck_wait"


def _v299_precision_cache_fresh(ticker: str, nowv: Optional[float] = None) -> bool:
    nowv = nowv or now_ts()
    try:
        cached = _precision_cache.get(str(ticker or "").upper()) or {}
        return bool(cached.get("precision_ok")) and (nowv - fnum(cached.get("precision_ts"), 0.0) <= V299_PRECISION_STALE_SEC)
    except Exception:
        return False


def _v299_priority_tickers_from_files(ttl_sec: float = 300.0) -> List[str]:
    out: List[str] = []
    seen: set[str] = set()
    for p in [
        FILES.get("sweep_precision_retry", BASE_DIR / "clean_sweep_precision_retry.json"),
        ...
        try:
            vals = _v295_read_target_payload_tickers(Path(p), ttl_sec=ttl_sec) if "_v295_read_target_payload_tickers" in globals() else []
            for t in vals:
...
def select_precision_targets(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:  # type: ignore[override]
    ...
    고정 개수 목표가 아니라 TTL/freshness 기준이다.
    - priority/retry/urgent/open 코인은 먼저 올린다.
    - 이미 정밀값이 fresh인 코인은 같은 scan에서 중복 직접조회 대상으로 올리지 않는다.
    - 나머지는 거래대금·변동·외부 fresh 등 정보 필요성이 있을 때만 큐에 넣는다.
    - V299_SIDE_TARGET_SAFETY는 서버 보호용 안전상한이며 전략 후보 제한이 아니다.
    """
    rows = [r for r in (rows or []) if isinstance(r, dict) and _v298_ticker(r) and _v298_ticker(r) not in STABLE_EXCLUDED]
    by_ticker: Dict[str, Dict[str, Any]] = {}
    for r in rows:
        ...
        if old is None or _v298_precision_candidate_score(r) > _v298_precision_candidate_score(old):
            by_ticker[t] = r
    nowv = now_ts()
    priority_tickers = _v299_priority_tickers_from_files(ttl_sec=300.0)
    selected: List[Dict[str, Any]] = []
    seen: set[str] = set()
    for t in priority_tickers:
        r = by_ticker.get(t)
        if r is None:
            continue
        rr = dict(r)
        rr["precision_target_source"] = "retry_urgent_open_priority"
        if t not in seen:
            seen.add(t); selected.append(rr)

    # open position은 다음 전략/청산 감시에도 필요하므로 우선순위만 부여. 없으면 무시.
    try:
        open_map = read_open() if callable(globals().get("read_open")) else {}
        for p in (open_map or {}).values():
            t = _v298_ticker(p)
            r = by_ticker.get(t)
            if r and t not in seen:
                rr = dict(r); rr["precision_target_source"] = "paper_open_watch"
                seen.add(t); selected.append(rr)
    except Exception:
        pass
    ...
    for t, r in by_ticker.items():
        if t in seen:
            continue
        rank = fint(r.get("turnover_rank"), 9999)
        ch = abs(fnum(r.get("change_24h") or r.get("change_rate"), 0.0))
        external = bool(r.get("ws_fresh")) or bool(r.get("micro_fresh"))
        fresh_precision = _v299_precision_cache_fresh(t, nowv)
        # fresh 정밀값이 있으면 직접보강 대상은 아니지만, 중요 이벤트면 계속 후보군에는 남길 수 있다.
        if fresh_precision and not external and rank > PRECISION_TURNOVER_CORE_RANK and ch < PRECISION_MOVE_TRIGGER_PCT:
            continue
        score = _v298_precision_candidate_score(r, set(priority_tickers))
        if score >= 0.85 or rank <= PRECISION_TURNOVER_CORE_RANK or ch >= PRECISION_MOVE_TRIGGER_PCT or external:
            scored.append((score, r))
    scored.sort(key=lambda x: x[0], reverse=True)
    for _score, r in scored:
        t = _v298_ticker(r)
        if t not in seen:
            rr = dict(r); rr["precision_target_source"] = "v299_ttl_priority_queue"
            seen.add(t); selected.append(rr)
        if len(selected) >= max(1, V299_SIDE_TARGET_SAFETY):
            break
    with _state_lock:
        STATE["precision_target_source"] = "v299_ttl_priority_queue"
        STATE["precision_target_note"] = f"v299 TTL/freshness queue selected {len(selected)} / priority {len([t for t in priority_tickers if t in by_ticker])} / source {len(rows)}"
        STATE["v299_precision_selected"] = len(selected)
        STATE["v299_precision_priority"] = len([t for t in priority_tickers if t in by_ticker])
        STATE["v299_precision_safety_guard"] = V299_SIDE_TARGET_SAFETY
    return selected


_v299_base_refresh_precision = refresh_precision

def refresh_precision(rows: List[Dict[str, Any]], targets: Optional[List[Dict[str, Any]]] = None) -> Tuple[int, int, int]:  # type: ignore[override]
    targets = targets if targets is not None else select_precision_targets(rows)
    # 이미 fresh인 target은 queue 등록/직접조회에서 빼서 CPU와 API를 보호한다.
    nowv = now_ts()
    need_targets: List[Dict[str, Any]] = []
    fresh_skip = 0
    for r in targets or []:
        t = _v298_ticker(r)
        if t and _v299_precision_cache_fresh(t, nowv) and str((r or {}).get("precision_target_source")) not in {"retry_urgent_open_priority", "paper_open_watch"}:
            fresh_skip += 1
            continue
        need_targets.append(r)
    ok, fail, have = _v299_base_refresh_precision(rows, need_targets)
    with _state_lock:
        STATE["v299_precision_fresh_skip"] = fresh_skip
        STATE["v299_precision_need_targets"] = len(need_targets)
    return ok, fail, have + fresh_skip


def _v299_target_rows(strict_rows: List[Dict[str, Any]], shadow_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    rows = list(strict_rows or []) + list(shadow_rows or [])
    out: List[Dict[str, Any]] = []
    seen: set[str] = set()
    nowv = now_ts()
    for r in rows:
        if not isinstance(r, dict):
            continue
        if not _v299_needs_external(r):
            continue
        t = _v298_ticker(r)
        if not t or t in seen or t in STABLE_EXCLUDED:
            continue
        # 같은 후보를 지나치게 자주 sidecar target으로 밀어넣지 않기 위한 scan-local dedup.
        rr = dict(r)
        rr["v299_target_reason"] = ",".join(_v299_reasons(r)[:5])
        rr["v299_target_ts"] = nowv
        seen.add(t); out.append(rr)
        if len(out) >= max(1, V299_SIDE_TARGET_SAFETY):
            break
    return out


def _v299_retire_row(row: Dict[str, Any], lane: str = "shadow") -> Dict[str, Any]:
    r = dict(row or {})
    r["lane"] = lane
    r["strategy_retired"] = True
    r["retired_strategy_key"] = STRATEGY_KEY
    r["retired_reason"] = STRATEGY_RETIRED_REASON
    r["trade_ready"] = False
    r["paper_bot_open"] = False
    r["open_eligible"] = False
    if str(r.get("final_entry_action") or "") == "paper_open":
        r["final_entry_action"] = "retired_observe"
    r.setdefault("final_entry_label", "폐기전략 관찰")
    reasons = _v288_as_list(r.get("final_entry_reasons"))
    if STRATEGY_RETIRED_REASON not in reasons:
        reasons.append("전략폐기_신규OPEN중지")
    r["final_entry_reasons"] = reasons[:12]
    return r
...
def _v288_write_strategy_files(strict_rows: List[Dict[str, Any]], shadow_rows: List[Dict[str, Any]], market: Dict[str, Any], rejects: Counter) -> None:  # type: ignore[override]
    ...
    payload = {
        "version": BOT_VERSION,
        "schema": "sweep_vwap_recovery_retired_v299_latest",
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        "scan_id": str(STATE.get("scan_id") or ""),
        "strategy_key": STRATEGY_KEY,
        "strategy_label": STRATEGY_LABEL,
        "strategy_retired": True,
        "retired_reason": STRATEGY_RETIRED_REASON,
        "strict_count": 0,
        "shadow_count": len(shadow_rows or []),
        "trade_ready_count": 0,
        "recheck_count": 0,
    ...
    save_json(FILES["retired_strategy_status"], {
        "version": BOT_VERSION,
        "updated_ts": nowv,
        "updated_text": now_text(nowv),
        "strategy_key": STRATEGY_KEY,
        ...
        "strategy_retired": True,
        "retired_reason": STRATEGY_RETIRED_REASON,
        "paper_new_open": "stopped",
        "paper_ledger": "preserved",
    })


def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:  # type: ignore[override]
    """v299: 폐기 전략은 신규 paper OPEN을 만들지 않는다.

    정보 배관 진단/관찰 row는 남기되, paper_candidates_latest는 비운다.
    기존 OPEN은 paper_bot이 정상 청산하고, CLOSED 장부는 보존된다.
    """
    ts = now_ts()
    scan_id = str(STATE.get("scan_id") or f"scan-{int(ts)}")
    all_rows = [_v299_retire_row(_v288_refresh_latest_row(r, "shadow", scan_id), "shadow") for r in list(strict or []) + list(shadow or [])]
    urgent_priority = _v299_target_rows([], all_rows)
    _v288_write_micro_urgent(urgent_priority, reason="v299_retired_strategy_info_watch")
    write_error = ""
    ok, err = write_jsonl_replace(FILES["paper_latest"], [])
    if not ok:
        write_error = f"paper_latest:{err}"
    ok2, err2 = write_jsonl_replace(FILES["shadow_latest"], all_rows)
    if not ok2:
        write_error = (write_error + " / " if write_error else "") + f"shadow_latest:{err2}"
    try:
        market = _v274_market_context(all_rows)
        rejects = Counter()
        for r in all_rows:
            for x in _v299_reasons(r):
                rejects[str(x)] += 1
        _v288_write_strategy_files([], all_rows, market, rejects)
    except Exception as exc:
        log_error("v299_retired_strategy_summary_write", exc)
    # 폐기전략이 paper open으로 넘어가는 것을 감시하기 위한 차단 로그
    try:
        for r in list(strict or []):
            if bool((r or {}).get("trade_ready") or (r or {}).get("paper_bot_open") or (r or {}).get("open_eligible")):
                append_jsonl(FILES["retired_strategy_blocked"], _v299_retire_row(r, "blocked"))
    except Exception:
        pass
    with _state_lock:
        STATE["latest_trade_ready"] = 0
        STATE["trade_ready_written"] = 0
        STATE["paper_latest_written"] = 0
        STATE["shadow_latest_written"] = len(all_rows)
        STATE["latest_final_recheck_wait"] = 0
        STATE["v299_urgent_watch_rows"] = len(urgent_priority)
        STATE["factory_mode"] = "v299_retired_strategy_info_factory"
        STATE["write_error"] = write_error
        STATE["strategy_retired"] = True
    return {
        "paper_attempt": len(strict or []),
        "shadow_attempt": len(shadow or []),
        "paper_written": 0,
        "shadow_written": len(all_rows),
        "trade_ready_written": 0,
        "strict_observe_written": 0,
        "paper_latest_written": 0,
        "shadow_latest_written": len(all_rows),
        "latest_trade_ready": 0,
        "latest_strict_observe": 0,
        "latest_final_recheck_wait": 0,
        "latest_final_observe": len(all_rows),
        "urgent_watch_rows": len(urgent_priority),
        "dup_skip_reason": {},
        "data_quality_note": "v299_retired_strategy_info_factory",
        "dup_skip": 0,
        "write_error": write_error,
        "last_ticker": str((all_rows or [{"ticker":"-"}])[-1].get("ticker") or "-"),
        "factory_mode": "v299_retired_strategy_info_factory",
        "archive_deferred": 1,
        "risk_sync_deferred": True,
        "strategy_retired": True,
    }
...
def v212_publish_final_candidate_state(strict_rows: List[Dict[str, Any]], shadow_rows: List[Dict[str, Any]], all_rows: List[Dict[str, Any]], pipe: Dict[str, Any]) -> str:  # type: ignore[override]
    """v299: final snapshot/target publish 경량화.

    export_candidates에서 이미 latest/snapshot/target을 정리했으므로 여기서는 상태 문자열만 만든다.
    """
    snap = int(pipe.get("urgent_watch_rows", 0) or 0)
    try:
        ws_obj = load_json(FILES.get("ws_targets", BASE_DIR / "clean_ws_targets.json"), {})
        micro_obj = load_json(FILES.get("micro_targets", BASE_DIR / "clean_micro_targets.json"), {})
        ws_n = len(ws_obj.get("targets") or []) if isinstance(ws_obj, dict) else 0
        micro_n = len(micro_obj.get("targets") or []) if isinstance(micro_obj, dict) else 0
    except Exception:
        ws_n = micro_n = 0
    return f"retired snapshot {snap} / WS target {ws_n} / micro target {micro_n} / v299 light"
...
def _v288_score_text(rows: Optional[List[Dict[str, Any]]] = None) -> str:  # type: ignore[override]
    ...
    lines = [
        "📊 단일 전략 성과 /score",
        f"전략: {STRATEGY_LABEL} ({STRATEGY_KEY})",
        "상태: ❌ 폐기 / 신규 paper OPEN 중지 / 기존 OPEN은 정상 청산",
        f"- 폐기 이유: {STRATEGY_RETIRED_REASON}",
        "",
        "[1] 최종 설계값",
        f"- 익절 +{V288_TAKE_PROFIT_PCT:.2f}% / 보호 +{V288_PROTECT_TRIGGER_PCT:.2f}% 이후 +{V288_PROTECT_FLOOR_PCT:.2f}% / 손절 {V288_STOP_LOSS_PCT:.2f}% / 시간 {V288_TIME_EXIT_MIN:.0f}분",
        "- 이 전략은 보정 대신 폐기. 다음 전략은 설계서 확정 후 별도 투입.",
        "",
        "[2] 현재 후보",
        f"- 신규 OPEN 후보 0개 / 폐기전략 관찰 row {len(rows)}개",
        "- paper_candidates_latest는 비움. shadow/진단만 유지.",
        "",
        "[3] CLOSED 성과",
        f"- {st['n']}전 {st['wins']}승 {st['losses']}패 / 승률 {st['wr']:.1f}% / 순합산 {st['total']:+.2f}% / 평균 {st['avg']:+.2f}% / 최대 {st['max']:+.2f}% / 최소 {st['min']:+.2f}%",
        "",
        "[4] 다음 원칙",
        "- 조건 찔끔 보정 금지",
        "- 정보배관은 다음 전략에도 쓰므로 TTL·중복조회·target 재구독 경로만 유지 점검",
    ]
    return "\n".join(lines)
...
def _v288_quality_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        f"전략: {STRATEGY_LABEL}",
        "- 상태: ❌ 폐기 / 신규 OPEN 중지",
        f"- 관찰 row {len(rows)} / paper OPEN 후보 0",
        "",
        "[폐기전략 관찰 사유 TOP]",
        *(f"- {k}: {v}" for k, v in c.most_common(8)),
        *( ["- 없음"] if not c else [] ),
...
def _v288_strategy_watch_text(full: bool = False) -> str:  # type: ignore[override]
    ...
    return "\n".join([
        ...
        f"전략: {STRATEGY_LABEL}",
        "- 상태: ❌ 폐기 / 신규 OPEN 중지",
        "- 7전략 관찰판도 폐기, sweep_vwap_recovery도 신규 진입 중지",
        "- 현재는 정보배관/정밀값/WS·micro target 상태만 다음 전략 대비로 점검",
        "",
        "[관찰 row]",
        *( _v288_row_summary(r) for r in rows[:10] ),
        *( ["- 없음"] if not rows else [] ),
    ])


_v299_base_health_text = _v288_health_text

def _v288_health_text() -> str:  # type: ignore[override]
    text = _v299_base_health_text()
    text = text.replace(f"✅ {BOT_VERSION} / 전략 {STRATEGY_LABEL}", f"✅ {BOT_VERSION} / 전략 {STRATEGY_LABEL} / ❌ 폐기")
    text = text.replace("- 허브: 전체시장 수집 → 직원: 정밀/VWAP/micro 보강 → 공장: sweep_vwap_recovery 단일 후보 저장", "- 허브: 전체시장 수집 → 직원: 정밀/VWAP/micro 보강 → 공장: 폐기전략 신규 OPEN 차단 + 정보배관 진단")
    text = text.replace("- 자동매수 OFF / paper 검증 전용 / CLOSED 30건 전 조건 변경 금지", "- 자동매수 OFF / 폐기전략 신규 OPEN 중지 / 기존 paper 장부 보존")
    return text
...
def _v288_write_command_caches(reason: str = "manual") -> None:  # type: ignore[override]
    try:
        ...
        log_error("v299_write_command_caches", exc)

try:
    STATE["version"] = BOT_VERSION
    STATE["strategy"] = STRATEGY_NAME
    STATE["phase_note"] = V299_NOTE
    STATE["strategy_retired"] = True
    STATE["retired_reason"] = STRATEGY_RETIRED_REASON
except Exception as exc:
    log_error("v299_init", exc)
