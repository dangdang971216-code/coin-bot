...
PREWATCH_MIN_VOL_RATIO = 2.30
PREWATCH_MIN_LEADER_SCORE = 1.15
PREWATCH_MIN_HIGH_GAP_PCT = 0.95
PREWATCH_MAX_HIGH_GAP_PCT = 2.20
PREWATCH_MIN_FROM_LOW_PCT = 0.55
PREWATCH_MIN_3M_PCT = 0.14
PREWATCH_MIN_5M_PCT = 0.00
PREWATCH_MIN_SCORE = 5.70
...
FAST_SCAN_PRELAUNCH_MIN_CHANGE5 = -1.15
FAST_SCAN_PRELAUNCH_MAX_CHANGE5 = 0.55
FAST_SCAN_PRELAUNCH_MIN_CHANGE3 = -0.45
FAST_SCAN_PRELAUNCH_MIN_VOL = 1.10
FAST_SCAN_PRELAUNCH_MIN_LEADER = 0.35
FAST_SCAN_PRELAUNCH_MIN_HIGH_GAP = 0.85
FAST_SCAN_PRELAUNCH_MIN_FROM_LOW = 0.05
FAST_SCAN_PRELAUNCH_MAX_FROM_LOW = 1.90
FAST_SCAN_PRELAUNCH_MIN_TURNOVER_5M = 12000000
...
def classify_fast_scan_grade_bucket(section: str, primary_reason: str, change_5: float, change_3: float, vol_ratio: float, leader_score: float, from_low: float, below_high: float, rsi: float, lite_score: float, item: dict | None = None) -> tuple[str, str]:
    ...
    if section == 'watch':
        ...
        prelaunch_like = (
            ...
            and max(FAST_SCAN_PRELAUNCH_MIN_CHANGE5, -0.75) <= change_5 <= 0.45
            and change_3 >= max(FAST_SCAN_PRELAUNCH_MIN_CHANGE3, -0.25)
            and vol_ratio >= max(FAST_SCAN_PRELAUNCH_MIN_VOL, 1.00)
            and leader_score >= max(FAST_SCAN_PRELAUNCH_MIN_LEADER, 0.32)
            and FAST_SCAN_PRELAUNCH_MIN_FROM_LOW <= from_low <= min(FAST_SCAN_PRELAUNCH_MAX_FROM_LOW, 1.75)
            and below_high >= max(FAST_SCAN_PRELAUNCH_MIN_HIGH_GAP, 0.95)
            and lite_score >= 11.6 and rsi < 79
            and safe_float(item.get('turnover_5m', 0), 0) >= max(10000000, FAST_SCAN_PRELAUNCH_MIN_TURNOVER_5M * 0.85)
            and not chart_bad
            and safe_float(item.get('change_30', 0), 0) >= -3.40
            and (
                int(safe_float(item.get('change5_rank', 999), 999)) <= 12
                or int(safe_float(item.get('turnover_rank', 999), 999)) <= 32
                or int(safe_float(item.get('leader_rank', 999), 999)) <= 24
            )
        )
        if rescue_like:
            return 'A', 'user_alert'
