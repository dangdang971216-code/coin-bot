...
def format_chart_quant_line(item: dict, short: bool = False) -> str:
    ...
    if short:
        return f"- 차트수치: {quant_label} / 몸통1봉 {body1:.0f}% / 종가강도 {close1:.0f}% / 윗꼬리 {upper:.2f}% / 눌림 {pullback:.2f}%→회복 {recovery:.2f}%({recovery_speed:.2f}/봉) / 재돌파 {re_score:.2f} / 유지 {hold_bars}봉 / 4시간 {htf_ready:.2f} / 일봉 {daily_ready:.2f}"
    return f"- 차트수치: {quant_label} / 몸통1봉 {body1:.0f}%·3봉 {body3:.0f}% / 종가강도 1봉 {close1:.0f}%·3봉 {close3:.0f}% / 윗꼬리 1봉 {upper:.2f}%·3봉 {upper3:.2f}% / 아랫꼬리 1봉 {lower:.2f}%·3봉 {lower3:.2f}% / 연속양봉 {bull_streak} / 연속음봉 {bear_streak} / 강한양봉 {strong_bulls}개 / 눌림 {pullback:.2f}%→회복 {recovery:.2f}%({recovery_speed:.2f}/봉, {pullback_bars}봉) / 압축 {comp_score:.2f} / 재돌파 {re_score:.2f}({re_success:.0f}%) / 유지 {hold_score:.2f}({hold_bars}봉) / 4시간준비 {htf_ready:.2f} / 일봉준비 {daily_ready:.2f}"
...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    lines = [
        ...
        f"- 위치: 저점대비 {safe_float(row.get('from_low', 0), 0):+.2f}% / 최근30분 고점까지 {safe_float(row.get('below_high', 999), 999):.2f}% 남음",
        ...
        if shortlist:
            ...
            for idx, item in enumerate(shortlist, start=1):
                lines.append(
                    ...
                    f"최근고점까지 {safe_float(item.get('below_high',999),999):.2f}%"
                )
