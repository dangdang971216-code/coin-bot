...
WATCH_STRONG_REALERT_MIN_SEC = 1800
WATCH_WEAK_REALERT_MIN_SEC = 3600
WATCH_BLOCK_IF_PENDING_30MIN_CHECK = True
WATCH_REPEAT_WINDOW_SEC = 3 * 3600
WATCH_REPEAT_MAX_PER_TICKER = 2
WATCH_REPEAT_REQUIRE_STRONGER_SEC = 2 * 3600
...
WATCH_MIN_EFFECTIVE_30M_MOVE_PCT = 0.22
WATCH_MIN_RECOVERY_FROM_LOW_PCT = 0.25
WATCH_MIN_RANGE_PCT = 0.18
WATCH_HIGH_VOL_MIN = 5.50
WATCH_HIGH_VOL_MIN_MOVE_PCT = 0.12
RECOVERY_ENTRY_MIN_30M_PCT = 0.15
RECOVERY_ENTRY_MIN_FROM_LOW_PCT = 0.30
RECOVERY_ENTRY_MIN_VOL_RATIO = 1.70
RECOVERY_ENTRY_MIN_LEADER_SCORE = 0.80
RECOVERY_ENTRY_MIN_HIGH_GAP_PCT = 0.85
RECOVERY_ENTRY_MIN_5M_PCT = 0.00
...
def should_promote_recovery_candidate_to_entry(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> bool:
    ...
    if strategy == "LEADER_WATCH" and leader_score >= 2.20 and vol_ratio >= 2.20 and max(from_low, chg_30, change_pct) >= 0.35:
        threshold -= 1
    if strategy in ["PREWATCH", "NEWLOW_RECOVER"] and vol_ratio >= 2.10 and high_gap_pct >= 1.00:
        threshold -= 1
    if chg_5 >= 0.18 and vol_ratio >= 1.80 and high_gap_pct >= 1.00 and max(from_low, chg_30, change_pct) >= 0.45:
        threshold -= 1

    if regime_name == "SIDEWAYS":
        threshold += REGIME_RECOVERY_ENTRY_SIDEWAYS_EXTRA_STRENGTH
        if chg_5 >= 0.18 and vol_ratio >= 1.80 and high_gap_pct >= 1.10:
            threshold -= 1
    if regime_name == "WEAK":
        threshold += 1
...
def is_low_zone_recovery_candidate(item: dict, judgement: str = "", caution: str = "") -> bool:
    ...
    if strategy == "LEADER_WATCH":
        return (
            ...
            and (
                (chg_30 >= 0.20 and vol_ratio >= 2.00)
                or (chg_5 >= 0.18 and vol_ratio >= 1.75)
            )
        )
...
def get_watch_section_key(item: dict, judgement: str = "", caution: str = "", regime: dict = None) -> str:
    ...
    if is_watch_late_candidate(item, judgement, caution):
        ...
        section = "watch" if strategy == "LEADER_WATCH" else "entry"
    ...
    if section == "entry":
        ...
        elif conclusion not in {"지금 볼만함", "지금만 잠깐 볼 만함", "지금 진입 가능"}:
            section = "recovery" if is_low_zone_recovery_candidate(item, judgement, caution) else "watch"
    ...
    if section == "entry" and trade_tier == "B":
        ...
        if high_gap_pct <= 0.80:
            section = "late"
        elif chg_5 < 0.18 or leader_score < 1.95:
            section = "recovery" if recovery_like else "watch"
        elif vol_ratio < 2.60:
            section = "recovery" if recovery_like else "watch"
        elif from_low < 0.80 and chg_30 < 0.80:
            section = "recovery"
        elif strategy in ["LEADER_WATCH", "PREPUMP", "PULLBACK"] and (chg_5 < 0.35 or high_gap_pct < 0.95):
            section = "recovery"
        elif strategy == "LEADER_WATCH" and (vol_ratio < 3.00 or leader_score < 2.10 or from_low < 0.85):
            section = "watch"

    if section == "entry" and strategy == "LEADER_WATCH":
        if chg_5 < 0.45 or from_low < 0.90 or high_gap_pct < 1.00:
            section = "recovery" if is_low_zone_recovery_candidate(item, judgement, caution) else "watch"
...
def format_alert_flow_line(item: dict, short: bool = False) -> str:
    ...
    if short:
        return f"- 흐름: 30분 {chg_30:+.2f}% / 저점대비 {from_low:+.2f}%"
...
def format_watch_alert_line(item: dict, order: int = 1, alert_type: str = "new", reason_text: str = "", section_override: str = "") -> str:
    ...
    if section_key in {"recovery", "late", "watch"}:
        ...
        lines = [
            ...
            format_alert_flow_line(item, short=True),
            f"- 요약: {summary}",
        ]
...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 섹션 제목이랑 코인별 결론이 서로 안 싸우게 다시 정렬",
        "- entry 기준을 더 조여서 구경만/지금 사기 별로는 위 칸에서 내림",
        "- BTC 리포트는 단기/중기/일봉을 같이 보도록 정리",
        "- B등급 약한 후보는 더 아래로 내리고 회복형만 남기게 조정",
        "- /scan_debug fallback은 오래된 약한 후보가 덜 보이게 재필터링",
...
def get_watch_quality_block_reason(item: dict, alert_type: str = "new"):
    ...
    if strategy == "EARLY" and tier == "B":
        ...
        if recent_spike and below_30m_high < 1.45:
            return "이미 한 번 튄 뒤라 초반 선점형 B에서 제외"
        if from_30m_low > 2.80 and below_30m_high < 1.30:
            return "저점에서 이미 많이 올라와서 초반 선점형 B에서 제외"
        if vol_5v20_ratio < 1.20 and leader_score < 1.85 and vol_ratio < 3.30:
            return "거래량이 이어지지 않아 초반 선점형 B에서 제외"
        if not recovery_like and not higher_low_hint and leader_score < 1.75 and vol_ratio < 3.60:
            return "초반처럼 보여도 힘이 약해서 버릴 후보"
        if leader_score < 1.55 and change_pct < 1.00 and vol_ratio < 3.10:
            return "초입 힘이 약해서 지금 볼 가치가 낮음"
        if high_gap_pct < 1.15 and vol_ratio < 3.90 and leader_score < 1.95:
    ...
    if strategy == "PREPUMP" and tier == "B":
        ...
        if high_gap_pct < 0.85:
            return "바로 위 저항이 가까워서 상승 시작형 B에서 제외"
        if leader_score < 1.05:
            return "큰 흐름 준비가 너무 약해서 상승 시작형 B에서 제외"
        if recent_spike and below_30m_high < 1.35:
            return "이미 한 번 튄 뒤라 지금 초반형으로 보기 어려움"
        if from_30m_low > 3.40 and below_30m_high < 1.55:
            return "저점에서 이미 많이 올라와서 지금 사기 늦음"
        if not recovery_like and not higher_low_hint and leader_score < 1.65 and vol_ratio < 6.10:
            return "반등 모양은 있는데 힘이 약해서 버릴 후보"
        if vol_5v20_ratio < 1.18 and leader_score < 1.85 and vol_ratio < 6.60:
            return "거래량이 이어지지 않아 상승 시작형 B에서 제외"
        if leader_score < 1.85 and change_pct < 1.85 and vol_ratio < 5.80:
            return "초반 힘이 부족해서 지금 볼 가치가 낮음"
        if high_gap_pct < 1.20 and vol_ratio < 7.40 and leader_score < 1.95:
            return "잠깐 튀고 끝날 가능성이 커서 제외"
    ...
    for ticker, data in cache.items():
        try:
            ...
            if change_1 <= -0.40 and change_3 <= -0.10:
                continue
            if max(change_1, change_3, change_5) < 0.12:
                continue
            if momentum_hint <= 0.12:
                continue
            if not (2.20 <= vol_ratio <= 4.6):
                continue
            if not (47 <= rsi <= 66):
                continue
            if not (0.70 <= high_gap_pct <= 2.0):
                continue
            if range_pct >= 2.6:
                continue
            if regime_name == "WEAK" and vol_ratio < 2.80:
                continue
            if leader_score < 1.10 and turnover_rank > 34 and surge_rank > 34:
                continue
            if is_recent_spike_near_high(df, current_price) or is_box_top_range_chase(df, current_price) or is_retest_after_failed_peak(df) or is_false_start_near_high(df, current_price):
                continue
            if rebound_from_low_pct >= 1.20 and high_gap_pct <= 1.05:
                continue
            if flip_count >= 6 and not box and change_5 < 0.70:
                continue
            if flip_count >= 7 and range_pct >= 1.35:
                continue
            if not (hl or box or half_hold):
                continue
            if leader_score < 1.20 and vol_ratio < 2.80:
                continue
    ...
    for ticker, data in cache.items():
        try:
            ...
            if regime_name == "WEAK" and vol_ratio < 2.40:
                continue
            if change_5 <= -3.2 or change_5 >= 2.4:
                continue
            if change_1 < 0.03 and change_3 < 0.10:
                continue
            if vol_ratio < 2.30:
                continue
            if leader_score < 1.05 and vol_ratio < 3.00:
                continue
            if not (0.22 <= low_gap_pct <= 1.00):
                continue
            if high_gap_pct <= 1.00:
                continue
            if range_pct >= 2.4:
                continue
            if pullback_from_peak_pct >= 0.55 and change_1 <= 0.18:
                continue
            if flip_count >= 6 and range_pct >= 1.0:
                continue
...
def scan_watchlist(shared_cache=None):
    ...
    if entry_entries:
        send_watch_alert_bundle(get_watch_section_header("entry", regime=regime), entry_entries[:3], section_key="entry")
    if recovery_entries:
    ...
    if watch_entries and not (entry_entries or recovery_entries or late_entries):
        send_watch_alert_bundle(get_watch_section_header("watch", regime=regime), watch_entries[:1], section_key="watch")
...
def get_scan_debug_candidate_section(change_5: float, vol_ratio: float, leader_score: float, rsi: float) -> str:
    ...
    if change_5 >= 1.60 and vol_ratio < 6.50:
        return "weak"
    if rsi >= 82:
        return "weak"
    if change_5 >= 0.45 and vol_ratio >= 2.00 and leader_score >= 0.95:
        return "strong"
    if change_5 >= 0.18 and vol_ratio >= 1.35 and leader_score >= 0.55:
        return "watch"
    return "weak"
...
def build_scan_debug_reasons(change_5: float, vol_ratio: float, leader_score: float, rsi: float, section: str = ""):
    ...
    if leader_score < 0.55:
        reasons.append("시장주목도가 아직 약함")
    if rsi >= 82 or (change_5 >= 1.60 and vol_ratio < 6.50):
...
def build_scan_debug_text():
    ...
    strong_items = [x for x in results if str(x.get("section", "")) == "strong"]
    watch_items = [x for x in results if str(x.get("section", "")) == "watch"]
    weak_items = [x for x in results if str(x.get("section", "")) == "weak"]
    ...
    append_group("✅ 지금 볼만한 후보", strong_items)
    append_group("👀 조금 더 보면 되는 후보", watch_items)
    append_group("⚠️ 거래량은 큰데 아직 약한 후보", weak_items[:3])
    return "\n".join(lines)
...
def append_debug_shortlist_parts(parts: list):
    try:
        ...
        shortlist = [x for x in results if str(x.get("section", "")) in {"strong", "watch"}]
        if not shortlist:
            shortlist = results
