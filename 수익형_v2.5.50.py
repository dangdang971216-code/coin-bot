...
SCAN_WATCHLIST_PREWATCH_LIMIT = 2
SCAN_WATCHLIST_NEWLOW_LIMIT = 2
SCAN_WATCHLIST_CACHE_LIMIT = 36
SCAN_WATCHLIST_PREPARE_MAX_SEC = 18
SCAN_WATCHLIST_PREPARE_MISSED_LIMIT = 10
SCAN_WATCHLIST_FALLBACK_LIMIT = 8
SCAN_WATCHLIST_COLLECT_HEARTBEAT_EVERY = 6
...
def collect_watch_signals_from_cache(cache, regime=None, started_at: float = None, time_limit_sec: float = SCAN_WATCHLIST_PREPARE_MAX_SEC):
    ...
    ranked_rows = get_watchlist_ranked_cache_items(cache, limit=SCAN_WATCHLIST_CACHE_LIMIT)
...
def build_watch_fallback_candidates_lite(cache, regime=None, started_at: float = None, time_limit_sec: float = 8.0):
    base_ts = safe_float(started_at, time.time())
    subset = get_watchlist_subset_cache(cache, limit=SCAN_WATCHLIST_CACHE_LIMIT)
    combined = []
    try:
...
def scan_watchlist(shared_cache=None):
    ...
    try:
        ...
        register_missed_review_market_candidates(cache, now_ts=time.time(), limit=SCAN_WATCHLIST_PREPARE_MISSED_LIMIT)
    except Exception as e:
