...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v225 micro fresh 단일판정 + 후보품질 관찰 안정판(조건/청산/BUY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
...
def _v216_quality_text() -> str:  # type: ignore[override]
    ...
    return (txt.replace("v223 기본은 캐시 전용", "v225 기본은 캐시 전용")
               .replace("v222 기본은 캐시 전용", "v225 기본은 캐시 전용")
               .replace("v221 기본은 캐시 전용", "v225 기본은 캐시 전용")
               .replace("v219 기본은 캐시 전용", "v225 기본은 캐시 전용"))
