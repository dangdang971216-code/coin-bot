...
VERSION = "bithumb_micro_sidecar_v0.10_urgent_rotate_wide_quality_2026-05-17"
TARGET_SAFETY_MAX = int(os.getenv("CLEAN_MICRO_TARGET_SAFETY_MAX", os.getenv("CLEAN_MICRO_MAX_TICKERS", "180")))
POLL_BATCH_MAX = int(os.getenv("CLEAN_MICRO_POLL_BATCH_MAX", "60"))
PRIORITY_KEEP = int(os.getenv("CLEAN_MICRO_PRIORITY_KEEP", "60"))
POLL_SEC = float(os.getenv("CLEAN_MICRO_POLL_SEC", "3.0"))
URGENT_FAST_SLEEP_SEC = float(os.getenv("CLEAN_MICRO_URGENT_FAST_SLEEP_SEC", "1.0"))
URGENT_POLL_BATCH_MAX = int(os.getenv("CLEAN_MICRO_URGENT_POLL_BATCH_MAX", "24"))
REQUEST_TIMEOUT = float(os.getenv("CLEAN_MICRO_REQUEST_TIMEOUT_SEC", "1.5"))
...
USER_AGENT = "coinbot-bithumb-micro-sidecar/0.10"
...
def load_urgent_targets() -> Tuple[List[str], str, float]:
    try:
        ...
        ttl = max(3.0, fnum(obj.get("ttl_sec"), 35.0))
        if now() - ts > ttl:
...
def load_targets() -> List[str]:
    ...
    for t in list(urgent or []) + list(targets or []):
        ...
        if t and t not in STABLE_EXCLUDED and t not in seen:
            final.append(t)
            seen.add(t)
        if len(final) >= TARGET_SAFETY_MAX:
            break
    URGENT_TARGETS = [t for t in urgent if t in seen][:80]
...
def select_poll_batch(targets: List[str], *, include_urgent: bool = False) -> List[str]:
    ...
    urgent 후보는 poll_loop의 fast lane에서 먼저 별도 수집한다.
    여기서는 기본적으로 urgent를 제외해 hot/current 회전이 밀리지 않게 한다.
    include_urgent=True는 호환용으로만 남긴다.
    """
    global ROTATE
    clean: List[str] = []
    seen = set()
    for t in targets or []:
        ...
        if tt and tt not in STABLE_EXCLUDED and tt not in seen:
            ...
            clean.append(tt)
    if not clean:
        STATS.update({"poll_batch": 0, "rotate_pos": ROTATE, "priority_batch": 0, "rotate_batch": 0, "urgent_batch": 0})
        return []

    max_batch = max(8, min(POLL_BATCH_MAX, len(clean)))
    urgent = [t for t in URGENT_TARGETS if t in seen]
    urgent = list(dict.fromkeys(urgent))[:min(len(urgent), max_batch)] if include_urgent else []
    urgent_set = set(urgent) if include_urgent else set(URGENT_TARGETS)
    rest_clean = [t for t in clean if t not in urgent_set]

    priority_room = max(0, min(PRIORITY_KEEP, max_batch - len(urgent), len(rest_clean)))
    priority = rest_clean[:priority_room]
    rest = rest_clean[priority_room:]
    room = max(0, max_batch - len(urgent) - len(priority))

    rotated: List[str] = []
    if rest and room > 0:
        start = ROTATE % len(rest)
        take = min(room, len(rest))
        rotated = [rest[(start + i) % len(rest)] for i in range(take)]
        ROTATE = (ROTATE + take) % len(rest)

    batch = list(dict.fromkeys(urgent + priority + rotated))
    STATS["poll_batch"] = len(batch)
    STATS["urgent_batch"] = len(urgent)
    STATS["priority_batch"] = len(priority)
    STATS["rotate_batch"] = len(rotated)
    STATS["rotate_pos"] = ROTATE
    STATS["priority_keep"] = priority_room
    STATS["rest_rotate_size"] = len(rest)
    STATS["poll_policy"] = "v0.9 urgent fast-lane first; normal target rotation excludes urgent duplicates"
    return batch
...
def urgent_status_snapshot() -> Dict[str, Any]:
    ...
    return {
        ...
        "urgent_poll_batch_max": URGENT_POLL_BATCH_MAX,
    }



def select_urgent_batch() -> List[str]:
    """v0.10: urgent 대상이 18~24개를 넘어도 일부만 고정 수집하지 않고 회전한다.

    stale/missing을 먼저 보되, 매 루프마다 위치를 돌려 전체 urgent 후보가 fast-lane을 탄다.
    """
    global URGENT_ROTATE
    clean = list(dict.fromkeys([to_ticker(t) for t in URGENT_TARGETS if to_ticker(t)]))
    if not clean:
        STATS["urgent_batch"] = 0
    ...
    for t in clean:
        ...
        if age <= STALE_SEC:
            ...
            stale_or_missing.append(t)
    ordered = stale_or_missing + fresh
    maxb = max(1, min(URGENT_POLL_BATCH_MAX, len(ordered)))
    start = URGENT_ROTATE % len(ordered)
    rotated = ordered[start:] + ordered[:start]
    batch = rotated[:maxb]
    URGENT_ROTATE = (URGENT_ROTATE + maxb) % max(1, len(ordered))
    STATS["urgent_batch"] = len(batch)
    STATS["urgent_deferred"] = max(0, len(ordered) - len(batch))
    STATS["urgent_rotate_pos"] = URGENT_ROTATE
    STATS["urgent_poll_batch_max"] = URGENT_POLL_BATCH_MAX
    return batch
    ...
    while not STOP:
        try:
            ...
            if not targets:
                ...
                time.sleep(POLL_SEC)
                continue
            # v0.9: urgent fast-lane first.
            # paper OPEN 직전/보류 품질 후보는 일반 target 회전에 섞지 않고 먼저 수집한다.
            urgent_batch = select_urgent_batch()
            urgent_done = 0
            urgent_elapsed = 0.0
            if urgent_batch:
                write_status("긴급수집중", "-")
                urgent_done, urgent_elapsed = poll_many(urgent_batch, label="urgent")
                ...
                STATS["urgent_batch_symbols"] = urgent_batch[:12]
                write_cache()
            ...
            if batch:
                write_status("수집중", "-")
                normal_done, normal_elapsed = poll_many(batch, label="normal")
