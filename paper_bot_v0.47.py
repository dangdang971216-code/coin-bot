...
DEFAULT_CONTROL = {
    ...
    "quick_stop_min_age_sec": 180,
    "quick_stop_max_age_sec": 480,
    "quick_stop_peak_under_pct": 0.10,
...
def _exit_rule_decision(pos: Dict[str, Any], control: Dict[str, Any], *, age_min: float, net: float, peak: float) -> Tuple[Optional[str], str]:
    """v0.47: 빠른손절/지지부진 조기종료 판단.

    기존 익절/손절/보호청산/시간종료를 지우지 않고, 진입 후 반응이 없는 후보만
    -1.2%까지 기다리기 전 또는 20분 지지부진 종료 전 먼저 정리한다.
    """
    ...
    if bool(control.get("quick_stop_enabled", True)):
        min_age = float_any(control.get("quick_stop_min_age_sec"), default=180.0)
        max_age = float_any(control.get("quick_stop_max_age_sec"), default=480.0)
        peak_under = float_any(control.get("quick_stop_peak_under_pct"), default=0.10)
