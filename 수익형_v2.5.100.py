...
def send_startup_message():
    lines = [
        ...
        "",
        "이번 핵심 변경",
        "- 너무 약한 미리 볼 후보는 더 줄이고, 실제로 먼저 붙는 쪽이 위에 오도록 정렬을 다시 조정",
        "- shortlist 상위인데 아직 늦지 않은 회복형은 rescue 승격이 조금 더 잘 되게 완화",
        "- /review_reset 추가: 분석용 덤프를 수동으로 깨끗하게 초기화 가능",
        "- 버전이 바뀌면 review_dump 관련 기록은 자동으로 초기화하고, 이전 버전 정보가 비어 있어도 첫 1회는 섞이지 않게 정리",
        "- 탐색 놓침/승격 놓침은 계속 보고, 늦었지만 크게 간 코인은 기준을 무조건 푸는 쪽으로 오해되지 않게 정리",
        "- /missed 에 최근 알림 결과도 같이 붙여서 놓친 쪽과 바로 비교하기 쉽게 정리",
        "- /fills 로 최근 매수가 알림가보다 얼마나 늦었는지 바로 확인 가능",
        "- /compare /review_dump 는 저장된 표본 기준으로 바로 보여주고, /scan_debug 는 빠른 후보 스냅샷 기준으로 바로 보이게 정리",
        "- shortlist 상위 + 거래대금 최상위인데 아직 안 늦은 회복형은 rescue에서 한 번 더 살릴 수 있게 소폭 보강",
        "- 재돌파/2차 파동 감지와 차트 구조 수치화를 추가해, 다시 붙는 흐름은 숫자로 더 잘 살리게 조정",
        "- 시장 돈 흐름 / 일봉 준비도 / 돌파 후 유지 확인을 후보 우선순위와 사용자 알림 차단에 반영",
        "",
    ]
...
def format_reverse_trace_block(row: dict, idx: int = 1) -> str:
    ...
    lines = [
        ...
        f"- 위치: 저점대비 {safe_float(row.get('from_low', 0), 0):+.2f}% / 최근고점까지 {safe_float(row.get('below_high', 999), 999):.2f}% 남음",
