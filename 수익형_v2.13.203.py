...
def export_candidates(strict: List[Dict[str, Any]], shadow: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # v203: 화면 표시용 overlay만으로는 /loss_review의 entry_context stale 문제가 남았다.
    # 공장 저장 직전에 최신 sidecar cache를 strict/shadow row에 다시 입히고, 그 값을 후보파일에 저장한다.
    # 조건을 조이거나 후보를 새 분류로 나누는 것이 아니라, 저장되는 snapshot 자체를 최신화하는 배관 수정이다.
    try:
        ...
        with _state_lock:
            STATE["factory_external_overlay"] = "v203_before_export"
            STATE["factory_external_overlay_ts"] = now_ts()
    except Exception as exc:
        log_error("factory_external_overlay", exc)
    result = {
...
_CLOSED_ROWS_CACHE: Dict[str, Any] = {"mtime": 0.0, "size": -1, "limit": 0, "rows": []}
...
def _cached_tail_jsonl(path: Path, max_lines: int = 20000) -> List[Dict[str, Any]]:
    try:
        ...
        if (
            ...
            and float(cache.get("mtime", 0.0)) == mtime
            and int(cache.get("size", -1)) == size
            and int(cache.get("limit", 0)) >= int(max_lines)
        ):
        ...
        cache.update({"path": str(path), "mtime": mtime, "size": size, "limit": int(max_lines), "rows": rows})
        return rows
    except Exception as exc:
...
def _latest_strict_overlay(limit: int = 600) -> List[Dict[str, Any]]:
...
def _external_health_snapshot() -> Dict[str, Any]:
    latest_strict = _latest_strict_overlay(600)
...
def health_text() -> str:
    ...
    return "\n".join(["🧭 건강상태 /health", "- 메인봇방용 간단 상태판입니다. 재시작/업그레이드는 가드봇 담당.", "", "[1/4] 메인봇", f"{scan.get('icon','❔')} {BOT_VERSION} / scan {scan.get('label','-')} / 단계 {scan.get('stage','-')} / 후보 {snap.get('total',0)}개", f"- 오류: {err_summary}", "", "[2/4] 외부정보", f"{ws_icon} WS: {snap.get('ws_state','-')} / 전체신선 {STATE.get('ws_fresh',0)} / 후보신선 {snap.get('ws_fresh',0)}/{snap.get('total',0)} / 오래됨 {snap.get('ws_stale',0)} / 없음 {snap.get('ws_missing',0)} / 최근 {snap.get('ws_age','-')}초", f"{micro_icon} 호가·체결: {snap.get('micro_state','-')} / 전체신선 {STATE.get('micro_fresh', STATE.get('micro_fresh_rows',0))} / 후보신선 {snap.get('micro_fresh',0)}/{snap.get('total',0)} / 오래됨 {snap.get('micro_stale',0)} / 없음 {snap.get('micro_missing',0)}", "", "[3/4] paper_bot", f"{p.get('icon','❔')} {p_status.get('version','?')} / running {p.get('running')} / OPEN {p_status.get('open_total','?')} / CLOSED {p_status.get('closed_total','?')} / 상태 {_age_text(fnum(p_status.get('updated_at'),0))} 전", "", "[4/4] 판독", *warnings])
...
def command_batch(update, context) -> None:
    funcs = [
        ...
        ("deploy", lambda: "\n".join(["📦 배포 상태 /deploy", f"- 메인봇 실행버전: {BOT_VERSION}", "- 메인봇 대상: 수익형_v2.13.203.py", "- 페이퍼봇 대상: paper_bot_v0.39.py"])),
    ]
    start = now_ts()
...
def _overlay_current_external_for_items(items: List[Dict[str, Any]], *, refresh: bool = True) -> List[Dict[str, Any]]:
    ...
    for r in items or []:
        ...
        if isinstance(ctx, dict):
            ...
            ctx["external_overlay_note"] = "v203_latest_cache_before_display_or_export"
...
def external_status_text() -> str:
    ...
    refresh_external_ws_cache()
    refresh_micro_cache()
    latest_strict = _overlay_current_external_for_items(tail_jsonl(FILES["paper_latest"], max_lines=1000), refresh=False)
    total = len(latest_strict)
...
def candidate_quality_text(full: bool = False) -> str:
    ...
    v_strict = [r for r in rows_since_current_version(rows) if str(r.get("lane")) == "strict"]
    latest_strict = _latest_strict_overlay(600)
    latest_shadow = tail_jsonl(FILES["shadow_latest"], max_lines=600)
...
def version_compare_text() -> str:
    rows = _strict_closed_rows()
    by_ver: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for r in rows: by_ver[row_brain_version(r)].append(r)
    preferred = [BOT_VERSION, "수익형 v2.13.202", "수익형 v2.13.201", "수익형 v2.13.200", "수익형 v2.13.198", "수익형 v2.13.197", "수익형 v2.13.195", "수익형 v2.13.194"]
    keys: List[str] = []
    for k in preferred:
        if k in by_ver and k not in keys: keys.append(k)
    for k in sorted(by_ver.keys(), reverse=True):
        if k not in keys and len(keys) < 6 and k != "버전 미기록": keys.append(k)
    lines = ["📊 버전 비교 /version_compare", "- 기본은 compact. 상세 손절은 /loss_review 202처럼 따로 확인", ""]
    for k in keys[:6]:
        arr = by_ver.get(k, []); st = score_stats(arr)
        stops = [r for r in arr if str(r.get("exit_reason") or "") == "stop_loss"]
        micro_counts = Counter(_micro_status_of(r) for r in stops); ws_counts = Counter(_ws_status_of(r) for r in stops)
        top_line = _outlier_summary(arr)[0] if arr else "- 최대수익 제외: 표본 없음"
        lines.append(f"{score_icon(st)} {k}: {st['n']}전 {st['wins']}승 {st['losses']}패 / 합산 {st['total']:+.2f}% / 평균 {st['avg']:+.2f}%{sample_note(st['n'])}")
        lines.append(top_line)
        lines.append(f"- 손절 {len(stops)}건 / WS {ws_counts.get('fresh',0)}/{ws_counts.get('stale',0)}/{ws_counts.get('missing',0)} / micro {micro_counts.get('fresh',0)}/{micro_counts.get('stale',0)}/{micro_counts.get('missing',0)}")
        lines.append(f"- 종료: {_reason_summary(arr)}")
        lines.append("")
...
def command_version_score(update, context) -> None:
    rows = [r for r in rows_since_current_version(load_closed(limit=8000)) if str(r.get("lane")) == "strict"]
    lines = ["📊 현재버전 성과 /version_score", _compact_stat_line("현재버전 정식 모의매매", rows), f"- 기준: {BOT_VERSION} / {version_baseline_text()} / 기존 기록 삭제 없음", f"- 주요종료: {_reason_summary(rows)}", "- 참고: 상세 손절은 /loss_review 203"]
    reply(update, "\n".join(lines))
...
def main() -> None:
    ...
    send_chunks("\n".join(["✅ 봇 시작 완료", f"현재 버전: {BOT_VERSION}", f"전략: {STRATEGY_NAME}", "v203 공장 저장 전 WS/micro 최신 overlay + health/quality 경량화(조건/청산/BY_READY 변경 없음)", "확인: /health /external_status /version_compare /version_score /quality /errorlog"]))
